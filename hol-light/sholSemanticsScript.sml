...
val typeset_closes_over = store_thm("typeset_closes_over",
  ...
  conj_tac >- (
    ...
    conj_tac >- metis_tac[MEM_EL,LENGTH_STRING_SORT,tvars_ALL_DISTINCT,MEM_STRING_SORT] >>
    simp[FLOOKUPD_def,Abbr`tyin`] >>
    BasicProvers.CASE_TAC >- (
      fs[ALOOKUP_FAILS] >>
      rfs[MEM_ZIP] >>
      metis_tac[tvars_ALL_DISTINCT,MEM_STRING_SORT,LENGTH_STRING_SORT,MEM_EL] ) >>
    Q.ISPECL_THEN[`ZIP(STRING_SORT(tvars tm),args)`,`n`]mp_tac ALOOKUP_ALL_DISTINCT_EL >>
...
val semantics_closes = prove(
  ...
  let val tac = (
    ...
    first_x_assum(qspecl_then[`x`,`EL n (STRING_SORT (tvars tm))`]mp_tac) >>
    BasicProvers.CASE_TAC >- (
...
val semantics_has_meaning = prove(
  ...
  conj_tac >- (
    rw[has_meaning_def] >- (
      ...
      metis_tac[] ) >>
    rw[Once semantics_cases] >>
    qspecl_then[`τ`,`Tyapp(Tydefined op p0) args`,`m`]mp_tac(CONJUNCT1 semantics_frees_exists) >>
    simp[] >>
    disch_then(qspec_then`τ'`mp_tac) >>
    simp[] >>
  ...
  conj_tac >- (
    ...
    strip_tac >>
    HINT_EXISTS_TAC >> fs[] >>
    qmatch_assum_abbrev_tac`semantics FEMPTY τ tm mz` >>
    qspecl_then[`FEMPTY`,`τ`,`tm`,`mz`]mp_tac(CONJUNCT2 semantics_frees_exists) >>
    simp[] >>
...
val semantics_simple_inst = store_thm("semantics_simple_inst",
  ...
  conj_tac >- (
    ...
    conj_tac >- (
      qmatch_abbrev_tac`typeset τ (tyinst (alist_to_fmap (ZIP (STRING_SORT(tvars tm),MAP (tyinst tyin) args))) rty) mp` >>
      pop_assum kall_tac >>
      first_x_assum(qspecl_then[`τ`,`tyin`]mp_tac) >>
      discharge_hyps >- (
        rw[] >>
        first_x_assum match_mp_tac >>
        fs[tyvars_tyinst] >>
        qmatch_assum_abbrev_tac`MEM a (tyvars y)` >>
        qexists_tac`y` >> rw[Abbr`y`] >>
        simp[FLOOKUPD_def] >>
        BasicProvers.CASE_TAC >>
        imp_res_tac ALOOKUP_MEM >>
        rfs[ALOOKUP_FAILS,MEM_ZIP] >>
        imp_res_tac tyvars_typeof_subset_tvars >>
        fs[tyvars_def,SUBSET_DEF] >>
        metis_tac[MEM_EL,LENGTH_STRING_SORT,MEM_STRING_SORT,tvars_ALL_DISTINCT]) >>
      qmatch_abbrev_tac`typeset τ t1 mp ⇒ typeset τ t2 mp` >>
      qsuff_tac`t1 = t2`>-rw[] >>
      unabbrev_all_tac >>
      ...
      BasicProvers.CASE_TAC >- (
        ...
        fs[tyvars_def,SUBSET_DEF] >>
        fs[MEM_ZIP,MEM_EL] >>
        metis_tac[LENGTH_ZIP,EL_ZIP,LENGTH_STRING_SORT,tvars_ALL_DISTINCT,MEM_STRING_SORT,MEM_EL] ) >>
      BasicProvers.CASE_TAC >- (
        fs[ALOOKUP_FAILS] >>
        imp_res_tac ALOOKUP_MEM >>
        rfs[MEM_ZIP] >>
    ...
    conj_tac >- (
      ...
      discharge_hyps >- (
        ...
        rfs[ALOOKUP_FAILS,MEM_ZIP] >>
        metis_tac[MEM_EL,tvars_ALL_DISTINCT,LENGTH_STRING_SORT,MEM_STRING_SORT]) >>
      qmatch_abbrev_tac`semantics FEMPTY τ t1 m ⇒ semantics FEMPTY τ t2 m` >>
      qsuff_tac`t1 = t2`>-rw[]>>
      unabbrev_all_tac >>
      simp[simple_inst_tvars] >>
      simp[FLOOKUPD_def,FLOOKUP_o_f,FLOOKUP_FUNION] >>
      ...
      BasicProvers.CASE_TAC >- (
        ...
        fs[tyvars_def,SUBSET_DEF] >>
        fs[MEM_ZIP,MEM_EL] >>
        metis_tac[LENGTH_ZIP,EL_ZIP,tvars_ALL_DISTINCT,MEM_STRING_SORT,LENGTH_STRING_SORT,MEM_EL] ) >>
      BasicProvers.CASE_TAC >- (
        fs[ALOOKUP_FAILS] >>
        imp_res_tac ALOOKUP_MEM >>
        rfs[MEM_ZIP] >>
      ...
      rfs[ALOOKUP_ZIP_MAP_SND] ) >>
    rw[] >>
    qmatch_assum_abbrev_tac`semantics FEMPTY τi (simple_inst tyin tm) m` >>
    qabbrev_tac`τi' = FUN_FMAP (λa. @m. typeset τ' (FLOOKUPD tyin' a (Tyvar a)) m) (set(tvars(simple_inst tyin tm)))` >>
    `∀a. MEM a (tvars(simple_inst tyin tm)) ⇒ typeset τ' (FLOOKUPD tyin' a (Tyvar a)) (τi' ' a)` by (
      rw[] >>
      simp[Abbr`τi'`,FUN_FMAP_DEF] >>
      SELECT_ELIM_TAC >>
      reverse conj_tac >- rw[] >>
      match_mp_tac(MP_CANON(CONJUNCT1(semantics_frees_exists))) >>
      qexists_tac`τ` >>
      ...
      simp[] >>
      rw[] >>
      fs[FLOOKUPD_def,Abbr`tyin`] ) >>
    `type_valuation τi'` by (
      simp[Abbr`τi'`,type_valuation_def,GSYM LEFT_FORALL_IMP_THM] >>
      rw[] >>
      SELECT_ELIM_TAC >>
      metis_tac[typeset_inhabited] ) >>
    last_x_assum(qspec_then`τi'`mp_tac) >>
    simp[] >>
    discharge_hyps >- simp[Abbr`τi'`] >>
    disch_then(qx_choosel_then[`mrty'`,`mp'`,`w'`]strip_assume_tac) >>
    map_every qexists_tac[`mrty'`,`mp'`,`w'`] >>
    simp[] >>
    conj_tac >- (
      first_x_assum(qspecl_then[`τ'`,`tyin'`]mp_tac) >>
      discharge_hyps >- (
        ...
        fs[tyvars_def,SUBSET_DEF] >>
        metis_tac[]) >>
      qmatch_abbrev_tac`typeset τ' t1 mz ⇒ typeset τ' t2 mz` >>
      qsuff_tac`t1 = t2`>-rw[] >>
      unabbrev_all_tac >>
      simp[GSYM tyinst_tyvars] >>
      simp[FLOOKUPD_def,FLOOKUP_o_f,FLOOKUP_FUNION] >>
      ...
      BasicProvers.CASE_TAC >- (
        ...
        fs[tyvars_def,SUBSET_DEF] >>
        fs[MEM_ZIP,MEM_EL] >>
        metis_tac[LENGTH_ZIP,EL_ZIP,tvars_ALL_DISTINCT,LENGTH_STRING_SORT,MEM_STRING_SORT,MEM_EL] ) >>
      BasicProvers.CASE_TAC >- (
        fs[ALOOKUP_FAILS] >>
        imp_res_tac ALOOKUP_MEM >>
        rfs[MEM_ZIP] >>
    ...
    discharge_hyps >- (
      simp[] >>
      `{x | ∃ty. VFREE_IN (Var x ty) tm} = {}` by (
        simp[EXTENSION] >>
        metis_tac[fresh_term_def,FINITE_EMPTY,VFREE_IN_ACONV] ) >>
      `DISJOINT (set (bv_names tm)) {x | ∃ty. VFREE_IN (Var x ty) tm}` by (
        simp[] ) >>
      simp[Abbr`tm`,fresh_term_def,VFREE_IN_simple_inst] ) >>
    ...
    BasicProvers.CASE_TAC >- (
      ...
      fs[tyvars_def,SUBSET_DEF] >>
      fs[MEM_ZIP,MEM_EL] >>
      metis_tac[LENGTH_ZIP,EL_ZIP,tvars_ALL_DISTINCT,MEM_STRING_SORT,MEM_EL,LENGTH_STRING_SORT] ) >>
    BasicProvers.CASE_TAC >- (
      fs[ALOOKUP_FAILS] >>
      imp_res_tac ALOOKUP_MEM >>
      rfs[MEM_ZIP] >>
  ...
  conj_tac >- (
    ...
    simp[] >>
    qmatch_abbrev_tac`simple_inst tyin tm has_type tyy` >>
    `tyy = tyinst tyin (typeof tm)` by (
      simp[Abbr`tyy`] >>
      last_x_assum mp_tac >>
      simp[Once semantics_cases] >> strip_tac >>
      `typeof tm = Fun rty Bool` by metis_tac[WELLTYPED_LEMMA] >>
      simp[] >>
      qmatch_assum_abbrev_tac`simple_inst tyin0 tm has_type Fun ty Bool` >>
      qspecl_then[`tm`,`tyin0`]mp_tac simple_inst_has_type >>
      discharge_hyps >- metis_tac[welltyped_def] >>
      simp[] >> strip_tac >>
      imp_res_tac WELLTYPED_LEMMA >>
      fs[] >>
      ...
      simp[FLOOKUPD_def,Abbr`tyin`,FLOOKUP_FUNION,FLOOKUP_o_f,Abbr`tyin0`] >>
      rw[] >>
      BasicProvers.CASE_TAC >- (
        imp_res_tac tyvars_typeof_subset_tvars >>
      ...
      rfs[ALOOKUP_ZIP_MAP_SND] ) >>
    rw[] >>
    match_mp_tac simple_inst_has_type >>
    last_x_assum mp_tac >>
    simp[Once semantics_cases] >> strip_tac >>
  ...
  conj_tac >- (
    ...
    BasicProvers.CASE_TAC >- (
      ...
      fs[tyvars_def,SUBSET_DEF] >>
      rfs[MEM_ZIP] >>
      metis_tac[MEM_EL,LENGTH_ZIP,EL_ZIP,tvars_ALL_DISTINCT,LENGTH_STRING_SORT,MEM_STRING_SORT] ) >>
    BasicProvers.CASE_TAC >- (
      fs[ALOOKUP_FAILS] >>
      imp_res_tac ALOOKUP_MEM >>
      rfs[MEM_ZIP] >>
...
val new_basic_type_definition_correct = store_thm("new_basic_type_definition_correct",
  ...
  imp_res_tac ACONV_welltyped >> pop_assum kall_tac >>
  imp_res_tac ACONV_TYPE >> pop_assum(assume_tac o SYM) >>
  `∀τ. type_valuation τ ∧ set (tvars tm) ⊆ FDOM τ ⇒
     ∃mrty mp z.
       typeset τ (typeof w) mrty ∧ semantics FEMPTY τ tm mp ∧ z <: mrty ∧
       holds mp z` by (
    gen_tac >> strip_tac >>
    ...
    strip_tac >>
    qexists_tac`m` >> rw[] >>
    `∃mp. semantics FEMPTY τ tm mp` by (
      imp_res_tac has_meaning_aconv >>
      fs[has_meaning_def] >>
      first_x_assum match_mp_tac >>
      simp[] >>
      fs[closes_def] >>
    ...
    discharge_hyps >- (
      ...
      simp[] >>
      fs[SUBMAP_DEF,SUBSET_DEF] >>
      match_mp_tac closes_aconv >>
      qexists_tac`tm` >>
      simp[ACONV_SYM] >>
      metis_tac[semantics_closes,FDOM_FEMPTY,term_valuation_FEMPTY] ) >>
    simp[Once semantics_cases] >>
    rw[] >>
    `semantics s t tm m1` by metis_tac[semantics_aconv] >>
    `semantics FEMPTY τ tm m1` by (
      match_mp_tac semantics_reduce >>
  ...
  conj_asm1_tac >- (
    ...
    reverse conj_asm2_tac >- (
      ...
      simp[] >>
      discharge_hyps >- (
        fs[tvars_simple_inst,SUBSET_DEF,tyvars_def,MEM_FOLDR_LIST_UNION] >>
        rw[] >>
        first_x_assum match_mp_tac >>
        qexists_tac`Tyvar x` >>
        simp[tyvars_def] >>
    ...
    simp[Once semantics_cases] >>
    srw_tac[DNF_ss][] >>
    `∃τ mp. type_valuation τ ∧ semantics FEMPTY τ tm mp` by (
      imp_res_tac has_meaning_aconv >>
      fs[has_meaning_def] >>
      qexists_tac`τ'` >> simp[] >>
      first_x_assum match_mp_tac >>
    ...
    `∃maty. typeset τ ty maty` by (
      ...
      imp_res_tac semantics_closes >>
      fs[closes_def,SUBSET_DEF] ) >>
    `∃mrty. typeset τ (typeof w) mrty` by (
      ...
      metis_tac[]) >>
    metis_tac[WELLTYPED,term_valuation_FEMPTY] ) >>
  conj_tac >- (
    rw[] >>
    match_mp_tac semantics_equation >>
    rw[] >>
    ...
    strip_tac >> fs[] >>
    pop_assum kall_tac >>
    `tm has_type (typeof p)` by metis_tac[WELLTYPED] >>
    rfs[] >>
    ...
    CONV_TAC (RESORT_EXISTS_CONV List.rev) >>
    map_every qexists_tac[`z`,`typeof w`,`mrty`,`mp`,`mrty`,`mp`] >>
    simp[] >>
    simp[Once semantics_cases] >>
    srw_tac[DNF_ss][] >>
    simp[Once semantics_cases] >>
    ...
    srw_tac[DNF_ss][] >>
    CONV_TAC (RESORT_EXISTS_CONV List.rev) >>
    map_every qexists_tac[`z`,`typeof w`,`mrty`,`mp`,`mrty`] >>
    simp[] >>
    simp[Abbr`r`] >>
    simp[Once semantics_cases] >>
    simp[Once semantics_cases] >>
    ...
    reverse conj_tac >- rw[] >>
    match_mp_tac APPLY_ABSTRACT >>
    simp[Abbr`c`,Abbr`b`,suchthat_def]) >>
  conj_asm1_tac >- (
    simp[welltyped_def] >>
    qexists_tac`Bool` >>
    simp[EQUATION_HAS_TYPE_BOOL] ) >>
  ...
  disch_then(qx_choosel_then[`mrty`,`mp`,`z`]strip_assume_tac) >>
  `semantics FEMPTY τ p mp` by metis_tac[semantics_aconv,term_valuation_FEMPTY] >>
    ...
    reverse conj_tac >- (
      ...
      discharge_hyps >- (
        ...
        conj_tac >- (
          ...
          imp_res_tac tyvars_typeof_subset_tvars >>
          fs[tyvars_def,SUBSET_DEF] >>
          `∃n. v = EL n (STRING_SORT (tvars tm)) ∧ n < LENGTH l` by metis_tac[MEM_EL,LENGTH_STRING_SORT,tvars_ALL_DISTINCT,MEM_STRING_SORT] >>
          Q.ISPECL_THEN[`ZIP((STRING_SORT(tvars tm)),l)`,`n`]mp_tac ALOOKUP_ALL_DISTINCT_EL >>
          simp[MAP_ZIP,EL_ZIP] >>
          rw[] >>
          match_mp_tac tyinst_tyvars1 >>
          simp[FLOOKUPD_def,FLOOKUP_DRESTRICT] >>
          rw[] >> fs[] >>
          first_x_assum (qspec_then`EL n (STRING_SORT(tvars tm))`mp_tac) >>
          simp[] ) >>
        simp[FDOM_DRESTRICT,tyvars_def] >>
        simp[SET_EQ_SUBSET,SUBSET_DEF,MEM_FOLDR_LIST_UNION,GSYM LEFT_FORALL_IMP_THM] >>
        simp[Abbr`tyn`,FLOOKUPD_def,Abbr`tvs`] >>
        rpt gen_tac >>
        BasicProvers.CASE_TAC >- (
      ...
      conj_asm1_tac >- (
        ...
        imp_res_tac tyvars_typeof_subset_tvars >>
        fs[tyvars_def,SUBSET_DEF] >>
        fs[Abbr`tyn`,FLOOKUPD_def] >>
        ntac 2 (pop_assum mp_tac) >>
        BasicProvers.CASE_TAC >- (
          fs[ALOOKUP_FAILS] >>
          rfs[Abbr`tvs`,MEM_ZIP] >>
          metis_tac[MEM_STRING_SORT,LENGTH_STRING_SORT,MEM_EL,tvars_ALL_DISTINCT] ) >>
        imp_res_tac ALOOKUP_MEM >>
        rfs[Abbr`tvs`,MEM_ZIP] >>
        metis_tac[MEM_EL] ) >>
      qsuff_tac`MEM x (tvars (simple_inst tyn tm))` >- (
        rfs[FLOOKUPD_def,FLOOKUP_DRESTRICT] >>
        metis_tac[semantics_11] ) >>
      simp[tvars_simple_inst] ) >>
    ...
