...
val compile_dec_contab_thm = store_thm("compile_dec_contab_thm",
  ``∀mn rs dec menv cenv env rd cs bs res cenv' rs'.
    env_rs menv cenv env rs rd cs bs ∧
    cenv' = dec_to_cenv mn dec ++ cenv ∧
    rs' = rs with contab := FST(compile_dec mn rs dec) ∧
    evaluate_dec mn menv cenv (SND cs) env dec res ∧
    SND res ≠ Rerr Rtype_error ∧
    closed_context menv cenv (SND cs) env ∧
    ("" ∉ new_dec_cns dec)
    ⇒
    env_rs menv cenv' env rs' rd cs bs``,
  Cases_on`dec`>>simp[compile_dec_def,FST_compile_fake_exp_contab,dec_to_cenv_def] >>
...
val compile_dec_val = store_thm("compile_dec_val",
  ...
  strip_tac >- (
    ...
    simp[compile_dec_def,dec_to_cenv_def,LibTheory.emp_def] >>
...
val compile_Cexp_append_code = store_thm("compile_Cexp_append_code",
  ``∀rsz menv env nl Ce.
    no_labs Ce ∧ set (free_vars Ce) ⊆ count (LENGTH env) ⇒
    let cs = compile_Cexp rsz menv env nl Ce in
    between_labels cs.out nl cs.next_label``,
  rw[compile_Cexp_def] >>
  qspecl_then[`LENGTH env`,`nl`,`Ce`]mp_tac (CONJUNCT1 label_closures_thm) >>
  simp[] >> strip_tac >>
  qspecl_then[`LENGTH env`,`menv`,`cs'`,`Ce'`]mp_tac compile_code_env_thm >>
  simp[Abbr`cs`,Abbr`cs'`] >>
  discharge_hyps >- (
    simp[ALL_DISTINCT_GENLIST,EVERY_GENLIST] ) >>
  strip_tac >>
  qspecl_then[`menv`,`env`,`TCNonTail`,`rsz+2`,`cs''`,`Ce'`]mp_tac(CONJUNCT1 compile_append_out) >>
  strip_tac >>
  simp[Abbr`cs'''`,between_labels_def] >>
...
val decs_cns_def = Define`
  (decs_cns _ [] = {}) ∧
  (decs_cns mn (d::ds) = dec_cns d ∪ (decs_cns mn ds DIFF (IMAGE (Long mn) (new_dec_cns d))))`
  ...
  ``∀mno menv cenv s env dec res. evaluate_decs mno menv cenv s env dec res ⇒
     ∀mn rs. ∃ck. ∀rs' bc00 rd bc bs bc0.
      ...
      | (s',cenv',Rerr(Rraise err)) =>
        ...
        bc_next^*bs bs' ∧
        err_bv err bv ∧
        env_rs menv (cenv'++cenv) env (rs' with renv := rs.renv) rd' (0,s') (bs' with stack := bs.stack)
      | (s',_) => T``,
...
val compile_top_thm = store_thm("compile_top_thm",
  ...
  strip_tac >- (
    ...
    rw[] >>
    qmatch_assum_abbrev_tac`bc_next^* bs1 bs2` >>
    map_every qexists_tac[`bv`,`bs2`,`rd'`] >>
    simp[Abbr`bs2`] >>
    cheat ) >>
  simp[])
