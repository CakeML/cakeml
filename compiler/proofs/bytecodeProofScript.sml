...
val with_same_sm = store_thm("with_same_sm",
  ``rd with sm := rd.sm = rd``,
  srw_tac[][theorem"refs_data_component_equality"])
...
val (Cv_bv_rules,Cv_bv_ind,Cv_bv_cases) = Hol_reln`
  ...
  (Cv_bv pp (CLitv (Char c)) (Number (& ORD c))) ∧
...
val Cv_bv_syneq = store_thm("Cv_bv_syneq",
  ...
  strip_tac >- (
    ...
    srw_tac[][Once Cv_bv_cases] >>
    fsrw_tac[DNF_ss][EVERY2_EVERY,EVERY_MEM,FORALL_PROD] >>
    rev_full_simp_tac(srw_ss())[MEM_ZIP] >> full_simp_tac(srw_ss())[MEM_ZIP] >>
    fsrw_tac[DNF_ss][] ) >>
  strip_tac >- (
    srw_tac[][] >> pop_assum mp_tac >>
    simp[Once syneq_cases] >>
    ...
    fsrw_tac[DNF_ss][] ) >>
  strip_tac >- (
    srw_tac[][el_check_def,FLOOKUP_DEF] >>
    ...
    simp[] >> strip_tac >>
    simp[] >>
    Cases_on`azb` >> rev_full_simp_tac(srw_ss())[] >> full_simp_tac(srw_ss())[syneq_cb_aux_def] >> rev_full_simp_tac(srw_ss())[EVERY_MEM] >>
  ...
  Cases_on`p0`>>full_simp_tac(srw_ss())[]>>PairCases_on`x` >>
  strip_tac >>
  ntac 2 (qpat_assum`X = Y`(mp_tac o SYM)) >>
  simp[syneq_cb_aux_def] >>
  strip_tac >> strip_tac >> full_simp_tac(srw_ss())[] >>
  rpt BasicProvers.VAR_EQ_TAC >>
  `b` by (
    full_simp_tac(srw_ss())[syneq_cb_aux_def] >> srw_tac[][] >> rev_full_simp_tac(srw_ss())[] ) >>
  ...
  simp[] >> strip_tac >>
  conj_asm1_tac >- (
    full_simp_tac(srw_ss())[syneq_cb_aux_def,EVERY_MEM,MEM_EL,GSYM LEFT_FORALL_IMP_THM] ) >>
  first_x_assum match_mp_tac >>
  fsrw_tac[DNF_ss][] >>
  first_x_assum match_mp_tac >>
  first_x_assum match_mp_tac >>
...
val Cv_bv_SUBMAP = store_thm("Cv_bv_SUBMAP",
  ...
  strip_tac >- srw_tac[][Once Cv_bv_cases,LENGTH_NIL] >>
  strip_tac >- srw_tac[][Once Cv_bv_cases,LENGTH_NIL] >>
  strip_tac >- srw_tac[][Once Cv_bv_cases,LENGTH_NIL] >>
  strip_tac >- srw_tac[][Once Cv_bv_cases,LENGTH_NIL] >>
  strip_tac >- (
    srw_tac[][Once Cv_bv_cases,LENGTH_NIL] >>
    full_simp_tac(srw_ss())[el_check_def,IS_PREFIX_THM,EVERY2_EVERY,EVERY_MEM,FORALL_PROD,LENGTH_NIL] >>
    PROVE_TAC[LESS_LESS_EQ_TRANS]) >>
  strip_tac >- (
    srw_tac[][] >> srw_tac[][Once Cv_bv_cases,LENGTH_NIL] >>
    full_simp_tac(srw_ss())[FLOOKUP_DEF,IS_PREFIX_THM,EVERY2_EVERY,EVERY_MEM,FORALL_PROD,LENGTH_NIL] ) >>
  strip_tac >- (
    srw_tac[][] >> srw_tac[][Once Cv_bv_cases,LENGTH_NIL] >>
    full_simp_tac(srw_ss())[FLOOKUP_DEF,IS_PREFIX_THM,EVERY2_EVERY,EVERY_MEM,FORALL_PROD,LENGTH_NIL] ) >>
  strip_tac >- ( srw_tac[][] >> simp[Once Cv_bv_cases] >> metis_tac[] ) >>
...
val Cv_bv_l2a_mono = store_thm("Cv_bv_l2a_mono",
  ...
  strip_tac >- srw_tac[][Once Cv_bv_cases] >>
  strip_tac >- srw_tac[][Once Cv_bv_cases] >>
  strip_tac >- srw_tac[][Once Cv_bv_cases] >>
  strip_tac >- srw_tac[][Once Cv_bv_cases] >>
  strip_tac >- srw_tac[][Once Cv_bv_cases] >>
  ...
  strip_tac >- (
    ...
    full_simp_tac(srw_ss())[el_check_def] >> metis_tac[]) >>
  rpt gen_tac >> strip_tac >>
  srw_tac[][Once Cv_bv_cases] >>
  full_simp_tac(srw_ss())[])
...
val no_closures_Cv_bv = store_thm("no_closures_Cv_bv",
  ...
  TRY (
    simp[Once Cv_bv_cases] >>
    full_simp_tac(srw_ss())[Once Cv_bv_cases] >>
    NO_TAC) >>
  full_simp_tac(srw_ss())[Cv_bv_cases_conv,Cv_bv_cases_vectorv] >>
  fsrw_tac[DNF_ss][EVERY2_EVERY,EVERY_MEM,MEM_ZIP] >>
  rev_full_simp_tac(srw_ss())[MEM_ZIP,GSYM LEFT_FORALL_IMP_THM] >> srw_tac[][] >>
...
val sv_refv_rel_def = Define`
  ...
  sv_refv_rel R (Varray vs) (ValueArray bvs) = LIST_REL R vs bvs ∧
  sv_refv_rel R _ _ = F`
...
val sv_refv_rel_cases = store_thm("sv_refv_rel_cases",
  ``sv_refv_rel R x y ⇔
    ...
    (∃vs bvs. x = Varray vs ∧ y = ValueArray bvs ∧ LIST_REL R vs bvs)``,
  Cases_on`x`>>Cases_on`y`>>srw_tac[][]>>
  Cases_on`l`>>srw_tac[][]>>Cases_on`t`>>srw_tac[][])

val sv_refv_rel_mono = store_thm("sv_refv_rel_mono",
  ``(∀x y. P x y ⇒ Q x y) ⇒ sv_refv_rel P x y ⇒ sv_refv_rel Q x y``,
  Cases_on`x`>>Cases_on`y`>>srw_tac[][]>>full_simp_tac(srw_ss())[sv_refv_rel_cases]>>metis_tac[EVERY2_mono])
...
val s_refs_with_pc = store_thm("s_refs_with_pc",
  ...
  srw_tac[][s_refs_def,good_rd_def])
...
val s_refs_append_code = store_thm("s_refs_append_code",
  ``∀rd s bs bs' ls.
    ...
    ⇒ s_refs rd s bs'``,
  srw_tac[][s_refs_def,fmap_rel_def,good_rd_def,FEVERY_DEF,UNCURRY] >>
  full_simp_tac(srw_ss())[EVERY2_EVERY,EVERY_MEM,FORALL_PROD] >> rpt strip_tac >>
  res_tac >>
  TRY (first_assum(match_exists_tac o concl) >> srw_tac[][]) >>
  TRY (match_mp_tac (GEN_ALL (MP_CANON optionTheory.OPTREL_MONO)) >> HINT_EXISTS_TAC >> srw_tac[][]) >>
  TRY (match_mp_tac (GEN_ALL (MP_CANON sv_refv_rel_mono)) >> HINT_EXISTS_TAC >> srw_tac[][]) >>
  match_mp_tac Cv_bv_l2a_mono_mp >>
  qexists_tac `mk_pp rd bs` >>
  srw_tac[][] >> metis_tac[bc_find_loc_aux_append_code])
...
val lookup_ct_imp_incsz = store_thm("lookup_ct_imp_incsz",
  ...
  Cases_on `b` >> srw_tac[][el_check_def] >>
  fsrw_tac[ARITH_ss][EL_CONS,PRE_SUB1,ADD1,EL_APPEND1] >>
  Cases_on`c`>>srw_tac[][el_check_def] >>
...
val lookup_ct_imp_incsz_many = store_thm("lookup_ct_imp_incsz_many",
  ``∀sz st refs gv b v st' sz' ls.
   ...
   ⇒ (lookup_ct sz' st' refs gv b = SOME v)``,
  Induct_on`sz' - sz` >- (
    srw_tac[][] >> `sz = sz'` by srw_tac[ARITH_ss][] >> full_simp_tac(srw_ss())[LENGTH_NIL] ) >>
  srw_tac[][] >> Cases_on`ls`>>full_simp_tac(srw_ss())[] >- fsrw_tac[ARITH_ss][] >>
  Cases_on`sz'`>>full_simp_tac(srw_ss())[ADD1] >>
...
val lookup_ct_change_refs = store_thm("lookup_ct_change_refs",
  ...
  srw_tac[][LET_THM] >>
  Cases_on`ct`>>full_simp_tac(srw_ss())[] >> srw_tac[][el_check_def] >> full_simp_tac(srw_ss())[] >>
  fsrw_tac[ARITH_ss][] >>
  Cases_on`c`>>full_simp_tac(srw_ss())[el_check_def]>>
  Cases_on`EL sz st`>>full_simp_tac(srw_ss())[] >> srw_tac[][]>>full_simp_tac(srw_ss())[]>>
  rpt(BasicProvers.CASE_TAC>>full_simp_tac(srw_ss())[]))
...
val fmap_rel_env_renv_with_irr = store_thm("fmap_rel_env_renv_with_irr",
  ...
  srw_tac[][fmap_rel_def,env_renv_def])
...
val Cenv_bs_with_irr = store_thm("Cenv_bs_with_irr",
  ``∀rd s env cenv sz bs bs'.
    ...
    ⇒
    Cenv_bs rd s env cenv sz bs'``,
  srw_tac[][Cenv_bs_def,s_refs_def,good_rd_def,env_renv_def,fmap_rel_def] >>
  rev_full_simp_tac(srw_ss())[o_f_FAPPLY])
...
val Cenv_bs_syneq_store = store_thm("Cenv_bs_syneq_store",
  ...
  PairCases_on`s`>>PairCases_on`s'`>>full_simp_tac(srw_ss())[csg_rel_def] >>
  simp_tac std_ss [] >>
  conj_tac >- metis_tac[] >>
  conj_asm1_tac >- metis_tac[EVERY2_EVERY] >>
  full_simp_tac(srw_ss())[EVERY2_EVERY,EVERY_MEM,FORALL_PROD] >>
  rev_full_simp_tac(srw_ss())[MEM_ZIP] >> full_simp_tac(srw_ss())[MEM_ZIP] >>
  full_simp_tac(srw_ss())[GSYM LEFT_FORALL_IMP_THM] >>
  conj_tac >- (
    srw_tac[][] >>
  ...
  rpt (first_x_assum(qspec_then`n`mp_tac)) >>
  Cases_on`EL n z`>>full_simp_tac(srw_ss())[optionTheory.OPTREL_def] >> srw_tac[][] >> full_simp_tac(srw_ss())[] >>
  metis_tac[Cv_bv_syneq,FST,SND])
...
val env_renv_imp_incsz = store_thm("env_renv_imp_incsz",
  ...
  srw_tac[][env_renv_def] >> srw_tac[][] >> full_simp_tac(srw_ss())[] >>
...
val Cenv_bs_imp_incsz = store_thm("Cenv_bs_imp_incsz",
  ...
  simp[Cenv_bs_def,s_refs_def,good_rd_def,ADD1] >> srw_tac[][] >>
  match_mp_tac env_renv_imp_incsz >>
  metis_tac[] )
...
val env_renv_imp_decsz = store_thm("env_renv_imp_decsz",
  ...
  srw_tac[][env_renv_def] >> full_simp_tac(srw_ss())[] >>
  full_simp_tac(srw_ss())[EVERY2_EVERY,EVERY_MEM,FORALL_PROD] >>
  rpt strip_tac >> res_tac >> pop_assum mp_tac >>
  BasicProvers.CASE_TAC >>
  pop_assum mp_tac >>
  srw_tac[][lookup_ct_incsz] >>
...
val Cenv_bs_imp_decsz = store_thm("Cenv_bs_imp_decsz",
  ``∀rd s env renv rsz bs bs' v.
    Cenv_bs rd s env renv (rsz+1) bs ∧
      ...
      (bs = bs' with stack := v::bs'.stack) ⇒
    Cenv_bs rd s env renv rsz bs'``,
  srw_tac[][Cenv_bs_def,fmap_rel_def,s_refs_def,FDOM_DRESTRICT,good_rd_def,ADD1] >> full_simp_tac(srw_ss())[] >>
  TRY (
    match_mp_tac env_renv_imp_decsz >>
    res_tac >> HINT_EXISTS_TAC >>
    simp[bc_state_component_equality] >>
    full_simp_tac(srw_ss())[IN_FRANGE] >> metis_tac[]) >>
...
val env_renv_CTLet_bound = store_thm("env_renv_CTLet_bound",
  ...
  srw_tac[][EVERY2_EVERY,EVERY_MEM,FORALL_PROD,env_renv_def] >>
  rev_full_simp_tac(srw_ss())[MEM_ZIP,MEM_EL] >> fsrw_tac[DNF_ss][] >>
...
val Cenv_bs_CTLet_bound = store_thm("Cenv_bs_CTLet_bound",
  ...
  srw_tac[][Cenv_bs_def,fmap_rel_def,IN_FRANGE] >>
...
val env_renv_imp_incsz_CTDec = store_thm("env_renv_imp_incsz_CTDec",
  ...
  HINT_EXISTS_TAC >> simp[] >>
  full_simp_tac(srw_ss())[EVERY2_EVERY] >> simp[MEM_ZIP] >>
...
val Cenv_bs_pops = store_thm("Cenv_bs_pops",
  ``∀vs rd s env renv rsz bs bs'.
    ...
    (bs = bs' with stack := vs ++ bs'.stack)
    ⇒ Cenv_bs rd s env renv rsz bs'``,
  Induct >> srw_tac[][] >- full_simp_tac(srw_ss())[] >>
  first_x_assum match_mp_tac >>
  simp[bc_state_component_equality] >>
...
val Cenv_bs_append_code = store_thm("Cenv_bs_append_code",
  ...
  reverse(srw_tac[][Cenv_bs_def]) >- PROVE_TAC[s_refs_append_code] >>
  full_simp_tac(srw_ss())[Cenv_bs_def,EVERY2_EVERY,EVERY_MEM,FORALL_PROD,s_refs_def,env_renv_def,fmap_rel_def] >> srw_tac[][] >>
  res_tac >>
...
val Cenv_bs_FUPDATE = store_thm("Cenv_bs_FUPDATE",
  ...
  reverse(reverse(srw_tac[][Cenv_bs_def,s_refs_def]) >> full_simp_tac(srw_ss())[el_check_def] >- full_simp_tac(srw_ss())[good_rd_def]) >>
  full_simp_tac(srw_ss())[EVERY2_EVERY,EVERY_MEM,FORALL_PROD,env_renv_def,fmap_rel_def,el_check_def] >>
...
val Cenv_bs_FUPDATE_LIST = store_thm("Cenv_bs_FUPDATE_LIST",
  ...
  srw_tac[][] >>
  srw_tac[][GENLIST,REVERSE_SNOC] >> simp[ADD1] >>
  REWRITE_TAC[ADD_ASSOC] >>
...
val Cenv_bs_FUPDATE_CTDec = store_thm("Cenv_bs_FUPDATE_CTDec",
  ...
  rpt gen_tac >>
  simp[Cenv_bs_def] >>
  reverse(reverse(srw_tac[][Cenv_bs_def,s_refs_def]) >> full_simp_tac(srw_ss())[compilerLibTheory.el_check_def] >- full_simp_tac(srw_ss())[good_rd_def]) >>
  full_simp_tac(srw_ss())[EVERY2_EVERY,EVERY_MEM,FORALL_PROD,env_renv_def,fmap_rel_def,compilerLibTheory.el_check_def] >>
  ...
  TRY(
    ...
    simp[]) >>
  strip_tac >>
  full_simp_tac(srw_ss())[option_case_NONE_F] >>
...
val Cenv_bs_FUPDATE_LIST_CTDec = store_thm("Cenv_bs_FUPDATE_LIST_CTDec",
  ...
  conj_tac >- (
    ...
    HINT_EXISTS_TAC >>
    simp[] ) >>
  reverse conj_tac >- full_simp_tac(srw_ss())[EVERY2_EVERY] >>
  match_mp_tac Cv_bv_l2a_mono_mp >>
  HINT_EXISTS_TAC >>
  simp[])
*)
...
val Cenv_bs_DOMSUB = store_thm("Cenv_bs_DOMSUB",
  ...
  srw_tac[][Cenv_bs_def,EVERY2_EVERY,env_renv_def,fmap_rel_def] >>
  metis_tac[o_f_FAPPLY])
...
val env_renv_change_store = store_thm("env_renv_change_store",
  ``env_renv rd rsz bs env renv ∧
    ...
    ⇒
    env_renv rd' rsz bs' env renv``,
  srw_tac[][env_renv_def,EVERY2_EVERY,EVERY_MEM,FORALL_PROD] >>
  ...
  `X = X'` by (
    ...
    conj_tac >- (
      rpt gen_tac >> strip_tac >>
      full_simp_tac(srw_ss())[s_refs_def,good_rd_def,FEVERY_DEF,UNCURRY,el_check_def] >>
      full_simp_tac(srw_ss())[SUBMAP_DEF,FDOM_DRESTRICT,FLOOKUP_DEF,DRESTRICT_DEF] >>
      srw_tac[][] >> full_simp_tac(srw_ss())[] >> full_simp_tac(srw_ss())[] >>
      BasicProvers.EVERY_CASE_TAC >> full_simp_tac(srw_ss())[] >>
      metis_tac[] ) >>
    rpt gen_tac >> strip_tac >>
    rev_full_simp_tac(srw_ss())[s_refs_def,EVERY2_EVERY,gvrel_def] >>
    full_simp_tac(srw_ss())[option_case_NONE_F] >> res_tac >>
  ...
  HINT_EXISTS_TAC >>
  simp[] >>
  full_simp_tac(srw_ss())[s_refs_def,good_rd_def,FEVERY_DEF,UNCURRY] >>
  full_simp_tac(srw_ss())[SUBMAP_DEF,SUBSET_DEF,DRESTRICT_DEF,IN_FRANGE] )
...
val Cenv_bs_change_store = store_thm("Cenv_bs_change_store",
  ``∀rd s env renv rsz bs rd' s' bs' rfs' ck' gv'.
    ...
    ⇒
    Cenv_bs rd' s' env renv rsz bs'``,
  srw_tac[][Cenv_bs_def,EVERY2_EVERY,EVERY_MEM,FORALL_PROD,fmap_rel_def] >>
...
val env_renv_shift = store_thm("env_renv_shift",
  ``∀rd sz bs env renv sz' bs' renv' st' ls ls'.
      ...
      env_renv rd sz' bs' env renv'``,
  srw_tac[][env_renv_def] >>
  full_simp_tac(srw_ss())[EVERY2_EVERY,EVERY_MEM] >>
  rpt(qpat_assum`X = Y`mp_tac) >> ntac 2 strip_tac >>
  full_simp_tac(srw_ss())[MEM_ZIP,GSYM LEFT_FORALL_IMP_THM,EL_MAP] >>
  full_simp_tac(srw_ss())[compilerLibTheory.el_check_def,option_case_NONE_F])
*)
...
val env_renv_append_imp = store_thm("env_renv_append_imp",
  ...
  srw_tac[][] >>
  full_simp_tac(srw_ss())[env_renv_def] >>
...
val env_renv_syneq = store_thm("env_renv_syneq",
  ...
  srw_tac[][] >>
  full_simp_tac(srw_ss())[env_renv_def,option_case_NONE_F] >> srw_tac[][] >>
  full_simp_tac(srw_ss())[EVERY2_EVERY,EVERY_MEM] >> rev_full_simp_tac(srw_ss())[] >>
  `LENGTH l2 = LENGTH l1` by metis_tac[] >>
  full_simp_tac(srw_ss())[MEM_ZIP,GSYM LEFT_FORALL_IMP_THM] >>
  srw_tac[][] >>
...
val Cenv_bs_bind_fv = store_thm("Cenv_bs_bind_fv",
  ...
  Cases_on`n< LENGTH vs` >- (
    full_simp_tac(srw_ss())[bind_fv_def,LET_THM] >> srw_tac[][] >>
    ...
    full_simp_tac(srw_ss())[EVERY_MEM] >> rev_full_simp_tac(srw_ss())[MEM_ZIP,GSYM LEFT_FORALL_IMP_THM] >>
  ...
  Cases_on`n = LENGTH vs` >- (
    ...
    Cases_on`EL ix defs`>>full_simp_tac(srw_ss())[] >>
  ...
  `lrecs = LENGTH recs` by ( unabbrev_all_tac >> srw_tac[][]) >>
  Cases_on`n < LENGTH vs + 1 + LENGTH recs` >- (
    ...
    simp[] >> strip_tac >> simp[] >>
    full_simp_tac(srw_ss())[FLOOKUP_DEF,DRESTRICT_DEF] >>
    full_simp_tac(srw_ss())[s_refs_def,good_rd_def,FEVERY_DEF,UNCURRY] >>
  ...
  qunabbrev_tac`lrecs` >> full_simp_tac(srw_ss())[] >>
  qmatch_assum_abbrev_tac`n < LENGTH vs + 1 + lrecs + LENGTH envs` >>
  `lrecs = LENGTH recs` by simp[Abbr`lrecs`] >> full_simp_tac(srw_ss())[] >>
...
val PushAnyInt_thm = store_thm("PushAnyInt_thm",
  ``∀i bs bc0 bc1.
       bs.code = bc0 ++ PushAnyInt i ++ bc1 ∧
       bs.pc = next_addr bs.inst_length bc0
       ⇒
       let bs' = bs with <| pc := next_addr bs.inst_length (bc0 ++ PushAnyInt i);
                            stack := (Number i)::bs.stack |>
       in bc_next^* bs bs'``,
  ho_match_mp_tac PushAnyInt_ind >> simp[] >>
  gen_tac >> rpt strip_tac >>
  Cases_on`i < 0` >> full_simp_tac(srw_ss())[] >- (
    qpat_assum`bs.code = X`mp_tac >>
    simp[Once PushAnyInt_def] >>
    strip_tac >>
    srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
    `bc_fetch bs = SOME(Stack(PushInt 0))` by (
      match_mp_tac bc_fetch_next_addr >>
      qexists_tac`bc0` >> simp[] ) >>
    simp[bc_eval1_def,bc_eval1_thm,bc_eval_stack_def,bump_pc_def] >>
    qmatch_abbrev_tac`bc_next^* bs1 bs2` >>
    first_x_assum(qspecl_then[`bs1`,`bc0++[Stack(PushInt 0)]`]mp_tac) >>
    simp[Abbr`bs1`,SUM_APPEND,FILTER_APPEND] >> strip_tac >>
    srw_tac[DNF_ss][Once RTC_CASES_RTC_TWICE] >>
    first_assum(match_exists_tac o concl) >> simp[] >>
    pop_assum kall_tac >>
    qmatch_abbrev_tac`bc_next^* bs1 bs2` >>
    `bc_fetch bs1 = SOME(Stack(Sub))` by (
      match_mp_tac bc_fetch_next_addr >>
      CONV_TAC SWAP_EXISTS_CONV >>
      qexists_tac`bc1` >> simp[Abbr`bs1`] >>
      simp[SUM_APPEND,FILTER_APPEND]) >>
    match_mp_tac RTC_SUBSET >>
    simp[bc_eval1_def,bc_eval1_thm,bc_eval_stack_def,bump_pc_def,Abbr`bs1`] >>
    simp[Abbr`bs2`,bc_state_component_equality] >>
    simp[SUM_APPEND,FILTER_APPEND,Once PushAnyInt_def,SimpRHS] ) >>
  Cases_on`i <= maxPushInt` >> full_simp_tac(srw_ss())[] >- (
    full_simp_tac(srw_ss())[Once PushAnyInt_def] >>
    `bc_fetch bs = SOME(Stack(PushInt i))` by (
      match_mp_tac bc_fetch_next_addr >>
      qexists_tac`bc0` >> simp[] ) >>
    match_mp_tac RTC_SUBSET >>
    simp[bc_eval1_def,bc_eval1_thm,bc_eval_stack_def,bump_pc_def] >>
    simp[bc_state_component_equality] >>
    simp[SUM_APPEND,FILTER_APPEND,Once PushAnyInt_def] ) >>
  qpat_assum`bs.code =X`mp_tac >>
  simp[Once PushAnyInt_def] >> strip_tac >>
  ...
  qmatch_abbrev_tac`bc_next^* bs1 bs2` >>
  srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
  qmatch_assum_abbrev_tac`bs.code = bc0 ++ i1 ++ [i2;i3] ++ l4 ++ bc1` >>
  `bc_fetch bs1 = SOME(Stack(PushInt maxPushInt))` by (
    match_mp_tac bc_fetch_next_addr >>
    qexists_tac`bc0++i1` >>
    simp[Abbr`bs1`] ) >>
  simp[bc_eval1_thm,bc_eval1_def,Abbr`i2`,bc_eval_stack_def,bump_pc_def] >>
  pop_assum kall_tac >> simp[Abbr`bs1`] >>
  qmatch_abbrev_tac`bc_next^* bs1 bs2` >>
  srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
  qmatch_assum_abbrev_tac`bs.code = bc0 ++ i1 ++ [i2;i3] ++ l4 ++ bc1` >>
  `bc_fetch bs1 = SOME(Stack Mult)` by (
    match_mp_tac bc_fetch_next_addr >>
    qexists_tac`bc0++i1++[i2]` >>
    simp[Abbr`bs1`] >>
    simp[FILTER_APPEND,SUM_APPEND,Abbr`i2`]) >>
  simp[bc_eval1_thm,bc_eval1_def,Abbr`i3`,bc_eval_stack_def,bump_pc_def,Abbr`bs1`] >>
  pop_assum kall_tac >>
  Cases_on`i % maxPushInt = 0` >> full_simp_tac(srw_ss())[Abbr`l4`] >- (
    srw_tac[DNF_ss][Once RTC_CASES1] >> disj1_tac >>
    simp[Abbr`bs2`,bc_state_component_equality] >>
    simp[SUM_APPEND,FILTER_APPEND,Abbr`i2`,Abbr`i1`] >>
    simp[Once PushAnyInt_def,SimpRHS] >>
    simp[SUM_APPEND,FILTER_APPEND] >>
    match_mp_tac integerTheory.INT_DIV_MUL_ID >>
    full_simp_tac(srw_ss())[maxPushInt_def] ) >>
  qmatch_assum_abbrev_tac`bs.code = bc0 ++ i1 ++ [i2;i3;i4;i5] ++ bc1` >>
  qmatch_abbrev_tac`bc_next^* bs1 bs2` >>
  srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
  `bc_fetch bs1 = SOME(i4)` by (
    match_mp_tac bc_fetch_next_addr >>
    qexists_tac`bc0++i1++[i2;i3]` >>
    simp[Abbr`bs1`] >>
    simp[FILTER_APPEND,SUM_APPEND,Abbr`i4`,Abbr`i2`,Abbr`i1`,Abbr`i3`]) >>
  simp[bc_eval1_thm,bc_eval1_def,Abbr`i4`,bc_eval_stack_def,bump_pc_def,Abbr`bs1`] >>
  pop_assum kall_tac >>
  qmatch_assum_abbrev_tac`bs.code = bc0 ++ i1 ++ [i2;i3;i4;i5] ++ bc1` >>
  qmatch_abbrev_tac`bc_next^* bs1 bs2` >>
  srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
  `bc_fetch bs1 = SOME(i5)` by (
    match_mp_tac bc_fetch_next_addr >>
    qexists_tac`bc0++i1++[i2;i3;i4]` >>
    simp[Abbr`bs1`] >>
    simp[FILTER_APPEND,SUM_APPEND,Abbr`i4`,Abbr`i2`,Abbr`i1`,Abbr`i3`,Abbr`i5`]) >>
  simp[bc_eval1_thm,bc_eval1_def,Abbr`i5`,bc_eval_stack_def,bump_pc_def,Abbr`bs1`] >>
  srw_tac[DNF_ss][Once RTC_CASES1] >> disj1_tac >>
  simp[Abbr`bs2`,bc_state_component_equality] >>
  simp[SUM_APPEND,FILTER_APPEND,Abbr`i2`,Abbr`i1`,Abbr`i3`,Abbr`i4`] >>
  simp[Once PushAnyInt_def,SimpRHS] >>
  simp[SUM_APPEND,FILTER_APPEND] >>
...
val PushAnyInt_labels = store_thm("PushAnyInt_labels",
  ...
  ho_match_mp_tac PushAnyInt_ind >> srw_tac[][] >>
  srw_tac[][Once PushAnyInt_def] >> full_simp_tac(srw_ss())[] >> srw_tac[][] >>
  rpt(
    rpt(match_mp_tac code_labels_ok_cons >> simp[]) >>
    match_mp_tac code_labels_ok_append >> srw_tac[][]))
val _ = export_rewrites["PushAnyInt_labels"]

val PushAnyInt_inst_uses_label = store_thm("PushAnyInt_inst_uses_label",
  ``∀i l. ¬EXISTS (inst_uses_label l) (PushAnyInt i)``,
  ho_match_mp_tac PushAnyInt_ind >> srw_tac[][] >>
  srw_tac[][Once PushAnyInt_def] >> full_simp_tac(srw_ss())[] >> srw_tac[][])
...
val FOLDL_emit_append_out = prove(
  ...
  Induct >> simp[compiler_result_component_equality] >> full_simp_tac(srw_ss())[])
  |> SIMP_RULE (srw_ss())[]
...
val compile_varref_thm = store_thm("compile_varref_thm",
  ``∀bs bc0 code bc1 cls sz cs b bv bs'.
      ...
      ⇒
      bc_next^* bs bs'``,
  ntac 7 gen_tac >> Cases >> srw_tac[][] >>
  TRY(Cases_on`c`>>full_simp_tac(srw_ss())[])>> full_simp_tac(srw_ss())[FOLDL_emit_append_out] >>
  TRY(
    qpat_assum`X = REVERSE code`(assume_tac o SIMP_RULE std_ss [Once SWAP_REVERSE]) >> full_simp_tac(srw_ss())[] >>
    qmatch_assum_abbrev_tac `code = x::ls1` >>
    `bc_fetch bs = SOME x` by (
      match_mp_tac bc_fetch_next_addr >>
      map_every qexists_tac [`bc0`,`ls1++bc1`] >>
      srw_tac[][Abbr`x`,Abbr`ls1`])) >>
  TRY (
    qmatch_abbrev_tac `bc_next^* bs bs'` >>
    `(bc_eval1 bs = SOME bs')` by (
      srw_tac[][bc_eval1_def,Abbr`x`] >>
      rev_full_simp_tac(srw_ss())[el_check_def] >>
      TRY (qpat_assum`OPTION_JOIN X = SOME bv`mp_tac >> srw_tac[][el_check_def]) >>
      srw_tac[][bc_eval_stack_def] >>
  ...
  >- (
    ...
    BasicProvers.EVERY_CASE_TAC >> srw_tac[][] >>
    qpat_assum `X = SOME bv` mp_tac >>
    BasicProvers.EVERY_CASE_TAC >> srw_tac[][] >>
    srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
    srw_tac[][bc_eval1_thm] >>
    srw_tac[][Once bc_eval1_def,Abbr`x`] >>
    srw_tac[DNF_ss][] >>
    srw_tac[][bc_eval_stack_def,bump_pc_def] >>
    qmatch_abbrev_tac`bc_next^* bs0 bs'` >>
    qunabbrev_tac`ls1` >>
    full_simp_tac(srw_ss())[] >>
    qmatch_assum_abbrev_tac `bs.code = ls0 ++ [x] ++ yy ++ [y;z;w] ++ bc1` >>
    srw_tac[DNF_ss][Once RTC_CASES_RTC_TWICE] >>
    qspecl_then[`&n`,`bs0`,`ls0++[x]`]mp_tac PushAnyInt_thm >>
    simp[Abbr`bs0`] >>
    simp[SUM_APPEND,FILTER_APPEND,Abbr`x`,ADD1] >>
    strip_tac >>
    first_assum(match_exists_tac o concl) >> simp[] >>
    pop_assum kall_tac >>
    qmatch_abbrev_tac`bc_next^* bs0 bs'` >>
    ...
    full_simp_tac(srw_ss())[FLOOKUP_DEF] >>
    unabbrev_all_tac >>
    full_simp_tac(srw_ss())[FDOM_DRESTRICT,DRESTRICT_DEF] >>
  ...
  >- (
    ...
    BasicProvers.EVERY_CASE_TAC >> srw_tac[][] >>
    srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
    srw_tac[][bc_eval1_thm,bc_eval1_def,Abbr`x`,bc_eval_stack_def,bump_pc_def] >>
    qmatch_abbrev_tac `bc_next^* bs0 bs'` >>
    srw_tac[DNF_ss][Once RTC_CASES_RTC_TWICE] >>
    qspecl_then[`&n`,`bs0`,`bc0++[Stack(Load sz)]`]mp_tac PushAnyInt_thm >>
    simp[Abbr`bs0`,SUM_APPEND,FILTER_APPEND] >>
    strip_tac >> first_assum(match_exists_tac o concl) >> simp[] >>
    pop_assum kall_tac >>
    qmatch_abbrev_tac `bc_next^* bs0 bs'` >>
    srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
    `bc_fetch bs0 = SOME(Stack El)` by (
      match_mp_tac bc_fetch_next_addr >>
      CONV_TAC SWAP_EXISTS_CONV >>
      qexists_tac`bc1` >>
      simp[Abbr`bs0`,SUM_APPEND,FILTER_APPEND] ) >>
    simp[bc_eval1_thm,bc_eval1_def,bc_eval_stack_def,bump_pc_def,Abbr`bs0`] >>
    srw_tac[DNF_ss][Once RTC_CASES1] >> disj1_tac >>
    unabbrev_all_tac >>
...
val compile_envref_next_label_inc = store_thm("compile_envref_next_label_inc",
  ...
  ntac 2 gen_tac >> Cases >> srw_tac[][FOLDL_emit_append_out])
...
val compile_envref_append_out = store_thm("compile_envref_append_out",
  ...
  ho_match_mp_tac compile_envref_ind >> srw_tac[][FOLDL_emit_append_out,EVERY_REVERSE,PushAnyInt_labels] >>
  rpt(match_mp_tac code_labels_ok_cons >> simp[]) >>
  match_mp_tac code_labels_ok_append >> srw_tac[][PushAnyInt_labels] >>
  rpt(match_mp_tac code_labels_ok_cons >> simp[]))
...
val compile_varref_append_out = store_thm("compile_varref_append_out",
  ...
  ntac 2 gen_tac >> Cases >> srw_tac[][compile_envref_append_out] >>
  rpt(match_mp_tac code_labels_ok_cons >> simp[]))
...
val bc_equal_list_Number = prove(
  ...
  gen_tac >> Cases >> simp[bc_equal_def] >> srw_tac[][])
...
val do_Ceq_to_bc_equal = Q.prove (
  ...
  srw_tac[][] >-
  (helper_tac >> TRY(full_simp_tac(srw_ss())[semanticPrimitivesTheory.lit_same_type_def,LIST_EQ_REWRITE]>>NO_TAC) >>
   simp[stringTheory.ORD_11] >>
   match_mp_tac bc_equal_list_Number >> srw_tac[][stringTheory.ORD_11] ) >-
  (helper_tac >> metis_tac [ALL_DISTINCT_EL_IMP]) >-
  (helper_tac >> full_simp_tac(srw_ss())[EVERY2_EVERY] >> metis_tac []) >-
  (helper_tac >> full_simp_tac(srw_ss())[EVERY2_EVERY] >> metis_tac []) >-
  (helper_tac >> full_simp_tac(srw_ss())[EVERY2_EVERY]) >-
  (helper_tac >> full_simp_tac(srw_ss())[EVERY2_EVERY]) >-
  (helper_tac >> full_simp_tac(srw_ss())[EVERY2_EVERY]) >-
  (helper_tac >> full_simp_tac(srw_ss())[EVERY2_EVERY]) >-
  helper_tac >-
...
val inst_uses_label_prim2_to_bc = store_thm("inst_uses_label_prim2_to_bc",
  ...
  srw_tac[][inst_uses_label_def])
val _ = export_rewrites["inst_uses_label_prim2_to_bc"]

val tac1 =
  Cases_on `v1` >> TRY (Cases_on `l:lit`) >> full_simp_tac(srw_ss())[] >>
  full_simp_tac(srw_ss())[Q.SPECL[`CLitv X`](CONJUNCT1 (Q.SPEC`pp`Cv_bv_cases))] >>
  full_simp_tac(srw_ss())[Q.SPEC`CLoc X`(CONJUNCT1 (Q.SPEC`pp`Cv_bv_cases))] >>
  rpt BasicProvers.VAR_EQ_TAC >>
...
val tac2 =
  `LENGTH (FST pp).sm = LENGTH s1` by full_simp_tac(srw_ss())[s_refs_def] >>
  ...
  full_simp_tac(srw_ss())[s_refs_def] >>
  simp[GSYM CONJ_ASSOC] >>
  conj_tac >- (
    full_simp_tac(srw_ss())[good_rd_def,FEVERY_DEF,UNCURRY,FLOOKUP_UPDATE] >>
    gen_tac >> strip_tac >>
    first_assum(fn th => first_x_assum(strip_assume_tac o MATCH_MP th)) >>
    conj_asm1_tac >- (spose_not_then strip_assume_tac >> full_simp_tac(srw_ss())[FLOOKUP_DEF]) >>
    simp[] >>
    match_mp_tac(MP_CANON(GEN_ALL(CONJUNCT1(SPEC_ALL Cv_bv_SUBMAP)))) >>
    first_assum(match_exists_tac o concl) >>
    simp[] >>
    full_simp_tac(srw_ss())[EVERY_MEM,FLOOKUP_DEF] >>
    metis_tac[] ) >>
  conj_tac >- full_simp_tac(srw_ss())[EVERY_MEM] >>
  conj_tac >- (
    full_simp_tac(srw_ss())[ALL_DISTINCT_APPEND,EVERY_MEM] >>
    metis_tac[] ) >>
  simp[integerTheory.INT_ABS] >>
  simp[CONJ_ASSOC] >>
  reverse conj_tac >- (
    simp[SUBMAP_DEF,DRESTRICT_DEF] >> srw_tac[][] >> srw_tac[][] ) >>
  rpt conj_tac >>
  TRY (
    simp[LIST_REL_EL_EQN,LENGTH_REPLICATE,EL_REPLICATE] >>
    TRY (
      Cases_on`pp`>>full_simp_tac(srw_ss())[]>>srw_tac[][]>>
      match_mp_tac(MP_CANON(GEN_ALL(CONJUNCT1(SPEC_ALL Cv_bv_SUBMAP)))) >>
      first_assum(match_exists_tac o concl) >>
      simp[] >>
      full_simp_tac(srw_ss())[EVERY_MEM,FLOOKUP_DEF,good_rd_def,FEVERY_DEF,UNCURRY] >>
      metis_tac[] ) >>
    full_simp_tac(srw_ss())[Once Cv_bv_cases] >> NO_TAC) >>
  match_mp_tac (MP_CANON (GEN_ALL EVERY2_mono)) >>
...
val prim2_to_bc_thm = store_thm("prim2_to_bc_thm",
  ...
  Cases_on`op` >- (
    ...
    (Cases_on`op`)>>full_simp_tac(srw_ss())[bump_pc_with_stack] >>
    TRY (
      Cases_on `do_Ceq v1 v2` >>
      full_simp_tac(srw_ss())[CBoolv_def] >>
      srw_tac[][Once Cv_bv_cases, bc_eval_stack_def,bc_state_component_equality] >>
      imp_res_tac do_Ceq_to_bc_equal >>
      srw_tac[][bump_pc_def,bc_fetch_with_refs] >>
      simp[s_refs_with_stack,conLangTheory.true_tag_def,conLangTheory.false_tag_def] >>
    ...
    full_simp_tac(srw_ss())[] >> srw_tac[][CBoolv_def] >> full_simp_tac(srw_ss())[Cv_bv_cases_lit,Cv_bv_cases_conv,Cv_bv_cases_vectorv] >> srw_tac[][] >>
    BasicProvers.EVERY_CASE_TAC >>
    srw_tac[][bc_eval_stack_def] >> full_simp_tac(srw_ss())[] >>
    TRY (Cases_on `b` >> srw_tac[][]) >>
    TRY (Cases_on `b'` >> srw_tac[][]) >>
    srw_tac[ARITH_ss][] >>
    full_simp_tac(srw_ss())[EVERY2_EVERY,EVERY_MEM,FORALL_PROD] >>
    fsrw_tac[DNF_ss][EL_ALL_DISTINCT_EL_EQ,el_check_def] >>
    full_simp_tac(srw_ss())[s_refs_def,good_rd_def] >>
    simp[bc_state_component_equality,
         conLangTheory.true_tag_def,
         conLangTheory.false_tag_def] >>
    full_simp_tac(srw_ss())[integerTheory.INT_NOT_LT] >>
    rev_full_simp_tac(srw_ss())[GSYM integerTheory.INT_ABS_EQ_ID] >>
    simp[arithmeticTheory.ADD1,CBoolv_def] >>
    Cases_on`Num i`>>simp[] >>
    Cases_on`pp`>>full_simp_tac(srw_ss())[]>>
    first_x_assum match_mp_tac >>
    simp[MEM_ZIP] >>
  ...
  Cases_on `v2` >> TRY (Cases_on `l:lit`) >> full_simp_tac(srw_ss())[]
  >- ( tac1 >> tac2 )
  >- (
    tac1 >>
    simp[s_refs_with_stack,s_refs_with_pc] >>
    ...
    qexists_tac`bs.refs` >>
    simp[] >>
    full_simp_tac(srw_ss())[s_refs_def,FLOOKUP_DEF,EVERY_MEM] >>
    rpt BasicProvers.VAR_EQ_TAC >>
    full_simp_tac(srw_ss())[good_rd_def] >>
    conj_asm1_tac >- (
      first_x_assum match_mp_tac >>
      metis_tac[MEM_EL] ) >>
    full_simp_tac(srw_ss())[LIST_REL_EL_EQN] >>
    ...
    rev_full_simp_tac(srw_ss())[integerTheory.int_le,integerTheory.INT_ABS] >>
    srw_tac[][] >> DECIDE_TAC)
  >- (
    qpat_assum`X = Rval v`mp_tac >>
    IF_CASES_TAC >> simp[] >> srw_tac[][] >>
    full_simp_tac(srw_ss())[Q.SPECL[`CLitv X`](CONJUNCT1 (Q.SPEC`pp`Cv_bv_cases))] >>
    srw_tac[][] >> simp[integerTheory.int_le] >>
    simp[Q.SPEC`CLoc X`(CONJUNCT1 (Q.SPEC`pp`Cv_bv_cases)),PULL_EXISTS] >>
    simp[bump_pc_def,bc_fetch_with_refs,bc_state_component_equality] >>
    full_simp_tac(srw_ss())[el_check_def] >>
    full_simp_tac(srw_ss())[integerTheory.INT_ABS] >>
    tac2 )
  >- (
    full_simp_tac(srw_ss())[Q.SPECL[`CLitv X`](CONJUNCT1 (Q.SPEC`pp`Cv_bv_cases))] >> srw_tac[][] >>
    Cases_on`v1`>>full_simp_tac(srw_ss())[] >>
    full_simp_tac(srw_ss())[Q.SPEC`CLoc X`(CONJUNCT1 (Q.SPEC`pp`Cv_bv_cases)),PULL_EXISTS] >>
    ...
    `LIST_REL (sv_refv_rel (Cv_bv (mk_pp (FST pp) (bs with code := bce)))) s1 (MAP ($' bs.refs) (FST pp).sm)` by (
      full_simp_tac(srw_ss())[s_refs_def] ) >>
    full_simp_tac(srw_ss())[el_check_def,LIST_REL_EL_EQN] >>
    pop_assum(qspec_then`n`mp_tac) >>
    simp[Once sv_refv_rel_cases,EL_MAP,FLOOKUP_DEF] >> srw_tac[][] >> simp[] >>
    full_simp_tac(srw_ss())[LIST_REL_EL_EQN] >> rev_full_simp_tac(srw_ss())[integerTheory.INT_ABS] >>
    ...
    full_simp_tac(srw_ss())[s_refs_def,good_rd_def,EVERY_MEM,MEM_EL,PULL_EXISTS] ))
...
val VfromListCode_aux_correct = prove(
  ``∀vl ls bs bc0 bc1 bvl st cnt pp loopcode.
      loopcode = DROP 2 (TAKE 22 VfromListCode) ∧
  ...
  conj_tac >- (
    ...
    EVAL_TAC >> simp[]) >>
  srw_tac[][] >>
  Cases_on`CvFromList vl`>>full_simp_tac(srw_ss())[]>>srw_tac[][]>>
  simp[PULL_EXISTS] >>
  srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
  `bc_fetch bs = SOME (Stack (Load 1))` by (
    match_mp_tac bc_fetch_next_addr >>
  ...
  full_simp_tac(srw_ss())[Cv_bv_cases_conv] >> srw_tac[][] >>
  simp[bc_eval1_thm,bc_eval1_def,bump_pc_def,bc_eval_stack_def,Abbr`bs1`] >>
  ...
  full_simp_tac(srw_ss())[bump_pc_def,bc_fetch_with_stack] >>
  qho_match_abbrev_tac`∃v vs. bc_next^* bs1 (bs2 v vs) ∧ P v vs` >>
  ...
  pop_assum kall_tac >>
  qho_match_abbrev_tac`∃v vs. bc_next^* bs1 (bs2 v vs) ∧ P v vs` >>
  `bc_fetch bs1 = SOME (Stack (PushInt 1))` by (next_addr_tac`9`) >>
  srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
  simp[bc_eval1_thm,bc_eval1_def,Abbr`bs1`,bc_eval_stack_def,bump_pc_def] >>
  pop_assum kall_tac >>
  qho_match_abbrev_tac`∃v vs. bc_next^* bs1 (bs2 v vs) ∧ P v vs` >>
  `bc_fetch bs1 = SOME (Stack El)` by (next_addr_tac`10`) >>
  srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
  simp[bc_eval1_thm,bc_eval1_def,Abbr`bs1`,bc_eval_stack_def,bump_pc_def] >>
  pop_assum kall_tac >>
  qho_match_abbrev_tac`∃v vs. bc_next^* bs1 (bs2 v vs) ∧ P v vs` >>
  `bc_fetch bs1 = SOME (Stack (Load 2))` by (next_addr_tac`11`) >>
  srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
  simp[bc_eval1_thm,bc_eval1_def,Abbr`bs1`,bc_eval_stack_def,bump_pc_def] >>
  pop_assum kall_tac >>
  qho_match_abbrev_tac`∃v vs. bc_next^* bs1 (bs2 v vs) ∧ P v vs` >>
  `bc_fetch bs1 = SOME (Stack (PushInt 0))` by (next_addr_tac`12`) >>
  srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
  simp[bc_eval1_thm,bc_eval1_def,Abbr`bs1`,bc_eval_stack_def,bump_pc_def] >>
  pop_assum kall_tac >>
  qho_match_abbrev_tac`∃v vs. bc_next^* bs1 (bs2 v vs) ∧ P v vs` >>
  `bc_fetch bs1 = SOME (Stack El)` by (next_addr_tac`13`) >>
  srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
  simp[bc_eval1_thm,bc_eval1_def,Abbr`bs1`,bc_eval_stack_def,bump_pc_def] >>
  pop_assum kall_tac >>
  qho_match_abbrev_tac`∃v vs. bc_next^* bs1 (bs2 v vs) ∧ P v vs` >>
  `bc_fetch bs1 = SOME (Stack (Store 2))` by (next_addr_tac`14`) >>
  srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
  simp[bc_eval1_thm,bc_eval1_def,Abbr`bs1`,bc_eval_stack_def,bump_pc_def] >>
  pop_assum kall_tac >>
  qho_match_abbrev_tac`∃v vs. bc_next^* bs1 (bs2 v vs) ∧ P v vs` >>
  `bc_fetch bs1 = SOME (Stack (Load 0))` by (next_addr_tac`15`) >>
  srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
  simp[bc_eval1_thm,bc_eval1_def,Abbr`bs1`,bc_eval_stack_def,bump_pc_def] >>
  pop_assum kall_tac >>
  qho_match_abbrev_tac`∃v vs. bc_next^* bs1 (bs2 v vs) ∧ P v vs` >>
  `bc_fetch bs1 = SOME (Stack (Load 2))` by (next_addr_tac`16`) >>
  srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
  simp[bc_eval1_thm,bc_eval1_def,Abbr`bs1`,bc_eval_stack_def,bump_pc_def] >>
  pop_assum kall_tac >>
  qho_match_abbrev_tac`∃v vs. bc_next^* bs1 (bs2 v vs) ∧ P v vs` >>
  `bc_fetch bs1 = SOME (Stack (Store 1))` by (next_addr_tac`17`) >>
  srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
  simp[bc_eval1_thm,bc_eval1_def,Abbr`bs1`,bc_eval_stack_def,bump_pc_def] >>
  pop_assum kall_tac >>
  qho_match_abbrev_tac`∃v vs. bc_next^* bs1 (bs2 v vs) ∧ P v vs` >>
  `bc_fetch bs1 = SOME (Stack (Store 1))` by (next_addr_tac`18`) >>
  srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
  simp[bc_eval1_thm,bc_eval1_def,Abbr`bs1`,bc_eval_stack_def,bump_pc_def] >>
  pop_assum kall_tac >>
  qho_match_abbrev_tac`∃v vs. bc_next^* bs1 (bs2 v vs) ∧ P v vs` >>
  `bc_fetch bs1 = SOME (Jump (Lab (VfromListLab + 1)))` by (next_addr_tac`19`) >>
  srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
  simp[bc_eval1_thm,bc_eval1_def,Abbr`bs1`,PULL_EXISTS] >>
  pop_assum kall_tac >>
  CONV_TAC(RESORT_EXISTS_CONV(sort_vars["n"])) >>
  ...
  rev_full_simp_tac(srw_ss())[] >> simp[Abbr`bs1`] >>
...
val VfromListCode_correct = prove(
  ...
  full_simp_tac(srw_ss())[contains_primitives_def,bc_find_loc_def] >>
  ...
  conj_tac >- (
    ...
    match_mp_tac bc_find_loc_aux_append_code >>
    match_mp_tac bc_find_loc_aux_ALL_DISTINCT >>
    qexists_tac`LENGTH bc0` >> rev_full_simp_tac(srw_ss())[] >>
  ...
  simp[Once VfromListCode_def] >>
  simp[Once VfromListCode_def] >> rev_full_simp_tac(srw_ss())[] >>
  simp[Abbr`bs1`] >>
  disch_then(qspec_then`pp`mp_tac) >> simp[] >>
  discharge_hyps >- (
    ...
    EVAL_TAC >>
    strip_tac >> simp[]  >> srw_tac[][] >>
    spose_not_then strip_assume_tac >> res_tac >> full_simp_tac(srw_ss())[]) >>
  strip_tac >>
  ...
  qunabbrev_tac`bs1'`>>full_simp_tac(srw_ss())[] >> pop_assum kall_tac >>
  ...
  full_simp_tac(srw_ss())[Abbr`P`] >> imp_res_tac LIST_REL_LENGTH >> simp[] >>
...
fun next_addr_tac [QUOTE s] =
    match_mp_tac bc_fetch_next_addr >> simp[Abbr`bs1`] >>
    qexists_tac`bc0 ++ (DROP 2 (TAKE ^(Parse.Term [QUOTE (s^":num")]) ImplodeCode))` >>
    simp[ImplodeCode_def] >>
    REWRITE_TAC[SUM_APPEND,FILTER_APPEND,MAP_APPEND] >>
    EVAL_TAC >> simp[]

val ImplodeCode_aux_correct = prove(
  ``∀vl ls bs bc0 bc1 bvl st cnt pp loopcode.
      loopcode = DROP 2 (TAKE 22 ImplodeCode) ∧
      bs.code = bc0 ++ loopcode ++ bc1 ∧
      ALL_DISTINCT (FILTER is_Label (bc0 ++ loopcode)) ∧
      bs.stack = Number cnt::bvl::st ∧
      bs.pc = next_addr bs.inst_length bc0 ∧
      ...
      ⇒
      let bs' = bs with <| pc := next_addr bs.inst_length (bc0++loopcode);
                           stack := Number (cnt + &LENGTH ls)::(REVERSE (MAP (Number o $& o ORD) ls))++st |> in
      bc_next^* bs bs'``,
  ...
  conj_tac >- (
    ...
    `bc_fetch bs = SOME (Stack (Load 1))` by (
      match_mp_tac bc_fetch_next_addr >>
      qexists_tac`bc0++[Label(ImplodeLab+1)]` >> simp[] >>
      simp[FILTER_APPEND,SUM_APPEND] >>
      simp[ImplodeCode_def] ) >>
    simp[bc_eval1_thm,bc_eval1_def,bump_pc_def,bc_eval_stack_def] >>
    qho_match_abbrev_tac`bc_next^* bs1 bs2` >>
    ...
    `bc_fetch bs1 = SOME (Stack (TagEq (block_tag+nil_tag)))` by (
      match_mp_tac bc_fetch_next_addr >> simp[Abbr`bs1`] >>
      qexists_tac`bc0 ++ (DROP 2 (TAKE 4 ImplodeCode))` >>
      simp[ImplodeCode_def] >>
      simp[SUM_APPEND,FILTER_APPEND] ) >>
    simp[bc_eval1_thm,bc_eval1_def,bump_pc_def,bc_eval_stack_def,Abbr`bs1`] >>
    qho_match_abbrev_tac`bc_next^* bs1 bs2` >>
    `bc_fetch bs1 = SOME (JumpIf (Lab (ImplodeLab+2)))` by (
      match_mp_tac bc_fetch_next_addr >> simp[Abbr`bs1`] >>
      qexists_tac`bc0 ++ (DROP 2 (TAKE 5 ImplodeCode))` >>
      simp[ImplodeCode_def] >>
      simp[SUM_APPEND,FILTER_APPEND] ) >>
    srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
    simp[bc_eval1_thm,bc_eval1_def,Abbr`bs1`,PULL_EXISTS] >>
    qexists_tac`next_addr bs.inst_length (bc0 ++ DROP 2 (TAKE 21 ImplodeCode))` >>
    conj_tac >- (
      simp[bc_find_loc_def] >>
      match_mp_tac bc_find_loc_aux_append_code >>
      match_mp_tac bc_find_loc_aux_ALL_DISTINCT >>
      qexists_tac`LENGTH bc0 + 18` >>
      rev_full_simp_tac(srw_ss())[] >>
      simp[ImplodeCode_def,EL_APPEND1,EL_APPEND2] >>
      simp[TAKE_APPEND1,TAKE_APPEND2] >>
      REWRITE_TAC[FILTER_APPEND] >>
      EVAL_TAC) >>
    match_mp_tac RTC_SUBSET >>
    ...
    EVAL_TAC >> simp[]) >>
  srw_tac[][] >>
  Cases_on`Cimplode vl`>>full_simp_tac(srw_ss())[]>>srw_tac[][]>>
  simp[PULL_EXISTS] >>
  srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
  `bc_fetch bs = SOME (Stack (Load 1))` by (
    match_mp_tac bc_fetch_next_addr >>
  ...
  `cons_tag ≠ nil_tag` by EVAL_TAC >> simp[] >> pop_assum kall_tac >>
  qho_match_abbrev_tac`bc_next^* bs1 bs2` >>
  `bc_fetch bs1 = SOME (JumpIf (Lab (ImplodeLab+2)))` by (next_addr_tac`5`) >>
  srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
  simp[bc_eval1_thm,bc_eval1_def,Abbr`bs1`,PULL_EXISTS] >>
  CONV_TAC(RESORT_EXISTS_CONV(sort_vars["n"])) >>
  qexists_tac`next_addr bs.inst_length (bc0 ++ DROP 2 (TAKE 21 ImplodeCode))` >>
  ...
  pop_assum kall_tac >>
  qho_match_abbrev_tac`bc_next^* bs1 bs2` >>
  `bc_fetch bs1 = SOME (Jump (Lab (ImplodeLab + 1)))` by (next_addr_tac`19`) >>
  srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
  simp[bc_eval1_thm,bc_eval1_def,Abbr`bs1`,PULL_EXISTS] >>
  pop_assum kall_tac >>
  CONV_TAC(RESORT_EXISTS_CONV(sort_vars["n"])) >>
  ...
  simp[bc_state_component_equality,Abbr`bs2`,arithmeticTheory.ADD1] >>
  conj_tac >- intLib.COOPER_TAC >>
  full_simp_tac(srw_ss())[Cv_bv_cases_lit])

val ImplodeCode_correct = prove(
  ``∀bce. contains_primitives bce ⇒
      ∀bs bcr vl st bvl ls pp.
      bs.code = bce ++ bcr ∧
      bc_fetch bs = SOME (Call (Lab ImplodeLab)) ∧
      bs.stack = bvl::st ∧
      Cv_bv pp vl bvl ∧
      Cimplode vl = SOME ls
      ⇒
      let bs' = bump_pc bs with stack := Block string_tag (MAP (Number o $& o ORD) ls) :: st in
      bc_next^* bs bs'``,
  simp[] >> srw_tac[][] >>
  srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
  simp[bc_eval1_thm,bc_eval1_def] >>
  full_simp_tac(srw_ss())[contains_primitives_def,bc_find_loc_def] >>
  simp[PULL_EXISTS] >>
  qexists_tac`next_addr bs.inst_length (TAKE (LENGTH bc0 + LENGTH VfromListCode) bs.code)` >>
  simp[RIGHT_EXISTS_AND_THM] >>
  conj_tac >- (
    BasicProvers.VAR_EQ_TAC >>
    match_mp_tac bc_find_loc_aux_append_code >>
    match_mp_tac bc_find_loc_aux_ALL_DISTINCT >>
    qexists_tac`LENGTH bc0 + LENGTH VfromListCode` >> rev_full_simp_tac(srw_ss())[] >>
    simp[EL_APPEND1,EL_APPEND2,ImplodeCode_def] >>
    simp[TAKE_APPEND1,TAKE_APPEND2]) >>
  ...
  `bc_fetch bs1 = SOME (Stack (PushInt 0))` by (
    match_mp_tac bc_fetch_next_addr >>
    simp[Abbr`bs1`] >>
    qexists_tac`bc0 ++ VfromListCode ++ [HD ImplodeCode]` >>
    simp[ImplodeCode_def,SUM_APPEND,FILTER_APPEND] ) >>
  srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
  simp[bc_eval1_thm,bc_eval1_def] >>
  simp[bc_eval_stack_def,bump_pc_def] >>
  simp[Abbr`bs1`] >>
  qho_match_abbrev_tac`bc_next^* bs1 bs2` >>
  qspec_then`vl`mp_tac ImplodeCode_aux_correct >> simp[] >>
  `bs1.code = bs.code` by simp[Abbr`bs1`] >>
  disch_then(qspec_then`bs1`mp_tac) >> simp[] >>
  disch_then(qspec_then`bc0++VfromListCode++(TAKE 2 ImplodeCode)`mp_tac) >> simp[] >>
  simp[Once ImplodeCode_def] >>
  simp[Once ImplodeCode_def] >>
  simp[Once ImplodeCode_def] >> rev_full_simp_tac(srw_ss())[] >>
  simp[Abbr`bs1`] >>
  disch_then(qspec_then`pp`mp_tac) >> simp[] >>
  discharge_hyps >- (
    rator_x_assum`ALL_DISTINCT`mp_tac >>
  ...
  qunabbrev_tac`bs1'`>>full_simp_tac(srw_ss())[] >> pop_assum kall_tac >>
  HINT_EXISTS_TAC >> simp[Abbr`bs1`] >>
  `bc_fetch bs3 = SOME (Stack (Cons string_tag))` by (
    match_mp_tac bc_fetch_next_addr >>
    simp[Abbr`bs3`] >>
    qexists_tac`bc0 ++ VfromListCode ++ (TAKE 22 ImplodeCode)` >>
    simp[ImplodeCode_def] >>
    REWRITE_TAC[FILTER_APPEND,SUM_APPEND,MAP_APPEND] >>
    EVAL_TAC >> simp[] ) >>
  srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
  simp[bc_eval1_thm,bc_eval1_def,Abbr`bs3`,bc_eval_stack_def,bump_pc_def] >>
  full_simp_tac(srw_ss())[] >> imp_res_tac LIST_REL_LENGTH >> simp[] >>
  simp[TAKE_APPEND1,TAKE_LENGTH_ID_rwt,DROP_APPEND1,DROP_LENGTH_NIL_rwt] >>
  qho_match_abbrev_tac`bc_next^* bs1 bs2` >>
  `bc_fetch bs1 = SOME Return` by (
    match_mp_tac bc_fetch_next_addr >>
    simp[Abbr`bs1`] >>
    qexists_tac`bc0 ++ VfromListCode ++ (FRONT ImplodeCode)` >>
    simp[ImplodeCode_def] >>
    REWRITE_TAC[FILTER_APPEND,SUM_APPEND,MAP_APPEND] >>
    EVAL_TAC >> simp[] ) >>
  match_mp_tac RTC_SUBSET >>
...
val ExplodeCode_aux_correct = prove(
  ...
  ho_match_mp_tac SNOC_INDUCT >> simp[] >> srw_tac[][] >- (
    qexists_tac`btl` >> srw_tac[][] >>
    qmatch_abbrev_tac`bc_next^* bs1 bs2` >>
    next_addr_tac"6" >>
    srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
...
val ExplodeCode_correct = prove(
  ...
  conj_tac >- (
    ...
    match_mp_tac bc_find_loc_aux_append_code >>
    match_mp_tac bc_find_loc_aux_ALL_DISTINCT >>
    qexists_tac`LENGTH bc0 + LENGTH VfromListCode + LENGTH ImplodeCode` >> rev_full_simp_tac(srw_ss())[] >>
  ...
  discharge_hyps >- (
    conj_tac >- (
      ...
      EVAL_TAC >> srw_tac[][] >> res_tac >> full_simp_tac(srw_ss())[] >>
      metis_tac[] ) >>
    simp[Once ExplodeCode_def] >>
    simp[FILTER_APPEND,SUM_APPEND] ) >>
  strip_tac >>
  ...
  full_simp_tac(srw_ss())[Abbr`bs1'`] >> pop_assum kall_tac >>
  srw_tac[DNF_ss][Once RTC_CASES_RTC_TWICE] >>
  first_assum(match_exists_tac o concl) >>
...
val prim1_to_bc_thm = store_thm("prim1_to_bc_thm",
  ...
  simp[] >> srw_tac[][] >>
  Cases_on`op = CChr` >- (
    full_simp_tac(srw_ss())[] >> Cases_on`v1`>>full_simp_tac(srw_ss())[] >>
    Cases_on`l`>>full_simp_tac(srw_ss())[] >>
    BasicProvers.EVERY_CASE_TAC >> full_simp_tac(srw_ss())[] >> srw_tac[][] >>
    full_simp_tac(srw_ss())[Cv_bv_cases_lit] >> srw_tac[][] >>
    simp[integerTheory.INT_ABS] >>
    `Num i < 256` by intLib.COOPER_TAC >>
    simp[stringTheory.ORD_CHR_RWT] >>
    `0 <= i` by intLib.COOPER_TAC >>
    full_simp_tac(srw_ss())[GSYM integerTheory.INT_OF_NUM] >>
  ...
  Cases_on`op = COrd` >- (
    ...
    `bs' = bs with code := bce` by (
      simp[Abbr`bs'`,bc_state_component_equality] ) >>
    srw_tac[][Abbr`bs'`] ) >>
  Cases_on`∃n. op = CProj n` >> full_simp_tac(srw_ss())[] >- (
    Cases_on`v1`>>full_simp_tac(srw_ss())[] >> srw_tac[][] >>
    BasicProvers.EVERY_CASE_TAC >> full_simp_tac(srw_ss())[] >> srw_tac[][] >>
    full_simp_tac(srw_ss())[Cv_bv_cases_conv] >> srw_tac[][] >>
    ...
    full_simp_tac(srw_ss())[el_check_def] >> BasicProvers.EVERY_CASE_TAC >> full_simp_tac(srw_ss())[] >>
    rev_full_simp_tac(srw_ss())[EVERY2_EVERY,EVERY_MEM] >> full_simp_tac(srw_ss())[MEM_ZIP,PULL_EXISTS] >> srw_tac[][] >>
    srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
    simp[bc_eval1_thm,bc_eval1_def,bc_eval_stack_def,bump_pc_def] >>
    srw_tac[DNF_ss][Once RTC_CASES1] >> disj1_tac >>
  ...
  Cases_on`op = CVfromList` >- (
    ...
    Cases_on`CvFromList v1`>>full_simp_tac(srw_ss())[]>>srw_tac[][]>>
  ...
  Cases_on`op = CImplode` >- (
    full_simp_tac(srw_ss())[] >> srw_tac[][] >>
    Cases_on`Cimplode v1`>>full_simp_tac(srw_ss())[]>>srw_tac[][]>>
  ...
  full_simp_tac(srw_ss())[LET_THM] >- (
    ...
    simp[Once Cv_bv_cases] >>
    `n = LENGTH rd.sm` by full_simp_tac(srw_ss())[s_refs_def] >>
    ...
    conj_tac >- (
      ...
      conj_tac >- (
        ...
        full_simp_tac(srw_ss())[FEVERY_DEF,DOMSUB_NOT_IN_DOM,UNCURRY] >>
        gen_tac >> strip_tac >>
        conj_asm1_tac >- (full_simp_tac(srw_ss())[FLOOKUP_DEF] >> metis_tac[]) >>
        simp[FLOOKUP_UPDATE] >>
        res_tac >> simp[] >>
        match_mp_tac(MP_CANON(GEN_ALL(CONJUNCT1(SPEC_ALL Cv_bv_SUBMAP)))) >>
      ...
      conj_tac >- (
        conj_tac >- (
          ...
          qexists_tac`mk_pp rd (bs with code := bce)` >>
          simp[] >>
          full_simp_tac(srw_ss())[good_rd_def,FEVERY_DEF,UNCURRY,FLOOKUP_DEF] >>
          metis_tac[] ) >>
        REWRITE_TAC[ONE] >>
        simp[REPLICATE] >>
        match_mp_tac(MP_CANON(GEN_ALL(CONJUNCT1(SPEC_ALL Cv_bv_SUBMAP)))) >>
      ...
      match_mp_tac (MP_CANON(GEN_ALL(CONJUNCT1(SPEC_ALL Cv_bv_SUBMAP)))) >>
      HINT_EXISTS_TAC >> simp[] >>
      full_simp_tac(srw_ss())[good_rd_def,FEVERY_DEF,UNCURRY,FLOOKUP_DEF] >> metis_tac[] ) >>
    simp[SUBMAP_DEF,DRESTRICT_DEF] >>
    srw_tac[][] >> srw_tac[][] >> full_simp_tac(srw_ss())[IN_FRANGE,DOMSUB_FAPPLY_THM] >> srw_tac[][] >> PROVE_TAC[])
  >- (
    simp[Once RTC_CASES1] >> srw_tac[DNF_ss][] >> disj2_tac >>
    simp[bc_eval_stack_def] >>
    simp[Once RTC_CASES1] >> srw_tac[DNF_ss][] >> disj1_tac >>
    ...
    Cases_on`v1`>>full_simp_tac(srw_ss())[] >>
    BasicProvers.EVERY_CASE_TAC>>full_simp_tac(srw_ss())[] >>
    full_simp_tac(srw_ss())[Q.SPEC`CLoc n`(CONJUNCT1(SPEC_ALL(Cv_bv_cases)))] >>
    srw_tac[][] >>
    simp[bc_eval1_thm,bc_eval1_def,Abbr`bs2`] >>
    `∃v. FLOOKUP bs.refs p = SOME (ValueArray [v]) ∧ Cv_bv (mk_pp rd (bs with code := bce)) a v` by (
      full_simp_tac(srw_ss())[s_refs_def,el_check_def,EVERY_MEM,FLOOKUP_DEF] >>
      first_x_assum(qspec_then`p`mp_tac) >>
      discharge_hyps >- metis_tac[MEM_EL] >> srw_tac[][] >>
      full_simp_tac(srw_ss())[LIST_REL_EL_EQN] >>
      rpt(first_x_assum(qspec_then`n`mp_tac)) >>
  ...
  >- ((* The IsBlock instruction *)
    ...
    Cases_on `v1` >> full_simp_tac(srw_ss())[] >>
    srw_tac[][] >>
    Cases_on`l`>>full_simp_tac(srw_ss())[Once Cv_bv_cases] >>
    simp[bc_eval_stack_def,bc_state_component_equality] >>
    full_simp_tac(srw_ss())[s_refs_def] >> simp[SUM_APPEND,FILTER_APPEND] >>
  ...
  >- ((*Length*)
    ...
    full_simp_tac(srw_ss())[Once Cv_bv_cases] >- (
      ...
      match_mp_tac s_refs_with_irr >>
      HINT_EXISTS_TAC >> simp[] ) >>
    `∃vs. FLOOKUP bs.refs p = SOME (ValueArray vs) ∧ LENGTH vs = LENGTH l` by (
      full_simp_tac(srw_ss())[s_refs_def,el_check_def,EVERY_MEM,FLOOKUP_DEF] >>
      first_x_assum(qspec_then`p`mp_tac) >>
      discharge_hyps >- metis_tac[MEM_EL] >> srw_tac[][] >>
      full_simp_tac(srw_ss())[LIST_REL_EL_EQN] >>
      rpt(first_x_assum(qspec_then`n`mp_tac)) >>
      simp[sv_refv_rel_cases,EL_MAP] >>
      srw_tac[][] >> srw_tac[][] >> full_simp_tac(srw_ss())[LIST_REL_EL_EQN]) >>
    simp[] >>
    simp[Once RTC_CASES1] >> srw_tac[DNF_ss][] >> disj1_tac >>
    simp[bc_state_component_equality] >>
    qexists_tac`rd.sm` >>
  ...
  >- ((*LengthByte*)
    ...
    HINT_EXISTS_TAC >> simp[] )
  >- ((*LengthBlock*)
    Cases_on`v1`>>full_simp_tac(srw_ss())[Cv_bv_cases_conv,Cv_bv_cases_vectorv]>>
    simp[bc_eval_stack_def] >>
    srw_tac[DNF_ss][Once RTC_CASES1] >> disj1_tac >>
    ...
    HINT_EXISTS_TAC >> simp[] )
  >- ((*LengthBlock (string) *)
    Cases_on`v1`>>full_simp_tac(srw_ss())[Cv_bv_cases_lit]>>full_simp_tac(srw_ss())[]>>srw_tac[][]>>
    simp[bc_eval_stack_def] >>
    srw_tac[DNF_ss][Once RTC_CASES1] >> disj1_tac >>
    simp[bc_state_component_equality,FILTER_APPEND,SUM_APPEND] >>
    simp[Cv_bv_cases_lit] >>
  ...
  >- ( (*TagEq*)
    simp[Once RTC_CASES1] >> srw_tac[DNF_ss][] >> disj1_tac >>
    Cases_on`v1`>>full_simp_tac(srw_ss())[]>>srw_tac[][]>>
    full_simp_tac(srw_ss())[Q.SPEC`CConv X Y`(CONJUNCT1(SPEC_ALL(Cv_bv_cases)))] >>
    srw_tac[][bc_eval_stack_def] >> simp[bc_state_component_equality] >>
    simp[Once Cv_bv_cases] >>
    simp[FILTER_APPEND,SUM_APPEND] >>
    qexists_tac`rd.sm`>>simp[CBoolv_def] >>
    full_simp_tac(srw_ss())[s_refs_def,good_rd_def] >>
    srw_tac[][bool_to_tag_def,conLangTheory.true_tag_def,conLangTheory.false_tag_def])
  >- ( (*Gupdate*)
    BasicProvers.EVERY_CASE_TAC >> full_simp_tac(srw_ss())[] >> srw_tac[][] >>
    simp[Once RTC_CASES1] >> srw_tac[DNF_ss][] >> disj2_tac >>
    Q.PAT_ABBREV_TAC`bs2:bc_state = X Y` >>
...
val FOLDL_emit_ceref_thm = store_thm("FOLDL_emit_ceref_thm",
  ...
  Induct >- (
    srw_tac[][Once SWAP_REVERSE,LET_THM] >>
    rpt (first_x_assum (mp_tac o SYM)) >> srw_tac[][]) >>
  qx_gen_tac`e` >> srw_tac[][] >>
  qmatch_assum_abbrev_tac`FOLDL (emit_ceref z) (sz + 1, s1) ls = X` >>
  first_x_assum(qspecl_then[`z`,`sz+1`,`s1`]mp_tac) >>
  simp[Abbr`X`,Abbr`s1`] >>
  disch_then(Q.X_CHOOSE_THEN`bc`strip_assume_tac) >> srw_tac[][] >>
...
val FOLDL_emit_ceenv_thm = store_thm("FOLDL_emit_ceenv_thm",
  ...
  disch_then(Q.X_CHOOSE_THEN`bcv`strip_assume_tac) >> srw_tac[][] >>
  simp[Once SWAP_REVERSE] >>
  simp[EVERY_REVERSE] >>
  conj_tac >- (
    match_mp_tac code_labels_ok_append >> simp[] ) >> srw_tac[][] >>
  `ell = EL e env0` by (
    simp[Abbr`ell`,el_check_def] ) >> srw_tac[][] >>
  Cases_on`lookup_ct (z-j) (DROP j bs.stack) (DRESTRICT bs.refs cls) bs.globals (EL e env0)`>>full_simp_tac(srw_ss())[] >>
  ...
  qmatch_assum_abbrev_tac`bc_next^* bs1 bs2'` >>
  `bs2' = bs2` by (
    srw_tac[][Abbr`bs2'`,Abbr`bs2`,Abbr`bs1`] >>
...
val cons_closure_thm = store_thm("cons_closure_thm",
  ``∀env0 sz nk s k refs envs.
      ...
      ∀bs bc0 bc1 cls.
        ...
        bc_next^* bs (bs with <| stack :=
          ...
          k bs.stack
                               ; pc := next_addr bs.inst_length (bc0 ++ code) |>)``,
  simp[cons_closure_def,UNCURRY,FOLDL_emit_append_out] >> rpt gen_tac >>
  ...
  full_simp_tac(srw_ss())[Abbr`s0`,Once SWAP_REVERSE] >>
  conj_tac >- (
    match_mp_tac code_labels_ok_cons >> simp[] >>
  ...
  match_mp_tac(SIMP_RULE std_ss [transitive_def] RTC_TRANSITIVE) >>
  qpat_assum`bs.code = X` mp_tac >>
  Q.PAT_ABBREV_TAC`bc2a = PushAnyInt X` >>
  Q.PAT_ABBREV_TAC`bc2:bc_inst list = [yy;y;zz;z]` >> strip_tac >>
  first_x_assum(qspecl_then[`bs3`,`bc0 ++ [Stack (Load k)] ++ bc`,`bc2a ++ bc2 ++ bc1'`,`cls`,`LENGTH refs + 1 + 2 * nk`]mp_tac) >>
  discharge_hyps >- (
    simp[Abbr`bs3`,SUM_APPEND,FILTER_APPEND] >>
    full_simp_tac(srw_ss())[EVERY_MEM] >> simp[DROP_APPEND2] ) >>
  ...
  qmatch_assum_abbrev_tac`Abbrev(bc2a = PushAnyInt i)` >>
  qspecl_then[`i`,`bs3`,`bc0++[Stack(Load k)]++bc++bc1`]mp_tac PushAnyInt_thm >>
  simp[Abbr`bs3`] >>
  strip_tac >>
  simp[Once RTC_CASES_RTC_TWICE] >>
  first_assum(match_exists_tac o concl) >> simp[] >>
  ...
  `bc_fetch bs3 = SOME (Stack (Cons 0))` by (
    match_mp_tac bc_fetch_next_addr >>
    srw_tac[][Abbr`bs3`] >>
    qexists_tac`bc0 ++ [Stack (Load k)] ++ bc ++ bc1 ++ bc2a` >>
    lrw[SUM_APPEND,FILTER_APPEND,Abbr`bc2a`] ) >>
  simp[Once RTC_CASES1] >> disj2_tac >>
  simp[bc_eval1_thm,bc_eval1_def] >>
  ...
  conj_tac >- (
    simp[bc_eval_stack_def,Abbr`bs3`,ADD1,Abbr`i`] >>
  ...
  `bc_fetch bs3 = SOME (Stack (PushInt 2))` by (
    ...
    qexists_tac`ls ++ bc1 ++ bc2a ++ (TAKE 1 ls2)` >>
  ...
  `bc_fetch bs3 = SOME (Stack (Cons closure_tag))` by (
    ...
    Q.PAT_ABBREV_TAC`ls = X ++ bc` >>
    Q.PAT_ABBREV_TAC`ls2:bc_inst list = X::Y` >>
    qexists_tac`ls ++ bc1 ++ bc2a ++ (TAKE 2 ls2)` >>
    lrw[SUM_APPEND,FILTER_APPEND,ADD1,Abbr`ls`,Abbr`ls2`] ) >>
  simp[Once RTC_CASES1] >> disj2_tac >>
  simp[bc_eval1_thm,bc_eval1_def] >>
  simp[bc_eval_stack_def,Abbr`bs3`,bump_pc_def] >>
  ...
  `bc_fetch bs3 = SOME (Stack (Store k))` by (
    ...
    Q.PAT_ABBREV_TAC`ls = X ++ bc` >>
    Q.PAT_ABBREV_TAC`ls2:bc_inst list = X::Y` >>
    qexists_tac`ls ++ bc1 ++ bc2a ++ (TAKE 3 ls2)` >>
    lrw[SUM_APPEND,FILTER_APPEND,ADD1,Abbr`ls`,Abbr`ls2`] ) >>
  simp[Once RTC_CASES1] >> disj2_tac >>
  simp[bc_eval1_thm,bc_eval1_def] >>
  simp[bc_eval_stack_def,Abbr`bs3`,bump_pc_def] >>
...
val num_fold_make_ref_thm = store_thm("num_fold_make_ref_thm",
  ``∀x nz s.
    ...
    (bs with <| stack := MAP RefPtr ps ++ bs.stack
              ...
              ; pc := next_addr bs.inst_length (bc0 ++ code) |>)``,
  gen_tac >> Induct >- (
    srw_tac[][Once num_fold_def,Once SWAP_REVERSE] >> srw_tac[][] >>
    qexists_tac`[]` >> srw_tac[][FUPDATE_LIST_THM] >>
    rpt (pop_assum (mp_tac o SYM)) >> srw_tac[][] ) >>
  simp[Once num_fold_def] >> gen_tac >>
  Q.PAT_ABBREV_TAC`s' = s with out := X` >>
  first_x_assum(qspec_then`s'`mp_tac) >>
  simp[] >> srw_tac[][] >>
  full_simp_tac(srw_ss())[Abbr`s'`,Once SWAP_REVERSE] >>
  conj_tac >- (
    rpt(match_mp_tac code_labels_ok_cons >> simp[]) ) >>
  srw_tac[][] >>
  simp[Once RTC_CASES1] >>
  ...
  `bc_fetch bs = SOME (Stack (PushInt x))` by (
    match_mp_tac bc_fetch_next_addr >>
    qexists_tac`bc0`>>srw_tac[][] ) >>
  srw_tac[][bc_eval1_thm,bc_eval1_def,bc_eval_stack_def,bump_pc_def] >>
  simp[Once RTC_CASES1] >>
  fsrw_tac[DNF_ss][] >> disj2_tac >>
  ...
  `bc_fetch bs1 = SOME (Stack (PushInt 1))` by (
    match_mp_tac bc_fetch_next_addr >>
    qexists_tac`bc0++[Stack (PushInt x)]`>>srw_tac[][Abbr`bs1`] >>
    simp[SUM_APPEND,FILTER_APPEND]) >>
  srw_tac[][bc_eval1_thm,bc_eval1_def,bc_eval_stack_def,bump_pc_def,Abbr`bs1`,LET_THM,Abbr`P`] >>
  qho_match_abbrev_tac`∃ps. P0 ps ∧ bc_next^* bs1 (bs2 ps)` >>
  `bc_fetch bs1 = SOME Ref` by (
    match_mp_tac bc_fetch_next_addr >>
    qexists_tac`bc0++[Stack (PushInt x);Stack (PushInt 1)]`>>srw_tac[][Abbr`bs1`] >>
    simp[SUM_APPEND,FILTER_APPEND]) >>
  srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
  srw_tac[][bc_eval1_thm,bc_eval1_def,bc_eval_stack_def,bump_pc_def,Abbr`bs1`,LET_THM] >>
  qho_match_abbrev_tac`∃ps. P0 ps ∧ bc_next^* bs1 (bs2 ps)` >>
  ...
  full_simp_tac(srw_ss())[Abbr`P0`] >>
  simp[Abbr`pps`,ALL_DISTINCT_APPEND] >> srw_tac[][] >> full_simp_tac(srw_ss())[] >>
 ...
 ``∀(defs:def list) s ls s' ls'.
   ...
   ∀bs bc0 bc1.
     ...
     bc_next^* bs (bs with <| stack :=  (REVERSE (MAP (CodePtr o THE o bc_find_loc bs o Lab o FST o THE o FST) defs))++bs.stack
                            ; pc := next_addr bs.inst_length (bc0 ++ code) |>)``,
  Induct >- (
    srw_tac[][Once SWAP_REVERSE] >>
    pop_assum(assume_tac o SYM) >> full_simp_tac(srw_ss())[] ) >>
  qx_gen_tac`def` >>
  PairCases_on`def` >>
  Cases_on`def0`>> TRY(PairCases_on`x`)>>srw_tac[][push_lab_def] >- (
    full_simp_tac(srw_ss())[] >> res_tac >> full_simp_tac(srw_ss())[] ) >>
  qmatch_assum_abbrev_tac`FOLDL push_lab (ss,lss) defs = (s',ls')` >>
  first_x_assum(qspecl_then[`ss`,`lss`,`s'`,`ls'`]mp_tac) >> simp[] >>
  strip_tac >> full_simp_tac(srw_ss())[Abbr`ss`,Once SWAP_REVERSE,Abbr`lss`] >>
  conj_tac >- ( srw_tac[][] >> full_simp_tac(srw_ss())[uses_label_thm] ) >>
  rpt gen_tac >> strip_tac >>
  ...
  srw_tac[][bc_eval1_thm,bc_eval1_def,bump_pc_def] >>
  Cases_on`bc_find_loc bs (Lab x0)`>>full_simp_tac(srw_ss())[] >>
  simp[CONJ_ASSOC] >>
  ...
  discharge_hyps >- (
    ...
    rev_full_simp_tac(srw_ss())[EVERY_MEM,bc_find_loc_def] ) >>
...
val FOLDL_cons_closure_thm = store_thm("FOLDL_cons_closure_thm",
  ``∀ecs env0 sz nk s k.
    ...
    (bs with <| stack := TAKE k bs.stack ++ bvs ++ (DROP nk bs.stack)
              ; pc := next_addr bs.inst_length (bc0 ++ code) |>)``,
  Induct >- (
    simp[Once SWAP_REVERSE,LENGTH_NIL_SYM,LENGTH_NIL] >> srw_tac[][] >>
  ...
  PairCases_on`p`>>full_simp_tac(srw_ss())[Once SWAP_REVERSE,ADD1] >> simp[] >>
  conj_tac >- (
    match_mp_tac code_labels_ok_append >> simp[] ) >>
  rpt gen_tac >> strip_tac >>
...
val num_fold_update_refptr_thm = store_thm("num_fold_update_refptr_thm",
  ``∀nz nk s k.
    ...
    (bs with <| refs := bs.refs |++ ZIP (DROP k rs,MAP (λv. ValueArray [v]) (DROP k vs))
              ; pc := next_addr bs.inst_length (bc0 ++ code)|>)``,
  Induct >- (
    srw_tac[][Once num_fold_def,Once SWAP_REVERSE,LENGTH_NIL,FUPDATE_LIST_THM] >>
    srw_tac[][] >> fsrw_tac[ARITH_ss][FUPDATE_LIST_THM] >>
    metis_tac[DROP_LENGTH_NIL,ZIP,FUPDATE_LIST_THM,with_same_pc,with_same_refs,RTC_CASES1,MAP2,MAP]) >>
  srw_tac[][Once num_fold_def,update_refptr_def] >>
  ...
  `bc_fetch bs = SOME x1` by (
    match_mp_tac bc_fetch_next_addr >>
    qexists_tac`bc0`>>srw_tac[][Abbr`x1`,Abbr`ls`] ) >>
...
val num_fold_store_thm = store_thm("num_fold_store_thm",
  ``∀nz k s. let s' = num_fold (λs. s with out := Stack (Store k)::s.out) s nz in
    ∃code.
      ...
      (bs with <| stack := (DROP nz vs) ++ (TAKE nz vs) ++ (DROP nz ws) ++ st
                ; pc := next_addr bs.inst_length (bc0 ++ code) |>)``,
  Induct >- (
    srw_tac[][Once num_fold_def,Once SWAP_REVERSE] >>
    lrw[] >>
    metis_tac[RTC_CASES1,with_same_stack,with_same_pc] ) >>
  simp[Once num_fold_def] >> srw_tac[][] >>
  ...
  `bc_fetch bs = SOME (Stack (Store k))`  by (
    match_mp_tac bc_fetch_next_addr >>
    qexists_tac`bc0` >> srw_tac[][] ) >>
  srw_tac[][Once RTC_CASES1] >> disj2_tac >>
  srw_tac[][bc_eval1_thm,bc_eval1_def] >>
...
val compile_closures_thm = store_thm("compile_closures_thm",
  ...
  simp[Once SWAP_REVERSE] >>
  conj_tac >- (
    strip_tac >> full_simp_tac(srw_ss())[EVERY_REVERSE,MAP_REVERSE,code_labels_ok_def,uses_label_thm] >>
    metis_tac[] ) >>
  rpt gen_tac >> strip_tac >>
  ...
  discharge_hyps >- (
    ...
    full_simp_tac(srw_ss())[EVERY_MEM,MEM_MAP,GSYM LEFT_FORALL_IMP_THM] >>
    ...
    qsuff_tac`lc = lookup_ct sz bs.stack (DRESTRICT bs.refs cls) bs.globals (EL n env)`>-srw_tac[][] >>
    simp[Abbr`lc`] >>
    ...
    `¬MEM p rs` by (
      ...
      simp[el_check_def,FLOOKUP_DEF,DRESTRICT_DEF] >> srw_tac[][] >>
      metis_tac[] ) >>
    simp[] >> srw_tac[][] >>
    match_mp_tac EQ_SYM >>
  ...
  conj_asm1_tac >- (
    ...
    reverse conj_tac >- (
      srw_tac[][] >>
      full_simp_tac(srw_ss())[EVERY_MEM] >>
      res_tac >> full_simp_tac(srw_ss())[UNCURRY] >>
      ntac 2 (first_x_assum(qspec_then`n`mp_tac)) >>
      simp[el_check_def] >> srw_tac[][] ) >>
    simp[] >> srw_tac[][] >> full_simp_tac(srw_ss())[FLOOKUP_DEF] >>
    simp[DRESTRICT_DEF,FDOM_FUPDATE_LIST,MEM_MAP,EXISTS_PROD] >>
    `¬MEM p rs` by (
      full_simp_tac(srw_ss())[EVERY_MEM,UNCURRY] >>
      spose_not_then strip_assume_tac >>
      res_tac >> rev_full_simp_tac(srw_ss())[el_check_def,FLOOKUP_DEF,DRESTRICT_DEF] ) >>
    srw_tac[][] >>
    match_mp_tac EQ_SYM >>
    match_mp_tac FUPDATE_LIST_APPLY_NOT_MEM_matchable >>
    simp[MAP_REVERSE,MAP_MAP_o,combinTheory.o_DEF] ) >>
  ...
  full_simp_tac(srw_ss())[MEM_EL] >> PROVE_TAC[] )
...
val stackshiftaux_thm = store_thm("stackshiftaux_thm",
  ...
  Cases_on`n=0`>>simp[LENGTH_NIL]>-(
    srw_tac[][] >> simp[Once RTC_CASES1] >> disj1_tac >>
    simp[bc_state_component_equality] ) >>
...
val GENLIST_Store_thm = store_thm("GENLIST_Store_thm",
  ...
  Induct >- (
    ...
    srw_tac[][Once RTC_CASES1] >> disj1_tac >>
    simp[bc_state_component_equality] ) >>
  simp[GENLIST_CONS] >> srw_tac[][] >>
  Q.PAT_ABBREV_TAC`f = λi:num. Store k` >>
  `f o SUC = f` by simp[Abbr`f`,FUN_EQ_THM] >> full_simp_tac(srw_ss())[] >>
  `bc_fetch bs = SOME (Stack (Store k))` by (
    match_mp_tac bc_fetch_next_addr >>
    qexists_tac`bc0` >> simp[] ) >>
  simp[Once RTC_CASES1] >> disj2_tac >>
...
val stackshift_thm = store_thm("stackshift_thm",
  ...
  Cases_on`j=1`>>simp[] >- (
    srw_tac[][] >>
    Cases_on`xs`>>full_simp_tac(srw_ss())[LENGTH_NIL] >>
    Cases_on`ys`>>full_simp_tac(srw_ss())[] >>
...
val pushret_append_out = store_thm("pushret_append_out",
  ...
  Cases >> srw_tac[][pushret_def] >> rpt(match_mp_tac code_labels_ok_cons >> simp[]))

val pushret_next_label = store_thm("pushret_next_label",
  ``∀t s. (pushret t s).next_label = s.next_label``,
  Cases >> srw_tac[][pushret_def])
val _ = export_rewrites["pushret_next_label"]

val tac =
  srw_tac[][compile_def] >>
  SIMPLE_QUANT_ABBREV_TAC[select_fun_constant``pushret``2"s"] >>
  qspecl_then[`t`,`s`]mp_tac(pushret_append_out) >> srw_tac[][] >> full_simp_tac(srw_ss())[Abbr`s`] >>
  full_simp_tac(srw_ss())[FILTER_APPEND,GSYM FILTER_EQ_NIL,combinTheory.o_DEF] >>
  full_simp_tac(srw_ss())[code_labels_ok_def,uses_label_thm]

val tac2 =
  srw_tac[][compile_def,LET_THM,UNCURRY] >>
  SIMPLE_QUANT_ABBREV_TAC[select_fun_constant``pushret``2"s"] >>
  qspecl_then[`t`,`s`]mp_tac(pushret_append_out) >> srw_tac[][] >> full_simp_tac(srw_ss())[Abbr`s`] >>
  full_simp_tac(srw_ss())[FILTER_APPEND,GSYM FILTER_EQ_NIL,combinTheory.o_DEF] >>
  full_simp_tac(srw_ss())[code_labels_ok_def,uses_label_thm] >>
...
val compile_append_out = store_thm("compile_append_out",
  ...
  strip_tac >- (
    simp[compile_def] >> srw_tac[][] >> srw_tac[][] >> full_simp_tac(srw_ss())[uses_label_thm]) >>
  strip_tac >- (
    simp[compile_def] >> srw_tac[][] >>
    SIMPLE_QUANT_ABBREV_TAC[select_fun_constant``pushret``2"s"] >>
    qspecl_then[`t`,`s`]mp_tac(pushret_append_out) >> srw_tac[][] >> full_simp_tac(srw_ss())[Abbr`s`] >>
    simp[] >>
    ...
    simp[CONJ_ASSOC] >>
    conj_tac >- (
      srw_tac[][] >> full_simp_tac(srw_ss())[] >> spose_not_then strip_assume_tac >> res_tac >> fsrw_tac[ARITH_ss][] >>
      `FILTER is_Label bc'' = []` by (simp[FILTER_EQ_NIL,EVERY_MEM,is_Label_rwt] >> metis_tac[]) >>
      full_simp_tac(srw_ss())[]) >>
    full_simp_tac(srw_ss())[uses_label_thm,code_labels_ok_def] >>
    srw_tac[][] >> metis_tac[] ) >>
  strip_tac >- (
    srw_tac[][compile_def,FOLDL_emit_append_out] >>
    SIMPLE_QUANT_ABBREV_TAC[select_fun_constant``pushret``2"s"] >>
    qspecl_then[`t`,`s`]mp_tac(pushret_append_out) >> srw_tac[][] >> full_simp_tac(srw_ss())[Abbr`s`] >>
    qspec_then`i`strip_assume_tac PushAnyInt_labels >>
    FULL_SIMP_TAC std_ss [GSYM FILTER_EQ_NIL,combinTheory.o_DEF] >>
    full_simp_tac(srw_ss())[FILTER_REVERSE,FILTER_APPEND] >>
    full_simp_tac(srw_ss())[code_labels_ok_def,uses_label_thm,EXISTS_REVERSE] >>
    srw_tac[][PushAnyInt_inst_uses_label] ) >>
  strip_tac >- tac >>
  strip_tac >- (
    simp[compile_def,FOLDL_emit_append_out] >> srw_tac[][] >>
    SIMPLE_QUANT_ABBREV_TAC[select_fun_constant``pushret``2"z"] >>
    qspecl_then[`t`,`z`]mp_tac(pushret_append_out) >> srw_tac[][] >> full_simp_tac(srw_ss())[Abbr`z`] >>
    simp[FOLDL_emit_append_out,FILTER_APPEND,ALL_DISTINCT_APPEND,FILTER_REVERSE,ALL_DISTINCT_REVERSE] >>
    full_simp_tac(srw_ss())[GSYM FILTER_EQ_NIL,combinTheory.o_DEF,EVERY_MAP,EVERY_REVERSE] >>
    SIMPLE_QUANT_ABBREV_TAC[select_fun_constant``ALL_DISTINCT``1"ls"] >>
    `ls = []` by simp[Abbr`ls`,FILTER_EQ_NIL,EVERY_MAP] >>
    full_simp_tac(srw_ss())[code_labels_ok_def,uses_label_thm,EXISTS_MEM,MEM_MAP,PULL_EXISTS] >>
    `ls' = []` by (
      simp[FILTER_EQ_NIL,Abbr`ls'`,GSYM combinTheory.o_DEF] ) >>
    simp[] >>
    Q.PAT_ABBREV_TAC`i:int = X` >>
    qspec_then`i`mp_tac PushAnyInt_inst_uses_label >>
  ...
  strip_tac >- (
    ...
    SIMPLE_QUANT_ABBREV_TAC[select_fun_constant``pushret``2"s"] >>
    qspecl_then[`t`,`s`]mp_tac(pushret_append_out) >> srw_tac[][] >> full_simp_tac(srw_ss())[Abbr`s`] >>
    srw_tac[][] >> full_simp_tac(srw_ss())[GSYM FILTER_EQ_NIL,combinTheory.o_DEF] >>
    full_simp_tac(srw_ss())[FILTER_APPEND] >> srw_tac[][] >>
    full_simp_tac(srw_ss())[code_labels_ok_def,uses_label_thm]) >>
  strip_tac >- tac2 >>
  strip_tac >- (
    srw_tac[][compile_def,LET_THM,UNCURRY,FOLDL_emit_append_out] >>
    SIMPLE_QUANT_ABBREV_TAC[select_fun_constant``pushret``2"s"] >>
    qspecl_then[`t`,`s`]mp_tac(pushret_append_out) >> srw_tac[][] >> full_simp_tac(srw_ss())[Abbr`s`] >>
    full_simp_tac(srw_ss())[FILTER_APPEND,GSYM FILTER_EQ_NIL,combinTheory.o_DEF] >>
    full_simp_tac(srw_ss())[code_labels_ok_def,uses_label_thm] >>
    ...
    Q.PAT_ABBREV_TAC`i: int = X` >>
    qspec_then`i`strip_assume_tac PushAnyInt_labels >>
    qspec_then`i`strip_assume_tac PushAnyInt_inst_uses_label >>
    FULL_SIMP_TAC std_ss [GSYM FILTER_EQ_NIL,combinTheory.o_DEF] >>
  ...
  strip_tac >- (
    ...
    BasicProvers.EVERY_CASE_TAC >> srw_tac[][] >>
    srw_tac[ARITH_ss][] >>
    full_simp_tac(srw_ss())[uses_label_thm] >>
    full_simp_tac(srw_ss())[FOLDL_emit_append_out] >>
    ...
    full_simp_tac(srw_ss())[FILTER_is_Label_Stack,MEM_MAP,EXISTS_REVERSE,EXISTS_MAP] >>
    full_simp_tac(srw_ss())[EVERY_MAP,MEM_FILTER,EVERY_MEM] >>
    ...
    srw_tac[][] >> spose_not_then strip_assume_tac >> res_tac >> DECIDE_TAC ) >>
  strip_tac >- (
    ...
    reverse conj_tac >- (
      full_simp_tac(srw_ss())[code_labels_ok_def,uses_label_thm] >>
      srw_tac[][] >> metis_tac[] ) >>
    srw_tac[][] >> full_simp_tac(srw_ss())[] >> spose_not_then strip_assume_tac >> res_tac >> DECIDE_TAC ) >>
  strip_tac >- (
    ...
    full_simp_tac(srw_ss())[GSYM FILTER_EQ_NIL,combinTheory.o_DEF] >>
    full_simp_tac(srw_ss())[uses_label_thm,EXISTS_REVERSE] >>
    srw_tac[][] >> TRY(metis_tac[]) >>
    fsrw_tac[DNF_ss][free_labs_defs_MAP,MEM_MAP,MEM_FLAT,MEM_GENLIST] >>
    Cases_on`MEM (Label l) code`>>full_simp_tac(srw_ss())[]>>
  ...
  strip_tac >- (
    srw_tac[][compile_def,LET_THM,FOLDL_emit_append_out] >> full_simp_tac(srw_ss())[] >>
    SIMPLE_QUANT_ABBREV_TAC[select_fun_constant``pushret``2"a"] >>
    qspecl_then[`t`,`a`]mp_tac(pushret_append_out) >> srw_tac[][] >> full_simp_tac(srw_ss())[Abbr`a`] >>
    Cases_on`uop`>>full_simp_tac(srw_ss())[]>>
    full_simp_tac(srw_ss())[FILTER_APPEND,GSYM FILTER_EQ_NIL,combinTheory.o_DEF] >> srw_tac[][] >> full_simp_tac(srw_ss())[] >>
    full_simp_tac(srw_ss())[code_labels_ok_def,uses_label_thm] >>
    srw_tac[][] >> TRY(metis_tac[]) >>
    Q.PAT_ABBREV_TAC`i:int = X` >>
    qspec_then`i`mp_tac PushAnyInt_inst_uses_label >>
    qspec_then`i`strip_assume_tac PushAnyInt_labels >>
    qspec_then`i`strip_assume_tac PushAnyInt_inst_uses_label >>
    FULL_SIMP_TAC std_ss [GSYM FILTER_EQ_NIL,combinTheory.o_DEF,FILTER_APPEND,FILTER_REVERSE] >>
    full_simp_tac(srw_ss())[] >>
    full_simp_tac(srw_ss())[EXISTS_MEM,FILTER_EQ_NIL,EVERY_MEM,is_Label_rwt] >>
    srw_tac[][] >> full_simp_tac(srw_ss())[] >> metis_tac[]) >>
  strip_tac >- (
    srw_tac[][compile_def,LET_THM] >> full_simp_tac(srw_ss())[] >>
    SIMPLE_QUANT_ABBREV_TAC[select_fun_constant``pushret``2"s"] >>
    qspecl_then[`t`,`s`]mp_tac(pushret_append_out) >> srw_tac[][] >> full_simp_tac(srw_ss())[Abbr`s`] >>
    full_simp_tac(srw_ss())[FILTER_APPEND,GSYM FILTER_EQ_NIL,combinTheory.o_DEF] >>
    fsrw_tac[DNF_ss][FILTER_APPEND,ALL_DISTINCT_APPEND,MEM_FILTER,is_Label_rwt,EVERY_MEM,between_def,MEM_MAP] >>
  ...
  strip_tac >- (
    ...
    srw_tac[][] >> metis_tac[]) >>
  strip_tac >- (
    gen_tac >> Cases >> simp[compile_def] >> srw_tac[][] >> full_simp_tac(srw_ss())[] >>
  ...
  strip_tac >- (
    gen_tac >> Cases >> simp[compile_def] >> srw_tac[][] >> full_simp_tac(srw_ss())[] >>
    srw_tac[][] >> full_simp_tac(srw_ss())[uses_label_thm] ) >>
  strip_tac >- (
    srw_tac[][compile_def,uses_label_thm] >>
    fsrw_tac[ARITH_ss][ADD1] ) >>
  strip_tac >- srw_tac[][compile_def] >>
  srw_tac[][compile_def] >> full_simp_tac(srw_ss())[] >>
  fsrw_tac[DNF_ss][FILTER_APPEND,ALL_DISTINCT_APPEND,MEM_FILTER,is_Label_rwt,EVERY_MEM,between_def,MEM_MAP] >>
  simp[CONJ_ASSOC] >>
  reverse conj_tac >- (
...
val jmpbc_thm = store_thm("jmpbc_thm",
  ...
  Cases_on`ck`>-(
    ...
    srw_tac[][Once RTC_CASES1] >> disj2_tac >>
    srw_tac[][bc_eval1_thm] >>
    srw_tac[][bc_eval1_def,Abbr`bs2`,Abbr`cka`] >>
...
val code_for_push_return = store_thm("code_for_push_return",
  ``∀rd bs bce bc0 code s' env v renv rsz bc1 args args1 bs' blvs benv st cl cl1 ret hdl.
    ...
    srw_tac[][code_for_push_def,code_for_return_def,LET_THM] >>
    qmatch_assum_rename_tac `Cv_bv _ v bv` >>
    map_every qexists_tac [`bv`,`rf`,`rd'`,`ck`,`gv`] >>
    full_simp_tac(srw_ss())[Cenv_bs_def,s_refs_def,good_rd_def] >>
    qmatch_assum_abbrev_tac`bc_next^* bs0 bs1` >>
    srw_tac[][] >>
    match_mp_tac (SIMP_RULE std_ss [transitive_def] RTC_TRANSITIVE) >>
    first_assum (exists_tac o rand o concl) >> full_simp_tac(srw_ss())[Abbr`bs0`] >>
    qpat_assum`bs.code = X`(assume_tac o SYM)>>full_simp_tac(srw_ss())[]>>
    match_mp_tac retbc_thm >>
    pop_assum(assume_tac o SYM) >>
    qexists_tac`bc0 ++ code` >> full_simp_tac(srw_ss())[Abbr`bs1`] >>
    qexists_tac`blvs`>>full_simp_tac(srw_ss())[]>>
    qexists_tac`args`>>full_simp_tac(srw_ss())[]>>
    simp[bc_state_component_equality])
...
val code_for_return_append_code = store_thm("code_for_return_append_code",
  ...
  rpt gen_tac >> strip_tac >>
  map_every qexists_tac[`bv`,`rf`,`rd'`,`ck`,`gv`] >> full_simp_tac(srw_ss())[] >>
...
val compile_string_thm = store_thm("compile_string_thm",
  ``∀f ls code bs bc0 bc1 bs'.
      ...
      bs' = bs with <| pc := next_addr bs.inst_length (bc0 ++ code)
                     ...
                     |>
      ⇒ bc_next^* bs bs'``,
  gen_tac >> Induct >> simp[] >> srw_tac[][] >- (
    pop_assum (SUBST1_TAC o SYM) >> srw_tac[][] ) >>
...
val compile_bindings_thm = store_thm("compile_bindings_thm",
  ...
  Induct_on`m`>- (
    srw_tac[][compile_def,FUPDATE_LIST_THM]>>
    Cases_on`t`>>full_simp_tac(srw_ss())[]>>
    srw_tac[][]>>full_simp_tac(srw_ss())[]) >>
  srw_tac[][compile_def,GENLIST_CONS,combinTheory.o_DEF] >>
...
val compile_mvars_SUBMAP = store_thm("compile_mvars_SUBMAP",
  ...
  TRY (
    ...
    discharge_hyps >- metis_tac[] >> srw_tac[][] >> NO_TAC)
  >- (
    BasicProvers.CASE_TAC >>
    BasicProvers.CASE_TAC >- (
...
val prefix_exists_lemma = store_thm("prefix_exists_lemma",
  ...
  Induct >> simp[] >>
  gen_tac >> Cases >> full_simp_tac(srw_ss())[] >>
...
val compile_labels_lemma = store_thm("compile_labels_lemma",
  ...
  srw_tac[][FILTER_REVERSE,ALL_DISTINCT_REVERSE] >>
  spose_not_then strip_assume_tac >>
  res_tac >> DECIDE_TAC)
...
;val compile_val = store_thm("compile_val",
  ...
  strip_tac >- (
    ...
    simp[] >>
    full_simp_tac(srw_ss())[Cenv_bs_def] >>
    full_simp_tac(srw_ss())[s_refs_def,good_rd_def] ) >>
  strip_tac >- (
    simp[compile_def] >>
    rpt gen_tac >> strip_tac >>
  ...
  strip_tac >- (
    ...
    conj_tac >- (
      ...
      `bc_next bs (bump_pc bs with stack := CodePtr (next_addr bs.inst_length (TAKE (LENGTH bc0 + 2 + LENGTH cc + 4) bs.code))::bs.stack)` by (
        ...
        full_simp_tac(srw_ss())[FILTER_REVERSE,ALL_DISTINCT_APPEND,ALL_DISTINCT_REVERSE,GSYM FILTER_EQ_NIL,combinTheory.o_DEF] >>
        ...
        rpt(qpat_assum`X.out = Y`kall_tac) >>
        srw_tac[][] >> spose_not_then strip_assume_tac >> res_tac >> fsrw_tac[ARITH_ss][]) >>
      qmatch_assum_abbrev_tac`bc_next bs bs1` >>
      ...
      conj_tac >- (
        ...
        match_mp_tac(SIMP_RULE std_ss [transitive_def] RTC_TRANSITIVE) >>
        full_simp_tac(srw_ss())[] >> HINT_EXISTS_TAC >> simp[] >>
        simp[RTC_eq_NRC] >>
    ...
    conj_tac >- (
      ...
      reverse conj_tac >- (
        ...
        simp[Abbr`bc00`] >>
        qexists_tac`blvs` >>
        simp[] >> full_simp_tac(srw_ss())[EVERY2_EVERY] ) >>
      simp[RTC_eq_NRC] >>
      qexists_tac`SUC(SUC 0)` >>
      simp[NRC] >>
      qho_match_abbrev_tac`∃bs1. bc_next bs0 bs1 ∧ P bs1` >>
  ...
  strip_tac >- (
    ...
    reverse(Cases_on`ALL_DISTINCT (FILTER is_Label (bc0 ++ code)) ∧
      ...
      conj_tac >- (
        ...
        simp[Abbr`cs1`,Abbr`cs0`,Once SWAP_REVERSE] >>
        srw_tac[][] >> full_simp_tac(srw_ss())[] >> full_simp_tac(srw_ss())[MEM_SING_APPEND] >>
        qsuff_tac`F`>-srw_tac[][] >>
    ...
    discharge_hyps >- (
      simp[Abbr`bss0`,TAKE_APPEND1,TAKE_APPEND2] >> full_simp_tac(srw_ss())[Cenv_bs_def]) >>
    ...
    conj_tac >- (
      ...
      discharge_hyps >- (
        `bs2.clock = SOME (FST (FST s'))` by (
          ...
          full_simp_tac(srw_ss())[Cenv_bs_def,s_refs_def,Abbr`bs2`,Abbr`bss0`]) >>
        simp[TAKE_APPEND1,TAKE_APPEND2,Abbr`bs2`,Abbr`bss0`,Abbr`aa`] >>
        conj_tac >- PROVE_TAC[] >>
        full_simp_tac(srw_ss())[SUM_APPEND,FILTER_APPEND,ALL_DISTINCT_APPEND,FILTER_REVERSE,EVERY_REVERSE] >>
      ...
      conj_tac >- (
        ...
        match_mp_tac(SIMP_RULE std_ss [transitive_def] RTC_TRANSITIVE) >>
        HINT_EXISTS_TAC >> simp[] >>
        full_simp_tac(srw_ss())[Abbr`bs4`] >>
        simp[Abbr`bs2`,Abbr`bss0`] >>
        ntac 5 (pop_assum kall_tac) >> srw_tac[][] >>
      ...
      full_simp_tac(srw_ss())[Abbr`bs2`,Abbr`bss0`] >>
    ...
    conj_tac >- (
      ...
      discharge_hyps >- (
        `bs2.clock = SOME (FST (FST s'))` by (
          imp_res_tac RTC_bc_next_clock_less >>
          full_simp_tac(srw_ss())[optionTheory.OPTREL_def,s_refs_def,Abbr`bs2`,Abbr`bss0`] >> rev_full_simp_tac(srw_ss())[] ) >>
        simp[TAKE_APPEND1,TAKE_APPEND2,Abbr`bs2`,Abbr`bss0`,Abbr`aa`,Abbr`co`] >>
        conj_tac >- PROVE_TAC[] >>
        full_simp_tac(srw_ss())[SUM_APPEND,FILTER_APPEND,ALL_DISTINCT_APPEND,FILTER_REVERSE,EVERY_REVERSE,Abbr`lab`] >>
        fsrw_tac[DNF_ss][MEM_FILTER,EVERY_MEM,is_Label_rwt,ALL_DISTINCT_REVERSE,MEM_MAP,between_def] >>
        fsrw_tac[ARITH_ss][Abbr`cs1`] >>
        srw_tac[][] >> spose_not_then strip_assume_tac >> res_tac >> fsrw_tac[ARITH_ss][] ) >>
      simp[] >>
      disch_then(qspecl_then[`env0`,`vs`,`v::klvs`,`bv::blvs`,`benv`,`ret`,`args`,`cl`,`st`]mp_tac o CONJUNCT2) >>
      full_simp_tac(srw_ss())[Abbr`tt`,ADD1,Abbr`bs2`] >>
      discharge_hyps >- (
        full_simp_tac(srw_ss())[Abbr`bss0`] >>
        full_simp_tac(srw_ss())[EVERY2_EVERY,EVERY_MEM,FORALL_PROD] >>
        srw_tac[][] >>
        match_mp_tac(MP_CANON (GEN_ALL(CONJUNCT1(SPEC_ALL Cv_bv_SUBMAP)))) >>
        simp[] >>
        ...
        full_simp_tac(srw_ss())[Cenv_bs_def,s_refs_def,good_rd_def,FEVERY_DEF,SUBMAP_DEF,DRESTRICT_DEF,UNCURRY]) >>
      simp[code_for_return_def] >>
      simp_tac(srw_ss()++DNF_ss)[] >>
    ...
    discharge_hyps >- (
      ...
      conj_asm1_tac >- (
        ...
        full_simp_tac(srw_ss())[] >> srw_tac[][] >> simp[DROP_APPEND1,DROP_APPEND2,DROP_LENGTH_NIL_rwt] ) >>
      conj_tac >- PROVE_TAC[] >>
      full_simp_tac(srw_ss())[s_refs_def,SUM_APPEND,FILTER_APPEND,ALL_DISTINCT_APPEND,FILTER_REVERSE,EVERY_REVERSE] >>
      fsrw_tac[DNF_ss][MEM_FILTER,EVERY_MEM,is_Label_rwt,ALL_DISTINCT_REVERSE,MEM_MAP,between_def] >>
      fsrw_tac[ARITH_ss][Abbr`cs1`] >>
      srw_tac[][] >> spose_not_then strip_assume_tac >> res_tac >> fsrw_tac[ARITH_ss][] ) >>
    simp[] >>
    ...
    conj_tac >- (
      ...
      full_simp_tac(srw_ss())[Abbr`bs4`] >>
      simp[Abbr`bs2`,Abbr`bss0`] ) >>
    full_simp_tac(srw_ss())[Abbr`bss0`,Abbr`bs2`] >>
    metis_tac[IS_PREFIX_TRANS,SUBMAP_TRANS,gvrel_trans]) >>
  strip_tac >- (
    simp[compile_def] >>
    rpt gen_tac >> strip_tac >>
  ...
  strip_tac >- (
    ...
    conj_asm1_tac >- (
      ntac 2 gen_tac >>
      full_simp_tac(srw_ss())[code_for_push_def,Cenv_bs_def,EVERY2_EVERY,EVERY_MEM,FORALL_PROD,env_renv_def] >>
      full_simp_tac(srw_ss())[MEM_EL,GSYM LEFT_FORALL_IMP_THM] >> rev_full_simp_tac(srw_ss())[] >>
      pop_assum mp_tac >>
      ...
      strip_tac >>
      strip_tac >>
      map_every qexists_tac [`[x]`,`bs.refs`,`rd`,`bs.clock`,`bs.globals`] >> srw_tac[][s_refs_with_stack] >- (
        match_mp_tac compile_varref_thm >> full_simp_tac(srw_ss())[] >>
        simp[bc_state_component_equality] >>
        rev_full_simp_tac(srw_ss())[el_check_def] >>
      ...
      >- (
        ...
        simp[option_case_NONE_F,EL_ZIP] >> srw_tac[][] >>
        qunabbrev_tac`X` >>
        imp_res_tac lookup_ct_imp_incsz >>
    ...
    rev_full_simp_tac(srw_ss())[el_check_def] >>
    qspecl_then[`sz`,`cs`,`EL n cenv`]strip_assume_tac compile_varref_append_out >> full_simp_tac(srw_ss())[] >>
    first_x_assum(qspec_then`REVERSE bc`mp_tac o CONV_RULE SWAP_FORALL_CONV) >> simp[] >>
    full_simp_tac(srw_ss())[Once SWAP_REVERSE] >> strip_tac >>
  ...
  strip_tac >- (
    ...
    simp[Once SWAP_REVERSE] >>
    conj_asm1_tac >- (
      simp[code_for_push_def,PULL_EXISTS] >> srw_tac[][] >>
      ...
      conj_tac >- (
        ...
        simp[bc_eval1_thm,bc_eval1_def,bc_eval_stack_def,bump_pc_def] >>
        simp[bc_state_component_equality,SUM_APPEND,FILTER_APPEND] >>
        rev_full_simp_tac(srw_ss())[Cenv_bs_def,s_refs_def,EVERY2_EVERY,EVERY_MEM] >>
        full_simp_tac(srw_ss())[MEM_ZIP,PULL_EXISTS,optionTheory.OPTREL_def] >>
        rpt (first_x_assum(qspec_then`n`mp_tac)) >>
  ...
  strip_tac >- (
    ...
    conj_asm1_tac >- (
      ...
      Cases_on`l`>>srw_tac[][compile_def,LET_THM,code_for_push_def,pushret_def,FOLDL_emit_append_out]>>
      TRY(qpat_assum`X = REVERSE code`(assume_tac o SIMP_RULE std_ss [Once SWAP_REVERSE])) >> full_simp_tac(srw_ss())[] >>
      TRY (
        ...
        `bc_fetch bs = SOME x` by (
          ...
          srw_tac[][Abbr`x`]) >> (
        srw_tac[][Once Cv_bv_cases] >>
        map_every qexists_tac [`bs.refs`,`rd`,`bs.clock`,`bs.globals`] >> reverse (srw_tac[][]) >- (
          match_mp_tac Cenv_bs_imp_incsz_irr >>
          qexists_tac`bs with code := bce` >>
          srw_tac[][bc_state_component_equality]  ) >>
        srw_tac[][RTC_eq_NRC] >>
        qexists_tac `SUC 0` >>
        srw_tac[][NRC] >>
        srw_tac[][bc_eval1_thm] >>
        ...
        simp[bc_state_component_equality])) >>
      TRY (
        qmatch_assum_abbrev_tac `code = [xx;x]` >>
        `bc_fetch bs = SOME xx` by (
          match_mp_tac bc_fetch_next_addr >>
          qexists_tac `bc0` >>
          srw_tac[][Abbr`xx`]) >>
        srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
        simp[bc_eval1_thm,bc_eval1_def,Abbr`xx`,bc_eval_stack_def,bump_pc_def] >>
        Q.PAT_ABBREV_TAC`bs1:bc_state = X Y` >>
        `bc_fetch bs1 = SOME x` by (
          match_mp_tac bc_fetch_next_addr >>
          CONV_TAC SWAP_EXISTS_CONV >>
          qexists_tac`bc1` >>
          simp[Abbr`x`,Abbr`bs1`,SUM_APPEND,FILTER_APPEND]) >>
        srw_tac[][Once Cv_bv_cases] >>
        srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
        simp[bc_eval1_thm,bc_eval1_def,Abbr`bs1`,Abbr`x`,bc_eval_stack_def,bump_pc_def] >>
        srw_tac[DNF_ss][Once RTC_CASES1] >> disj1_tac >>
        simp[bc_state_component_equality,SUM_APPEND,FILTER_APPEND] >>
        qexists_tac`rd` >> simp[] >>
        match_mp_tac Cenv_bs_imp_incsz_irr >>
        ...
        srw_tac[][bc_state_component_equality] ) >>
      TRY (
        simp[PULL_EXISTS,Cv_bv_cases_lit] >>
        qspecl_then[`i`,`bs`,`bc0`]mp_tac PushAnyInt_thm >>
        simp[] >> strip_tac >>
        srw_tac[DNF_ss][Once RTC_CASES_RTC_TWICE] >>
        first_assum(match_exists_tac o concl) >> simp[] >>
        srw_tac[DNF_ss][Once RTC_CASES1] >> disj1_tac >>
      ...
      conj_tac >- (
        ...
        srw_tac[DNF_ss][Once RTC_CASES_RTC_TWICE] >>
        qpat_assum`bs.code = X`mp_tac >>
        Q.PAT_ABBREV_TAC`i:int=X` >> strip_tac >>
        qmatch_assum_abbrev_tac`bs.code = bc0 ++ bc00 ++ PushAnyInt i ++ bc11 ++ bc1` >>
        qspecl_then[`i`,`bs1`,`bc0++bc00`]mp_tac PushAnyInt_thm >>
        simp[Abbr`bs1`] >> strip_tac >>
        first_assum(match_exists_tac o concl) >> simp[] >>
        pop_assum kall_tac >>
        srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
        Q.PAT_ABBREV_TAC`bs1:bc_state = X Y` >>
        `bc_fetch bs1 = SOME(Stack(Cons string_tag))` by (
          ...
          qexists_tac`bc1` >> simp[SUM_APPEND,FILTER_APPEND] ) >>
        simp[bc_eval1_thm,bc_eval1_def,bc_eval_stack_def,bump_pc_def] >>
        srw_tac[DNF_ss][Once RTC_CASES1] >> disj1_tac >>
        simp[Abbr`bs1`,Abbr`bs2`,bc_state_component_equality,SUM_APPEND,FILTER_APPEND,bc_eval_stack_def,Abbr`i`] >>
        simp[TAKE_APPEND2,DROP_APPEND2,stringTheory.IMPLODE_EXPLODE_I,Abbr`bc11`]) >>
      simp[] >>
      match_mp_tac Cenv_bs_imp_incsz_irr >>
      qexists_tac`bs with code := bce` >>
      simp[]) >>
    rpt gen_tac >> strip_tac >>
    rpt gen_tac >> strip_tac >>
    qspecl_then[`cenv`,`TCNonTail`,`sz`,`cs`,`(CLit l)`]strip_assume_tac(CONJUNCT1 compile_append_out) >> full_simp_tac(srw_ss())[] >>
    full_simp_tac(srw_ss())[Once SWAP_REVERSE] >>
    `code = REVERSE bc ++ (retbc (LENGTH args) (LENGTH klvs))` by (
      Cases_on`l`>>full_simp_tac(srw_ss())[compile_def,pushret_def,LET_THM,FOLDL_emit_append_out] >>
      srw_tac[][] >> full_simp_tac(srw_ss())[Once SWAP_REVERSE]) >>
    match_mp_tac code_for_push_return >> simp[] >> full_simp_tac(srw_ss())[] >>
    first_assum (match_exists_tac o concl) >> simp[] >>
    qexists_tac`REVERSE args`>>fsrw_tac[ARITH_ss][EVERY2_EVERY]) >>
  strip_tac >- (
    ...
    conj_asm1_tac >- (
      ...
      disch_then(assume_tac o SIMP_RULE std_ss [Once SWAP_REVERSE]) >> full_simp_tac(srw_ss())[] >>
      qmatch_assum_abbrev_tac `code = ls0 ++ xx ++ [x]` >>
      fsrw_tac[DNF_ss][] >>
      ...
      conj_tac >- (
        ...
        HINT_EXISTS_TAC >>
        full_simp_tac(srw_ss())[Abbr`bs05`] >>
        qmatch_abbrev_tac`bc_next^* bs0 bs2` >>
        srw_tac[][Once RTC_CASES_RTC_TWICE] >>
        qmatch_assum_abbrev_tac`Abbrev(xx = PushAnyInt i)` >>
        qspecl_then[`i`,`bs0`,`bc0 ++ REVERSE cx`]mp_tac PushAnyInt_thm >>
        simp[Abbr`bs0`] >> strip_tac >> HINT_EXISTS_TAC >> simp[] >> pop_assum kall_tac >>
        simp[Abbr`bs2`] >>
        qmatch_abbrev_tac`bc_next^* bs0 bs2` >>
        ...
        srw_tac[DNF_ss][Once RTC_CASES1] >> disj2_tac >>
        srw_tac[][bc_eval1_thm] >>
        srw_tac[][bc_eval1_def,Abbr`x`,bump_pc_def] >>
        imp_res_tac Cevaluate_list_LENGTH >>
        full_simp_tac(srw_ss())[EVERY2_EVERY] >>
        srw_tac[ARITH_ss][bc_eval_stack_def,Abbr`bs0`,Abbr`i`] >>
        srw_tac[DNF_ss][Once RTC_CASES1] >> disj1_tac >>
        unabbrev_all_tac >>
      ...
      `Cenv_bs rd' s' denv cenv sz (bs1 with stack := bs.stack)` by (
        ...
        `LENGTH exps = LENGTH bvs` by full_simp_tac(srw_ss())[EVERY2_EVERY] >> full_simp_tac(srw_ss())[] >>
        metis_tac[Cenv_bs_CTLet_bound]) >>
      match_mp_tac Cenv_bs_imp_incsz_irr >>
      qexists_tac `bs1 with stack := bs.stack` >>
      srw_tac[][bc_state_component_equality,Abbr`bs1`] ) >>
  ...
  (* Let *) strip_tac >- (
    (* Seq *) reverse(Cases_on`bd`) >- (
      ...
      conj_tac >- (
        ...
        discharge_hyps >- (
          simp[] >>
          conj_asm1_tac >- full_simp_tac(srw_ss())[Abbr`cs1`,Abbr`cs0`] >>
          conj_tac >- (
          ...
          conj_tac >- (
            simp[Abbr`bs3`,bump_pc_def,Abbr`bs2`] ) >>
          conj_tac >- (
            rev_full_simp_tac(srw_ss())[Abbr`bs3`,bump_pc_def,Abbr`bs2`] >>
            full_simp_tac(srw_ss())[Cenv_bs_def,s_refs_def] >>
            imp_res_tac bc_next_clock_less >>
            imp_res_tac RTC_bc_next_clock_less >>
            full_simp_tac(srw_ss())[optionTheory.OPTREL_def] >> full_simp_tac(srw_ss())[] ) >>
          full_simp_tac(srw_ss())[Abbr`cs1`,Abbr`cs0`] >>
          full_simp_tac(srw_ss())[Cenv_bs_def,s_refs_def,SUM_APPEND,FILTER_APPEND,ALL_DISTINCT_APPEND,FILTER_REVERSE,EVERY_REVERSE] >>
          fsrw_tac[DNF_ss][MEM_FILTER,EVERY_MEM,is_Label_rwt,ALL_DISTINCT_REVERSE,MEM_MAP,between_def] >>
          fsrw_tac[ARITH_ss][] >>
          srw_tac[][] >> spose_not_then strip_assume_tac >> res_tac >> fsrw_tac[ARITH_ss][] ) >>
        disch_then(mp_tac o CONJUNCT1) >>
        ...
        conj_tac >- (
          ...
          `bs12 = bs2` by (
            ...
            full_simp_tac(srw_ss())[Abbr`cs1`,Abbr`cs0`] >> simp[SUM_APPEND,FILTER_APPEND] ) >>
          metis_tac[RTC_TRANSITIVE,transitive_def,RTC_SUBSET] ) >>
        rev_full_simp_tac(srw_ss())[SUM_APPEND,FILTER_APPEND,Abbr`cs1`,Abbr`cs0`] >>
        fsrw_tac[ARITH_ss][] >>
        metis_tac[IS_PREFIX_TRANS,SUBMAP_TRANS,gvrel_trans] ) >>
      conj_tac >- (
        Q.PAT_ABBREV_TAC`cs0:compiler_result = X exp` >>
        ...
        conj_tac >- (
          ...
          `bs16 = bs06` by (
            simp[Abbr`bs16`,Abbr`bs06`,bc_state_component_equality,bump_pc_def,Abbr`bs05`] ) >>
          metis_tac[RTC_TRANSITIVE,transitive_def,RTC_SUBSET] ) >>
        full_simp_tac(srw_ss())[] >> metis_tac[IS_PREFIX_TRANS,SUBMAP_TRANS,gvrel_trans] ) >>
      rpt gen_tac >>
      Q.PAT_ABBREV_TAC`cs0:compiler_result = X exp` >>
      Q.PAT_ABBREV_TAC`cs1 = compiler_result_out_fupd X Y` >>
      qspecl_then[`cenv`,`t`,`sz`,`cs1`,`exp'`](Q.X_CHOOSE_THEN`cb`strip_assume_tac)(CONJUNCT1 compile_append_out) >>
    ...
    `all_vlabs_csg s' ∧ all_vlabs v ∧
     ...
     Cenv_bs rd' s' (v::env) (CTLet(sz+1)::cenv) (sz+1) (bs2 with code := bce)` by (
      ...
      match_mp_tac Cenv_bs_FUPDATE >>
      qexists_tac`bs2 with <| stack := bs.stack; code := bce|>` >>
      simp[bc_state_component_equality,Abbr`bs2`] >> full_simp_tac(srw_ss())[] >>
      match_mp_tac Cenv_bs_change_store >>
      map_every qexists_tac [`rd`,`s`,`bs with <| pc := next_addr bs.inst_length (bc0 ++ REVERSE cc); code := bce|>`,`rf`] >>
      simp[bc_state_component_equality] >>
      full_simp_tac(srw_ss())[Cenv_bs_def,s_refs_def,good_rd_def,env_renv_def,fmap_rel_def]) >>
    full_simp_tac(srw_ss())[] >>
    conj_tac >- (
      ...
      simp[Abbr`cs1`,Once SWAP_REVERSE] >> srw_tac[][] >>
      ...
      discharge_hyps >- (
        ...
        conj_tac >- PROVE_TAC[] >>
        conj_tac >- (
          imp_res_tac RTC_bc_next_clock_less >> rev_full_simp_tac(srw_ss())[optionTheory.OPTREL_def] >>
          full_simp_tac(srw_ss())[Cenv_bs_def,s_refs_def] ) >>
        full_simp_tac(srw_ss())[Cenv_bs_def,s_refs_def,SUM_APPEND,FILTER_APPEND,ALL_DISTINCT_APPEND,FILTER_REVERSE,EVERY_REVERSE] >>
        fsrw_tac[DNF_ss][MEM_FILTER,EVERY_MEM,is_Label_rwt,ALL_DISTINCT_REVERSE,MEM_MAP,between_def] >>
        fsrw_tac[ARITH_ss][Abbr`cs1`] >>
      ...
      simp[] >>
      conj_tac >- (
        srw_tac[][Once RTC_CASES2] >> disj2_tac >>
        HINT_EXISTS_TAC >> simp[] >>
        pop_assum kall_tac >>
        qmatch_abbrev_tac`bc_next bs0 bs1` >>
        `bc_fetch bs0 = SOME (Stack (Pops 1))` by (
    ...
    conj_tac >- (
      ...
      map_every qexists_tac[`br`,`rf'`,`rd''`,`ck'`,`gv'`] >>
      conj_tac >- (
        full_simp_tac(srw_ss())[] >> qmatch_abbrev_tac`bc_next^* bss bs5` >>
        match_mp_tac(SIMP_RULE std_ss [transitive_def] RTC_TRANSITIVE) >>
  ...
  strip_tac >- (
    ...
    (Cases_on `¬∃cc1. code = REVERSE cx ++ cc1` >- (
      ...
      simp[Once SWAP_REVERSE,Abbr`cs1`] >> srw_tac[][] >> full_simp_tac(srw_ss())[])) >> full_simp_tac(srw_ss())[] >>
    first_x_assum (qspecl_then[`rd`,`cs`,`cenv`,`sz`,`bs`,`bce`,`bcr`,`bc0`,`REVERSE cx`]mp_tac) >>
    simp[GSYM FORALL_AND_THM] >>
    disch_then(qspec_then`TCNonTail`mp_tac) >> simp[] >>
    Cases_on`err`>>simp[]>>
  ...
  strip_tac >- (
    ...
    reverse(Cases_on`∃bc10. bs.code = bc0 ++ cc ++ bc10`) >- (
      ...
      srw_tac[][Once SWAP_REVERSE_SYM] >> full_simp_tac(srw_ss())[] >> rev_full_simp_tac(srw_ss())[] ) >>
    full_simp_tac(srw_ss())[] >>
    first_x_assum(qspecl_then[`bs`,`bc0`,`bc10`,`FDOM rd.cls`]mp_tac) >>
    simp[] >>
    discharge_hyps >- (
      qpat_assum`∀rd yy. Z`kall_tac >>
      full_simp_tac(srw_ss())[SUM_eq_0,MEM_MAP,GSYM LEFT_FORALL_IMP_THM,EVERY_MEM] >>
      conj_asm1_tac >- (
        fsrw_tac[DNF_ss][free_labs_defs_MAP,MEM_FLAT,MEM_GENLIST] >>
        rpt strip_tac >> res_tac >>
        PairCases_on`e` >>
        Cases_on`e0`>>full_simp_tac(srw_ss())[]) >>
      ...
      `code_env_cd bce ((LENGTH env,LENGTH defs,n),(THE(FST def)),(SND def))` by (
        ...
        simp_tac(srw_ss()++DNF_ss)[] >>
        qexists_tac`n` >>
        PairCases_on`def` >> Cases_on`def0` >> full_simp_tac(srw_ss())[] >>
        PairCases_on`x`>>simp[] ) >>
      PairCases_on`def`>>Cases_on`def0`>>full_simp_tac(srw_ss())[]>>PairCases_on`x`>>
      qpat_assum`X ++ A = Y ++ Z`(assume_tac o SYM) >>
      full_simp_tac(srw_ss())[code_env_cd_def,bc_find_loc_def] >>
      conj_tac >- (
        qmatch_abbrev_tac`IS_SOME X` >>
        Cases_on`X`>>srw_tac[][bc_find_loc_def]>>
      ...
      full_simp_tac(srw_ss())[good_cd_def] >>
      `LENGTH cenv = LENGTH env` by full_simp_tac(srw_ss())[Cenv_bs_def,EVERY2_EVERY,env_renv_def] >>
      full_simp_tac(srw_ss())[EVERY_MEM] >>
      qx_gen_tac`z` >>
      reverse conj_tac >- (
        full_simp_tac(srw_ss())[bind_fv_def,LET_THM,MEM_FILTER,MEM_GENLIST] ) >>
    ...
    `rd.cls ⊑ rd'.cls` by (
      ...
      srw_tac[][] >> match_mp_tac EQ_SYM >>
      match_mp_tac FUPDATE_LIST_APPLY_NOT_MEM >>
      simp[MAP_GENLIST,combinTheory.o_DEF,MEM_GENLIST] >>
      full_simp_tac(srw_ss())[Cenv_bs_def,s_refs_def,good_rd_def,FEVERY_DEF,UNCURRY,FLOOKUP_DEF] >>
      metis_tac[MEM_EL]) >>
     ...
     EVERY (combin$C $< ccs.next_label o dest_Label) (FILTER is_Label (bc0++cc))` by (
      ...
      conj_tac >- (
        ...
        reverse conj_asm2_tac >- (
          ...
          full_simp_tac(srw_ss())[code_env_cd_def,bc_find_loc_def,good_cd_def] >>
          simp[Once Cv_bv_cases,el_check_def] >>
          qpat_assum`X = bind_fv A B C` mp_tac >>
          simp[bind_fv_def] >>
          strip_tac >>
          conj_asm1_tac >- (
            srw_tac[][EVERY_FILTER,EVERY_GENLIST] ) >>
          conj_asm1_tac >- (
          ...
          `i < LENGTH envs` by srw_tac[][Abbr`envs`] >>
          ...
          simp[option_case_NONE_F] >> srw_tac[][] >> simp[] >>
          match_mp_tac (GEN_ALL (MP_CANON (CONJUNCT1 (SPEC_ALL Cv_bv_SUBMAP)))) >>
          HINT_EXISTS_TAC >> simp[] >>
          simp[Abbr`rd'`] ) >>
        match_mp_tac Cenv_bs_change_store >>
        ...
        reverse conj_tac >- (
          ...
          simp[MAP_ZIP] >>
          metis_tac[] ) >>
        full_simp_tac(srw_ss())[Cenv_bs_def,env_renv_def,s_refs_def,fmap_rel_def,good_rd_def] >>
        conj_tac >- (
          full_simp_tac(srw_ss())[FEVERY_DEF,UNCURRY,FDOM_FUPDATE_LIST] >>
          ...
          Cases_on`MEM x rs` >> simp[] >- (
            ...
            `cls = (env,defs,i)` by (
              ...
              asm_simp_tac(srw_ss()++DNF_ss)[Abbr`P`,Abbr`y`] >> rev_full_simp_tac(srw_ss())[] >>
            ...
            BasicProvers.CASE_TAC >- (
              imp_res_tac ALOOKUP_FAILS >>
              rev_full_simp_tac(srw_ss())[MEM_ZIP] >> metis_tac[] ) >>
            imp_res_tac ALOOKUP_MEM >>
            rev_full_simp_tac(srw_ss())[MEM_ZIP,EL_MAP] >> full_simp_tac(srw_ss())[EL_MAP] >>
            full_simp_tac(srw_ss())[LIST_REL_EL_EQN] >>
            first_x_assum(qspec_then`n`mp_tac) >>
            simp[EL_MAP] >>
            simp[Abbr`vs`] >>
          ...
          qsuff_tac`(bv = bs.refs ' x) ∧ (dd = rd.cls ' x)` >- (
            ...
            qexists_tac`rd` >> simp[] >> rev_full_simp_tac(srw_ss())[]) >>
        ...
        conj_tac >- (
          ...
          full_simp_tac(srw_ss())[EVERY_MEM] ) >>
        full_simp_tac(srw_ss())[Cenv_bs_def,s_refs_def,EVERY2_EVERY,good_rd_def,EVERY_MEM,FORALL_PROD] >>
        rator_x_assum`RTC`kall_tac >>
    ...
    Cases_on`beh`>>full_simp_tac(srw_ss())[] >- (
      conj_tac >- (
        ...
        `bs1.code = bs.code` by srw_tac[][Abbr`bs1`] >>
        simp[Once SWAP_REVERSE] >> strip_tac >>
        ...
        discharge_hyps >- (
          rpt (BasicProvers.VAR_EQ_TAC) >>
          full_simp_tac(srw_ss())[Cenv_bs_def,s_refs_def,Abbr`bs1`,FILTER_APPEND,ALL_DISTINCT_APPEND] >>
          full_simp_tac(srw_ss())[GSYM FILTER_EQ_NIL,combinTheory.o_DEF] ) >>
        disch_then(mp_tac o CONJUNCT1) >>
        simp[code_for_push_def] >>
        ...
        conj_tac >- full_simp_tac(srw_ss())[Abbr`bs1`] >>
        conj_asm2_tac >- (
          match_mp_tac Cenv_bs_imp_incsz_irr >>
          ...
          simp[bc_state_component_equality] >>
          full_simp_tac(srw_ss())[Cenv_bs_def,s_refs_def,good_rd_def,Abbr`bs1`] ) >>
        full_simp_tac(srw_ss())[Abbr`rd'`] >>
        reverse conj_tac >- (
          conj_tac >- metis_tac[SUBMAP_TRANS] >>
          full_simp_tac(srw_ss())[Abbr`bs1`] ) >>
        ...
        srw_tac[][] >- (
          ...
          match_mp_tac FUPDATE_LIST_APPLY_NOT_MEM >>
          simp[MAP_ZIP] >>
          full_simp_tac(srw_ss())[Cenv_bs_def,s_refs_def,good_rd_def,FEVERY_DEF,UNCURRY] >>
          metis_tac[] ) >>
        pop_assum mp_tac >>
      ...
      full_simp_tac(srw_ss())[FOLDL_FUPDATE_LIST] >>
      simp[Abbr`cenv'`] >>
      ...
      discharge_hyps >- (
        ...
        conj_tac >- (
          full_simp_tac(srw_ss())[EVERY2_EVERY,EVERY_MEM,FORALL_PROD] >>
          `rd'.sm = rd.sm` by srw_tac[][Abbr`rd'`] >>
          srw_tac[][] >>
          match_mp_tac (MP_CANON (GEN_ALL (CONJUNCT1 (SPEC_ALL Cv_bv_SUBMAP)))) >> simp[] >>
          qexists_tac`rd` >> simp[] ) >>
        full_simp_tac(srw_ss())[EVERY2_EVERY] >>
        ...
        conj_tac >- (
          ...
          full_simp_tac(srw_ss())[Abbr`bvs`,EL_MAP] >>
          qabbrev_tac`def = EL n defs` >>
          `MEM def defs` by metis_tac[MEM_EL] >>
          full_simp_tac(srw_ss())[EVERY_MEM] >>
          ...
          conj_asm1_tac >- (
            srw_tac[][EVERY_FILTER,EVERY_GENLIST] ) >>
          conj_asm1_tac >- (
            srw_tac[][] >> fsrw_tac[ARITH_ss][EVERY2_EVERY] >> rev_full_simp_tac(srw_ss())[] >>
            fsrw_tac[ARITH_ss][] ) >>
          conj_asm1_tac >- (
            qsuff_tac`MEM (Label x0) bce` >- (
          ...
          rator_x_assum`Cenv_bs`mp_tac >>
          simp[Cenv_bs_def,env_renv_def,EVERY2_EVERY,EVERY_MEM] >> strip_tac >>
          `LENGTH blvs = LENGTH klvs` by srw_tac[][] >> fsrw_tac[ARITH_ss][] >>
          `LENGTH args = LENGTH vs` by srw_tac[][] >> fsrw_tac[ARITH_ss][] >>
      ...
      srw_tac[][] >- (
        ...
        match_mp_tac FUPDATE_LIST_APPLY_NOT_MEM >>
        simp[MAP_ZIP] >>
        full_simp_tac(srw_ss())[Cenv_bs_def,s_refs_def,good_rd_def,FEVERY_DEF,UNCURRY,Abbr`klvs2`] >>
        simp[MAP_ZIP,TAKE_LENGTH_ID_rwt,Abbr`bvs`] >>
        metis_tac[] ) >>
      full_simp_tac(srw_ss())[Abbr`xx`,FDOM_FUPDATE_LIST]) >>
    rpt gen_tac >>
    ...
    discharge_hyps >- (
      rpt (BasicProvers.VAR_EQ_TAC) >>
      full_simp_tac(srw_ss())[Cenv_bs_def,s_refs_def,Abbr`bs1`,FILTER_APPEND,ALL_DISTINCT_APPEND] >>
      full_simp_tac(srw_ss())[GSYM FILTER_EQ_NIL,combinTheory.o_DEF] >> NO_TAC)) >>
    ...
    >- (
      ...
      rpt strip_tac >> full_simp_tac(srw_ss())[] >> rev_full_simp_tac(srw_ss())[] >> pop_assum mp_tac >>
      discharge_hyps >- (
        CONV_TAC(RESORT_EXISTS_CONV(List.rev)) >>
        qexists_tac`st` >> simp[] >>
        qexists_tac`args`>>simp[] >>
    ...
    reverse conj_tac >- (
      ...
      match_mp_tac FUPDATE_LIST_APPLY_NOT_MEM_matchable >>
      simp[MAP_ZIP] >>
      full_simp_tac(srw_ss())[Cenv_bs_def,s_refs_def,good_rd_def,FEVERY_DEF] >>
      metis_tac[] ) >>
    ntac 5 (pop_assum kall_tac) >>
    qmatch_assum_abbrev_tac`bc_next^* bs bs1` >>
    metis_tac[RTC_TRANSITIVE,transitive_def]) >>
  strip_tac >- (
    ...
    strip_tac >> rev_full_simp_tac(srw_ss())[] >> full_simp_tac(srw_ss())[] >>
    BasicProvers.VAR_EQ_TAC >>
    qspecl_then[`cenv`,`TCNonTail`,`sz`,`cs`,`exp`](Q.X_CHOOSE_THEN`cx`strip_assume_tac)(CONJUNCT1 compile_append_out) >> full_simp_tac(srw_ss())[] >>
    Q.PAT_ABBREV_TAC`cs0 = compile cenv X sz cs exp` >>
    qspecl_then[`cenv`,`sz+1`,`cs0`,`exps`](Q.X_CHOOSE_THEN`cxs`strip_assume_tac)(CONJUNCT2 (CONJUNCT2 compile_append_out)) >> full_simp_tac(srw_ss())[] >>
    reverse(Cases_on`∃bc10. code = REVERSE cx ++ REVERSE cxs ++ bc10`) >- (
      fsrw_tac[DNF_ss][] >>
      rpt conj_tac >> rpt gen_tac >>
      srw_tac[][FOLDL_emit_append_out,Once SWAP_REVERSE,Abbr`mtk`] >>
      TRY(Cases_on`t`)>>full_simp_tac(srw_ss())[Once SWAP_REVERSE]) >> full_simp_tac(srw_ss())[] >>
    reverse(Cases_on`bs.code = bc0 ++ REVERSE cx ++ REVERSE cxs ++ bc10 ++ bc1`) >- full_simp_tac(srw_ss())[] >>
    full_simp_tac(srw_ss())[Once SWAP_REVERSE] >>
    ...
    discharge_hyps >- (
      ...
      conj_tac >- (
        imp_res_tac RTC_bc_next_clock_less >>
        rev_full_simp_tac(srw_ss())[Cenv_bs_def,s_refs_def,optionTheory.OPTREL_def]) >>
    ...
    let
      fun tac0 t =
        ...
        `Cv_bv (rd'', l2a) cl bf` by (
          ...
          qexists_tac`rd'` >>
          srw_tac[][] >>
          full_simp_tac(srw_ss())[Cenv_bs_def,s_refs_def,good_rd_def,FEVERY_DEF,UNCURRY,SUBMAP_DEF,SUBSET_DEF]) >>
        pop_assum mp_tac >>
        ...
        `bs0' = bs0` by (
          ...
          imp_res_tac RTC_bc_next_clock_less >>
          rev_full_simp_tac(srw_ss())[s_refs_def,Cenv_bs_def,optionTheory.OPTREL_def] )
      val tac5 =
        full_simp_tac(srw_ss())[el_check_def] >>
        ...
        `code_env_cd bce ((LENGTH fenv,LENGTH defs,n),cc,azb)` by (
          ...
          discharge_hyps >- (
            ...
            simp_tac(srw_ss()++DNF_ss)[] >>
            qexists_tac`n` >> simp[]) >>
          full_simp_tac(srw_ss())[EVERY_MEM,vlabs_csg_def] >>
          metis_tac[] ) >>
        pop_assum mp_tac >>
        qunabbrev_tac`cc` >>
        PairCases_on`azb` >>
        ...
        discharge_hyps >- (
          full_simp_tac(srw_ss())[good_cd_def] >>
          `ALL_DISTINCT (FILTER is_Label bce)` by full_simp_tac(srw_ss())[contains_primitives_def] >>
          conj_tac >- (
          ...
          conj_tac >- (
            ...
            qspecl_then[`s`,`env`,`exp`,`s',Rval (CRecClos fenv defs n)`]mp_tac(CONJUNCT1 Cevaluate_all_vlabs) >>
            qspecl_then[`s'`,`env`,`exps`,`((count',s''),g),Rval vs`]mp_tac(CONJUNCT2 Cevaluate_all_vlabs) >>
            simp[] >> full_simp_tac(srw_ss())[EVERY_MEM,all_vlabs_csg_def] >> metis_tac[MEM_EL] ) >>
          conj_tac >- (
            qspecl_then[`s`,`env`,`exp`,`s',Rval (CRecClos fenv defs n)`]mp_tac(CONJUNCT1 Cevaluate_all_vlabs) >>
          ...
          conj_tac >- (
            ...
            simp[EVERY_MEM] >>
            qx_gen_tac`z` >> strip_tac >>
            qsuff_tac`z ∈ Y`>-(full_simp_tac(srw_ss())[EVERY_MEM,vlabs_csg_def]>>metis_tac[IN_UNION])>>
          ...
          conj_tac >- (
            ...
            metis_tac[] ) >>
          conj_asm1_tac >- (
            full_simp_tac(srw_ss())[Abbr`bs1`,Abbr`l2a`] >>
            ...
            pop_assum mp_tac >>
            REWRITE_TAC[GSYM APPEND_ASSOC] >>
            srw_tac[][EL_LENGTH_APPEND,TAKE_LENGTH_APPEND] >>
            simp[FILTER_APPEND] ) >>
          conj_tac >- simp[Abbr`bs1`] >>
          conj_tac >- simp[Abbr`bs1`] >>
          conj_tac >- (
            ...
            qexists_tac`e`>>simp[]>>
            full_simp_tac(srw_ss())[s_refs_def,Cenv_bs_def,good_rd_def] >>
            TRY (
            conj_tac >- (
              ...
              match_mp_tac(GEN_ALL(MP_CANON(EVERY2_MEM_MONO))) >>
              HINT_EXISTS_TAC >> simp[] >>
              simp[FORALL_PROD] >> full_simp_tac(srw_ss())[EVERY2_EVERY] >>
          ...
          full_simp_tac(srw_ss())[ALL_DISTINCT_APPEND,FILTER_APPEND] ) >>
        full_simp_tac(srw_ss())[]
      fun tac6 t =
        full_simp_tac(srw_ss())[] >> ntac 2 (pop_assum kall_tac) >>
        `bc_fetch bs3 = SOME (Stack (Load (LENGTH exps)))` by (
          match_mp_tac bc_fetch_next_addr >>
          srw_tac[][Abbr`bs3`,REVERSE_APPEND] >>
          qexists_tac`bc0 ++ REVERSE cx ++ REVERSE cxs` >>
          srw_tac[][] ) >>
        simp_tac(srw_ss()++DNF_ss)[Once RTC_CASES1] >> disj2_tac >>
        srw_tac[][bc_eval1_thm,bc_eval1_def,bc_eval_stack_def] >>
        `LENGTH exps = LENGTH bvs` by (full_simp_tac(srw_ss())[EVERY2_EVERY] >> metis_tac[Cevaluate_list_LENGTH] ) >>
        simp[Abbr`bs3`] >>
        lrw[EL_APPEND2] >>
        simp[bump_pc_with_stack] >> full_simp_tac(srw_ss())[bc_fetch_with_stack] >>
        simp[bump_pc_def] >>
        qpat_assum`bc_fetch X = Y` kall_tac >>
        qho_match_abbrev_tac t >>
        `bc_fetch bs1 = SOME (Stack (PushInt 1))` by (
          ...
          srw_tac[][Abbr`ls`,REVERSE_APPEND,FILTER_APPEND,SUM_APPEND,ADD1] ) >>
        simp_tac(srw_ss()++DNF_ss)[Once RTC_CASES1] >> disj2_tac >>
        srw_tac[][bc_eval1_thm,bc_eval1_def] >>
        simp[Abbr`bs1`,bc_eval_stack_def] >>
        full_simp_tac(srw_ss())[bump_pc_with_stack,bc_fetch_with_stack] >>
        simp[bump_pc_def] >>
        qpat_assum`bc_fetch X = Y` kall_tac >>
        qho_match_abbrev_tac t >>
        `bc_fetch bs1 = SOME (Stack El)` by (
          ...
          srw_tac[][Abbr`bs1`] >>
          Q.PAT_ABBREV_TAC`ls = bc0 ++ X ++ Y` >>
          qexists_tac`ls ++ [Stack (Load (LENGTH bvs)); Stack (PushInt 1)]` >>
          srw_tac[][Abbr`ls`,REVERSE_APPEND,FILTER_APPEND,SUM_APPEND,ADD1] >> simp[] ) >>
        simp_tac(srw_ss()++DNF_ss)[Once RTC_CASES1] >> disj2_tac >>
        srw_tac[][bc_eval1_thm,bc_eval1_def] >>
        simp[Abbr`bs1`,bc_eval_stack_def] >>
        full_simp_tac(srw_ss())[bump_pc_with_stack,bc_fetch_with_stack] >>
        ...
        simp[Abbr`bs1`,bc_eval_stack_def] >>
        lrw[EL_APPEND2,EL_APPEND1] >>
        full_simp_tac(srw_ss())[bc_fetch_with_stack,bump_pc_with_stack] >>
        srw_tac[][bump_pc_def] >>
        qpat_assum`bc_fetch X = Y` kall_tac >>
        qho_match_abbrev_tac t >>
        `bc_fetch bs1 = SOME (Stack (PushInt 0))` by (
        ...
        simp_tac(srw_ss()++DNF_ss)[Once RTC_CASES1] >> disj2_tac >>
        srw_tac[][bc_eval1_thm,bc_eval1_def] >>
        simp[Abbr`bs1`,bc_eval_stack_def,bump_pc_def] >>
        qpat_assum`bc_fetch X = Y` kall_tac >>
        qho_match_abbrev_tac t >>
        `bc_fetch bs1 = SOME (Stack El)` by (
          match_mp_tac bc_fetch_next_addr >>
        ...
        `bc_next^* bs1 (bs1 with <| clock := SOME cka; pc := next_addr bs.inst_length (BUTLASTN (1 + LENGTH bc1) bs.code) |>)` by (
          ...
          Cases_on`ck`>>simp[]>-(
            ...
            imp_res_tac RTC_bc_next_clock_less >>
            rev_full_simp_tac(srw_ss())[Cenv_bs_def,s_refs_def,optionTheory.OPTREL_def,bc_state_component_equality] >> full_simp_tac(srw_ss())[] >>
      ...
      val tac1 =
        ...
        full_simp_tac(srw_ss())[bc_fetch_with_stack,bump_pc_with_stack] >>
        fsrw_tac[ARITH_ss][] >>
        srw_tac[][bump_pc_def] >> rev_full_simp_tac(srw_ss())[] >>
        qpat_assum`bc_fetch X = Y` kall_tac >>
        qpat_assum`bc_fetch X = Y` kall_tac >>
      ...
      val tac3 =
        ...
        `REVERSE mtk = mtk` by srw_tac[][Abbr`mtk`] >>
        disch_then(qspec_then`bc1`mp_tac) >> simp[] >>
        `(LENGTH exps = LENGTH bvs) ∧ (LENGTH klvs = LENGTH blvs)` by (
          full_simp_tac(srw_ss())[EVERY2_EVERY] >> imp_res_tac Cevaluate_list_LENGTH >> full_simp_tac(srw_ss())[] ) >>
        simp[]
      val tac20 =
        qmatch_assum_abbrev_tac`Cv_bv (ps',l2a) cl bf` >>
        `Cv_bv (rd'', l2a) cl bf` by (
      ...
      val tac21 =
        `ck' = SOME(FST (FST s'))` by (
          imp_res_tac RTC_bc_next_clock_less >>
          rev_full_simp_tac(srw_ss())[Cenv_bs_def,s_refs_def,optionTheory.OPTREL_def] >> full_simp_tac(srw_ss())[] ) >>
        BasicProvers.VAR_EQ_TAC >>
        qmatch_assum_abbrev_tac`bc_next^* bs bs05` >>
      ...
      fun tac23 st =
        ...
        discharge_hyps >- (
          ...
          conj_asm1_tac >- (
            ...
            simp[Abbr`bs1`] >>
            imp_res_tac RTC_bc_next_clock_less >>
            rev_full_simp_tac(srw_ss())[Abbr`bs05`,optionTheory.OPTREL_def,PRE_SUB1] >>
            rev_full_simp_tac(srw_ss())[Cenv_bs_def,s_refs_def] >> srw_tac[][] ) >>
          conj_tac >- (
            match_mp_tac Cenv_bs_bind_fv >>
            qexists_tac`ccenv`>>simp[]>>
            simp[Abbr`bs1`]>>
    ...
    in
      ...
      Cases_on`beh`>>full_simp_tac(srw_ss())[] >- (
        ...
        conj_tac >- (
          ...
          `LENGTH vs = LENGTH bvs` by metis_tac[EVERY2_EVERY] >> full_simp_tac(srw_ss())[] >>
          ...
          simp[] >>
          full_simp_tac(srw_ss())[s_refs_def,Abbr`l2a`,good_rd_def] >>
          simp[Abbr`bs0`,bc_state_component_equality] >>
          conj_tac >- metis_tac[SUBMAP_TRANS] >>
      ...
      Cases_on`t`>>simp[Once SWAP_REVERSE] >> strip_tac >- (
        conj_tac >- (
          ...
          full_simp_tac(srw_ss())[] >>
          rev_full_simp_tac(srw_ss())[Abbr`bs2`] >>
          metis_tac[IS_PREFIX_TRANS,SUBMAP_TRANS,gvrel_trans]) >>
        srw_tac[][] >>
        qmatch_assum_abbrev_tac`Cevaluate ((cka,s''),g) env' exp' Z` >> qunabbrev_tac`Z` >>
      ...
      conj_tac >- (
        ...
        full_simp_tac(srw_ss())[] >> rev_full_simp_tac(srw_ss())[Abbr`bs2`] >>
        metis_tac[IS_PREFIX_TRANS,SUBMAP_TRANS,gvrel_trans]) >>
      simp[GSYM AND_IMP_INTRO] >> strip_tac >>
      tac20 >>
      strip_tac >>
  ...
  strip_tac >- (
    ...
    reverse(Cases_on`∃bc10. code = REVERSE cx ++ REVERSE cxs ++ bc10`) >- (
      fsrw_tac[DNF_ss][] >>
      rpt conj_tac >> rpt gen_tac >>
      srw_tac[][FOLDL_emit_append_out,Once SWAP_REVERSE] >>
      TRY(Cases_on`t`)>>full_simp_tac(srw_ss())[Once SWAP_REVERSE]) >> full_simp_tac(srw_ss())[] >>
    reverse(Cases_on`bs.code = bc0 ++ REVERSE cx ++ REVERSE cxs ++ bc10 ++ bc1`) >- full_simp_tac(srw_ss())[] >>
    last_x_assum(qspecl_then[`rd`,`cs`,`cenv`,`sz`,`bs`,`bce`,`bcr`,`bc0`,`REVERSE cx`]mp_tac) >>
    ...
    `ck''=SOME 0` by(
      imp_res_tac RTC_bc_next_clock_less >>
      rev_full_simp_tac(srw_ss())[optionTheory.OPTREL_def,Cenv_bs_def,s_refs_def] ) >>
    BasicProvers.VAR_EQ_TAC >>
    qmatch_assum_abbrev_tac`Cv_bv (ps',l2a) cl bf` >>
    `Cv_bv (rd'', l2a) cl bf` by (
      match_mp_tac (MP_CANON (GEN_ALL (CONJUNCT1 (SPEC_ALL Cv_bv_SUBMAP)))) >> simp[] >>
    ...
    simp[Abbr`cl`,Once Cv_bv_cases,el_check_def] >>
    disch_then(qx_choosel_then[`a`,`azb`,`bve`,`cd`,`envs`,`l`,`recs`]strip_assume_tac) >>
    full_simp_tac(srw_ss())[FOLDL_emit_append_out,Once SWAP_REVERSE] >>
    ...
    full_simp_tac(srw_ss())[Abbr`bs1`,Abbr`bs1'`] >> pop_assum kall_tac >>
  ...
  strip_tac >- (
    ...
    reverse(Cases_on`∃bc10. code = REVERSE cx ++ REVERSE cxs ++ bc10`) >- (
      fsrw_tac[DNF_ss][] >>
      rpt gen_tac >>
      srw_tac[][Once SWAP_REVERSE,FOLDL_emit_append_out] >>
      Cases_on`t`>>full_simp_tac(srw_ss())[Once SWAP_REVERSE]) >> full_simp_tac(srw_ss())[] >>
    reverse(Cases_on`bs.code = bc0 ++ REVERSE cx ++ REVERSE cxs ++ bc10 ++ bc1`) >- full_simp_tac(srw_ss())[] >>
    full_simp_tac(srw_ss())[Once SWAP_REVERSE] >>
    full_simp_tac(srw_ss())[Once SWAP_REVERSE] >>
    last_x_assum(qspecl_then[`rd`,`cs`,`cenv`,`sz`,`bs`,`bce`,`bcr`,`bc0`,`REVERSE cx`]mp_tac) >>
  ...
  strip_tac >- (
    ...
    qspecl_then[`cenv`,`TCNonTail`,`sz`,`cs`,`exp`](Q.X_CHOOSE_THEN`cx`strip_assume_tac)(CONJUNCT1 compile_append_out) >> full_simp_tac(srw_ss())[] >>
    Q.PAT_ABBREV_TAC`cs0 = compile cenv X sz cs exp` >>
    qspecl_then[`cenv`,`sz+1`,`cs0`,`es`](Q.X_CHOOSE_THEN`cxs`strip_assume_tac)(CONJUNCT2 (CONJUNCT2 compile_append_out)) >> full_simp_tac(srw_ss())[] >>
    reverse(Cases_on`∃bc10. code = REVERSE cx ++ REVERSE cxs ++ bc10`) >- (
      fsrw_tac[DNF_ss][FOLDL_emit_append_out] >>
      conj_tac >> rpt gen_tac >>
      Cases_on`t`>>srw_tac[][]>>full_simp_tac(srw_ss())[Once SWAP_REVERSE]) >> full_simp_tac(srw_ss())[] >>
    reverse(Cases_on`bs.code = bc0 ++ REVERSE cx ++ REVERSE cxs ++ bc10 ++ bc1`) >- full_simp_tac(srw_ss())[] >>
    full_simp_tac(srw_ss())[Once SWAP_REVERSE] >>
    full_simp_tac(srw_ss())[Once SWAP_REVERSE] >>
    last_x_assum(qspecl_then[`rd`,`cs`,`cenv`,`sz`,`bs`,`bce`,`bcr`,`bc0`,`REVERSE cx`]mp_tac) >>
  ...
  strip_tac >- (
    ...
    reverse(Cases_on`∃bc10. code = REVERSE bc ++ prim1_to_bc uop ++ bc10`) >- (
      srw_tac[][] >> full_simp_tac(srw_ss())[FOLDL_emit_append_out] >> rev_full_simp_tac(srw_ss())[] >>
      TRY(Cases_on`t`)>>full_simp_tac(srw_ss())[pushret_def,Once SWAP_REVERSE]) >>
    full_simp_tac(srw_ss())[] >>
    ...
    Cases_on`res`>>full_simp_tac(srw_ss())[] >- (
      ...
      `P` by (
        ...
        simp[Abbr`bs1`] >>
        discharge_hyps >- full_simp_tac(srw_ss())[Cenv_bs_def] >>
        ...
        map_every qexists_tac[`rd`,`s`,`bs with <| code := bce|>`]>>
        simp[bc_state_component_equality] >>
        full_simp_tac(srw_ss())[Cenv_bs_def,s_refs_def,good_rd_def,UNCURRY,FEVERY_DEF,SUBSET_DEF] >>
        metis_tac[SUBMAP_TRANS,IS_PREFIX_TRANS,gvrel_trans] ) >>
    ...
    Cases_on`uop`>>full_simp_tac(srw_ss())[]>>
    Cases_on`e`>>simp[]>>
    Cases_on`v`>>full_simp_tac(srw_ss())[] >> srw_tac[][]>>
    BasicProvers.EVERY_CASE_TAC >> full_simp_tac(srw_ss())[] >>
    Cases_on`l`>>full_simp_tac(srw_ss())[] >>
    BasicProvers.EVERY_CASE_TAC >> full_simp_tac(srw_ss())[]) >>
  strip_tac >- (
    ...
    Q.PAT_ABBREV_TAC`cs1 = compiler_result_out_fupd X Y` >>
    qspecl_then[`t`,`cs1`](Q.X_CHOOSE_THEN`cp`strip_assume_tac)pushret_append_out >> pop_assum kall_tac >>
    simp[Once SWAP_REVERSE,Abbr`cs1`] >> srw_tac[][] >>
    first_x_assum (qspecl_then[`rd`,`cs`,`cenv`,`sz`,`bs`,`bce`,`bcr`,`bc0`,`REVERSE cx`]mp_tac) >>
    simp[] >>
    disch_then(qspec_then`TCNonTail`mp_tac)>>simp[]) >>
  strip_tac >- (
    ...
    reverse conj_tac >- (
      ...
      Cases_on`op`>>simp[]>>
      Cases_on`v2`>>TRY(Cases_on`l`)>>simp[]>>srw_tac[][]>>
      Cases_on`v1`>>TRY(Cases_on`l`)>>full_simp_tac(srw_ss())[semanticPrimitivesTheory.lit_same_type_def]>>srw_tac[][]>>
    ...
    simp[FORALL_AND_THM]>>
    conj_asm1_tac >- (
      srw_tac[][compile_def,LET_THM,pushret_def] >>
      ...
      `∃bv0 bv1. bvs = [bv0;bv1]` by (
        Cases_on `bvs` >> full_simp_tac(srw_ss())[] >>
        Cases_on `t` >> full_simp_tac(srw_ss())[LENGTH_NIL] ) >> full_simp_tac(srw_ss())[] >> srw_tac[][] >>
      fsrw_tac[DNF_ss][] >>
      qmatch_assum_abbrev_tac `bc_next^* bs bs0` >>
      ...
      full_simp_tac(srw_ss())[Abbr`bs0`] >>
      `FST pp = sms` by srw_tac[][Abbr`pp`] >> full_simp_tac(srw_ss())[] >>
      ...
      simp[] >>
      discharge_hyps >- (
        full_simp_tac(srw_ss())[Cenv_bs_def,Abbr`pp`] >>
      ...
      map_every qexists_tac [`rf1`,`rd1`,`cks`,`gvs`,`bv`] >> full_simp_tac(srw_ss())[] >>
      conj_tac >- (
        srw_tac[][Once RTC_CASES2] >> disj2_tac >>
        qmatch_assum_abbrev_tac `bc_next^* bs bs0` >>
        qexists_tac `bs0` >> srw_tac[][] >>
      ...
      HINT_EXISTS_TAC >>
      simp[Abbr`bs0`] ) >>
    full_simp_tac(srw_ss())[Once compile_def,LET_THM,pushret_def] >>
    ...
    full_simp_tac(srw_ss())[Once SWAP_REVERSE] >> srw_tac[][] >> full_simp_tac(srw_ss())[] >>
    match_mp_tac code_for_push_return >>
    first_assum(match_exists_tac o concl) >>
    simp[] >>
    qexists_tac`REVERSE args`>>full_simp_tac(srw_ss())[EVERY2_EVERY]) >>
  strip_tac >- (
    simp[] >>
    rpt gen_tac >> strip_tac >>
    rpt gen_tac >> strip_tac >>
    ...
    Q.PAT_ABBREV_TAC`cs1 = compiler_result_out_fupd X Y` >>
    qspecl_then[`t`,`cs1`](Q.X_CHOOSE_THEN`cp`strip_assume_tac)pushret_append_out >> pop_assum kall_tac >>
    simp[Once SWAP_REVERSE,Abbr`cs1`,Abbr`cs0`] >> srw_tac[][] >>
  ...
  strip_tac >- (
    ...
    reverse conj_tac >- (
      ...
      Cases_on`b`>>Cases_on`v2`>>simp[CevalUpd_def]>>srw_tac[][]>>
      Cases_on`l`>>full_simp_tac(srw_ss())[]>>
      Cases_on`v1`>>full_simp_tac(srw_ss())[]>>
    ...
    conj_asm1_tac >- (
      ...
      Q.PAT_ABBREV_TAC`U = case b of UpB => X | _ => Update` >> srw_tac[][] >>
      qspecl_then[`cenv`,`TCNonTail`,`sz`,`cs`,`e1`](Q.X_CHOOSE_THEN`cx`strip_assume_tac)(CONJUNCT1 compile_append_out) >>
      qmatch_assum_abbrev_tac`cs0.out = cx ++ cs.out` >>
      qspecl_then[`cenv`,`TCNonTail`,`sz+1`,`cs0`,`e2`](Q.X_CHOOSE_THEN`cy`strip_assume_tac)(CONJUNCT1 compile_append_out) >>
      ...
      disch_then (qx_choosel_then [`bvs`,`rfs`,`sms`,`cks`,`gvs`] strip_assume_tac) >>
      full_simp_tac(srw_ss())[EVERY2_EVERY] >>
      `∃bv0 bv1 bv2. bvs = [bv0;bv1;bv2]` by (simp[]) >> full_simp_tac(srw_ss())[] >> srw_tac[][] >>
      fsrw_tac[DNF_ss][] >>
      qmatch_assum_abbrev_tac `bc_next^* bs bs0` >>
      `∃i n. v2 = CLitv (IntLit i) ∧ v1 = CLoc n ∧ v = CConv 0 []` by (
        Cases_on`b`>>Cases_on`v2`>>full_simp_tac(srw_ss())[]>>Cases_on`l`>>full_simp_tac(srw_ss())[]>>
        Cases_on`v1`>>full_simp_tac(srw_ss())[] >>
        fsrw_tac[ARITH_ss][conLangTheory.tuple_tag_def] >>
        qpat_assum`(X,Y) = Z`mp_tac >>
        ...
        Cases_on`l`>>simp[]>>
        rpt BasicProvers.CASE_TAC >>
        simp[conLangTheory.tuple_tag_def]) >>
      full_simp_tac(srw_ss())[Q.SPEC`CLitv l`(CONJUNCT1(SPEC_ALL Cv_bv_cases))]>>
      full_simp_tac(srw_ss())[Q.SPEC`CLoc n`(CONJUNCT1(SPEC_ALL Cv_bv_cases))]>>
      rpt BasicProvers.VAR_EQ_TAC >>
      simp[Q.SPEC`CConv x n`(CONJUNCT1(SPEC_ALL Cv_bv_cases))]>>
      CONV_TAC(RESORT_EXISTS_CONV List.rev) >>
      ...
      srw_tac[][RTC_eq_NRC,PULL_EXISTS] >>
      CONV_TAC SWAP_EXISTS_CONV >>
      qexists_tac`SUC(SUC(SUC 0))` >>
      srw_tac[DNF_ss][NRC] >>
      `bc_fetch bs0 = SOME U` by (
        ...
        simp[Abbr`bs0`] >>
        Cases_on`b`>>srw_tac[][Abbr`U`]) >>
      simp[bc_eval1_thm] >>
      Cases_on`b`>>full_simp_tac(srw_ss())[Abbr`U`]>>
      srw_tac[][Once bc_eval1_def] >-(
        simp[Abbr`bs0`] >>
        qpat_assum`(X,Y) = Z`mp_tac >>
        simp[] >>
        BasicProvers.CASE_TAC>>
        ...
        `∃v. FLOOKUP rfs p = SOME (ValueArray [v])` by (
          ...
          full_simp_tac(srw_ss())[EVERY_MEM] >> srw_tac[][] >> metis_tac[MEM_EL] ) >>
        simp[] >>
        simp[bump_pc_def] >>
        Q.PAT_ABBREV_TAC`bs1:bc_state = X Y` >>
        `bc_fetch bs1 = SOME(Stack(PushInt 0))` by (
          ...
          simp[Abbr`bs1`] >>
          CONV_TAC SWAP_EXISTS_CONV >>
          qexists_tac`Stack(Cons 3)::bc1`>>simp[] >>
          simp[FILTER_APPEND,SUM_APPEND] ) >>
        simp[Once bc_eval1_def,bump_pc_def,bc_eval_stack_def] >>
        pop_assum kall_tac >> simp[Abbr`bs1`] >>
        Q.PAT_ABBREV_TAC`bs1:bc_state = X Y` >>
        `bc_fetch bs1 = SOME(Stack(Cons unit_tag))` by (
          match_mp_tac bc_fetch_next_addr >>
          simp[Abbr`bs1`] >>
          CONV_TAC SWAP_EXISTS_CONV >>
          qexists_tac`bc1`>>simp[] >>
        ...
        simp[Abbr`bs1`,bc_state_component_equality] >>
        simp[bc_eval_stack_def] >>
        conj_tac >- (REWRITE_TAC[conLangTheory.tuple_tag_def] >> simp[]) >>
        conj_tac >- simp[FILTER_APPEND,SUM_APPEND] >>
        reverse conj_tac >- ( srw_tac[][] >> metis_tac[MEM_EL] ) >>
        ...
        `Cenv_bs sms s0 env cenv sz (bs0 with stack := bs.stack)` by (
          ...
          qexists_tac`bs0` >>
          simp[Abbr`bs0`,bc_state_component_equality] >>
          REWRITE_TAC[conLangTheory.tuple_tag_def] >> simp[] >>
          metis_tac[Cenv_bs_CTLet_bound] ) >>
        Q.PAT_ABBREV_TAC`bs1:bc_state = X Y` >>
        ...
        simp[Abbr`bs0`,Abbr`bs1`] >>
        conj_tac >- (
          full_simp_tac(srw_ss())[Cenv_bs_def,s_refs_def,Abbr`s0`] >>
          full_simp_tac(srw_ss())[good_rd_def,FLOOKUP_UPDATE] >>
          conj_tac >- (
            full_simp_tac(srw_ss())[FEVERY_DEF,UNCURRY] >>
            srw_tac[][] >> metis_tac[MEM_EL] ) >>
          conj_tac >- full_simp_tac(srw_ss())[EVERY_MEM] >>
          rator_x_assum`LIST_REL`kall_tac >>
          rator_x_assum`LIST_REL`mp_tac >>
          simp[LIST_REL_EL_EQN,EL_LUPDATE,EL_MAP,FAPPLY_FUPDATE_THM] >>
          srw_tac[][] >> srw_tac[][] >> srw_tac[][sv_refv_rel_cases,LUPDATE_def] >>
          simp_tac(srw_ss())[conLangTheory.tuple_tag_def,LUPDATE_def] >>
      ...
      >- (
        ...
        rev_full_simp_tac(srw_ss())[GSYM integerTheory.INT_ABS_EQ_ID] >>
        full_simp_tac(srw_ss())[el_check_def] >>
        `∃vs. FLOOKUP rfs p = SOME (ValueArray vs) ∧ LENGTH vs = LENGTH l` by (
          full_simp_tac(srw_ss())[Cenv_bs_def,s_refs_def] >>
          rator_x_assum`LIST_REL`kall_tac >>
          rator_x_assum`LIST_REL`mp_tac >>
          simp[LIST_REL_EL_EQN] >>
          disch_then(qspec_then`n`mp_tac) >>
          simp[sv_refv_rel_cases,EL_MAP,FLOOKUP_DEF] >>
          full_simp_tac(srw_ss())[EVERY_MEM] >> srw_tac[][] >> metis_tac[MEM_EL,LIST_REL_EL_EQN] ) >>
        simp[] >>
        simp[bump_pc_def] >>
        Q.PAT_ABBREV_TAC`bs1:bc_state = X Y` >>
        `bc_fetch bs1 = SOME(Stack(PushInt 0))` by (
        ...
        simp[Abbr`bs1`,bc_state_component_equality] >>
        simp[bc_eval_stack_def] >>
        conj_tac >- (simp_tac(srw_ss())[conLangTheory.tuple_tag_def]) >>
        conj_tac >- simp[FILTER_APPEND,SUM_APPEND] >>
        reverse conj_tac >- ( srw_tac[][] >> metis_tac[MEM_EL] ) >>
        Q.PAT_ABBREV_TAC`bs0:bc_state = X Y` >>
        match_mp_tac Cenv_bs_imp_incsz_irr >>
        ...
        `Cenv_bs sms s0 env cenv sz (bs0 with stack := bs.stack)` by (
          ...
          qexists_tac`bs0` >>
          simp[Abbr`bs0`,bc_state_component_equality] >>
          (simp_tac(srw_ss())[conLangTheory.tuple_tag_def]) >>
          simp[] >>
          metis_tac[Cenv_bs_CTLet_bound] ) >>
        Q.PAT_ABBREV_TAC`bs1:bc_state = X Y` >>
        `Cenv_bs sms s0 env cenv sz (bs1 with refs := bs0.refs)` by (
        ...
        conj_tac >- (
          ...
          srw_tac[][] >> srw_tac[][] >> srw_tac[][sv_refv_rel_cases,LUPDATE_def] >- (
            ...
            first_x_assum(qspec_then`n`mp_tac) >>
            simp[sv_refv_rel_cases,PULL_EXISTS] >>
            full_simp_tac(srw_ss())[FLOOKUP_DEF] ) >>
          metis_tac[EL_ALL_DISTINCT_EL_EQ,sv_refv_rel_def] ) >>
        srw_tac[][FDOM_DRESTRICT] >>
        metis_tac[MEM_EL] )
      >- (
        simp[Abbr`bs0`] >>
        ...
        `FLOOKUP rfs p = SOME (ByteArray ws)` by (
          ...
          disch_then(qspec_then`n`mp_tac) >>
          simp[sv_refv_rel_cases,EL_MAP,FLOOKUP_DEF] >>
          full_simp_tac(srw_ss())[EVERY_MEM] >> metis_tac[MEM_EL] ) >>
        simp[] >>
        rev_full_simp_tac(srw_ss())[integerTheory.INT_ABS,integerTheory.int_le,wordsTheory.w2n_lt] >>
        simp[bump_pc_def] >>
        Q.PAT_ABBREV_TAC`bs1:bc_state = X Y` >>
        `bc_fetch bs1 = SOME(Stack(PushInt 0))` by (
          match_mp_tac bc_fetch_next_addr >>
        ...
        simp[Abbr`bs1`,bc_state_component_equality] >>
        simp[bc_eval_stack_def] >>
        qpat_assum`0 = tuple_tag`(assume_tac o SYM)>>full_simp_tac(srw_ss())[] >>rev_full_simp_tac(srw_ss())[] >>
        conj_tac >- simp[FILTER_APPEND,SUM_APPEND] >>
        reverse conj_tac >- ( srw_tac[][] >> metis_tac[MEM_EL] ) >>
        Q.PAT_ABBREV_TAC`bs0:bc_state = X Y` >>
        match_mp_tac Cenv_bs_imp_incsz_irr >>
    ...
    simp[Once SWAP_REVERSE] >> srw_tac[][] >>
    rev_full_simp_tac(srw_ss())[Once SWAP_REVERSE] >>
    match_mp_tac code_for_push_return >>
    first_assum (match_exists_tac o concl) >> simp[] >>
    qexists_tac`REVERSE args`>>full_simp_tac(srw_ss())[EVERY2_EVERY]) >>
  strip_tac >- (
    ...
    qspecl_then[`t`,`cs2`](Q.X_CHOOSE_THEN`cp`strip_assume_tac)pushret_append_out >> pop_assum kall_tac >>
    simp[Once SWAP_REVERSE,Abbr`cs2`,Abbr`cs1`,Abbr`cs0`] >> srw_tac[][] >>
  ...
  strip_tac >- (
    ...
    full_simp_tac std_ss [prove(``w::x::y::cs0.out=[w;x;y]++cs0.out``,srw_tac[][])] >>
    full_simp_tac std_ss [prove(``x::y::(cs0).out=[x;y]++(cs0).out``,srw_tac[][])] >>
    ...
    reverse(Cases_on`∃bc10. code = REVERSE be1 ++ REVERSE lc2 ++ bc10`) >- (
      ...
      simp[Abbr`cs3`] >> rev_full_simp_tac(srw_ss())[] >>
    ...
    full_simp_tac(srw_ss())[] >>
    disch_then(CHOOSE_THEN strip_assume_tac o SIMP_RULE (srw_ss()) [code_for_push_def,LET_THM,Once Cv_bv_cases,CBoolv_def] o CONJUNCT1) >>
    qmatch_assum_abbrev_tac`bc_next^* bs bs1` >>
    let
      val tac1 =
        srw_tac[][] >>
      ...
      val tac2 =
        ...
        discharge_hyps >- (
          qspecl_then[`s`,`env`,`exp`,`(s',Rval (CBoolv T))`]mp_tac(CONJUNCT1 Cevaluate_vlabs) >>
          simp[] >> strip_tac >>
          qspecl_then[`s`,`env`,`exp`,`(s',Rval (CBoolv T))`]mp_tac(CONJUNCT1 Cevaluate_all_vlabs) >>
          simp[] >> strip_tac >>
          conj_tac >- ( fsrw_tac[DNF_ss][SUBSET_DEF,EVERY_MEM,vlabs_csg_def] >> metis_tac[] ) >>
          conj_tac >- ( fsrw_tac[DNF_ss][SUBSET_DEF,EVERY_MEM,vlabs_csg_def] >> metis_tac[] ) >>
          conj_tac >- (
      ...
      val tac6 =
        ...
        `bc_fetch bs1 = SOME (JumpIf (Lab nl))` by (
           ...
           srw_tac[][]) >>
        srw_tac[][bc_eval1_def,Abbr`bs1`,LET_THM] >>
        srw_tac[][Abbr`bs2`,bc_state_component_equality,conLangTheory.true_tag_def] >>
        srw_tac[][bc_find_loc_def] >>
        qmatch_abbrev_tac`bc_find_loc_aux ls il nl 0 = SOME (next_addr il ls0)` >>
        `∃ls1. ls = ls0 ++ ls1` by srw_tac[][Abbr`ls`,Abbr`ls0`] >>
      ...
      val tac7 =
        ...
        `bc_fetch bs1 = SOME (JumpIf (Lab nl))` by (
           ...
           srw_tac[][]) >>
        srw_tac[][Once bc_eval1_def] >>
        srw_tac[][Abbr`bs1`,LET_THM] >>
        srw_tac[DNF_ss][] >>
        simp[LEFT_EXISTS_AND_THM,conLangTheory.false_tag_def] >>
        conj_tac >- (
          qmatch_abbrev_tac`∃n. X = SOME n` >>
          qsuff_tac`~(X = NONE)` >- ( Cases_on`X`>>srw_tac[][]) >>
          qunabbrev_tac`X` >>
          simp[bc_find_loc_def] >>
          spose_not_then strip_assume_tac >>
          imp_res_tac bc_find_loc_aux_NONE >>
          full_simp_tac(srw_ss())[] >> rev_full_simp_tac(srw_ss())[Abbr`lc2`] ) >>
        ...
        `bc_fetch bs1 = SOME (Jump (Lab (nl+1)))` by (
          ...
          srw_tac[ARITH_ss][FILTER_APPEND,SUM_APPEND] ) >>
        srw_tac[][bc_eval1_def] >>
        srw_tac[][Abbr`bs2`,Abbr`bs1`,bc_state_component_equality] >>
        srw_tac[][bc_find_loc_def] >>
        qmatch_abbrev_tac`bc_find_loc_aux ls il nl1 0 = SOME (next_addr il ls2)` >>
        `∃ls1. ls = ls2 ++ ls1` by (
          srw_tac[][Abbr`ls2`,Abbr`ls`] ) >>
        pop_assum SUBST1_TAC >>
        match_mp_tac bc_find_loc_aux_append_code >>
        match_mp_tac bc_find_loc_aux_ALL_DISTINCT >>
      ...
      val tac5 =
        ...
        `ck = SOME (FST (FST s'))` by (
          ...
          rev_full_simp_tac(srw_ss())[optionTheory.OPTREL_def,Cenv_bs_def,s_refs_def] ) >>
        discharge_hyps >- (
          qmatch_assum_abbrev_tac`Cevaluate s env exp (s',Rval(CBoolv F))` >>
          qspecl_then[`s`,`env`,`exp`,`(s',Rval (CBoolv F))`]mp_tac(CONJUNCT1 Cevaluate_vlabs) >>
          simp[] >> strip_tac >>
          qspecl_then[`s`,`env`,`exp`,`(s',Rval (CBoolv F))`]mp_tac(CONJUNCT1 Cevaluate_all_vlabs) >>
          simp[] >> strip_tac >>
    ...
    in
      conj_tac >- (
        tac1 >>
        Cases_on `b1` >> full_simp_tac(srw_ss())[] >- (
          tac2 >>
          disch_then(mp_tac o CONJUNCT1) >>
          ...
          `bc_next^* bs03 bs05` by (
            ...
            `bc_fetch bs03 = SOME (JumpIf (Lab nl))` by (
              ...
              srw_tac[][]) >>
            srw_tac[][bc_eval1_thm] >>
            srw_tac[][bc_eval1_def,Abbr`bs03`,LET_THM] >>
            simp[Abbr`bs05`,bc_state_component_equality,conLangTheory.true_tag_def] >>
            reverse conj_tac >- (
              imp_res_tac RTC_bc_next_clock_less >>
              rev_full_simp_tac(srw_ss())[optionTheory.OPTREL_def,Cenv_bs_def,s_refs_def] ) >>
            srw_tac[][bc_find_loc_def] >>
          ...
          conj_tac >- (
            ...
            `bc_next bs11 bs13` by (
              srw_tac[][bc_eval1_thm] >>
              srw_tac[][bc_eval1_def,Abbr`bs13`,Abbr`bs11`] >>
              srw_tac[][bc_state_component_equality] >>
              srw_tac[][bc_find_loc_def] >>
        ...
        `bc_next^* bs03 bs05` by (
          ...
          srw_tac[][bc_eval1_thm] >>
          srw_tac[][Once bc_eval1_def] >>
          srw_tac[][Abbr`bs03`,LET_THM] >>
          srw_tac[DNF_ss][conLangTheory.false_tag_def] >>
          srw_tac[][bc_find_loc_def] >>
          srw_tac[][LEFT_EXISTS_AND_THM] >- (
            qmatch_abbrev_tac`∃n. X = SOME n` >>
            qsuff_tac `X ≠ NONE` >- (Cases_on`X` >> srw_tac[][]) >>
            qunabbrev_tac`X` >>
          ...
          `bc_fetch bs03 = SOME (JumpIf (Lab nl))` by (
            full_simp_tac(srw_ss())[bc_fetch_def,Abbr`bs03`] >> rev_full_simp_tac(srw_ss())[REVERSE_APPEND] ) >>
          srw_tac[][bump_pc_def] >>
          srw_tac[][Abbr`bs03`] >>
          ...
          `bc_fetch bs03 = SOME (Jump (Lab (nl + 1)))` by (
            ...
            srw_tac[][Abbr`bs03`,Abbr`lc2`] >>
            qexists_tac`bc0 ++ REVERSE be1 ++ [JumpIf (Lab nl)]` >>
            srw_tac[][REVERSE_APPEND,FILTER_APPEND,SUM_APPEND] >>
            srw_tac[ARITH_ss][] ) >>
          srw_tac[][bc_eval1_def] >>
          srw_tac[][Abbr`bs03`,Abbr`bs06`,Abbr`bs05`] >>
          simp[bc_state_component_equality] >>
          reverse conj_tac >- (
            imp_res_tac RTC_bc_next_clock_less >>
            rev_full_simp_tac(srw_ss())[optionTheory.OPTREL_def,Cenv_bs_def,s_refs_def] ) >>
        ...
        conj_tac >- (
          ...
          `bs08 = bs07` by (
            srw_tac[][Abbr`bs08`,Abbr`bs07`] >>
            srw_tac[][bc_state_component_equality] >>
            srw_tac[][FILTER_APPEND] ) >>
          metis_tac[RTC_TRANSITIVE,transitive_def] ) >>
        simp[] >>
        conj_tac >- (
          match_mp_tac Cenv_bs_with_irr >>
      ...
      conj_tac >- (
        ...
        Cases_on `b1` >> full_simp_tac(srw_ss())[] >- (
          ...
          `ck = SOME (FST (FST s'))` by (
            ...
            rev_full_simp_tac(srw_ss())[optionTheory.OPTREL_def,Cenv_bs_def,s_refs_def] ) >>
          discharge_hyps >- (
            qmatch_assum_abbrev_tac`Cevaluate s env exp (s',Rval(CBoolv T))` >>
            qspecl_then[`s`,`env`,`exp`,`(s',Rval (CBoolv T))`]mp_tac(CONJUNCT1 Cevaluate_vlabs) >>
            simp[] >> strip_tac >>
            qspecl_then[`s`,`env`,`exp`,`(s',Rval (CBoolv T))`]mp_tac(CONJUNCT1 Cevaluate_all_vlabs) >>
            simp[] >>
          ...
          full_simp_tac(srw_ss())[Abbr`tt`,Once SWAP_REVERSE] >>
          simp[Abbr`cs2`] >>
          ...
          discharge_hyps >- (
            ...
            qexists_tac`P`>>srw_tac[][Abbr`P`,Abbr`Q`] >>
            match_mp_tac (GEN_ALL (MP_CANON (CONJUNCT1 (SPEC_ALL Cv_bv_SUBMAP)))) >>
            simp[] >>
            qexists_tac`rd` >>
            simp[] >>
            full_simp_tac(srw_ss())[SUBMAP_DEF,DRESTRICT_DEF,s_refs_def,good_rd_def,Cenv_bs_def,UNCURRY,FEVERY_DEF] )) >>
          simp[code_for_return_def] >>
          simp_tac(srw_ss()++DNF_ss)[] >>
          tac3) >>
        qmatch_assum_abbrev_tac`(compile cenv tt sz cs3 e3).out = X` >> qunabbrev_tac`X` >>
        ...
        discharge_hyps >- (
          ...
          qexists_tac`rd` >>
          simp[] >>
          full_simp_tac(srw_ss())[SUBMAP_DEF,DRESTRICT_DEF,good_rd_def,s_refs_def,Cenv_bs_def,UNCURRY,FEVERY_DEF] )) >>
        tac4) >>
      Cases >> rpt gen_tac >> full_simp_tac(srw_ss())[] >> strip_tac >- (
        tac1 >- (
          Cases_on `b1` >> full_simp_tac(srw_ss())[] >- (
            tac2 >>
  ...
  strip_tac >- (
    ...
    Q.PAT_ABBREV_TAC`lc3 = [Label x;y]` >>
    Q.PAT_ABBREV_TAC`lc2 = [Label x;y;z]` >>
    BasicProvers.VAR_EQ_TAC >> simp[] >> full_simp_tac(srw_ss())[] >>
    gen_tac >>
  ...
  strip_tac >- (
    ...
    conj_asm1_tac >- (
      ...
      `bc_fetch bs1 = SOME(Stack(PushInt 0))` by (
        ...
        simp[FILTER_APPEND,SUM_APPEND] ) >>
      simp[bc_eval1_thm] >>
      simp[Once bc_eval1_def,bump_pc_def] >>
      simp[bc_eval_stack_def] >>
      simp[Abbr`bs1`] >>
      Q.PAT_ABBREV_TAC`bs1:bc_state  = X Y` >>
      `bc_fetch bs1 = SOME(Stack(Cons unit_tag))` by (
  ...
  strip_tac >- (
    ...
    map_every qexists_tac[`bs.refs`,`rd`,`bs.clock`,`bs.globals`] >>
    srw_tac[][RTC_eq_NRC] >>
    TRY (qexists_tac`0` >>srw_tac[][]) >>
    TRY (qmatch_abbrev_tac `Cenv_bs rd s B C D E` >>
         qmatch_assum_abbrev_tac `Cenv_bs rd s B C D E'` >>
         qsuff_tac`E = E'`>-srw_tac[][] >>
         unabbrev_all_tac) >>
    srw_tac[][bc_state_component_equality]) >>
  strip_tac >- (
    rpt gen_tac >> strip_tac >>
    rpt gen_tac >> strip_tac >>
    full_simp_tac(srw_ss())[] >> BasicProvers.VAR_EQ_TAC >> simp[] >>
    pop_assum mp_tac >>
    simp[compile_def] >>
    qspecl_then[`cenv`,`TCNonTail`,`sz`,`cs`,`exp`]mp_tac(CONJUNCT1 compile_append_out) >>
    ...
    discharge_hyps >- (
      ...
      conj_tac >- ( fsrw_tac[DNF_ss][SUBSET_DEF,EVERY_MEM,vlabs_csg_def] >> metis_tac[] ) >>
      conj_tac >- ( fsrw_tac[DNF_ss][SUBSET_DEF,EVERY_MEM,vlabs_csg_def] >> metis_tac[] ) >>
      match_mp_tac compile_labels_lemma >> full_simp_tac(srw_ss())[Abbr`cs0`] >>
    ...
    conj_tac >- (
      ...
      srw_tac[][Abbr`bs2`,Abbr`bs3`,bc_state_component_equality,REVERSE_APPEND] ) >>
    qpat_assum`X = vs'`(assume_tac o SYM) >>
    fsrw_tac[ARITH_ss][EVERY2_EVERY,ADD1] >> rev_full_simp_tac(srw_ss())[] >>
    conj_tac >- (
      match_mp_tac (MP_CANON (GEN_ALL (CONJUNCT1 (SPEC_ALL Cv_bv_SUBMAP)))) >> simp[] >>
      qexists_tac `rd'` >>
      simp[] >>
      full_simp_tac(srw_ss())[SUBMAP_DEF,DRESTRICT_DEF,Cenv_bs_def,s_refs_def,good_rd_def,UNCURRY,FEVERY_DEF] ) >>
    conj_tac >- (
      qmatch_abbrev_tac `Cenv_bs rd'' s2 renv env0 sz0 bsx` >>
      qmatch_assum_abbrev_tac `Cenv_bs rd'' s2 renv env0 sz0 bsy` >>
      `bsx = bsy` by (
        srw_tac[][Abbr`bsx`,Abbr`bsy`,bc_state_component_equality,REVERSE_APPEND] ) >>
      srw_tac[][] ) >>
    metis_tac[SUBMAP_TRANS,IS_PREFIX_TRANS,gvrel_trans]) >>
  strip_tac >- (
    rpt gen_tac >> strip_tac >>
