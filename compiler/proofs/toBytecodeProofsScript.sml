...
val Cenv_bs_def = Define`
  Cenv_bs rd Cmenv s Cenv (menv:string|->num list) (renv:ctenv) sz csz bs ⇔
    env_renv rd sz bs Cenv renv ∧
    fmap_rel (env_renv rd 0 (bs with stack := DROP csz (bs.stack))) Cmenv (MAP CTDec o_f menv) ∧
    s_refs rd s bs`
...
val Cenv_bs_imp_incsz = store_thm("Cenv_bs_imp_incsz",
  ...
  rw[Cenv_bs_def,s_refs_def,good_rd_def,env_renv_def,fmap_rel_def] >> rw[] >> fs[] >>
  TRY (
  ...
  simp[EVERY2_EVERY,EVERY_MEM,FORALL_PROD,GSYM AND_IMP_INTRO,MEM_ZIP] >>
  simp_tac(srw_ss()++DNF_ss)[] >>
  simp[EL_MAP,el_check_def] >>
  gen_tac >> strip_tac >>
  disch_then(qspec_then`n`mp_tac) >>
  simp[AND_IMP_INTRO,CONJ_SYM] >>
  simp[GSYM AND_IMP_INTRO] >>
  BasicProvers.CASE_TAC >>
  simp[ADD1,EL_APPEND1])
...
val Cenv_bs_imp_decsz = store_thm("Cenv_bs_imp_decsz",
  ``∀rd menv s env rmenv renv rsz bs bs'.
    Cenv_bs rd menv s env rmenv renv (rsz+1) bs ∧
      (CTLet (rsz+1) ∉ set renv) ∧
      (CTDec (LENGTH bs'.stack) ∉ set renv) ∧
      (LENGTH bs'.stack ∉ BIGUNION (IMAGE set (FRANGE rmenv))) ∧
      (∃v p e. bs = bs' with <| stack := v::bs'.stack; pc := p; handler := e |>) ⇒
    Cenv_bs rd menv s env rmenv renv rsz bs'``,
  ...
  first_x_assum(qspec_then`x`mp_tac)>>
  simp[env_renv_def,EVERY2_MAP,el_check_def] >>
  strip_tac >>
  match_mp_tac(GEN_ALL(MP_CANON EVERY2_MEM_MONO)) >>
  HINT_EXISTS_TAC >> simp[ADD1,FORALL_PROD] >>
  fs[EVERY2_EVERY,MEM_ZIP,GSYM LEFT_FORALL_IMP_THM,GSYM AND_IMP_INTRO] >>
  gen_tac >> strip_tac >> strip_tac >>
  BasicProvers.CASE_TAC >>
  ntac 2 (pop_assum mp_tac) >>
  BasicProvers.CASE_TAC >>
  simp[AND_IMP_INTRO,CONJ_SYM] >>
  simp[GSYM AND_IMP_INTRO,EL_APPEND1] >>
  strip_tac >> spose_not_then strip_assume_tac >>
  `EL n (rmenv ' x) = LENGTH bs'.stack` by DECIDE_TAC >>
  `MEM (LENGTH bs'.stack) (rmenv ' x)` by metis_tac[MEM_EL] >>
  fs[IN_FRANGE] >> metis_tac[])
...
val Cenv_bs_CTLet_bound = store_thm("Cenv_bs_CTLet_bound",
  ``Cenv_bs rd menv s env rmenv renv rsz bs ∧ (CTLet n) ∈ set renv ⇒ n ≤ rsz``,
...
val Cenv_bs_CTDec_bound = store_thm("Cenv_bs_CTDec_bound",
  ``Cenv_bs rd menv s env rmenv renv rsz bs ∧ ((CTDec n) ∈ set renv ∨ n ∈ BIGUNION (IMAGE set (FRANGE rmenv))) ⇒ n < LENGTH bs.stack``,
  rw[Cenv_bs_def,fmap_rel_def,IN_FRANGE] >>
...
val Cenv_bs_pops = store_thm("Cenv_bs_pops",
  ``∀vs rd menv s env rmenv renv rsz bs bs'.
    ...
    (∀n. n ∈ BIGUNION (IMAGE set (FRANGE rmenv)) ⇒ n < LENGTH bs'.stack) ∧
  ...
  fsrw_tac[DNF_ss][] >> TRY(PROVE_TAC[])>>
  match_mp_tac Cenv_bs_imp_decsz >>
  qmatch_assum_abbrev_tac`Cenv_bs rd menv s env rmenv renv x y` >>
  qexists_tac`y` >>
  unabbrev_all_tac >>
...
val Cenv_bs_with_pc = store_thm("Cenv_bs_with_pc",
  ``Cenv_bs rd menv s rmenv env env0 sz0 (bs with pc := p) = Cenv_bs rd menv s rmenv env env0 sz0 bs``,
  rw[Cenv_bs_def,env_renv_def,fmap_rel_def,s_refs_with_pc])
...
val Cenv_bs_append_code = store_thm("Cenv_bs_append_code",
  ``∀rd menv s rmenv env env0 sz0 bs bs' ls.
    Cenv_bs rd menv s rmenv env env0 sz0 bs ∧ (bs' = (bs with code := bs.code ++ ls)) ⇒
    Cenv_bs rd menv s rmenv env env0 sz0 bs'``,
...
val Cenv_bs_FUPDATE = store_thm("Cenv_bs_FUPDATE",
  ``∀rd menv s env env0 rmenv sz0 bs v bv bs'.
    Cenv_bs rd menv s env rmenv env0 sz0 bs ∧
    Cv_bv (mk_pp rd bs') v bv ∧
    (bs' = bs with stack := bv::bs.stack)
    ⇒
    Cenv_bs rd menv s (v::env) rmenv ((CTLet (sz0 + 1))::env0) (sz0 + 1) bs'``,
  reverse(reverse(rw[Cenv_bs_def,s_refs_def]) >> fs[el_check_def] >- fs[good_rd_def]) >- (
    ...
    imp_res_tac lookup_ct_imp_incsz >>
    simp[] ) >>
  fsrw_tac[DNF_ss][fmap_rel_def] >> rw[] >>
  first_x_assum(qspec_then`x`mp_tac) >>
  simp[] >>
  simp[env_renv_def] >> strip_tac >>
  match_mp_tac(GEN_ALL(MP_CANON EVERY2_MEM_MONO)) >>
  HINT_EXISTS_TAC >> simp[ADD1,FORALL_PROD] >>
  fs[EVERY2_EVERY,MEM_ZIP,GSYM LEFT_FORALL_IMP_THM,GSYM AND_IMP_INTRO,EL_MAP,el_check_def,ADD1] >>
  gen_tac >> strip_tac >> strip_tac >>
  BasicProvers.CASE_TAC >>
  ntac 2 (pop_assum mp_tac) >>
  BasicProvers.CASE_TAC >>
...
val Cenv_bs_FUPDATE_LIST = store_thm("Cenv_bs_FUPDATE_LIST",
  ``∀vs rd menv s env rmenv cenv sz bs bs bvs bs' env' cenv' sz'.
  Cenv_bs rd menv s env rmenv cenv sz bs ∧
...
val Cenv_bs_DOMSUB = store_thm("Cenv_bs_DOMSUB",
  ``∀rd menv s env rmenv renv rsz bs.
    Cenv_bs rd menv s env rmenv renv rsz bs ∧ (env ≠ []) ∧ (renv ≠ []) ⇒
    Cenv_bs rd menv s (TL env) rmenv (TL renv) rsz bs``,
...
val env_renv_change_store = store_thm("env_renv_change_store",
  ``env_renv rd rsz bs env renv ∧
    s_refs rd s bs ∧
    s_refs rd' s' (bs with refs := rfs') ∧
    rd.sm ≼ rd'.sm ∧ rd.cls ⊑ rd'.cls ∧
    DRESTRICT bs.refs (COMPL (set rd.sm)) ⊑ DRESTRICT rfs' (COMPL (set rd'.sm))
    ⇒
    env_renv rd' rsz (bs with refs := rfs') env renv``,
...
val Cenv_bs_change_store = store_thm("Cenv_bs_change_store",
  ``∀rd menv s env rmenv renv rsz bs rd' s' bs' rfs'.
    Cenv_bs rd menv s env rmenv renv rsz bs ∧
    s_refs rd' s' bs' ∧
    (bs' = bs with refs := rfs') ∧
    DRESTRICT bs.refs (COMPL (set rd.sm)) ⊑ DRESTRICT rfs' (COMPL (set rd'.sm)) ∧
...
val code_for_push_def = Define`
  code_for_push rd menv bs bce bc0 code s s' env vs rmenv renv rsz =
    ∃bvs rf rd'.
    ...
    Cenv_bs rd' menv s' env rmenv renv (rsz+(LENGTH vs)) (bs' with code := bce) ∧
...
val code_for_push_return = store_thm("code_for_push_return",
  ``∀rd menv bs bce bc0 code s s' env v rmenv renv rsz bc1 args args1 bs' blvs benv st cl cl1 ret hdl.
    code_for_push rd menv bs bce bc0 code s s' env [v] rmenv renv rsz ∧
...
val Cenv_bs_bind_fv = store_thm("Cenv_bs_bind_fv",
  ``∀menv rmenv cenv ccenv benv bve ret bvs a st fenv defs ix recs envs env pp rd bs s vs az e l e'.
    fmap_rel (env_renv rd 0 bs) menv (MAP CTDec o_f rmenv) ∧
    ...
    ⇒ Cenv_bs rd menv s env rmenv cenv 0 bs``,
  rw[Cenv_bs_def] >>
...
val Cenv_bs_with_irr = store_thm("Cenv_bs_with_irr",
  ``∀rd menv s env rmenv cenv sz bs bs'.
    Cenv_bs rd menv s env rmenv cenv sz bs ∧
...
val compile_val = store_thm("compile_val",
  ...
  strip_tac >- (
    ...
    conj_tac >- (
      ...
      first_x_assum(qspecl_then[`rd`,`cmnv`,`cs0`,`cenv`,`sz+2`,`bss0`,`bce`,`bcr`,`TAKE (LENGTH bc0+2) bs.code`,`REVERSE cc`
      ...
      discharge_hyps >- (
        ...
        conj_tac >- (
          ...
          qmatch_abbrev_tac`Cenv_bs rd menv s env cmnv cenv (sz + 1 + 1) bs0` >>
          match_mp_tac Cenv_bs_imp_incsz >>
          qexists_tac`bs0 with stack := TL bs0.stack` >>
          simp[Abbr`bs0`,bc_state_component_equality] >>
          qmatch_abbrev_tac`Cenv_bs rd menv s env cmnv cenv (sz + 1) bs0` >>
          match_mp_tac Cenv_bs_imp_incsz >>
          qexists_tac`bs0 with stack := TL bs0.stack` >>
          simp[Abbr`bs0`,bc_state_component_equality] >>
      ...
      conj_tac >- (
        ...
        rw[] >> spose_not_then strip_assume_tac >> res_tac >> DECIDE_TAC ) >>
      qmatch_assum_abbrev_tac`Cenv_bs rd' menv s2 env cmnv cenv (sz + 3) bs0` >>
      `Cenv_bs rd' menv s2 env cmnv cenv sz (bs0 with stack := DROP 3 bs0.stack)` by (
        match_mp_tac Cenv_bs_pops >>
        qexists_tac`TAKE 3 bs0.stack` >>
        simp[Abbr`bs0`] >>
        HINT_EXISTS_TAC >>
        simp[bc_state_component_equality] >>
        `(bs with code := bce).stack = bs.stack` by rw[] >>
        metis_tac[Cenv_bs_CTLet_bound,SIMP_RULE(srw_ss())[]Cenv_bs_CTDec_bound] ) >>
      fs[] >>
      match_mp_tac Cenv_bs_imp_incsz >>
      HINT_EXISTS_TAC >>
    ...
    discharge_hyps >- (
      ...
      conj_tac >- (
        SUBST1_TAC(DECIDE``sz + 2 = (sz + 1) + 1``) >>
        qmatch_abbrev_tac`Cenv_bs rd menv s eenv cmnv cenv (sz + 1 + 1) bs0` >>
        match_mp_tac Cenv_bs_imp_incsz >>
        qexists_tac`bs0 with stack := TL bs0.stack` >>
        simp[Abbr`bs0`,bc_state_component_equality] >>
        qmatch_abbrev_tac`Cenv_bs rd menv s eenv cmnv cenv (sz + 1) bs0` >>
        match_mp_tac Cenv_bs_imp_incsz >>
        qexists_tac`bs0 with stack := TL bs0.stack` >>
        simp[Abbr`bs0`,bc_state_component_equality] >>
  ...
  strip_tac >- (
    ...
    strip_tac >>
    qmatch_assum_abbrev_tac`bc_next^* bs bss0` >>
    last_x_assum(qspecl_then[`rd`,`cmnv`,`cs0`,`cenv`,`sz+2`,`bss0`,`bce`,`bcr`,`bc0++(TAKE 2 code)`,`REVERSE cc`,`(DROP (2 + LENGTH cc) code) ++ bc1`]mp_tac) >>
    discharge_hyps >- (
    ...
    `all_Clocs v ⊆ count (LENGTH s') ∧
     ...
     BIGUNION (IMAGE all_Clocs (set env)) ⊆ count (LENGTH s') ∧
     BIGUNION (IMAGE all_Clocs (set s')) ⊆ count (LENGTH s') ∧
     all_vlabs_menv menv ∧ EVERY all_vlabs s' ∧ all_vlabs v ∧
     ...
     Cenv_bs rd' menv s' (v::env) cmnv (CTLet(sz+1)::cenv) (sz+1) (bs2 with code := bce)` by (
    ...
    conj_tac >- (
      ...
      first_x_assum(qspecl_then[`rd'`,`cmnv`,`cs1`,`CTLet (sz+1)::cenv`,`sz+1`,`bs2`,`bce`,`bcr`
      ...
      qmatch_assum_abbrev_tac`Cenv_bs rd'' menv s'' (v::env) cmnv (CTLet(sz+1)::cenv) (sz+2) bs0` >>
      qexists_tac`bs0` >>
      simp[Abbr`bs0`,bc_state_component_equality] >>
      reverse conj_tac >- (
        ASSUME_TAC(prove(``∀x y. (x with code := y).stack = x.stack``,simp[])) >>
        metis_tac[Cenv_bs_CTLet_bound,SIMP_RULE(srw_ss())[]Cenv_bs_CTDec_bound] ) >>
  ...
  strip_tac >- (
    ...
    qmatch_assum_abbrev_tac`code_for_push rd menv bs bce bc0 ccode s s renv v cmnv cenv rsz` >>
    map_every qexists_tac [`menv`,`bc0`,`ccode`,`renv`,`cmnv`,`cenv`,`rsz`] >> rw[] >>
    qexists_tac`REVERSE args`>>fsrw_tac[ARITH_ss][EVERY2_EVERY]) >>
  strip_tac >- (
    ...
    conj_asm1_tac >- (
      ...
      fsrw_tac[DNF_ss][] >>
      first_x_assum (qspecl_then[`rd`,`cmnv`,`cs`,`cenv`,`sz`,`bs`,`bce`,`bcr`,`bc0`,`ls0`]mp_tac) >>
      simp[Abbr`ls0`] >>
      simp[code_for_push_def] >>
      ...
      qmatch_assum_abbrev_tac`Cenv_bs rd' menv s' denv cmnv cenv sz1 bs1` >>
      `Cenv_bs rd' menv s' denv cmnv cenv sz (bs1 with stack := bs.stack)` by (
        match_mp_tac Cenv_bs_pops >>
        ...
        metis_tac[Cenv_bs_CTLet_bound,CTDec_bound_lemma] ) >>
      match_mp_tac Cenv_bs_imp_incsz >>
  ...
  strip_tac >- (
    ...
    first_x_assum (qspecl_then[`rd`,`cmnv`,`cs`,`cenv`,`sz`,`bs`,`bce`,`bcr`,`bc0`,`REVERSE cx`]mp_tac) >>
    simp[] >>
  ...
  strip_tac >- (
    ...
    conj_asm1_tac >- (
      ...
      fs[Once SWAP_REVERSE] >>
      qmatch_assum_abbrev_tac `code = ls0 ++ [x]` >>
      first_x_assum(qspecl_then[`rd`,`cmnv`,`cs`,`cenv`,`sz`,`bs`,`bce`,`bcr`,`bc0`,`ls0`,`x::bc1`]mp_tac) >>
      discharge_hyps >- (
      ...
      reverse conj_tac >- fs[] >>
      qmatch_assum_abbrev_tac`Cenv_bs rd' menv s' ccenv cmnv csenv sz1 bs1` >>
      `Cenv_bs rd' menv s' ccenv cmnv csenv sz (bs1 with stack := bs.stack)` by (
        match_mp_tac Cenv_bs_imp_decsz >>
        qexists_tac `bs1` >>
        ...
        fs[] >>
        TRY DECIDE_TAC >>
        `LENGTH bs.stack < LENGTH bs.stack` by metis_tac[] >>
        DECIDE_TAC) >>
      qunabbrev_tac`sz1` >>
      match_mp_tac Cenv_bs_imp_incsz >>
      qexists_tac`bs1 with stack := bs.stack` >>
    ...
    fs[Once SWAP_REVERSE] >>
    qmatch_assum_abbrev_tac`code_for_push rd menv bs bce bc0 ccode s s' renv v cmnv cenv csz` >>
    map_every qexists_tac[`menv`,`bc0`,`ccode`,`renv`,`cmnv`,`cenv`,`csz`] >>
    rw[Abbr`ccode`] >>
  ...
  strip_tac >- (
    ...
    qspecl_then[`cmnv`,`cenv`,`TCNonTail`,`sz`,`cs`,`exp`]strip_assume_tac(CONJUNCT1 compile_append_out) >>
    fs[Once SWAP_REVERSE] >>
    qmatch_assum_abbrev_tac`code_for_push rd menv bs bce bc0 ccode s s' ccenv v cmnv csenv csz` >>
    map_every qexists_tac[`menv`,`bc0`,`ccode`,`ccenv`,`cmnv`,`csenv`,`csz`] >>
    rw[Abbr`ccode`] >>
    qexists_tac`REVERSE args` >> fs[EVERY2_EVERY]) >>
  strip_tac >- (
    simp[] >>
  ...
  strip_tac >- (
    ...
    reverse(Cases_on`ALL_DISTINCT (FILTER is_Label (bc0 ++ code)) ∧ ∃cc1. code = REVERSE cc++cc1`) >- (
      ...
      rw[] >> spose_not_then strip_assume_tac >> res_tac >> DECIDE_TAC) >>
    pop_assum strip_assume_tac >>
    last_x_assum(qspecl_then[`rd`,`cmnv`,`cs`,`cenv`,`sz`,`bs`,`bce`,`bcr`,`bc0`,`REVERSE cc`,`(DROP (LENGTH cc) code) ++ bc1`]mp_tac) >>
    discharge_hyps >- (
    ...
    conj_tac >- (
      ...
      first_x_assum(qspecl_then[`rd'`,`cmnv`,`cs1`,`CTLet(sz+1)::cenv`,`sz+1`,`bs2`,`bce`,`bcr`,`bc0++REVERSE cc`,`REVERSE cb`]mp_tac) >>
      simp[] >>
      discharge_hyps >- (
        simp[Abbr`bs2`] >>
        conj_tac >- PROVE_TAC[] >>
      ...
      qexists_tac`bs0` >>
      simp[Abbr`bs0`,bc_state_component_equality] >>
      reverse conj_tac >- metis_tac[Cenv_bs_CTLet_bound,CTDec_bound_lemma] >>
      `env = TL (v::env) ∧ cenv = TL (CTLet (sz+1)::cenv)` by simp[] >>
      ntac 2 (pop_assum SUBST1_TAC) >>
      match_mp_tac Cenv_bs_DOMSUB >> simp[]) >>
    conj_tac >- (
  ...
  strip_tac >- (
    ...
    reverse(Cases_on`∃bc10. bs.code = bc0 ++ cc ++ bc10`) >- (
      ...
      conj_tac >> TRY conj_tac >> rpt gen_tac >>
    ...
    `BIGUNION
       (IMAGE all_Clocs
         (set (GENLIST (CRecClos env defs) (LENGTH defs)))) ⊆ count (LENGTH s) ∧
     ALL_DISTINCT (FILTER is_Label bce) ∧
     ...
     Cenv_bs rd' menv s (GENLIST (CRecClos env defs) (LENGTH defs) ++ env)
          cmnv cenv' (sz+LENGTH defs) (bs1 with code := bce) ∧
     ALL_DISTINCT (FILTER is_Label bc0) ∧
    ...
    Cases_on`beh`>>fs[] >- (
      conj_tac >- (
        ...
        simp[] >> strip_tac >>
        first_x_assum(qspecl_then[`rd'`,`cmnv`,`ccs`,`cenv'`,`sz+(LENGTH defs)`,`bs1`,`bce`,`bcr`,`bc0++cc`,`REVERSE ce`]mp_tac) >>
        simp[] >>
        discharge_hyps >- (
          rpt (BasicProvers.VAR_EQ_TAC) >>
    ...
    simp[code_for_return_def] >>
    simp_tac(srw_ss()++DNF_ss)[] >>
    map_every qx_gen_tac[`bv`,`rf`,`rd''`] >> strip_tac >>
    map_every qexists_tac[`bv`,`rf`,`rd''`] >>
    fs[] >>
    reverse conj_tac >- (
  ...
  strip_tac >- (
    ...
    fs[Once SWAP_REVERSE] >>
    fs[Once SWAP_REVERSE] >>
    last_x_assum(qspecl_then[`rd`,`cmnv`,`cs`,`cenv`,`sz`,`bs`,`bce`,`bcr`,`bc0`,`REVERSE cx`]mp_tac) >>
    ...
    qmatch_assum_abbrev_tac`bc_next^* bs bs1` >>
    last_x_assum(qspecl_then[`rd'`,`cmnv`,`cs0`,`cenv`,`sz+1`,`bs1`,`bce`,`bcr`,`bc0 ++ REVERSE cx`,`REVERSE cxs`]mp_tac) >>
    simp[Abbr`bs1`] >>
    discharge_hyps >- (
      simp[Abbr`cs0`] >>
    ...
    let
      val tac1 =
        ...
        pop_assum (assume_tac o SYM) >>
        first_x_assum (qspecl_then[`rd''`,`cmnv`,`csc`,`MAP CTEnv ccenv`,`0`,`bs1`,`bce`,`bcr`,`cb0 ++ [Label l]`,`REVERSE cc`,`cb1 ++ bcr`]mp_tac) >>
        simp[] >>
        discharge_hyps >- (
          fs[good_cd_def] >>
          conj_tac >- (
            qspecl_then[`menv`,`s`,`env`,`exp`,`(s',Cval (CRecClos fenv defs n))`]mp_tac (CONJUNCT1 Cevaluate_Clocs) >>
            qspecl_then[`menv`,`s'`,`env`,`exps`,`(s'',Cval vs)`]mp_tac (CONJUNCT2 Cevaluate_Clocs) >>
            simp[] >>
          ...
          conj_tac >- (
            ...
            simp[]) >>
          conj_tac >- (
            qspecl_then[`menv`,`s'`,`env`,`exps`,`s'',Cval vs`]mp_tac(CONJUNCT2 Cevaluate_all_vlabs) >>
            qspecl_then[`menv`,`s`,`env`,`exp`,`s',Cval (CRecClos fenv defs n)`]mp_tac(CONJUNCT1 Cevaluate_all_vlabs) >>
            simp[] ) >>
          conj_tac >- (
          ...
          conj_tac >- (
            qspecl_then[`menv`,`s`,`env`,`exp`,`s',Cval (CRecClos fenv defs n)`]mp_tac(CONJUNCT1 Cevaluate_vlabs) >>
            qspecl_then[`menv`,`s'`,`env`,`exps`,`s'',Cval vs`]mp_tac(CONJUNCT2 Cevaluate_vlabs) >>
            simp[] >>
        ...
        discharge_hyps >- (
          ...
          conj_tac >- (
            ...
            conj_tac >- (
              ...
              env_renv_change_store
              qmatch_assum_abbrev_tac`fmap_rel (env_renv ps' 0 bs') menv X` >>
              qexists_tac`bs'` >>
    ...
    in
      Cases_on`beh`>>fs[] >- (
        qmatch_assum_rename_tac`Cevaluate menv s'' env' exp' (s''',Cval v)`[]>>
        conj_tac >- (
          ...
          match_mp_tac Cenv_bs_imp_incsz >>
          qunabbrev_tac`bs0` >>
          qmatch_assum_abbrev_tac`Cenv_bs rd menv s renv cmnv cenv sz bs0` >>
      ...
      (tac2 (
        ...
        disch_then(qspec_then`REVERSE args`mp_tac) >> simp[] >>
        disch_then(qspec_then`bc1`mp_tac) >> simp[] >>
      ...
      simp[code_for_return_def,Abbr`P`] >>
      disch_then(qx_choosel_then[`bv`,`rf'`,`rd'''`]strip_assume_tac) >>
      map_every qexists_tac[`rf'`,`bv`,`rd'''`] >>
      fs[] >>
      reverse conj_tac >- metis_tac[IS_PREFIX_TRANS,SUBMAP_TRANS] >>
      simp[Abbr`bs2`]
    end ) >>
  strip_tac >- (
    ...
    `LENGTH s ≤ LENGTH s' ∧ LENGTH s' ≤ LENGTH s''` by PROVE_TAC[Cevaluate_store_SUBSET,FST] >>
    simp[] >>
    disch_then(mp_tac o SIMP_RULE(srw_ss()++DNF_ss)[code_for_push_def,LET_THM] o CONJUNCT1) >>
    disch_then(qx_choosel_then[`rf`,`rd'`,`bf`]strip_assume_tac) >>
    qmatch_assum_abbrev_tac`bc_next^* bs bs1` >>
    ...
    strip_tac >>
    rpt gen_tac >> strip_tac >>
    first_x_assum(qspecl_then[`v'`,`bf::ig`,`sp`,`hdl`,`st`]mp_tac) >>
    simp[] >>
    simp[code_for_return_def] >>
    disch_then(qx_choosel_then[`bv`,`rf'`,`rd''`]strip_assume_tac) >>
    map_every qexists_tac[`bv`,`rf'`,`rd''`] >>
  ...
  strip_tac >- (
    ...
    fs[Once SWAP_REVERSE] >>
    last_x_assum(qspecl_then[`rd`,`cs`,`cenv`,`sz`,`bs`,`bce`,`bcr`,`bc0`,`REVERSE cx`]mp_tac) >>
    `LENGTH s ≤ LENGTH s'` by PROVE_TAC[Cevaluate_store_SUBSET,FST] >>
    simp[] >>
    strip_tac >>
    rpt gen_tac >> strip_tac >>
    first_x_assum(qspecl_then[`TCNonTail`,`v`,`ig`,`sp`,`hdl`,`st`]mp_tac) >>
    simp[]) >>
  strip_tac >- (
    rpt gen_tac >> strip_tac >>
    ...
    Cases_on`beh`>>fs[] >- (
      ...
      `P` by (
        ...
        strip_tac >>
        qmatch_assum_abbrev_tac`bc_next^* bs bs1` >>
        qspecl_then[`rd'`,`uop`,`s'`,`v`,`s''`,`a`,`bs1`,`bc0 ++ REVERSE bc`,`bc10++bc1`,`bce`,`bs.stack`,`bv`]
  ...
  strip_tac >- (
    ...
    simp[Once CONJ_ASSOC] >>
    reverse conj_tac >- (
      qpat_assum`CevalUpd X Y Z = A`mp_tac>>
      Cases_on`v1`>>fs[]>>rw[]) >>
    qpat_assum`bce++bcr=X`mp_tac>>
    qpat_assum`bs.code=X`mp_tac>>
    simp[IMP_CONJ_THM]>>
  ...
  strip_tac >- (
    ...
    let
      ...
      val tac5 =
        ...
        simp[Abbr`bs1`,Once SWAP_REVERSE] >>
        discharge_hyps >- (
          qmatch_assum_abbrev_tac`Cevaluate s env exp (s',Cval(CLitv(Bool F)))` >>
          conj_tac >- metis_tac[SUBSET_DEF,IN_COUNT,LESS_LESS_EQ_TRANS] >>
          qspecl_then[`s`,`env`,`exp`,`(s',Cval (CLitv (Bool F)))`]mp_tac(CONJUNCT1 Cevaluate_Clocs) >>
    ...
    in
      ...
      conj_tac >- (
        ...
        Cases_on `b1` >> fs[] >- (
          ...
          simp[Abbr`bs1`] >>
          discharge_hyps >- (
            qmatch_assum_abbrev_tac`Cevaluate s env exp (s',Cval(CLitv(Bool T)))` >>
            conj_tac >- metis_tac[SUBSET_DEF,LESS_LESS_EQ_TRANS,IN_COUNT] >>
            qspecl_then[`s`,`env`,`exp`,`(s',Cval (CLitv (Bool T)))`]mp_tac(CONJUNCT1 Cevaluate_Clocs) >>
            qspecl_then[`s`,`env`,`exp`,`(s',Cval (CLitv (Bool T)))`]mp_tac(CONJUNCT1 Cevaluate_vlabs) >>
            simp[] >> strip_tac >>
  ...
  strip_tac >- (
    ...
    strip_tac >>
    map_every qexists_tac[`rf`,`rd''`,`bv`,`bvs`] >>
    qmatch_assum_abbrev_tac`bc_next^* bs bs1` >>
    qmatch_assum_abbrev_tac`bc_next^* bs1 bs2` >>
    conj_tac >- (
      qmatch_abbrev_tac `bc_next^* bs bs3` >>
  ...
  strip_tac >- (
    ...
    simp[Once SWAP_REVERSE] >> strip_tac >>
    last_x_assum(qspecl_then[`rd`,`cs`,`cenv`,`sz`,`bs`,`bce`,`bcr`,`bc0`,`REVERSE be`]mp_tac) >>
    `LENGTH s ≤ LENGTH s'` by metis_tac[Cevaluate_store_SUBSET,FST] >>
    simp[] >>
    disch_then(qspec_then`TCNonTail`mp_tac) >>
    simp[] ) >>
  rpt gen_tac >> strip_tac >>
  ...
  first_x_assum(qspecl_then[`rd'`,`cs0`,`cenv`,`sz+1`,`bs1`,`bce`,`bcr`,`bc0 ++ REVERSE be`,`REVERSE bes`]mp_tac) >>
  simp[Abbr`bs1`] >>
  disch_then(qspecl_then[`bv0::ig`,`sp`,`hdl`,`st`]mp_tac) >>
  discharge_hyps >- (
    conj_tac >- PROVE_TAC[SUBSET_DEF,IN_COUNT,LESS_LESS_EQ_TRANS] >>
    conj_tac >- PROVE_TAC[Cevaluate_Clocs,FST] >>
    qspecl_then[`s`,`env`,`exp`,`(s',Cval v)`]mp_tac(CONJUNCT1 Cevaluate_vlabs) >>
