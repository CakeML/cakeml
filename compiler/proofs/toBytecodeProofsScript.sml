...
val compile_val = store_thm("compile_val",
  ...
  strip_tac >- (
    ...
    conj_tac >- (
      ...
      discharge_hyps >- (
        `bs2.clock = SOME (FST s')` by (
          fs[s_refs_def,Abbr`bs2`,Abbr`bss0`] ) >>
    ...
    discharge_hyps >- (
      ...
      rw[] >> spose_not_then strip_assume_tac >> res_tac >> fsrw_tac[ARITH_ss][] ) >>
    simp[] >>
    disch_then(qspecl_then[`tt`]mp_tac) >>
    discharge_hyps >- (
      simp[Abbr`bs2`] >>
      simp[Abbr`tt`] >>
      Cases_on`t`>>fs[] >>
      qexists_tac`bv::blvs`>>simp[] >>
      qexists_tac`args`>>simp[] ) >>
    metis_tac[SIMP_RULE std_ss [transitive_def] RTC_TRANSITIVE]) >>
  strip_tac >- (
    simp[compile_def] >>
    rpt gen_tac >> strip_tac >>
    rpt gen_tac >>
    ...
    discharge_hyps >- (
      ...
      rw[] >> res_tac >> simp[] ) >>
    rw[] >>
    pop_assum(qspec_then`TCNonTail`mp_tac) >>
    simp[] >>
  ...
  strip_tac >- (
    ...
    TRY (qmatch_abbrev_tac `Cenv_bs rd menv A B cmnv C D csz E` >>
         qmatch_assum_abbrev_tac `Cenv_bs rd menv A B cmnv C D csz E'` >>
         qsuff_tac`E = E'`>-rw[] >>
         unabbrev_all_tac) >>
    rw[bc_state_component_equality]) >>
  strip_tac >- (
