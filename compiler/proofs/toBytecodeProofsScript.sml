    ...
    let
      ...
      val tac =
        ...
        discharge_hyps >- (
          ...
          qexists_tac`args`>>simp[])
    in
      conj_tac (* TODO: don't understand why this doesn't work *)
        >- (tac >> t1)
        >- (tac >> t2)
    end) >>
  strip_tac >- (
  ...
  strip_tac >- (
    ...
    let
      val tac1 =
        ...
        `bc_next^* bs1 (bs1 with <| clock := OPTION_MAP (K (FST s')) bs1.clock; pc := next_addr bs.inst_length (BUTLASTN (1 + LENGTH bc1) bs.code) |>)` by (
          simp[Abbr`mtk`] >>
          Cases_on`ck`>>simp[]>-(
