...
Ancestors
  fromSexp scheme_ast scheme_parsing
  scheme_to_cake
Libs
  preamble
...
Definition cake_prog_to_string_def:
  cake_prog_to_string ast =
    sexp_to_string (listsexp (MAP decsexp ast))
End
...
Definition compile_def:
  compile (s:string) =
    ...
    | INL err_str => cake_for_err (concat [«PARSE ERROR: »; implode err_str; «\n»])
    | INR ast =>
      (case static_scope_check ast of
      | INL err_str => cake_for_err (concat [«TYPE ERROR: »; err_str; «\n»])
      | INR ast' =>
       (case codegen ast of
        | INL err_str => cake_for_err (concat [«CODEGEN ERROR: »; err_str; «\n»])
...
Definition main_function_def:
  main_function s = compile (explode s)
End
