...
[~ApplyK_NONE:]
  cont_rel ks kv ∧
  nsLookup env.v (Short var) = SOME kv ∧
  (m, inner) = cps_transform_app n (Var (Short t)) [] es (Var (Short var)) ∧
  scheme_env env ∧
  env_rel se env ∧
  ¬ MEM var vconses ∧
  ¬ MEM t vconses ∧
...
[~ApplyK_SOME:]
  cont_rel ks kv ∧
  nsLookup env.v (Short var) = SOME kv ∧
  (m, inner) = cps_transform_app n (Var (Short fnt))
...
[~BeginK:]
  cont_rel ks kv ∧
  nsLookup env.v (Short var) = SOME kv ∧
  (m, inner) = cps_transform_seq n (Var (Short var)) es e ∧
  scheme_env env ∧
  env_rel se env ∧
  ¬ MEM var vconses ∧
  var ≠ "_"
...
[~SetK:]
  cont_rel ks kv ∧
  nsLookup env.v (Short var) = SOME kv ∧
  (m, inner) = refunc_set n (Var (Short t)) (Var (Short var)) x ∧
  scheme_env env ∧
  env_rel se env ∧
  ¬ MEM var vconses ∧
  ¬ MEM t vconses ∧
...
Proof
  ...
  >~ [‘Exp e’] >- (
    ...
    >~ [‘Begin es e’] >- (
      ...
      >> simp[Once e_ce_rel_cases]
      >> irule_at Any EQ_REFL
      >> qpat_assum ‘cps_transform _ _ = _’ $ irule_at (Pos $ el 2) o GSYM
      >> qpat_assum ‘scheme_env _’ $ simp
        o curry ((::) o swap) [scheme_env_def]
        o SRULE [scheme_env_def]
      >> gvs[env_rel_cases]
      >> simp[Once cont_rel_cases]
      >> qpat_assum ‘cont_rel _ _’ $ irule_at (Pos hd)
      >> pop_assum $ irule_at (Pos $ el 2) o GSYM
      >> simp[]
      >> qpat_assum ‘scheme_env _’ $ simp
        o curry ((::) o swap) [scheme_env_def]
        o SRULE [scheme_env_def]
          ...
          >~ [‘"SEqv"’] >- (
            ...
            >> gvs[vcons_list_def, seqv_def] >- (
              ...
              >> simp[can_pmatch_all_def, pmatch_def, nsLookup_def,
                ...
                pat_bindings_def]
              >> Cases_on ‘x=v’
              >> simp[Ntimes evaluate_def 5, do_if_def, do_app_def, do_eq_def]
              >> simp[can_pmatch_all_def, pmatch_def, nsLookup_def,
                same_type_def, same_ctor_def, evaluate_match_def,
                pat_bindings_def]
              >> cheat
