...
/\
(! c d s env xs cb.
(! l. (cb =( INR l)) ==>  l IN( FDOM  c) /\ (((FLOOKUP d) l) =( SOME (((cbod_fvs c) cb),xs,[],0))))
==>
...
(remove_mat_var v ((p,sk)::pes) =
  ...
  CLet fk) (((CFun []) ((INL (((remove_mat_var v) pes)))))))
    (((((remove_mat_vp fk) sk) v) p))))`;
...
/\
(calculate_ldefs c ls (CLetrec ns defs e) =(((
  FOLDL
    (\ ls . \x . (case x of (_,cb) =>
      (case cb of INR l =>
