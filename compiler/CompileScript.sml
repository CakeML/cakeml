...
val _ = Define `
 (extend_rec_env cenv env defs vs = APPEND ( APPEND ( REVERSE vs)
...
/\
(! c s env e es s' env' defs n cb b s'' vs r.
...
/\
(! c env1 env2 defs1 defs2 d1 d2. EVERY2
  (syneq_cb c (if d1 < LENGTH defs1 then LENGTH defs1 else 0) env1 env2)
  defs1 defs2
==>
...
/\
(! c k env1 env2 xs1 xs2. EVERY2 (syneq_var c k env1 env2) ( MAP (\ (n,m) . n) xs1) ( MAP (\ (n,m) . n) xs2)
==>
...
/\
(! c k env1 env2 cn es1 es2. EVERY2 (syneq_exp c k env1 env2) es1 es2
==>
