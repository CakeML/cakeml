...
Definition compile_def:
  compile asm_conf c p =
    ...
    let (s,p,l,bl,n1,n2,n3,n4,cm,names) = bvl_to_bvi$compile c.clos_conf.start c.bvl_conf names p in
    let c = c with clos_conf updated_by (λc. c with start:=s) in
    let c = c with bvl_conf updated_by (λc. c with
      <| inlines := l; bvi_inlines := bl;
         next_name1 := n1; next_name2 := n2; next_name3 := n3;
         next_name4 := n4; cpr_map := cm |>) in
