...
val compile_def = Define`
  compile c p =
    ...
    let _ = empty_ffi (strlit "finished: flat_to_con") in
...
val prim_config_def = Define`
  prim_config =
    FST (to_dec <| source_conf := empty_config; flat_conf := empty_config |> (prim_types_program))`;
