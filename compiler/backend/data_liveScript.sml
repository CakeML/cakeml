...
val is_pure_def = Define `
  ...
  (* TODO: mark small wordops/wordhifts/wordcmps/conversions as pure *)
  (is_pure (WordOp _ _) = F) /\
  (is_pure (WordShift _ _ _) = F) /\
  (is_pure (WordCmp _ _) = F) /\ (* WordCmp is pure for all sizes if integer comparisons are pure *)
  (is_pure (WordFromInt _) = F) /\
  (is_pure (WordToInt _) = F) /\
  (is_pure (WordToWord _ _) = F) /\
