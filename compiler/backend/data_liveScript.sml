...
val compile_def = Define `
  (compile _ Skip live = (Skip,live)) /\
  (compile _ (Return n) live = (Return n, insert n () LN)) /\
  (compile _ (Raise n) live = (Raise n, insert n () LN)) /\
  (compile _ (Move n1 n2) live =
    (Move n1 n2, insert n2 () (delete n1 live))) /\
  (compile arch_size (Seq c1 c2) live =
     let (d2,l2) = compile arch_size c2 live in
     let (d1,l1) = compile arch_size c1 l2 in
       (Seq d1 d2, l1)) /\
  (compile _ Tick live = (Tick,live)) /\
  (compile _ (MakeSpace k names) live =
     let l1 = inter names live in (MakeSpace k l1,l1)) /\
  (compile arch_size (Assign v op vs NONE) live =
  ...
  (compile _ (Assign v op vs (SOME names)) live =
     let l1 = inter names (list_insert vs (delete v live)) in
       (Assign v op vs (SOME l1),l1)) /\
  (compile arch_size (If n c2 c3) live =
     let (d3,l3) = compile arch_size c3 live in
     let (d2,l2) = compile arch_size c2 live in
       (If n d2 d3, insert n () (union l2 l3))) /\
  (compile _ (Call NONE dest vs handler) live =
     (Call NONE dest vs handler,list_to_num_set vs)) /\
  (compile _ (Call (SOME (n,names)) dest vs NONE) live =
     let l1 = inter names (delete n live) in
     let l2 = list_insert vs l1 in
       (Call (SOME (n,l1)) dest vs NONE,l2)) /\
  (compile arch_size (Call (SOME (n,names)) dest vs (SOME (v,c))) live =
     let (d,l3) = compile arch_size c live in
