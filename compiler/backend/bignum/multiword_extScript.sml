...
Theorem MAP_AND_F:
  ∀xs. MAP ($/\ F) xs = REPLICATE (LENGTH xs) F
Proof
  Induct >> simp []
QED
...
Theorem bits_bitwise_LENGTH:
  ∀op xs xb ys yb zs b.
    bits_bitwise op (xs, xb) (ys, yb) = (zs, b) ⇒
    LENGTH zs = MAX (LENGTH xs) (LENGTH ys)
Proof
  recInduct bits_bitwise_ind >> rw [bits_bitwise_def]
  >> rpt (pairarg_tac >> gvs [])
  >> simp [MAX_DEF]
...
Theorem int_or_sign:
  int_or i j < 0 ⇔ i < 0 ∨ j < 0
Proof
  rw [int_or_def, int_bitwise_def, bits_of_int_def, int_of_bits_sign]
  >> rw [GSYM SND_pair] >> pairarg_tac >> gvs []
  >> drule bits_bitwise_rest >> simp []
QED
...
Theorem BITWISE_OR_zero_left:
  ∀k n ys.
    LENGTH ys ≤ k ⇒
    BITWISE k $\/ 0 (num_of_bits ys) = (num_of_bits ys)
Proof
  Induct >> rw []
...
Theorem BITWISE_OR_zero_right:
  ∀k n xs.
    LENGTH xs ≤ k ⇒
    BITWISE k $\/ (num_of_bits xs) 0 = (num_of_bits xs)
Proof
  Induct >> rw []
  >- simp [num_of_bits_def, BITWISE_def]
  >> Cases_on ‘xs’
...
Theorem bits_bitwise_and_BITWISE:
  ∀ys xs zs k.
    bits_bitwise $/\ (xs,F) (ys,F) = (zs,F) ∧
    LENGTH xs ≤ k ∧ LENGTH ys ≤ k ⇒
    BITWISE k $/\ (num_of_bits xs) (num_of_bits ys) =
    num_of_bits zs
Proof
  Induct >> rw []
  >- simp [num_of_bits_def, MAP_LAMBDA_F, num_of_bits_REPLICATE_F]
  >> Cases_on ‘xs’
  >- gvs [num_of_bits_def, MAP_AND_F, num_of_bits_REPLICATE_F]
  >> Cases_on ‘k’ >> fs []
  >> simp [BITWISE_EVAL]
  >> gvs [bits_bitwise_def]
  >> rpt (pairarg_tac >> gvs [])
...
Theorem bits_bitwise_or_not_not_and:
  bits_bitwise $\/ (MAP $¬ xs,b₁) (MAP $¬ ys,b₂) = (zs,b₃)
  ⇔
  bits_bitwise $/\ (xs,¬b₁) (ys,¬b₂) = (MAP $¬ zs,¬b₃)
Proof
...
Theorem bits_bitwise_or_not_not_w2n:
  ∀xs ys zs.
    bits_bitwise $\/ (MAP $¬ xs, T) (MAP $¬ ys,T) = (zs,T) ∧
    LENGTH xs ≤ dimindex (:α) ∧ LENGTH ys ≤ dimindex (:α)
    ⇒
...
Proof
  rpt gen_tac
  >> rewrite_tac [neg_n2w_SUC, WORD_NOT_NOT, word_and_n2w]
  >> simp [bits_bitwise_or_not_not_and]
  >> simp [dimword_def, BITWISE_LT_2EXP]
  >> strip_tac
  >> drule bits_bitwise_and_BITWISE >> simp []
QED
...
Theorem bits_bitwise_or_w2n:
  ∀xs ys zs.
    bits_bitwise $\/ (xs,F) (ys,F) = (zs,F) ∧
    LENGTH xs ≤ dimindex (:α) ∧
    LENGTH ys ≤ dimindex (:α)
    ⇒
    w2n ((n2w (num_of_bits xs) :α word) ||
         (n2w (num_of_bits ys) :α word)) =
    (num_of_bits zs)
Proof
  rw [word_or_n2w]
  >> drule_all bits_bitwise_or_BITWISE
  >> simp [dimword_def, BITWISE_LT_2EXP]
  >> strip_tac
  >> drule_then assume_tac bits_bitwise_LENGTH
  >> irule LTE_TRANS
  >> irule_at Any num_of_bits_lt
  >> simp []
QED
...
Proof
  recInduct b2mw'_ind >> rw []
  >> Cases_on ‘k = 0’ >> gvs []
  >- fs [b2mw_eq_nil, b2mw'_0, mw2n_def, num_of_bits_def]
...
Proof
  ...
  >-
   (simp [Once b2mw'_def]
    >> gvs [mw_and_keep_nil_left, mw2n_b2mw, b2mw_eq_nil]
    >> simp [MAP_AND_T])
...
Proof
  recInduct b2mw'_ind >> rw []
  >> Cases_on ‘k = 0’ >> gvs []
  >- (gvs [mw_and_nil_left, mw2n_def, b2mw_eq_nil, MAP_LAMBDA_F,
...
Proof
  recInduct b2mw'_ind >> rw []
  >> Cases_on ‘k = 0’ >> gvs []
  >- gvs [b2mw_eq_nil, b2mw'_0, mw_and_keep_nil_left, MAP_word1_comp_word1_comp,
...
Theorem mw2n_mw_or_keep:
  ∀xs ys zs.
    bits_bitwise $\/ (xs,F) (ys,F) = (zs,F) ∧
    LENGTH (b2mw xs : 'a word list) ≤ LENGTH (b2mw ys : 'a word list)  ⇒
    mw2n (mw_or_keep (b2mw xs) (b2mw ys : 'a word list)) =
    num_of_bits zs
Proof
  recInduct b2mw_ind >> rw []
  >> Cases_on ‘xs = []’ >> gvs []
  >- simp [MAP_OR_F, mw_or_keep_nil_left, mw2n_b2mw]
  >> once_rewrite_tac [b2mw_def] >> simp []
  >> IF_CASES_TAC >> gvs [b2mw_eq_nil]
  >> simp [mw_or_keep_def, mw2n_def]
  >> drule $ iffLR bits_bitwise_TAKE_DROP
  >> disch_then $ qspec_then ‘dimindex (:α)’ assume_tac >> fs []
  >> first_x_assum drule
  >> impl_tac >- gvs [LENGTH_b2mw, SUB_CEILING_DIV]
  >> strip_tac >> simp []
  >> qspec_then ‘TAKE (dimindex (:α)) xs’ mp_tac bits_bitwise_or_w2n
  >> disch_then drule
...
Proof
  ...
  >> qmatch_goalsub_abbrev_tac ‘int_of_bits bs’
  >> Cases_on ‘bs’
  >> gvs [oneline int_of_bits_def]
  >> drule_then assume_tac bits_bitwise_rest >> gvs []
  >> rewrite_tac [n2mw_eq_b2mw]
  >> qmatch_goalsub_abbrev_tac
       ‘mw_fix (mw_or_keep (b2mw xs) (b2mw ys)) =
        b2mw (bits_of_num (num_of_bits zs))’
  >> ‘b2mw (bits_of_num (num_of_bits zs)) = n2mw (num_of_bits zs)’ by
    simp [n2mw_eq_b2mw]
  >> gvs [n2mw_mw2n]
  >> AP_TERM_TAC
  >> irule mw2n_mw_or_keep
...
Theorem mw2n_mw_or_ones_not_not:
  ∀l xs l' ys zs.
    bits_bitwise $\/ (MAP $¬ xs,T) (MAP $¬ ys,T) = (zs,T) ∧ l ≤ l' ∧
    LENGTH (b2mw xs : 'a word list) ≤ l ∧ LENGTH (b2mw ys : 'a word list) ≤ l' ⇒
    mw2n
...
Proof
  recInduct b2mw'_ind >> rw []
  >> Cases_on ‘k = 0’ >> gvs []
  >- gvs [b2mw_eq_nil, MAP_OR_T, b2mw'_0, mw_or_ones_nil_left,
          num_of_bits_REPLICATE_F, mw2n_REPLICATE_0]
  >> once_rewrite_tac [b2mw'_def] >> simp []
  >> simp [mw2n_def, mw_or_ones_def]
  >> drule $ iffLR bits_bitwise_TAKE_DROP
  ...
  >> strip_tac >> simp []
  >> fs [GSYM MAP_TAKE]
  >> qspec_then ‘TAKE (dimindex (:α)) xs’ mp_tac bits_bitwise_or_not_not_w2n
  >> disch_then drule
  >> impl_tac >- simp [LENGTH_TAKE_EQ]
  >> strip_tac >> simp []
  >> simp [MAP_TAKE, MAP_DROP, num_of_bits_TAKE_DROP_dimindex]
...
Proof
  ...
  >> drule_then assume_tac bits_bitwise_rest >> gvs []
  >> rewrite_tac [n2mw_eq_b2mw]
  >> rename1 ‘(mw_or_ones_flip (_ (_ (_ n))) (_ (_ (_ m))))’
  >> simp [Req0 mw_bits_of_int_b2mw, mw2n_mw_int_of_bits, int_not_def,
           int_calculate]
  >> ‘Num (&n - 1) = n - 1’ by (DEP_REWRITE_TAC [INT_SUB] >> simp [])
  >> ‘Num (&m - 1) = m - 1’ by (DEP_REWRITE_TAC [INT_SUB] >> simp [])
  >> ntac 2 $ pop_assum SUBST_ALL_TAC
  >> rw [mw_or_ones_flip_def]
  >> irule mw2n_mw_or_ones_not_not
  >> fs [GSYM n2mw_eq_b2mw, LENGTH_n2mw_LESS_LENGTH_n2mw, LENGTH_b2mw',
         bits_bitwise_or_sym]
QED
