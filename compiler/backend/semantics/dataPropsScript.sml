...
Theorem do_space_alt:
  do_space op vs s =
      ...
      else consume_space (op_space_req op (LENGTH vs) (arch_size s.limits)) s
Proof
...
Theorem do_app_lim_swap_aux[local]:
  ∀op vs s q s' lim. arch_size lim = arch_size s.limits /\ do_app op vs s = Rval (q,s')
    ⇒ ∃safe peak smx.
...
Theorem do_app_err_lim_swap:
  ∀op vs s e lim. arch_size lim = arch_size s.limits /\ do_app op vs s = Rerr e
    ⇒ do_app op vs (s with <|limits := lim|>) =
      Rerr e
Proof
  do_app_swap_tac
...
Theorem do_app_preserves_arch_size:
  !op a b x y. do_app op a b = Rval (x,y)
   ==> arch_size y.limits = arch_size b.limits
Proof
  Cases \\ rw[do_app_def,case_eq_thms,do_app_aux_def,do_stack_def,arch_size_def
             ,do_space_def] \\ fs[]
         \\ fs[consume_space_def,case_eq_thms]
         \\ rveq \\ fs[]
  \\ TRY (fs[with_fresh_ts_def]
      \\ FULL_CASE_TAC \\ fs[]
      \\ FULL_CASE_TAC \\ fs[check_lim_def] \\ NO_TAC
  )
  \\ TRY (fs[with_fresh_ts_def]
      \\ FULL_CASE_TAC \\ fs[check_lim_def] \\ NO_TAC
  )
  \\ TRY (fs[with_fresh_ts_def]
          \\ FULL_CASE_TAC \\ fs[]
          \\ FULL_CASE_TAC \\ fs[]
          \\ FULL_CASE_TAC \\ fs[check_lim_def]
          \\ rveq \\ fs[] \\ NO_TAC)
  \\ fs[do_install_def]
...
Theorem arch_size_simp:
  !x y. arch_size x.limits = arch_size y.limits <=> x.limits.arch_64_bit = y.limits.arch_64_bit
Proof
  rw[] \\ eq_tac \\ rw[]
  \\ fs[arch_size_def]
  \\ FULL_CASE_TAC \\ fs[]
...
Theorem arch_size_pop_env:
  !s2 x. pop_env s2 = SOME x ==> arch_size x.limits = arch_size s2.limits
Proof
  rw[]
  \\ fs[pop_env_def]
  \\ FULL_CASE_TAC \\ fs[]
...
Theorem arch_size_set_var[simp]:
  !a b c. arch_size (set_var a b c).limits = arch_size c.limits
Proof
  simp[set_var_def]
QED

Theorem arch_size_call_env[simp]:
  !a b c. arch_size (call_env a b c).limits = arch_size c.limits
Proof
  simp[call_env_def]
QED

Theorem arch_size_dec_clock[simp]:
  !s. arch_size (dec_clock s).limits = arch_size s.limits
Proof
  simp[dec_clock_def]
QED

Theorem arch_size_push_env[simp]:
  !a b c. arch_size (push_env a b c).limits = arch_size c.limits
Proof
  rw[] \\ Cases_on`b` \\ simp[push_env_def]
QED

Theorem evaluate_Call_eq_NONE_get_vars:
  evaluate (Call x y z a,s) = (NONE,c)
  ==> ?xs args1 prog ss.
    get_vars z s.locals = SOME xs
Proof
...
Theorem evaluate_preserves_arch_size:
  !a b c d. evaluate (a,b) = (c,d) ==>
  arch_size b.limits = arch_size d.limits
Proof
 recInduct evaluate_ind \\ REPEAT STRIP_TAC
 \\ full_simp_tac(srw_ss())[evaluate_def]
 \\ every_case_tac
 \\
 full_simp_tac(srw_ss())[call_env_def,flush_state_def,fromList_def,op_requires_names_def]
 \\ imp_res_tac do_app_err >> full_simp_tac(srw_ss())[] >> rev_full_simp_tac(srw_ss())[]
 \\ SRW_TAC [] [] \\ full_simp_tac(srw_ss())[LET_DEF] \\ SRW_TAC [] [] \\ full_simp_tac(srw_ss())[]
 \\ rpt(TOP_CASE_TAC >> fs[] >> rveq)
 \\ TRY(fs[cut_state_opt_def,case_eq_thms]
    \\ fs[cut_state_def,case_eq_thms]
    \\ rveq
    \\ simp[] \\ NO_TAC
 )
 >-(Cases_on`op` \\
   fs[do_app_def,case_eq_thms,pair_case_eq,do_app_aux_def,do_space_def]
   \\ rveq \\ fs[do_stack_def,data_spaceTheory.op_space_req_def,op_space_reset_def
   , cut_state_opt_def,pair_case_eq,case_eq_thms]
   \\ rw[]
   \\ fs[cut_state_def,case_eq_thms,pair_case_eq]
   \\ rveq \\ fs[]
   ...
   \\ TRY(fs[consume_space_def] \\ rveq \\ simp[] \\ NO_TAC)
   \\ fs[consume_space_def]
   \\ rveq
   \\ fs[with_fresh_ts_def]
   \\ FULL_CASE_TAC \\ fs[check_lim_def]
 ...
 >-(Cases_on`op`
   ...
   , cut_state_opt_def,pair_case_eq,case_eq_thms]
   \\ rw[]
   \\ fs[cut_state_def,case_eq_thms,pair_case_eq] \\ rveq \\ fs[]
   \\ fs[with_fresh_ts_def,case_eq_thms,pair_case_eq,check_lim_def]
   \\ rveq \\ fs[consume_space_def] \\ rveq \\ fs[]
   \\ fs[do_install_def,case_eq_thms,pair_case_eq]
   \\ Cases_on`s.compile_oracle 0` \\ fs[]
   \\ rename1`s.compile_oracle 0 = (xx,yy)`
   \\ Cases_on`yy` \\ fs[]
   \\ FULL_CASE_TAC \\ fs[]
   \\ FULL_CASE_TAC \\ fs[]
   \\ FULL_CASE_TAC \\ fs[]
...
Theorem evaluate_swap_limits:
  ∀c s r s' limits.
   arch_size limits = arch_size s.limits /\
   evaluate (c,s) = (r,s') ⇒
   ?smx safe peak.
...
Proof
  ...
  >- (
     ...
     \\ TRY (mp_tac (Q.SPECL[`op`,`xs`,`s with <|locals :=
     x'|>`,`q`,`r'`,`limits'`]do_app_lim_swap)
         \\ fs[]
         \\ rw[]
         \\ fs[state_component_equality] \\ NO_TAC
     )
     \\ TRY (mp_tac (Q.SPECL[`op`,`xs`,`s with <|locals :=
     x'|>`,`e`,`limits'`]do_app_err_lim_swap)
         \\ fs[]
         \\ rw[]
         \\ fs[state_component_equality] \\ NO_TAC
     )
  ...
  >- (fs[evaluate_def,ELIM_UNCURRY]
     ...
     metis_tac[evaluate_preserves_arch_size]
     \\ fs[]
  ...
  >- (Cases_on `handler` >> fs []
      >- (Cases_on `s.clock = 0` >> fs []
          ...
          >> pop_assum mp_tac \\ (impl_tac >- fs[]) \\ strip_tac
          >> qpat_abbrev_tac `smnew = OPTION_MAP2 MAX _ _`
          >> qpat_abbrev_tac `ssnew = (_ /\ _)`
...
Theorem evaluate_zero_limits_FST[local]:
  ∀c s r s'.
   FST(evaluate (c,s)) =
   FST(evaluate (c,s with limits := (zero_limits with arch_64_bit :=
   s.limits.arch_64_bit)))
Proof
  rw[] >>
  Cases_on `evaluate (c,s)` >>
  Q.PAT_ABBREV_TAC `X = zero_limits with arch_64_bit := _` >>
  `arch_size X = arch_size s.limits` by (simp[Abbr`X`]
     \\ simp[arch_size_def,zero_limits_def]
  )
...
Theorem evaluate_zero_limits_ffi[local]:
  ∀c s r s'.
   ...
   (SND(evaluate (c,s with limits := zero_limits with arch_64_bit :=
   s.limits.arch_64_bit))).ffi
Proof
  rw[] >>
  Cases_on `evaluate (c,s)` >>
  Q.PAT_ABBREV_TAC `X = zero_limits with arch_64_bit := _` >>
...
Proof
  rw[do_app_def,do_stack_def,do_space_def,do_app_aux_def,do_install_def,with_fresh_ts_def,
     ...
     CaseEq "v",CaseEq"ref",CaseEq"ffi_result",CaseEq"eq_result",
...
∀c s r s' limits.
         ...
         ∃smx safe peak.
             ...
             (r,
              s' with
              <|limits := limits; stack_max := smx; safe_for_space := safe;
                peak_heap_length := peak|>)
Proof
  rw[evaluate_swap_limits]
QED
...
Proof
  rpt strip_tac >>
  drule_then(qspec_then `s.limits` strip_assume_tac) evaluate_swap_limits2 >>
