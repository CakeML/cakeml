...
val do_app_with_stack = time Q.prove(
  `do_app op vs arch_size (s with stack := z) =
   map_result (λ(x,y). (x,y with <| stack := z ;
                                    safe_for_space := do_app_safe op vs (s with stack := z)|>))
              I (do_app op vs arch_size s)`,
  Cases_on `do_app op vs arch_size (s with stack := z)`
  \\ Cases_on `op`
...
val do_app_with_locals = time Q.prove(
  `do_app op vs arch_size (s with locals := z) =
   map_result (λ(x,y). (x,y with <| locals := z ;
                                    safe_for_space := do_app_safe op vs (s with locals := z)|>))
                       I (do_app op vs arch_size s)`,
  Cases_on `do_app op vs arch_size (s with locals := z)`
  \\ Cases_on `op`
  \\ ntac 2 (fs [do_app_aux_def,list_case_eq,option_case_eq,v_case_eq,
              bool_case_eq,ffiTheory.call_FFI_def,do_app_def,do_space_def,
              with_fresh_ts_def,closSemTheory.ref_case_eq,do_install_def,
...
Theorem do_app_err:
   do_app op vs arch_size s = Rerr e ⇒ (e = Rabort Rtype_error)
                             \/
                             (?i x. op = FFI i /\ e = Rabort (Rffi_error x))
Proof
...
Theorem do_app_const:
   do_app op vs arch_size x = Rval (y,z) ⇒
    z.stack = x.stack ∧ z.handler = x.handler ∧ z.locals = x.locals ∧
    z.clock = x.clock ∧ z.compile = x.compile
Proof
...
Theorem do_app_locals:
   (do_app op x arch_size s = Rval (q,r)) ==>
   (do_app op x arch_size (s with locals := extra) =
...
Theorem Seq_Skip:
  evaluate (Seq c Skip,s) arch_size = evaluate (c,s) arch_size
Proof
  full_simp_tac(srw_ss())[evaluate_def]
  \\ Cases_on `evaluate (c,s) arch_size` \\ full_simp_tac(srw_ss())[LET_DEF] \\ SRW_TAC [] []
QED

Theorem evaluate_safe_alt:
  !c s arch_size. evaluate_safe c s arch_size = (SND (evaluate(c,s) arch_size)).safe_for_space
Proof
  rw [evaluate_safe_def,SND]
  \\ Cases_on `evaluate(c,s) arch_size`
  \\ rw []
QED

Theorem do_app_safe_swap:
  ∀op vs arch_size s q s' safe. do_app op vs arch_size s = Rval (q,s')
    ⇒ do_app op vs arch_size (s with safe_for_space := safe) =
...
Proof
  ...
  \\ fs [list_case_eq,option_case_eq,v_case_eq,bool_case_eq,closSemTheory.ref_case_eq
        ...
        , semanticPrimitivesTheory.eq_result_case_eq,pair_case_eq]
...
Theorem do_app_err_safe_swap:
  ∀op vs arch_size s e safe. do_app op vs arch_size s = Rerr e
    ⇒ do_app op vs arch_size (s with safe_for_space := safe) =
      Rerr e
Proof
  Cases \\ rw [do_app_def
...
Theorem evaluate_safe_swap:
  ∀c s arch_size r s' safe. evaluate (c,s) arch_size = (r,s') ⇒
    evaluate (c,s with safe_for_space := safe) arch_size =
      (r,s' with safe_for_space := evaluate_safe c (s with safe_for_space :=
      safe) arch_size)
Proof
  let val full_fs = fs[ get_var_def, set_var_def
   ...
   in recInduct evaluate_ind \\ REPEAT STRIP_TAC
      ...
      >- (fs[evaluate_def] (* Seq *)
         ...
         \\ every_case_tac
         \\ fs [evaluate_safe_alt,evaluate_def]
         \\ qpat_x_assum `∀safe. evaluate (c1,_) arch_size = _` (ASSUME_TAC o Q.SPEC `safe`)
      ...
      >- (fs [evaluate_def]
         ...
         \\ rw []
         >- (Q.EXISTS_TAC `NONE`
            \\ qmatch_asmsub_abbrev_tac `evaluate s_foo _ = (_,s')`
            \\ qmatch_asmsub_abbrev_tac `evaluate (prog,s_bar) _ = (NONE, s'_bar)`
            \\ Q.EXISTS_TAC `s'_bar`
            \\ qmatch_goalsub_abbrev_tac `evaluate (prog,bar_s) _`
            \\ `bar_s = s_bar`
         ...
         \\ qpat_x_assum `evaluate _ _ = (_,s2)` (K ALL_TAC)
         \\ qmatch_asmsub_abbrev_tac `evaluate (prog,s_bar) _ = (SOME v6, s'_bar)`
         \\ Q.EXISTS_TAC `s'_bar`
         \\ qmatch_goalsub_abbrev_tac `evaluate (prog,bar_s) _`
         \\ `bar_s = s_bar`
               by (UNABBREV_ALL_TAC \\ Cases_on `handler` \\ fs[push_env_def])
...
Theorem evaluate_stack_swap:
  !c ^s arch_size.
     let sfs = (λxs. evaluate_safe c (s with stack := xs) arch_size)
     in case evaluate (c,s) arch_size of
...
Proof
  ...
  >- (fs[evaluate_def,LET_DEF] (* Seq *)
     \\ Cases_on `evaluate (c1,s) arch_size` \\ fs[LET_DEF]
     \\ Cases_on `evaluate (c2,r) arch_size` \\ fs[LET_DEF]
  ...
  >- (fs[evaluate_def,evaluate_safe_def] (* If *)
     \\ Cases_on `evaluate (c1,s) arch_size` \\ fs[LET_DEF]
     \\ Cases_on `evaluate (c2,s) arch_size` \\ fs[LET_DEF]
  ...
  >- (fs[evaluate_def,evaluate_safe_alt] (* Call *)
    ...
    \\ Cases_on `evaluate (r,call_env q (push_env x' (IS_SOME handler)
    (dec_clock ^s))) arch_size` \\ fs[]
    \\ Cases_on `q''` \\ fs []
    \\ Cases_on `x''` \\ fs[]
    >- (Cases_on `handler`
    ...
    \\ Cases_on `x''` \\ fs[]
    \\ Q.MATCH_ASSUM_RENAME_TAC `evaluate (r,call_env q (push_env x8 T
    (dec_clock s))) arch_size =
          (SOME (Rerr (Rraise b)),s9)`
    \\ Cases_on `evaluate (r''',set_var q'' b s9) arch_size` \\ fs[]
    ...
    >- (STRIP_TAC (* NONE *)
      ...
      \\ ONCE_ASM_REWRITE_TAC [] \\ rw []
      \\ qpat_abbrev_tac `ss = (SND (evaluate _ _)).safe_for_space`
    ...
    THEN1 (* Rraise *)
     (FIRST_ASSUM (MP_TAC o Q.SPEC `(call_env q (push_env x8 T
      ...
      \\ Cases_on `h` \\ rfs [] \\ fs[]
      \\ qpat_abbrev_tac `sf = (SND (evaluate (r,_) _)).safe_for_space`
...
Theorem evaluate_stack:
   !c ^s arch_size.
      case evaluate (c,s) arch_size of
      | (SOME (Rerr(Rabort Rtype_error)),s1) => T
...
Theorem evaluate_NONE_jump_exc:
  (evaluate (c,^s) arch_size = (NONE,u1)) /\ (jump_exc u1 = SOME x) ==>
    (jump_exc s = SOME (s with <| stack := x.stack ;
                                  handler := x.handler ;
                                  locals := x.locals |>))
Proof
  REPEAT STRIP_TAC \\ MP_TAC (Q.SPECL [`c`,`s`,`arch_size`] evaluate_stack) \\ full_simp_tac(srw_ss())[]
...
Theorem evaluate_NONE_jump_exc_ALT:
   (evaluate (c,^s) arch_size = (NONE,u1)) /\ (jump_exc s = SOME x) ==>
...
val evaluate_locals_LN_lemma = Q.prove(
  `!c ^s arch_size.
      ...
      ((SND (evaluate (c,s) arch_size)).locals = LN) \/
      ?t. FST (evaluate (c,s) arch_size) = SOME (Rerr(Rraise t))`,
...
Theorem evaluate_locals_LN:
   !c ^s res t arch_size.
      (evaluate (c,s) arch_size = (res,t)) /\ res <> NONE /\ res <> SOME (Rerr(Rabort Rtype_error)) ==>
...
Theorem evaluate_locals:
   !c s arch_size res s2 vars l.
      ...
      ?w safe. (evaluate (c, s with locals := l) arch_size =
             (res,if res = NONE
...
Theorem evaluate_mk_ticks:
   !p s n.
    evaluate (mk_ticks n p, s) arch_size
    =
    if s.clock < n then
      (SOME (Rerr(Rabort Rtimeout_error)), s with <| clock := 0; locals := fromList []; stack := [] |>)
    else
      evaluate (p, FUNPOW dec_clock n s) arch_size
Proof
  Induct_on `n` >>
...
Theorem do_app_Rerr:
   dataSem$do_app op vs arch_size s = Rerr e ⇒ (e = Rabort Rtype_error)
                             \/
                             (?i x. op = FFI i /\ e = Rabort (Rffi_error x))
Proof
...
Theorem do_app_with_clock:
  do_app op vs arch_size (s with clock := z) =
   map_result (λ(x,y). (x,y with clock := z)) I (do_app op vs arch_size s)
Proof
  ...
  Cases_on `do_app op vs arch_size s` >>
...
Theorem do_app_change_clock:
   (do_app op args arch_size s1 = Rval (res,s2)) ==>
   (do_app op args arch_size (s1 with clock := ck) = Rval (res,s2 with clock := ck))
Proof
  Cases_on `op = Install` THEN1
   (fs [do_app_def,do_install_def]
    \\ every_case_tac \\ fs []
...
Theorem do_app_change_clock_err:
  (do_app op args arch_size s1 = Rerr e) ==>
  (do_app op args arch_size (s1 with clock := ck) = Rerr e)
Proof
  Cases_on `op = Install` THEN1
   (fs [do_app_def,do_install_def]
    \\ every_case_tac \\ fs []
...
Theorem do_app_io_events_mono:
   do_app x y arch_size z = Rval (a,b) ⇒
...
Theorem semantics_Term_IMP_PREFIX:
   semantics ffi prog co cc start arch_size = Terminate tt l ==> ffi.io_events ≼ l
Proof
...
Theorem Resource_limit_hit_implements_semantics:
  implements {Terminate Resource_limit_hit ffi.io_events}
       {semantics ffi (fromAList prog) co cc start arch_size}
Proof
  fs [implements_def,extend_with_resource_limit_def]
  \\ Cases_on `semantics ffi (fromAList prog) co cc start arch_size` \\ fs []
