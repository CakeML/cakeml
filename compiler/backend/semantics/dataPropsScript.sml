...
val do_app_with_stack = time Q.prove(
  ...
  \\ ntac 2 (fs [do_app_aux_def,list_case_eq,option_case_eq,v_case_eq,
          ...
          rw[EQ_IMP_THM] >> rw[stack_consumed_def,allowed_op_def]
    );
...
val do_app_with_locals = time Q.prove(
  ...
  \\ ntac 2 (fs [do_app_aux_def,list_case_eq,option_case_eq,v_case_eq,
          ...
          rveq >> fs []) >>
          fs [allowed_op_def]>>
          rw [state_component_equality] \\ simp [Once CONJ_COMM] \\ rw[EQ_IMP_THM] >> rw[stack_consumed_def,allowed_op_def]);
...
Triviality size_of_cons_le_lemma:
  size_of (x::xs) refs seen = (s,nrefs,nseen) /\
  size_of (xs) refs seen = (s',nrefs',nseen') ==>
  s' <= s
Proof
  Cases_on `xs` >> rw[size_of_def] >>
  rpt(pairarg_tac >> fs[] >> rveq)
QED

Triviality size_of_append_le_lemma:
  !xs ys refs seen s nrefs nseen s' nrefs' nseen'.
  size_of (xs ++ ys) refs seen = (s,nrefs,nseen) /\
  size_of (ys) refs seen = (s',nrefs',nseen') ==>
  s' <= s
Proof
  Induct >> rw[] >> fs[] >>
  Cases_on `size_of (xs ++ ys) refs seen` >> Cases_on `r` >>
  res_tac >> imp_res_tac size_of_cons_le_lemma >> fs[]
QED

Theorem size_of_cons_le:
  FST(size_of xs refs seen) <= FST(size_of (x::xs) refs seen)
Proof
  Cases_on `size_of xs refs seen` >> Cases_on `r` >>
  Cases_on `size_of (x::xs) refs seen` >> Cases_on `r` >> metis_tac[FST,size_of_cons_le_lemma]
QED

Theorem size_of_append_le:
  FST(size_of ys refs seen) <= FST(size_of (xs ++ ys) refs seen)
Proof
  Cases_on `size_of ys refs seen` >> Cases_on `r` >>
  Cases_on `size_of (xs++ys) refs seen` >> Cases_on `r` >> metis_tac[FST,size_of_append_le_lemma]
QED

Theorem size_of_seen_grows:
  !xs refs seen n nrefs nseen.
  size_of xs refs seen = (n,nrefs,nseen) ==>
  domain seen ⊆ domain nseen
Proof
  ho_match_mp_tac size_of_ind >> rw[size_of_def] >>
  fs[CaseEq "option",CaseEq"prod",CaseEq"bool",CaseEq"ref"] >>
  rpt(pairarg_tac >> fs[] >> rveq) >>
  metis_tac[SUBSET_TRANS]
QED

Triviality SOME_NONE_NOT_EQ:
  x <> SOME() <=> x = NONE
Proof
  Cases_on `x` >> rw[]
QED
...
Proof
  rw[size_of_heap_def,ELIM_UNCURRY,cut_locals_def] >>
  PURE_TOP_CASE_TAC >-
    (fs[SimpL ``arithmetic$<=``,stack_to_vs_def,toList_def,toListA_def] >>
     fs[stack_to_vs_def] >> metis_tac[APPEND_ASSOC,size_of_append_le]) >>
  PURE_TOP_CASE_TAC >-
    (fs[SimpL ``arithmetic$<=``,stack_to_vs_def,toList_def,toListA_def]) >>
  fs[cut_env_def] >> rveq >>
  fs[stack_to_vs_def] >>
