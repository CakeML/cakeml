...
(store_alloc (W8array (REPLICATE (Num (ABS ( n))) w)) s.refs)
    ...
    | (Op Aw8sub_unsafe, [Loc lnum; Litv (IntLit i)]) =>
        (case store_lookup lnum s.refs of
            SOME (W8array ws) =>
              if i <( 0 : int) then
                NONE
    ...
    | (Op Aw8update_unsafe, [Loc lnum; Litv (IntLit i); Litv (Word8 w)]) =>
        (case store_lookup lnum s.refs of
          SOME (W8array ws) =>
            if i <( 0 : int) then
              NONE
...
(store_alloc (Varray (REPLICATE (Num (ABS ( n))) v)) s.refs)
    ...
    | (Op Asub_unsafe, [Loc lnum; Litv (IntLit i)]) =>
        (case store_lookup lnum s.refs of
            SOME (Varray vs) =>
              if i <( 0 : int) then
                NONE
    ...
    | (Op Aupdate_unsafe, [Loc lnum; Litv (IntLit i); v]) =>
        (case store_lookup lnum s.refs of
          SOME (Varray vs) =>
            if i <( 0 : int) then
              NONE
