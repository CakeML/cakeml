...
Theorem simple_val_rel_Word_left:
  simple_val_rel vr ==> !x w. vr (Word w) x = (x = Word w)
Proof
...
Theorem val_rel_to_words_lemma:
  simple_val_rel vr ==> (!xs ys. LIST_REL vr xs ys ==>
    !ns. (xs = MAP (Word o w2v) (ns:word64 list)) = (ys = MAP (Word o w2v) ns))
Proof
  disch_tac \\ ho_match_mp_tac LIST_REL_ind
  \\ rw []
  \\ Cases_on `ns` \\ fs []
  \\ imp_res_tac simple_val_rel_Word_left
  \\ eq_tac \\ rw [] \\ fs []
  \\ rfs [simple_val_rel_alt]
QED

Theorem v_rel_to_list_byte2:
 simple_val_rel vr ⇒
     ∀y x.
         vr x y ⇒
         ∀ns.
             v_to_list x = SOME (MAP (Word o w2v) ns) ⇔
