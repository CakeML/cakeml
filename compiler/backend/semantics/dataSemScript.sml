...
Definition word_size_def:
  word_size arch64 w = let arch_size = if arch64 then 64 else 32 in
      data_space$alloc_size (LENGTH w) arch_size
End
...
Definition size_of_def:
  ...
  (size_of lims (x::y::ys) refs seen =
    ...
    let (n2,refs2,seen2) = size_of lims [x] refs1 seen1 in
      (n1+n2,refs2,seen2)) /\
  (size_of lims [Word w] refs seen = (word_size lims.arch_64_bit w, refs, seen)) /\
...
val do_space_def = Define `
  do_space op vs ^s =
    ...
    then  SOME (s with <| space := 0
                        ...
                        |>)
    else if op_space_req op (LENGTH vs) (arch_size s.limits) = 0 then SOME s
...
(* TODO: wrong *)
∧ (lim_safe lims (WordToInt _) _ =
   (1 < lims.length_limit)
  )
∧ (lim_safe lims (WordFromInt _) _ =
   (1 < lims.length_limit)
  )
∧ (lim_safe lims (WordOp _ _) _ =
   (1 < lims.length_limit)
  )
∧ (lim_safe lims (WordShift _ _ _) _ =
   (1 < lims.length_limit)
  )
∧ (lim_safe lims (WordToWord _ _) _ =
   (1 < lims.length_limit)
  )
...
val do_app_aux_def = Define `
  do_app_aux op ^vs ^s =
            ...
            if 0 ≤ i ∧ (LENGTH b=8) then
              let ptr = (LEAST ptr. ¬(ptr IN domain s.refs)) in
                Rval (RefPtr ptr, s with <|refs := insert ptr
                  (ByteArray f (REPLICATE (Num i) (v2w b))) s.refs|>)
            else Rerr (Rabort Rtype_error)
          | _ => Rerr (Rabort Rtype_error))
            ...
            (if 0 ≤ i ∧ i < &LENGTH bs ∧ (LENGTH b = 8)
             then
               Rval (Unit, s with refs := insert ptr
                 (ByteArray f (LUPDATE (v2w b) (Num i) bs)) s.refs)
             else Error)
         | _ => Error)
    ...
    | (CopyByte T,[RefPtr src; Number srcoff; Number len]) =>
       (case lookup src s.refs of
        | SOME (ByteArray _ ws) =>
           (case copy_array (ws,srcoff) len NONE of
            | SOME ds =>
              let ptr = (LEAST ptr. ~(ptr IN domain s.refs)) in
              Rval (RefPtr ptr, s with
                    refs := insert ptr (ByteArray T ds) s.refs)
            | _ => Error)
        | _ => Error)
