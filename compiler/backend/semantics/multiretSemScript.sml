(*
  The formal semantics of MultiRetLang
*)
open preamble multiretLangTheory;
local open backend_commonTheory bvlSemTheory in end;

val _ = temp_delsimps ["lift_disj_eq", "lift_imp_disj"]

val _ = new_theory "multiretSem";

Overload num_stubs[local] = “bvl_num_stubs”
Type exp[local] = “:multiretLang$exp”
...
Datatype:
  state =
    <| refs    : num |-> bvlSem$v ref
     ; clock   : num
     ; global  : num option
     ; compile : 'c -> (num # num # exp) list -> (word8 list # word64 list # 'c) option
     ; compile_oracle : num -> 'c # (num # num # exp) list
     ; code    : (num # exp) num_map
     ; ffi     : 'ffi ffi_state |>
End
...
val s = ``(s:('c,'ffi) multiretSem$state)``
...
(* The semantics of expression evaluation is defined next. For
   ...
   defined to evaluate a list of expressions. *)
...
Theorem do_app_const:
   (multiretSem$do_app op args s1 = Rval (res,s2)) ==>
    (s2.clock = s1.clock)
Proof
...
Theorem evaluate_def[compute] = REWRITE_RULE [fix_clock_evaluate] evaluate_def;
...
Definition semantics_def:
  ...
  let es = [multiretLang$Call 0 (SOME start) [] NONE] in
