...
val installed_def = Define`
  installed bytes cbspace bitmaps data_sp ffi_names (r1,r2) (mc_conf:('a,'state,'b) machine_config) ms shmem_extra ⇔
    ∃t m io_regs cc_regs bitmap_ptr bitmaps_dm sdm.
      ...
      ?old_ffi_names. ffi_names = SOME old_ffi_names ∧
      ~MEM "MappedRead" old_ffi_names ∧ ~MEM "MappedWrite" old_ffi_names ∧
      mc_conf.ffi_names = old_ffi_names ++ FST shmem_extra ∧
      MAP (\rec. rec.entry_pc + mc_conf.target.get_pc ms) (SND shmem_extra) =
