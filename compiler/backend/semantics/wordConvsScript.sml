...
Definition inst_ok_less_def:
  ...
  (inst_ok_less c (Mem m r (Addr r' w)) ⇔
     if m IN {Load; Store; Load32; Store32} then addr_offset_ok c w
     else if m IN { Load16; Store16} then (if c.ISA = Ag32 then T else hw_offset_ok c w)
...
Definition full_inst_ok_less_def:
  ...
  (full_inst_ok_less c (ShareInst op r ad) =
    ...
    | SOME (Addr _ w) =>
      ...
      then (if c.ISA = Ag32 then T else hw_offset_ok c w)
      else byte_offset_ok c w
    | NONE => F) ∧
