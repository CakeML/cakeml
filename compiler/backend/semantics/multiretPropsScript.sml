(*
  Properties about MultiRetLang and its semantics
*)
open preamble multiretSemTheory;
local open bvlPropsTheory in end;

val _ = temp_delsimps ["lift_disj_eq", "lift_imp_disj"]

val _ = new_theory"multiretProps";
...
Theorem bvl_to_multiret_id:
   bvl_to_multiret (multiret_to_bvl s) s = s
Proof
  EVAL_TAC \\ full_simp_tac(srw_ss())[multiretSemTheory.state_component_equality]
QED

Theorem bvl_to_multiret_with_refs:
   bvl_to_multiret (x with refs := y) z = bvl_to_multiret x z with <| refs := y |>
Proof
  EVAL_TAC
QED

Theorem bvl_to_multiret_with_clock:
   bvl_to_multiret (x with clock := y) z = bvl_to_multiret x z with <| clock := y |>
Proof
  EVAL_TAC
QED

Theorem bvl_to_multiret_with_ffi:
   bvl_to_multiret (x with ffi := y) z = bvl_to_multiret x z with ffi := y
Proof
  EVAL_TAC
QED

Theorem bvl_to_multiret_code[simp]:
   (bvl_to_multiret x y).code = y.code
Proof
  EVAL_TAC
QED

Theorem bvl_to_multiret_clock[simp]:
   (bvl_to_multiret x y).clock = x.clock
Proof
  EVAL_TAC
QED

Theorem multiret_to_bvl_refs[simp]:
   (multiret_to_bvl x).refs = x.refs
Proof
  EVAL_TAC
QED

Theorem multiret_to_bvl_code[simp]:
   (multiret_to_bvl x).code = map (K ARB) x.code
Proof
  EVAL_TAC
QED

Theorem multiret_to_bvl_clock[simp]:
   (multiret_to_bvl x).clock = x.clock
Proof
  EVAL_TAC
QED

Theorem multiret_to_bvl_ffi[simp]:
   (multiret_to_bvl x).ffi = x.ffi
Proof
  EVAL_TAC
QED

Theorem multiret_to_bvl_to_multiret_with_ffi:
   bvl_to_multiret (multiret_to_bvl x with ffi := f) x = x with ffi := f
Proof
  EVAL_TAC \\ rw[state_component_equality]
QED

Theorem domain_multiret_to_bvl_code[simp]:
   domain (multiret_to_bvl s).code = domain s.code
Proof
  srw_tac[][multiret_to_bvl_def,domain_map]
QED
...
val case_eq_thms =
  CONJ (CaseEq "multiretSem$result") $
  CONJ
  (prove_case_eq_thm {nchotomy = multiretLangTheory.exp_nchotomy, case_def = multiretLangTheory.exp_case_def})
...
val inc_clock_def = Define `
  inc_clock n (s:('c,'ffi) multiretSem$state) = s with clock := s.clock + n`;
...
Theorem dec_clock_with_code[simp]:
   multiretSem$dec_clock n (s with code := c) = dec_clock n s with code := c
Proof
  EVAL_TAC
QED
...
Theorem dec_clock0[simp]:
   !n (s:('c,'ffi) multiretSem$state). dec_clock 0 s = s
Proof
...
val do_app_inv_clock = Q.prove(
  ...
  \\ Q.SPEC_TAC(`REVERSE a`,`a`) \\ gen_tac \\ CASE_TAC
  \\ fs[multiretSemTheory.do_app_def,case_eq_thms,pair_case_eq,
        inc_clock_def,bvl_to_multiret_def,multiret_to_bvl_def] \\ rw[] \\ rfs []
...
Proof
  ...
  THEN1
   (gvs [CaseEq"prod"]
    \\ Cases_on ‘v2 = Rerr (Rabort Rtimeout_error)’ \\ gvs []
    \\ gvs [AllCaseEqs()])
  ...
  THEN1 (* LetCall *)
   (gvs [CaseEq"prod"]
    \\ Cases_on ‘v5 = Rerr (Rabort Rtimeout_error)’ \\ gvs []
    \\ gvs [CaseEq"multiretSem$result"]
    \\ gvs [CaseEq"option",CaseEq"prod"]
    \\ gvs [CaseEq"bool",CaseEq"prod"]
    \\ IMP_RES_TAC dec_clock_inv_clock \\ gvs []
    \\ Cases_on ‘v = Rerr (Rabort Rtimeout_error)’ \\ gvs []
    \\ gvs [AllCaseEqs()])
  THEN1 (* TailCall *)
   (gvs [CaseEq"prod"]
    \\ Cases_on ‘v5 = Rerr (Rabort Rtimeout_error)’ \\ gvs []
...
Proof
  rw[do_app_def,case_eq_thms,pair_case_eq,bvl_to_multiret_def] \\ rw[] \\
...
val evaluate_global_mono_lemma = Q.prove(
  ...
  \\ Cases_on `op = Install`
  \\ fs[do_app_def,case_eq_thms,pair_case_eq] \\ rw[bvl_to_multiret_def]
  \\ fs[do_app_aux_def,case_eq_thms] \\ rw[]
...
Proof
  rw[multiretSemTheory.do_app_def,case_eq_thms,pair_case_eq] >>
...
Theorem do_app_with_code:
   multiretSem$do_app op vs s = Rval (r,s') ⇒
...
Proof
  ...
  \\ fs[bvl_to_multiret_def,multiret_to_bvl_def,bvlSemTheory.do_app_def,case_eq_thms]
  \\ TRY (pairarg_tac \\ fs [])
...
Theorem do_app_with_code_err:
   multiretSem$do_app op vs s = Rerr e ⇒
...
Proof
  ...
  \\ imp_res_tac bvlPropsTheory.do_app_change_clock
  \\ TRY (pairarg_tac \\ fs [])
  \\ fs[multiret_to_bvl_def,bvl_to_multiret_def]
  \\ fs [do_install_def,UNCURRY] \\ every_case_tac \\ fs []
QED
...
Proof
  ...
  >- (* TailCall *)
   (Cases_on `evaluate (xs,env,s1)` >>
    ...
    rev_full_simp_tac(srw_ss())[inc_clock_def, dec_clock_def] >>
    `ck + r.clock - (ticks + 1) = r.clock - (ticks + 1) + ck` by srw_tac [ARITH_ss] [ADD1] >>
    last_x_assum $ qspec_then`ck`mp_tac >> gvs [] >>
    gvs [AllCaseEqs()])
QED
...
Proof
  ...
  \\ fs[bvl_to_multiret_def,multiret_to_bvl_def]
...
Theorem evaluate_genlist_vars:
   !skip env n (st:('c,'ffi) multiretSem$state).
    n + skip ≤ LENGTH env ⇒
...
val get_code_labels_def = tDefine"get_code_labels"
  ...
  (wf_rel_tac`measure exp_size`
   \\ simp[multiretLangTheory.exp_size_def]
   \\ rpt conj_tac \\ rpt gen_tac
   \\ Induct_on`es`
   \\ rw[multiretLangTheory.exp_size_def]
   \\ simp[] \\ res_tac \\ simp[]);
