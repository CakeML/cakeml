...
(* This is backend$compile but using inc_config and without labLang phase *)
...
(* This mirrors compile_cake from backend_asmTheory without labLang phases *)
...
(* TODO: remove [local] tag in backend_asm *)
...
(* sanity check *)
Theorem compile_cake_alt_thm:
  compile_cake_alt ac c p = SOME (c',bm,compiled_p_lab) ⇒
  compile_alt ac c p = (c',bm,compiled_p_lab)
Proof
  rw[compile_cake_alt_def,compile_alt_def,data_to_word_compile_orac_def]>>
  rpt(pairarg_tac>>gvs[])>>
  gvs[AllCaseEqs()]>>
  drule word_to_word_inlogic_thm>>
  strip_tac>>gvs[]>>
  rpt(pairarg_tac>>gvs[])>>
  gvs[data_to_wordTheory.compile_0_def,data_to_wordTheory.compile_def]
QED
...
Theorem each_inlogic_some_some:
  ...
  each_inlogic asm_conf rest = SOME p1' ⇒
  each_inlogic asm_conf (((name_num,arg_count,prog),h')::rest) = SOME (h ++ p1')
Proof
  strip_tac >>
  last_x_assum mp_tac >>
  simp[each_inlogic_def] >>
  every_case_tac >>
  rw[]
QED

Theorem each_inlogic_head_none:
  each_inlogic asm_conf [((name_num,arg_count,prog),h')] = NONE ⇒
  each_inlogic asm_conf (((name_num,arg_count,prog),h')::rest) = NONE
Proof
  rw[each_inlogic_def] >>
  every_case_tac >>
  gvs[each_inlogic_def]
...
Theorem each_inlogic_rest_none:
  each_inlogic asm_conf rest = NONE ⇒
  each_inlogic asm_conf (((name_num,arg_count,prog),h')::rest) = NONE
Proof
  rw[each_inlogic_def] >>
  every_case_tac >>
...
Theorem each_inlogic_append:
  ...
  each_inlogic asm_conf (ZIP (p2, p2_orac)) = SOME p2' ⇒
  each_inlogic asm_conf (ZIP (p1 ++ p2, p1_orac ++ p2_orac)) = SOME (p1' ++ p2')
Proof
  Induct_on ‘p1’ >>
  rw[ZIP_def, each_inlogic_def] >>
  Cases_on ‘p1_orac’
  >- (fs[])
  >- (qpat_x_assum ‘SUC _ = _ ’ mp_tac >>
      ...
      strip_tac >>
      Cases_on ‘each_inlogic asm_conf [(((name_num,arg_count,prog),h'))]’
      >- (drule each_inlogic_head_none >>
          rw[] >>
          qexists ‘ZIP (p1,t)’ >> gvs[])
      >- (Cases_on ‘each_inlogic asm_conf (ZIP(p1, t))’
          >- (drule each_inlogic_rest_none >>
              rw[] >> gvs[])
          >- (pop_assum mp_tac >>
              drule each_inlogic_some_some >>
              disch_then (fn th => strip_tac >> drule th) >>
              rw[] >>
              gvs[] >>
...
Definition init_icompile_def:
  ...
  let (col, word_init1) = word_to_word$compile word_to_word_conf asm_conf word_init in
...
Definition end_icompile_def:
  ...
  let (col, word_end1) = word_to_word$compile word_to_word_conf asm_conf word_end in
...
Definition icompile_def:
  ...
  let (col, icompiled_p_word1) = word_to_word$compile word_to_word_conf asm_conf icompiled_p_word in
...
Definition fold_icompile_def:
  (fold_icompile ic asm_conf [] = SOME(ic,[]))
  ∧
...
Definition fold_icompile_cake_def:
  (fold_icompile_cake ic asm_conf [] [] = SOME (ic,[])) ∧
  (fold_icompile_cake ic asm_conf (p :: ps) (orac::oracs) =
  case icompile_cake ic asm_conf p orac of NONE => NONE
  | SOME (ic',  p') =>
    (case fold_icompile_cake ic' asm_conf ps oracs of NONE => NONE
    | SOME (ic'', ps') =>
      SOME (ic'', p' ++ ps'))) ∧
  (fold_icompile_cake ic asm_conf _ _ = NONE)
End
...
Proof
  ...
  gvs[source_to_sourceTheory.compile_def, source_letTheory.compile_decs_def] >>
...
Proof
  rw[] >>
  gvs[icompile_cake_def] >> rpt (pairarg_tac >> gvs[]) >> rw[source_to_source_compile_append] >>
  gvs[AllCaseEqs()] >> rpt (pairarg_tac >> gvs[]) >>
  every_case_tac >> gvs[] >>
  rpt (pairarg_tac >> gvs[]) >>
  drule_all icompile_icompile_source_to_flat >> strip_tac >> gvs[] >>
...
Theorem init_icompile_icompile_end_icompile_d2w_orac:
  ...
  data_to_word_compile_orac asm_conf data_conf (word_conf with col_oracle := init_orac ++ ic_orac ++ end_orac)
                       (data_init ++ data_ic ++ data_end) =
    SOME (word_conf with col_oracle := [], word_init1 ++ word_ic1 ++ word_end1)
Proof
...
Theorem word_to_word_compile_empty_orac:
  word_conf_ok word_to_word_conf ∧
  word_to_word$compile word_to_word_conf asm_conf p = (col, p') ⇒
  col = []
Proof
  rw[word_conf_ok_def, word_to_wordTheory.compile_def, word_to_wordTheory.next_n_oracle_def]
  >> rw[DROP_def]
QED
...
Proof
  ...
  >- (drule word_to_word_compile_empty_orac >> disch_then drule >>
      strip_tac >>
      qpat_x_assum ‘compile _ _ _ = (col'', _)’ assume_tac >>
      drule word_to_word_compile_empty_orac >> disch_then drule >>
      strip_tac >>
      drule word_to_word_compile_empty_orac >> disch_then rev_drule >>
      strip_tac >> gvs[word_conf_ok_def] >>
      rw[backendTheory.inc_config_component_equality]  >>
      rw[word_to_wordTheory.config_component_equality]
  )
...
Theorem init_icompile_icompile_end_icompile_cake:
  ...
  conf_ok c
  ⇒
  let word_to_word_conf = c.inc_word_to_word_conf with col_oracle := init_orac ++ ic_orac ++ end_orac in
  let c' = c with inc_word_to_word_conf := word_to_word_conf in
  let compiled_p = lab_init ++ icompiled_p_lab ++ lab_end in
  compile_cake_alt asm_conf c' p = SOME (c_after_ic, bm_after_ic, compiled_p)
Proof
  rw[init_icompile_cake_def, icompile_cake_def, end_icompile_cake_def, conf_ok_def] >>
  gvs[AllCaseEqs()] >> rpt (pairarg_tac >> gvs[]) >> every_case_tac >> gvs[] >>
  rpt (pairarg_tac >> gvs[]) >>
  fs[compile_cake_alt_def, conf_ok_def] >> rpt (pairarg_tac >> gvs[]) >>
  (* source-to-flat *)
  drule_all init_icompile_icompile_end_icompile_s2f >>
  ...
  simp[config_prog_pair_rel_def] >> strip_tac >>
  gvs[] >>
  (* bvl-to-bvi and to_data *)
  drule_all init_icompile_icompile_end_icompile_b2b >>
  simp[config_prog_rel_b2b_def] >> strip_tac >> gvs[] >>
  gvs[bvi_to_data_compile_prog_append_all] >>
  (* data-to-word *)
  drule init_icompile_icompile_end_icompile_d2w_orac >>
  disch_then drule>>
  disch_then drule>>
  disch_then rev_drule>>
  strip_tac>>
...
Theorem icompile_eq_cake:
  init_icompile_cake (asm_conf: 'a asm_config) (c: inc_config) init_orac =
    SOME (ic, lab_init)
  ∧
  fold_icompile_cake ic asm_conf p ic_orac = SOME (ic', icompiled_p_lab)
  ∧
  end_icompile_cake asm_conf ic' c end_orac = SOME (c_after_ic, bm_after_ic, lab_end)
  ∧
  conf_ok c
...
(* leave out the reg count because it complicates the definition of fold_icompile_to_livesets *)
Definition icompile_source_to_livesets_def:
  ...
  let ic' = ic with <| source_iconf := source_iconf';
    ...
    SOME (data, ic', pword0: (num # num # 'a wordLang$prog) list)
End
...
Definition init_icompile_source_to_livesets_def:
  ...
  in
    ((reg_count, data), ic, word0_init_lvs)
End
...
Definition end_icompile_source_to_livesets_def:
  ...
  let c' = c with
    ...
    (data, c', word0_end_lvs)
End


Theorem icompile_icompile_source_to_livesets:
  icompile_source_to_livesets asm_conf ic p1 = SOME (d1, ic', p1') ∧
  icompile_source_to_livesets asm_conf ic' p2 = SOME (d2, ic'', p2') ⇒
  icompile_source_to_livesets asm_conf ic (p1 ++ p2) = SOME (d1 ++ d2, ic'', p1' ++ p2')
Proof
...
Definition fold_icompile_source_to_livesets_def:
  fold_icompile_source_to_livesets asm_conf ic [] = SOME ([], ic, [])
  ∧
...
Theorem icompile_source_to_livesets_none:
  ...
  ∧
  (icompile_source_to_livesets asm_conf ic p = SOME (d, ic', some_p) ∧
   icompile_source_to_livesets asm_conf ic' p' = NONE ⇒
   icompile_source_to_livesets asm_conf ic (p ++ p') = NONE)
Proof
...
Theorem init_icompile_icompile_end_icompile_s2lvs:
  init_icompile_source_to_livesets asm_conf c = ((reg_count_ic, d_init), ic, word0_init)
  ∧
  icompile_source_to_livesets asm_conf ic p = SOME (d_ic, ic', icompiled_p)
  ∧
  end_icompile_source_to_livesets asm_conf ic' c = (d_end, c_after_ic, word0_end)
  ∧
...
Theorem icompile_eq_s2livesets:
  init_icompile_source_to_livesets asm_conf c = ((reg_count_ic, d_init), ic, word0_init)
  ∧
  fold_icompile_source_to_livesets asm_conf ic p = SOME (d_ic, ic', icompiled_p)
  ∧
  end_icompile_source_to_livesets asm_conf ic' c = (d_end, c_after_ic, word0_end)
  ∧
