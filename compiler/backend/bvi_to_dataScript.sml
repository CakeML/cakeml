...
val iAssign_def = Define `
  iAssign arch_size n1 op vs live env =
    if op_requires_names op then
      ...
      let k = op_space_req op (LENGTH vs) arch_size in
...
val compile_def = tDefine "compile" `
  (compile _ (n:num) (env:num list) tail live [] =
    (Skip,[]:num list,n)) /\
  (compile arch_size n env tail live (x::y::xs) =
     let (c1,v1,n1) = compile arch_size n env F live [x] in
     let (c2,vs,n2) = compile arch_size n1 env F (HD v1::live) (y::xs) in
       (Seq c1 c2, HD v1 :: vs, n2)) /\
  (compile _ n env tail live [(Var v):bvi$exp] =
     let var = any_el v env 0n in
     if tail
     then (Return var, [n], n+1)
     else (Skip, [var], MAX n (var+1))) /\
  (compile arch_size n env tail live [If x1 x2 x3] =
     let (c1,v1,n1) = compile arch_size n env F live [x1] in
     let (c2,v2,n2) = compile arch_size n1 env tail live [x2] in
     let (c3,v3,n3) = compile arch_size n2 env tail live [x3] in
       if tail then
         (Seq c1 (If (HD v1) c2 c3),[n3],n3+1)
       else
         (Seq c1 (If (HD v1) (Seq c2 (Move n3 (HD v2)))
                             (Seq c3 (Move n3 (HD v3)))),[n3],n3+1)) /\
  (compile arch_size n env tail live [Let xs x2] =
     let (c1,vs,n1) = compile arch_size n env F live xs in
     let (c2,v2,n2) = compile arch_size n1 (vs ++ env) tail live [x2] in
       (Seq c1 c2, v2, n2)) /\
  (compile arch_size n env tail live [Raise x1] =
     let (c1,v1,n1) = compile arch_size n env F live [x1] in
       (Seq c1 (Raise (HD v1)), v1, n1)) /\
  (compile arch_size n env tail live [Op op xs] =
     let (c1,vs,n1) = compile arch_size n env F live xs in
     let c2 = Seq c1 (iAssign arch_size n1 op (REVERSE vs) live env) in
       (if tail then Seq c2 (Return n1) else c2, [n1], n1+1)) /\
  (compile arch_size n env tail live [Tick x1] =
     let (c1,v1,n1) = compile arch_size n env tail live [x1] in
       (Seq Tick c1, v1, n1)) /\
  (compile arch_size n env tail live [Call ticks dest xs NONE] =
     let (c1,vs,n1) = compile arch_size n env F live xs in
     let ret = (if tail then NONE
                else SOME (n1, list_to_num_set (live ++ env))) in
       (Seq c1 (mk_ticks ticks (Call ret dest vs NONE)), [n1], n1+1)) /\
  (compile arch_size n env tail live [Call ticks dest xs (SOME handler)] =
     let (c1,vs,n1) = compile arch_size n env F live xs in
     let (c2,v,n2) = compile arch_size (n1+1) (n1::env) F live [handler] in
...
val compile_LESS_EQ_lemma = Q.prove(
  `!arch_size n env tail live xs.
      n <= SND (SND (compile arch_size n env tail live xs))`,
...
Theorem compile_LESS_EQ:
   !arch_size n env tail live xs c vs new_var.
      (compile arch_size n env tail live xs = (c,vs,new_var)) ==> n <= new_var
Proof
...
val compile_LENGTH_lemma = Q.prove(
  `!arch_size n env tail live xs.
      (LENGTH (FST (SND (compile arch_size n env tail live xs))) = LENGTH xs)`,
  HO_MATCH_MP_TAC compile_ind \\ REPEAT STRIP_TAC
  \\ SIMP_TAC std_ss [compile_def] \\ SRW_TAC [] []
...
Theorem compile_LENGTH:
   !arch_size n env tail live xs c vs new_var.
      (compile arch_size n env tail live xs = (c,vs,new_var)) ==> (LENGTH vs = LENGTH xs)
Proof
...
Theorem compile_SING_IMP:
   (compile arch_size n env tail live [x] = (c,vs,new_var)) ==> ?t. vs = [t]
Proof
...
val optimise_def = Define `
  optimise arch_size prog = data_space$compile (simp (FST (data_live$compile prog LN arch_size)) Skip) arch_size`
...
val compile_exp = Define `
  compile_exp arch_size arg_count exp =
    optimise arch_size (FST (compile arch_size arg_count (COUNT_LIST arg_count) T [] [exp]))`
