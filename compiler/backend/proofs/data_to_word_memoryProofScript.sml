...
[~RefPtrThunk:] (
  (∃v.
    ...
    v_inv conf v refs (x,f,tf,heap)) ⇒
   v_inv conf (RefPtr _ n) refs (x,f,tf,heap:'a ml_heap))
*)
[~BlockNil:] (
  vs = [] ∧
  n < dimword(:'a) DIV 16 ∧
...
Proof
  `∀conf x refs y f tf heap heap1 a stack ts ts' tag xs.
  ...
  \\ let val ind =
       v_inv_ind
         |> Q.SPEC `λconf v refs (x,f,tf,heap). P conf v refs x f tf heap`
         |> SIMP_RULE std_ss []
         |> Q.GEN `P`
