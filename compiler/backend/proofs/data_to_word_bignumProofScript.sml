...
Proof
  ...
  \\ `env_to_list (LN : α word_loc sptree$num_map) r.permute =
     ([],\n. r.permute (n+1))` by
    (fs [wordSemTheory.env_to_list_def,wordSemTheory.list_rearrange_def])
  \\ fs []
  THEN1
   (fs [wordSemTheory.pop_env_def,wordSemTheory.push_env_def]
    \\ gvs [EVAL “fromAList (toAList (insert 0 ret_val LN))”]
    \\ fs [EVAL ``domain (fromAList [(0,ret_val)])``,wordSemTheory.set_vars_def]
  ...
  \\ gvs [EVAL “(union (fromAList []) (fromAList (toAList (insert 0 ret_val LN))))”]
  \\ gvs [wordSemTheory.set_vars_def]
  \\ fs [wordSemTheory.state_component_equality]
  \\ fs [fromAList_def,insert_shadow]
  \\ gvs [alist_insert_def]
  \\ simp [Once sptreeTheory.insert_def]
...
fun print_tac s goal = let
  val _ = numSimps.clear_arith_caches();
  val _ = print ("print_tac: " ^ s ^ "\n")
  in all_tac goal end;
...
Proof
  ...
  \\ print_tac "AnyArith_thm: impl_tac"
  \\ impl_tac THEN1
   (simp [LENGTH_REPLICATE]
    ...
    \\ fs []
    \\ print_tac "AnyArith_thm: impl_tac (B)"
    ...
    \\ print_tac "AnyArith_thm: impl_tac (C)"
    ...
    \\ print_tac "AnyArith_thm: impl_tac (D)"
    ...
    THEN1
     (Cases_on `j = 0` THEN1
      ...
      \\ print_tac "AnyArith_thm: impl_tac (E)"
      THEN1
       (drule heap_lookup_SPLIT
        \\ strip_tac \\ rveq
        \\ qpat_x_assum `_ (fun2set _)` mp_tac
     ...
     (Cases_on `i = 0` THEN1
      ...
      \\ `lookup 2 s9.locals = SOME (Word w)` by
        (qunabbrev_tac `s9`
         ...
         \\ asm_rewrite_tac [])
      \\ full_simp_tac std_ss [GSYM APPEND_ASSOC]
      \\ print_tac "AnyArith_thm: impl_tac (F)"
      \\ rpt_drule word_ml_inv_get_var_IMP
      ...
      THEN1
       (drule heap_lookup_SPLIT
        ...
        \\ asm_exists_tac \\ asm_rewrite_tac [])
      \\ full_simp_tac std_ss [heap_lookup_Unused_Bignum]
      \\ print_tac "AnyArith_thm: impl_tac (G)"
      THEN1
       (drule heap_lookup_SPLIT
        \\ strip_tac \\ rveq
        \\ qpat_x_assum `_ (fun2set _)` mp_tac
  ...
  \\ print_tac "AnyArith_thm: check point 2"
  \\ `∃new_c.
        state_rel c r1 r2
   ...
   (qmatch_asmsub_abbrev_tac `clock_write new_clock_val`
    ...
    >- (
      ...
      `t2.stack_max = s1'.stack_max` by fs [wordSemTheory.state_component_equality] >>
      ...
      ho_match_mp_tac backendPropsTheory.option_le_trans >>
      qexists_tac ` OPTION_MAP2 MAX
                 (OPTION_MAP2 MAX max
                    (OPTION_MAP2 $+ s1.locals_size (stack_size s1.stack)))
                 (OPTION_MAP2 $+
                    (OPTION_MAP2 $+ s1.locals_size (stack_size s1.stack))
                    (lookup Bignum_location s1.stack_size))` >>
      conj_tac
      >- (
        Cases_on `stack_size s1.stack` >> Cases_on ` s1.locals_size` >> Cases_on `max` >>
        Cases_on `lookup Bignum_location s1.stack_size` >>
        fs [MAX_DEF, OPTION_MAP2_DEF]) >>
      dxrule evaluate_stack_max >>
      fs [state_fn_updates, stack_size_eq2, wordSemTheory.stack_size_frame_def] >>
  ...
  \\ print_tac "AnyArith_thm: check point 3"
  \\ simp [wordSemTheory.get_store_def]
  ...
  \\ print_tac "AnyArith_thm: check point 8"
...
Theorem env_to_list_domain_eq:
  env_to_list y2 t.permute = (l',permute) ∧
  MAP FST l' = MAP FST ms ⇒
  domain (fromAList ms) = domain y2
Proof
  rw [] \\ drule env_to_list_lookup_equiv \\ rw []
  \\ fs [domain_fromAList]
  \\ qpat_x_assum ‘MAP _ _ = _’ $ assume_tac o GSYM
  \\ gvs [] \\ simp [EXTENSION,domain_lookup]
  \\ qpat_x_assum ‘∀n. ALOOKUP _ n = _’ $ assume_tac o GSYM
  \\ gvs [] \\ rewrite_tac [GSYM IS_SOME_EXISTS]
  \\ rewrite_tac [IS_SOME_ALOOKUP_EQ]
QED

Theorem size_of_LE_append:
  !vs ys lims refs seen.
    FST (size_of lims ys refs seen) <= FST (size_of lims (vs ++ ys) refs seen)
Proof
  Induct \\ rw []
  \\ once_rewrite_tac [data_to_word_gcProofTheory.size_of_cons]
  \\ Cases_on `size_of lims (vs ++ ys) refs seen`
  \\ rename1 `_ = (n1,rr1)` \\ PairCases_on `rr1`
  \\ Cases_on `size_of lims [h] rr10 rr11`
  \\ rename1 `size_of lims [h] _ _ = (n2,rr2)` \\ PairCases_on `rr2`
  \\ fs []
  \\ last_x_assum (qspecl_then [`ys`,`lims`,`refs`,`seen`] assume_tac) \\ gvs []
QED

Theorem size_of_heap_LE_args:
  !vs s. size_of_heap s <= size_of_heap_args vs s
Proof
  rw [dataSemTheory.size_of_heap_def, dataSemTheory.size_of_heap_args_def]
  \\ qspecl_then [`vs`,`stack_to_vs s`,`s.limits`,`s.refs`,`LN`] mp_tac size_of_LE_append
  \\ rpt (pairarg_tac \\ fs [])
QED
   ...
   !index r.
      ...
      ∃q r'.
        (λ(res,s1).
          ...
          (evaluate
            (MustTerminate
              (Call (SOME ([1],adjust_sets (get_names names_opt),Skip,n,l))
...
Proof
        ...
        <|locals :=
          ...
          stack_max :=
            OPTION_MAP2 MAX
              (OPTION_MAP2 MAX
                 (OPTION_MAP2 MAX t.stack_max
                    (stack_size (StackFrame t.locals_size (toAList y1) l' NONE::t.stack)))
                 (OPTION_MAP2 $+
                    (stack_size (StackFrame t.locals_size (toAList y1) l' NONE::t.stack))
                    (lookup (Arith_location index) t.stack_size)))
              (OPTION_MAP2 $+
                 (stack_size (StackFrame t.locals_size (toAList y1) l' NONE::t.stack))
                 (lookup AnyArith_location t.stack_size));
          permute := permute; clock := MustTerminate_limit (:α) - 2;
          termdep := t.termdep - 1|>`
     by (unabbrev_all_tac \\ fs [LET_DEF, wordSemTheory.call_env_def,
         wordSemTheory.push_env_def,wordSemTheory.dec_clock_def]  \\ pairarg_tac \\ fs [])
  \\ Cases_on `env_to_list y2 t.permute` \\ fs [LET_DEF]
  ...
  \\ `state_rel c n l
    ...
    \\ pairarg_tac \\ fs [] \\ rveq
    \\ rw [state_rel_def]
    >~ [`lookup 0 _ = SOME (Loc _ _)`] >- fs [Once state_rel_def, lookup_insert, fromList_def, fromList2_def]
    >~ [`IS_SOME (lookup (adjust_var _) _)`] >-
       fs [Once state_rel_def, call_env_def, lookup_insert, wordSemTheory.dec_clock_def, fromList2_def, fromList_def]
    >~ [`heap_in_memory_store _ _ _ _ _ _ _ _ _`] >-
       (fs [state_rel_def] \\ asm_exists_tac
        \\ fs [call_env_def, push_env_def, lookup_insert, fromList_def, push_env_termdep, fromList2_def, dec_clock_def]
        \\ metis_tac [])
    >~ [`option_le _ (OPTION_MAP2 MAX _ _)`] >-
       (fs [Once state_rel_def]
        \\ qpat_x_assum `option_le _ (OPTION_MAP2 _ _ _)` mp_tac
        \\ qmatch_goalsub_abbrev_tac `option_le _ stka` \\ strip_tac
        \\ qmatch_goalsub_abbrev_tac `option_le _ stkb`
        \\ `option_le stka stkb` by
          (unabbrev_all_tac \\ fs [stack_size_eq2, wordSemTheory.stack_size_frame_def]
           \\ Cases_on `t.stack_max` \\ Cases_on `s.locals_size` \\ Cases_on `stack_size t.stack`
           \\ Cases_on `lookup (Arith_location index) s.stack_frame_sizes`
           \\ Cases_on `lookup AnyArith_location s.stack_frame_sizes` \\ fs [MAX_DEF, OPTION_MAP2_DEF])
        \\ metis_tac [backendPropsTheory.option_le_trans])
    \\ fs [Once state_rel_def] \\ metis_tac [])
  ...
  \\ simp [Once call_env_def,wordSemTheory.dec_clock_def,do_app_def,
           ...
           call_env_def,wordSemTheory.call_env_def]
  ...
  \\ `domain t2.locals = domain y1 ∪ domain y2` by (
     qpat_x_assum ‘pop_env _ = SOME t2’ assume_tac
     \\ rename [‘pop_env r3 = SOME t2’]
     ...
     \\ qspecl_then [`AnyArith_code c`,`t with
           <|locals := lcnew;
             locals_size := lookup AnyArith_location t.stack_size;
             stack := StackFrame t.locals_size (toAList y1) q NONE::t.stack;
             stack_max := smnew; permute := r'';
             clock := MustTerminate_limit (:α) - 2; termdep := t.termdep - 1|>`] mp_tac
         (wordPropsTheory.evaluate_stack_swap
            |> INST_TYPE [``:'b``|->``:'c``,``:'c``|->``:'ffi``])
    \\ fs [] \\ fs [wordSemTheory.pop_env_def,wordSemTheory.dec_clock_def]
    \\ Cases_on `r3.stack` \\ fs [] \\ Cases_on `h` \\ fs []
    \\ rename1 `r2.stack = StackFrame lsz ns ms opt::t'`
    \\ unabbrev_all_tac
  ...
  \\ conj_tac THEN1
   (simp [lookup_insert]
    \\ rename [‘_ = (SOME (Result (Loc n l) [rv]),r3)’]
    \\ Cases_on `r3.stack` \\ fs [] \\ Cases_on `h` \\ fs []
...
Theorem EVERY_IMP_ALOOKUP:
  !xs P. EVERY P xs ==> !x y. ALOOKUP xs x = SOME y ==> P (x, y)
Proof
        ...
        (q = SOME NotEnoughSpace ⇒
         ...
         (c.gc_kind <> None ==>
            ...
            let jl = bignum_size s.limits.arch_64_bit i2 in
              il + jl <= 2 ** c.len_size ==>
              s.limits.heap_limit <
                2 * il + 2 * jl + size_of_heap_args [Number i1; Number i2] x)) ∧
        (q ≠ SOME NotEnoughSpace ⇒
         state_rel c l1 l2
           (x with
            <|locals := insert dest (Number r) x.locals; space := 0;
...
Proof
  ...
  THEN1
   (strip_tac
    >- (rpt strip_tac \\ gs []
        \\ qspecl_then [`[Number i1; Number i2]`,`x`] mp_tac size_of_heap_LE_args
        \\ fs [])
    \\ fs [state_rel_thm] \\ rfs [])
  \\ fs []
  ...
  \\ fs [GSYM wordSemTheory.stack_size_def,wordSemTheory.stack_size_frame_def]
  \\ fs [wordSemTheory.call_env_def]
  \\ `option_le t.stack_max s.stack_max /\
      option_le (OPTION_MAP2 $+ (stack_size t.stack) t.locals_size)
          t.stack_max /\
      t.locals_size = s.locals_size` by fs [state_rel_thm]
  \\ qpat_x_assum `evaluate (Call _ _ _ _,_) = _` kall_tac
  \\ fs [wordSemTheory.evaluate_def,list_Seq_def]
  \\ simp [CaseEq"option",CaseEq"wordSem$result",CaseEq"bool",pair_case_eq,
           CaseEq"list",CaseEq"stack_frame",wordSemTheory.pop_env_def]
  \\ rpt strip_tac \\ rveq \\ fs []
  \\ match_mp_tac backendPropsTheory.option_le_trans
  \\ asm_exists_tac \\ fs []
  \\ qpat_x_assum `option_le _ _` kall_tac
  \\ qpat_x_assum `option_le _ _` mp_tac
  \\ fs [wordSemTheory.stack_size_def,wordSemTheory.stack_size_frame_def]
  \\ fs [GSYM wordSemTheory.stack_size_def,wordSemTheory.stack_size_frame_def]
