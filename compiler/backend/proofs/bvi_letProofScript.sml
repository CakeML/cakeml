...
Theorem env_rel_REPLICATE_0[local]:
  !vs ax d env1 env2.
    env_rel ax d env1 env2 ==>
    env_rel (REPLICATE (LENGTH vs) 0 ++ ax) d (vs ++ env1) (vs ++ env2)
Proof
  Induct \\ fs [REPLICATE,env_rel_def,v_rel_def,LLOOKUP_def]
QED
...
Proof
  ...
  \\ fs [] \\ rveq
  >- suspend "Call"
  \\ Cases_on `?xs op. h = Op op xs` \\ fs []
  >- suspend "Op"
  \\ Cases_on ‘∃force_loc n. h = Force force_loc n’ \\ gvs []
  >- suspend "Force"
  \\ Cases_on `?es. h = Return es` \\ fs []
  >- suspend "Return"
  \\ Cases_on `?rets ticks dest args body. h = LetCall rets ticks dest args body`
  \\ fs [] \\ rveq
  >- suspend "LetCall"
  \\ Cases_on `?ys y. h = Let ys y` \\ fs []
  >- suspend "Let"
  \\ Cases_on `h` \\ fs []
QED

Resume evaluate_env_rel[CONS]:
...
Resume evaluate_env_rel[RaiseTick]:
...
Resume evaluate_env_rel[Return]:
  (rw [] \\ qpat_x_assum `!_ _ _ _ _. bb` mp_tac
   \\ once_rewrite_tac [IMP_COMM] \\ strip_tac
   \\ gvs [compile_def,evaluate_def]
   \\ gvs [evaluate_def,CaseEq"prod"]
...
Resume evaluate_env_rel[LetCall]:
  (qpat_x_assum `!_ _ _ _ _. bb` mp_tac
   \\ once_rewrite_tac [IMP_COMM] \\ strip_tac
   \\ gvs [compile_def]
   \\ first_assum $ qspec_then ‘args’ mp_tac \\ gvs []
   ...
   >- (first_x_assum $ irule \\ gvs []
       ...
       \\ irule env_rel_REPLICATE_0 \\ fs [])
   \\ gvs [])
QED

Resume evaluate_env_rel[Let]:
