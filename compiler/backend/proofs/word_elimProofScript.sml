...
val wf_findWordRef = Q.store_thm("wf_findWordRef",
    ...
    recInduct findWordRef_ind >>
    rw[findWordRef_def, wf_union, wf_def, wf_insert] >>
...
val lookup_analyseWordCode = Q.store_thm ("lookup_analyseWordCode",
    ...
    ⇒ lookup n (analyseWordCode code) = SOME (findWordRef prog)`,
    Induct >> fs[FORALL_PROD] >> fs[analyseWordCode_def] >>
    fs[lookup_insert] >> rw[]
);

val removeWordCode_thm = Q.store_thm("removeWordCode_thm",
    `∀ n reachable v l . n ∈ domain reachable ∧ MEM (n, v) l
    ⇒ MEM (n, v) (removeWordCode reachable l)`,
    Induct_on `l` >> rw[] >> fs[removeWordCode_def] >> fs[domain_lookup] >>
...
val analyseWordCode_reachable_thm = Q.store_thm("analyseWordCode_reachable_thm",
    ...
    analyseWordCode code = t ∧  start = insert n () (LN:num_set) ∧
        domain start ⊆ domain tree ∧ tree = mk_wf_set_tree t
    ⇒ domain (closure_spt start tree) =
        {a | ∃ n . isReachable tree n a ∧ n ∈ domain start}`,
    rw[] >> fs[domain_insert] >>
    qspecl_then
        [`mk_wf_set_tree (analyseWordCode code)`, `insert n () LN`]
        mp_tac closure_spt_thm >>
    `wf_set_tree(mk_wf_set_tree (analyseWordCode code))` by
...
val noInstall_find_code = Q.store_thm("noInstall_find_code",
    ...
    noInstall_code code ∧ find_code dest args code = SOME (args1, expr)
    ⇒ noInstall expr`,
    rw[noInstall_code_def] >> Cases_on `dest` >> fs[find_code_def] >>
...
val noInstall_evaluate_const_code = Q.store_thm("noInstall_evaluate_const_code",
    ...
    >- (fs[noInstall_def, dec_clock_def, call_env_def, push_env_def,
...
val getLocals_thm = Q.store_thm("getLocals_thm",
    ...
    >- (Cases_on `destWordLoc a` >> fs[] >> rw[domain_union] >>
        fs[lookup_def] >>
        Cases_on `n = 0` >> fs[] >> rveq >> fs[destWordLoc_def] >>
        Cases_on `a` >> fs[destWordLoc_def] >> Cases_on `EVEN n` >> fs[] >>
        metis_tac[])
);
...
val domain_getLocals_lookup = Q.store_thm ("domain_getLocals_lookup",
    ...
    >> Induct_on `t`
        ...
        >- (rw[getLocals_def, domain_def, lookup_def] >>
            Cases_on `a` >> fs[destWordLoc_def])
        >- (rw[getLocals_def, domain_def, lookup_def] >>
...
val getLocals_insert_Loc = Q.store_thm("getLocals_insert_Loc",
    `∀ k n1 n0 (locals : ('a word_loc) num_map).
        domain (getLocals (insert k (Loc n1 n0) locals)) ⊆
            domain (getLocals locals) ∪ {n1}`,
        rw[] >> fs[SUBSET_DEF] >>  rw[domain_getLocals_lookup] >>
        fs[lookup_insert] >> rw[] >>
...
val domain_getStore = Q.store_thm("domain_getStore",
    `∀ n store . n ∈ domain (getStore store) ⇔
...
val getStore_update = Q.store_thm("getStore_update",
    `∀ store k v . domain (getStore (store |+ (k,v))) ⊆ domain (getStore store)
        ∪ (case destWordLoc v of | NONE => {} | SOME n => {n})`,
    Cases_on `v` >> fs[destWordLoc_def] >> rw[] >> fs[SUBSET_DEF] >>
    rw[domain_getStore] >>
...
val get_NumWordLoc_alist_thm = Q.store_thm ("get_NumWordLoc_alist_thm",
    `∀ n e l . (∃ n0 . MEM (Loc n n0) (MAP SND l)) ⇔
        n ∈ domain (get_NumWordLoc_alist l)`,
    fs[get_NumWordLoc_alist_def] >> fs[MEM_MAP, MEM_FILTER] >>
    rw[] >> EQ_TAC >> rw[]
...
val get_NumWordLoc_alist_getLocals = Q.store_thm (
    "get_NumWordLoc_alist_getlocals",
    `∀ e . domain (getLocals (fromAList e)) ⊆ domain (get_NumWordLoc_alist e)`,
    Induct >> rw[] >>
    fs[fromAList_def, get_NumWordLoc_alist_def, domain_def, getLocals_def] >>
    Cases_on `h` >> Cases_on `r` >> fs[destWordLoc_def, fromAList_def]
    >- (qspecl_then [`q`, `Word c`, `(fromAList e)`] mp_tac getLocals_insert >>
        rw[destWordLoc_def] >> imp_res_tac SUBSET_TRANS)
    >- (qspecl_then [`q`, `Loc n n0`, `(fromAList e)`]
            mp_tac getLocals_insert >> rw[destWordLoc_def] >>
...
val getStack_def = Define ` (* stack : ('a stack_frame) list *)
    (getStack [] = LN:num_set) ∧
    (getStack ((StackFrame e _)::xs) =
        union (get_NumWordLoc_alist e) (getStack xs))
`

val getStack_ind = theorem "getStack_ind";

val getStack_hd_thm = Q.store_thm ("getStack_hd_thm",
    `∀ stack reachable l opt t . domain (getStack stack) ⊆ domain reachable ∧
        stack = StackFrame l opt::t
        ⇒ domain (getLocals (fromAList l)) ⊆ domain reachable ∧
          domain (getStack t) ⊆ domain (reachable)`,
            recInduct getStack_ind >> rw[]
            >- (Cases_on `e` >>
                fs[getStack_def, domain_union, fromAList_def, getLocals_def] >>
...
val getStack_CONS = Q.store_thm("getStack_CONS",
    `∀ h t . domain (getStack [h]) ⊆ domain (getStack (h::t)) ∧
        domain (getStack t) ⊆ domain (getStack (h::t))`,
    Cases_on `h` >> fs[getStack_def, domain_union]
);

val getStack_enc_stack = Q.store_thm("getStack_enc_stack",
    `∀ stack reachable . domain (getStack stack) ⊆ domain reachable
...
val getStack_dec_stack = Q.store_thm("getStack_dec_stack",
    ...
    domain (getStack stack) ⊆ domain reachable ∧
    ...
    >- (`∀ n n0 . MEM (Loc n n0) (DROP (LENGTH l) locs)
        ...
        fs[getStack_def, domain_union] >> rw[]
        >- (fs[SUBSET_DEF, GSYM get_NumWordLoc_alist_thm] >> rw[] >>
            fs[MEM_MAP] >>
...
val s_val_eq_getStack = Q.store_thm("s_val_eq_getStack",
    ...
    ⇒ getStack stack1 = getStack stack2`,
        recInduct getStack_ind >> rw[] >> Cases_on `stack2` >>
        fs[s_val_eq_def] >>
        Cases_on `h` >> fs[s_frame_val_eq_def, getStack_def] >>
        first_x_assum drule >> rw[] >> Cases_on `v0` >> Cases_on `o'` >>
        fs[s_frame_val_eq_def] >> rw[] >>
        `MAP (destWordLoc o SND) e = MAP (destWordLoc o SND) l` by
...
val domain_getMemory = Q.store_thm ("domain_getMemory",
    `∀ mem (mdom : 'a word set) n . (n ∈ domain (getMemory mem mdom)
...
val getMemory_update = Q.store_thm("getMemory_update",
    `∀ k v (memory : 'a word -> 'a word_loc) (mdomain : 'a word set) .
        (domain (getMemory ((k =+ v) memory) mdomain)) ⊆
            (domain (getMemory memory mdomain))
        ∪ (case (destWordLoc v) of | NONE => {} | SOME n => {n})`,
        Cases_on `v` >> fs[destWordLoc_def] >> rw[] >> fs[SUBSET_DEF] >>
        rw[domain_getMemory] >>
...
val codeClosed_def = Define`
    codeClosed reachable c1 ⇔ ∃ code1 . c1 = fromAList code1 ∧
        ...
        isReachable (mk_wf_set_tree (analyseWordCode code1)) n m ⇒
        m ∈ domain reachable
`
...
val gcNoNewLocs_def = Define`
    gcNoNewLocs (g:'a gc_fun_type) ⇔  ∀ sta mem mdom sto wl mem1 sto1 sta1 .
        ...
        domain (getStack sta1) ⊆ domain (getStack sta) ∧
        domain (getMemory mem1 mdom) ⊆ domain (getMemory mem mdom) ∧
...
val getLocals_fromList2_extension = Q.store_thm("getLocals_fromList2_extension",
    `∀  x y ys. x ∈ domain (getLocals (fromList2 ys))
    ⇒ x ∈ domain (getLocals (fromList2 (ys ⧺ [y])))`,
        rw[] >> fs[domain_getLocals_lookup] >> qexists_tac `k` >>
        qexists_tac `n1` >>
...
val getLocals_fromList2_FRONT = Q.store_thm("getLocals_fromList2_FRONT",
    ...
    qexists_tac `domain (getLocals (fromList2 x))` >>
    reverse(CONJ_TAC) >- metis_tac[getLocals_fromList2]
        >- (`∃ y ys . x = SNOC y ys` by metis_tac[SNOC_CASES] >>
            ...
            imp_res_tac getLocals_fromList2_extension >> fs[])
);

val getMemory_write_bytearray_lemma = Q.store_thm(
    "getMemory_write_bytearray_lemma",
    `∀ mem mdom reachable c r be . domain(getMemory mem mdom) ⊆ domain reachable
    ⇒ domain (getMemory (write_bytearray c r mem mdom be) mdom) ⊆
        domain reachable`,
    ...
    qspecl_then [`byte_align c`, `Word (set_byte c h c' be)`,
        ...
        mp_tac getMemory_update >> rw[destWordLoc_def] >>
        metis_tac[SUBSET_TRANS]
);

val stack_list_rearrange_lemma = Q.store_thm("stack_list_rearrange_lemma",
    `∀ s reachable locs opt .
        domain (getLocals s.locals) ⊆ domain reachable ∧
        domain (getStack s.stack) ⊆ domain reachable
    ⇒ domain (getStack (StackFrame (list_rearrange (s.permute 0)
    (QSORT key_val_compare (toAList (inter s.locals locs)))) opt::s.stack))
        ⊆ domain reachable`,
    rw[] >> fs[getStack_def, domain_union] >> rw[SUBSET_DEF] >>
    imp_res_tac get_NumWordLoc_alist_thm >>
...
val removeWordCode_thm_FST = Q.store_thm("removeWordCode_thm_FST",
    ...
    MEM n (MAP FST (removeWordCode reachable l)))`,
    rw[] >> EQ_TAC >> rw[]
    >- (Induct_on `l` >> rw[] >> fs[removeWordCode_def] >> fs[domain_lookup] >>
    Cases_on `IS_SOME (lookup (FST h) reachable)` >> fs[])
    >> fs[removeWordCode_def]
...
val word_state_rel_inst_SOME = Q.store_thm ("word_state_rel_inst_SOME",
    ...
    >- (fs[assign_def] >> fs[word_exp_def] >> fs[set_var_def] >>
        fs[word_state_rel_def] >> rw[] >> fs[domain_findLocState] >>
        qspecl_then [`n`, `Word c`, `s.locals`] mp_tac getLocals_insert >>
        rw[destWordLoc_def] >> imp_res_tac SUBSET_TRANS)
    >- (strip_tac >> strip_tac >> strip_tac >> strip_tac >> strip_tac >>
        strip_tac >>
        ...
        >- (fs[assign_def] >>
            ...
            fs[word_state_rel_def, domain_findLocState] >> rw[] >>
            fs[word_exp_def] >> fs[the_words_def] >> EVERY_CASE_TAC >> fs[] >>
           qspecl_then [`n`, `Word z'`, `s.locals`] mp_tac getLocals_insert >>
        ...
        >- (EVERY_CASE_TAC >> fs[set_var_def] >>
            ...
            >| [
                qspecl_then [`n2`, `Word 1w`,
                    ...
                    mp_tac getLocals_insert  ,
                qspecl_then [`n2`, `Word 0w`,
                `insert n (Word (c + c')) s.locals`] mp_tac getLocals_insert
              ] >>
                rw[destWordLoc_def] >> rw[] >> imp_res_tac SUBSET_TRANS)
        >- (EVERY_CASE_TAC >> fs[set_var_def] >>
            fs[word_state_rel_def, domain_findLocState] >> rw[] >> fs[] >>
    ...
    >- (strip_tac >> strip_tac >> strip_tac >> strip_tac >> strip_tac >>
        ...
        >- (fs[word_op_def, mem_load_def] >> Cases_on `c + c' ∈ s.mdomain` >>
            ...
            qspecl_then [`n`, `s.memory (c + c')`, `s.locals`]
                mp_tac getLocals_insert >> rw[] >>
            Cases_on `s.memory (c + c')` >> fs[destWordLoc_def] >> rw[]
            >- imp_res_tac SUBSET_TRANS >>
            `n'' ∈ domain reachable` by
                metis_tac[SUBSET_DEF, domain_getMemory] >>
                fs[SUBSET_DEF] >> metis_tac[])
        >- (fs[word_op_def, mem_load_byte_aux_def] >>
            Cases_on `s.memory (byte_align (c + c'))` >> fs[] >>
            Cases_on `byte_align (c + c') ∈ s.mdomain` >> fs[] >>
            fs[set_var_def] >> rw[] >> fs[domain_findLocState] >>
        ...
        >- (fs[word_op_def, get_var_def] >>
            ...
            qspecl_then [`c + c'`, `x`, `s.memory`, `s.mdomain`]
                mp_tac getMemory_update >> rw[] >>
            Cases_on `x` >> fs[destWordLoc_def] >> rw[]
            >- imp_res_tac SUBSET_TRANS >>
            `n'' ∈ domain reachable` by
                metis_tac[SUBSET_DEF, domain_getLocals_lookup] >>
            fs[SUBSET_DEF] >> metis_tac[])
        >- (fs[word_op_def, get_var_def] >>
            ...
            Cases_on `s.memory (byte_align (c + c'))` >> fs[] >>
            Cases_on `byte_align (c + c') ∈ s.mdomain` >> fs[] >>
            rw[] >> fs[domain_findLocState] >>
            qspecl_then [`byte_align (c + c')`,
                `Word(set_byte (c + c') (w2w c'') c''' s.be)`,
                `s.memory`, `s.mdomain`] mp_tac getMemory_update >>
            rw[] >> fs[destWordLoc_def] >> rw[] >> imp_res_tac SUBSET_TRANS))
    >- (strip_tac >> strip_tac >> strip_tac >> strip_tac >> strip_tac >>
        strip_tac >>
        fs[get_var_def, get_fp_var_def, set_fp_var_def, set_var_def] >>
        `t.fp_regs = s.fp_regs` by fs[word_state_rel_def] >>
        EVERY_CASE_TAC >> fs[] >> fs[word_state_rel_def, domain_findLocState] >>
        rw[] >> fs[] >>
        `∀ w . domain (getLocals (insert n (Word w) s.locals)) ⊆
            domain reachable` by
            (rw[] >>
             qspecl_then [`n`, `Word w`, `s.locals`] mp_tac getLocals_insert >>
             fs[destWordLoc_def] >> rw[] >> imp_res_tac SUBSET_TRANS) >> fs[] >>
        rfs[] >>
        qspecl_then [`n`, `Word ((31 >< 0) x)`, `s.locals`]
            mp_tac getLocals_insert >>
        qspecl_then [`n0`, `Word ((63 >< 32) x)`,
            `insert n (Word ((31 >< 0) x)) s.locals`] mp_tac getLocals_insert >>
        rw[destWordLoc_def] >> imp_res_tac SUBSET_TRANS)
);
...
val word_state_rel_jump_exc = Q.store_thm("word_state_rel_jump_exc",
    ...
    fs[domain_findLocState] >>
    `domain (getStack (StackFrame l (SOME (q,l1,l2))::t')) ⊆
        domain reachable` by metis_tac[getStack_LASTN] >>
...
val word_state_rel_alloc = Q.store_thm("word_state_rel_alloc",
    ...
    res ≠ SOME Error ∧ gcNoNewLocs s.gc_fun
    ⇒ alloc c n s = (res, s1) ⇒
    ...
    `word_state_rel reachable (push_env (inter s.locals n) NONE
        ...
        (set_store AllocSize (Word c) t))` by (
            simp[push_env_def, env_to_list_def, set_store_def,
                 word_state_rel_def, domain_findLocState] >> rw[]
            >- (qspecl_then [`s.store`, `AllocSize`, `Word c`] mp_tac
                getStore_update >> fs[destWordLoc_def] >>
                rw[] >> imp_res_tac SUBSET_TRANS)
    ...
    >>  rw[] >> fs[] >>
        ...
        rfs[] >>
        strip_tac >> rveq >> fs[destResultLoc_def, word_state_rel_def] >>
        rw[] >>
        fs[domain_findLocState]
        >- (drule getStack_hd_thm >> rw[] >> metis_tac[])
        >- (fs[getLocals_def, getStack_def])
        >- (drule getStack_hd_thm >> rw[] >> metis_tac[])
        >- (fs[getLocals_def, getStack_def])
);
...
val word_removal_lemma = Q.store_thm ("word_removal_lemma",
    `∀ program state result new_state reachable removed_state .
        ...
        noInstall program ∧ noInstall_code state.code ∧
        noInstall_code removed_state.code ∧
    ...
    (* CALL - DONE !!! *)
        ...
        `find_code dest (add_ret_loc ret x) s.code =
            find_code dest (add_ret_loc ret x) removed_state.code` by (
                ...
                >> `x' ∈ domain reachable` by
                        fs[findWordRef_def, SUBSET_DEF, domain_union] >>
        ...
        >- (Cases_on `handler` >> fs[] >> Cases_on `s.clock = 0` >> fs[]
            >- (fs[call_env_def, fromList2_def] >> rw[word_state_rel_def] >>
                rw[findLocState_def, domain_union,
                    getLocals_def, getStack_def] >>
            ...
            >- (`word_state_rel reachable (call_env x'0 (dec_clock s))
                (call_env x'0 (dec_clock removed_state))` by (
                    ...
                    fs[findLocState_def, domain_union, domain_findLocState] >>
                    fs[add_ret_loc_def] >>
                    ...
                    >- (Cases_on `LAST (h::t)` >> fs[] >>
                        ...
                        qspecl_then [`args`, `s`, `(h::t)`, `n`, `0`] mp_tac
                            get_vars_getLocals >> strip_tac >>
                        `n ∈ domain reachable` by (qspec_then `s` mp_tac
                            domain_findLocState >> rw[] >>
                    ...
                    >>  fs[findWordRef_def, domain_insert] >>
                        ...
                        Cases_on `x` >> fs[]
                        >- (rw[SUBSET_DEF] >>
                            fs[SUBSET_DEF, fromList2_def, getLocals_def])
                        >- (qspecl_then [`args`, `s`, `(h::t)`,
                            `fromList2 (h::t)`] mp_tac getLocals_fromList2 >>
                            rw[] >> metis_tac[SUBSET_TRANS])) >>
                ...
                fs[] >> rw[] >> fs[])
                    >- (rw[dec_clock_def, call_env_def] >>
                        ...
                        >- (EVERY_CASE_TAC >> fs[] >> rveq >>
                            fs[] >> fs[noInstall_code_def] >> res_tac)
                        >- (EVERY_CASE_TAC >> fs[] >> rveq >> fs[] >>
                            fs[noInstall_code_def] >> res_tac)
                        >- (`∃ y ys . x = SNOC y ys` by
                            ...
                            `n ∈ domain reachable` by (
                                qspecl_then [`args`, `s`, `SNOC (Loc n 0) x'0`,
                                    `n`, `0`] mp_tac get_vars_getLocals >>
                                strip_tac >> fs[MEM_SNOC] >> rfs[] >>
                                qspec_then `s` mp_tac domain_findLocState >>
                                rw[] >> fs[SUBSET_UNION, SUBSET_DEF]) >>
                            fs[codeClosed_def] >> simp[SUBSET_DEF] >> rw[] >>
                            last_x_assum drule >> disch_then match_mp_tac >>
                            fs[isReachable_def] >> match_mp_tac RTC_SINGLE >>
                            fs[isAdjacent_def] >>
                            `∃ aSetx . lookup n (analyseWordCode code1) =
                                SOME aSetx ∧ x ∈ domain aSetx` by (
                                rfs[lookup_fromAList] >>
                                drule lookup_analyseWordCode >> fs[]) >>
                            drule lookup_mk_wf_set_tree >> rw[] >>
                            asm_exists_tac >> fs[] >> fs[domain_lookup] >>
                            `wf_set_tree (mk_wf_set_tree
                                (analyseWordCode code1))`
        ...
        >>  PairCases_on `x'` >> fs[] >> Cases_on `domain x'1' = {}` >> fs[] >>
            ...
            >- (fs[call_env_def, fromList2_def] >> rw[word_state_rel_def] >>
                ...
                fs[domain_findLocState, SUBSET_DEF, destResultLoc_def])
            >>  fs[add_ret_loc_def] >>
                fs[findWordRef_def, domain_findLocState, domain_union] >>
                `domain (findWordRef x'1) ⊆ domain reachable` by (
                    Cases_on `dest` >> fs[find_code_def, codeClosed_def]
                    >- (Cases_on `LAST (Loc x'3 x'4 :: x)`>> fs[] >>
                        Cases_on `lookup n s.code` >> fs[] >>
                        Cases_on `n0` >> fs[] >> Cases_on `x'` >> fs[] >>
                        rveq >> Cases_on `x` >> rfs[bad_dest_args_def] >> rw[] >>
                        `n ∈ domain reachable` by (
                            `MEM (Loc n 0) (h::t)` by
                                metis_tac[MEM, LAST_DEF, MEM_LAST] >>
                            qspecl_then [`args`, `s`, `h::t`, `n`, `0`] mp_tac
                                get_vars_getLocals >> rw[] >> fs[SUBSET_DEF]) >>
                        rw[SUBSET_DEF] >>
                        qpat_x_assum `∀ n m . n ∈ _ ∧ _ ⇒ _`
                            (qspecl_then [`n`, `x`] mp_tac) >>
                        reverse(impl_tac) >> fs[] >> fs[isReachable_def] >>
                        match_mp_tac RTC_SINGLE >> fs[isAdjacent_def] >>
                        `wf_set_tree(mk_wf_set_tree (analyseWordCode code1))` by
                            metis_tac[mk_wf_set_tree_thm] >>
                        fs[wf_set_tree_def] >>
                        qexists_tac `findWordRef r` >>
                        fs[lookup_analyseWordCode] >>
                        `lookup n (analyseWordCode code1) =
                            SOME (findWordRef r)` by
                            fs[lookup_fromAList, lookup_analyseWordCode] >>
                        imp_res_tac lookup_mk_wf_set_tree >>
                        `wf (findWordRef r) ∧ wf y` by
                            metis_tac[wf_findWordRef] >>
                        `y = findWordRef r` by (
                            fs[spt_eq_thm, domain_lookup, EXTENSION] >> rw[] >>
                            Cases_on `lookup n' y` >> fs[] >>
                            Cases_on `lookup n' (findWordRef r)` >>
                            fs[] >> rfs[] >>
                    ...
                    >- (Cases_on `lookup x' s.code` >> fs[] >> Cases_on `x''` >>
                        ...
                        `wf_set_tree(mk_wf_set_tree (analyseWordCode code1))` by
                            metis_tac[mk_wf_set_tree_thm] >>
                        fs[wf_set_tree_def] >> qexists_tac `findWordRef r` >>
                        fs[lookup_analyseWordCode] >>
                        `lookup x' (analyseWordCode code1) =
                            SOME (findWordRef r)` by
                            (match_mp_tac lookup_analyseWordCode >>
                        ...
                        `y = findWordRef r` by
                            (fs[spt_eq_thm, domain_lookup, EXTENSION] >> rw[] >>
                                Cases_on `lookup n y` >> fs[] >>
                                Cases_on `lookup n (findWordRef r)` >> fs[] >>
                                rfs[] >>
                                qpat_x_assum `∀ x . lookup _ _ = _ ⇔ _`
                                    (qspec_then `n` mp_tac) >> rw[] >> fs[]) >>
                            rveq >>
                        fs[domain_lookup, SUBSET_DEF] >> metis_tac[]) ) >>
                `codeClosed reachable (call_env x'0 (push_env
                ...
                `word_state_rel reachable (call_env x'0 (push_env
                    ...
                    (dec_clock removed_state)))` by (
                        `∀ e . MEM e x'0 ⇒ (case destWordLoc e of | NONE => {}
                            | SOME n => {n}) ⊆ domain reachable` by (
                        rw[] >> Cases_on `e` >>
                        fs[destWordLoc_def, SUBSET_EMPTY] >> Cases_on `dest` >>
                        fs[find_code_def] >>
                        EVERY_CASE_TAC >> fs[] >> rveq >>
                      ...
                      >- (fs[push_env_def, env_to_list_def] >>
                          fs[word_state_rel_def, domain_findLocState] >> rw[]
                          >- (rw[SUBSET_DEF] >> fs[domain_getLocals_lookup] >>
                ...
                `gcNoNewLocs (call_env x'0 (push_env (inter s.locals x'1')
                ...
                >- (Cases_on `w ≠ Loc x'3 x'4` >> fs[] >> fs[pop_env_def] >>
                    ...
                    >- (Cases_on `domain (fromAList l) =
                        ...
                        rw[] >> fs[] >> `noInstall x'1` by
                            metis_tac[noInstall_find_code] >> fs[] >>
                        ...
                        `r.code = s.code` by (fs[call_env_def, dec_clock_def] >>
                            Cases_on `handler`
                            >- (fs[push_env_def, env_to_list_def] >>
                                imp_res_tac noInstall_evaluate_const_code >>
                                fs[])
                            >- (PairCases_on `x'` >> fs[] >>
                                fs[push_env_def, env_to_list_def] >>
                                imp_res_tac noInstall_evaluate_const_code >>
                                fs[])) >>
                        fs[] >> fs[word_state_rel_def, domain_findLocState] >>
                        rw[]
                        >- (`domain (getLocals (fromAList l)) ⊆
                                domain reachable` by
                                imp_res_tac getStack_hd_thm >>
                            qspecl_then [`x'0'`, `w0`, `fromAList l`] mp_tac
                                getLocals_insert >> rw[] >>
                            Cases_on `w0` >> fs[destWordLoc_def] >> rw[]
                            >- imp_res_tac SUBSET_TRANS >>
                            fs[destResultLoc_def] >> fs[SUBSET_DEF] >>
                            metis_tac[])
                ...
                >- (Cases_on `handler` >> fs[] >>
                    `noInstall x'1` by metis_tac[noInstall_find_code] >> fs[]
                ...
                >>  first_x_assum (qspecl_then [`reachable`,
                    ...
                    rw[] >> rw[] >> rfs[dec_clock_def] >> `noInstall x'1` by
                        metis_tac[noInstall_find_code] >> fs[]
,   (* FFI - DONE!!! *)
...
,   (* Install - DONE!!! *)
        fs[noInstall_def]
,   (* LocValue - DONE!!! *)
        simp[wordSemTheory.evaluate_def] >> fs[findWordRef_def] >>
        ...
        fs[domain_findLocState] >>
        `domain (getLocals (insert r (Loc l1 0) s.locals)) ⊆
            domain (getLocals s.locals) ∪ {l1}`
        by (metis_tac[getLocals_insert_Loc]) >> fs[SUBSET_DEF] >> rw[] >>
        res_tac >> fs[]
...
,   (* Raise - DONE!!! *)
        ...
        >> Cases_on `x` >> fs[destResultLoc_def] >> fs[SUBSET_DEF] >>
            qspecl_then [`n'`, `s.locals`] mp_tac domain_getLocals_lookup >>
            rw[] >>
...
,   (* Return - DONE!!! *)
        ...
        fs[call_env_def, fromList2_def, word_state_rel_def,
            domain_findLocState, getLocals_def]
        >> Cases_on `x` >> fs[destResultLoc_def] >> fs[SUBSET_DEF] >>
            qspecl_then [`n''`, `s.locals`] mp_tac domain_getLocals_lookup >>
            rw[] >>
            fs[domain_findLocState] >> res_tac >> fs[]
,   (* Seq - DONE!!! *)
        ...
        strip_tac >> rveq >> fs[noInstall_def, findWordRef_def] >> rw[] >>
        fs[] >> rfs[] >>
        ...
        r.code = new_state.code ∧ r.gc_fun = s.gc_fun` by
            metis_tac[noInstall_evaluate_const_code, evaluate_consts] >>
        rveq >> fs[]
,   (* MustTerminate - DONE!!! *)
        ...
        first_x_assum (qspecl_then [`reachable`,
            `removed_state with <|clock := MustTerminate_limit (:'a);
                termdep := n|>`] mp_tac) >>
        impl_tac >> rw[] >>
        fs[noInstall_def, word_state_rel_def, findWordRef_def,
            domain_findLocState]
,   (* Tick - DONE!!! *)
        simp[wordSemTheory.evaluate_def] >>
        Cases_on `s.clock = 0` >> fs[]
        >- (fs[call_env_def, fromList2_def] >> rw[word_state_rel_def] >>
            ...
            fs[domain_findLocState, SUBSET_DEF, destResultLoc_def])
        >- (rw[] >> fs[dec_clock_def, word_state_rel_def, domain_findLocState,
            destResultLoc_def])
,   (* Store - DONE!!! *)
        simp[wordSemTheory.evaluate_def] >>
        ...
        fs[word_state_rel_def, domain_findLocState, destResultLoc_def] >>
        qspecl_then [`c`, `x`, `s.memory`, `s.mdomain`] mp_tac
            getMemory_update >> fs[] >>
        Cases_on `x` >> fs[destWordLoc_def] >> rw[]
        >- metis_tac[SUBSET_TRANS] >>
        `n ∈ domain reachable` by (imp_res_tac domain_getLocals_lookup >>
            fs[SUBSET_DEF]) >>
        fs[SUBSET_DEF] >> metis_tac[]
,   (* Set - DONE!!! *)
        simp[wordSemTheory.evaluate_def] >>
        ...
        `n ∈ domain reachable` by (Cases_on `exp` >> fs[word_exp_def]
            ...
            >- (`MAP (λa. word_exp s a) l =
                    ...
                    `word_state_rel reachable s removed_state` by
                        fs[word_state_rel_def, domain_findLocState] >>
                    metis_tac[word_state_rel_word_exp]) >> fs[] >>
...
,   (* Get - DONE!!! *)
        ...
        Cases_on `x` >> fs[destWordLoc_def] >- metis_tac[SUBSET_TRANS] >>
        rw[] >>
...
,   (* Assign - DONE!!! *)
        ...
        fs[set_var_def] >> rw[] >> fs[word_state_rel_def, domain_findLocState,
            destResultLoc_def] >>
        qspecl_then [`v`, `x`, `s.locals`] mp_tac getLocals_insert >>
        Cases_on `x` >> fs[destWordLoc_def] >- metis_tac[SUBSET_TRANS] >>
        rw[] >>
...
,   (* Skip - DONE!!! *)
        ...
        fs[word_state_rel_def, destResultLoc_def]
    ]
);
...
val word_removal_thm = Q.store_thm("word_removal_thm",
    `∀ start state result new_state r reachable code1.
        ...
        domain (findLocState state) ⊆ domain reachable ∧
        gcNoNewLocs state.gc_fun ∧
        reachable = closure_spt (insert start () LN)
                        (mk_wf_set_tree (analyseWordCode code1)) ∧
        ALL_DISTINCT (MAP FST code1) ∧ noInstall_code state.code
        ⇒ ∃ s .
            wordSem$evaluate (Call NONE (SOME start) [0] NONE,
                state with code := fromAList (removeWordCode reachable code1)) =
                    (result, s)`,
    rpt strip_tac >>
    ...
    >>  qspecl_then [`code1`, `analyseWordCode code1`,
                     `insert start () LN`, `start`,
            `mk_wf_set_tree (analyseWordCode code1)`]
            mp_tac analyseWordCode_reachable_thm >>
        reverse(impl_tac >> rw[])
        >- (fs[codeClosed_def] >> qexists_tac `code1` >>
            fs[] >> fs[isReachable_def] >>
            metis_tac[RTC_TRANSITIVE, transitive_def])
        >- (fs[findWordRef_def, isReachable_def, RTC_REFL])
        >- (fs[noInstall_code_def] >> rw[] >> first_x_assum match_mp_tac >>
            qexists_tac `k` >>
            qexists_tac `n` >> fs[] >> fs[lookup_fromAList] >>
            imp_res_tac ALOOKUP_MEM >>
            imp_res_tac removeWordCode_thm >>
        ...
        >- (fs[word_state_rel_def, code_rel_def, lookup_fromAList] >>
            ...
            Cases_on `ALOOKUP code1 n` >> fs[]
                >- (fs[ALOOKUP_NONE] >> drule removeWordCode_thm_FST >> rw[] >>
                    ...
                    rw[])
                >- (imp_res_tac ALOOKUP_MEM >> drule removeWordCode_thm >>
                    rw[] >>
                    pop_assum (qspecl_then [`n`, `closure_spt x y`] mp_tac) >>
                    rw[] >> res_tac >>
                    `ALL_DISTINCT (MAP FST
                        (removeWordCode (closure_spt x y) code1))` by (
                        fs[removeWordCode_def] >>
        ...
        >- (fs[evaluate_def, get_vars_def, bad_dest_args_def,
            ...
            fs[lookup_fromAList] >> drule lookup_analyseWordCode >> rw[] >>
            fs[domain_lookup] >>
