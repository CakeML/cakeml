...
Proof
  cheat (* TODO: restructure do_app case analysis for new Src wrapper *)
QED
(*
  rw []
  ...
  >- (
    qpat_assum `this_case (Src Explode)` kall_tac
  ...
  >- (
    qpat_assum `this_case (Src Vsub)` kall_tac
    \\ simp [find_v_globalsL_EL_trans]
    \\ fs [EVERY_EL]
  )
  >- (
    qpat_assum `this_case (Src Vsub_unsafe)` kall_tac
    \\ simp [find_v_globalsL_EL_trans]
    \\ fs [EVERY_EL]
  )
  >- metis_tac[find_v_globalsL_REPLICATE,SUBSET_TRANS]
  >- (
    qpat_assum `this_case (Src Asub)` kall_tac
    \\ fs [semanticPrimitivesTheory.store_lookup_def]
  ...
  >- (
    qpat_assum `this_case (Src Aupdate)` kall_tac
    \\ fs [semanticPrimitivesTheory.store_lookup_def]
  ...
  >- (
    qpat_assum `this_case (Src Asub_unsafe)` kall_tac
    \\ fs [semanticPrimitivesTheory.store_lookup_def]
    \\ DEP_REWRITE_TAC [find_v_globalsL_EL_trans, not_v_has_Eval_EVERY_EL]
    \\ imp_res_tac find_refs_globals_EL
    \\ imp_res_tac EVERY_EL_IMP
  ...
  >- (
    qpat_assum `this_case (Src Aupdate_unsafe)` kall_tac
    \\ fs [semanticPrimitivesTheory.store_lookup_def]
    \\ DEP_REWRITE_TAC (listTheory.IMP_EVERY_LUPDATE
        :: RES_CANON find_refs_globals_LUPDATE)
    \\ simp []
  ...
  >- (
    qpat_assum `this_case (Src ListAppend)` kall_tac
    \\ imp_res_tac v_has_Eval_v_to_list
