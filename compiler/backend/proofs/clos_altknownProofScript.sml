...
val v_rel_app_def = Define `
  ...
  (v_rel_app c g (Closure loc_opt pargs1 env1 num_args e1) v args1 <=>
	 ...
	 | SOME args1' => LIST_REL val_approx_val aargs args1' /\
       ...
       v = Closure loc_opt pargs2 env2 num_args e2) /\
...
val v_rel_app_NONE = Q.store_thm(
  ...
  Cases_on `v1` \\ simp [v_rel_app_def]);
...
(*
val v_rel_dest_closure_SOME_Full = Q.store_thm(
  "v_rel_dest_closure_SOME_Full",
  `dest_closure max_app lopt1 f1 vs1 = SOME (Full_app e1 env1 args1) /\
   v_rel c g f1 f2 /\
   LIST_REL (v_rel c g) vs1 vs2 /\ loptrel f2 n lopt1 lopt2 /\ LENGTH vs2 = n ==>
   ?aenv e2 env2 args2.
      dest_closure max_app lopt2 f2 vs2 = SOME (Full_app e2 env2 args2) ∧
      LIST_REL val_approx_val aenv env2 ∧
      LIST_REL (v_rel c g) env1 env2 ∧ LIST_REL (v_rel c g) args1 args2 ∧
      exp_rel c aenv g e1 e2`,
...
val known_correct0 = Q.prove(
  `(!xs env1 (s0:(val_approx num_map#'c,'ffi) closSem$state) res1 s env2 t0 c g0 g g' aenv eas.
      ...
      ?res2 t.
        ...
        state_rel c g' s t) /\
   (!lopt1 f1 args1 (s0:(val_approx num_map#'c,'ffi) closSem$state) res1 s lopt2 f2 args2 t0 c g g' argsopt.
      evaluate_app lopt1 f1 args1 s0 = (res1, s) /\
      ssgc_free s0 /\ vsgc_free f1 /\ EVERY vsgc_free args1 /\
      oracle_state_sgc_free s0.compile_oracle /\
      co_every_Fn_vs_NONE s0.compile_oracle /\
      oracle_states_subspt s0.compile_oracle /\
      subspt g (next_g s0) /\ subspt (next_g s) g' /\
  ...
  THEN1
   (say "evaluate_app CONS"
    ...
    THEN1 ((* dest_closure returns Full_app *)    
       ...
       (drule dest_closure_SOME_IMP \\ strip_tac \\ rveq
        ...
        THEN1
         (IF_CASES_TAC \\ fs [] \\ rveq
          ...
          \\ `fv_max (num_args + LENGTH env2) [e]` by cheat
          ...
          \\ rename1 `evaluate_app _ f1 _`
          \\ qmatch_asmsub_abbrev_tac `evaluate_app _ _ args`
      ...
      \\ fs [DROP_NIL]
      \\ fs [DROP_LENGTH_TOO_LONG, LENGTH_REVERSE]

      \\ rfs [DROP_LENGTH_TOO_LONG, LENGTH_REVERSE]
      \\ fs [DROP_NIL]
      ...
      \\ disch_then drule
      \\ fs [PULL_EXISTS]
      \\ drule v_rel_upd_inline_factor
      \\ disch_then (qspec_then `k` assume_tac)
      \\ disch_then drule \\ simp []
      \\ fs [v_rel_upd_inline_factor]
      ...
      \\ fs [PULL_EXISTS]
        ...
        \\ fs [loptrel_def, check_loc_def] \\ EVERY_CASE_TAC \\ fs []
