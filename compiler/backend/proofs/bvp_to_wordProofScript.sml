...
val word_full_gc_def = Define `
  word_full_gc conf (all_roots,new,old,m,dm) =
...
val one_and_or_3 = prove(
  ``(3w && (w || 3w)) = 3w``,
  fs [fcpTheory.CART_EQ,word_or_def,word_and_def,fcpTheory.FCP_BETA,
      wordsTheory.word_index] \\ rw [] \\ rpt eq_tac \\ rw [] \\ fs []);

val ODD_not_zero = prove(
  ``ODD n ==> n2w n <> 0w``,
  CCONTR_TAC \\ full_simp_tac std_ss []
  \\ `((n2w n):'a word) ' 0 = (0w:'a word) ' 0` by metis_tac []
  \\ fs [wordsTheory.word_index,bitTheory.BIT_def,bitTheory.BITS_THM]
  \\ fs [dimword_def,bitTheory.ODD_MOD2_LEM])

val three_not_0 = store_thm("three_not_0[simp]",
  ``3w <> 0w``,
  match_mp_tac ODD_not_zero \\ fs []);
...
val shift_to_zero = prove(
  ``3w >>> 2 = 0w``,
  fs [fcpTheory.CART_EQ,word_lsr_def,fcpTheory.FCP_BETA,wordsTheory.word_index,
      DISJ_EQ_IMP] \\ rw [] \\ decide_tac)

val shift_around_under_big_shift = prove(
  ``!w n k. n <= k ==> (w << n >>> n << k = w << k)``,
  fs [fcpTheory.CART_EQ,word_lsr_def,fcpTheory.FCP_BETA,wordsTheory.word_index,
      DISJ_EQ_IMP,word_lsl_def] \\ rw []
  \\ Cases_on `k <= i` \\ fs []
  \\ `i - k < dimindex (:'a) /\ i − k + n < dimindex (:'a)` by decide_tac \\ fs []
  \\ fs [fcpTheory.CART_EQ,word_lsr_def,fcpTheory.FCP_BETA,wordsTheory.word_index,
      DISJ_EQ_IMP,word_lsl_def] \\ rw [])
...
val select_shift_out = prove(
  ...
  fs [fcpTheory.CART_EQ,word_lsr_def,fcpTheory.FCP_BETA,wordsTheory.word_index,
      DISJ_EQ_IMP,word_lsl_def,word_bits_def,word_or_def] \\ rw []
  \\ eq_tac \\ rw []
  \\ `i = 0` by decide_tac \\ fs []);

val shift_length_NOT_ZERO = store_thm("shift_length_NOT_ZERO[simp]",
  ``shift_length conf <> 0``,
  fs [shift_length_def] \\ decide_tac);
...
val LE_DIV_LT_IMP = prove(
  ...
  rw[] \\ `k < l DIV 2 ** m` by decide_tac
  \\ fs [X_LT_DIV,MULT_CLAUSES,GSYM ADD1]
  \\ Cases_on `2 ** m` \\ fs []
  \\ decide_tac);
...
val word_bits_eq_slice_shift = store_thm("word_bits_eq_slice_shift",
  ``((k -- n) w) = (((k '' n) w) >>> n)``,
  fs [fcpTheory.CART_EQ,word_or_def,word_and_def,fcpTheory.FCP_BETA,
         word_lsr_def,word_index,word_slice_def,word_bits_def] \\ rw []
  \\ Cases_on `i + n < dimindex (:'a)` \\ fs []
  \\ fs [fcpTheory.CART_EQ,word_or_def,word_and_def,fcpTheory.FCP_BETA,
         word_lsr_def,word_index,word_slice_def,word_bits_def] \\ rw []
...
val word_slice_lsl_eq_0 = prove(
  ``(k '' n) (w << (k + 1)) = 0w``,
  fs [fcpTheory.CART_EQ,word_or_def,word_and_def,fcpTheory.FCP_BETA,
      word_lsr_def,word_index,word_slice_def,word_bits_def,word_lsl_def] \\ rw []
  \\ CCONTR_TAC \\ fs [] \\ decide_tac);
...
val read_length_lemma = prove(
  ...
  fs [word_bits_eq_slice_shift,word_slice_or,can_select_def,DECIDE ``n+2-1=n+1n``]
  \\ fs [DECIDE ``2+n=n+1+1n``,word_slice_lsl_eq_0,word_slice_2_3_eq_0]);
...
val memcpy_thm = prove(
  ...
  \\ fs [] \\ imp_res_tac (DECIDE ``n+1n<k ==> n<k``) \\ fs []
  \\ rpt var_eq_tac \\ SEP_R_TAC \\ fs [WORD_LEFT_ADD_DISTRIB]);

val word_payload_IMP = prove(
  ``word_payload addrs ll tags tt1 conf = (h,ts,T) ==> LENGTH ts = ll``,
  Cases_on `tags` \\ fs [word_payload_def] \\ rw [] \\ fs []);

val LESS_EQ_IMP_APPEND = prove(
  ``!n xs. n <= LENGTH xs ==> ?ys zs. xs = ys ++ zs /\ LENGTH ys = n``,
  Induct_on `xs` \\ fs [] \\ Cases_on `n` \\ fs [LENGTH_NIL]
  \\ rw [] \\ res_tac \\ fs [] \\ rw []
  \\ qexists_tac `h::ys` \\ fs []);
...
val word_list_APPEND = store_thm("word_list_APPEND",
  ...
  Induct \\ fs [word_list_def,SEP_CLAUSES,STAR_ASSOC,ADD1,
...
val word_gc_move_thm = prove(
  ...
  reverse (Cases_on `x`) \\ fs [gc_move_def] THEN1
   (rw [] \\ fs [word_heap_def,SEP_CLAUSES]
    ...
    \\ qexists_tac `xs` \\ fs [heap_length_def])
  \\ CASE_TAC \\ fs []
  ...
  \\ drule LE_DIV_LT_IMP \\ fs [] \\ strip_tac
  ...
  THEN1
   (helperLib.SEP_R_TAC \\ fs [LET_THM,theWord_def,is_fws_ptr_OR_3]
    \\ rw [] \\ qexists_tac `xs` \\ fs []
    \\ fs [update_addr_def,shift_to_zero]
    \\ `2 <= shift_length conf` by (fs [shift_length_def] \\ decide_tac)
    \\ fs [shift_around_under_big_shift]
    \\ fs [get_addr_def,select_shift_out]
    \\ fs [select_get_lowerbits,heap_length_def])
  ...
  \\ discharge_hyps THEN1
   (fs [ADD1,SUM_APPEND,X_LE_DIV,RIGHT_ADD_DISTRIB]
    \\ Cases_on `2 ** shift_length conf` \\ fs []
    \\ Cases_on `n` \\ fs [MULT_CLAUSES]
    \\ Cases_on `n'` \\ fs [MULT_CLAUSES] \\ decide_tac)
  \\ rpt strip_tac
  \\ fs [word_addr_def,word_add_n2w,ADD_ASSOC] \\ rw []
  \\ fs [word_heap_APPEND,word_heap_def,SEP_CLAUSES,word_el_def,LET_THM]
  \\ fs [word_list_def]
  \\ SEP_W_TAC \\ qexists_tac `zs` \\ fs []
  \\ reverse conj_tac THEN1
   (fs [update_addr_def,get_addr_def,select_shift_out,select_get_lowerbits,ADD1])
  \\ pop_assum mp_tac
  \\ fs [AC STAR_ASSOC STAR_COMM]
  \\ fs [heap_length_def,SUM_APPEND,el_length_def,ADD1]
...
val word_gc_move_roots_thm = prove(
  ...
  Induct THEN1
   (fs [gc_move_list_def,word_gc_move_roots_def,word_heap_def,SEP_CLAUSES]
    \\ rw [] \\ qexists_tac `xs` \\ fs [heap_length_def])
  ...
  \\ split_pair_tac \\ fs []
  \\ rpt var_eq_tac \\ fs []
  \\ `c'` by imp_res_tac gc_move_list_ok \\ fs []
  ...
  \\ fs [WORD_LEFT_ADD_DISTRIB,heap_length_def,SUM_APPEND,GSYM word_add_n2w]);
...
val word_payload_T_IMP = prove(
  ...
  Cases_on `tag` \\ fs [word_payload_def] \\ rw [] \\ fs [LENGTH_NIL]
...
val word_gc_move_loop_thm = prove(
  ``!h1 h2 a n heap c0 limit h11 a1 n1 heap1 i1 pa1 m1 c1 xs frame m.
      ...
      (word_gc_move_loop conf (new + n2w (heap_length h1) * bytes_in_word,n2w a,
  ...
  THEN1
   (fs [gc_move_loop_def] \\ rpt var_eq_tac \\ fs []
    \\ pop_assum mp_tac \\ once_rewrite_tac [word_gc_move_loop_eq] \\ fs []
    \\ strip_tac \\ rpt var_eq_tac \\ fs []
    \\ qexists_tac `xs` \\ fs [AC STAR_COMM STAR_ASSOC])
  ...
  \\ Cases_on `h5` \\ fs [] \\ rpt var_eq_tac \\ fs []
  \\ qpat_assum `word_gc_move_loop _ _ = _` mp_tac
  \\ once_rewrite_tac [word_gc_move_loop_eq]
  \\ IF_CASES_TAC THEN1
   (`F` by all_tac
    \\ fs [heap_length_def,SUM_APPEND,el_length_def,
           WORD_LEFT_ADD_DISTRIB,GSYM word_add_n2w]
    \\ pop_assum mp_tac
    \\ Q.PAT_ABBREV_TAC `x = bytes_in_word * n2w (SUM (MAP el_length h1))` \\ fs []
    \\ full_simp_tac std_ss [GSYM WORD_ADD_ASSOC,addressTheory.WORD_EQ_ADD_CANCEL]
    \\ fs [bytes_in_word_def,word_add_n2w,word_mul_n2w]
    \\ fs [NOT_LESS] \\ fs [GSYM heap_length_def]
    ...
    \\ fs [] \\ rfs [good_dimindex_def,dimword_def]
    \\ rfs [good_dimindex_def,dimword_def] \\ decide_tac)
  \\ Cases_on `b5`
  \\ fs [word_heap_APPEND,word_heap_def,SEP_CLAUSES,STAR_ASSOC,word_el_def]
  \\ qpat_assum `_ (fun2set (m,dm))` assume_tac
  \\ fs [LET_THM]
  \\ split_pair_tac \\ fs []
  \\ full_simp_tac (std_ss++sep_cond_ss) [cond_STAR] \\ fs []
  \\ full_simp_tac std_ss [word_list_def] \\ SEP_R_TAC
  \\ fs [isWord_def,theWord_def] \\ rfs []
  \\ qcase_tac `word_payload _ _ tag _ conf = _`
  \\ drule word_payload_T_IMP
  \\ Cases_on `h ' 0` \\ fs [] \\ strip_tac \\ fs []
  THEN1
   (fs [gc_move_list_def] \\ rpt var_eq_tac \\ fs []
    \\ fs [heap_length_def,el_length_def,SUM_APPEND]
    \\ qpat_assum `!xx. nn` mp_tac
    \\ fs [GSYM word_add_n2w,WORD_LEFT_ADD_DISTRIB]
    \\ ntac 2 strip_tac \\ fs [SEP_CLAUSES]
    \\ first_x_assum match_mp_tac
    \\ qexists_tac `xs` \\ qexists_tac `m` \\ fs [])
  \\ qpat_assum `gc_move_list _ = _` mp_tac
  \\ once_rewrite_tac [gc_move_list_ALT] \\ strip_tac
  \\ fs [LET_THM] \\ split_pair_tac \\ fs []
  \\ ntac 5 var_eq_tac
  \\ drule word_gc_move_list_thm \\ fs []
  \\ ntac 2 strip_tac \\ fs []
  \\ split_pair_tac \\ fs []
  \\ first_x_assum drule \\ strip_tac
  \\ SEP_F_TAC
  \\ `LENGTH l5 < dimword (:α)` by
   (fs [NOT_LESS] \\ qpat_assum `_ <= heap_length heap` mp_tac
    \\ qpat_assum `heap_length heap <= _ ` mp_tac
    \\ rpt (pop_assum kall_tac) \\ fs [X_LE_DIV]
  ...
  \\ `word_payload xs' (LENGTH l5) tag r conf =
       (h,MAP (word_addr conf) xs',T)` by
         (match_mp_tac word_payload_swap \\ fs [])
  \\ fs [] \\ rfs []
  \\ first_x_assum match_mp_tac \\ fs []
  \\ fs [heap_length_def,el_length_def,SUM_APPEND]
  \\ fs [GSYM word_add_n2w,WORD_LEFT_ADD_DISTRIB,SEP_CLAUSES]
  \\ qpat_assum `_ = (i1,pa1,m1,c1)` (fn th => fs [GSYM th])
  \\ qexists_tac `xs1'` \\ fs []
  \\ qexists_tac `m1'` \\ fs []
  \\ qpat_assum `_ (fun2set (m1',dm))` mp_tac
  \\ fs [word_heap_APPEND,heap_length_def,el_length_def,SUM_APPEND]
  \\ fs [GSYM word_add_n2w,WORD_LEFT_ADD_DISTRIB,SEP_CLAUSES]
  \\ fs [AC STAR_ASSOC STAR_COMM,word_heap_APPEND]);
...
val word_list_exists_thm = store_thm("word_list_exists_thm",
  ...
  \\ rw [] \\ eq_tac \\ rw []
  THEN1
   (Cases_on `xs` \\ fs [ADD1] \\ fs [word_list_def]
    \\ qexists_tac `h` \\ fs []
    \\ qexists_tac `t` \\ fs [SEP_CLAUSES])
  \\ qexists_tac `w::xs` \\ fs [word_list_def,ADD1,STAR_ASSOC,cond_STAR]);
...
val word_list_exists_ADD = store_thm("word_list_exists_ADD",
  ...
  Induct \\ fs [word_list_exists_thm,SEP_CLAUSES,ADD_CLAUSES]
  \\ fs [STAR_ASSOC,ADD1,GSYM word_add_n2w,WORD_LEFT_ADD_DISTRIB]);
...
val word_full_gc_thm = prove(
  ...
  strip_tac \\ fs [full_gc_def,LET_THM]
  \\ split_pair_tac \\ fs []
  \\ split_pair_tac \\ fs []
  \\ rpt var_eq_tac \\ fs []
  \\ fs [word_heap_def,word_el_def]
  \\ fs [SEP_CLAUSES]
  \\ imp_res_tac gc_move_loop_ok \\ fs[]
  \\ drule word_gc_move_roots_thm
  \\ fs [word_list_exists_def,SEP_CLAUSES,SEP_EXISTS_THM,word_heap_heap_expand]
  \\ full_simp_tac (std_ss++sep_cond_ss) [cond_STAR]
  \\ fs [word_full_gc_def,LET_THM]
  \\ split_pair_tac \\ fs []
  \\ split_pair_tac \\ fs []
  ...
  \\ fs [word_heap_APPEND,word_heap_heap_expand]
  \\ pop_assum mp_tac
  \\ fs [STAR_ASSOC]
...
val word_gc_fun_lemma = prove(
  ...
  strip_tac
  \\ rewrite_tac [word_gc_fun_def] \\ fs []
  \\ fs [heap_in_memory_store_def,FLOOKUP_DEF,theWord_def,LET_THM]
  \\ split_pair_tac
  \\ fs [finite_mapTheory.FDOM_FUPDATE_LIST,FUPDATE_LIST,FAPPLY_FUPDATE_THM]
  \\ rpt var_eq_tac \\ fs []
  \\ `s ' Globals::MAP SND stack = MAP (word_addr c) (v'::xs)` by
    (fs [LIST_REL_EQ_MAP] \\ CONV_TAC (DEPTH_CONV ETA_CONV) \\ fs [])
  ...
  \\ Cases_on `roots2` \\ fs []
  \\ `LENGTH xs = LENGTH stack` by metis_tac [LENGTH_MAP] \\ fs []
  \\ fs [listTheory.MAP_ZIP]
  \\ fs [LIST_REL_EQ_MAP]
  \\ CONV_TAC (DEPTH_CONV ETA_CONV) \\ fs []
  \\ imp_res_tac full_gc_IMP \\ fs []
  \\ rfs[heap_length_APPEND,heap_length_heap_expand]
  \\ `heap_length heap2 + (heap_length heap - heap_length heap2) =
      heap_length heap` by decide_tac \\ fs []) |> GEN_ALL
...
val word_gc_fun_IMP_FILTER = prove(
  ...
  fs [word_gc_fun_def,LET_THM,word_gc_fun_def,word_full_gc_def]
...
val compile_correct = store_thm("compile_correct",
  ...
  THEN1 (* Seq *)
   (once_rewrite_tac [bvp_to_wordTheory.comp_def] \\ fs []
    ...
    THEN1 (fs [] \\ rpt strip_tac \\ fs [] \\ rw [] \\ Cases_on `q''` \\ fs []
   ...
   (once_rewrite_tac [bvp_to_wordTheory.comp_def] \\ fs []
    ...
    \\ Cases_on `x'` \\ fs [] THEN1
     (Cases_on `pop_env r'` \\ fs [] \\ rw []
      ...
      \\ imp_res_tac state_rel_pop_env_set_var_IMP \\ fs [] \\ rw []
      \\ pop_assum mp_tac \\ fs []
      \\ match_mp_tac (GEN_ALL evaluate_IMP_domain_EQ) \\ fs []
      \\ asm_exists_tac \\ fs [])
