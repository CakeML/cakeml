...
Ancestors
  ...
  bvi_inlineProof[qualified] bvi_tmcProof[qualified]
  bvi_cprProof[qualified] bvl_to_bvi bviProps
Libs
...
Theorem aux_code_installed_APPEND[local]:
  !xs ys.
...
Theorem bv_ok_SUBSET_IMP[local]:
  !refs x refs2.
...
Theorem bv_ok_IMP_adjust_bv_eq[local]:
  !b2 a1 b3.
      bv_ok refs a1 /\
...
Theorem evaluate_ok_lemma[local]:
...
Theorem evaluate_IMP_bv_ok[local]:
...
Theorem do_build_ok:
  ∀parts m n r q rs.
    do_build m n parts r.refs = (q,rs) ∧
    state_ok r ∧ (∀k. bv_ok r.refs (m k)) ⇒
    state_ok (r with refs := rs) ∧ bv_ok rs q
Proof
  Induct
  THEN1 fs [do_build_def,state_ok_def]
  \\ fs [do_build_def] \\ rpt gen_tac
  \\ pairarg_tac \\ gvs []
  \\ strip_tac
  \\ ‘(r with refs := rs) = ((r with refs := refs1) with refs := rs)’ by fs []
  \\ asm_rewrite_tac []
  \\ first_x_assum irule \\ gvs []
  \\ first_x_assum $ irule_at Any
  \\ Cases_on ‘h’ \\ gvs [do_part_def]
  \\ TRY (gvs [state_ok_def] \\ rw [APPLY_UPDATE_THM]
          \\ gvs [bv_ok_def,EVERY_MEM,MEM_MAP,PULL_EXISTS] \\ NO_TAC)
  \\ gvs [state_ok_def]
  \\ gvs [APPLY_UPDATE_THM,FLOOKUP_UPDATE]
  \\ rw [] \\ fs [EVERY_MEM] \\ rw [] \\ res_tac
  THEN1
   (TOP_CASE_TAC \\ gvs [] \\ rw []
    \\ irule bv_ok_SUBSET_IMP
    \\ fs [FDOM_FUPDATE,SUBSET_DEF]
    \\ first_x_assum $ irule_at Any \\ fs [])
  THEN1
   (TOP_CASE_TAC \\ gvs [] \\ rw []
    \\ TOP_CASE_TAC \\ gvs [] \\ rw []
    \\ last_x_assum (qspec_then ‘k’ mp_tac) \\ fs [] \\ rw [] \\ res_tac
    \\ irule bv_ok_SUBSET_IMP
    \\ fs [FDOM_FUPDATE,SUBSET_DEF]
    \\ first_x_assum $ irule_at Any \\ fs [])
  THEN1 fs [bv_ok_def]
...
Proof
  ...
  srw_tac[][iEval_def,iEvalOp_def,do_app_aux_def,bEvalOp_def,do_int_app_def,
  bvl_to_bvi_id,backend_commonTheory.small_enough_int_def,bvl_to_bvi_with_refs] >- (
...
Theorem evaluate_AllocGlobal_code[local]:
   ...
   ∃p1 c extra.
     (p1 ≠ p ⇒ p1 ∉ FDOM s.refs) ∧
     n+k+1 ≤ LENGTH ls + extra ∧
     evaluate ([SND AllocGlobal_code],[Number (& k)],inc_clock c s) =
       (Rval [Unit],
        s with <| global := SOME p1; refs := s.refs
          |+ (p, ValueArray ((Number(&(SUC(n+k))))::ls))
          |+ (p1, ValueArray ((Number(&(SUC(n+k))))::
                              ls ++ (REPLICATE extra (Number 0))))|>)
Proof
  strip_tac
  \\ simp [AllocGlobal_code_def]
  \\ simp [Once iEval_def,iEvalOp_def]
  \\ ntac 7 $ simp [Once iEval_def,iEvalOp_def,inc_clock_def,do_app_aux_def]
  \\ simp [Once iEval_def,iEvalOp_def,inc_clock_def,do_app_aux_def,do_int_app_def]
  ...
  \\ ntac 4 $ simp [Once iEval_def]
  \\ simp [iEvalOp_def,inc_clock_def,do_app_aux_def,bvlSemTheory.do_app_def,do_int_app_def]
  \\ ntac 3 $ simp [Once iEval_def]
  \\ simp [iEvalOp_def,inc_clock_def,do_app_aux_def,bvlSemTheory.do_app_def]
  \\ simp [bvl_to_bvi_id]
  \\ ntac 8 $ simp [Once iEval_def]
  \\ simp [iEvalOp_def,do_app_aux_def,bvlSemTheory.do_app_def]
  \\ simp [EVAL “small_enough_int 0”]
  \\ ntac 5 $ simp [Once iEval_def]
  \\ simp [iEvalOp_def,do_app_aux_def,bvlSemTheory.do_app_def,integerTheory.INT_ADD,do_int_app_def]
  \\ fs [ADD1] \\ CASE_TAC
  >-
   (gvs [bvl_to_bvi_def,bvi_to_bvl_def,iEval_def,state_component_equality]
    \\ qexists_tac ‘0’ \\ gvs [LUPDATE_def])
  ...
  \\ simp [iEvalOp_def,do_app_aux_def,bvlSemTheory.do_app_def,integerTheory.INT_ADD,do_int_app_def]
  \\ gvs [bvl_to_bvi_def,bvi_to_bvl_def]
  \\ ntac 2 $ simp [Once iEval_def]
  \\ simp [iEvalOp_def,do_app_aux_def,bvlSemTheory.do_app_def,integerTheory.INT_ADD]
  \\ ntac 10 $ simp [Once iEval_def]
  \\ simp [iEvalOp_def,do_app_aux_def,bvlSemTheory.do_app_def,do_int_app_def]
  \\ simp [EVAL “small_enough_int 0”,EVAL “small_enough_int 1”]
  \\ simp [do_int_app_def]
  \\ gvs [bvl_to_bvi_def,bvi_to_bvl_def,find_code_def,dec_clock_def]
  \\ qabbrev_tac ‘new_p = LEAST ptr. ptr ≠ p ∧ ptr ∉ FDOM s.refs’
  \\ qexists_tac ‘new_p’ \\ fs []
  \\ Q.REFINE_EXISTS_TAC ‘ccc+1’ \\ fs []
  \\ gvs [AllCaseEqs()]
  ...
  \\ qexists_tac ‘c’ \\ fs []
  \\ simp [state_component_equality]
  \\ qmatch_goalsub_abbrev_tac ‘REPLICATE kk’
  \\ qexists_tac ‘kk’ \\ fs [Abbr‘kk’]
...
Proof
  ...
  \\ simp[iEval_def,iEvalOp_def,do_app_aux_def,bEvalOp_def,do_int_app_def,
...
Proof
  ...
  \\ simp [ToListByte_code_def,Once evaluate_def,do_app_def,do_int_app_def,
        do_app_aux_def,
        bvlSemTheory.do_app_def,inc_clock_def,bvl_to_bvi_def,bvi_to_bvl_def]
  \\ simp [Once evaluate_def,do_app_def,do_app_aux_def,bvlSemTheory.do_app_def]
  \\ ntac 11 (simp [Once evaluate_def,do_app_def,do_int_app_def,
...
Proof
  ...
  \\ Cases_on`ps` \\ fs[]
  \\ Cases_on`wss` \\ fs[]
  \\ rw[evaluate_def,iEvalOp_def,do_app_aux_def,bEvalOp_def,do_int_app_def,
...
Proof
  ...
  >- (
    rw[evaluate_def,iEvalOp_def,do_app_aux_def,do_int_app_def,bEvalOp_def,bvl_to_bvi_id]
  ...
  \\ Cases_on`wss` \\ fs[]
  \\ rw[evaluate_def,iEvalOp_def,do_app_aux_def,bEvalOp_def,do_int_app_def,
        bvl_to_bvi_id,backend_commonTheory.small_enough_int_def,
...
Theorem compile_exps_Var_list[local]:
...
Theorem do_eq_adjust_lemma[local]:
  (!refs x1 x2 b.
              refs = r.refs /\
...
Proof
  ...
  \\ Cases_on `op = MemOp Update` \\ fs [] THEN1
   (fs [AllCaseEqs(),PULL_EXISTS] \\ rw []
    ...
    \\ rename [‘i < _:int’]
    \\ ‘∃k. i = &k’ by intLib.COOPER_TAC \\ gvs [adjust_bv_def]
    \\ full_simp_tac(srw_ss())[bEvalOp_def,SWAP_REVERSE_SYM]
    \\ SIMP_TAC std_ss [Once bvi_to_bvl_def] \\ full_simp_tac(srw_ss())[]
    \\ `FLOOKUP t2.refs (b2 n) =
        SOME (ValueArray (MAP (adjust_bv b2) xs))` by
...
Proof
  rw[in_ns_def,backend_commonTheory.bvl_to_bvi_namespaces_def]
  \\ qspecl_then[`5`,`x`,`1`]mp_tac(Q.GENL[`n`,`x`,`k`]MOD_LIFT_PLUS_IFF)
  \\ simp[]
QED
...
val goal = ``
   λ(xs,env,s1).
   !n res s2 (t1:('c,'ffi) bviSem$state) n2 ys aux b1.
  ...
  \\ fs [] \\ first_x_assum old_drule \\ fs []
    ...
    \\ Cases_on `?i. op = IntOp (Const i)` \\ full_simp_tac(srw_ss())[] THEN1
     (note_tac "Op: Const" \\ CONV_TAC SWAP_EXISTS_CONV \\ Q.EXISTS_TAC `b2`
      ...
      \\ full_simp_tac(srw_ss())[] \\ full_simp_tac(srw_ss())[compile_op_def]
      \\ qpat_x_assum `do_app _ _  _ = _` mp_tac
      \\ disch_then (mp_tac o SRULE[AllCaseEqs(),bEvalOp_def,iEval_def,oneline do_int_app_def])
      \\ rw[] \\ fs[] \\ Cases_on `c1`
      \\ full_simp_tac(srw_ss())[compile_int_thm,bEvalOp_def,iEval_def] \\ rveq
      \\ full_simp_tac(srw_ss())[iEval_def,iEvalOp_def,oneline do_int_app_def]
    ...
    \\ Cases_on`op = GlobOp AllocGlobal` \\ full_simp_tac(srw_ss())[] THEN1
        (note_tac "Op: AllocGlobal" \\ simp[compile_op_def]
         \\ gvs [bvlSemTheory.do_app_def,AllCaseEqs()]
         \\ rename [‘~(ii < 0:int)’]
         \\ ‘∃k. ii = & k’ by (Cases_on ‘ii’ \\ gvs [])
         \\ drule_then assume_tac bvlPropsTheory.evaluate_IMP_LENGTH
         \\ gvs [LENGTH_EQ_NUM_compute]
         ...
         \\ ‘c + c' + t1.clock + 1 = (c + c' + 1) + t1.clock’ by fs []
    ...
    >- (
      ...
      \\ Induct_on ‘parts’ \\ fs [do_build_const_def,do_build_def]
      THEN1
      ...
      \\ reverse (rpt strip_tac)
      THEN1
       (gvs [] \\ first_x_assum (qspec_then ‘a’ mp_tac)
        \\ fs [APPLY_UPDATE_THM] \\ rw []
        \\ metis_tac [LEAST_NOTIN_FDOM])
      THEN1
...
Theorem evaluate_REPLICATE_0[local]:
  ...
  impl_tac >- (
    ...
    conj_tac >- (
      ...
      >- (
        ...
        \\ impl_tac >- fs[]
        \\ strip_tac
        \\ `m MOD nss = (m - num_stubs) MOD nss` by
             (`?d. m = num_stubs + d` by (qexists_tac `m - num_stubs` \\ fs[])
              \\ asm_simp_tac std_ss []
              \\ assume_tac (EVAL ``nss``) \\ assume_tac (EVAL ``num_stubs``)
              \\ fs [arithmeticTheory.MOD_TIMES2 |> Q.SPEC `nss`]
              \\ simp [Once arithmeticTheory.ADD_COMM]
              \\ simp [arithmeticTheory.MOD_TIMES, GSYM (EVAL ``num_stubs``)])
        \\ assume_tac (EVAL ``nss``)
        \\ `(m - num_stubs) MOD nss < nss` by fs[]
        \\ full_simp_tac std_ss [in_ns_def]
        \\ `(m - num_stubs) MOD 5 < 5` by fs[] \\ decide_tac )
...
Theorem compile_prog_distinct_locs:
   compile_prog start n prog = (k,prog1,n1) /\ ALL_DISTINCT (MAP FST prog) ==>
    ...
    EVERY (λn. ¬in_ns 2 (n - num_stubs) ∧ ¬in_ns 3 (n - num_stubs) ∧ ¬in_ns 4 (n - num_stubs))
      (MAP FST prog1)
Proof
...
(* --- no_mutcons: syntactic absence of MutCons operations ---
   ...
   unconditionally: compile_op compiles out the mem_ops reserved for
   bvi_tmc (bvlSem$do_app is Error on them), so no bvi code produced by
   bvl_to_bvi contains them.  The lemmas below carry the property
   through bvi_let and bvi_tailrec to the two input_condition
   obligations in compile_semantics below. *)
...
Theorem compile_op_no_mutcons[local]:
  EVERY no_mutcons c1 ⇒ no_mutcons (compile_op op c1)
Proof
  strip_tac
...
Theorem compile_exps_sing_no_mutcons[local]:
  (∀n e c aux n1.
     compile_exps_sing n e = (c,aux,n1) ⇒
     no_mutcons c ∧ EVERY (no_mutcons o SND o SND) (append aux)) ∧
  (∀n es c aux n1.
     compile_exps_list n es = (c,aux,n1) ⇒
...
(* NB free_names must be qualified: bvi_tmcProof defines an identically
   printing (and identically stated) constant of the same name. *)
Theorem EVERY_free_names_not_in_ns_2[local]:
   EVERY (λn. ¬in_ns 2 (n − num_stubs)) (MAP FST xs) ⇒
   EVERY (bvi_tailrecProof$free_names (num_stubs + 2) o FST) xs
Proof
  rw[EVERY_MEM, EVERY_MAP, in_ns_def,
     bvi_tailrecProofTheory.free_names_def]
  \\ res_tac \\ CCONTR_TAC \\ gvs []
  \\ first_x_assum drule
  \\ qpat_x_assum `_ = FST e` (assume_tac o GSYM) \\ gvs []
...
Theorem cpr_inline_semantics[local]:
  bvi_cpr$compile_prog b (num_stubs + 4, LN) code = ((n4,cm),code4) ∧
  bvi_inline$compile_prog code4 = (bl,code5) ∧
  bvi_cprProof$input_condition (num_stubs + 4) code ∧
  (∀k st cfg p. co k = ((st,cfg),p) ⇒
     bvi_cprProof$input_condition (FST st) p) ∧
  FST (FST (co 0)) = (n4,cm) ∧ FST (SND (FST (co 0))) = bl ∧
  semantics ffi (fromAList code) co
    (state_cc (bvi_cpr$compile_prog b) (state_cc bvi_inline$compile_inc cc))
    start ≠ Fail ⇒
  semantics ffi (fromAList code5)
    (state_co bvi_inline$compile_inc (state_co (bvi_cpr$compile_prog b) co))
    cc start =
  semantics ffi (fromAList code) co
    (state_cc (bvi_cpr$compile_prog b) (state_cc bvi_inline$compile_inc cc))
    start
Proof
  ...
  >> ‘semantics ffi (fromAList code) co
        (state_cc (bvi_cpr$compile_prog b) (state_cc bvi_inline$compile_inc cc))
        start =
      semantics ffi (fromAList code4) (state_co (bvi_cpr$compile_prog b) co)
        (state_cc bvi_inline$compile_inc cc) start’
    by (
      irule bvi_cprProofTheory.compile_prog_semantics
      >> simp [PULL_EXISTS]
      >> qexistsl_tac [‘num_stubs + 4’,‘(n4,cm)’] >> simp []
      >> rpt strip_tac
      >- metis_tac []
      >> drule_all bvi_cprProofTheory.compile_prog_MEM >> strip_tac >> gvs []
      >> drule bvi_cprProofTheory.compile_prog_next_mono >> strip_tac
      >> gvs [bvi_cprProofTheory.input_condition_def, EVERY_MEM, MEM_FILTER,
              MEM_MAP, PULL_EXISTS]
      >> rename1 ‘MEM y0 code’
      >> Cases_on ‘num_stubs ≤ FST y0’ >- metis_tac []
      >> simp [])
  >> gvs[]
  >> irule bvi_inlineProofTheory.compile_prog_semantics
  >> simp[backendPropsTheory.FST_state_co]
  >> drule bvi_cprProofTheory.compile_prog_ALL_DISTINCT
  >> gvs[bvi_cprProofTheory.input_condition_def]
QED

Theorem bvi_tailrec_not_in_ns_4[local]:
  bvi_tailrec$compile_prog b n ys = (n1,zs) ∧ in_ns 2 n ∧
  (∀x. MEM x (MAP FST ys) ∧ num_stubs ≤ x ⇒ ¬in_ns 4 x) ∧
  MEM x (MAP FST zs) ∧ num_stubs ≤ x ⇒
  ¬in_ns 4 x
Proof
  rpt strip_tac
...
Theorem bvi_tmc_not_in_ns_4[local]:
  bvi_tmc$compile_prog b n ys = (n1,zs) ∧ in_ns 3 n ∧
  (∀x. MEM x (MAP FST ys) ∧ num_stubs ≤ x ⇒ ¬in_ns 4 x) ∧
  MEM x (MAP FST zs) ∧ num_stubs ≤ x ⇒
  ¬in_ns 4 x
Proof
...
Theorem tailrec_co_not_in_ns_4[local]:
  in_ns 2 (FST (SND (SND (FST (co k))))) ∧
  MEM x (MAP FST (SND (state_co (bvi_tailrec$compile_prog b)
                         (state_co compile_inc
                            (state_co (bvl_inline$compile_inc l s e) co)) k))) ∧
  num_stubs ≤ x ⇒
  ¬in_ns 4 x
Proof
  simp [state_co_def]
  >> rpt (pairarg_tac >> gvs [])
  >> strip_tac
  >> drule_then irule bvi_tailrec_not_in_ns_4 >> simp []
  >> rpt strip_tac
  >> drule_all compile_inc_next_range >> rw [] >> gvs [in_ns_def]
QED

Theorem compile_semantics:
   compile start c names prog =
     (start',prog',inlines,bvi_inlines,n1,n2,n3,n4,cm,names') ∧
   FST (FST (co 0)) = inlines /\
   FST (SND (FST (co 0))) = n1 /\
   FST (SND (SND (FST (co 0)))) = n2 /\
   FST (SND (SND (SND (FST (co 0))))) = n3 /\
   FST (SND (SND (SND (SND (FST (co 0)))))) = (n4,cm) /\
   FST (SND (SND (SND (SND (SND (FST (co 0))))))) = bvi_inlines /\
   (∀n. ALL_DISTINCT (MAP FST (SND (co n))) ∧
        ...
        num_stubs ≤ FST (FST(SND(SND(SND(SND(FST(co n))))))) ∧
        in_ns 4 (FST (FST(SND(SND(SND(SND(FST(co n))))))))) /\
   ALL_DISTINCT (MAP FST prog) ==>
...
Proof
  ...
  THEN1
   (disch_then (assume_tac o GSYM) \\ fs []
    ...
    (* conclusion branch: thread the bvi_cpr and bvi_inline passes through *)
    >- (
      disch_then assume_tac >> fs []
      >> irule cpr_inline_semantics >> simp [backendPropsTheory.FST_state_co]
      >> ‘in_ns 2 (num_stubs + 2) ∧ in_ns 3 (num_stubs + 3) ∧
          in_ns 4 (num_stubs + 4)’ by EVAL_TAC
      >> subgoal ‘∀x. MEM x (MAP FST code) ∧ num_stubs ≤ x ⇒ ¬in_ns 4 x’
      >- (
        rpt strip_tac
        >> ‘ALL_DISTINCT (MAP FST prog'')’
          by (imp_res_tac bvl_inlineProofTheory.compile_prog_names >> fs [])
        >> drule_all compile_prog_distinct_locs
        >> strip_tac >> fs [EVERY_MEM]
        >> qpat_x_assum ‘∀n. MEM n (MAP FST code) ⇒ _’ drule >> strip_tac
        >> qabbrev_tac ‘y = x - num_stubs’
        >> ‘x = num_stubs + y’ by simp [Abbr ‘y’]
        >> gvs []
        >> metis_tac [in_ns_add_num_stubs, ADD_COMM])
      >> ‘∀x. MEM x (MAP FST code'') ∧ num_stubs ≤ x ⇒ ¬in_ns 4 x’
        by metis_tac [bvi_tailrec_not_in_ns_4, bvi_tmc_not_in_ns_4]
      >> conj_tac
      >- (
        rpt gen_tac >> strip_tac
        >> qpat_assum ‘state_co _ _ k = _’ (mp_tac o Q.AP_TERM ‘FST’)
        >> simp [backendPropsTheory.FST_state_co] >> strip_tac
        >> qpat_x_assum ‘∀n. ALL_DISTINCT _ ∧ _’ (qspec_then ‘k’ mp_tac)
        >> simp [] >> strip_tac
        >> qpat_x_assum ‘state_co _ _ k = _’ mp_tac
        >> simp [Once state_co_def]
        >> qmatch_goalsub_abbrev_tac ‘_ trk = _ ⇒ _’
        >> ‘in_ns 2 (FST (SND (SND (FST (co k))))) ∧
            ∀x. MEM x (MAP FST (SND trk)) ∧ num_stubs ≤ x ⇒ ¬in_ns 4 x’
          by (simp [Abbr ‘trk’] >> metis_tac [tailrec_co_not_in_ns_4])
        >> PairCases_on ‘trk’ >> simp []
        >> pairarg_tac >> simp [] >> strip_tac >> gvs []
        >> qpat_x_assum ‘∀k n cfg prog. _ ⇒ bvi_tmcProof$input_condition _ _’
             drule
        >> strip_tac
        >> irule cpr_input_condition_intro >> simp []
        >> conj_tac
        >- (
          rpt gen_tac >> strip_tac
          >> drule_then irule bvi_tmc_not_in_ns_4
          >> gvs [bvi_tmcProofTheory.input_condition_def, in_ns_def])
        >> drule bvi_tmcProofTheory.compile_prog_ALL_DISTINCT
        >> gvs [bvi_tmcProofTheory.input_condition_def])
      >> irule cpr_input_condition_intro >> simp []
      >> drule bvi_tmcProofTheory.compile_prog_ALL_DISTINCT
    ...
    THEN1
     (rw[]
      ...
      \\ rveq
      \\ qpat_x_assum `(num_stubs + 2 + k' * nss) MOD nss = 3` mp_tac
      \\ rewrite_tac [GSYM arithmeticTheory.ADD_ASSOC]
      \\ once_rewrite_tac [GSYM in_ns_def]
      \\ rewrite_tac [in_ns_add_num_stubs] \\ rewrite_tac [in_ns_def]
      \\ assume_tac (EVAL ``nss``)
      \\ `2 + k' * 5 = 5 * k' + 2` by simp[]
      \\ asm_simp_tac std_ss [arithmeticTheory.MOD_TIMES]
      \\ simp[arithmeticTheory.MOD_TIMES])
    \\ conj_tac
    (* input_condition for the bvi_tailrec output code' (ns 3) *)
    >-
     (`!x. MEM x (MAP FST code') /\ x MOD nss = 3 ==> x <= num_stubs` by (
      ...
      \\ `ALL_DISTINCT (MAP FST prog'')` by
           (imp_res_tac bvl_inlineProofTheory.compile_prog_names \\ fs[])
      \\ `ALL_DISTINCT (MAP FST code) /\
          EVERY (λn. ¬in_ns 2 (n - num_stubs) ∧ ¬in_ns 3 (n - num_stubs) ∧ ¬in_ns 4 (n - num_stubs)) (MAP FST code)` by
           (drule compile_prog_distinct_locs \\ disch_then drule \\ simp[])
      \\ `EVERY (bvi_tailrecProof$free_names (num_stubs+2) o FST) code` by (
           simp[GSYM EVERY_MAP]
           \\ qpat_x_assum `EVERY (λn. ¬in_ns 2 (n - num_stubs) ∧ ¬in_ns 3 (n - num_stubs) ∧ ¬in_ns 4 (n - num_stubs)) (MAP FST code)` mp_tac
           \\ rpt (pop_assum kall_tac)
           \\ simp[EVERY_MEM, bvi_tailrecProofTheory.free_names_def,
                   bvi_tmcProofTheory.free_names_def, in_ns_def]
           \\ rw[] \\ res_tac \\ CCONTR_TAC
           \\ fs[bvi_tailrecProofTheory.free_names_def, bvi_tmcProofTheory.free_names_def]
           \\ `MEM (FST e) (MAP FST code)` by (simp[MEM_MAP] \\ metis_tac[])
           \\ qpat_x_assum `∀n. MEM n (MAP FST code) ⇒ _` drule \\ strip_tac
           \\ qpat_x_assum `_ = FST e` (assume_tac o GSYM)
           \\ fs[] \\ assume_tac (EVAL ``nss``) \\ fs[arithmeticTheory.MOD_TIMES])
      \\ `ALL_DISTINCT (MAP FST code')` by (
           drule bvi_tailrecProofTheory.compile_prog_ALL_DISTINCT
           \\ impl_tac
           >- (conj_tac >- fs[]
               \\ fs[EVERY_MEM, bvi_tailrecProofTheory.free_names_def, bvi_tmcProofTheory.free_names_def])
           \\ strip_tac \\ fs[])
      \\ simp[bvi_tmcProofTheory.input_condition_def, EVAL ``(num_stubs + 3) MOD nss``]
      \\ conj_tac
      >- (simp[EVERY_MEM, bvi_tmcProofTheory.free_names_def, bvi_tailrecProofTheory.free_names_def]
          \\ rw[] \\ CCONTR_TAC \\ fs[]
          \\ `MEM (FST e) (MAP FST code')` by (simp[MEM_MAP] \\ metis_tac[])
          \\ `FST e MOD nss = 3` by
               (qpat_x_assum `num_stubs + (k * nss + 3) = FST e` (assume_tac o GSYM)
                \\ asm_simp_tac std_ss [] \\ assume_tac (EVAL ``nss``)
                \\ once_rewrite_tac [GSYM in_ns_def] \\ rewrite_tac [in_ns_add_num_stubs]
                \\ rewrite_tac [in_ns_def]
                \\ `k * 5 + 3 = 5 * k + 3` by simp[]
    ...
    \\ `m2 MOD nss = 2` by
         (qpat_x_assum `Abbrev (m2 = _)` (assume_tac o REWRITE_RULE [markerTheory.Abbrev_def])
          \\ fs[in_ns_def])
    \\ `!x. MEM x (MAP FST r) ==> x MOD nss <> 3` by (
         rpt strip_tac
         \\ drule (GEN_ALL bvi_tailrecProofTheory.compile_prog_MEM)
         \\ disch_then drule \\ strip_tac
         >- (qpat_x_assum `Abbrev (xs = _)` mp_tac
             \\ qmatch_goalsub_abbrev_tac `compile_inc V P`
             \\ Cases_on `compile_inc V P` \\ strip_tac
             \\ `xs = r'` by fs[markerTheory.Abbrev_def] \\ fs[]
             \\ drule (GEN_ALL compile_inc_next_range)
             \\ disch_then drule
             \\ `x MOD nss <> 0 /\ x MOD nss <> 1` by fs[]
             \\ rw[] \\ fs[] \\ rfs[])
         \\ rveq \\ assume_tac (EVAL ``nss``)
         \\ qpat_x_assum `(m2 + k' * nss) MOD nss = 3` mp_tac
         \\ `m2 + k' * 5 = 5 * k' + m2` by simp[]
         \\ asm_simp_tac std_ss [arithmeticTheory.MOD_TIMES] \\ fs[])
    \\ `ALL_DISTINCT (MAP FST r)` by (
         ...
         \\ qpat_x_assum`_ MOD _ = _`mp_tac
         \\ EVAL_TAC \\ simp[])
    \\ `num_stubs <= n /\ n MOD nss = 3` by
         (`FST (SND (SND (SND (FST (co k))))) = n` by fs[]
          \\ fs[in_ns_def])
    \\ simp[bvi_tmcProofTheory.input_condition_def]
    \\ conj_tac
    >- (simp[EVERY_MEM, bvi_tmcProofTheory.free_names_def, bvi_tailrecProofTheory.free_names_def]
        \\ rw[] \\ CCONTR_TAC \\ fs[]
        \\ `MEM (FST e) (MAP FST r)` by (simp[MEM_MAP] \\ metis_tac[])
        \\ `FST e MOD nss = 3` by
             (qpat_x_assum `n + k' * nss = FST e` (fn th => once_rewrite_tac[GSYM th])
              \\ `0 < nss` by EVAL_TAC
              \\ `n + k' * nss = k' * nss + n` by simp[]
    ...
    \\ conj_tac >-
     (fs [markerTheory.Abbrev_def]
      ...
      \\ drule compile_inc_no_mutcons \\ strip_tac
      \\ gvs []
      \\ drule_all bvi_tailrec_compile_prog_no_mutcons \\ simp [])
    \\ simp[EVERY_MEM, MEM_FILTER] \\ rw[] \\ strip_tac
    \\ `MEM (FST e) (MAP FST r)` by (simp[MEM_MAP] \\ metis_tac[])
...
Theorem cpr_distinct_names[local]:
  bvi_cpr$compile_prog b (num_stubs + 4,LN) xs = ((n4,cm),ys) ∧
  EVERY (λn. data_num_stubs ≤ n) (MAP FST xs) ∧ ALL_DISTINCT (MAP FST xs) ∧
  (∀x. MEM x (MAP FST xs) ∧ num_stubs ≤ x ⇒ ¬in_ns 4 x) ⇒
  EVERY (λn. data_num_stubs ≤ n) (MAP FST ys) ∧ ALL_DISTINCT (MAP FST ys)
Proof
  strip_tac
  >> ‘bvi_cprProof$input_condition (num_stubs + 4) xs’
    by (irule cpr_input_condition_intro >> simp [] >> EVAL_TAC)
  >> conj_tac
  >- (
    rw [EVERY_MEM]
    >> drule_all bvi_cprProofTheory.compile_prog_MEM >> rw []
    >> gvs [EVERY_MEM]
    >> ‘data_num_stubs ≤ num_stubs’ by EVAL_TAC
    >> simp [])
  >> drule bvi_cprProofTheory.compile_prog_ALL_DISTINCT
  >> gvs [bvi_cprProofTheory.input_condition_def]
QED
...
Proof
  ...
  >> strip_tac >> rpt (pairarg_tac>>fs[]>>rveq>>fs[])
  (* bvi_inline preserves the set of names, and bvi_cpr only adds fresh
     ns-4 names, so the goal is about the bvi_tmc output *)
  \\ imp_res_tac bvi_inlineProofTheory.compile_inc_MAP_FST \\ fs[]
  \\ irule cpr_distinct_names
  ...
  >- (
    rpt gen_tac >> strip_tac
    >> ‘in_ns 2 (num_stubs + 2) ∧ in_ns 3 (num_stubs + 3)’ by EVAL_TAC
    >> ‘∀z. MEM z (MAP FST code) ∧ num_stubs ≤ z ⇒ ¬in_ns 4 z’
      by (
        qx_gen_tac ‘z’ >> rpt strip_tac
        >> drule_all compile_prog_distinct_locs
        >> strip_tac >> fs [EVERY_MEM]
        >> qpat_x_assum ‘∀n. MEM n (MAP FST code) ⇒ _’ drule >> strip_tac
        >> qabbrev_tac ‘y = z - num_stubs’
        >> ‘z = num_stubs + y’ by simp [Abbr ‘y’]
        >> gvs []
        >> metis_tac [in_ns_add_num_stubs, ADD_COMM])
    >> metis_tac [bvi_tailrec_not_in_ns_4, bvi_tmc_not_in_ns_4])
  \\ once_rewrite_tac [CONJ_COMM]
  \\ `ALL_DISTINCT (MAP FST code) /\
      EVERY (λn. ¬in_ns 2 (n - num_stubs) ∧ ¬in_ns 3 (n - num_stubs) ∧ ¬in_ns 4 (n - num_stubs)) (MAP FST code)` by
       (drule compile_prog_distinct_locs \\ disch_then drule \\ simp[])
  \\ `EVERY (λn. data_num_stubs ≤ n) (MAP FST code)` by (
       qpat_x_assum `compile_prog n0 0 prog = (k,code,_)`
          (strip_assume_tac o REWRITE_RULE [bvl_to_bviTheory.compile_prog_def])
       \\ pop_assum mp_tac
       \\ Cases_on `compile_list 0 prog` \\ simp[] \\ strip_tac \\ rveq
       \\ simp[EVERY_APPEND] \\ conj_tac >- EVAL_TAC
       \\ qpat_x_assum `compile_list 0 prog = _` assume_tac
       \\ drule (SIMP_RULE std_ss [] compile_list_distinct_locs)
  ...
  >- (conj_tac >- fs[]
      \\ fs[EVERY_MEM, bvi_tailrecProofTheory.free_names_def, bvi_tmcProofTheory.free_names_def])
  \\ strip_tac
  \\ drule bvi_tmcProofTheory.compile_prog_ALL_DISTINCT \\ impl_tac
  >- (conj_asm1_tac >- fs[]
      \\ simp[EVERY_MAP, EVERY_MEM, bvi_tmcProofTheory.free_names_def,
              bvi_tailrecProofTheory.free_names_def]
      \\ rw[] \\ CCONTR_TAC \\ fs[]
      \\ `MEM (FST e) (MAP FST code')` by (simp[MEM_MAP] \\ metis_tac[])
      \\ drule (GEN_ALL bvi_tailrecProofTheory.compile_prog_MEM)
      \\ disch_then drule \\ strip_tac
      \\ TRY (assume_tac (EVAL ``nss``) \\ fs[]
              \\ `5 * k' + 3 = 5 * k'' + 2` by
                   (qpat_x_assum `5 * k' + (num_stubs + 3) = _` mp_tac \\ decide_tac)
              \\ pop_assum mp_tac \\ intLib.ARITH_TAC \\ NO_TAC)
      \\ `FST e MOD nss = 3` by
           (`FST e = num_stubs + 3 + nss * k'` by fs[]
            \\ assume_tac (EVAL ``num_stubs``) \\ assume_tac (EVAL ``nss``) \\ fs[]
            \\ simp[arithmeticTheory.MOD_TIMES, Once arithmeticTheory.ADD_COMM])
      \\ drule compile_prog_avoids_nss_3
      \\ disch_then (qspec_then `FST e` mp_tac)
      \\ impl_tac >- fs[] \\ fs[])
  \\ strip_tac
  \\ reverse conj_tac >- fs[]
  \\ simp[EVERY_MEM] \\ rw[]
  ...
  \\ disch_then drule \\ strip_tac
  \\ TRY (fs[backend_commonTheory.data_num_stubs_def, backend_commonTheory.bvl_num_stubs_def] \\ NO_TAC)
  \\ drule (GEN_ALL bvi_tailrecProofTheory.compile_prog_MEM)
  \\ disch_then drule \\ strip_tac
  \\ fs[EVERY_MEM, backend_commonTheory.data_num_stubs_def, backend_commonTheory.bvl_num_stubs_def]
QED
...
Theorem ALL_DISTINCT_MAP_FST_SND_full_co:
   ...
   (FST (SND (SND (SND (FST (co n))))) MOD bvl_to_bvi_namespaces = 3) ∧
   (FST (FST (SND (SND (SND (SND (FST (co n)))))))
      MOD bvl_to_bvi_namespaces = 4) ∧
   bvl_num_stubs ≤ FST (FST (SND (SND (SND (SND (FST (co n)))))))
  ⇒
...
Proof
  ...
  >- (
    gvs []
    >> qpat_x_assum ‘Abbrev (TS = _)’
         (assume_tac o REWRITE_RULE [markerTheory.Abbrev_def])
    >> qmatch_asmsub_abbrev_tac ‘bvl_to_bvi$compile_inc N1 P1’
    >> namedCases_on ‘bvl_to_bvi$compile_inc N1 P1’ ["ci_n ci_p"]
    >> namedCases_on ‘bvi_tailrec$compile_prog c.do_tailrec
                        (FST (SND (SND (FST (co n))))) ci_p’ ["tr_n tr_p"]
    >> namedCases_on ‘bvi_tmc$compile_prog c.do_tmc
                        (FST (SND (SND (SND (FST (co n)))))) tr_p’ ["tm_n tm_p"]
    >> gvs []
    >> ‘∀x. MEM x (MAP FST ci_p) ∧ num_stubs ≤ x ⇒ ¬in_ns 4 x’
      by (
        rpt gen_tac >> strip_tac
        >> drule_all compile_inc_next_range >> rw [] >> gvs [in_ns_def])
    >> ‘∀x. MEM x (MAP FST TS) ∧ num_stubs ≤ x ⇒ ¬in_ns 4 x’
  ...
  >- (
    ...
    \\ rpt strip_tac \\ fs[EVAL ``nss``])
      ...
      \\ disch_then drule \\ strip_tac
      \\ `FST e MOD nss = 3` by
       (qpat_x_assum `M + k * nss = FST e` (assume_tac o GSYM)
        \\ asm_simp_tac std_ss [] \\ assume_tac (EVAL ``nss``)
        \\ simp[Once arithmeticTheory.ADD_COMM] \\ simp[arithmeticTheory.MOD_TIMES] \\ fs[])
      \\ TRY (
       `FST e MOD nss = 2` by
         (qpat_x_assum `FST e = m2 + k' * nss` assume_tac
          \\ asm_simp_tac std_ss [] \\ assume_tac (EVAL ``nss``)
          \\ simp[Once arithmeticTheory.ADD_COMM] \\ simp[arithmeticTheory.MOD_TIMES] \\ fs[])
       \\ fs[] \\ NO_TAC)
      \\ qpat_x_assum `Abbrev (xs = _)` mp_tac
      \\ qmatch_goalsub_abbrev_tac `compile_inc V P`
      \\ Cases_on `compile_inc V P` \\ strip_tac
      \\ `xs = r'''` by fs[markerTheory.Abbrev_def] \\ rveq
      \\ drule (GEN_ALL compile_inc_next_range)
      \\ disch_then drule
      \\ `FST e MOD nss <> 0 /\ FST e MOD nss <> 1` by fs[]
      \\ rw[] \\ fs[] \\ rfs[])
  \\ simp[]
QED
...
Theorem compile_get_code_labels:
   ...
   bvl_to_bvi$compile start c names prog =
     (loc,code,inlines,bvi_inlines,n1,n2,n3,n4,cm,names') ⇒
   BIGUNION (set (MAP (get_code_labels o SND o SND) code)) ⊆
     ...
     { num_stubs + 4 + k * nss | k | num_stubs + 4 + k * nss < n4 }
Proof
  ...
  \\ irule SUBSET_TRANS
  \\ first_assum (irule_at (Pos hd))
  \\ drule (GEN_ALL bvi_cprProofTheory.compile_prog_good_code_labels)
  \\ disch_then irule
  \\ simp []
  \\ reverse conj_tac
  ...
  \\ imp_res_tac bvi_tmcProofTheory.compile_prog_keeps_names
  \\ imp_res_tac bvi_cprProofTheory.compile_prog_keeps_names
  \\ fs [SUBSET_DEF]
  \\ metis_tac []
QED
