...
Proof
  ...
  \\ Cases_on`bs` \\ fs[]
  \\ simp[Once iEval_def,Once iEval_def,case_eq_thms]
  \\ simp[iEval_def,iEvalOp_def,do_app_aux_def,case_eq_thms]
  \\ simp[case_eq_thms,PULL_EXISTS,do_app_def,do_app_aux_def]
  \\ ntac 2 (simp[Once bEvalOp_def,backend_commonTheory.small_enough_int_def])
  \\ simp[bvl_to_bvi_id]
  ...
  ∀bs rest p (s:('c,'ffi) bviSem$state).
    ...
    ∃c.
      evaluate ([SND ToListByte_code],
        [list_to_v (MAP (\b. Word (w2v b)) rest);
         Number (& (LENGTH bs)); RefPtr p],inc_clock c s) =
        (Rval [list_to_v (MAP (\b. Word (w2v b)) bs ++
                          MAP (\b. Word (w2v b)) rest)],
...
val compile_string_thm = Q.prove(
  `∀str s ls vs.
   ...
   evaluate (MAPi (λn c. Op UpdateByte [Op (WordConst (fixwidth 8 (n2v (ORD c)))) []; compile_int (&(n + LENGTH vs)); Var 0]) str,
  ...
  \\ rw[evaluate_def,do_app_def,do_app_aux_def,backend_commonTheory.small_enough_int_def,v2w_fixwidth_n2v]
  \\ fs[compile_int_thm]
  \\ rw[bvlSemTheory.do_app_def]
  \\ simp[bvl_to_bvi_with_refs,bvl_to_bvi_id]
  \\ simp[case_eq_thms,pair_case_eq,v2w_fixwidth_n2v]
  ...
  \\ rw[]
  \\ POP_ASSUM(mp_tac)
  \\ qpat_abbrev_tac `X = MAPi _ str`
  \\ qpat_abbrev_tac `Y = MAPi _ str`
  \\ `X=Y` by (UNABBREV_ALL_TAC \\ rpt(ABS_TAC ORELSE MK_COMB_TAC \\ simp[]))
  \\ fs[]
  \\ rw[state_component_equality]
  \\ MK_COMB_TAC \\ simp[]
  \\ rw[LIST_EQ_REWRITE]
  \\ simp[EL_TAKE]
  \\ simp[EL_LUPDATE,EL_APPEND_EQN]
  \\ TOP_CASE_TAC \\ simp[]
...
val do_app_adjust = Q.prove(
  ...
  \\ Cases_on `op = UpdateByte` \\ fs [] THEN1
   (strip_tac
    \\ `?n i i'. REVERSE a = [RefPtr n; Number i; Word i']` by
  ...
  \\ TRY (rename1 `MAP (adjust_bv b2) (REVERSE a)` \\ Cases_on `REVERSE a` \\ fs[] \\ rename1 `h::t` \\ Cases_on `t` \\ fs[]
   ...
   \\ TRY (rename1 `MAP (adjust_bv b2) (REVERSE a)` \\ Cases_on `REVERSE a` \\ fs[] \\ rename1 `h::t` >> Cases_on `h` \\ fs[]
        ...
        \\ POP_ASSUM (ASSUME_TAC o GSYM) \\ fs[adjust_bv_def] \\ NO_TAC
      )
   \\ TRY (rename1 `MAP (adjust_bv b2) (REVERSE a)` \\ Cases_on `REVERSE a` \\ fs[] \\ rename1 `h::t` \\ Cases_on `h` \\ fs[]
    ...
    \\ TRY (rename1 `MAP (adjust_bv b2) (REVERSE a)` \\ Cases_on `REVERSE a` \\ fs[] \\ rename1 `h::t` \\ Cases_on `h` \\ fs[]
            ...
            >- (simp[bvl_to_bvi_id] \\ rename1 `q = adjust_bv b2 q` \\ Cases_on `q`
                ...
                \\ fs[Boolv_def])
            >> fs[])
    \\ TRY (rename1 `MAP (adjust_bv b2) (REVERSE a)` \\ Cases_on `REVERSE a` \\ fs[] \\ rename1 `REVERSE a = h::t`
        ...
        \\ rename1 `[] = l` \\ Cases_on `l` \\ fs[] \\ NO_TAC)
    \\ TRY (rename1 `deubug () = WordConst l` \\ Cases_on `REVERSE a` \\ fs[] \\ rveq \\ simp[adjust_bv_def,bvl_to_bvi_id])
    \\ TRY (rename1 `MAP (adjust_bv b2) (REVERSE a)` \\ Cases_on `REVERSE a` \\ fs[] \\ Cases_on `h` \\ fs[]
            \\ Cases_on `t` \\ fs[adjust_bv_def] \\ Cases_on `h` \\ fs[adjust_bv_def] \\ Cases_on `t'` \\ fs[]
            \\ Cases_on `LENGTH l = 64 ∧ LENGTH l' = 64` \\ fs[] \\ rveq \\ simp[bvl_to_bvi_id,adjust_bv_def] \\ NO_TAC)
    \\ TRY (rw[] \\ fs[case_eq_thms] \\ rfs[] \\ rw[adjust_bv_def] \\ NO_TAC)
    \\ TRY (rw[] \\ fs[case_eq_thms,PULL_EXISTS,adjust_bv_def,bvl_to_bvi_id]
           \\ rw[adjust_bv_def] \\ NO_TAC)
)
  ...
  THEN1 (* Op *)
   (note_tac "Op" \\
    ...
    \\ Cases_on`op = CopyByte T` \\ fs[] >- (
        ...
        \\ fs[SWAP_REVERSE_SYM]
        \\ imp_res_tac evaluate_IMP_LENGTH \\ fs[] \\ rw[]
        \\ simp[compile_op_def]
        ...
        by (
          fs[state_rel_def]
          \\ ntac 4 (first_x_assum(fn a => qspec_then`src`mp_tac a \\ qspec_then`src'`mp_tac a))
          \\ rw[]
          \\ fs[FLOOKUP_DEF]
          )) \\ fs[]
