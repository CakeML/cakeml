...
Proof
  ...
  >> rw []
  >> gvs [Once evaluate_CONS, evaluate_def]
  >> reverse $ Cases_on ‘h < LENGTH env’ >> gvs []
  >- (gvs [Once evaluate_CONS, evaluate_def, shift_vars_def])
  >> gvs [CaseEq "prod"]
  >> Cases_on ‘v3 = Rerr (Rabort Rtype_error)’
  >-
   (gvs [Once evaluate_CONS, evaluate_def, shift_vars_def]
    >> first_x_assum drule
...
Resume evaluate_rewrite_tmc[op_opt]:
  ...
  >> reverse $ Cases_on ‘v3' ≠ Rerr (Rabort Rtype_error)’
  >- (gvs [])
  >> gvs [CaseEq "prod"]
  >> rename [‘evaluate (MAP (λn. Var n) left,vs ++ env,u) = (Rval (MAP (λn. (vs ++ env)❲n❳) left),u)’]
  (* Inductive hypothesis on left *)
  >> last_assum $ qspecl_then [‘(MAP (λn. Var n) left’, ‘u’] mp_tac
  ...
  >-
   (gvs [rewrite_wrapper_def, dest_Cons_def, rewrite_wrapper_cons_def, cb_to_hb_def, CaseEq "prod", evaluate_def]
    ...
    >-
     (rw []
      >> simp [Once evaluate_def, hb_to_bvi_wrapper_def, allocate_holes_def, allocate_holes_aux_def]
      >> 
         )
                         
    >> drule evaluate_hb_to_bvi_wrapper
    >> disch_then $ qspecl_then [‘tag’, ‘left’, ‘child’, ‘right’, ‘hole’, ‘call_ts’, ‘call_args’, ‘loc_opt'’] mp_tac
    >> gvs [cb_to_hb_def]
    >> disch_then drule
    >> impl_tac >- (spose_not_then assume_tac >> gvs [])
    >> strip_tac
    >> qexists ‘t''’
    >> gvs []
    >> cheat (* Need a conclusion that implies state_rel *))
  >> cheat
...
Definition alloc_head_state_rel_def:
  alloc_head_state_rel tag left right env ptr val s1 s2 ⇔
    ptr ∉ FDOM s1.refs ∧
    ∃mut_block.
      mut_block = MutBlock tag (MAP (λn. EL n env) (REVERSE right)) val (MAP (λn. EL n env) (REVERSE left)) ∧
      s2 = s1 with refs := s1.refs |+ (ptr,mut_block)
End

Definition alloc_state_rel_def:
  (alloc_state_rel tag left Hole right env top_ptr hole_ptr hole_idx hole_val s1 s2 ⇔
     alloc_head_state_rel tag left right env top_ptr hole_val s1 s2 ∧
     hole_ptr = top_ptr ∧
     hole_idx = LENGTH right) ∧
  (alloc_state_rel tag left (HoleBlock tag' left' hole' right') right env top_ptr hole_ptr hole_idx hole_val s1 s2 ⇔
     ∃s3 child_ptr.
       alloc_head_state_rel tag left right env top_ptr (RefPtr F child_ptr) s1 s2 ∧
...
Proof
  ...
  >-
   (gvs [cb_to_hb_def, mut_cons_def]
    ...
    >> strip_tac >> gvs []
    (* call *)
    >> gvs [cb_to_bvi_def, optimise_call_def, evaluate_def, evaluate_APPEND, CaseEq "prod"]
    (* call args *)
    >> gvs [REVERSE_APPEND, TAKE_APPEND, DROP_APPEND, GSYM MAP_REVERSE, GSYM MAP_TAKE, GSYM MAP_DROP, DROP_LENGTH_TOO_LONG]
    >> ‘TAKE (LENGTH right) (REVERSE right) = REVERSE right’ by (gvs [LENGTH_REVERSE, TAKE_LENGTH_ID])
    >> simp [EL_APPEND_EQN]
  ...
  >> gvs [cb_to_hb_def]
  >> disch_then drule
  >> gvs [alloc_env_rel_def]
  >> disch_then $ qspecl_then [‘loc_opt’, ‘RefPtr F new_ptr::(env2' ++ [RefPtr F hole_ptr; Number hole_idx])’] mp_tac
  >> gvs []
  >> disch_then $ qspecl_then [‘1’, ‘0’, ‘t'’] mp_tac
  >> gvs []
  >> ‘new_ptr ∉ FDOM s.refs’ by cheat
  >> gvs [alloc_state_rel_def]
QED

Definition alloc_res_rel_def:
  alloc_res_rel refs r ⇔
...
Theorem evaluate_hb_to_mutcons:
  ∀cb tag left child right hb call_ts call_args loc env1 env2 s t r f c.
    ...
    r ≠ Rerr (Rabort Rtype_error) ⇒
    ∃t' env3 new_ptr.
      evaluate ([hb_to_mutcons hb],env2,s) = (Rval [RefPtr F new_ptr],t') ∧
      (*alloc_res_rel s.refs r' ∧*)
      env_rel T f env1 env3 ∧
      hole_has_val f env1 env3 t'.refs (RefPtr F new_ptr)
      (*f SUBMAP f' ∧*)
...
Definition alloc_env_rel_def:
  alloc_env_rel refs l i env1 env2 ⇔
    ∃zs new_ptr.
      env2 = zs ++ env1 ∧
      LENGTH zs = l ∧
      l > i ∧
      zs❲i❳ = RefPtr F new_ptr ∧
      new_ptr ∉ FDOM refs
End
