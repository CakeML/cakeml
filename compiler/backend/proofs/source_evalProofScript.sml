...
Proof
  ...
  \\ rw []
  \\ TRY (Cases_on ‘ys’ ORELSE Cases_on ‘xs’ \\ gs[terminationTheory.do_eq_def] \\ NO_TAC)
...
Theorem concrete_v_enc_real_cmp:
  !real_cmp. concrete_v (enc_real_cmp real_cmp)
Proof
  Cases
  \\ simp [enc_real_cmp_def]
QED

Theorem concrete_v_enc_real_uop:
  !real_uop. concrete_v (enc_real_uop real_uop)
Proof
  Cases
  \\ simp [enc_real_uop_def]
QED

Theorem concrete_v_enc_real_bop:
  !real_bop. concrete_v (enc_real_bop real_bop)
Proof
  Cases
  \\ simp [enc_real_bop_def]
QED
...
Proof
  ...
  \\ simp [enc_op_def, concrete_v_enc_opn, concrete_v_enc_opb,
    ...
    concrete_v_enc_fp_bop, concrete_v_enc_fp_top, concrete_v_enc_real_cmp,
    concrete_v_enc_real_uop, concrete_v_enc_real_bop ]
QED

Theorem concrete_v_enc_scope:
  ∀ sc. concrete_v (enc_scope sc)
Proof
  Cases \\ simp[enc_scope_def]
QED
...
Proof
  ...
  \\ simp [EVERY_MEM, PULL_EXISTS, concrete_v_enc_id, enc_pair_def,
    ...
    concrete_v_enc_op, concrete_v_enc_lop, concrete_v_enc_scope]
  \\ rw [] \\ res_tac
QED
...
Definition orac_extended_wf_def:
  orac_extended_wf (comp_f : compiler_fun) orac =
  (!i x. comp_f (orac i) = SOME x ==> FST x (FST (SND (orac (SUC i)))))
End
...
Theorem insert_do_eval:
  ...
  (recorded_orac_wf f (orac_s es).oracle ==>
    recorded_orac_wf f (orac_s es').oracle)
Proof
...
Theorem insert_declare_env:
  ...
  (recorded_orac_wf f (orac_s es1).oracle ==>
    recorded_orac_wf f (orac_s es2).oracle)
Proof
...
Proof
  ...
  >- (
    ...
    \\ fs [orac_agrees_def]
    \\ every_case_tac \\ fs []
    \\ first_x_assum (fn t => drule t
        \\ CHANGED_TAC (simp [recorded_orac_wf_def]))
    \\ rw []
    \\ first_x_assum drule
    \\ simp [ADD1]
  )
...
(* Creates extra oracle data like mk_eval_oracle, but without any
...
Proof
  ...
  \\ `is_record e.compiler e.compiler_state (SOME (EvalOracle orac_st))`
  ...
  \\ rpt strip_tac \\ fs []
  >- (
    drule_then irule mk_eval_oracle_agrees
    \\ asm_exists_tac
    \\ simp []
  )
  >- (
