...
val setup_tac = Cases_on`word_exp st exp`>>full_simp_tac(srw_ss())[]>>
      imp_res_tac apply_colour_exp_lemma>>
...
``∀prog st cst f live.
  ...
  >- (*Inst*)
    ...
    >-
      ...
      full_simp_tac(srw_ss())[word_exp_def,set_var_def,strong_locals_rel_def,get_writes_def
    ...
    >-
      ...
      >>
      full_simp_tac(srw_ss())[apply_colour_exp_def,word_state_eq_rel_def]>>
      full_simp_tac(srw_ss())[set_var_def,strong_locals_rel_def,lookup_insert,get_writes_def
        ,get_writes_inst_def]>>
      srw_tac[][]>>
    ...
    >-
      ...
      Cases_on`lookup n0 st.locals`>>full_simp_tac(srw_ss())[strong_locals_rel_def]>>
      res_tac>>
      full_simp_tac(srw_ss())[lookup_insert]>>
      srw_tac[][]>>
      Cases_on`n=n'`>>full_simp_tac(srw_ss())[]>>
      `f n' ≠ f n` by
        (full_simp_tac(srw_ss())[domain_union,get_writes_inst_def,get_writes_def]>>
        FULL_SIMP_TAC bool_ss [INJ_DEF]>>
        first_x_assum(qspecl_then[`n'`,`n`]mp_tac)>>
        discharge_hyps>-srw_tac[][]>>
        metis_tac[])>>
      full_simp_tac(srw_ss())[])
    >-
  ...
  >- (*Assign*)
    ...
    full_simp_tac(srw_ss())[strong_locals_rel_def]>>srw_tac[][]>>
    full_simp_tac(srw_ss())[lookup_insert]>>Cases_on`n=n'`>>full_simp_tac(srw_ss())[get_writes_def]>>
    `f n' ≠ f n` by
      (FULL_SIMP_TAC bool_ss [INJ_DEF]>>
      first_x_assum(qspecl_then [`n`,`n'`] mp_tac)>>
      srw_tac[][domain_union,domain_delete])>>
    full_simp_tac(srw_ss())[domain_union])
  >- (*Get*)
    (exists_tac>>
    EVERY_CASE_TAC>>
    full_simp_tac(srw_ss())[colouring_ok_def,set_var_def,strong_locals_rel_def,get_live_def]>>
    full_simp_tac(srw_ss())[LET_THM,get_writes_def]>>srw_tac[][]>>
    full_simp_tac(srw_ss())[lookup_insert]>>Cases_on`n'=n`>>full_simp_tac(srw_ss())[]>>
    `f n' ≠ f n` by
  ...
  >- (*MustTerminate*)
    ...
    discharge_hyps>- full_simp_tac(srw_ss())[strong_locals_rel_def,get_live_def]>>
    strip_tac>>
  ...
  >- (*Alloc*)
    ...
    >> (* FFI *)
      ...
      imp_res_tac strong_locals_rel_get_var>>full_simp_tac(srw_ss())[]>>
      Cases_on`cut_env s st.locals`>>full_simp_tac(srw_ss())[]>>
      `domain s ⊆ (n0 INSERT n1 INSERT domain s)` by full_simp_tac(srw_ss())[SUBSET_DEF]>>
      imp_res_tac strong_locals_rel_subset>>
      imp_res_tac cut_env_lemma>>
      pop_assum mp_tac >> discharge_hyps>-
        (match_mp_tac (GEN_ALL INJ_less)>>metis_tac[])>>
      srw_tac[][]>>FULL_CASE_TAC>>full_simp_tac(srw_ss())[]>>
      Cases_on`call_FFI st.ffi n x'`>>full_simp_tac(srw_ss())[strong_locals_rel_def]>>
      srw_tac[][]>>
      metis_tac[domain_lookup]));
  ...
  >-
    ...
    >>
      ...
      >-
        ...
        discharge_hyps>- (full_simp_tac(srw_ss())[prog_size_def]>>DECIDE_TAC)>>
        ...
        TRY(qexists_tac`B`>>full_simp_tac(srw_ss())[Abbr`B`]) >>
      ...
      >>
        ...
        TRY(qexists_tac`A`>>
          full_simp_tac(srw_ss())[Abbr`A`,domain_union,domain_numset_list_insert]>>NO_TAC)>>
        TRY(qexists_tac`B`>>full_simp_tac(srw_ss())[Abbr`B`]>>NO_TAC) >>
...
val clash_tree_colouring_ok = store_thm("clash_tree_colouring_ok",``
  ...
  CONJ_TAC>-
    fs[numset_list_delete_def,check_partial_col_def]>>
  CONJ_TAC>-
    (ntac 5 strip_tac>>TOP_CASE_TAC>>fs[numset_list_delete_def]>>strip_tac>>
    imp_res_tac check_partial_col_INJ>>fs[numset_list_delete_def,numset_list_insert_def,check_partial_col_def]>>
    fs[GSYM domain_insert_eq_union]>>
    rw[]>-
      (match_mp_tac (GEN_ALL INJ_less)>>
      HINT_EXISTS_TAC>>fs[])>>
    ...
    >>
      ...
      disch_then kall_tac>>
      strip_tac>>
      first_x_assum(qspecl_then[`x''`,`r`] mp_tac)>>
      rw[]>>
      fs[])>>
  CONJ_TAC>-
    cheat>>
...
val size_tac = discharge_hyps>- (full_simp_tac(srw_ss())[prog_size_def]>>DECIDE_TAC)
...
val mov_eval_head = prove(``
  ...
  qpat_assum `A=rst` (sym_sub_tac)>>full_simp_tac(srw_ss())[])
...
val get_vars_exists = prove(``
  ...
  Induct>>full_simp_tac(srw_ss())[get_var_def,get_vars_def]>>srw_tac[][]>>
  full_simp_tac(srw_ss())[domain_lookup])
...
fun fcs t r = Cases_on t>>Cases_on r>>full_simp_tac(srw_ss())[]
...
``∀prog st cst ssa na.
  ...
  >- (*Call*)
   ...
   Cases_on`o'`
    ...
    >-
    (*No handler*)
    (qpat_assum`A=pp0` (sym_sub_tac)>>full_simp_tac(srw_ss())[Abbr`prog`]>>
    qpat_assum`A=stack_mov` (sym_sub_tac)>>full_simp_tac(srw_ss())[]>>
    ...
    discharge_hyps>-
      ...
      >>
        ...
        res_tac>>
        full_simp_tac(srw_ss())[]>>
        qpat_assum`A=SOME v` SUBST_ALL_TAC>>full_simp_tac(srw_ss())[])
    >>
    ...
    >>
      ...
      srw_tac[][]>>qexists_tac`perm`>>full_simp_tac(srw_ss())[]>>
      pop_assum(qspec_then`envy.stack` mp_tac)>>
      (discharge_hyps>- (unabbrev_all_tac>>full_simp_tac(srw_ss())[]))>>
      srw_tac[][]>>full_simp_tac(srw_ss())[])
  >>
    (*Handler reasoning*)
    qpat_assum`A=(pp0,pp1,pp2)` mp_tac>>PairCases_on`x''`>>full_simp_tac(srw_ss())[]>>
    LET_ELIM_TAC>>
    rev_full_simp_tac(srw_ss())[]>>
    qpat_assum`A=pp0` (sym_sub_tac)>>full_simp_tac(srw_ss())[Abbr`prog'`]>>
    qpat_assum`A=stack_mov` (sym_sub_tac)>>full_simp_tac(srw_ss())[]>>
  ...
  >- (*Seq*)
    ...
    size_tac>>
    discharge_hyps>>full_simp_tac(srw_ss())[every_var_def]>>srw_tac[][]>>
  ...
  >- (*If*)
   (qpat_abbrev_tac `A = ssa_cc_trans B C D` >>
    ...
    qpat_assum`B = A0` sym_sub_tac>>full_simp_tac(srw_ss())[evaluate_def]>>
    Cases_on`get_var n st`>>full_simp_tac(srw_ss())[]>>
    Cases_on`x`>>full_simp_tac(srw_ss())[]>>
    Cases_on`get_var_imm r st`>>full_simp_tac(srw_ss())[]>>
    Cases_on`x`>>full_simp_tac(srw_ss())[]>>
  ...
  >>
    ...
    Cases_on`call_FFI st.ffi n x'`>>full_simp_tac(srw_ss())[]>>
    Q.SPECL_THEN [`st`,`ssa`,`na+2`,`ls`,`cst`] mp_tac list_next_var_rename_move_preserve>>
    discharge_hyps_keep>-
...
val fake_moves_conventions = prove(``
  ...
  full_simp_tac(srw_ss())[call_arg_convention_def,every_stack_var_def,fake_moves_def])
...
val fix_inconsistencies_conventions = prove(``
  ...
  call_arg_convention a ∧
  call_arg_convention b``,
  full_simp_tac(srw_ss())[fix_inconsistencies_def,call_arg_convention_def,every_stack_var_def,UNCURRY]>>
  rpt strip_tac>>
...
``∀prog ssa na.
  ...
  (size_tac>>discharge_hyps>-full_simp_tac(srw_ss())[])>>
  strip_tac>>
...
val oracle_colour_ok_conventions = prove(``
  ...
  full_simp_tac(srw_ss())[oracle_colour_ok_def,LET_THM]>>EVERY_CASE_TAC>>full_simp_tac(srw_ss())[]>>
  metis_tac[])
...
val pre_post_conventions_word_alloc = prove(``
  ...
  full_simp_tac(srw_ss())[pre_alloc_conventions_def,post_alloc_conventions_def,word_alloc_def]>>
  srw_tac[][]>>
  FULL_CASE_TAC>>full_simp_tac(srw_ss())[]>>
  imp_res_tac oracle_colour_ok_conventions >>full_simp_tac(srw_ss())[post_alloc_conventions_def]>>
...
val word_alloc_correct = store_thm("word_alloc_correct",``
  ...
  >-
    (imp_res_tac oracle_colour_ok_correct>>full_simp_tac(srw_ss())[LET_THM,Abbr`cprog`]>>
    qexists_tac`perm'`>>srw_tac[][]>>full_simp_tac(srw_ss())[])
  >>
  ...
  discharge_hyps>-
    ...
    >-
      ...
      full_simp_tac(srw_ss())[clash_sets_to_sp_g_undir])
    >-
      full_simp_tac(srw_ss())[word_state_eq_rel_def]
    >>
      ...
      discharge_hyps>-
        (srw_tac[][Abbr`clash_graph`]>>full_simp_tac(srw_ss())[clash_sets_to_sp_g_undir])
      >>
...
val ssa_cc_trans_distinct_tar_reg = prove(``
  ...
  >-
    ...
    full_simp_tac(srw_ss())[ssa_cc_trans_inst_def,LET_THM,next_var_rename_def,every_var_def,every_var_inst_def,every_var_imm_def]>>
  ...
  >-
    full_simp_tac(srw_ss())[every_var_def]
  >-
    (full_simp_tac(srw_ss())[every_var_def]>>qpat_assum`A = (B,C,D,E)`mp_tac>>full_simp_tac(srw_ss())[fix_inconsistencies_def,fake_moves_def]>>LET_ELIM_TAC>>
