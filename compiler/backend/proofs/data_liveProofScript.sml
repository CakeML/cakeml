...
val is_pure_do_app_Rerr_IMP = Q.prove(
  `is_pure op asize /\ do_app op xs s = Rerr e ==>
...
val small_alloc = Q.prove(`!n. 0 < asize-2 ==> is_small asize n ==> alloc_size n asize = 0`,
...
val is_pure_do_app_Rval_IMP = Q.prove(
  `is_pure op (arch_size s.limits) /\ do_app op x s = Rval (q,r)
  ...
  \\ rw[] \\ fs [state_component_equality,is_pure_def
                ...
                ,do_stack_def]
  \\ `0 < arch_size s.limits - 2` by (rw[arch_size_def])
  \\ metis_tac[small_alloc]);

val evaluate_compile = Q.prove(
  `!c s1 res s2 l2 t1 l1 d.
      (evaluate (c,s1) = (res,s2)) /\ state_rel s1 t1 l1 /\
      (compile c l2 (arch_size s1.limits) = (d,l1)) /\ (res <> SOME (Rerr (Rabort Rtype_error))) /\
  ...
  THEN1 (* Assign *)
    (Cases_on `names_opt` THEN1
      (fs [compile_def]
       \\ Cases_on `lookup dest l2 = NONE ∧ is_pure op (arch_size s.limits)` \\ fs []
       THEN1
  ...
  THEN1 (* Seq *)
   (fs [evaluate_def]
    ...
    \\ `arch_size u1.limits = arch_size s.limits` by
    metis_tac[evaluate_preserves_arch_size]
    \\ `arch_size u2.limits = arch_size s.limits` by
    metis_tac[evaluate_preserves_arch_size]
    \\ `?x2 l5. compile c2 l2 (arch_size s.limits) = (x2,l5)` by METIS_TAC [PAIR]
    \\ `?x1 l6. compile c1 l5 (arch_size s.limits) = (x1,l6)` by METIS_TAC [PAIR]
  ...
  THEN1 (* If *)
   (Q.ABBREV_TAC `l9 = l2` \\ POP_ASSUM (K ALL_TAC)
    \\ `?d3 l3. compile c2 l9 (arch_size s.limits) = (d3,l3)` by METIS_TAC [PAIR]
    \\ `?d2 l2. compile c1 l9 (arch_size s.limits) = (d2,l2)` by METIS_TAC [PAIR]
  ...
  \\ `?d6 l6. compile handle l2 (arch_size s.limits) = (d6,l6)` by METIS_TAC [PAIR]
  \\ fs [compile_def,LET_DEF,evaluate_def]
  \\ `t1.clock = s.clock /\ t1.code = s.code` by fs [state_rel_def]
  \\ Cases_on `get_vars args s.locals` \\ fs []
  \\ IMP_RES_TAC state_rel_IMP_get_vars \\ fs []
  ...
  \\ `arch_size r.limits = arch_size s.limits` by(
      imp_res_tac evaluate_preserves_arch_size
      \\ pop_assum mp_tac \\ simp[call_env_def]
  )
  \\ fs[]
...
Theorem compile_correct:
   !c s. FST (evaluate (c,s)) <> SOME (Rerr(Rabort Rtype_error)) /\
         FST (evaluate (c,s)) <> NONE ==>
           ∃ls sm safe peak.
             evaluate (FST (compile c LN (arch_size s.limits)),s) =
...
Proof
  ...
  \\ Cases_on `evaluate (FST (compile c LN (arch_size s.limits)),s)` \\ fs []
  ...
  \\ (Q.ISPECL_THEN [`FST (compile c LN (arch_size s.limits))`,`s`]mp_tac evaluate_stack)
  \\ fs [] \\ Cases_on `x` \\ fs []
