...
val state_rel_IMP_do_app = Q.prove(
  `(do_app op args arch_size s1 = Rval (v,s2)) /\
    state_rel s1 t1 anything ==>
    (s1.handler = s2.handler) /\ (s1.stack = s2.stack) /\
    (∃safe. do_app op args arch_size t1 = Rval (v,s2 with <| locals := t1.locals ;
                                                   stack := t1.stack ;
                                                   handler := t1.handler ;
                                                   safe_for_space := safe|>))`,
  STRIP_TAC
...
val state_rel_IMP_do_app_err = Q.prove(
  `(do_app op args arch_size s1 = Rerr e) /\ state_rel s1 t1 anything ==>
    (do_app op args arch_size t1 = Rerr e)`,
  STRIP_TAC
...
val is_pure_do_app_Rerr_IMP = Q.prove(
  `is_pure op arch_size /\ do_app op xs arch_size s = Rerr e ==>
...
val is_pure_do_app_Rval_IMP = Q.prove(
  `0 < arch_size - 2 /\ is_pure op arch_size /\ do_app op x arch_size s = Rval (q,r) ==> r = s`,
  Cases_on `op` \\ TRY(fs [is_pure_def,do_app_def,do_app_aux_def]
  ...
  \\ rw[] \\ fs [state_component_equality,is_pure_def,data_spaceTheory.op_space_req_def] >> NO_TAC)
  \\ fs[is_pure_def,do_app_def,do_app_aux_def,do_space_def,dataLangTheory.op_space_reset_def,data_spaceTheory.op_space_req_def]
  \\ BasicProvers.TOP_CASE_TAC
  \\ STRIP_TAC
  \\ Cases_on `0 < arch_size -2`
   \\ fs[small_alloc]
   \\ fs[case_eq_thms]
  );
...
val evaluate_compile = Q.prove(
  `!c s1 arch_size res s2 l2 t1 l1 d.
      ...
      ?t2. (evaluate (d,t1) arch_size = (res,t2)) /\
  ...
  THEN1 (* Assign *)
    (Cases_on `names_opt` THEN1
      (fs [compile_def]
       \\ Cases_on `lookup dest l2 = NONE ∧ is_pure op arch_size` \\ fs []
       THEN1
     ...
     \\ Q.MATCH_ASSUM_RENAME_TAC `do_app op vs arch_size t = _`
  ...
  THEN1 (* If *)
   (Q.ABBREV_TAC `l9 = l2` \\ POP_ASSUM (K ALL_TAC)
    ...
    \\ `?d2 l2. compile c1 l9 arch_size = (d2,l2)` by METIS_TAC [PAIR]
    ...
    \\ Cases_on `isBool T x` \\ fs [] THEN1
     (Q.PAT_X_ASSUM `xxx = evaluate (c1,s) arch_size` (ASSUME_TAC o GSYM) \\ fs []
    ...
    \\ Cases_on `isBool F x` \\ fs [] THEN1
     (Q.PAT_X_ASSUM `xxx = evaluate (c2,s) arch_size` (ASSUME_TAC o GSYM) \\ fs []
      \\ FIRST_X_ASSUM (MP_TAC o Q.SPECL [`l9`,`t1`]) \\ fs []
      \\ MATCH_MP_TAC IMP_IMP \\ STRIP_TAC
      \\ ONCE_REWRITE_TAC [EQ_SYM_EQ] \\ REPEAT STRIP_TAC \\ fs []
      \\ fs [state_rel_def,domain_union]))
  ...
  THEN1 (* Call with ret = NONE *)
   (`s.clock = t1.clock /\ s.code = t1.code` by fs [state_rel_def]
    ...
    \\ Cases_on `evaluate (r,call_env q (dec_clock s)) arch_size` \\ fs []
    ...
    \\ fs [] \\ Cases_on `res2` \\ fs [] THEN1
     (fs [call_env_def,dec_clock_def] \\ REPEAT STRIP_TAC
      ...
      \\ qmatch_asmsub_abbrev_tac  `evaluate (r,p) _ = (SOME _, ss)`
      \\ qmatch_goalsub_abbrev_tac `evaluate (r, ss') _`
  ...
  \\ `?d6 l6. compile handle l2 arch_size = (d6,l6)` by METIS_TAC [PAIR]
  \\ fs [compile_def,LET_DEF,evaluate_def]
  \\ `t1.clock = s.clock /\ t1.code = s.code` by fs [state_rel_def]
  \\ Cases_on `get_vars args s.locals` \\ fs []
  \\ IMP_RES_TAC state_rel_IMP_get_vars \\ fs []
...
Theorem compile_correct:
   !c s. 0 < arch_size - 2 /\ FST (evaluate (c,s) arch_size) <> SOME (Rerr(Rabort Rtype_error)) /\
         FST (evaluate (c,s) arch_size) <> NONE ==>
           ∃safe. evaluate (FST (compile c LN arch_size),s) arch_size = (I ##
           λs. s with safe_for_space := safe) (evaluate (c,s) arch_size)
Proof
  REPEAT STRIP_TAC
  ...
  \\ fs [] \\ REPEAT STRIP_TAC
  \\ Cases_on `evaluate (c,s) arch_size` \\ fs []
  \\ Cases_on `evaluate (FST (compile c LN arch_size),s) arch_size` \\ fs []
