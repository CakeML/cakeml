...
val code_rel_def = Define `
  code_rel (bvi_code : (num # bvi$exp) num_map)
           (data_code : (num # dataLang$prog) num_map) arch_size <=>
...
val state_rel_def = Define `
  state_rel (s:('c,'ffi) bviSem$state) (t:('c,'ffi) dataSem$state) arch_size <=>
    (s.clock = t.clock) /\
    code_rel s.code t.code arch_size /\
...
Theorem optimise_correct:
   !c s. 0<arch_size-2 /\ FST (evaluate (c,s) arch_size) <> SOME (Rerr(Rabort Rtype_error)) /\
         ...
         ∃safe. evaluate (optimise arch_size c,s) arch_size = (I ## λx. x with safe_for_space :=
         safe) (evaluate (c,s) arch_size)
Proof
  fs[optimise_def] \\ REPEAT STRIP_TAC \\ Cases_on `evaluate (c,s) arch_size` \\ fs[]
  \\ qspecl_then [`c`,`s`] ASSUME_TAC data_liveProofTheory.compile_correct   \\ rfs []
  \\ qspecl_then [`FST (compile c LN arch_size)`,`s`] (ASSUME_TAC o GSYM) simp_correct \\ rfs []
  \\ pop_assum (ASSUME_TAC o GSYM)
  \\ qspecl_then [`simp (FST (compile c LN arch_size)) Skip`,`s`] ASSUME_TAC data_spaceProofTheory.compile_correct
  \\ rfs [] \\ METIS_TAC []
QED
...
Theorem state_rel_safe:
  ∀s s' arch_size safe. state_rel s s' arch_size = state_rel s (s' with safe_for_space :=
  safe) arch_size
Proof
  rw [state_rel_def]
QED

Theorem jump_exc_safe:
  ∀s t safe. jump_exc s = SOME t
    ⇒ jump_exc (s with safe_for_space := safe) = SOME (t with safe_for_space := safe)
Proof
...
val compile_correct = Q.prove(
   ...
   ( `?c1 vs n1. compile arch_size n corr F live xs = (c1,vs,n1)` by METIS_TAC [PAIR]
    ...
    \\ `var_corr (REVERSE a) (REVERSE vs) (map data_to_bvi_v env1)` by
     (UNABBREV_ALL_TAC
      ...
      \\ full_simp_tac(srw_ss())[EVERY2_REVERSE])
    \\ IMP_RES_TAC get_vars_thm
    \\ IMP_RES_TAC get_vars_lift_thm
    \\ `state_rel r (t2 with <|locals := env1; space := 0|>) arch_size` by
    ...
    >- (
      ...
      >- (
        ...
        \\ fs[compile_prog_def] )
      \\ `r.compile = λcfg prog. t2.compile cfg (compile_prog arch_size prog)` by fs[state_rel_def]
      \\ `t2.compile_oracle = (I ## (compile_prog arch_size)) o r.compile_oracle` by fs[state_rel_def]
    ...
    \\ Cases_on `op = Greater \/ op = GreaterEq` THEN1
     (fs []
      ...
      \\ fs [EVAL ``dataSem$do_app Less [Number i'; Number i] arch_size t``]
      \\ fs [EVAL ``dataSem$do_app LessEq [Number i'; Number i] arch_size t``]
    ...
    THEN1
     (`op_space_reset op` by
      ...
      \\ `state_rel r (t2 with <|locals := env1; space := 0; safe_for_space :=
      SAFE0 |>) arch_size`
         by fs[state_rel_def]
    ...
    \\ Cases_on `op_space_req op (LENGTH vs) arch_size = 0`
    ...
    \\ IMP_RES_TAC compile_LESS_EQ \\ full_simp_tac(srw_ss())[lookup_insert]
    THEN1
     (`state_rel r t2 arch_size` by fs[state_rel_def]
     \\ first_assum (mp_then Any mp_tac data_to_bvi_do_app)
     \\ rpt (disch_then (first_assum o mp_then Any mp_tac))
     \\ rw []
     \\ rfs [dataLangTheory.op_space_reset_def
     ...
     THEN1 (full_simp_tac(srw_ss())[LIST_REL_EL_EQN,
      ...
      THEN1 (POP_ASSUM MP_TAC \\ Cases_on `jump_exc t1` \\ full_simp_tac(srw_ss())[]
         ...
         \\ full_simp_tac(srw_ss())[jump_exc_def])
      \\ full_simp_tac(srw_ss())[var_corr_def,get_var_def,lookup_map])
    \\ qspecl_then [`t2`,`op_space_req op (LENGTH vs) arch_size`,`env1`]
         (CHOOSE_THEN (fn t => fs [t,lookup_map]))
           (GEN_ALL consume_space_add_space)
    THEN1
     (`state_rel r (t2 with <|locals := env1; space := 0; safe_for_space := sf|>) arch_size` by fs[state_rel_def]
     \\ first_assum (mp_then Any mp_tac data_to_bvi_do_app)
     \\ rpt (disch_then (first_assum o mp_then Any mp_tac))
     \\ rw []
     \\ rfs [dataLangTheory.op_space_reset_def
     ...
     \\ REPEAT STRIP_TAC
      ...
      \\ full_simp_tac(srw_ss())[var_corr_def,get_var_def,lookup_map]))
  THEN1 (* Tick *)
   (`?c1 v1 n1. compile arch_size n corr tail live [x] = (c1,v1,n1)` by METIS_TAC [PAIR]
    \\ FULL_SIMP_TAC std_ss [LET_DEF,evaluate_def]
    ...
    \\ FIRST_X_ASSUM (MP_TAC o Q.SPECL [`arch_size`,`dec_clock t1`,`n`,`corr`,`tail`,`live`])
    \\ FULL_SIMP_TAC std_ss []
    \\ MATCH_MP_TAC IMP_IMP \\ STRIP_TAC
  ...
  THEN1 (* Call *)
   ( Cases_on `handler` THEN1 (* Call without handler *)
     (`?c1 vs n1. compile arch_size n corr F live xs = (c1,vs,n1)` by METIS_TAC [PAIR]
      ...
      \\ Cases_on `tail` THEN1
       (`evaluate ([exp],MAP data_to_bvi_v args',dec_clock (ticks + 1) r) = (res,s2)` by
        ...
        \\ FIRST_X_ASSUM (qspecl_then
             [`arch_size`,`call_env args' (FUNPOW dec_clock (ticks+1) t2)`,
               `LENGTH args'`,
        ...
        \\ REV_FULL_SIMP_TAC (srw_ss()) [FUNPOW_dec_clock_code,GSYM state_rel_safe]
        \\ DISCH_THEN (fn t => fs [t])
        \\ drule_then (qspec_then `safe` ASSUME_TAC) jump_exc_safe
        \\ fs [state_fupdcanon])
      ...
      THEN1
       (Cases_on`e`>>full_simp_tac(srw_ss())[data_to_bvi_result_def]>>
        ...
        \\ full_simp_tac(srw_ss())[FUNPOW_dec_clock_code,GSYM state_rel_safe]
      ...
      \\ `pop_env (t2' with safe_for_space := safe) = SOME (t2' with
         <| stack := t2.stack; locals := env2 ; safe_for_space := safe|>)` by
    ...
    \\ (* Call with handle *)
      `?c1 vs n1. compile arch_size n corr F live xs = (c1,vs,n1)` by METIS_TAC [PAIR]
      \\ `?c2 v n2. compile arch_size (n1+1) (n1::corr) F live [x] = (c2,v,n2)` by
            METIS_TAC [PAIR]
      ...
      \\ Cases_on `isException res4` THEN1
       (Cases_on `res4` \\ full_simp_tac(srw_ss())[LET_DEF] \\ full_simp_tac(srw_ss())[]
        ...
        \\ fs [] \\ rveq
        \\ Cases_on `evaluate (c2,set_var n1 z' t2') arch_size` \\ full_simp_tac(srw_ss())[]
        \\ FIRST_X_ASSUM (MP_TAC o Q.SPECL
              [`arch_size`,`set_var n1 z' t2'`,`n1+1`,`n1::corr`,`F`,`live`]) \\ full_simp_tac(srw_ss())[]
        \\ MATCH_MP_TAC IMP_IMP \\ STRIP_TAC THEN1
        ...
        \\ reverse (Cases_on `q`) \\ full_simp_tac(srw_ss())[] THEN1
         ...
         \\ `set_var n1 z' t2' with safe_for_space := safe = set_var n1 z' (t2' with safe_for_space := safe)`
            by rw [set_var_def]
         \\ fs [GSYM state_rel_safe]
         \\ disch_then (fn t => fs [t])
         \\ qmatch_goalsub_abbrev_tac `state_safe_for_space_fupd (K SAFE0) _`
         \\ drule_then (qspec_then `SAFE0` ASSUME_TAC) jump_exc_safe
         \\ fs [state_fupdcanon]
        ...
        \\ `(t2'.stack = t2.stack) /\ (t2'.handler = t2.handler)` by
         (REPEAT STRIP_TAC \\ full_simp_tac(srw_ss())[set_var_def,jump_exc_def,call_env_def,
          ...
          \\ full_simp_tac(srw_ss())[jump_exc_def,LASTN_LENGTH_ID |> Q.SPEC `x::xs` |> RW [LENGTH,ADD1]]
          \\ full_simp_tac(srw_ss())[dataSemTheory.state_component_equality])
        \\ qpat_x_assum `evaluate (c2, _) _ = (_,r')` (mp_then Any (qspec_then `safe` ASSUME_TAC) evaluate_safe_swap)
...
Proof
  ...
  \\ Q.EXISTS_TAC `t2 with safe_for_space := safe`
  \\ Q.EXISTS_TAC `r`
  \\ rw [GSYM state_rel_safe]
QED

val state_rel_dec_clock = Q.prove(
  `state_rel s1 t1 arch_size ⇒ state_rel (dec_clock 1 s1) (dec_clock t1) arch_size`,
  srw_tac[][state_rel_def,bviSemTheory.dec_clock_def,dataSemTheory.dec_clock_def])

Theorem compile_part_evaluate:
   0 < arch_size - 2 ∧ evaluate ([Call 0 (SOME start) [] NONE],[],s1) = (res,s2) ∧
   res ≠ Rerr (Rabort Rtype_error) ∧ state_rel s1 t1 arch_size ∧
   isEmpty t1.locals ∧ (∀x. res = Rerr (Rraise x) ⇒ jump_exc t1 ≠ NONE)
   ⇒ ∃r t2.
      evaluate ((Call NONE (SOME start) [] NONE),t1) arch_size = (SOME r,t2) ∧
      state_rel s2 t2 arch_size ∧ res_list (data_to_bvi_result r) = res
Proof
  ...
  \\ PairCases_on`p`
  \\ `lookup start t1.code = SOME (p0,compile_exp arch_size p0 p1)`
  ...
  \\ full_simp_tac(srw_ss())[]
  \\ mp_tac (GEN_ALL compile_exp_correct)
  \\ QUANT_TAC [("arch_size", `arch_size`, []),("res",`res`,[])]
  ...
  \\ `(∀v. res = Rerr (Rraise v) ⇒ jump_exc (dec_clock t1) ≠ NONE)` by (
     simp[lookup_def,dataSemTheory.dec_clock_def]
     \\ full_simp_tac(srw_ss())[jump_exc_def]
     \\ every_case_tac
  ) \\ fs[]
  \\ `!n. (∀k. n <= k ⇒ lookup k (dec_clock t1).locals = NONE)` by simp[lookup_def,dataSemTheory.dec_clock_def]
  \\ fs[]
  \\ disch_then(mp_tac o Q.SPEC `s2`)
  \\ QUANT_TAC [("env",`[]`,[])]
  \\ QUANT_TAC [("exp'",`p1`,[])]
  \\ fs[]
  \\ `v3 = res` by (Cases_on `v3` \\ fs[] \\ Cases_on `e` \\ fs[])
  \\ fs[]
  \\ `s' = s2` by (Cases_on `res` \\ fs[] \\ Cases_on `e` \\ fs[])
  \\ fs[]
  \\ disch_then(mp_tac o Q.SPEC `0`) \\ fs[]
  \\ first_x_assum (assume_tac o Q.SPEC `0`) \\ fs[]
  \\ `call_env [] (dec_clock t1) = dec_clock t1` by(simp[call_env_def,fromList_def] \\ EVAL_TAC >> simp[dataSemTheory.state_component_equality])
  \\ fs[]
  \\ every_case_tac \\ fs[]
QED
...
Theorem compile_prog_evaluate:
   0 < arch_size - 2 ∧ evaluate ([Call 0 (SOME start) [] NONE],[],
     initial_state ffi0 (fromAList prog) co (λcfg prog. cc cfg (compile_prog arch_size prog)) k) = (r,s) ∧
   r ≠ Rerr (Rabort Rtype_error) ∧ (∀x. r ≠ Rerr (Rraise x))
   ⇒  ∃r2 s2.
       evaluate (Call NONE (SOME start) [] NONE,
         initial_state ffi0 (fromAList (compile_prog arch_size prog)) ((I ##
         compile_prog arch_size) o co) cc T 0 0 k) arch_size = (SOME r2,s2) ∧
         state_rel s s2 arch_size ∧ res_list (data_to_bvi_result r2) = r
Proof
  srw_tac[][]
  \\ match_mp_tac (GEN_ALL compile_part_evaluate) \\ fs[]
  \\ asm_exists_tac >> simp[]
...
Theorem FST_compile_part[simp]:
   FST (compile_part arch_size a) = (FST a)
Proof
...
Theorem compile_prog_semantics:
  (0 < arch_size - 2 /\
  semantics (ffi0:'ffi ffi_state) (fromAList prog) co (λcfg prog. cc cfg
  (compile_prog arch_size prog)) start ≠ Fail) ⇒
  semantics ffi0 (fromAList (compile_prog arch_size prog)) ((I ## compile_prog
  arch_size) o co) cc start arch_size =
  semantics ffi0 (fromAList prog) co (λcfg prog. cc cfg (compile_prog arch_size prog)) start
Proof
  ...
  >- (qx_gen_tac`res`>>srw_tac[][]
     ...
     >- (gen_tac >> strip_tac >> rveq >> simp[]
        ...
        \\ qmatch_assum_abbrev_tac`dataSem$evaluate (bxps,bs) _ = _`
        \\ qspecl_then[`exps`,`[]`,`ss`]mp_tac
        (INST_TYPE[beta|->``:'ffi``]bviPropsTheory.evaluate_add_to_clock_io_events_mono)
        \\ Q.ISPECL_THEN[`bxps`,`bs`,`arch_size`]mp_tac dataPropsTheory.evaluate_add_clock_io_events_mono
        ...
        \\ qpat_x_assum `evaluate _ _ = (SOME r',s')` assume_tac
 ...
 >- (last_x_assum(qspec_then`k`mp_tac)
     ...
     \\ impl_tac
      >- (simp[] \\ conj_tac >> spose_not_then strip_assume_tac >> full_simp_tac(srw_ss())[])
     \\ strip_tac
     \\ UNABBREV_ALL_TAC
 ...
 \\ PURE_ASM_REWRITE_TAC[]
 \\ impl_tac
 >- (UNABBREV_ALL_TAC \\ simp[] \\ conj_tac >> spose_not_then strip_assume_tac >> full_simp_tac(srw_ss())[]
    \\ last_x_assum(qspec_then`k`mp_tac) >> simp[])
 \\ strip_tac >> simp[]
 \\ full_simp_tac(srw_ss())[state_rel_def]
QED
...
Theorem get_code_labels_iAssign[simp]:
  ∀a b c d e. get_code_labels (iAssign a arch_size b c d e) = closLang$assign_get_code_label b
Proof
...
Theorem get_code_labels_compile:
  ∀a arch_size b c d e. get_code_labels (FST (compile a arch_size b c d e)) ⊆
