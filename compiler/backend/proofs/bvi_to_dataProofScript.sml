...
val state_rel_def = Define `
  state_rel (s:('c,'ffi) bviSem$state) (t:('c,'ffi) dataSem$state) <=>
    (s.clock = t.clock) /\
    code_rel s.code t.code (arch_size t.limits) /\
    (t.compile_oracle = ((I ## bvi_to_data$compile_prog (arch_size t.limits)) o s.compile_oracle)) /\
    (s.compile = (λcfg prog. t.compile cfg (bvi_to_data$compile_prog (arch_size
    t.limits) prog))) /\
...
val compile_RANGE_lemma = Q.prove(
  `!asize n env tail live xs.
      EVERY (\v. v < (SND (SND (compile asize n env tail live xs))))
        (FST (SND (compile asize n env tail live xs)))`,
...
val compile_part_thm = Q.prove(
  `compile_part asize = λ(x,y). (x, (λ(a,b). (a, compile_exp asize a b)) y)`,
...
Proof
  ...
  >- (Cases_on `z` \\ Cases_on `z'` \\ fs [data_to_bvi_ref_def,data_to_bvi_v_def]
     ...
     \\ rw [data_to_bvi_ref_def] \\ rfs [refs_rel_LEAST_eq,lookup_map,lookup_insert])
  >- (Q.EXISTS_TAC `t`
     \\ Q.EXISTS_TAC `Boolv b`
     \\ rw [data_to_bvi_v_Boolv]
     \\ drule_then (assume_tac o GSYM)
                   (data_to_bvi_do_eq |> SIMP_RULE std_ss [lookup_map]
                                      |> CONJUNCTS |> hd)
     \\ rw [])
 \\ Cases_on `z` \\ fs [data_to_bvi_ref_def,data_to_bvi_v_def]
 \\ rfs [data_to_bvi_v_def,Unit_def,bvlSemTheory.Unit_def
        ,data_to_bvi_ref_def,Boolv_def,bvlSemTheory.Boolv_def]
QED
...
>>>>>>> origin/master
    ...
    THEN1
     (`lookup k t2.locals = SOME x` by
      ...
      \\ RES_TAC \\ FULL_SIMP_TAC std_ss [])
    \\ REPEAT (FULL_SIMP_TAC (srw_ss()) [var_corr_def,get_var_def,lookup_insert,lookup_map,
              call_env_def,push_env_def,dataSemTheory.dec_clock_def,flush_state_def,LET_DEF,
              bviSemTheory.dec_clock_def]
               \\ FULL_SIMP_TAC (srw_ss()) [jump_exc_def]
               \\ BasicProvers.EVERY_CASE_TAC))
  (* Call with handle *)
  ...
  \\ Cases_on `isException res4` THEN1
   (Cases_on `res4` \\ full_simp_tac(srw_ss())[LET_DEF] \\ full_simp_tac(srw_ss())[]
    ...
    \\ reverse (Cases_on `q`) \\ full_simp_tac(srw_ss())[] THEN1
     ...
     \\ qexists_tac `ls''`
     \\ conj_tac >- fs [state_rel_def]
     \\ conj_tac >- fs [data_to_bvi_result_def]
...
Theorem compile_exp_correct:
   ^(compile_exp_lemma |> concl |> dest_imp |> fst) ==>
    ∃t2 prog vs next_var r.
      evaluate (compile_exp (arch_size t1.limits) n exp,t1) = (SOME r,t2) /\
...
Proof
  ...
  \\ PairCases_on`p`
  \\ `lookup start t1.code = SOME (p0,compile_exp (arch_size t1.limits) p0 p1)`
...
Theorem compile_prog_evaluate:
   evaluate ([Call 0 (SOME start) [] NONE],[],
     initial_state ffi0 (fromAList prog) co (λcfg prog. cc cfg (compile_prog
     (arch_size lims) prog)) k) = (r,s) ∧
   r ≠ Rerr (Rabort Rtype_error) ∧ (∀x. r ≠ Rerr (Rraise x))
   ⇒  ∃r2 s2.
       evaluate (Call NONE (SOME start) [] NONE,
         initial_state ffi0 (fromAList (compile_prog (arch_size lims) prog)) ((I ## compile_prog
         (arch_size lims)) o co) cc T lims ss k) = (SOME r2,s2) ∧
...
Theorem FST_compile_part[simp]:
   FST (compile_part asize a) = (FST a)
Proof
...
Proof
  ...
  \\ disch_then drule
  \\ rw[Abbr`a`,Abbr`b`]
  \\ qmatch_asmsub_abbrev_tac`FST (compile a b asize)`
