...
val comp_correct = Q.store_thm("comp_correct",
  ...
  \\ (* Call *)
      ...
      impl_tac>-
        ...
        CONJ_TAC>-
         (qunabbrev_tac `stack_state` \\ simp_tac (srw_ss()) []
          \\ drule find_code_IMP_get_labels
          \\ fs [get_labels_def]
          \\ imp_res_tac stackPropsTheory.evaluate_consts \\ fs [] \\ rfs []
          \\ unabbrev_all_tac \\ simp [])>>
        CONJ_TAC>-
          (`EVEN (max_var r)` by
              (ho_match_mp_tac max_var_IMP>>
    ...
    impl_tac THEN1
     (fs [comp_def,get_labels_def,IN_DEF]
      \\ imp_res_tac stackPropsTheory.evaluate_consts \\ fs []) >>
    strip_tac>>
...
val EVEN_DIV_2_props = prove(``
  a MOD 2 = 0 ∧ b MOD 2 = 0 ∧ a ≠ b ⇒
  a DIV 2 ≠ b DIV 2 ∧ a DIV 2 ≠ 0 ``,cheat)
...
val word_to_stack_full_inst_ok_less = store_thm("word_to_stack_inst_ok_less",``
  ...
  >-
    ...
    fs wconvs)
  >-
    (every_case_tac>> fs[full_inst_ok_less_def]>>
    PairCases_on`kf`>>
    rpt(EVAL_TAC>>rw[]))
  >-
    (PairCases_on`kf`>>
