...
Theorem evaluate_wLive[local]:
  ...
  ?t5:('a,'c,'ffi) stackSem$state bs5.
    ...
    state_rel ac k 0 0 (push_env envs ^nn s with <|locals := LN; locals_size := SOME 0|>) t5 (f'::lens) /\
...
Proof
  ...
  >~ [`the (_ - _) _ = _ - t.stack_space`]
  >- (
    fs[IS_SOME_EXISTS,OPTION_MAP2_DEF,PULL_EXISTS] >> fs[] >>
    fs[the_eqn,CaseEq"option",stack_size_eq] >>
    rveq >> fs[])
  >~[`IS_SOME (stack_size (_::s.stack))`]
  >- fs[OPTION_MAP2_DEF,IS_SOME_EXISTS,MAX_DEF,the_eqn,CaseEq"option",stack_size_eq]
  >~[`flat_exp_conventions pp`]
  >- metis_tac[]
  >~[`post_alloc_conventions k pp`]
  >- metis_tac[]
  \\ fsrw_tac[][wf_def]
...
Proof
  ...
  THEN1 (
    ...
    \\ `~ (LENGTH t4.stack <
         t4.stack_space + stack_free dest' (LENGTH args) (k,f,f'))`
    ...
    THEN1 ( (* Hit stack limit case *)
      ...
      \\ qhdtm_x_assum `state_rel` (STRIP_ASSUME_TAC o SRULE[state_rel_def])
      \\ fs[]
      \\ PRED_ASSUM is_forall kall_tac
      \\ CONJ_TAC >-(
  ...
  \\ BasicProvers.TOP_CASE_TAC \\ fs[]
  \\ BasicProvers.TOP_CASE_TAC \\ fs[]
  \\ drule (Q.SPECL[`x`,`SOME(x0,(x1,x2),x3,x4,x5)`] (Q.GENL[`args'`,`ret`]call_dest_lemma))
  \\ strip_tac \\rfs[]
  \\ BasicProvers.TOP_CASE_TAC \\ fs[]>>
  ...
  impl_keep_tac>- (
    ...
    >- (
      ...
      `list_max ls < 2*f' + 2*k` by
       ...
       rpt (IF_CASES_TAC >>
          rpt (POP_ASSUM mp_tac >> IF_CASES_TAC)) \\
       rw[]) >>
  ...
  >- (
    ...
    `t5.use_stack` by
       (*Naive way too slow*)
       (qpat_x_assum `state_rel _ _ _ _ _ t5 _`
       (PURE_REWRITE_TAC o single o SRULE[state_rel_def])) >>
    ...
    (fn g =>
         qabbrev_tac `stack_state = ^((strip_tac \\ qexists_tac`0`
   ...
   !(prog:'a wordLang$prog) (s:('a,num # 'c,'ffi) wordSem$state) k f f' res s1 t bs lens.
     ...
     ?ck t1:('a,'c,'ffi) stackSem$state res1.
       ...
       then res1 = SOME (Halt (Word 2w)) /\
         ...
         case res of
         | NONE => state_rel ac k f f' s1 t1 lens
         | SOME (Result _ y) => state_rel ac k 0 0 s1 t1 lens /\
