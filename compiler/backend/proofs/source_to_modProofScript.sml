...
val has_bools_def = Define `
  has_bools genv ⇔
    FLOOKUP genv (1n, SOME bool_id) = SOME (0n, TypeStamp "true" bool_type_num) ∧
    FLOOKUP genv (0, SOME bool_id) = SOME (0, TypeStamp "false" bool_type_num)`;

val has_lists_def = Define `
  has_lists genv ⇔
    FLOOKUP genv (cons_id, SOME list_id) = SOME (2n, TypeStamp "::" list_type_num) ∧
    FLOOKUP genv (nil_id, SOME list_id) = SOME (0, TypeStamp "nil" list_type_num)`;

val has_exns_def = Define `
  has_exns genv ⇔
    FLOOKUP genv (div_id, NONE) = SOME (0n, div_stamp) ∧
    FLOOKUP genv (chr_id, NONE) = SOME (0, chr_stamp) ∧
    FLOOKUP genv (subscript_id, NONE) = SOME (0, subscript_stamp) ∧
    FLOOKUP genv (bind_id, NONE) = SOME (0, bind_stamp)`;
...
val genv_c_ok_def = Define `
  genv_c_ok genv_c ⇔
    ...
    (!cn1 cn2 l1 l2 stamp1 stamp2.
      FLOOKUP genv_c cn1 = SOME (l1, stamp1) ∧
      FLOOKUP genv_c cn2 = SOME (l2, stamp2)
      ⇒
      (ctor_same_type (SOME stamp1) (SOME stamp2) ⇔ ctor_same_type (SOME cn1) (SOME cn2))) ∧
    (!cn1 cn2 l1 l2 stamp.
      FLOOKUP genv_c cn1 = SOME (l1, stamp) ∧
      FLOOKUP genv_c cn2 = SOME (l2, stamp)
      ⇒
      cn1 = cn2)`;
...
val (v_rel_rules, v_rel_ind, v_rel_cases) = Hol_reln `
  ...
  (!genv cn cn' vs vs'.
    ...
    FLOOKUP genv.c cn' = SOME (LENGTH vs, cn)
    ⇒
  ...
  (!genv comp_map shadowers env.
    ...
    (!x arity stamp.
      ...
      ∃cn. nsLookup comp_map.c x = SOME cn ∧
        FLOOKUP genv.c cn = SOME (arity, stamp))
    ⇒
   ...
   (!genv vs v.
    v_rel genv (Conv cn vs) v ⇔
      ?vs' cn'.
        ...
        | NONE => cn' = NONE
        | SOME cn =>
          ?cn2. cn' = SOME cn2 ∧ FLOOKUP genv.c cn2 = SOME (LENGTH vs, cn)) ∧
   (!genv l v.
    v_rel genv (Loc l) v ⇔
      (v = Loc l)) ∧
   (!genv vs v.
...
val v_rel_weakening = Q.prove (
  `(!genv v v'.
    v_rel genv v v'
    ⇒
    ∀l. v_rel <| v := genv.v++l; c := genv.c |> v v') ∧
   (!genv env env'.
    env_rel genv env env'
    ⇒
    !l. env_rel <| v := genv.v++l; c := genv.c |> env env') ∧
   (!genv comp_map shadowers env.
    global_env_inv genv comp_map shadowers env
    ⇒
    !l.global_env_inv <| v := genv.v++l; c := genv.c |> comp_map shadowers env)`,
...
val sv_rel_weakening = Q.prove (
  `(!genv sv sv_i1.
    ...
    ⇒
    ∀l. sv_rel (<| v := genv.v++l; c := genv.c|>) sv sv_i1)`,
...
val (env_all_rel_rules, env_all_rel_ind, env_all_rel_cases) = Hol_reln `
  (!genv map env_v_local env env' locals.
    ...
    env_all_rel genv map
      ...
      <| c := FST o_f genv.c; v := env'; exh_pat := F |>
      locals)`;
...
val do_app = Q.prove (
  ...
  >- ((* Shift *)
      srw_tac[][semanticPrimitivesPropsTheory.do_app_cases, modSemTheory.do_app_def] >>
      TRY (rename1 `shift8_lookup s w1 n`) >>
      TRY (rename1 `shift64_lookup s w1 n`) >>
      full_simp_tac(srw_ss())[v_rel_eqns, result_rel_cases, v_rel_lems]
      \\ Cases_on`w1` \\ Cases_on`s` \\ fs[shift8_lookup_def,shift64_lookup_def])
  >- ((* Equality *)
      srw_tac[][semanticPrimitivesPropsTheory.do_app_cases, modSemTheory.do_app_def] >>
      full_simp_tac(srw_ss())[v_rel_eqns, result_rel_cases, v_rel_lems] >>
      every_case_tac >>
  ...
  `!genv vs vs_i1 env e.
    ...
    ⇒
     ∃comp_map env_i1 locals t1 ts.
       env_all_rel genv comp_map env <| c := FST o_f genv.c; v := env_i1; exh_pat := F|> locals ∧
...
val pmatch = Q.prove (
  `(!cenv s p v env r env' env'' genv env_i1 s_i1 v_i1 comp_map.
    ...
    ⇒
    ?r_i1.
      pmatch (FST o_f genv.c) s_i1 (compile_pat comp_map p) v_i1 env_i1 = r_i1 ∧
      match_result_rel genv env'' r r_i1) ∧
   (!cenv s ps vs env r env' env'' genv env_i1 s_i1 vs_i1 comp_map.
    ...
    ⇒
    ?r_i1.
      pmatch_list (FST o_f genv.c) s_i1 (MAP (compile_pat comp_map) ps) vs_i1 env_i1 = r_i1 ∧
      match_result_rel genv env'' r r_i1)`,
  ...
  >- (
    ...
    strip_tac >>
    rw [] >>
    rw [pmatch_def, FLOOKUP_o_f, eta2, match_result_rel_def] >>
    fs [semanticPrimitivesTheory.same_ctor_def]
    >- metis_tac [genv_c_ok_def]
    >- (
      `cn = cn2` by metis_tac [genv_c_ok_def] >>
...
val compile_exp_correct' = Q.prove (
  ...
  >- ( (* Constructors *)
    ...
    >- (
      ...
      rw [EVERY2_REVERSE]
      >- (
        imp_res_tac modPropsTheory.evaluate_length >>
        fs [LENGTH_REVERSE] >>
        metis_tac [LIST_REL_LENGTH])
    ...
    >- (
      ...
      fs [v_rel_eqns] >>
      res_tac >>
      fs [evaluate_def, FLOOKUP_o_f, compile_exps_length]))
  >- ((* Variable lookup *)
  ...
  >- (
    ...
    ntac 2 (BasicProvers.TOP_CASE_TAC >> full_simp_tac(srw_ss())[]) >- (
      ...
      first_x_assum drule >>
      disch_then(qspecl_then [`dec_clock s'_i1`,`t1`,`ts'`] mp_tac)>>
      impl_tac >- (
      ...
      rw [] >>
      `env_i1 with v := env_i1' = <| v := env_i1'; c := env_i1.c; exh_pat := F |>`
      by rw [modSemTheory.environment_component_equality] >>
      fs [env_all_rel_cases]) >>
    BasicProvers.TOP_CASE_TAC >>
    BasicProvers.TOP_CASE_TAC >>
    strip_tac >> rveq >>
  ...
  >- ( (* Let *)
    ...
    qpat_abbrev_tac`env2 = env_i1 with v updated_by _` >>
    first_x_assum(qspecl_then[`comp_map`,`env2`]mp_tac) >>
    simp[Abbr`env2`] >>
...
val invariant_def = Define `
  invariant genv idx comp_map env s s_i1 ⇔
    ...
    (*
    compile env : name -> ctor_id # type_id
    source_eval env : name -> arity # stamp (conN # type_id or exn_id)
    mod_eval env : ctor_id # type_id -> arity
    genv : ctor_id # type_id -> arity # stamp
    *)

val global_env_inv_extend = Q.prove (
  `
  global_env_inv genv comp_map ∅ env ∧
  env_rel genv pat_env pat_env'
  ⇒
  global_env_inv
    <|v := globals ⧺ MAP SOME (REVERSE (MAP SND pat_env')); c := genv.c|>
    (extend_env
       <|c := nsEmpty;
         v := alist_to_ns (alloc_defs tt (LENGTH globals) (REVERSE (pat_bindings p [])))|> comp_map) ∅
    (extend_dec_env <|v := pat_env; c := nsEmpty|> env)`,
...
val compile_decs_correct = Q.prove (
  `!s env ds s' r comp_map s_i1 idx idx' comp_map' ds_i1 t t' genv.
    ...
    ?(s'_i1:'a modSem$state) genv' cenv' r_i1.
      modSem$evaluate_decs <| c := FST o_f genv.c; v := []; exh_pat := F |> s_i1 ds_i1 = (s'_i1,cenv',r_i1) ∧
      FST o_f genv'.c = FUNION cenv' (FST o_f genv.c) ∧
      (!env'.
        r = Rval env'
        ⇒
        r_i1 = NONE ∧
  ...
  >- (
    ...
    disch_then drule >>
    rw [] >>
    rename1 `evaluate_decs <|v := []; c := FUNION cenv1 (FST o_f genv.c); exh_pat := F|> s1 ds2 = (s2, cenv2, r2)` >>
    MAP_EVERY qexists_tac [`s2`, `genv''`, `FUNION cenv2 cenv1`, `r2`] >>
    rw [FUNION_ASSOC]
    >- (
  ...
  >- ( (* Let *)
    ...
    >- ( (* Expression evaluates *)
      ...
      `match_result_rel genv [] (pmatch env.c st'.refs p answer ([]++[]))
             (pmatch (FST o_f genv.c) st1'.refs (compile_pat comp_map p) answer1 [])`
      by (
      ...
      >- ( (* No match *)
        rw [PULL_EXISTS] >>
        MAP_EVERY qexists_tac [`genv`, `<| v := nsEmpty; c := nsEmpty|>`, `<| v := nsEmpty; c := nsEmpty|>`] >>
        rw [v_rel_lems, extend_env_def, extend_dec_env_def] >>
        fs [v_rel_eqns] >>
        metis_tac [])
      >- ( (* match *)
        drule pmatch_evaluate_vars_lem >>
        disch_then (qspecl_then [`REVERSE tt`, `F`] mp_tac)>>simp[]>>
        rfs[reverse_bind_locals_list] >>
        strip_tac >>
        rpt var_eq_tac >>
        rename1 `evaluate _ _ _ = (st2, Rval (MAP SND pat_env))` >>
        qexists_tac `<| v := s_i1.globals ⧺ MAP SOME (REVERSE (MAP SND pat_env)); c := genv.c |>` >>
        rw []
        >- cheat
        >- (
          fs [s_rel_cases] >>
          ...
          rw []))
          (*
        fs [v_rel_eqns, extend_env_def, extend_dec_env_def, s_rel_cases] >>
        rw []
        >- (
          rename1 `env_rel _ (alist_to_ns pe) pe'` >>
          fs [nsLookup_nsAppend_some, nsLookup_alist_to_ns_some]
          >- (
            drule env_rel_lookup >>
            disch_then drule >>
            rw [] >>
            ...
            fs [MEM_EL] >>
            rename [`(name,_) = EL n _`, `(name,_) = EL n' _`] >>
            MAP_EVERY qexists_tac [`LENGTH s_i1.globals + LENGTH pe' - n' - 1`, `v'`] >>
            rw [GSYM PULL_EXISTS]
            >- (
              `ALL_DISTINCT (MAP FST pe')` by cheat >>
              `ALOOKUP (REVERSE pe') x = SOME v2` by metis_tac [alookup_distinct_reverse] >>
              ALOOKUP_alloc_defs
              *)
      >- (
        imp_res_tac evaluate_length >>
        rfs [bind_locals_list_def, LENGTH_MAP2])
      >- (
        drule pmatch_evaluate_vars_lem >>
  ...
  >- ( (* Letrec *)
    ...
    >- ( (* One function *)
      ...
      rw [evaluate_decs_def, evaluate_dec_def, compile_exp_def, evaluate_def,
          ...
          nsLookup_nsBind, build_rec_env_def]
      qexists_tac `genv` >>
      rw [] >>
      fs [invariant_def] >>
      qmatch_goalsub_abbrev_tac `_ = s_i1.globals ++ [SOME cl]` >>
      qexists_tac `genv with v := s_i1.globals ++ [SOME cl]` >>
      fs [Abbr`cl`, s_rel_cases] >>
      rw []
      >- (
        fs [v_rel_eqns, extend_env_def, extend_dec_env_def] >>
        rw [] >>
      ...
      metis_tac []
          ...
          REWRITE_TAC [invariant_def]
          rpt conj_tac
          simp_tac (std_ss) [ALOOKUP_def]
