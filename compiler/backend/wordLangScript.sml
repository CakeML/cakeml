...
val every_var_def = Define `
  ...
  (every_var P (FFI ffi_index n fval names) =
    (P_num_lst P (n::fval) /\ every_name P names)) /\
...
val every_stack_var_def = Define `
  (every_stack_var P (FFI ffi_index n fval names) =
    every_name P names) ∧
...
val max_var_def = Define `
  ...
  (max_var (DataBufferWrite r1 r2) =
    MAX r1 r2) ∧
  (max_var (FFI ffi_index n fval numset) =
    list_max ((n::fval)++MAP FST (toAList numset))) ∧
