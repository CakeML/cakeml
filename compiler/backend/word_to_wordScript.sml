...
Definition compile_single_def:
  compile_single two_reg_arith reg_count alg c ((name_num:num,arg_count,prog,md),col_opt) =
  ...
  let reg_prog = word_alloc name_num c alg reg_count rm_prog col_opt in
    (name_num,arg_count,reg_prog,md)
End

Definition full_compile_single_def:
  full_compile_single two_reg_arith reg_count alg c p =
  let (name_num,arg_count,reg_prog,md) = compile_single two_reg_arith reg_count alg c p in
    (name_num,arg_count,remove_must_terminate reg_prog,md)
End
...
Definition full_compile_single_for_eval_def:
  full_compile_single_for_eval two_reg_arith reg_count alg c p =
    let ((name_num,arg_count,prog,md),col_opt) = p in
    let prog = word_simp$compile_exp prog in
    ...
    let cp_prog = copy_prop cse_prog in
    let _ = empty_ffi «finished: word_copy» in
    let two_prog = three_to_two_reg_prog two_reg_arith cp_prog in
    ...
    let _ = empty_ffi «finished: word_remove» in
      (name_num,arg_count,rmt_prog,md)
End
...
Theorem compile_alt:
    ...
    let names = MAP (λ(w,x,y,z). w) progs in
    let args = MAP (λ(w,x,y,z). x) progs in
    let ps = MAP (λ(w,x,y,z). y) progs in
    let mds = MAP (\(w,x,y,z). z) progs in
    ...
    let _ = empty_ffi «finished: word_remove» in
    (col,ZIP(names,ZIP(args,ZIP(rmt_ps,mds))))
Proof
