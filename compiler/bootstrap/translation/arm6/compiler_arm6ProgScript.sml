...
val res = translate compiler_arm6_def
...
val main = process_topdecs`
  fun main u =
    ...
    in
      case compiler_arm6 cl (read_all [])  of
...
val main_spec = Q.store_thm("main_spec",
  `app (p:'ffi ffi_proj) ^(fetch_v "main" st)
     ...
     (POSTv uv. &UNIT_TYPE () uv *
      STDOUT (out ++ (FLAT (MAP explode (append (FST(compiler_arm6 (TL(MAP implode cl)) inp)))))) *
      STDERR (err ++ explode (SND(compiler_arm6 (TL(MAP implode cl)) inp))) *
