...
val full_compiler_riscv_def = Define `
  full_compiler_riscv cl inp =
    if has_version_flag cl then
      (List [current_build_info_str], implode"", F)
    else
      let (a,b) = compiler_riscv cl inp in (a,b,T)`
...
val main_spec = Q.store_thm("main_spec",
  `app (p:'ffi ffi_proj) ^(fetch_v "main" st)
     ...
     (POSTv uv.
       &UNIT_TYPE () uv *
       (let (out,err,b) = full_compiler_riscv (TL (MAP implode cl)) (get_stdin fs) in
  ...
  >-
   (pairarg_tac \\ fs []
    \\ rfs [full_compiler_riscv_def] \\ rw []
    \\ xapp
...
val semantics_compiler_prog =
  ...
  |> SIMP_RULE (srw_ss()) [STD_streams_add_stderr,STD_streams_add_stdout,STD_streams_fastForwardFD,
                           AND_IMP_INTRO,GSYM CONJ_ASSOC, full_compiler_riscv_def]
