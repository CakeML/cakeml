...
(* if the above is attempted without the "spec32" line, the translator seems to
   break, and *loses* the ability to translate the expression datatype:

   time hol2deep ``(Panop i [(BaseAddr :32 panLang$exp); i'])``
*)
