...
open preamble
     ...
     arm8_configTheory
     arm8_targetTheory
     arm8_targetLib asmLib

val _ = new_theory "to_lab_arm8Bootstrap";
...
val to_data_thm0 =
  MATCH_MP change_config_thm to_data_arm8_thm
  |> Q.GEN`c2` |> ISPEC bootstrap_conf

val same_config = prove(to_data_thm0 |> concl |> rator |> rand,
  REWRITE_TAC[init_conf_def,arm8_backend_config_def]
  \\ EVAL_TAC
...
(*
val (ls,ty) = data_prog_arm8_def |> rconc |> listSyntax.dest_list
val data_prog_arm8' =  listSyntax.mk_list(List.take(ls,10),ty)
val data_prog_arm8_shorten = mk_thm([],``data_prog_arm8 = ^data_prog_arm8'``)
val to_data_thm1 = PURE_REWRITE_RULE[data_prog_arm8_shorten]to_data_thm1
val to_data_thm = to_data_thm1
*)

val stack_to_lab_thm = save_thm("stack_to_lab_thm",
  compilationLib.compile_to_lab data_prog_arm8_def to_data_thm1 "lab_prog");
