...
Triviality b2s__STRING[simp]:
  ∀s' ss.
  implode (bytes2string (n2w (ORD s')::MAP (n2w ∘ ORD) ss)) = implode $ STRING s' ss
Proof
...
Triviality enc_names_section_blank_conv:
  ∀x ex.
  enc_names_section x = SOME ex ∧ NULL (append ex) ⇒
  x = blank_names
Proof
...
Triviality names_not_blank:
  ∀x.
  x ≠ blank_names ⇒ x.mname ≠ «» ∨ ¬NULL x.fnames ∨ ¬NULL x.lnames
Proof
...
Triviality names_not_blank':
  ∀x.
  x ≠ blank_names ⇒ ¬NULL (string2bytes $ explode x.mname) ∨ ¬NULL x.fnames ∨ ¬NULL x.lnames
Proof
...
Theorem dec_enc_names_passThrough:
  ∀b rest. b ≠ 0w ⇒ dec_names_section $ (b::rest) = ret (b::rest) []
Proof
  rw[Once dec_names_section_def]
QED
...
Proof
     rpt strip_tac
  \\ qspecl_then [‘0w’, ‘dec_byte’, ‘b’, ‘enc’] dxrule_all enc_dec_section_passThrough_rest
  \\ simp[]
QED
...
Proof
     rpt strip_tac
  \\ qspec_then ‘[]’ dxrule_all pt0_rest
  \\ simp[]
QED
...
Proof
     rpt strip_tac
  >> qspecl_then [‘1w’, ‘dec_functype’, ‘b’, ‘enc’] (drule_at Any) enc_dec_section_passThrough_rest
  >> simp[]
QED
...
Proof
     rpt strip_tac
  >> qspec_then ‘[]’ (drule_at Any) pt1_rest
  >> simp[]
QED
...
Proof
     rpt strip_tac
  >> qspecl_then [‘3w’, ‘dec_u32’, ‘b’, ‘enc’] (drule_at Any) enc_dec_section_passThrough_rest
  >> simp[]
QED
...
Proof
     rpt strip_tac
  >> qspec_then ‘[]’ (drule_at Any) pt3_rest
  >> simp[]
QED
...
Proof
     rpt strip_tac
  >> qspecl_then [‘5w’, ‘dec_limits’, ‘b’, ‘enc’] (drule_at Any) enc_dec_section_passThrough_rest
  >> simp[]
QED
...
Proof
     rpt strip_tac
  >> qspec_then ‘[]’ (drule_at Any) pt5_rest
  >> simp[]
QED
...
Proof
     rpt strip_tac
  >> qspecl_then [‘6w’, ‘dec_global’, ‘b’, ‘enc’] (drule_at Any) enc_dec_section_passThrough_rest
  >> simp[]
QED
...
Proof
     rpt strip_tac
  >> qspec_then ‘[]’ (drule_at Any) pt6_rest
  >> simp[]
QED



Triviality pt10_rest:
  ∀rest b enc x xs es. b ≠ 10w ∧
    enc_section b enc (x::xs) = SOME es ⇒
    dec_section 10w dec_code (es a++ rest) = ret (es a++ rest) []
Proof
     rpt strip_tac
  >> qspecl_then [‘10w’, ‘dec_code’, ‘b’, ‘enc’] (drule_at Any) enc_dec_section_passThrough_rest
  >> simp[]
QED
...
Proof
     rpt strip_tac
  >> qspec_then ‘[]’ (drule_at Any) pt10_rest
  >> simp[]
QED
...
Proof
  ...
  >> namedCases_on ‘l_names’ ["", "l ls"]
    ...
    *)
    ...
    >> imp_res_tac pt0_rest (* for any given goal, some of these hyps may be superfluous *)
    >> imp_res_tac pt12
    (**)
    >> imp_res_tac widget0
    >> imp_res_tac widget1
...
Proof
  ...
  >> namedCases_on ‘datas’   ["","d ds"]
    ...
    (**)
    >> imp_res_tac pt1
    >> imp_res_tac pt1_rest
    >> imp_res_tac pt3
    >> imp_res_tac pt3_rest
    >> imp_res_tac pt5
    >> imp_res_tac pt5_rest
    >> imp_res_tac pt6
    >> imp_res_tac pt6_rest
    >> imp_res_tac pt10
    (**)
...
Triviality foo:
  ...
  enc_names_section n = SOME encn ⇒
  ∃xs.
  append encn = 0w :: xs ∧ ∃ys len.
  dec_u32 xs = ret ys len
Proof
  ntac 5 $ strip_tac
  \\ imp_res_tac enc_names_section_nEmp
  >> imp_res_tac names_not_blank'
