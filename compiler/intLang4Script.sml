...
(*val row_to_i4 : list (maybe varN) -> pat_exh -> list (maybe varN) * nat * (exp_i4 -> exp_i4)*)
(*val cols_to_i4 : list (maybe varN) -> nat -> nat -> list pat_exh -> list (maybe varN) * nat * (exp_i4 -> exp_i4)*)
 val row_to_i4_defn = Hol_defn "row_to_i4" `

(row_to_i4 (NONE::bvs) (Pvar_exh x) = ((SOME x::bvs), 0, (\ e .  e)))
/\
(row_to_i4 bvs (Plit_exh _) = (bvs, 0, (\ e .  e)))
/\
(row_to_i4 bvs (Pcon_exh _ ps) = (cols_to_i4 bvs( 0)( 0) ps))
/\
(row_to_i4 bvs (Pref_exh p) =  
...
(*val exp_to_i4 : list (maybe varN) -> exp_exh -> exp_i4*)
(*val exps_to_i4 : list (maybe varN) -> list exp_exh -> list exp_i4*)
(*val funs_to_i4 : list (maybe varN) -> list (varN * varN * exp_exh) -> list exp_i4*)
(* assumes the value being matched is most recently bound *)
(*val pes_to_i4 : list (maybe varN) -> list (pat_exh * exp_exh) -> exp_i4*)
 val exp_to_i4_defn = Hol_defn "exp_to_i4" `

(exp_to_i4 bvs (Raise_exh e) = (Raise_i4 (exp_to_i4 bvs e)))
/\
(exp_to_i4 bvs (Handle_exh e1 pes) =  
(Handle_i4 (exp_to_i4 bvs e1) (pes_to_i4 (NONE::bvs) pes)))
/\
(exp_to_i4 _ (Lit_exh l) = (Lit_i4 l))
/\
(exp_to_i4 bvs (Con_exh tag es) = (Con_i4 tag (exps_to_i4 bvs es)))
/\
(exp_to_i4 bvs (Var_local_exh x) =  
((case misc$find_index (SOME x) bvs( 0) of
    SOME k => Var_local_i4 k
  | NONE => Lit_i4 Unit (* should not happen *)
  )))
/\
(exp_to_i4 _ (Var_global_exh n) = (Var_global_i4 n))
/\
(exp_to_i4 bvs (Fun_exh x e) = (Fun_i4 (exp_to_i4 (SOME x::bvs) e)))
/\
(exp_to_i4 bvs (Uapp_exh uop e) = (Uapp_i4 (uop_to_i4 uop) (exp_to_i4 bvs e)))
/\
(exp_to_i4 bvs (App_exh op e1 e2) =  
(App_i4 op (exp_to_i4 bvs e1) (exp_to_i4 bvs e2)))
/\
(exp_to_i4 bvs (If_exh e1 e2 e3) =  
(sIf_i4 (exp_to_i4 bvs e1) (exp_to_i4 bvs e2) (exp_to_i4 bvs e3)))
/\
(exp_to_i4 bvs (Mat_exh e pes) =  
(sLet_i4 (exp_to_i4 bvs e) (pes_to_i4 (NONE::bvs) pes)))
/\
(exp_to_i4 bvs (Let_exh (SOME x) e1 e2) =  
(sLet_i4 (exp_to_i4 bvs e1) (exp_to_i4 (SOME x::bvs) e2)))
/\
(exp_to_i4 bvs (Let_exh NONE e1 e2) =  
(Seq_i4 (exp_to_i4 bvs e1) (exp_to_i4 bvs e2)))
/\
(exp_to_i4 bvs (Letrec_exh funs e) =  
(let bvs = ((MAP (\p .  
  (case (p ) of ( (f,_,_) ) => SOME f )) funs) ++ bvs) in
  Letrec_i4 (funs_to_i4 bvs funs) (exp_to_i4 bvs e)))
/\
(exp_to_i4 _ (Extend_global_exh n) = (Extend_global_i4 n))
/\
(exps_to_i4 _ [] = ([]))
/\
(exps_to_i4 bvs (e::es) =  
