...
fun zero x = x = 0
fun cons(x,y) =  x::y
fun make_vector (x,y) = Array.array x y
...
fun pormat(control_string: string, values: pormatValue list) =
   let
      fun loop(i: int, values: pormatValue list) =
...
fun positive_min(x, y) = if negative x then y else imin(x, y)
