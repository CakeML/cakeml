...
open lem_pervasivesTheory semanticPrimitivesTheory astTheory compilerLibTheory intLangTheory toIntLangTheory toBytecodeTheory bytecodeTheory intLang2Theory;
...
val _ = Define `
 (init_compiler_state =  
(<| contab := (((((((FEMPTY |+ (NONE, tuple_tag)) |+ ((SOME(Short"nil")), nil_tag)) |+ ((SOME(Short"::")), cons_tag)) |+ ((SOME(Short"Bind")), bind_tag)) |+ ((SOME(Short"Div")), div_tag)) |+ ((SOME(Short"Eq")), eq_tag))
              ,[(eq_tag,SOME(Short"Eq"))
               ;(div_tag,SOME(Short"Div"))
               ;(bind_tag,SOME(Short"Bind"))
               ;(cons_tag,SOME(Short"::"))
               ;(nil_tag,SOME(Short"nil"))
               ;(tuple_tag,NONE)]
              , 6)
   ; renv := []
   ; rmenv := FEMPTY
