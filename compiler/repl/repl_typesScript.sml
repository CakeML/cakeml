...
[repl_types_init:]
  (∀ffi rs decs types (s:'ffi semanticPrimitives$state) env.
...
[repl_types_eval:]
  (∀ffi rs decs types new_types (s:'ffi semanticPrimitives$state) env new_env new_s.
     repl_types (ffi,rs) (types,s,env) ∧
...
[repl_types_exn:]
  (∀ffi rs decs types new_types (s:'ffi semanticPrimitives$state) env e new_s.
     repl_types (ffi,rs) (types,s,env) ∧
     infertype_prog types decs = Success new_types ∧
...
[repl_types_str_assign:]
  (∀ffi rs types (s:'ffi semanticPrimitives$state) env t name loc new_store.
     repl_types (ffi,rs) (types,s,env) ∧
     MEM (name,Str,loc) rs ∧
