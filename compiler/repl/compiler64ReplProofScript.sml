...
Proof
  ...
  \\ `compiler_agrees (compiler_inst asm_conf) (id1,s1_v,decs) (s2_v,bs_v,ws_v)` by (
...
Theorem evaluate_Con = SIMP_CONV std_ss [evaluate_def,do_con_check_def,build_conv_def]
  “evaluate st env [Con NONE xs]”;

Theorem evaluate_Var = SIMP_CONV std_ss [evaluate_def,do_con_check_def]
  “evaluate st env [Var n]”;

Theorem evaluate_Lit = SIMP_CONV std_ss [evaluate_def]
  “evaluate st env [Lit n]”;

Theorem evaluate_list = List.take (evaluate_def |> CONJUNCTS,2) |> LIST_CONJ;

Theorem evaluate_App_Opapp = SIMP_CONV std_ss [evaluate_def,REVERSE_DEF,APPEND]
...
Overload eval_res_stamp[local] =
  (COMPILER64MAINPROG_EVAL_RES_TYPE_def
...
Theorem evaluate_eval:
  ∀(st:'ffi semanticPrimitives$state).
    ...
    SOME (Conv NONE [host_v;
      Conv NONE [Conv NONE [s1_v; Litv (IntLit (&next_gen))];
                 Conv NONE [env_v; Litv (IntLit (&env_id))]; decs_v]]) ∧
    BACKEND_CONFIG_TYPE s1 s1_v ∧
    LIST_TYPE DEC_TYPE decs decs_v ∧
...
Proof
  ...
  \\ assume_tac compiler64mainprog_compiler_for_eval_v_thm
  \\ drule_all Arrow_IMP
  \\ disch_then (qspec_then `dec_clock (dec_clock st)`
       (qx_choosel_then [`host_env`,`host_exp`,`host_junk`,`host_fun`,
                        `host_cost`,`host_state`,`host_res`] strip_assume_tac))
  \\ simp []
  \\ IF_CASES_TAC \\ fs []
  \\ Cases_on `host_res = Rerr (Rabort Rtimeout_error)` \\ gvs []
  \\ qmatch_goalsub_abbrev_tac `do_opapp [host_fun; compile_arg]`
  \\ `PAIR_TYPE (PAIR_TYPE NUM NUM)
        (PAIR_TYPE BACKEND_CONFIG_TYPE (LIST_TYPE DEC_TYPE))
        ((env_id,0),(s1,decs)) compile_arg` by (
  ...
  \\ disch_then (qx_choosel_then
       [`compile_env`,`compiler_call_body`,`compile_junk`,`compile_val`,
        `compile_cost`,`compile_result_state`,`compile_res`] strip_assume_tac)
  \\ simp []
  \\ IF_CASES_TAC \\ fs []
  \\ Cases_on `compile_res = Rerr (Rabort Rtimeout_error)` \\ gvs []
  \\ fs [Abbr `compile_state`,dec_clock_def]
  \\ simp [Once evaluate_def,can_pmatch_all_def,pmatch_def,evaluate_Var]
  \\ Cases_on `compiler_for_eval host ((env_id,0),s1,decs)`
  ...
  >- (
    ...
    \\ fs [build_conv_def]
    \\ CONV_TAC (DEPTH_CONV ml_progLib.nsLookup_conv) \\ simp []
    \\ qexistsl_tac [`compile_cost + (host_cost + 3)`,
                     `host_junk ++ compile_junk`]
    \\ simp [APPEND_ASSOC])
  \\ qmatch_asmsub_rename_tac `compiler_for_eval host _ = SOME compiled_output`
  \\ PairCases_on `compiled_output`
  \\ qmatch_asmsub_rename_tac
       `compiler_for_eval host _ = SOME (compiled_conf,compiled_bytes,compiled_words)`
  \\ fs [ml_translatorTheory.PAIR_TYPE_def,pmatch_def]
  \\ CONV_TAC (DEPTH_CONV ml_progLib.nsLookup_conv) \\ simp []
  \\ fs [semanticPrimitivesTheory.same_ctor_def]
  \\ simp [Once evaluate_def,astTheory.pat_bindings_def,pmatch_def]
  \\ CONV_TAC (DEPTH_CONV ml_progLib.nsLookup_conv) \\ simp []
  ...
  \\ qmatch_goalsub_rename_tac
       `do_opapp [eval_prim_v; Conv NONE [_; _; decs_v; next_conf_v; bytes_v; words_v]]`
  ...
  \\ qmatch_goalsub_abbrev_tac `evaluate primitive_state primitive_env`
  \\ `primitive_state.eval_state = SOME (EvalDecs s)` by
    fs [Abbr `primitive_state`]
  \\ fs [compiler_for_eval_def]
  \\ drule_then drule evaluate_Eval
  \\ disch_then drule \\ gvs []
  \\ rpt (disch_then drule)
  \\ disch_then (qspecl_then [`env1`,`primitive_env`] mp_tac)
  \\ impl_tac >- (unabbrev_all_tac \\ fs [])
  \\ strip_tac \\ fs []
  \\ pop_assum kall_tac
  \\ IF_CASES_TAC THEN1 fs []
  \\ fs [dec_clock_def,Abbr `primitive_state`]
  \\ qmatch_goalsub_abbrev_tac `evaluate_decs decl_state`
  \\ `∃result_state decl_result.
        evaluate_decs (decl_state with eval_state := NONE) env1 decs =
        (result_state,decl_result)` by metis_tac [PAIR]
  \\ drule (evaluatePropsTheory.eval_no_eval_simulation |> CONJUNCTS |> last)
  \\ disch_then (qspec_then
       `SOME (EvalDecs (add_decs_generation (s with compiler_state := next_conf_v)))`
       mp_tac)
  \\ impl_keep_tac >- (
    fs [] \\ unabbrev_all_tac \\ fs [] \\ metis_tac [APPEND_ASSOC])
  \\ fs [Abbr `decl_state`] \\ strip_tac \\ pop_assum kall_tac
  \\ reverse (Cases_on `decl_result`) \\ fs []
  >- (
    qmatch_asmsub_rename_tac `evaluate_decs _ _ _ = (_,Rerr decl_error)`
    \\ reverse (namedCases_on `decl_error` ["raised_exn","abort_reason"])
    \\ fs []
    >- (Cases_on `abort_reason` \\ fs [])
    ...
    \\ qexistsl_tac [`host_junk ++ compile_junk`,`result_state`,
                     `compile_cost + (host_cost + 5)`,`2`,`compiled_conf`]
    \\ simp [APPEND_ASSOC,semanticPrimitivesTheory.state_component_equality])
  \\ qmatch_asmsub_rename_tac
       `evaluate_decs _ _ _ = (result_state,Rval result_env)`
  ...
  \\ simp [Once evaluate_def,evaluate_Var,evaluate_Con,evaluate_list,
           namespaceTheory.nsOptBind_def,do_con_check_def,build_conv_def]
  \\ CONV_TAC (DEPTH_CONV ml_progLib.nsLookup_conv) \\ simp []
  \\ qexistsl_tac [`host_junk ++ compile_junk`,`result_env`,`result_state`,
                   `compile_cost + (host_cost + 5)`,`2`,`compiled_conf`]
  \\ simp [APPEND_ASSOC,semanticPrimitivesTheory.state_component_equality]
QED
...
Theorem evaluate_repl:
  ∀(st:'ffi semanticPrimitives$state) s repl_str arg_str
   ...
   types decs input_str env_id s1 parse host host_v.
     st.eval_state = SOME (EvalDecs s) ∧
     s.compiler = compiler_inst (host_config host) ∧
     s.compiler_state = s1_v ∧
     s.decode_decs = v_fun_abs decs_allowed (LIST_v DEC_v) ∧
...
Proof
  ...
  \\ disch_then (qspec_then `st with clock := st.clock − 2`
       (qx_choosel_then [`check_env`,`check_body`,`junk`,`check_value`,
                        `ck`,`check_state`,`res`] strip_assume_tac))
  \\ fs [] \\ IF_CASES_TAC \\ fs []
...
Proof
  ...
  \\ qmatch_goalsub_abbrev_tac `(st2, Rerr (Rabort Rtimeout_error))`
  \\ disch_then (qspec_then `st2 with clock := st2.clock − 1`
       (qx_choosel_then [`next_env`,`next_body`,`next_junk`,`next_value`,
