...
open lem_pervasivesTheory semanticPrimitivesTheory astTheory bigStepTheory exhLangTheory conLangTheory decLangTheory ffiTheory;
...
val _ = Define `
 (do_app_pat ((cnt,s,t),genv) op vs =  
((case (op,vs) of
      (Op_pat (Op_i2 (Opn op)), [Litv_pat (IntLit n1); Litv_pat (IntLit n2)]) =>
        if ((op = Divide) \/ (op = Modulo)) /\ (n2 =( 0 : int)) then
          SOME (((cnt,s,t),genv), Rerr (Rraise (prim_exn_pat div_tag)))
        else
          SOME (((cnt,s,t),genv), Rval (Litv_pat (IntLit (opn_lookup op n1 n2))))
    | (Op_pat (Op_i2 (Opb op)), [Litv_pat (IntLit n1); Litv_pat (IntLit n2)]) =>
        SOME (((cnt,s,t),genv), Rval (Boolv_pat (opb_lookup op n1 n2)))
    | (Op_pat (Op_i2 Equality), [v1; v2]) =>
        (case do_eq_pat v1 v2 of
            Eq_type_error => NONE
          | Eq_closure => SOME (((cnt,s,t),genv), Rerr (Rraise (prim_exn_pat eq_tag)))
          | Eq_val b => SOME (((cnt,s,t),genv), Rval(Boolv_pat b))
        )
    | (Op_pat (Op_i2 Opassign), [Loc_pat lnum; v]) =>
        (case store_assign lnum (Refv v) s of
          SOME st => SOME (((cnt,st,t),genv), Rval (Conv_pat tuple_tag []))
        | NONE => NONE
        )
    ...
    | (Op_pat (Op_i2 Opref), [v]) =>
        let (s',n) = (store_alloc (Refv v) s) in
          SOME (((cnt,s',t),genv), Rval (Loc_pat n))
    | (Op_pat (Init_global_var_i2 idx), [v]) =>
        if idx < LENGTH genv then
          (case EL idx genv of
              NONE => SOME (((cnt,s,t), LUPDATE (SOME v) idx genv), Rval (Conv_pat tuple_tag []))
            | SOME _ => NONE
          )
        else
          NONE
    | (Op_pat (Op_i2 Aw8alloc), [Litv_pat (IntLit n); Litv_pat (Word8 w)]) =>
        if n <( 0 : int) then
          SOME (((cnt,s,t),genv), Rerr (Rraise (prim_exn_pat subscript_tag)))
        else
          let (st,lnum) =            
(store_alloc (W8array (REPLICATE (Num (ABS ( n))) w)) s)
          in
            SOME (((cnt,st,t),genv), Rval (Loc_pat lnum))
              ...
              if i <( 0 : int) then
                ...
                let n = (Num (ABS ( i))) in
                  if n >= LENGTH ws then
                    ...
                    SOME (((cnt,s,t),genv), Rval (Litv_pat (Word8 (EL n ws))))
          | _ => NONE
        )
    | (Op_pat (Op_i2 Aw8length), [Loc_pat n]) =>
        (case store_lookup n s of
            SOME (W8array ws) =>
              SOME (((cnt,s,t),genv),Rval (Litv_pat (IntLit (int_of_num (LENGTH ws)))))
          | _ => NONE
        )
    ...
    | (Op_pat (Op_i2 Ord), [Litv_pat (Char c)]) =>
          SOME (((cnt,s,t),genv), Rval (Litv_pat(IntLit(int_of_num(ORD c)))))
...
(if (i <( 0 : int)) \/ (i >( 255 : int)) then
    ...
    | (Op_pat (Op_i2 (Chopb op)), [Litv_pat (Char c1); Litv_pat (Char c2)]) =>
        SOME (((cnt,s,t),genv), Rval (Boolv_pat (opb_lookup op (int_of_num(ORD c1)) (int_of_num(ORD c2)))))
    | (Op_pat (Op_i2 Implode), [v]) =>
          (case v_pat_to_char_list v of
            SOME ls =>
              SOME (((cnt,s,t),genv), Rval (Litv_pat (StrLit (IMPLODE ls))))
          | NONE => NONE
          )
    | (Op_pat (Op_i2 Explode), [Litv_pat (StrLit str)]) =>
        SOME (((cnt,s,t),genv), Rval (char_list_to_v_pat (EXPLODE str)))
    | (Op_pat (Op_i2 Strlen), [Litv_pat (StrLit str)]) =>
        SOME (((cnt,s,t),genv), Rval (Litv_pat(IntLit(int_of_num(STRLEN str)))))
    | (Op_pat (Op_i2 VfromList), [v]) =>
          (case v_to_list_pat v of
              SOME vs =>
                SOME (((cnt,s,t),genv), Rval (Vectorv_pat vs))
            | NONE => NONE
          )
    ...
    | (Op_pat (Op_i2 Vlength), [Vectorv_pat vs]) =>
        SOME (((cnt,s,t),genv), Rval (Litv_pat (IntLit (int_of_num (LENGTH vs)))))
...
(store_alloc (Varray (REPLICATE (Num (ABS ( n))) v)) s)
          in 
            SOME (((cnt,s',t),genv), Rval (Loc_pat lnum))
              ...
              let n = (Num (ABS ( i))) in
                if n >= LENGTH vs then
                  ...
                  (case store_assign lnum (Varray (LUPDATE v n vs)) s of
                      NONE => NONE
                    | SOME s' => SOME (((cnt,s',t),genv), Rval (Conv_pat tuple_tag []))
                  )
        | _ => NONE
      )
    | (Op_pat (Op_i2 (FFI n)), [Loc_pat lnum]) =>
        (case store_lookup lnum s of
          SOME (W8array ws) =>
    ...
    | (Tag_eq_pat n, [Conv_pat tag _]) =>
        SOME (((cnt,s,t),genv), Rval (Boolv_pat (tag = n)))
...
(evaluate_pat ck env s1 e1 (s2, Rerr (Rabort a)))
==>
evaluate_pat ck env s1 (Handle_pat e1 e2) (s2, Rerr (Rabort a)))
...
(evaluate_list_pat ck env s1 (REVERSE es) (((count,s2,t2),genv2), Rval vs) /\
(do_opapp_pat (REVERSE vs) = SOME (env2, e2)) /\
(ck ==> ~ (count =( 0))) /\
evaluate_pat ck env2 (((if ck then count -  1 else count),s2,t2),genv2) e2 bv)
==>
...
ck)
==>
evaluate_pat ck env s1 (App_pat (Op_pat (Op_i2 Opapp)) es) ((( 0,s2,t2),genv2), Rerr (Rabort Rtimeout_error)))
