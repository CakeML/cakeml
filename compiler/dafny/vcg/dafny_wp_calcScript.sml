...
Definition get_vars_exp_def:
  ...
  get_vars_exp (OldHeap e) = get_vars_exp e ∧
...
Definition freevars_aux_def:
  ...
  (freevars_aux (OldHeap e) = freevars_aux e) ∧
...
Proof
  ...
  >~ [‘OldHeap’] >-
    gvs[evaluate_exp_def, AllCaseEqs(),freevars_aux_def,unuse_old_heap_def, use_old_heap_def]
  >~ [‘Prev’] >- (
...
(* if b = T, this checks that no Olds appear
  if b = F, then neither of the Old/OldHeap ops at all *)
Definition no_Old_def:
  (no_Old b (Old _) ⇔ F) ∧
  (no_Old b (OldHeap e) ⇔ (if b then no_Old b e else F)) ∧
  (no_Old b (Lit _) ⇔ T) ∧
  (no_Old b (Var _) ⇔ T) ∧
  (no_Old b (Prev e) ⇔ no_Old b e) ∧
  (no_Old b (PrevHeap e) ⇔ no_Old b e) ∧
  (no_Old b (SetPrev e) ⇔ no_Old b e) ∧
  (no_Old b (If tst thn els) ⇔
     no_Old b tst ∧ no_Old b thn ∧ no_Old b els) ∧
  (no_Old b (UnOp _ e) ⇔ no_Old b e) ∧
  (no_Old b (BinOp _ e₀ e₁) ⇔
     no_Old b e₀ ∧ no_Old b e₁) ∧
  (no_Old b (ArrLen arr) ⇔ no_Old b arr) ∧
  (no_Old b (ArrSel arr idx) ⇔ no_Old b arr ∧ no_Old b idx) ∧
  (no_Old b (FunCall _ args) ⇔ EVERY (λe. no_Old b e) args) ∧
  (no_Old b (Forall _ term) ⇔ no_Old b term) ∧
  (no_Old b (Let binds body) ⇔
     EVERY (λe. no_Old b e) (MAP SND binds) ∧ no_Old b body) ∧
  (no_Old b (ForallHeap mods e) ⇔
     EVERY (λe. no_Old b e) mods ∧ no_Old b e)
Termination
...
Definition no_Prev_def:
  ...
  (no_Prev b (OldHeap e) ⇔ no_Prev b e) ∧
...
Definition replace_OldHeap_def:
  (replace_OldHeap (OldHeap e) = PrevHeap (replace_OldHeap e)) ∧
  (replace_OldHeap (Old e) = Old (replace_OldHeap e)) ∧
  (replace_OldHeap (Lit l) = Lit l) ∧
  (replace_OldHeap (Var v) = Var v) ∧
  (replace_OldHeap (Prev e) = replace_OldHeap e) ∧
  (replace_OldHeap (PrevHeap e) = replace_OldHeap e) ∧
  (replace_OldHeap (SetPrev e) = replace_OldHeap e) ∧
  (replace_OldHeap (If tst thn els) =
     If (replace_OldHeap tst) (replace_OldHeap thn) (replace_OldHeap els)) ∧
  (replace_OldHeap (UnOp u e) = UnOp u (replace_OldHeap e)) ∧
  (replace_OldHeap (BinOp b e₀ e₁) =
     BinOp b (replace_OldHeap e₀) (replace_OldHeap e₁)) ∧
  (replace_OldHeap (ArrLen arr) = ArrLen (replace_OldHeap arr)) ∧
  (replace_OldHeap (ArrSel arr idx) = ArrSel (replace_OldHeap arr) (replace_OldHeap idx)) ∧
  (replace_OldHeap (FunCall f args) = FunCall f (MAP replace_OldHeap args)) ∧
  (replace_OldHeap (Forall v term) = Forall v (replace_OldHeap term)) ∧
  (replace_OldHeap (Let binds body) =
     Let (MAP (λ(v,e). (v, replace_OldHeap e)) binds) (replace_OldHeap body)) ∧
  (replace_OldHeap (ForallHeap mods e) =
     ForallHeap (MAP replace_OldHeap mods) (replace_OldHeap e))
Termination
  wf_rel_tac ‘measure $ exp_size’
  \\ rpt strip_tac
  \\ gvs [list_size_pair_size_MAP_FST_SND]
               ...
               [SetPrev $ ForallHeap (MAP Var callee_mod_arg_vars) $
                Foralls (ZIP (ret_names, MAP SND mspec.outs))
                  (imp (Let (ZIP(MAP FST mspec.ins ++ MAP FST mspec.outs,
                          ...
                          (conj (MAP replace_OldHeap mspec.ens)))
                       (conj post))])
...
Definition wrap_Old_def:
  ...
  wrap_Old vs (Old e) =
  Old (wrap_Old vs e) ∧
  wrap_Old vs (OldHeap e) =
  OldHeap (wrap_Old vs e) ∧
...
Proof
  ...
  >~ [‘OldHeap’] >-
    gvs[evaluate_exp_def,AllCaseEqs(),no_Prev_def,use_old_heap_def,unuse_old_heap_def]
  >~ [‘Prev’] >-
    gvs[evaluate_exp_def,AllCaseEqs(),no_Prev_def]
  >~ [‘PrevHeap’] >-
    gvs[evaluate_exp_def,AllCaseEqs(),no_Prev_def]
...
Theorem evaluate_exp_no_old:
  (∀s env e s' r h l.
     evaluate_exp s env e = (s', r) ∧ no_Old F e ⇒
     evaluate_exp (s with <| heap_old := h; locals_old := l |>) env e =
     (s' with <| heap_old := h; locals_old := l |>, r)) ∧
  (∀s env es s' r h l.
     evaluate_exps s env es = (s', r) ∧ EVERY (λe. no_Old F e) es ⇒
...
Theorem no_Old_conj:
  ∀xs. no_Old b (conj xs) = EVERY (no_Old b) xs
Proof
...
Proof
  ...
  >~ [‘OldHeap’] >-
   (qpat_x_assum ‘_ = wrap_Old _ _’ mp_tac
    ...
    \\ simp[use_old_heap_def]
    \\ disch_then (drule_at Any)
    \\ simp[unuse_old_heap_def])
  >~ [‘Prev’] >-
   (qpat_x_assum ‘_ = wrap_Old _ _’ mp_tac
    \\ simp [Once $ oneline wrap_Old_def]
    \\ simp [AllCaseEqs()]
...
Triviality evaluate_rhs_exp_ignore_unused:
  ...
  st.locals = xs ++ ys ++ zs ⇒
  evaluate_rhs_exp (st with locals := xs ++ zs) env lhs =
    (st' with locals := xs ++ zs, r)
Proof
  Cases_on ‘lhs’
  \\ rpt strip_tac
  >~ [‘ExpRhs’] >-
   (gvs [evaluate_rhs_exp_def, get_vars_rhs_exp_def, AllCaseEqs()]
    \\ imp_res_tac evaluate_exp_ignore_unused \\ simp [])
  >~ [‘ArrAlloc’] >-
   (gvs [evaluate_rhs_exp_def, get_vars_rhs_exp_def, AllCaseEqs()]
    \\ imp_res_tac evaluate_exp_with_clock \\ gvs []
...
Triviality evaluate_rhs_exps_ignore_unused:
  ∀rhss st st' r.
    ...
    st.locals = xs ++ ys ++ zs ⇒
    evaluate_rhs_exps (st with locals := xs ++ zs) env rhss =
      (st' with locals := xs ++ zs, r)
Proof
  Induct >- (simp [evaluate_rhs_exps_def])
  \\ rpt strip_tac
  \\ gvs [evaluate_rhs_exps_def, get_vars_rhs_exp_def, AllCaseEqs()]
  \\ imp_res_tac evaluate_rhs_exp_ignore_unused \\ gvs []
...
Triviality assign_value_ignore_unused:
  ...
  ∃xs₁ zs₁.
    st'.locals = xs₁ ++ ys ++ zs₁ ∧
    assign_value (st with locals := xs ++ zs) env lhs val =
    (st' with locals := xs₁ ++ zs₁, r) ∧
    MAP FST xs₁ = MAP FST xs ∧ MAP FST zs₁ = MAP FST zs
Proof
  Induct_on ‘lhs’
...
Triviality assign_values_ignore_unused:
  ∀st env lhss vals st' r xs ys zs.
    ...
    ∃xs₁ zs₁.
      st'.locals = xs₁ ++ ys ++ zs₁ ∧
      assign_values (st with locals := xs ++ zs) env lhss vals =
      (st' with locals := xs₁ ++ zs₁, r) ∧
      MAP FST xs₁ = MAP FST xs ∧ MAP FST zs₁ = MAP FST zs
Proof
  ...
  >-
   (reverse $ gvs [AllCaseEqs()]
    \\ imp_res_tac assign_value_ignore_unused \\ gvs []
    >- (irule_at (Pos hd) EQ_REFL \\ simp [])
    \\ last_x_assum drule
    \\ rename [‘xs₃ ++ _ ++ zs₃’]  (* in goal *)
...
Proof
  ...
  >~ [‘Assign’] >-
   (gvs [evaluate_stmt_def,get_vars_stmt_def]
    \\ reverse $ gvs [AllCaseEqs()]
    \\ imp_res_tac evaluate_rhs_exps_same_locals \\ gvs []
    \\ imp_res_tac evaluate_rhs_exps_ignore_unused \\ gvs []
    >- (irule_at (Pos hd) EQ_REFL \\ simp [])
    \\ imp_res_tac assign_values_ignore_unused \\ gvs []
    \\ irule_at (Pos hd) EQ_REFL \\ simp [])
  >~ [‘While’] >-
   (gvs [evaluate_stmt_def,get_vars_stmt_def]
    \\ gvs [AllCaseEqs()]
    \\ gvs [get_vars_stmt_def,STOP_def, dec_clock_def]
    \\ imp_res_tac evaluate_exp_with_clock \\ gvs []
    \\ imp_res_tac evaluate_exp_ignore_unused \\ gvs []
    (* todo using rpt as try here is a bit yuck *)
    \\ rpt $ irule_at (Pos hd) EQ_REFL \\ simp []
    \\ first_x_assum drule
    \\ disch_then $ qspecl_then [‘xs’, ‘zs’] assume_tac \\ gvs []
    >-
     (last_x_assum drule \\ simp []
      \\ rename [‘xs₁ ++ _ ++ zs₁’]
      \\ disch_then $ qspecl_then [‘xs₁’, ‘zs₁’] assume_tac \\ gvs []
...
Proof
  ...
  \\ dxrule_then assume_tac DISJOINT_set_IMP
  \\ ‘ALL_DISTINCT (MAP FST vs1 ++ MAP FST vs2)’ by
    (gvs [strict_locals_ok_def, ALL_DISTINCT_APPEND])
  \\ simp [strict_locals_ok_def]
  \\ rpt strip_tac
  >- (* vs1 *)
   (simp [ALOOKUP_APPEND]
...
Theorem eval_true_ForallHeap_NIL =
  eval_true_ForallHeap |> Q.INST [‘mods’|->‘[]’] |> GEN_ALL |> SRULE [] |> SPEC_ALL;

Theorem eval_stmt_Then_both:
  eval_stmt st env stmt₁ st₁ Rcont ∧
  eval_stmt st₁ env stmt₂ st₂ res ⇒
  eval_stmt st env (Then stmt₁ stmt₂) st₂ res
Proof
  rw [eval_stmt_def,evaluate_stmt_def,AllCaseEqs()] \\ fs [SF DNF_ss]
  \\ disj1_tac
  \\ dxrule (evaluate_stmt_add_to_clock |> cj 1)
  \\ disch_then $ qspec_then ‘ck2’ mp_tac
  \\ dxrule (evaluate_stmt_add_to_clock |> cj 1)
  \\ disch_then $ qspec_then ‘ck1'’ mp_tac
  \\ simp [] \\ rpt strip_tac
...
Theorem eval_stmt_ArrAlloc:
  eval_exp st env len_e (IntV (& len)) ∧
  eval_exp st env el_e el_v ∧
  update_local_aux st.locals arr_v (ArrV len (LENGTH st.heap) el_ty) = SOME new_l ⇒
  eval_stmt st env
    (Assign [(VarLhs arr_v, ArrAlloc len_e el_e el_ty)])
    (st with <| heap := st.heap ++ [HArr (REPLICATE len el_v) el_ty] ;
                locals := new_l |>) Rcont
Proof
  rw [eval_exp_def,eval_stmt_def,evaluate_stmt_def,AllCaseEqs(),PULL_EXISTS]
  \\ fs [evaluate_rhs_exps_def,AllCaseEqs(),PULL_EXISTS]
  \\ gvs [assign_values_def,assign_value_def,AllCaseEqs(),PULL_EXISTS,
          evaluate_rhs_exp_def,alloc_array_def,oneline val_to_num_def,
          update_local_def]
  \\ dxrule (evaluate_exp_add_to_clock |> cj 1)
  \\ disch_then $ qspec_then ‘ck2’ mp_tac
  \\ dxrule (evaluate_exp_add_to_clock |> cj 1)
  \\ disch_then $ qspec_then ‘ck1'’ mp_tac
  \\ simp [] \\ rpt strip_tac
  \\ last_x_assum $ irule_at $ Pos hd \\ fs [SNOC_APPEND]
  \\ simp [SF CONJ_ss]
  \\ simp [state_component_equality]
QED

Theorem eval_true_Forall_IMP =
  eval_true_Foralls
    |> Q.SPEC ‘[(x,ty)]’
...
Proof
  ...
  \\ dxrule eval_true_ForallHeap_NIL \\ simp []
  \\ strip_tac
  \\ irule_at Any eval_stmt_Then_both
  \\ irule_at Any eval_stmt_ArrAlloc
  \\ dxrule eval_true_CanEval \\ strip_tac
  \\ drule_all eval_exp_get_type \\ strip_tac
  \\ qpat_x_assum ‘get_type locals len_e = INR IntT’ assume_tac
  \\ dxrule eval_true_CanEval \\ strip_tac
  \\ drule_all eval_exp_get_type
  \\ simp [Once all_values_def]
  \\ strip_tac \\ gvs []
  \\ ‘∃len. i = & len’ by
   (qpat_x_assum ‘eval_true st env (int_le (int_lit 0) len_e)’ mp_tac
    \\ pop_assum mp_tac
    \\ simp [eval_true_def,eval_exp_def,evaluate_exp_def,do_sc_def,do_bop_def,
             AllCaseEqs()]
    ...
    \\ dxrule (evaluate_exp_add_to_clock |> cj 1)
    \\ disch_then $ qspec_then ‘ck1'’ mp_tac
    \\ fs [intLib.COOPER_PROVE “0 ≤ i ⇔ ∃n. (i:int) = & n”])
  \\ gvs []
  \\ first_assum $ irule_at $ Pos hd
  \\ first_assum $ irule_at $ Pos hd
  ...
  \\ simp []
  \\ ‘valid_mod st.heap [] new_heap’ by
    (simp [Abbr‘new_heap’,valid_mod_def,oEL_THM,EL_APPEND1])
  \\ first_x_assum drule \\ strip_tac
  \\ dxrule eval_true_Forall_IMP
  \\ simp [all_values_def,PULL_EXISTS]
  \\ disch_then $ qspecl_then [‘len’,‘LENGTH st.heap’] mp_tac
  \\ qmatch_goalsub_abbrev_tac ‘eval_true s3’
  \\ strip_tac
  ...
  \\ ‘eval_true s4 env (conj reqs)’ by
    (qabbrev_tac ‘s5 = st with heap := new_heap’
     \\ qabbrev_tac ‘new_l2 = (arr_v,SOME arr_loc)::st.locals’
     \\ fs [eval_true_def]
     \\ drule eval_exp_no_Prev
     \\ disch_then drule
     \\ disch_then $ qspecl_then [‘st.locals_prev’,‘st.heap_prev’] mp_tac
     \\ fs [GSYM eval_true_def]
     \\ ‘s4 = s5 with locals := new_l’ by
           gvs [Abbr‘s4’,Abbr‘s5’,state_component_equality]
     \\ pop_assum $ rewrite_tac o single
     \\ qmatch_goalsub_abbrev_tac ‘eval_true s9’
     \\ ‘s9 = (s5 with locals := new_l2)’ by
           gvs [Abbr‘s3’,Abbr‘s5’,Abbr‘s9’,state_component_equality]
     \\ pop_assum $ rewrite_tac o single
     \\ match_mp_tac EQ_IMPLIES
     \\ rewrite_tac [eval_true_def]
  ...
  \\ first_assum $ irule_at $ Pos hd
  \\ fs [Abbr‘s4’] \\ gvs []
  \\ qpat_x_assum ‘valid_mod new_heap _ _’ mp_tac
  \\ simp [valid_mod_def] \\ rw []
...
Theorem no_Old_replace_OldHeap:
  ∀y.
    no_Old F (replace_OldHeap y) ⇒
    no_Old T (replace_OldHeap y)
Proof
...
Theorem conj_replace_OldHeap:
  conj (MAP replace_OldHeap xs) = replace_OldHeap (conj xs)
Proof
  Induct_on ‘xs’ \\ fs [replace_OldHeap_def,conj_def]
...
Theorem evaluate_exp_replace_OldHeap:
  (∀st env e st' r h hp.
     ...
     st.heap = h ∧ st.heap_old = hp ∧
     evaluate_exp st env e = (st', r) ⇒
     evaluate_exp (st with <|heap_prev:=hp|>) env (replace_OldHeap e) =
      (st' with <|heap_prev:=hp|>, r)) ∧
  (∀st env es st' r h hp.
     EVERY (no_Prev F) es ∧ EVERY (no_Old T) es ∧
     st.heap = h ∧ st.heap_old = hp ∧
     evaluate_exps st env es = (st', r) ⇒
     evaluate_exps (st with <|heap_prev:=hp|>) env (MAP replace_OldHeap es) =
      (st' with <|heap_prev:=hp|>, r))
Proof
  ho_match_mp_tac evaluate_exp_ind
  \\ rpt strip_tac
  >~ [‘Lit’] >-
    gvs[evaluate_exp_def,replace_OldHeap_def,state_component_equality]
  >~ [‘Var’] >-
    gvs[evaluate_exp_def,replace_OldHeap_def,state_component_equality,AllCaseEqs()]
  >~ [‘If’] >- (
    gvs[evaluate_exp_def,replace_OldHeap_def,AllCaseEqs(),no_Prev_def,no_Old_def,oneline do_cond_def]>>
    imp_res_tac evaluate_exp_with_clock \\ gvs [])
  >~ [‘UnOp’] >-
  ...
  >~ [‘OldHeap’] >- (
    ...
    gvs[no_Prev_def,no_Old_def,use_old_heap_def,use_prev_heap_def]>>
    simp[unuse_prev_heap_def,unuse_old_heap_def])
  >~ [‘Prev’] >- gvs[no_Prev_def]
...
Proof
  ...
  \\ disch_then $ qspecl_then [‘st.locals_prev’,‘st.heap_prev’] mp_tac
  \\ strip_tac
  \\ irule IMP_eval_exp_replace_OldHeap \\ simp []
  \\ pop_assum mp_tac
  \\ match_mp_tac EQ_IMPLIES
  \\ rpt AP_THM_TAC \\ AP_TERM_TAC
  \\ simp [state_component_equality]
