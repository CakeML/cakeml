  ...
  sexpop s =
    ...
    | Atom a =>
      (if a = «Equality» then SOME Equality else
       ...
       if a = «Aw8sub_unsafe» then SOME Aw8sub_unsafe else
       if a = «Aw8update_unsafe» then SOME Aw8update_unsafe else
       ...
       if a = «XorAw8Str_unsafe» then SOME XorAw8Str_unsafe else
       ...
       if a = «Asub_unsafe» then SOME Asub_unsafe else
       if a = «Aupdate_unsafe» then SOME Aupdate_unsafe else
       if a = «ForceThunk» then SOME (ThunkOp ForceThunk) else
       if a = «ConfigGC» then SOME ConfigGC else
       if a = «Eval» then SOME Eval else
       if a = «Env_id» then SOME Env_id else NONE)
    | Expr [Atom tag; arg] =>
...
Definition opsexp_def:
  ...
  (opsexp Aw8sub_unsafe = Atom «Aw8sub_unsafe») ∧
  (opsexp Aw8update_unsafe = Atom «Aw8update_unsafe») ∧
  ...
  (opsexp XorAw8Str_unsafe = Atom «XorAw8Str_unsafe») ∧
  ...
  (opsexp Asub_unsafe = Atom «Asub_unsafe») ∧
  (opsexp Aupdate_unsafe = Atom «Aupdate_unsafe») ∧
  (opsexp ConfigGC = Atom «ConfigGC») ∧
  (opsexp Eval = Atom «Eval») ∧
  (opsexp Env_id = Atom «Env_id») ∧
