...
val sexptype_alt_intro1 = Q.store_thm("sexptype_alt_intro1",
...
val sexppat_alt_intro1 = Q.store_thm("sexppat_alt_intro1",
...
val sexpop_def = Define`
  ...
  if s = "Opw8Andw" then SOME (Opw (WordSize 8) Andw) else
  if s = "Opw8Orw" then SOME (Opw (WordSize 8) Orw) else
  if s = "Opw8Xor" then SOME (Opw (WordSize 8) Xor) else
  if s = "Opw8Add" then SOME (Opw (WordSize 8) Add) else
  if s = "Opw8Sub" then SOME (Opw (WordSize 8) Sub) else
  if s = "Opw64Andw" then SOME (Opw (WordSize 64) Andw) else
  if s = "Opw64Orw" then SOME (Opw (WordSize 64) Orw) else
  if s = "Opw64Xor" then SOME (Opw (WordSize 64) Xor) else
  if s = "Opw64Add" then SOME (Opw (WordSize 64) Add) else
  if s = "Opw64Sub" then SOME (Opw (WordSize 64) Sub) else
  ...
  if s = "W8toInt" then SOME (WordToInt (WordSize 8)) else
  if s = "W64fromInt" then SOME (WordFromInt (WordSize 64)) else
  if s = "W64toInt" then SOME (WordToInt (WordSize 64)) else
  ...
  (sexpop (SX_CONS (SX_SYM s) (SX_NUM n)) =
    if s = "Shift8Lsl" then SOME (Shift (WordSize 8) Lsl n) else
    if s = "Shift8Lsr" then SOME (Shift (WordSize 8) Lsr n) else
    if s = "Shift8Asr" then SOME (Shift (WordSize 8) Asr n) else
    if s = "Shift8Ror" then SOME (Shift (WordSize 8) Ror n) else
    if s = "Shift64Lsl" then SOME (Shift (WordSize 64) Lsl n) else
    if s = "Shift64Lsr" then SOME (Shift (WordSize 64) Lsr n) else
    if s = "Shift64Asr" then SOME (Shift (WordSize 64) Asr n) else
    if s = "Shift64Ror" then SOME (Shift (WordSize 64) Ror n) else NONE) ∧
  (sexpop _ = NONE)`;
...
val sexpexp_alt_intro1 = Q.store_thm("sexpexp_alt_intro1",
  `sexpexp = sexpexp_alt ∧
...
val opsexp_def = Define`
  ...
  (opsexp (Opw (WordSize 8) Andw) = SX_SYM "Opw8Andw") ∧
  (opsexp (Opw (WordSize 8) Orw) = SX_SYM "Opw8Orw") ∧
  (opsexp (Opw (WordSize 8) Xor) = SX_SYM "Opw8Xor") ∧
  (opsexp (Opw (WordSize 8) Add) = SX_SYM "Opw8Add") ∧
  (opsexp (Opw (WordSize 8) Sub) = SX_SYM "Opw8Sub") ∧
  (opsexp (Opw (WordSize 64) Andw) = SX_SYM "Opw64Andw") ∧
  (opsexp (Opw (WordSize 64) Orw) = SX_SYM "Opw64Orw") ∧
  (opsexp (Opw (WordSize 64) Xor) = SX_SYM "Opw64Xor") ∧
  (opsexp (Opw (WordSize 64) Add) = SX_SYM "Opw64Add") ∧
  (opsexp (Opw (WordSize 64) Sub) = SX_SYM "Opw64Sub") ∧
  (opsexp (Shift (WordSize 8) Lsl n) = SX_CONS (SX_SYM "Shift8Lsl") (SX_NUM n)) ∧
  (opsexp (Shift (WordSize 8) Lsr n) = SX_CONS (SX_SYM "Shift8Lsr") (SX_NUM n)) ∧
  (opsexp (Shift (WordSize 8) Asr n) = SX_CONS (SX_SYM "Shift8Asr") (SX_NUM n)) ∧
  (opsexp (Shift (WordSize 8) Ror n) = SX_CONS (SX_SYM "Shift8Ror") (SX_NUM n)) ∧
  (opsexp (Shift (WordSize 64) Lsl n) = SX_CONS (SX_SYM "Shift64Lsl") (SX_NUM n)) ∧
  (opsexp (Shift (WordSize 64) Lsr n) = SX_CONS (SX_SYM "Shift64Lsr") (SX_NUM n)) ∧
  (opsexp (Shift (WordSize 64) Asr n) = SX_CONS (SX_SYM "Shift64Asr") (SX_NUM n)) ∧
  (opsexp (Shift (WordSize 64) Ror n) = SX_CONS (SX_SYM "Shift64Ror") (SX_NUM n)) ∧
  ...
  (opsexp (WordFromInt (WordSize 8)) = SX_SYM "W8fromInt") ∧
  (opsexp (WordToInt (WordSize 8)) = SX_SYM "W8toInt") ∧
  (opsexp (WordFromInt (WordSize 64)) = SX_SYM "W64fromInt") ∧
  (opsexp (WordToInt (WordSize 64)) = SX_SYM "W64toInt") ∧
