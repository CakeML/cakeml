...
val sexpop_def = Define`
  ...
  if s = "FPuopFPToWord" then SOME (FP_uop FP_ToWord) else
  if s = "FPuopFPFromWord" then SOME (FP_uop FP_FromWord) else
...
val opsexp_def = Define`
  ...
  (opsexp (FP_uop FP_ToWord) = SX_SYM "FPuopFPToWord") /\
  (opsexp (FP_uop FP_FromWord) = SX_SYM "FPuopFPFromWord") /\
