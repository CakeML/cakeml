...
open lem_pervasivesTheory libTheory astTheory semanticPrimitivesTheory lem_list_extraTheory bigStepTheory conLangTheory decLangTheory ffiTheory;
...
(*val get_tags : list pat_i2 -> nat_map nat_set -> maybe (nat_map nat_set)*)
 val _ = Define `
...
((case p of
      Pcon_i2 (SOME (tag,_)) ps' =>
        if EVERY is_unconditional ps' then
          let a = (LENGTH ps') in
          (case sptree$lookup a acc of
            SOME tags =>
              get_tags ps (sptree$insert a (sptree$delete tag tags) acc)
          | NONE => NONE
          )
        else NONE
    | _ => NONE
      ...
      Pcon_i2 (SOME (tag,TypeId t)) ps'::ps =>
        (EVERY is_unconditional ps' /\
          (case FLOOKUP exh t of
              NONE => F
            | SOME tags =>
              (case get_tags ps (sptree$map (\ n. sptree$fromList (GENLIST (\ _ .  () ) n)) tags) of
                NONE => F
              | SOME result =>
                  EVERY (\ s. s = sptree$LN) (sptree$toList result)
              )
          ))
    | _ => F
  )))`;
...
val _ = Define `
 (do_app_exh ((count,s,t),genv) op vs =  
((case (op, vs) of
      (Op_i2 (Opn op), [Litv_exh (IntLit n1); Litv_exh (IntLit n2)]) =>
        if ((op = Divide) \/ (op = Modulo)) /\ (n2 =( 0 : int)) then
          SOME (((count,s,t),genv), Rerr (Rraise (prim_exn_exh div_tag)))
        else
          SOME (((count,s,t),genv), Rval (Litv_exh (IntLit (opn_lookup op n1 n2))))
    | (Op_i2 (Opb op), [Litv_exh (IntLit n1); Litv_exh (IntLit n2)]) =>
        SOME (((count,s,t),genv), Rval (Boolv_exh (opb_lookup op n1 n2)))
    | (Op_i2 Equality, [v1; v2]) =>
        (case do_eq_exh v1 v2 of
            Eq_type_error => NONE
          | Eq_closure => SOME (((count,s,t),genv), Rerr (Rraise (prim_exn_exh eq_tag)))
          | Eq_val b => SOME (((count,s,t),genv), Rval (Boolv_exh b))
        )
    | (Op_i2 Opassign, [Loc_exh lnum; v]) =>
        (case store_assign lnum (Refv v) s of
            SOME st => SOME (((count,st,t),genv), Rval (Conv_exh tuple_tag []))
          | NONE => NONE
        )
    | (Op_i2 Opref, [v]) =>
        let (s',n) = (store_alloc (Refv v) s) in
          SOME (((count,s',t),genv), Rval (Loc_exh n))
    ...
    | (Init_global_var_i2 idx, [v]) =>
        if idx < LENGTH genv then
          (case EL idx genv of
              NONE => SOME (((count,s,t), LUPDATE (SOME v) idx genv), (Rval (Conv_exh tuple_tag [])))
            | SOME x => NONE
          )
        else
          NONE
    | (Op_i2 Aw8alloc, [Litv_exh (IntLit n); Litv_exh (Word8 w)]) =>
        if n <( 0 : int) then
          SOME (((count,s,t),genv), Rerr (Rraise (prim_exn_exh subscript_tag)))
        else
          let (s',lnum) =            
(store_alloc (W8array (REPLICATE (Num (ABS ( n))) w)) s)
          in 
            SOME (((count,s',t),genv), Rval (Loc_exh lnum))
              ...
              if i <( 0 : int) then
                ...
                let n = (Num (ABS ( i))) in
                  if n >= LENGTH ws then
                    ...
                    SOME (((count,s,t),genv), Rval (Litv_exh (Word8 (EL n ws))))
          | _ => NONE
        )
    | (Op_i2 Aw8length, [Loc_exh n]) =>
        (case store_lookup n s of
            SOME (W8array ws) =>
              SOME (((count,s,t),genv),Rval (Litv_exh(IntLit(int_of_num(LENGTH ws)))))
          | _ => NONE
         )
              ...
              let n = (Num (ABS ( i))) in
                if n >= LENGTH ws then
                  ...
                  (case store_assign lnum (W8array (LUPDATE w n ws)) s of
                      NONE => NONE
                    | SOME s' => SOME (((count,s',t),genv), Rval (Conv_exh tuple_tag []))
                  )
        | _ => NONE
      )
    | (Op_i2 Ord, [Litv_exh (Char c)]) =>
          SOME (((count,s,t),genv), Rval (Litv_exh(IntLit(int_of_num(ORD c)))))
...
(if (i <( 0 : int)) \/ (i >( 255 : int)) then
    ...
    | (Op_i2 (Chopb op), [Litv_exh (Char c1); Litv_exh (Char c2)]) =>
        SOME (((count,s,t),genv), Rval (Boolv_exh (opb_lookup op (int_of_num(ORD c1)) (int_of_num(ORD c2)))))
    | (Op_i2 Implode, [v]) =>
          (case v_exh_to_char_list v of
            SOME ls =>
              SOME (((count,s,t),genv), Rval (Litv_exh (StrLit (IMPLODE ls))))
          | NONE => NONE
          )
    | (Op_i2 Explode, [Litv_exh (StrLit str)]) =>
        SOME (((count,s,t),genv), Rval (char_list_to_v_exh (EXPLODE str)))
    | (Op_i2 Strlen, [Litv_exh (StrLit str)]) =>
        SOME (((count,s,t),genv), Rval (Litv_exh(IntLit(int_of_num(STRLEN str)))))
    | (Op_i2 VfromList, [v]) =>
          (case v_to_list_exh v of
              SOME vs =>
                SOME (((count,s,t),genv), Rval (Vectorv_exh vs))
            | NONE => NONE
          )
    ...
    | (Op_i2 Vlength, [Vectorv_exh vs]) =>
        SOME (((count,s,t),genv), Rval (Litv_exh (IntLit (int_of_num (LENGTH vs)))))
...
(evaluate_exh ck env s1 e (s2, Rerr (Rabort a)))
==>
evaluate_exh ck env s1 (Handle_exh e pes) (s2, Rerr (Rabort a)))
...
(evaluate_list_exh ck env (s1,genv) (REVERSE es) (((count,s2,t2),genv'), Rval vs) /\
(do_opapp_exh (REVERSE vs) = SOME (env', e)) /\
(ck ==> ~ (count =( 0))) /\
evaluate_exh ck env' (((if ck then count -  1 else count),s2,t2),genv') e bv)
==>
evaluate_exh ck env (s1,genv) (App_exh (Op_i2 Opapp) es) bv)

/\ (! ck env es vs env' e s1 s2 t2 count genv.
(evaluate_list_exh ck env s1 (REVERSE es) (((count,s2,t2), genv), Rval vs) /\
(do_opapp_exh (REVERSE vs) = SOME (env', e)) /\
(count = 0) /\
ck)
==>
evaluate_exh ck env s1 (App_exh (Op_i2 Opapp) es) ((( 0,s2,t2),genv), Rerr (Rabort Rtimeout_error)))
...
evaluate_exh ck env' ((count,s,t),genv) e bv)
==>
evaluate_match_exh ck env ((count,s,t),genv) v ((p,e)::pes) bv)

/\ (! ck genv env v p e pes bv s t count.
(ALL_DISTINCT (pat_bindings_exh p []) /\
(pmatch_exh s p v env = No_match) /\
evaluate_match_exh ck env ((count,s,t),genv) v pes bv)
==>
evaluate_match_exh ck env ((count,s,t),genv) v ((p,e)::pes) bv)`;
val _ = export_theory()
