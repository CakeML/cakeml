...
val _ = Define `
 (get_tagenv ((next,tagenv,inv13994),acc) = tagenv)`;
...
val _ = Define `
 (alloc_tag tn cn ((next,tagenv,inv13995),acc) =
  (((next+ 1),insert_tag_env cn (next,SOME tn) tagenv,inv13995 |+ (next, (cn,tn))),acc |+ (cn, (next,SOME tn))))`;
...
evaluate_prompt_i2 ck exh genv s1 (Prompt_i2 ds) (s2,                                           
 (MAP SOME env ++ GENLIST (\n7815 .  
  (case (n7815 ) of ( _ ) => NONE )) (num_defs ds - LENGTH env)),
                                           SOME err))`;
