...
val _ = Define `
 (get_tagenv ((next,tagenv,inv9694),acc) = tagenv)`;
...
val _ = Define `
 (alloc_tag tn cn ((next,tagenv,inv9695),acc) =
  (((next+ 1),insert_tag_env cn (next,SOME tn) tagenv,inv9695 |+ (next, (cn,tn))),acc |+ (cn, (next,SOME tn))))`;
...
evaluate_prompt_i2 ck exh genv s1 (Prompt_i2 ds) (s2,                                           
 (MAP SOME env ++ GENLIST (\n5580 .  
  (case (n5580 ) of ( _ ) => NONE )) (num_defs ds - LENGTH env)),
                                           SOME err))`;
