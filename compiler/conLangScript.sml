...
(*
val get_pcons : list pat -> maybe (list conN)
let rec get_pcons [] = Just []
and get_pcons (p::ps) =
  match p with
    | Pcon c ps' ->
        if List.all is_pvar ps' then
          match get_pcons ps with
            | Nothing -> Nothing
            | Just cons -> Just (id_to_n c::cons)
          end
        else
          Nothing
    | _ -> Nothing
...
val exhaustive_match : tag_env -> list pat -> bool
let exhaustive_match tagenv ps =
  match ps with
    | [Pvar _] -> true
    | (Pcon c ps'::ps) ->
      match get_pcons tagenv (Pcon c ps'::ps) with
          Nothing -> false
        | Just [] -> false
        | Just tags -> 
            Set.fromList (snd (lookup_tag_env c tagenv)) = Set.fromList tags
      end
  end
*)
...
(exp_to_i2 tagenv (Handle_i1 e pes) =  
 (Handle_i2 (exp_to_i2 tagenv e) (add_default_case T (exhaustive_match tagenv (MAP FST pes)) (pat_exp_to_i2 tagenv pes))))
/\
...
(Mat_i2 (exp_to_i2 tagenv e) (add_default_case F (exhaustive_match tagenv (MAP FST pes)) (pat_exp_to_i2 tagenv pes))))
/\
