...
open lem_pervasivesTheory libTheory astTheory semanticPrimitivesTheory lem_list_extraTheory bigStepTheory modLangTheory ffiTheory;
...
(*val do_app_i2 : store_trace v_i2 -> op_i2 -> list v_i2 -> maybe (store_trace v_i2 * result v_i2 v_i2)*)
val _ = Define `
 (do_app_i2 (s,t) op vs =  
((case (op, vs) of
      (Op_i2 (Opn op), [Litv_i2 (IntLit n1); Litv_i2 (IntLit n2)]) =>
        if ((op = Divide) \/ (op = Modulo)) /\ (n2 =( 0 : int)) then
          SOME ((s,t), Rerr (Rraise (prim_exn_i2 div_tag "Div")))
        else
          SOME ((s,t), Rval (Litv_i2 (IntLit (opn_lookup op n1 n2))))
    | (Op_i2 (Opb op), [Litv_i2 (IntLit n1); Litv_i2 (IntLit n2)]) =>
        SOME ((s,t), Rval (Boolv_i2 (opb_lookup op n1 n2)))
    | (Op_i2 Equality, [v1; v2]) =>
        (case do_eq_i2 v1 v2 of
            Eq_type_error => NONE
          | Eq_closure => SOME ((s,t), Rerr (Rraise (prim_exn_i2 eq_tag "Eq")))
          | Eq_val b => SOME ((s,t), Rval (Boolv_i2 b))
        )
    | (Op_i2 Opassign, [Loc_i2 lnum; v]) =>
        (case store_assign lnum (Refv v) s of
            SOME st => SOME ((st,t), Rval (Conv_i2 NONE []))
          | NONE => NONE
        )
    | (Op_i2 Opref, [v]) =>
        let (s',n) = (store_alloc (Refv v) s) in
          SOME ((s',t), Rval (Loc_i2 n))
    ...
    | (Op_i2 Aw8alloc, [Litv_i2 (IntLit n); Litv_i2 (Word8 w)]) =>
        if n <( 0 : int) then
          SOME ((s,t), Rerr (Rraise (prim_exn_i2 subscript_tag "Subscript")))
        else
          let (s',lnum) =            
(store_alloc (W8array (REPLICATE (Num (ABS ( n))) w)) s)
          in 
            SOME ((s',t), Rval (Loc_i2 lnum))
              ...
              if i <( 0 : int) then
                ...
                let n = (Num (ABS ( i))) in
                  if n >= LENGTH ws then
                    ...
                    SOME ((s,t), Rval (Litv_i2 (Word8 (EL n ws))))
          | _ => NONE
        )
    | (Op_i2 Aw8length, [Loc_i2 n]) =>
        (case store_lookup n s of
            SOME (W8array ws) =>
              SOME ((s,t),Rval (Litv_i2(IntLit(int_of_num(LENGTH ws)))))
          | _ => NONE
         )
              ...
              let n = (Num (ABS ( i))) in
                if n >= LENGTH ws then
                  ...
                  (case store_assign lnum (W8array (LUPDATE w n ws)) s of
                      NONE => NONE
                    | SOME s' => SOME ((s',t), Rval (Conv_i2 NONE []))
                  )
        | _ => NONE
      )
    | (Op_i2 Ord, [Litv_i2 (Char c)]) =>
          SOME ((s,t), Rval (Litv_i2(IntLit(int_of_num(ORD c)))))
...
(if (i <( 0 : int)) \/ (i >( 255 : int)) then
    ...
    | (Op_i2 (Chopb op), [Litv_i2 (Char c1); Litv_i2 (Char c2)]) =>
        SOME ((s,t), Rval (Boolv_i2 (opb_lookup op (int_of_num(ORD c1)) (int_of_num(ORD c2)))))
    | (Op_i2 Implode, [v]) =>
          (case v_i2_to_char_list v of
            SOME ls =>
              SOME ((s,t), Rval (Litv_i2 (StrLit (IMPLODE ls))))
          | NONE => NONE
          )
    | (Op_i2 Explode, [Litv_i2 (StrLit str)]) =>
        SOME ((s,t), Rval (char_list_to_v_i2 (EXPLODE str)))
    | (Op_i2 Strlen, [Litv_i2 (StrLit str)]) =>
        SOME ((s,t), Rval (Litv_i2(IntLit(int_of_num(STRLEN str)))))
    | (Op_i2 VfromList, [v]) =>
          (case v_to_list_i2 v of
              SOME vs =>
                SOME ((s,t), Rval (Vectorv_i2 vs))
            | NONE => NONE
          )
    ...
    | (Op_i2 Vlength, [Vectorv_i2 vs]) =>
        SOME ((s,t), Rval (Litv_i2 (IntLit (int_of_num (LENGTH vs)))))
...
(evaluate_i2 ck env s1 e (s2, Rerr (Rabort a)))
==>
evaluate_i2 ck env s1 (Handle_i2 e pes) (s2, Rerr (Rabort a)))
...
(evaluate_list_i2 ck (exh,genv,env) s1 (REVERSE es) ((count,s2,t2), Rval vs) /\
(do_opapp_i2 (REVERSE vs) = SOME (env', e)) /\
(ck ==> ~ (count =( 0))) /\
evaluate_i2 ck (exh,genv,env') ((if ck then count -  1 else count),s2,t2) e bv)
==>
evaluate_i2 ck (exh,genv,env) s1 (App_i2 (Op_i2 Opapp) es) bv)

/\ (! ck env es vs env' e s1 s2 t2 count.
(evaluate_list_i2 ck env s1 (REVERSE es) ((count,s2,t2), Rval vs) /\
(do_opapp_i2 (REVERSE vs) = SOME (env', e)) /\
(count = 0) /\
ck)
==>
evaluate_i2 ck env s1 (App_i2 (Op_i2 Opapp) es) (( 0,s2,t2), Rerr (Rabort Rtimeout_error)))

/\ (! ck env op es vs res s1 s2 s3 t2 t3 count.
(evaluate_list_i2 ck env s1 (REVERSE es) ((count,s2,t2), Rval vs) /\
(do_app_i2 (s2,t2) op (REVERSE vs) = SOME ((s3,t3), res)) /\
(op <> Op_i2 Opapp))
==>
evaluate_i2 ck env s1 (App_i2 op es) ((count,s3,t3), res))
...
evaluate_i2 ck (exh,genv,env') (count,s,t) e bv)
==>
evaluate_match_i2 ck (exh,genv,env) (count,s,t) v ((p,e)::pes) err_v bv)

/\ (! ck exh genv env v p e pes bv s t count err_v.
(ALL_DISTINCT (pat_bindings_i2 p []) /\
(pmatch_i2 exh s p v env = No_match) /\
evaluate_match_i2 ck (exh,genv,env) (count,s,t) v pes err_v bv)
==>
evaluate_match_i2 ck (exh,genv,env) (count,s,t) v ((p,e)::pes) err_v bv)`;
