...
open lem_pervasivesTheory libTheory astTheory semanticPrimitivesTheory lem_list_extraTheory bigStepTheory ffiTheory;
...
(*val do_app_i1 : store_trace v_i1 -> op -> list v_i1 -> maybe (store_trace v_i1 * result v_i1 v_i1)*)
val _ = Define `
 (do_app_i1 (s,t) op vs =  
((case (op, vs) of
      (Opn op, [Litv_i1 (IntLit n1); Litv_i1 (IntLit n2)]) =>
        if ((op = Divide) \/ (op = Modulo)) /\ (n2 =( 0 : int)) then
          SOME ((s,t), Rerr (Rraise (prim_exn_i1 "Div")))
        else
          SOME ((s,t), Rval (Litv_i1 (IntLit (opn_lookup op n1 n2))))
    | (Opb op, [Litv_i1 (IntLit n1); Litv_i1 (IntLit n2)]) =>
        SOME ((s,t), Rval (Boolv_i1 (opb_lookup op n1 n2)))
    | (Equality, [v1; v2]) =>
        (case do_eq_i1 v1 v2 of
            Eq_type_error => NONE
          | Eq_closure => SOME ((s,t), Rerr (Rraise (prim_exn_i1 "Eq")))
          | Eq_val b => SOME ((s,t), Rval (Boolv_i1 b))
        )
    | (Opassign, [Loc_i1 lnum; v]) =>
        (case store_assign lnum (Refv v) s of
            SOME st => SOME ((st,t), Rval (Conv_i1 NONE []))
          | NONE => NONE
        )
    | (Opref, [v]) =>
        let (s',n) = (store_alloc (Refv v) s) in
          SOME ((s',t), Rval (Loc_i1 n))
    | (Opderef, [Loc_i1 n]) =>
    ...
    | (Aw8alloc, [Litv_i1 (IntLit n); Litv_i1 (Word8 w)]) =>
        if n <( 0 : int) then
          SOME ((s,t), Rerr (Rraise (prim_exn_i1 "Subscript")))
        else
          let (s',lnum) =            
(store_alloc (W8array (REPLICATE (Num (ABS ( n))) w)) s)
          in 
            SOME ((s',t), Rval (Loc_i1 lnum))
              ...
              if i <( 0 : int) then
                ...
                let n = (Num (ABS ( i))) in
                  if n >= LENGTH ws then
                    ...
                    SOME ((s,t), Rval (Litv_i1 (Word8 (EL n ws))))
          | _ => NONE
        )
    | (Aw8length, [Loc_i1 n]) =>
        (case store_lookup n s of
            SOME (W8array ws) =>
              SOME ((s,t),Rval (Litv_i1(IntLit(int_of_num(LENGTH ws)))))
          | _ => NONE
         )
              ...
              let n = (Num (ABS ( i))) in
                if n >= LENGTH ws then
                  ...
                  (case store_assign lnum (W8array (LUPDATE w n ws)) s of
                      NONE => NONE
                    | SOME s' => SOME ((s',t), Rval (Conv_i1 NONE []))
                  )
        | _ => NONE
        )
    | (Ord, [Litv_i1 (Char c)]) =>
          SOME ((s,t), Rval (Litv_i1(IntLit(int_of_num(ORD c)))))
...
(if (i <( 0 : int)) \/ (i >( 255 : int)) then
    ...
    | (Chopb op, [Litv_i1 (Char c1); Litv_i1 (Char c2)]) =>
        SOME ((s,t), Rval (Boolv_i1 (opb_lookup op (int_of_num(ORD c1)) (int_of_num(ORD c2)))))
    | (Implode, [v]) =>
          (case v_i1_to_char_list v of
            SOME ls =>
              SOME ((s,t), Rval (Litv_i1 (StrLit (IMPLODE ls))))
          | NONE => NONE
          )
    | (Explode, [Litv_i1 (StrLit str)]) =>
        SOME ((s,t), Rval (char_list_to_v_i1 (EXPLODE str)))
    | (Strlen, [Litv_i1 (StrLit str)]) =>
        SOME ((s,t), Rval (Litv_i1(IntLit(int_of_num(STRLEN str)))))
    | (VfromList, [v]) =>
          (case v_to_list_i1 v of
              SOME vs =>
                SOME ((s,t), Rval (Vectorv_i1 vs))
            | NONE => NONE
          )
    ...
    | (Vlength, [Vectorv_i1 vs]) =>
        SOME ((s,t), Rval (Litv_i1 (IntLit (int_of_num (LENGTH vs)))))
...
(evaluate_i1 ck env s1 e (s2, Rerr (Rabort a)))
==>
evaluate_i1 ck env s1 (Handle_i1 e pes) (s2, Rerr (Rabort a)))
...
(evaluate_list_i1 ck env s1 (REVERSE es) ((count,s2,t2), Rval vs) /\
(do_opapp_i1 (all_env_i1_to_genv env) (REVERSE vs) = SOME (env', e)) /\
(ck ==> ~ (count =( 0))) /\
evaluate_i1 ck env' ((if ck then count -  1 else count),s2,t2) e bv)
==>
...
ck)
==>
evaluate_i1 ck env s1 (App_i1 Opapp es) (( 0,s2,t2), Rerr (Rabort Rtimeout_error)))

/\ (! ck env op es vs res s1 s2 s3 t2 t3 count.
(evaluate_list_i1 ck env s1 (REVERSE es) ((count,s2,t2), Rval vs) /\
(do_app_i1 (s2,t2) op (REVERSE vs) = SOME ((s3,t3), res)) /\
(op <> Opapp))
==>
evaluate_i1 ck env s1 (App_i1 op es) ((count,s3,t3), res))
...
evaluate_i1 ck (genv,cenv,env') (count,s,t) e bv)
==>
evaluate_match_i1 ck (genv,cenv,env) (count,s,t) v ((p,e)::pes) err_v bv)

/\ (! ck genv cenv env v p e pes bv s t count err_v.
(ALL_DISTINCT (pat_bindings p []) /\
(pmatch_i1 cenv s p v env = No_match) /\
evaluate_match_i1 ck (genv,cenv,env) (count,s,t) v pes err_v bv)
==>
evaluate_match_i1 ck (genv,cenv,env) (count,s,t) v ((p,e)::pes) err_v bv)`;
...
(*val no_dup_mods_i1 : list prompt_i1 -> count_store_trace v_i1 * set tid_or_exn * set modN -> bool*)
val _ = Define `
...
(*val no_dup_top_types_i1 : list prompt_i1 -> count_store_trace v_i1 * set tid_or_exn * set modN -> bool*)
val _ = Define `
...
(*val evaluate_whole_prog_i1 : bool -> list (maybe v_i1) -> envC -> count_store_trace v_i1 * set tid_or_exn * set modN -> list prompt_i1 -> (count_store_trace v_i1 * set tid_or_exn * set modN) * envC * list (maybe v_i1) * maybe (error_result v_i1) -> bool*)
val _ = Define `
