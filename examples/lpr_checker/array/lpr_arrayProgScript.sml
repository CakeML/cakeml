...
Theorem reindex_hash_spec:
  ...
  app (p : 'ffi ffi_proj)
    ...
    [fmlv; lsv; hv]
    (HASHTABLE (LIST_TYPE INT) UNIT_TYPE hash_func order_lists h hv * ARRAY fmlv fmllsv)
    (POSTv uv.
      &(UNIT_TYPE () uv) *
      HASHTABLE (LIST_TYPE INT) UNIT_TYPE hash_func order_lists
       (h |++ MAP (\x. (x,())) (SND (reindex fmlls ls))) hv *
      ARRAY fmlv fmllsv)
Proof
  Induct>>rw[reindex_def]>>
  ...
  xlet_auto
  >-
    (xsimpl>> qexists_tac`ARRAY fmlv fmllsv`>>qexists_tac`h'`>>xsimpl)>>
  xapp>>
  simp[]>>xsimpl>>
  qexists_tac`emp`>>
  qexists_tac`h' |+ (x,())`>>
  xsimpl>>
  Cases_on`reindex fmlls ls`>>simp[]>>
  simp[FUPDATE_LIST_THM]>>
  xsimpl
QED
...
Theorem hash_contains_spec:
  ...
  app (p : 'ffi ffi_proj)
    ...
    [hv; clsv]
    (HASHTABLE (LIST_TYPE INT) UNIT_TYPE hash_func order_lists h hv)
    (POSTv bv.
...
Proof
  ...
  xmatch>>
  rpt xlet_autop>>
  xlet`POSTv v. HASHTABLE (LIST_TYPE INT) UNIT_TYPE hash_func order_lists FEMPTY v * ARRAY fmlv fmllsv`
  >- (
    xapp_spec (HashtableProofTheory.hashtable_empty_spec|>INST_TYPE[alpha|->``:int list``,beta |-> ``:unit``])>>
    ...
    qexists_tac`UNIT_TYPE`>>xsimpl>>
    simp[theorem "hash_func_v_thm"])>>
  xlet_autop>>
  xapp>>
  qexists_tac`ARRAY fmlv fmllsv`>>xsimpl>>
  qexists_tac`(FEMPTY |++ MAP (λx. (x,())) (SND (reindex fmlls ls)))`>>
  qexists_tac`h::h'::t'`>>simp[LIST_TYPE_def]>>
  xsimpl>>
  Cases_on`reindex fmlls ls`>>simp[FDOM_FUPDATE_LIST,MEM_MAP,EXISTS_PROD,SUBSET_DEF,EVERY_MEM]
QED
...
Proof
  ...
  \\ qexists_tac`x`
  \\ xsimpl
  \\ rw[check_unsat_sem_def,check_unsat_1_sem_def,check_unsat_2_sem_def,check_unsat_3_sem_def]
  \\ every_case_tac
