...
*)
Definition parse_single_vb_num_aux_def:
  parse_single_vb_num_aux (str:mlstring) (i:num) (len:num) (ex:num) (n:num) =
  if i <= len then
    let v = ORD (strsub str i) in
      if v >= 128 then (* msb is set *)
        parse_single_vb_num_aux str (i+1) len (ex*128) ((v-128)*ex+n)
      else
        (v*ex+n,i + 1)
  else (0, i)
Termination
...
Definition parse_single_vb_num_def:
  parse_single_vb_num x offset len =
  parse_single_vb_num_aux x offset len 1 0
End
...
Proof
  ...
  List.rev [`x`, `i`, `len`, `ex`, `n`,`v`,`offset`,`offset'`] >>
  ho_match_mp_tac parse_single_vb_num_aux_ind >>
  rpt gen_tac  >> strip_tac >>
  simp[Once parse_single_vb_num_aux_def] >>
  rw[] >> fs[] >>
  fs[Once parse_single_vb_num_aux_def] >>
  gvs[AllCaseEqs()]
QED
...
Termination
  WF_REL_TAC `measure (λ(x,offset,len).len - offset)` >>
  CONV_TAC (ONCE_DEPTH_CONV SYM_CONV) >>
  rewrite_tac [parse_single_vb_num_def] >>
  rpt (gen_tac ORELSE disch_then strip_assume_tac) >>
  drule_at (Pos last) parse_single_vb_num_aux_offset >>
  fs[]
End

(* strlen x - 1 max index strlen x - 1 - 1 ignore last char*)
Definition parse_vb_string_def:
  parse_vb_string x = parse_vb_string_aux x 0 (strlen x - 1 - 1)
End
...
Definition parse_vb_PR_hint_def:
  ...
  case parse_vb_until_nn xs [] of (n,clause,rest) =>
    parse_vb_PR_hint n rest ((id,clause)::acc)
Termination
  WF_REL_TAC `measure (\(id, xs, acc). if id = 0 then 0 else (LENGTH xs) + 1)`>>
  rw[]>>
