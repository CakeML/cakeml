...
Definition check_red_def:
  check_red pres (ordsub :ord_s option) obj b tcb fml id c (s: subst_raw )
    (pfs:scope) idopt =
  if check_pres pres s then
    ( let nc = not c in
      ...
      let (rsubs,rscopes) = red_subgoals ordsub w c obj hs in
          ...
          (case idopt of
            NONE =>
              ...
              let gfml = mk_core_fml b fml in
                ...
                (hs ∨ ¬ untouched ⇒
                  check_fresh_aspo fml c obj s ordsub) ∧
...
Definition check_sstep_def:
  check_sstep sstep (pres : num_set option) ordsub obj tcb
    (fml:pbf) (id:num) =
  case sstep of
    Lstep lstep => check_lstep lstep F fml id
  | Red c s pfs idopt =>
    case check_red pres ordsub obj F tcb fml id c s pfs idopt of
...
Theorem extract_scopes_MEM_INL:
  ∀scopes pfs s b fml sg res id pf n.
  extract_scopes scopes pfs s b fml sg = SOME res ∧
  MEM (sc,spf) pfs ∧
  MEM (SOME (INL id,n), pf) spf ⇒
  ∃scs cpfs c.
    mk_scope scopes sc = SOME scs ∧
    lookup_core_only b fml id = SOME c ∧
    MEM (scs,cpfs) res ∧
    MEM (SOME ([not (subst s c)],n),pf) cpfs
Proof
  ho_match_mp_tac extract_scopes_ind>>
  rw[extract_scopes_def]>>
  gvs[AllCaseEqs()]
  >- (
...
Theorem extract_scopes_MEM_INR:
  ∀scopes pfs s b fml sg res id pf n.
  extract_scopes scopes pfs s b fml sg = SOME res ∧
  MEM (sc,spf) pfs ∧
  MEM (SOME (INR id,n), pf) spf ⇒
  ∃scs cpfs.
    id < LENGTH sg ∧
    mk_scope scopes sc = SOME scs ∧
    MEM (scs,cpfs) res ∧
    MEM (SOME (EL id sg,n),pf) cpfs
Proof
  ho_match_mp_tac extract_scopes_ind>>
  rw[extract_scopes_def]>>
  gvs[AllCaseEqs()]
...
Theorem lookup_extract_scoped_pids_l:
  ∀ls accl accr l r.
  extract_scoped_pids ls accl accr = (l,r) ∧
  lookup i l ≠ NONE ⇒
  lookup i accl ≠ NONE ∨
  ∃scopt spf n pf.
    extract_scope_val scopt ∧
    MEM (scopt,spf) ls ∧
    MEM (SOME (INL i,n),pf) spf
Proof
  ho_match_mp_tac extract_scoped_pids_ind>>
  rw[extract_scoped_pids_def]
  >- (
    pairarg_tac>>gvs[]>>
    metis_tac[lookup_extract_pids_l,PAIR])>>
  metis_tac[lookup_extract_pids_l,PAIR]
QED

Theorem lookup_extract_scoped_pids_r:
  ∀ls accl accr l r.
  extract_scoped_pids ls accl accr = (l,r) ∧
  lookup i r ≠ NONE ⇒
  lookup i accr ≠ NONE ∨
  ∃scopt spf n pf.
    extract_scope_val scopt ∧
    MEM (scopt,spf) ls ∧
    MEM (SOME (INR i,n),pf) spf
Proof
  ho_match_mp_tac extract_scoped_pids_ind>>
  rw[extract_scoped_pids_def]
  >- (
    pairarg_tac>>gvs[]>>
    metis_tac[lookup_extract_pids_r,PAIR])>>
  metis_tac[lookup_extract_pids_r,PAIR]
QED
...
Theorem sat_obj_po_esc_F:
  sat_obj_po_esc pres aspoopt fopt (λw. F) s t ⇔
  sat_obj_po pres aspoopt fopt s t
Proof
...
Proof
  rw[EXTENSION]>>
  gvs[dom_subst_def,LIST_TO_SET_MAP,lookup_list_list_insert,vec_lookup_num_man_to_vec]
QED

Theorem good_aspo_imp_po_of_aspo_refl:
  good_aspo x ⇒
  po_of_aspo x w w
Proof
  PairCases_on`x`>>rw[good_aspo_def]>>
  gvs[reflexive_def]
QED
...
Theorem substitution_redundancy_obj_po_spec:
  OPTION_ALL (λord1.
    ...
    (~untouched_order w (SND ord1) ⇒ f ∪ {not c} ∪ set gs ⊨ set fs)) (OPTION_MAP FST ordsub) ∧
  OPTION_ALL good_aspo_subst ordsub ∧
  (∀x. x ∈ pres ⇒ w x = NONE) ∧
  dom_subst b w ordsub = (fs,gs) ∧
  f ∪ {not c} ∪ set gs ⊨ ((f ∪ {c}) ⇂ w ∪
    (case obj of NONE => {}
      | SOME obj => {obj_constraint w obj}))
  ⇒
  redundant_wrt_obj_po f pres (OPTION_MAP FST ordsub) obj c
Proof
  ...
  Cases_on`ordsub`>>gvs[dom_subst_def]
  >- (
...
Theorem check_red_correct_extra:
  ...
  OPTION_ALL good_aspo_subst ordsub ∧
  (tcb ⇒ core_only_fml T fml ⊨ core_only_fml b fml) ∧
  check_red (pres: num_set option) ordsub obj b tcb fml id
  ...
  | NONE =>
    sat_obj_po (pres_set_spt pres ∪ extra) (OPTION_MAP FST ordsub) obj
...
Proof
  ...
  qsuff_tac ‘redundant_wrt_obj_po (core_only_fml (b ∨ tcb) fml)
                (pres_set_spt pres ∪ extra) (OPTION_MAP FST ordsub) obj c’
  >- (
...
Theorem check_sstep_correct:
  ...
  case check_sstep step pres ordsub obj tcb fml id of
  | SOME (fml',id') =>
      id ≤ id' ∧
      id_ok fml' id' ∧
      sat_obj_po (pres_set_spt pres) (OPTION_MAP FST ordsub) obj
        (core_only_fml F fml)
        (core_only_fml F fml') ∧
      sat_obj_po (pres_set_spt pres) (OPTION_MAP FST ordsub) obj
        (core_only_fml T fml')
  ...
  | NONE => T
Proof
  Cases_on`step`>>rw[check_sstep_def]
  >- (
  ...
  >- ( (* Red *)
    ...
    >- (
      ...
      >- (
        ...
        Cases_on`ordsub`>>
        fs[good_aspo_subst_def]>>
        metis_tac[satisfies_SUBSET,core_only_fml_T_SUBSET_F, good_aspo_imp_po_of_aspo_refl])
      >- (
        rw[]>>qexists_tac`w`>>fs[]>>
        Cases_on`ordsub`>>
        fs[good_aspo_subst_def]>>
    ...
    CONJ_TAC >- (
      ...
      rw[SUBSET_DEF]>>
      Cases_on`ordsub`>>gvs[good_aspo_subst_def])>>
    rw[]>>fs[]>>
...
Theorem good_aspo_imp_po_of_aspo_trans:
  good_aspo x ∧
  po_of_aspo x w w' ∧
  po_of_aspo x w' w'' ⇒
  po_of_aspo x w w''
...
Proof
  ...
  Cases_on`ord`>>fs[]>>
  metis_tac[good_aspo_imp_po_of_aspo_trans]
QED
...
Datatype:
  cstep =
  (* Derivation steps *)
  | Dom npbc subst_raw scope (num option)
  | Sstep sstep

  (* Deletion steps *)
  | CheckedDelete num subst_raw scope (num option)
      ...
      SOME (v,b) =>
        SOME (
            mk_bit_lit b
              ...
              | SOME res => res))
    | NONE =>
        OPTION_MAP (INR o mk_lit) (vec_lookup us_xs n))
  in
...
Definition trans_subst_def:
  (trans_subst (f,g,us,vs,as) ws bs cs =
  let us_vs = list_list_insert us vs in
  let vs_ws = list_list_insert vs ws in
  let as_bs = list_list_insert as bs in
  let as_cs = list_list_insert as cs in
  let lsubst =
    (λn.
      case lookup n us_vs of
        SOME x => SOME (INR (Pos x))
      | NONE =>
        case lookup n vs_ws of
          SOME x => SOME (INR (Pos x))
        | NONE =>
          case lookup n as_bs of
            SOME x => SOME (INR (Pos x))
          | NONE => NONE) in
  let rsubst =
    (λn.
      case lookup n vs_ws of
        SOME x => SOME (INR (Pos x))
      | NONE =>
        case lookup n as_cs of
          SOME x => SOME (INR (Pos x))
        | NONE => NONE) in
  let lhs =
    g ++ MAP (subst lsubst) g ++
    MAP (subst rsubst) g ++
    f ++ MAP (subst lsubst) f in
  let rhs = MAP (subst rsubst) f in
  (lhs,rhs))
End
...
Definition check_reflexivity_def:
  ...
  let (lhs,rhs) = refl_subst ord in
  let fml = build_fml F id lhs in
  let id = LENGTH lhs + id in
...
Datatype:
  proof_conf =
    <|
     ...
     ; enum : num          (* number of solutions enumerated *)
     ; ord : ord_s option
...
Theorem SUM_GE_1[local]:
 1 ≤
 SUM
   (MAP (λv. eval_term w (if wb (f v) then -1 else 1,f v)) lits)
  ⇔
  ∃v. v ∈ set lits ∧ w (f v) ≠ wb (f v)
Proof
  Induct_on`lits`>>fs[oneline b2n_def]>>
  rw[]>>
  gvs[]>>
  metis_tac[]
QED
...
Definition check_cstep_dom_def:
  check_cstep_dom c s pfs idopt (fml:pbf) (pc:proof_conf) =
  (case pc.ord of
    NONE => NONE
  | SOME aspo =>
  if check_pres pc.pres s ∧ check_fresh_aspo fml c pc.obj s pc.ord then
  ( let nc = not c in
...
Definition check_cstep_sstep_def:
  check_cstep_sstep sstep (fml:pbf) (pc:proof_conf) =
...
Definition check_cstep_checkeddelete_def:
  check_cstep_checkeddelete n s pfs idopt (fml:pbf) (pc:proof_conf) =
...
Definition check_cstep_uncheckeddelete_def:
  check_cstep_uncheckeddelete ls (fml:pbf) (pc:proof_conf) =
...
Definition check_cstep_transfer_def:
  check_cstep_transfer ls (fml:pbf) (pc:proof_conf) =
  (case do_transfer fml ls of
    NONE => NONE
  | SOME fml' => SOME (fml', pc))
End

Definition check_cstep_strengthentocore_def:
  check_cstep_strengthentocore b (fml:pbf) (pc:proof_conf) =
    SOME (
      if b then map (λ(c,b). (c,T)) fml else fml,
      pc with tcb := b)
End

Definition check_cstep_loadorder_def:
  check_cstep_loadorder name xs (fml:pbf) (pc:proof_conf) =
  (case ALOOKUP pc.orders name of
    NONE => NONE
  | SOME ord' =>
    if guard_ord_t ord' xs
    then
      SOME (
        map (λ(c,b). (c,T)) fml,
        pc with ord := mk_ordsub ord' xs)
    else NONE)
End

Definition check_cstep_unloadorder_def:
  check_cstep_unloadorder (fml:pbf) (pc:proof_conf) =
  (case pc.ord of
    NONE => NONE
  | SOME spo =>
...
Definition check_cstep_storeorder_def:
  check_cstep_storeorder name vars gspec f pfsr pfst (fml:pbf) (pc:proof_conf) =
...
Definition check_cstep_obj_def:
  check_cstep_obj w mi bopt (fml:pbf) (pc:proof_conf) =
...
Definition check_cstep_changeobj_def:
  check_cstep_changeobj b fc' pfs (fml:pbf) (pc:proof_conf) =
...
Definition check_cstep_checkobj_def:
  check_cstep_checkobj fc' (fml:pbf) (pc:proof_conf) =
  if check_eq_obj pc.obj fc'
  then SOME (fml,pc)
  else NONE
End

Definition check_cstep_assertobj_def:
  check_cstep_assertobj i (fml:pbf) (pc:proof_conf) =
  if pc.obj ≠ NONE then
...
Definition check_cstep_changepres_def:
  check_cstep_changepres b v c pfs (fml:pbf) (pc:proof_conf) =
...
Definition check_cstep_sol_def:
  check_cstep_sol w (fml:pbf) (pc:proof_conf) =
...
Definition check_cstep_checkpres_def:
  check_cstep_checkpres ls' (fml:pbf) (pc:proof_conf) =
...
Definition check_cstep_def:
  (check_cstep cstep
    ...
    (pc : proof_conf) =
  case cstep of
  | Dom c s pfs idopt => check_cstep_dom c s pfs idopt fml pc
  | Sstep sstep => check_cstep_sstep sstep fml pc
  | CheckedDelete n s pfs idopt =>
      check_cstep_checkeddelete n s pfs idopt fml pc
  | UncheckedDelete ls => check_cstep_uncheckeddelete ls fml pc
  | Transfer ls => check_cstep_transfer ls fml pc
  | StrengthenToCore b => check_cstep_strengthentocore b fml pc
  | LoadOrder name xs => check_cstep_loadorder name xs fml pc
  | UnloadOrder => check_cstep_unloadorder fml pc
  | StoreOrder name vars gspec f pfsr pfst =>
      check_cstep_storeorder name vars gspec f pfsr pfst fml pc
  | Obj w mi bopt => check_cstep_obj w mi bopt fml pc
  | ChangeObj b fc' pfs => check_cstep_changeobj b fc' pfs fml pc
  | CheckObj fc' => check_cstep_checkobj fc' fml pc
  | AssertObj i => check_cstep_assertobj i fml pc
  | ChangePres b v c pfs => check_cstep_changepres b v c pfs fml pc
  | Sol w => check_cstep_sol w fml pc
  | CheckPres ls' => check_cstep_checkpres ls' fml pc
  )
End
...
Definition bimp_pres_def:
  bimp_pres pres1 ex1 C1 pres2 ex2 C2 ⇔
  ∃f.
    (
    INJ f
      (proj_pres pres1 {w | satisfies w C1} DIFF ex1)
      (proj_pres pres2 {w' | satisfies w' C2} DIFF ex2)
    )
End
...
Theorem bimp_pres_refl[simp]:
  bimp_pres pres n X pres n X
Proof
  rw[bimp_pres_def]>>
  metis_tac[INJ_ID]
QED
...
Theorem sat_obj_po_bimp_pres:
  sat_obj_po pres ord obj A B ⇒
  bimp_pres pres n A pres n B
Proof
...
Theorem all_core_valid_conf:
  OPTION_ALL good_aspo aspo ∧
  all_core fml ⇒
  valid_conf chk pres aspo obj tcb fml
Proof
  rw[valid_conf_def]>>
...
Theorem check_obj_imp:
  check_obj obj s ls b = SOME (v,ww) ⇒
  satisfies ww (set ls) ∧
  eval_obj obj ww = v
Proof
...
Theorem bimp_pres_update_pres_1:
  pres_only c pres v ∧
  (∀w.
      satisfies w (core_only_fml T fml) ⇒
      v_iff_npbc_sem v c w) ⇒
...
(* from updated to pre-updated *)
Theorem bimp_pres_update_pres_2:
  pres_only c pres v ∧
  (∀w.
      satisfies w (core_only_fml T fml) ⇒
      v_iff_npbc_sem v c w) ⇒
...
Theorem OPTION_ALL_good_aspo_subst_good_aspo:
  OPTION_ALL good_aspo_subst ords ⇒
  OPTION_ALL good_aspo (OPTION_MAP FST ords)
Proof
  Cases_on`ords`>>rw[good_aspo_subst_def]
QED

Theorem bimp_pres_obj_NONE:
  opt_le bound (SOME 0) ⇒
  (bimp_pres_obj bound pres1 NONE C1 pres2 NONE C2)
Proof
  rw[bimp_pres_obj_def,eval_obj_def,opt_le_def,oneline opt_lt_def]>>
  gvs[AllCasePreds()]>>
...
Theorem if_opt_lt_opt_le[simp]:
  opt_le (if opt_lt (SOME v) b then SOME v else b) (SOME v)
Proof
  rw[oneline opt_lt_def,opt_le_def]>>
  every_case_tac>>gvs[]>>
  intLib.ARITH_TAC
QED

Theorem FINITE_pres_set_spt:
  FINITE (pres_set_spt pres)
Proof
  rw[oneline pres_set_spt_def]>>
  TOP_CASE_TAC>>fs[]
QED
...
Proof
  ...
  `FINITE {s | s ⊆ pres_set_spt pres}` by
    ...
    metis_tac[FINITE_pres_set_spt])>>
  metis_tac[SUBSET_FINITE]
QED

Theorem ns_CARD0[local,simp]:
  ns ⊆ proj_pres (pres_set_spt pres) s ∧ CARD ns = 0 ∧ P
  ⇔
  ns = {} ∧ P
Proof
  eq_tac>>rw[]>>
...
Definition cstep_ok_def:
  cstep_ok fml pc fml' pc' ⇔
  pc.id ≤ pc'.id ∧
  id_ok fml' pc'.id ∧
  valid_conf pc'.chk pc'.pres (OPTION_MAP FST pc'.ord) pc'.obj pc'.tcb fml' ∧
...
(* From the structural facts about a step — how the two core views move and
   the order relation between them — together with unchanged configuration
   fields, to check_cstep_correct's conclusion. *)
Theorem cstep_ok_of_str:
  OPTION_ALL good_aspo_subst pc.ord ∧
  EVERY (good_aord_t o SND) pc.orders ∧
  valid_conf pc.chk pc.pres (OPTION_MAP FST pc.ord) pc.obj pc.tcb fml ∧
  conf_eq pc pc' ∧
...
Proof
  ...
  strip_tac>>
  gvs[valid_conf_def,opt_le_def]>>
  rw[]>>
  metis_tac[sat_obj_po_trans,sat_obj_po_bimp_pres,sat_obj_po_bimp_pres_obj]
QED
...
Theorem check_cstep_strengthentocore_str:
  ...
  id_ok fml pc.id ∧
  OPTION_ALL good_aspo_subst pc.ord ⇒
  case check_cstep_strengthentocore b fml pc of NONE => T
  | SOME (fml',pc') =>
    pc' = pc with tcb := b ∧
    id_ok fml' pc'.id ∧
    sat_obj_po (pres_set_spt pc.pres) (OPTION_MAP FST pc.ord) pc.obj
...
Proof
  rpt gen_tac>>
  strip_tac>>
  simp[check_cstep_strengthentocore_def]>>
  drule OPTION_ALL_good_aspo_subst_good_aspo>>
  strip_tac>>
  IF_CASES_TAC>>
...
Theorem check_cstep_sstep_str:
  ...
  OPTION_ALL good_aspo_subst pc.ord ∧
  (pc.tcb ⇒ core_only_fml T fml ⊨ core_only_fml F fml) ⇒
  case check_cstep_sstep s fml pc of NONE => T
  | SOME (fml',pc') =>
...
Theorem check_cstep_transfer_str:
  ...
  OPTION_ALL good_aspo_subst pc.ord ∧
  (pc.tcb ⇒ core_only_fml T fml ⊨ core_only_fml F fml) ⇒
  case check_cstep_transfer ls fml pc of NONE => T
  | SOME (fml',pc') =>
    conf_eq pc pc' ∧ pc.tcb = pc'.tcb ∧
    pc.id ≤ pc'.id ∧
    id_ok fml' pc'.id ∧
...
Theorem check_cstep_checkeddelete_str:
  ...
  OPTION_ALL good_aspo_subst pc.ord ∧
  (pc.tcb ⇒ core_only_fml T fml ⊨ core_only_fml F fml) ⇒
  case check_cstep_checkeddelete n s pfs idopt fml pc of NONE => T
  | SOME (fml',pc') =>
    conf_eq pc pc' ∧ pc.tcb = pc'.tcb ∧
    pc.id ≤ pc'.id ∧
    id_ok fml' pc'.id ∧
...
Proof
  rpt gen_tac>>
  strip_tac>>
  simp[check_cstep_checkeddelete_def]>>
  every_case_tac>>simp[]>>
...
Theorem check_cstep_dom_str:
  ...
  sat_obj_po_esc (pres_set_spt pc.pres) (OPTION_MAP FST pc.ord) pc.obj esc
    (core_only_fml T fml) (core_only_fml F fml) ⇒
  case check_cstep_dom p l l0 o' fml pc of NONE => T
  | SOME (fml',pc') =>
    conf_eq pc pc' ∧
    pc'.tcb = pc.tcb ∧
    pc.id ≤ pc'.id ∧
...
Proof
  rpt gen_tac>>
  fs[check_cstep_dom_def,check_hash_goals_def]>>
  Cases_on`pc.ord`>>fs[]>>
  pairarg_tac>>gvs[]>>
  TOP_CASE_TAC>>
  pop_assum mp_tac>>
...
Theorem check_cstep_dom_correct:
  ∀p l l0 o' fml pc.
  id_ok fml pc.id ∧
  OPTION_ALL good_aspo_subst pc.ord ∧
  EVERY (good_aord_t o SND) pc.orders ∧
  valid_conf pc.chk pc.pres (OPTION_MAP FST pc.ord) pc.obj pc.tcb fml ⇒
  case check_cstep_dom p l l0 o' fml pc of NONE => T
  | SOME (fml',pc') => cstep_ok fml pc fml' pc'
Proof
  rpt gen_tac>>
  strip_tac>>
  Cases_on`pc.ord`
  >- simp[check_cstep_dom_def]>>
  `sat_obj_po (pres_set_spt pc.pres) (OPTION_MAP FST pc.ord) pc.obj
     (core_only_fml T fml) (core_only_fml F fml) ∧
   (pc.tcb ⇒ core_only_fml T fml ⊨ core_only_fml F fml)` by
    gvs[valid_conf_def,valid_req_def]>>
  TOP_CASE_TAC>>simp[]>>
  TOP_CASE_TAC>>simp[]>>
  match_mp_tac cstep_ok_of_str>>
  qspecl_then [`p`,`l`,`l0`,`o'`,`fml`,`pc`,`λw. F`] mp_tac check_cstep_dom_str>>
  simp[sat_obj_po_esc_F]>>
  strip_tac>>
  CONJ_TAC >- gvs[]>>
  irule sat_obj_po_more>>
  irule_at Any core_only_fml_T_SUBSET_F>>
  simp[]
QED

Theorem check_cstep_sstep_correct:
  ∀s fml pc.
  id_ok fml pc.id ∧
  OPTION_ALL good_aspo_subst pc.ord ∧
  EVERY (good_aord_t o SND) pc.orders ∧
...
Proof
  rpt gen_tac>>
  strip_tac>>
  `pc.tcb ⇒ core_only_fml T fml ⊨ core_only_fml F fml` by fs[valid_conf_def]>>
  TOP_CASE_TAC>>simp[]>>
  TOP_CASE_TAC>>simp[]>>
  match_mp_tac cstep_ok_of_str>>
...
Theorem check_cstep_checkeddelete_correct:
  ∀n l l0 o' fml pc.
  id_ok fml pc.id ∧
  OPTION_ALL good_aspo_subst pc.ord ∧
  EVERY (good_aord_t o SND) pc.orders ∧
  valid_conf pc.chk pc.pres (OPTION_MAP FST pc.ord) pc.obj pc.tcb fml ⇒
  case check_cstep_checkeddelete n l l0 o' fml pc of NONE => T
  | SOME (fml',pc') => cstep_ok fml pc fml' pc'
Proof
  rpt gen_tac>>
  strip_tac>>
  ...
  TOP_CASE_TAC>>simp[]>>
  match_mp_tac cstep_ok_of_str>>
  qspecl_then [`n`,`l`,`l0`,`o'`,`fml`,`pc`] mp_tac check_cstep_checkeddelete_str>>
  simp[]
QED

Theorem check_cstep_uncheckeddelete_correct:
  ∀l fml pc.
  id_ok fml pc.id ∧
  OPTION_ALL good_aspo_subst pc.ord ∧
  EVERY (good_aord_t o SND) pc.orders ∧
  valid_conf pc.chk pc.pres (OPTION_MAP FST pc.ord) pc.obj pc.tcb fml ⇒
  case check_cstep_uncheckeddelete l fml pc of NONE => T
  | SOME (fml',pc') => cstep_ok fml pc fml' pc'
Proof
  rpt gen_tac>>
  fs[cstep_ok_def,check_cstep_uncheckeddelete_def]>>
  strip_tac>>
  IF_CASES_TAC >> simp[]>>
  ...
  CONJ_TAC >- (
    ...
    fs[all_core_def,EVERY_MEM,MEM_toAList,FORALL_PROD,OPTION_ALL_good_aspo_subst_good_aspo]>>
    rw[lookup_FOLDL_delete]>>
    metis_tac[])>>
  CONJ_TAC >- (
    rw[]>>
    match_mp_tac bimp_pres_SUBSET>>
    rw[core_only_fml_def,SUBSET_DEF]>>
    fs[lookup_FOLDL_delete]>>
    metis_tac[]
  )>>
...
Theorem check_cstep_transfer_correct:
  ∀l fml pc.
  id_ok fml pc.id ∧
  OPTION_ALL good_aspo_subst pc.ord ∧
  EVERY (good_aord_t o SND) pc.orders ∧
  valid_conf pc.chk pc.pres (OPTION_MAP FST pc.ord) pc.obj pc.tcb fml ⇒
  case check_cstep_transfer l fml pc of NONE => T
  | SOME (fml',pc') => cstep_ok fml pc fml' pc'
Proof
  rpt gen_tac>>
  strip_tac>>
  ...
  TOP_CASE_TAC>>simp[]>>
  match_mp_tac cstep_ok_of_str>>
  qspecl_then [`l`,`fml`,`pc`] mp_tac check_cstep_transfer_str>>
  simp[]
QED

Theorem check_cstep_loadorder_correct:
  ∀m l fml pc.
  id_ok fml pc.id ∧
  OPTION_ALL good_aspo_subst pc.ord ∧
  EVERY (good_aord_t o SND) pc.orders ∧
  valid_conf pc.chk pc.pres (OPTION_MAP FST pc.ord) pc.obj pc.tcb fml ⇒
  case check_cstep_loadorder m l fml pc of NONE => T
  | SOME (fml',pc') => cstep_ok fml pc fml' pc'
Proof
  rpt gen_tac>>
  fs[cstep_ok_def,check_cstep_loadorder_def]>>
  every_case_tac>>
  simp[]>>
  drule ALOOKUP_MEM>>
...
Theorem check_cstep_unloadorder_correct:
  ∀fml pc.
  id_ok fml pc.id ∧
  OPTION_ALL good_aspo_subst pc.ord ∧
  EVERY (good_aord_t o SND) pc.orders ∧
  valid_conf pc.chk pc.pres (OPTION_MAP FST pc.ord) pc.obj pc.tcb fml ⇒
  case check_cstep_unloadorder fml pc of NONE => T
  | SOME (fml',pc') => cstep_ok fml pc fml' pc'
Proof
  rpt gen_tac>>
  fs[cstep_ok_def,check_cstep_unloadorder_def]>>
  rw[]>>
  every_case_tac>>
  fs[]>>
...
Theorem check_cstep_storeorder_correct:
  ∀m p l l0 l1 p0 fml pc.
  id_ok fml pc.id ∧
  OPTION_ALL good_aspo_subst pc.ord ∧
  EVERY (good_aord_t o SND) pc.orders ∧
  valid_conf pc.chk pc.pres (OPTION_MAP FST pc.ord) pc.obj pc.tcb fml ⇒
  case check_cstep_storeorder m p l l0 l1 p0 fml pc of NONE => T
  | SOME (fml',pc') => cstep_ok fml pc fml' pc'
Proof
  rpt gen_tac>>
  fs[cstep_ok_def,check_cstep_storeorder_def]>>
  ...
  >- ( (* reflexivity *)
    ...
    >- (
      ...
      first_x_assum drule>>
      simp[]>>rw[]>>
      irule unsatisfiable_not_sat_implies>>
      fs[core_only_fml_def,Abbr`fmll`,core_only_fml_build_fml] >>
      once_rewrite_tac [subst_eta] >> fs [] >>
      gvs [o_DEF,ALOOKUP_ZIP_MAP_SND,OPTION_MAP_CASE])
    >- (
      gvs[EL_MAP]>>
      match_mp_tac unsatisfiable_not_sat_implies>>
  ...
  >- ( (* transitivity *)
    ...
    >- (
      ...
      once_rewrite_tac [subst_eta] >> fs [] >>
      qmatch_goalsub_abbrev_tac ‘A ∪ set (MAP (subst f1) _) ∪ set (MAP (subst f2) _) ∪ D ∪ _’ >>
      strip_tac >>
      qmatch_goalsub_abbrev_tac ‘A ∪ set (MAP (subst g1) _) ∪ set (MAP (subst g2) _) ∪ D ∪ _’ >>
      qsuff_tac ‘f1 = g1 ∧ f2 = g2’ >- (rw [] >> gvs []) >>
      unabbrev_all_tac >>
      fs [FUN_EQ_THM] >> rw [] >>
      gvs [ALOOKUP_APPEND,ALOOKUP_ZIP_MAP_SND,good_aord_def,OPTION_MAP_CASE] >>
      ntac 4 (CASE_TAC >> fs []) )
    >- (
      gvs[EL_MAP]>>
      match_mp_tac unsatisfiable_not_sat_implies>>
      ...
      once_rewrite_tac [subst_eta] >> fs [] >>
      rw[]>>DISJ2_TAC>>
      gvs [o_DEF,ALOOKUP_ZIP_MAP_SND,OPTION_MAP_CASE,ALOOKUP_APPEND,good_aord_def]>>
      qmatch_goalsub_abbrev_tac ‘subst f1’ >>
      qmatch_asmsub_abbrev_tac ‘subst f2’ >>
      qsuff_tac ‘f1 = f2’ >-
        (rw [] >> gvs []) >>
      unabbrev_all_tac >>
      fs [FUN_EQ_THM] >> rw [] >>
...
Theorem check_cstep_storeorder_str:
  id_ok fml pc.id ∧
  EVERY (good_aord_t o SND) pc.orders ∧
  check_cstep_storeorder m p l l0 l1 p0 fml pc = SOME (fml',pc') ⇒
  fml' = fml ∧
...
Proof
  strip_tac>>
  qspecl_then [`m`,`p`,`l`,`l0`,`l1`,`p0`,`fml`,
    `pc with <| chk := F; ord := NONE; tcb := F |>`] mp_tac
    check_cstep_storeorder_correct>>
  impl_tac >- simp[valid_conf_def,valid_req_def]>>
  gvs[check_cstep_storeorder_def,AllCaseEqs()]>>
  simp[cstep_ok_def]
QED

Theorem check_cstep_obj_correct:
  ∀l b o' fml pc.
  id_ok fml pc.id ∧
  OPTION_ALL good_aspo_subst pc.ord ∧
  EVERY (good_aord_t o SND) pc.orders ∧
  valid_conf pc.chk pc.pres (OPTION_MAP FST pc.ord) pc.obj pc.tcb fml ⇒
  case check_cstep_obj l b o' fml pc of NONE => T
  | SOME (fml',pc') => cstep_ok fml pc fml' pc'
Proof
  rpt gen_tac>>
  fs[cstep_ok_def,check_cstep_obj_def]>>
  strip_tac>>
  every_case_tac>>simp[]
  >- (
  ...
  metis_tac[INJ_ID,INT_LE_REFL]
QED

Theorem check_cstep_changeobj_correct:
  ∀b p l fml pc.
  id_ok fml pc.id ∧
  OPTION_ALL good_aspo_subst pc.ord ∧
  EVERY (good_aord_t o SND) pc.orders ∧
  valid_conf pc.chk pc.pres (OPTION_MAP FST pc.ord) pc.obj pc.tcb fml ⇒
  case check_cstep_changeobj b p l fml pc of NONE => T
  | SOME (fml',pc') => cstep_ok fml pc fml' pc'
Proof
  rpt gen_tac>>
  fs[cstep_ok_def,check_cstep_changeobj_def]>>
  strip_tac>>
...
Theorem check_cstep_checkobj_correct:
  ∀p fml pc.
  id_ok fml pc.id ∧
  OPTION_ALL good_aspo_subst pc.ord ∧
  EVERY (good_aord_t o SND) pc.orders ∧
  valid_conf pc.chk pc.pres (OPTION_MAP FST pc.ord) pc.obj pc.tcb fml ⇒
  case check_cstep_checkobj p fml pc of NONE => T
  | SOME (fml',pc') => cstep_ok fml pc fml' pc'
Proof
  rpt gen_tac>>
  fs[cstep_ok_def,check_cstep_checkobj_def]>>
  every_case_tac>>rw[]>>
  metis_tac[INJ_ID]
QED

Theorem check_cstep_assertobj_correct:
  ∀i fml pc.
  id_ok fml pc.id ∧
  OPTION_ALL good_aspo_subst pc.ord ∧
  EVERY (good_aord_t o SND) pc.orders ∧
  valid_conf pc.chk pc.pres (OPTION_MAP FST pc.ord) pc.obj pc.tcb fml ⇒
  case check_cstep_assertobj i fml pc of NONE => T
  | SOME (fml',pc') => cstep_ok fml pc fml' pc'
Proof
  rpt gen_tac>>
  fs[cstep_ok_def,check_cstep_assertobj_def]>>
  every_case_tac>>fs[]>>
  strip_tac>>
...
Theorem check_cstep_changepres_correct:
  ∀b n p l fml pc.
  id_ok fml pc.id ∧
  OPTION_ALL good_aspo_subst pc.ord ∧
  EVERY (good_aord_t o SND) pc.orders ∧
  valid_conf pc.chk pc.pres (OPTION_MAP FST pc.ord) pc.obj pc.tcb fml ⇒
  case check_cstep_changepres b n p l fml pc of NONE => T
  | SOME (fml',pc') => cstep_ok fml pc fml' pc'
Proof
  rpt gen_tac>>
  fs[cstep_ok_def,check_cstep_changepres_def]>>
  (* ChangePres *)
  rw[]>>
...
Theorem check_cstep_checkpres_correct:
  ∀s fml pc.
  id_ok fml pc.id ∧
  OPTION_ALL good_aspo_subst pc.ord ∧
  EVERY (good_aord_t o SND) pc.orders ∧
  valid_conf pc.chk pc.pres (OPTION_MAP FST pc.ord) pc.obj pc.tcb fml ⇒
  case check_cstep_checkpres s fml pc of NONE => T
  | SOME (fml',pc') => cstep_ok fml pc fml' pc'
Proof
  rpt gen_tac>>
  fs[cstep_ok_def,check_cstep_checkpres_def]>>
  every_case_tac>>rw[]>>
  metis_tac[INJ_ID]
QED

Theorem check_cstep_sol_correct:
  ∀l fml pc.
  id_ok fml pc.id ∧
  OPTION_ALL good_aspo_subst pc.ord ∧
  EVERY (good_aord_t o SND) pc.orders ∧
  valid_conf pc.chk pc.pres (OPTION_MAP FST pc.ord) pc.obj pc.tcb fml ⇒
  case check_cstep_sol l fml pc of NONE => T
  | SOME (fml',pc') => cstep_ok fml pc fml' pc'
Proof
  rpt gen_tac>>
  fs[cstep_ok_def,check_cstep_sol_def]>>
  strip_tac>>
  every_case_tac>>simp[]>>
  gvs[update_bound_def,update_dbound_def]>>
  `pc.id ∉ domain fml` by fs[id_ok_def]>>
...
Theorem check_cstep_strengthentocore_correct:
  ∀b fml pc.
  id_ok fml pc.id ∧
  OPTION_ALL good_aspo_subst pc.ord ∧
  EVERY (good_aord_t o SND) pc.orders ∧
...
Proof
  ...
  TOP_CASE_TAC>>simp[]>>
  match_mp_tac cstep_ok_of_str>>
  qspecl_then [`b`,`fml`,`pc`] mp_tac check_cstep_strengthentocore_str>>
  simp[conf_eq_def]
QED

Theorem check_cstep_correct_ok:
  ∀cstep fml pc.
  id_ok fml pc.id ∧
  OPTION_ALL good_aspo_subst pc.ord ∧
  EVERY (good_aord_t o SND) pc.orders ∧
...
Proof
  ...
  >~[`Dom`] >- (
    ...
    match_mp_tac check_cstep_dom_correct>>
    simp[])
  >~[`Sstep`] >- (
    strip_tac>>
    simp[check_cstep_def]>>
    match_mp_tac check_cstep_sstep_correct>>
    simp[])
  >~[`CheckedDelete`] >- (
    strip_tac>>
    simp[check_cstep_def]>>
    match_mp_tac check_cstep_checkeddelete_correct>>
    simp[])
  >~[`UncheckedDelete`] >- (
    strip_tac>>
    simp[check_cstep_def]>>
    match_mp_tac check_cstep_uncheckeddelete_correct>>
    simp[])
  >~[`Transfer`] >- (
    strip_tac>>
    simp[check_cstep_def]>>
    match_mp_tac check_cstep_transfer_correct>>
    simp[])
  >~[`StrengthenToCore`] >- (
    strip_tac>>
    simp[check_cstep_def]>>
    match_mp_tac check_cstep_strengthentocore_correct>>
    simp[])
  >~[`LoadOrder`] >- (
    strip_tac>>
    simp[check_cstep_def]>>
    match_mp_tac check_cstep_loadorder_correct>>
    simp[])
  >~[`UnloadOrder`] >- (
    strip_tac>>
    simp[check_cstep_def]>>
    match_mp_tac check_cstep_unloadorder_correct>>
    simp[])
  >~[`StoreOrder`] >- (
    strip_tac>>
    simp[check_cstep_def]>>
    match_mp_tac check_cstep_storeorder_correct>>
    simp[])
  >~[`Obj`] >- (
    strip_tac>>
    simp[check_cstep_def]>>
    match_mp_tac check_cstep_obj_correct>>
    simp[])
  >~[`ChangeObj`] >- (
    strip_tac>>
    simp[check_cstep_def]>>
    match_mp_tac check_cstep_changeobj_correct>>
    simp[])
  >~[`CheckObj`] >- (
    strip_tac>>
    simp[check_cstep_def]>>
    match_mp_tac check_cstep_checkobj_correct>>
    simp[])
  >~[`AssertObj`] >- (
    strip_tac>>
    simp[check_cstep_def]>>
    match_mp_tac check_cstep_assertobj_correct>>
    simp[])
  >~[`ChangePres`] >- (
    strip_tac>>
    simp[check_cstep_def]>>
    match_mp_tac check_cstep_changepres_correct>>
    simp[])
  >~[`CheckPres`] >- (
    strip_tac>>
    simp[check_cstep_def]>>
    match_mp_tac check_cstep_checkpres_correct>>
    simp[])
  >~[`Sol`] >- (
    strip_tac>>
    simp[check_cstep_def]>>
    match_mp_tac check_cstep_sol_correct>>
    simp[])
QED
...
Proof
  strip_tac>>
  qspecl_then [`cstep`,`fml`,`pc`] mp_tac check_cstep_correct_ok>>
  simp[]>>
  TOP_CASE_TAC>>simp[]>>
  TOP_CASE_TAC>>simp[cstep_ok_def]
QED
...
Theorem check_csteps_correct:
  ...
  id_ok fml pc.id ∧
  OPTION_ALL good_aspo_subst pc.ord ∧
  EVERY (good_aord_t ∘ SND) pc.orders ∧
  valid_conf pc.chk pc.pres (OPTION_MAP FST pc.ord) pc.obj pc.tcb fml ∧
...
Proof
  ...
  >- ( (* HDSat *)
    ...
    >- (
      ...
      metis_tac[])>>
    Cases_on`check_obj obj wm fmlls NONE`>>gvs[]>>
    Cases_on`x`>>
    drule check_obj_imp>>
    rw[satisfiable_def]>>
    metis_tac[])
