...
val compile_correct_applied =
  MATCH_MP compile_correct mcis_compiled
  |> SIMP_RULE(srw_ss())[LET_THM,ml_progTheory.init_state_env_thm,GSYM AND_IMP_INTRO]
  |> C MATCH_MP cake_pb_mcis_not_fail
  |> C MATCH_MP x64_backend_config_ok
  |> REWRITE_RULE[cake_pb_mcis_sem_sing,AND_IMP_INTRO]
...
val cake_pb_mcis_code_def = Define `
  cake_pb_mcis_code = (code, data, config)
  `;
...
Theorem machine_code_sound:
  ...
  machine_sem mc (basis_ffi cl fs) ms ⊆
    extend_with_resource_limit
      {Terminate Success (cake_pb_mcis_io_events cl fs)} ∧
  ∃out err.
    extract_fs fs (cake_pb_mcis_io_events cl fs) =
        ...
        (
          ...
          (LENGTH cl = 4 ∧
            (
              ...
              ∃l u.
                out = mcis_bound_str l u ∧
                (∀vs. is_cis vs gp gt ⇒ CARD vs ≤ u) ∧
                (∃vs. is_cis vs gp gt ∧ l ≤ CARD vs)
            ))
        )
    )
Proof
  strip_tac>>
  fs[installed_x64_def,cake_pb_mcis_code_def,cake_pb_mcis_run_def]>>
  drule cake_pb_mcis_compiled_thm>>
  simp[AND_IMP_INTRO]>>
  disch_then drule>>
  ...
  >- (
    ...
    gvs[]>>
    Cases_on`bounds`>>
    qpat_x_assum`mcis_sem _ _ _` mp_tac>>
    simp[mcis_sem_def,print_mcis_str_def]>>
    rw[]>>
    metis_tac[])>>
  metis_tac[]
QED
