...
Ancestors
  ...
  mcliqueProg clique mcliqueCompile
Libs
  preamble
...
val (cake_pb_mclique_not_fail,cake_pb_mclique_sem_sing) = cake_pb_mclique_sem
  |> SRULE [mclique_compiled,ml_progTheory.prog_syntax_ok_semantics]
  |> MATCH_MP semantics_prog_Terminate_not_Fail |> CONJ_PAIR

val compile_correct_applied =
  MATCH_MP compile_correct (cj 1 mclique_compiled)
  |> SIMP_RULE(srw_ss())[LET_THM,ml_progTheory.init_state_env_thm,GSYM AND_IMP_INTRO]
  |> C MATCH_MP cake_pb_mclique_not_fail
  |> C MATCH_MP x64_backend_config_ok
  |> REWRITE_RULE[cake_pb_mclique_sem_sing,AND_IMP_INTRO]
  |> REWRITE_RULE[Once (GSYM AND_IMP_INTRO)]
  |> C MATCH_MP (CONJ(UNDISCH x64_machine_config_ok)(UNDISCH x64_init_ok))
  |> DISCH(#1(dest_imp(concl x64_init_ok)))
  |> REWRITE_RULE[AND_IMP_INTRO]

Theorem cake_pb_mclique_compiled_thm =
  CONJ compile_correct_applied cake_pb_mclique_output
  |> DISCH_ALL
  (* |> check_thm *)
...
Definition cake_pb_mclique_code_def:
  cake_pb_mclique_code = (code, data, info)
End

(* A standard run of cake_pb_mclique
  satisfying all the default assumptions *)
Definition cake_pb_mclique_run_def:
  cake_pb_mclique_run cl fs mc ms ⇔
  wfcl cl ∧ wfFS fs ∧ STD_streams fs ∧ hasFreeFD fs ∧
  installed_x64 cake_pb_mclique_code mc ms
End
    ...
    (out ≠ strlit"" ⇒
      ∃g.
        ...
        (
          ...
          (LENGTH cl = 3 ∧
            (
              out = mclique_eq_str (CARD (maximal_cliques g)) ∨
              ∃l.
                out = mclique_bound_str l ∧
                l ≤ CARD (maximal_cliques g)
            ))
        )
    )
Proof
  strip_tac>>
  fs[installed_x64_def,cake_pb_mclique_code_def,cake_pb_mclique_run_def]>>
  drule cake_pb_mclique_compiled_thm>>
  simp[AND_IMP_INTRO]>>
  disch_then drule>>
  ...
  >- (
    ...
    strip_tac>>
    gvs[]>>
    qpat_x_assum`mclique_sem _ _` mp_tac>>
    simp[mclique_sem_def,print_mclique_str_def]>>
    rw[]>>
    gvs[AllCasePreds()]>>
    metis_tac[])>>
  metis_tac[]
