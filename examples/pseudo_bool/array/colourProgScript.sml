...
Definition parse_pbf_ann_def:
  parse_pbf_ann tokss =
  (let
    nocomments = FILTER pb_parse$nocomment_line tokss
  in
    case parse_obj_maybe nocomments of
      (SOME obj,rest) =>
      (case parse_annots rest [] of
        NONE => NONE
      | SOME anns => SOME (obj,anns))
    | _ => NONE)
End

val r = translate parse_obj_def;
val r = translate parse_obj_maybe_def;
val r = translate parse_annots_def;
val r = translate parse_pbf_ann_def;
...
Definition get_annots_def:
  get_annots fs f =
  if inFS_fname fs f then
    parse_pbf_ann (MAP toks (all_lines_file fs f))
  else NONE
End

Overload "annots_TYPE" = ``
(PAIR_TYPE
  (PAIR_TYPE
...
Theorem parse_pbf_full_spec:
  ...
  app (p:'ffi ffi_proj) ^(fetch_v"parse_pbf_full"(get_ml_prog_state()))
    ...
    (STDIO fs)
    (POSTv v.
    & (∃err. (SUM_TYPE STRING_TYPE annots_TYPE
    (case get_annots fs f of
      NONE => INL err
...
val _ = translate mk_key_ann_def;

val _ = use_sub_check false;

Theorem mk_key_ann_side[local]:
  mk_key_ann_side x
Proof
  rw[fetch "-" "mk_key_ann_side_def"]>>
  CCONTR_TAC>>fs[]
QED

val _ = mk_key_ann_side |> update_precondition;
...
Quote add_cakeml:
  ...
  then
    case parse_pbf_full f2 of
      Inl err => Inl err
    | Inr (obj,anns) =>
      case lazy_full_encode_ann g obj anns of
        None =>
...
Proof
  ...
  >- (
    ...
    qexists_tac`INL err`>>
    simp[SUM_TYPE_def])>>
  rename1`get_annots _ _ = SOME anns`>>
  Cases_on`anns`>>fs[PAIR_TYPE_def]>>
  xmatch>>
  xlet_autop>>
  ...
  >- (
    ...
    simp[SUM_TYPE_def])>>
  gvs[npbc_arrayProgTheory.PAIR_TYPE_SPLIT]>>
  xmatch>>
  rpt xlet_autop>>
  xcon>>
  xsimpl>>
...
Proof
  ...
  >- (
    ...
    fs[STD_streams_add_stderr, STD_streams_stdout,add_stdo_nil]>>
    xsimpl)>>
  `?k n objf. y = (k,n,objf)` by metis_tac[PAIR]>>
  fs[PAIR_TYPE_def]>>
  xmatch>>
  xlet_autop>>
  ...
  qpat_x_assum`prob_TYPE (mk_prob2 _) _`assume_tac>>
  disch_then drule>>
