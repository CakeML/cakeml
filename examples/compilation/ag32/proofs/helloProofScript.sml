...
val extract_fs_extract_writes = Q.store_thm("extract_fs_extract_writes",
   ...
   *)
   (* -- *)
   (* nothing has changed except the IOStream of interest *)
...
fun next_tac n =
  ...
  in
    ...
    \\ simp[GSYM get_mem_word_def]
    \\ `^sn.PC = s.PC + n2w(4 * ^(numSyntax.term_of_int n))`
    by ( simp[Abbr`^sn`, dfn'Normal_PC, dfn'LoadMEMByte_PC, dfn'Shift_PC] )
    \\ `byte_aligned ^sn.PC`
    by (
      simp[]
      \\ irule byte_aligned_add \\ simp[]
    ...
    \\ first_assum(qspec_then`^(numSyntax.term_of_int n)`mp_tac)
    \\ impl_tac >- EVAL_TAC
    \\ simp_tac(srw_ss())[ag32_ffi_write_check_conf_code_def, ag32_ffi_write_load_noff_code_def]
    \\ `^sn.MEM = s.MEM` by simp[Abbr`^sn`, dfn'Normal_MEM, dfn'LoadMEMByte_MEM, dfn'Shift_MEM]
    \\ simp[]
    \\ disch_then kall_tac
    \\ simp[ag32_targetProofTheory.Decode_Encode]
    \\ simp[ag32Theory.Run_def]
...
val ag32_ffi_write_thm = Q.store_thm("ag32_ffi_write_thm",
  `bytes_in_memory (s.R 1w) conf s.MEM md ∧
   ...
   code_start_offset (LENGTH ffi_names) + lc + ld ≤ memory_size ∧
