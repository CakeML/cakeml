...
val hello_good_init_state = Q.store_thm("hello_good_init_state",
  `SUM (MAP strlen cl) + LENGTH cl ≤ cline_size ∧
   ...
   good_init_state hello_machine_config (FUNPOW Next (hello_startup_clock ms0 inp cl) ms0)
     (basis_ffi cl fs) helloCompile$code 0
     ...
     (λk. Word
       ...
       ({w | (hello_start_asm_state (cl,inp)).regs 2 <=+ w ∧
        ...
        ∪ {w | n2w (code_start_offset (LENGTH (THE config.ffi_names)) + LENGTH helloCompile$code) <=+ w ∧
               w <+ n2w(code_start_offset (LENGTH (THE config.ffi_names)) + LENGTH helloCompile$code + 4 * LENGTH helloCompile$data) })
     io_regs
     cc_regs`,
  strip_tac
  \\ rewrite_tac[hello_machine_config_def, hello_start_asm_state_def]
  ...
  \\ conj_tac >- (fs[hello_init_memory_def])
  \\ drule hello_startup_clock_def
  \\ disch_then drule
  \\ disch_then drule \\ rw[]
  \\ fs[hello_start_asm_state_def]);
...
val ag32_installed = Q.store_thm("ag32_installed",
  ...
  \\ drule (GEN_ALL init_asm_state_RTC_asm_step)
  \\ disch_then drule
  pull out LENGTH ffis ≤ LENGTH FFI_codes as a lemma from ag32_good_init_state
