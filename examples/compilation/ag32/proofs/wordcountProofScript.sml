...
val LENGTH_wordcount_init_memory_words = Q.store_thm("LENGTH_wordcount_init_memory_words",
  `SUM (MAP strlen cl) + LENGTH cl ≤ cline_size ∧ LENGTH inp ≤ stdin_size ⇒
   (LENGTH (wordcount_init_memory_words cl inp) = 27585200)`, (* adjust as necessary *)
...
val wordcount_interference_implemented = Q.store_thm("wordcount_interference_implemented",
  ...
  >- (
    ...
    >- (
      ...
      \\ simp[] )
    \\ cheat (* encoded asm step doesn't touch stdin pointer. may need to improve interference_implemented (see same cheat in helloProofScript.sml) *)
    )
  \\ conj_tac
  >- (
    strip_tac
...
val wordcount_ag32_next = Q.store_thm("wordcount_ag32_next",
  ...
  \\ impl_tac >- (
    ...
    \\ rpt(disch_then drule)
    \\ strip_tac
    \\ cheat (* initial stdin offset and length are correct (see same cheat in helloProofScript.sml) *))
  \\ strip_tac
