...
Definition next_state_def:
  next_state s is (circ: ('a, 'l, 'i) circuit) (next: 'l -> 'a) =
  eval_circuit s is circ ∘ next
End

Definition is_reset_def:
  is_reset s is (circ: ('a, 'l, 'i) circuit) (reset: 'l -> 'a) =
  ∀l. eval_circuit s is circ (reset l)
End

(* spo = strict partial order *)
Theorem stratification_reset_exists:
  is_stratified reset spo circ is ⇒ ∃s. is_reset s is circ reset
Proof
  cheat
QED
