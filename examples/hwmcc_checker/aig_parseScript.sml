...
(* Abbreviations:
   - {i,l}ren = {input,latch}renaming (map)
   - m{i,l}cnt = model{input,latch}count *)

(* OPT It might be more efficient to directly compute the updated literals in
   shared_{...} instead of a lookup on the map generated by default_shared. *)
Definition default_shared_def:
  default_shared micnt mlcnt wicnt wlcnt =
  let
    mlstart = micnt + 1;
    wlstart = wicnt + 1;
    irena = GENLIST (λk. (1 + k, 2 + 2 * k)) (MIN micnt wicnt);
    lrena = GENLIST (λk. (wlstart + k, 2 * (mlstart + k))) (MIN mlcnt wlcnt);
  in
    (fromAList irena, fromAList lrena)
End
...
(* Reused at other places, thus it is factored into its own function. *)
...
val [parsed_iren, parsed_lren] =
  EVAL “(^(wmaps).shared_inputs, ^(wmaps).shared_latches)”
