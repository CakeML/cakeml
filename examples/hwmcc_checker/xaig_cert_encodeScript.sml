...
Definition ext_gate_def:
  ext_gate ((n, gty): ('a # ('a,'i,'l) gty)) = (Orig n, ext_gty gty)
End
...
Definition encode_equiv_def:
  encode_equiv (xaig: ('a ext, 'i, 'l) xaig) name xys =
  let
...
Theorem xeval_gate_encode_equiv_ext:
  xeval_gate ss (encode_equiv xaig name xys) (Ext n) =
  if n = name then
    EVERY (λ(x,y). xeval_lit ss xaig x ⇔ xeval_lit ss xaig y) xys
  else xeval_gate ss xaig (Ext n)
Proof
  rw [xeval_lit_def, encode_equiv_def]
  >- (
    eq_tac
    >> rw [EVERY_EL]
    >> rpt strip_tac
...
Theorem xeval_lit_encode_equiv_ext:
  xeval_lit ss (encode_equiv xaig name xys) (Gate (Ext n), b) =
  if n = name then
    b ⇎ EVERY (λ(x,y). xeval_lit ss xaig x ⇔ xeval_lit ss xaig y) xys
  else xeval_lit ss xaig (Gate (Ext n), b)
Proof
  simp [xeval_lit_def, xeval_gate_encode_equiv_ext]
  >> IF_CASES_TAC >> gvs []
QED

Theorem eval_lit_encode_equiv_ext_lit:
  xeval_lit ss (encode_equiv xaig name xys) (ext_lit n) =
  xeval_lit ss xaig (ext_lit n)
Proof
  namedCases_on ‘n’ ["v b"] >> Cases_on ‘v’
  >> simp [ext_lit_def, ext_var_def, encode_equiv_def, xeval_lit_def,
...
Definition encode_is_reset_def:
  encode_is_reset (aig: ('a ext, 'i, 'l) aig) name reset ls =
...
Definition encode_lits_hold_def:
  encode_lits_hold
    (aig: ('a ext, 'i, 'l) aig) name (lits: ('a ext,'i,'l) lit list) =
...
Theorem eval_lit_encode_lits_hold_ext_lit[simp]:
  eval_lit ss (encode_lits_hold aig name lits) (ext_lit lit) =
  eval_lit ss aig (ext_lit lit)
Proof
...
Definition ileft_reset_def:
  ileft_reset = ext_reset ∘ left_reset
End

Definition iright_reset_def:
  iright_reset = ext_reset ∘ right_reset
End

Definition ileft_name_lits_def:
  ileft_name_lits = MAP (ext_lit ∘ left_name_lit)
End

Definition iright_name_lits_def:
  iright_name_lits = MAP (ext_lit ∘ right_name_lit)
End

Definition imerge_aigs_def:
  imerge_aigs aig₀ aig₁ = ext_aig (merge_aigs aig₀ aig₁)
End

Theorem eval_lit_imerge_aig_ext_lit[simp]:
  eval_lit ss (imerge_aigs aig₀ aig₁) (ext_lit lit) ⇔
...
Definition encode_is_next_def:
  encode_is_next
    (aig: (('a + 'b) ext, 'i + 'j, 'l + 'l) aig)
    (name: mlstring)
    (next: ('l -> ('a,'i,'l) lit))
    (latches: 'l list)
  =
  encode_is_next_with aig name (ext_lit ∘ left_lit) (ext_lit ∘ right_lit)
    next latches
End
...
Definition encode_signal_imply_aux_def:
  (encode_signal_imply_aux
     (aig: ('a ext, 'i, 'l) aig)
     (signal::rest : ('a ext, 'i, 'l) lit list)
     (signal'::rest': ('a ext, 'i, 'l) lit list)
     (next: num)
...
Definition encode_transition_cond_def:
  ...
  let
    ...
    aig  = encode_is_next aig «mnext» (left_name_lit ∘ mnext) mlatches;
    aig  = encode_is_next aig «wnext» (right_name_lit ∘ wnext) klatches;
    lhss =
    [(Gate (Named (Ext «mnext»)), F);
     (Gate (Named (Ext «mcnstrs0»)), F);
     (Gate (Named (Ext «mcnstrs1»)), F);
...
Definition encode_base_cond_def:
  ...
  ⇔
    let
      aig  = ext_aig waig;
      aig  = encode_is_reset aig «wreset» (ext_reset wreset) wlatches;
      aig  = encode_lits_hold aig «wcnstrs» (MAP ext_lit wcnstrs);
      aig  = encode_lits_hold aig «wsafes» (MAP ext_lit wsafes);
      lhss =
        [(Gate (Named (Ext «wreset»)),F);
         (Gate (Named (Ext «wcnstrs»)),F)];
...
Definition encode_induction_cond_def:
  ...
  =
    let
      aig  = ext_aig (pair_aigs waig waig);
      aig  = encode_lits_hold aig «wcnstrs0»
      ...
      aig  = encode_is_next aig «wnext» wnext wlatches;
      lhss =
        [(Gate (Named (Ext «wsafes0»)), F);
         (Gate (Named (Ext «wnext»)), F);
         (Gate (Named (Ext «wcnstrs1»)), F);
         (Gate (Named (Ext «wcnstrs0»)), F)];
      rhss = [(Gate (Named (Ext «wsafes1»)), F)]
    in
...
Theorem is_reset_ext[local,simp]:
  is_reset ss (ext_aig aig) (ext_reset reset) latches ⇔
...
Proof
  simp [is_reset_def, imerge_aigs_def, ileft_reset_def, left_reset_def,
        eval_lit_def, ext_reset_def, PULL_EXISTS]
QED
...
Theorem lits_hold_encode_lits_hold_ext_lit[local,simp]:
  lits_hold ss (encode_lits_hold aig name preds') (set (MAP ext_lit preds)) ⇔
    lits_hold ss aig (set (MAP ext_lit preds))
Proof
...
Theorem encode_lives_hold_aux_eval_lit_ext[local]:
  ∀live aig next aig' outs.
    encode_lives_hold_aux aig live next = (aig',outs)
    ⇒
    (eval_lit ss aig' (ext_lit lit) ⇔ eval_lit ss aig (ext_lit lit))
Proof
  Induct >> rw [encode_lives_hold_aux_def]
  >> rpt (pairarg_tac >> gvs [])
  >> last_x_assum drule >> simp []
...
Theorem encode_signal_imply_aux_eval_lit_ext[local]:
  ∀aig signals signals' next aig' outs.
    encode_signal_imply_aux aig signals signals' next = (aig',outs)
    ⇒
    (eval_lit ss aig' (ext_lit lit) ⇔ eval_lit ss aig (ext_lit lit))
...
Proof
  ...
  >> drule encode_lives_hold_aux_eval_lit_ext >> simp []
QED
...
Proof
  ...
  >> drule encode_signal_imply_aux_eval_lit_ext
  >> simp []
QED
