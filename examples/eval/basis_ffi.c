...
void cml_exit(int arg) {
  ...
  if (arg != 0) {
    fprintf(stderr,"Program exited with nonzero exit code %d.\n", arg);
  }
  #endif
...
int main (int local_argc, char **local_argv) {
  ...
  int mprotres = mprotect(&cake_text_begin, &cake_codebuffer_end - &cake_text_begin,
                          PROT_READ | PROT_WRITE | PROT_EXEC);
  if (mprotres != 0) {
    #ifdef STDERR_MEM_EXHAUST
    fprintf(stderr,"mprotect() failed %d to set permissions for CakeML code buffer.\n", mprotres);
    perror("mprotect");
    #endif
    exit(3);
  }

  printf("Passing control to CakeML.\n");
