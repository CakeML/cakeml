...
Proof
  ...
  first_x_assum(qspecl_then [`s`,`p`,`(head_key_t s (TL fts))`,
    `ft_mem(FibTree k (new_anode v' b n p (head_key l') (LENGTH l'))
...
Proof
  ...
  >- (
    ...
    >- (
      qexists `F` >>
      fs[Once fts_has_cases, FLOOKUP_DEF, new_dnode_def]
      ) >>
    fs[Once fts_has_cases, FLOOKUP_DEF, new_dnode_def] >>
    fs[Once fts_has_cases]
    )
  >- (
    ...
    fs[head_key_def, FLOOKUP_DEF, new_dnode_def]
    )
  >> fs[fib_heap_shape_ok_def] >>
...
Theorem lemma_insert_new_min_inv:
  !fh fts k v e.
    ...
    fib_heap_inv fh fts /\
    (v <=+ fts_hd_value fts) ==>
    fib_heap_inv (fh |+ (k,v,e)) (FibTree k (new_dnode v e F) []::fts)
Proof
  fs[fib_heap_inv_def] >>
  rpt strip_tac
  >- gvs[FLOOKUP_DEF]
  >- (
    ...
    >- (
      fs[Once fts_has_cases]
      >- fs[new_dnode_def, FLOOKUP_SIMP]
      >- (
        qpat_assum `fts_has k' (new_dnode v' e' m) fts` mp_tac >>
    ...
    >- (
      qpat_assum `fts_has k' (new_dnode v' e' m)
                    (FibTree k (new_dnode v e F) []::fts)` mp_tac >>
      pure_rewrite_tac[Once fts_has_cases] >>
      rfs[] >>
      simp[DISJ_SYM] >>
      simp[Once fts_has_cases] >>
...
Theorem lemma_insert_old_min_inv:
  !fh fts k v e.
    ...
    fib_heap_inv fh fts /\
    ~(v <=+ fts_hd_value fts) ==>
    fib_heap_inv (fh |+ (k,v,e)) (fts ++ [FibTree k (new_dnode v e F) []])
Proof
  fs[fib_heap_inv_def] >>
  rpt strip_tac
  >- gvs[FLOOKUP_SIMP]
  >- (
    ...
    >- (
      qpat_x_assum `fts_has k' (new_dnode v' e' m)
                    (fts ++ [FibTree k (new_dnode v e F) []])` mp_tac >>
      simp[fts_has_append_thm] >>
      disch_tac >> gvs[] >>
    ...
    qpat_x_assum `fts_has k' (new_dnode v' e' m)
                  [FibTree k (new_dnode v e F) []]` mp_tac >>
    simp[Once fts_has_cases] >>
    simp[Once fts_has_cases]
    )
  >- (
    ...
    >- (
       ...
       simp[fts_is_min_def,new_dnode_def]
       ) >>
    Cases_on `h` >>
    fs[head_key_def,fts_hd_value_def,head_key_t_def] >>
    Cases_on `k = k'`
    >- fs[FLOOKUP_SIMP, fts_is_min_def,new_dnode_def] >>
...
Proof
  ...
  >- (
    ...
    strip_tac >> gvs[] >>
    SEP_W_TAC >>
    qexists `[FibTree a' (new_dnode v e F) []]` >>
  ...
  >- (
    ...
    >- (
      ...
      strip_tac >> gvs[] >>
      SEP_W_TAC >>
      qexists `[FibTree a' (new_dnode v e F) []; FibTree k' v' l]` >>
      ...
      fs[fts_hd_value_def,new_dnode_def]
      ) >>
    fs[fib_heap_append_def,before_off_def,next_off_def] >>
    SEP_R_TAC >>
    IF_CASES_TAC >> fs[] >>
    ...
    strip_tac >> gvs[] >>
    SEP_W_TAC >>
    qexists `[FibTree a' v' l; FibTree k (new_dnode v e F) []]` >>
  ...
  >- (
    ...
    >- (
      ...
      strip_tac >> gvs[] >>
      SEP_W_TAC >>
      qexists `[FibTree a' (new_dnode v e F) []; FibTree lk lv ts;
                FibTree k' v' l]` >>
      simp[ann_fts_def, ann_fts_seg_def, last_key_def, last_key_t_def,
           fts_mem_def, SEP_CLAUSES, head_key_def, ft_mem_def, new_anode_def,
           new_dnode_def, head_key_t_def, ones_def, STAR_ASSOC] >>
      ...
      qspecl_then [`fh |+ (a',v,e)`,`a'`,`(new_dnode v e F)`,`[]`,
  ...
  >- (
    ...
    strip_tac >> gvs[] >>
    SEP_W_TAC >>
    qexists `[FibTree a' (new_dnode v e F) []; FibTree sk sv sts] ++
  ...
  qexists `[FibTree a' fv fts; FibTree sk sv sts] ++ t ++
           [FibTree lk lv lts; FibTree k (new_dnode v e F) []]` >>
...
Proof
  ...
  simp[head_key_def] >>
  fs[ann_fts_def, ann_fts_seg_def, last_key_def, last_key_t_def, fts_mem_def,
     SEP_CLAUSES, head_key_def, ft_mem_def, new_anode_def,new_dnode_def,
...
Definition fts_meld_def:
  (fts_meld [] ys = ys) /\
  (fts_meld xs [] = xs) /\
  (fts_meld (FibTree k1 v1 l1::xs) (FibTree k2 v2 l2::ys) =
...
Theorem logical_meld_fib_heap_inv_weak:
  !fh1 fh2 xs ys.
  (fib_heap_inv_weak fh1 xs) /\
  (fib_heap_inv_weak fh2 ys) /\
  (DISJOINT (FDOM fh1) (FDOM fh2))
  ==>
  (fib_heap_inv_weak (FUNION fh1 fh2) (xs ++ ys))
Proof
  rpt strip_tac >>
  fs[fib_heap_inv_weak_def] >>
  rpt conj_tac
...
Theorem fib_heap_meld_imp_fts_meld_mem_thm:
  !frame p fts fts' m dm a' m' c.
  (fts_mem (ann_fts p fts) * fts_mem (ann_fts p fts') * frame)
    (fun2set (m,dm)) /\
  fib_heap_meld (head_key fts,head_key fts',m,dm) = (a',m',c) ==>
  (fts_mem (ann_fts p (fts_meld fts fts')) * frame)
    (fun2set (m',dm)) /\
  c = T /\
  LENGTH (fts_meld fts fts') = LENGTH (fts ++ fts') /\
  (a' = head_key (fts_meld fts fts'))
Proof
...
Theorem fib_heap_meld:
  ∀frame fh1 fts fh2 fts' m dm a' m' c.
    (fts_mem (ann_fts 0w fts) * fts_mem(ann_fts 0w fts') * frame)
      (fun2set (m,dm)) ∧
    fib_heap_inv fh1 fts /\
    fib_heap_inv fh2 fts' /\
    DISJOINT (FDOM fh1) (FDOM fh2) /\
    fib_heap_meld (head_key fts, head_key fts', m, dm) = (a', m', c) ⇒
    ?fts.
    (fts_mem (ann_fts 0w fts) * frame) (fun2set (m',dm)) /\
    fib_heap_inv (FUNION fh1 fh2) fts /\ c
Proof
  rpt gen_tac >> strip_tac >>
...
Theorem fib_heap_meld3:
  ∀frame fh1 fts fh2 fts' m dm a' m' c.
    ...
    DISJOINT (FDOM fh1) (FDOM fh2) /\
    fib_heap_meld (head_key fts, head_key fts', m, dm) = (a', m', c) ⇒
    (fts_mem (ann_fts 0w (fts_meld fts fts')) * frame) (fun2set (m',dm)) /\
    a' = head_key (fts_meld fts fts') /\
    fib_heap_inv (FUNION fh1 fh2) (fts_meld fts fts') /\ c
Proof
  rpt gen_tac >> disch_tac >> fs[] >>
  imp_res_tac fib_heap_meld_imp_fts_meld_mem_thm >>
  gvs[] >>
  metis_tac[fts_meld]
QED


Theorem fib_heap_meld_weak:
  ∀frame fh1 fts fh2 fts' m dm a' m' c.
    (fts_mem (ann_fts 0w fts) * fts_mem(ann_fts 0w fts') * frame)
      (fun2set (m,dm)) ∧
    fib_heap_inv_weak fh1 fts /\
    fib_heap_inv_weak fh2 fts' /\
    DISJOINT (FDOM fh1) (FDOM fh2) /\
    fib_heap_meld (head_key fts, head_key fts', m, dm) = (a', m', c) ⇒
    (fts_mem (ann_fts 0w (fts_meld fts fts')) * frame) (fun2set (m',dm)) /\
    a' = head_key (fts_meld fts fts') /\
    fib_heap_inv_weak (FUNION fh1 fh2) (fts_meld fts fts') /\ c
Proof
  rpt gen_tac >> disch_tac >> fs[] >>
  imp_res_tac fib_heap_meld_imp_fts_meld_mem_thm >>
  gvs[] >>
  metis_tac[fts_meld_weak]
QED
...
Proof
  ...
  qexists `[FibTree k (new_dnode v e F) []]` >>
  fs[ann_fts_def, ann_fts_seg_def, last_key_def, last_key_t_def,fts_mem_def,
     SEP_CLAUSES, head_key_def, ft_mem_def, new_anode_def,
     new_dnode_def, head_key_t_def, ones_def, STAR_ASSOC] >>
  simp[fib_heap_inv_def] >>
...
(*
Theorem fib_heap_inv_weak_split_last_thm:
  all_disjoint ((fhx,xs)::(fhy,[y])::rest) /\
  fib_heap_inv_weak (FUNION fhx fhy) (xs ++ [y]) ==>
  fib_heap_inv_weak (FUNION fhx fhy) (xs ++ [y]) =
  fib_heap_inv_weak fhx xs /\ fib_heap_inv_weak fhy [y]
Proof
  fs[fib_heap_inv_weak_def] >>
  fs[all_disjoint_def] >>
  rpt strip_tac >>
...
Proof
  ...
  rfs[] >>
  qpat_x_assum `EL (LENGTH l) rl = (m, SOME (FibTree k' v' l'))` kall_tac >>
  qspecl_then [`fh2`,`rl`,`k'`,`v'`,`l'`] assume_tac lemma_fib_heap_inv_union_el >>
  rfs[] >>
  qspecl_then [`fh1`,`fh2'`,`k`,`v`,`l`,`k'`,`v'`,`l'`] assume_tac fts_merge_trees >>
  gvs[DISJOINT_SYM] >>
  first_x_assum(qspecl_then [`LUPDATE (FEMPTY,NONE) (LENGTH l) rl`,`FUNION fh1 fh2'`,
    `fh_union (LUPDATE (FEMPTY,NONE) (LENGTH l) rl)`, `k''`,`v''`,`l''` ]
    assume_tac) >>
  rfs[] >>
  `LENGTH l < LENGTH rl` by gvs[] >>
  drule lemma_fib_heap_inv_union_imp_lupdate_fempty >> strip_tac >>
  qabbrev_tac `fh_rl = (fh2' ⊌ fh_union rl❲LENGTH l ↦ (FEMPTY,NONE)❳)` >>
  `fh_rl = fh_union rl` by fs[Once fib_heap_inv_union_def] >>
  qpat_x_assum `fib_heap_inv_union fh_rl rl` mp_tac >>
  simp[] >> strip_tac >>
  fs[] >>
  rfs[lemma_fhts_to_ts_empty_upd] >>
  unabbrev_all_tac >>
  gvs[] >>
  drule_all EQ_SYM >>
  qpat_x_assum `fh2' ⊌ fh_union rl❲LENGTH l ↦ (FEMPTY,NONE)❳ = fh_union rl`
    kall_tac >>
  simp[FUNION_ASSOC]
QED
...
Proof
  ...
  fs[PULL_FORALL] >>
  rename [`fib_heap_inv_weak fh1 (FibTree k arr l::fts)`] >>
  pop_assum mp_tac >>
  simp[fts_link_root_list_def] >>
  ...
  qspecl_then [`q`,`196`,`rl`,`fh1'`, `fh2`, `k`,
    `arr`,`l`] assume_tac fts_link_trees2 >>
  gvs[] >>
  first_x_assum(qspecl_then [`fh2'`,`FUNION fh1' fh2`] assume_tac) >>
  rfs[DISJOINT_SYM] >>
  metis_tac[FUNION_COMM,DISJOINT_SYM,FUNION_ASSOC]
QED
...
Proof
  ...
  pairarg_tac >> simp[] >>
  strip_tac >> gvs[] >>
  drule lemma_fib_heap_inv_union_replicate_imp_fempty >> strip_tac >>
  gvs[] >>
  ...
  simp[fib_heap_inv_union_empty_thm] >> strip_tac >>
  gvs[] >>
...
Proof
  ...
  >- (
    fs[fib_heap_inv_def] >>
    qpat_x_assum `∀k v e.FLOOKUP fh k = SOME (v,e) ==>
                  ∃m. fts_has k (new_dnode v e m) []` mp_tac >>
    simp[Once fts_has_cases] >>
    strip_tac >>
    fs[lemma_empty_map] >>
    Cases_on `xs`
...
Definition fib_heap_set_parent_def:
  ...
  =
    ...
    if a = l then
      ...
      fib_heap_set_parent (n-1) (a_n,l) (m,dm,c)
End
...
Proof
  ...
  >- (
    fs[ann_fts_def, ann_fts_seg_def, last_key_def,fts_mem_def,
         SEP_CLAUSES, head_key_def, ft_mem_def, new_anode_def,
         new_dnode_def, head_key_t_def, last_key_t_def,ones_def, STAR_ASSOC] >>
    rpt strip_tac >> gvs[] >>
    pop_assum mp_tac >>
...
Definition fib_heap_extract_min_def:
  ...
  =
    ...
    let (m,c) = (fib_heap_set_parent n (a_child,b_child) (m,dm,c)) in
...
Proof
  ...
  strip_tac >>
  first_x_assum (qspecl_then [`np`,`p`,`xs ++ [FibTree a v l]`,`t`,
    `m (|a + 6w * bytes_in_word |-> Word np |)`, `dm`,`frame`,
    `_0`,`m'`] mp_tac) >> simp[] >>
  simp[head_key_t_eq_head_key_thm] >>
  Cases_on `xs`
  >- (
  ...
  qspecl_then [`np`,`head_key_t 0w t`,
...
Theorem fib_heap_meld2:
  ∀frame fh1 fts fh2 fts' m dm a' m' c.
    ...
    ?fts.
    (fts_mem (ann_fts 0w fts) * frame) (fun2set (m',dm)) /\
    fib_heap_inv_weak (FUNION fh1 fh2) fts /\ c
Proof
  rpt gen_tac >> strip_tac >>
  drule_all fib_heap_meld_weak >>
  strip_tac >> gvs[] >>
  qexists `fts_meld fts fts'` >> simp[]
QED
  ...
  >- (
    ...
    >- (
      ...
      qspecl_then [`ones k [v.value;FST v.edges;b2w T;b2w v.mark;k;k;0w;child_a;
        ...
        `0w`,`(FibTree child_a v' l::t')`,`t`,`m3`,`dm`,`new_a`,`m'''`,`c''`]
          mp_tac fib_heap_meld_imp_fts_meld_mem_thm >>
      fs[ann_fts_def, ann_fts_seg_def, last_key_def, last_key_t_def,
         fts_mem_def,SEP_CLAUSES, head_key_def, ft_mem_def, new_anode_def,
         new_dnode_def, head_key_t_def, ones_def, STAR_ASSOC] >>
      fs[AC STAR_ASSOC STAR_COMM] >>
      fs[STAR_ASSOC] >> strip_tac >>
      strip_tac >> gvs[] >>
      qexistsl [`v.value`,`v.edges`] >>
      fs[ann_fts_def, ann_fts_seg_def, last_key_def, last_key_t_def,
         fts_mem_def,SEP_CLAUSES, head_key_def, ft_mem_def, new_anode_def,
...
Theorem fib_heap_rm_min:
  ...
  ==>
  ?v e.
  (fts_mem (ann_fts 0w fts') * empty_node a_min (v,e) * frame)
    (fun2set(m',dm)) /\
  (fib_heap_inv_weak (fh1 \\ min) fts')
Proof
  rpt gen_tac >> strip_tac >>
  drule_all fib_heap_rm_min_mem >> strip_tac >> gvs[] >>
  qexistsl [`v`,`e`] >> simp[] >>
  metis_tac[fts_rm_min]
QED
...
Theorem fib_heap_rm_min:
  ...
  196 < dimword (:'a) /\
  fib_heap_rm_min ((a:'a word),m,dm) = (min,a',m',T)
  ==>
  ?fts v e.
...
Proof
  ...
  >- (
    ...
    fs[fib_heap_inv_weak_empty_thm,SEP_CLAUSES,empty_node_def]
   ) >>
  ...
  qspecl_then [`frame`,`fts`,`fh1`,`m`,`dm`,`min'`,`n_a`,`m''`]
    mp_tac fib_heap_pop >> simp[] >>
  Cases_on `c` >> simp[]
  >- (
    strip_tac >>
    qpat_x_assum `(fts_mem (ann_fts 0w fts) * frame) (fun2set (m,dm))` kall_tac >>
    qpat_x_assum `fib_heap_pop (head_key fts,m,dm) = (min',n_a,m'',T)` kall_tac >>
    qpat_x_assum `fib_heap_inv fh1 fts` kall_tac >>
    fs[SEP_EXISTS_THM,PULL_EXISTS,SEP_CLAUSES,fib_heap_weak_def] >>
    full_simp_tac (std_ss ++ sep_cond_ss) [cond_STAR] >>
    qpat_x_assum `fib_heap_inv_weak fh1 fts` kall_tac >>
    pairarg_tac >>
    Cases_on `fts` >> fs[head_key_t_def,head_key_def] >>
    Cases_on `h` >> fs[head_key_t_def] >>
    `k + 7w * bytes_in_word <> 0w` by cheat >> fs[] >>
    fs[ann_fts_def, ann_fts_seg_def, last_key_def, last_key_t_def,
       fts_mem_def,SEP_CLAUSES, head_key_def, ft_mem_def, new_anode_def,
       new_dnode_def, head_key_t_def, ones_def, STAR_ASSOC,
       rank_off_def,mark_off_def] >>
    ...
    >- (
      gvs[head_key_t_def] >>
      pairarg_tac >>
      imp_res_tac lemma_fib_heap_inv_weak_empty_fts_imp_empty_map >>
      rename [`fib_heap_meld (0w,head_key_t 0w fts',m2,dm) = (new_a,m3,c3)`] >>
      fs[] >>
      qspecl_then [`fts_mem (ann_fts 0w [FibTree a v []]) * frame`,
        `FEMPTY`,`[]`,`(FDIFF fh1 (FDOM fh2))`,`fts'`,
        `m2`, `dm`,`new_a`, `m3`,`c3`] mp_tac fib_heap_meld2 >>
      simp[fib_heap_inv_weak_empty_thm,head_key_t_def,head_key_def] >>
      fs[ann_fts_def, ann_fts_seg_def, last_key_def, last_key_t_def,
         fts_mem_def,SEP_CLAUSES, head_key_def, ft_mem_def, new_anode_def,
         new_dnode_def, head_key_t_def, ones_def, STAR_ASSOC] >>
      fs[AC STAR_ASSOC STAR_COMM] >>
      rpt strip_tac >> gvs[STAR_ASSOC] >>
      qexistsl [`fts`,`v.value`,`v.edges`] >>
      simp[ann_fts_def, ann_fts_seg_def, last_key_def, last_key_t_def,
           fts_mem_def,SEP_CLAUSES, head_key_def, ft_mem_def, new_anode_def,
           new_dnode_def, head_key_t_def, ones_def, STAR_ASSOC,
           empty_node_def] >>
      SEP_W_TAC >>
      fs[AC STAR_ASSOC STAR_COMM,b2w_def] >>
      fs[STAR_ASSOC] >>
      drule_all lemma_fib_heap_inv_weak_child >> strip_tac >>
      qspecl_then [`fh2 \\ a`,`(FDIFF fh1 (FDOM fh2))`,`[]`,`fts`]
        mp_tac logical_meld_fib_heap_inv_weak >>
      simp[] >>
      simp[lemma_fdom_disjoint_delete,lemma_funion_domsub_fdiff]
      ) >>
    Cases_on `h` >>
    `k <> 0w` by full_simp_tac (std_ss ++ sep_cond_ss)
      [cond_STAR,fts_mem_def, SEP_CLAUSES,STAR_ASSOC,ann_fts_seg_def,ann_fts_def,
       ft_mem_def,new_anode_def,new_dnode_def,ones_def] >>
    fs[head_key_t_def] >>
    `SUC (LENGTH t') < dimword (:'a)` by simp[] >> fs[] >>
    qspecl_then [`SUC (LENGTH t')`,`0w`,`a`,`[]`,`(FibTree k v' l::t')`,`m''`,`dm`,
      `ones a [v.value;FST v.edges;b2w T;b2w v.mark;a;a;0w;k;n2w (SUC (LENGTH t'))] *
       edges_ones (FST v.edges) (SND v.edges) *
       fts_mem (ann_fts 0w t) * frame`,`child_a`,`m'''`]
      mp_tac fib_heap_set_parent >>
    fs[ann_fts_def, ann_fts_seg_def, last_key_def, last_key_t_def,
       fts_mem_def,SEP_CLAUSES, head_key_def, ft_mem_def, new_anode_def,
       new_dnode_def, head_key_t_def, ones_def, STAR_ASSOC] >>
    fs[AC STAR_ASSOC STAR_COMM] >>
    ...
    >- (
      strip_tac >> gvs[] >>
      pairarg_tac >>
      rename [`fib_heap_meld (child_a,head_key_t 0w fts',m3,dm) = (new_a,m4,c3)`] >>
      fs[] >>
      qspecl_then [` ones a [v.value;FST v.edges;b2w T;b2w v.mark;a;a;
                             0w;child_a;n2w (SUC (LENGTH t'))] *
        edges_ones (FST v.edges) (SND v.edges) * frame`,
        `fh2 \\ a`,`(FibTree child_a v' l::t')`,`FDIFF fh1 (FDOM fh2)`,`fts'`,
        `m3`, `dm`,`new_a`, `m4`,`c3`] mp_tac fib_heap_meld_weak >>
      drule_all lemma_fib_heap_inv_weak_child >> strip_tac >>
      simp[lemma_fdom_disjoint_delete,head_key_def,head_key_t_def] >>
      fs[ann_fts_def, ann_fts_seg_def, last_key_def, last_key_t_def,
         fts_mem_def,SEP_CLAUSES, head_key_def, ft_mem_def, new_anode_def,
         new_dnode_def, head_key_t_def, ones_def, STAR_ASSOC] >>
      fs[AC STAR_ASSOC STAR_COMM] >>
      rpt strip_tac >> gvs[STAR_ASSOC] >>
      qexistsl [`fts_meld (FibTree child_a v' l::t') fts'`,`v.value`,`v.edges`] >>
      simp[ann_fts_def, ann_fts_seg_def, last_key_def, last_key_t_def,
           fts_mem_def,SEP_CLAUSES, head_key_def, ft_mem_def, new_anode_def,
           new_dnode_def, head_key_t_def, ones_def, STAR_ASSOC,
           empty_node_def] >>
      ...
      fs[AC STAR_ASSOC STAR_COMM,b2w_def] >>
      fs[STAR_ASSOC] >>
      drule_all lemma_funion_domsub_fdiff >> strip_tac >> gvs[]
      ) >>
    simp[] >>
    pairarg_tac >> simp[]
    ) >>
...
Theorem fib_heap_merge_trees_mem_thm:
  ...
  ==>
  (fts_mem (ann_fts 0w [fts_merge_trees (FibTree k v l) (FibTree k' v' l')]) * frame)
    (fun2set(m',dm)) /\ c /\
  a = head_key [fts_merge_trees (FibTree k v l) (FibTree k' v' l')]
Proof
  rpt gen_tac >> disch_tac >> fs[] >>
  pop_assum mp_tac >>
  ...
  full_simp_tac (std_ss ++ sep_cond_ss) [cond_STAR] >>
  SEP_R_TAC >> simp[] >>
  IF_CASES_TAC >> simp[] >> pairarg_tac >> gvs[]
  >- (
    disch_tac >> gvs[] >>
    fs[GSYM head_key_def,GSYM last_key_def] >>
    fs[GSYM lemma_ann_fts_arb_list] >>
    qspecl_then [`frame * ones a [v.value;FST v.edges;b2w T; b2w v.mark;
      a;a;0w;head_key l; n2w (LENGTH l)] * edges_ones (FST v.edges) (SND v.edges)`,
      `a`,`l`,`[FibTree k' v' l']`,`m⦇k' + 6w * bytes_in_word ↦ Word a⦈`,`dm`,
      `new_child`,`m''`,`c`] mp_tac fib_heap_meld_imp_fts_meld_mem_thm >>
    fs[head_key_t_def,head_key_def] >>
    fs[ann_fts_def, ann_fts_seg_def, last_key_def, last_key_t_def,
       fts_mem_def,SEP_CLAUSES, head_key_def, ft_mem_def, new_anode_def,
       new_dnode_def, head_key_t_def, ones_def, STAR_ASSOC] >>
  ...
  fs[GSYM lemma_ann_fts_arb_list] >>
  qspecl_then [`frame * ones a [v'.value;FST v'.edges;b2w T; b2w v'.mark;
    a;a;0w;head_key l'; n2w (LENGTH l')] *
    edges_ones (FST v'.edges) (SND v'.edges)`,
    `a`,`l'`,`[FibTree k v l]`,`m⦇k + 6w * bytes_in_word ↦ Word a⦈`,`dm`,
    `new_child`,`m''`,`c`] mp_tac fib_heap_meld_imp_fts_meld_mem_thm >>
  fs[ann_fts_def, ann_fts_seg_def, last_key_def, last_key_t_def,
...
Theorem fib_heap_merge_trees:
  ...
  fib_heap_inv fh2 [FibTree k' v' l'] /\
  DISJOINT (FDOM fh1) (FDOM fh2) /\
  fib_heap_merge_trees (k,k',m,dm) = (a,m',c) ==>
  ?ft.
  (fts_mem (ann_fts 0w [ft]) * frame) (fun2set(m',dm)) /\
  c /\
  a = head_key [ft] /\
  fib_heap_inv (FUNION fh1 fh2) [ft]
Proof
  rpt gen_tac >> disch_tac >> fs[] >>
  qexists `fts_merge_trees (FibTree k v l) (FibTree k' v' l')` >>
  drule_all fib_heap_merge_trees_mem_thm >> strip_tac >> simp[] >>
  metis_tac[fts_merge_trees]
QED
...
Theorem fib_heap_collect_array_mem_thm:
  !n arr rl frame acc m dm c acc' m'.
...
Proof
  Induct >> rpt gen_tac >> disch_tac >> fs[] >> pop_assum mp_tac
  >- (
    simp[Once fib_heap_collect_array_def,read_mem_def] >>
    pairarg_tac >> simp[] >>
    strip_tac >> gvs[] >>
    `0 < LENGTH rl` by simp[] >>
    drule_all LESS_LENGTH  >> strip_tac >> fs[] >>
    fs[reb_array_mem_def] >>
    Cases_on `y` >> fs[]
    >- (
      simp[reb_array_mem_def] >>
      SEP_R_TAC >> fs[] >>
      qspecl_then [`one(arr,Word 0w) * reb_array_mem arr 1w ys2 * frame`,
        `0w`,`[]`,`acc`,`m`,`dm`,`acc'`,`m'`,`T`]
        mp_tac fib_heap_meld_imp_fts_meld_mem_thm >>
      fs[fts_mem_def,ann_fts_def,SEP_CLAUSES,STAR_ASSOC,fts_meld_def,
         head_key_def,head_key_t_def] >>
      strip_tac >>
      simp[Once fts_collect_array_def] >>
    ...
    simp[write_mem_def] >>
    strip_tac >> gvs[] >>
    qspecl_then [`one(arr,Word k) * reb_array_mem arr 1w ys2 * frame`,
      `0w`,`[FibTree k v l]`,`acc`,`m`,`dm`,`acc'`,`m''`,`T`]
      mp_tac fib_heap_meld_imp_fts_meld_mem_thm >>
    fs[fts_meld_def,head_key_def,head_key_t_def] >>
    fs[AC STAR_ASSOC STAR_COMM] >>
    fs[STAR_ASSOC] >>
    strip_tac >>
    simp[Once fts_collect_array_def] >>
  ...
  pairarg_tac >> gvs[] >>
  `SUC n < LENGTH rl` by simp[] >>
  drule_all LESS_LENGTH >> strip_tac >> fs[] >>
  fs[reb_array_mem_append_thm] >>
  Cases_on `y` >> fs[reb_array_mem_def]
  >- (
    SEP_R_TAC >> fs[] >>
    qspecl_then [`reb_array_mem arr 0w rl * frame`,
      `0w`,`[]`,`acc`,`m`,`dm`,`acc''`,`m''`,`c''`]
      mp_tac fib_heap_meld_imp_fts_meld_mem_thm >>
    fs[head_key_def,head_key_t_def,SEP_CLAUSES] >>
    simp[fts_mem_def,ann_fts_def,SEP_CLAUSES,STAR_ASSOC,fts_meld_def,
         head_key_def,head_key_t_def,reb_array_mem_append_thm,reb_array_mem_def] >>
    fs[AC STAR_ASSOC STAR_COMM] >>
    strip_tac >> fs[STAR_ASSOC] >>
    strip_tac >>
    first_x_assum (qspecl_then [`arr`,`rl`,`frame`,`acc`,`m''`,`dm`,
      `T`,`acc'`,`m'`] mp_tac) >> fs[] >>
    fs[reb_array_mem_append_thm,reb_array_mem_def,SEP_CLAUSES,fts_meld_def,
       head_key_def] >>
    fs[AC STAR_ASSOC STAR_COMM] >>
    fs[STAR_ASSOC] >> strip_tac >>
  ...
  qspecl_then [`reb_array_mem arr 0w ys1 * one (arr + n2w (SUC n), Word k) *
    ...
    `0w`,`[FibTree k v l]`,`acc`,`m`,`dm`,`acc''`,`m''`,`c''`]
    mp_tac fib_heap_meld_imp_fts_meld_mem_thm >>
  fs[SEP_CLAUSES,head_key_t_def,head_key_def] >>
  fs[AC STAR_ASSOC STAR_COMM] >>
  strip_tac >> fs[STAR_ASSOC] >>
  `k <> 0w` by full_simp_tac (std_ss ++ sep_cond_ss) [cond_STAR,
     ann_fts_def, ann_fts_seg_def, last_key_def, last_key_t_def,
  ...
  simp[] >>
  strip_tac >>
  first_x_assum (qspecl_then [`arr`,`LUPDATE NONE (LENGTH ys1) rl`,`frame`,
    `fts_meld [FibTree k v l] acc`,
    `m''(|(arr + n2w (SUC n)) |-> Word 0w|)`,`dm`,`T`,`acc'`,`m'`] mp_tac) >> fs[] >>
  simp[LUPDATE_APPEND,LUPDATE_DEF,reb_array_mem_append_thm,reb_array_mem_def,
...
Proof
  ...
  >- (
    strip_tac >> simp[] >>
    qpat_x_assum `fts_extract_min fts = (a_min, fts',rl,c)` mp_tac >>
    simp[fts_extract_min_def] >>
    pairarg_tac >> gvs[] >>
    pairarg_tac >> gvs[] >>
    strip_tac >> gvs[] >>
    pairarg_tac >> simp[] >>
    qspecl_then [`n`,`empty_node a_min (v,e) * frame`,`fts''`,`arr`,
      `m''`,`dm`,`a''`,`m'''`] mp_tac fib_heap_reb_mem_thm >>
    fs[AC STAR_ASSOC STAR_COMM] >>
    fs[STAR_ASSOC] >> strip_tac >>
    strip_tac >> gvs[] >>
    qexistsl [`v`,`e`] >>
    drule_all lemma_fts_reb_empty_array >> strip_tac >> simp[]
    ) >>
  pairarg_tac >> gvs[]
QED
...
Proof
  rpt gen_tac >> disch_tac >> fs[] >>
  Cases_on `fts_extract_min fts` >>
  Cases_on `r` >>
  Cases_on `r'` >>
  rename [`fts_extract_min fts = (min,fts',e_rl,flag)`] >>
  qexists `fts'` >>
  qspecl_then [`frame`,`fts`,`arr`,`m`,`dm`,`min`,`fts'`,`e_rl`,`flag`, `n`,
    `min'`,`a'`,`m'`] mp_tac fib_heap_ext_min_mem >>
  strip_tac >> gvs[] >>
  qexistsl [`v`,`e`] >> simp[] >>
  irule fts_extract_min >>
