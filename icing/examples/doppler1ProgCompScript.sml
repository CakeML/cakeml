...
val _ = new_theory "doppler1ProgComp";

val _ = translation_extends "cfSupport";

Definition theAST_pre_def:
  theAST_pre = \ (x:(string, string) id).
  if x = Short "T" then (((-30)/(1), (50)/(1)):real#real)
  else if x = Short "u" then (((-100)/(1), (100)/(1)):real#real)
  else if x = Short "v" then (((20)/(1), (20000)/(1)):real#real)
  else (0,0)
End

Definition theAST_def:
  theAST =
  [ Dlet unknown_loc (Pvar "doppler1")
    (Fun "u"(Fun "v"(Fun "T"
      (FpOptimise Opt
        (Let (SOME "t1")
        (App (FP_bop FP_Add)
