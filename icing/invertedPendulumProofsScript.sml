...
open source_to_sourceTheory source_to_sourceProofsTheory CakeMLtoFloVerTheory
     CakeMLtoFloVerProofsTheory icing_optimisationProofsTheory icing_optimisationsLib invertedPendulumProgCompTheory cfSupportTheory;
...
val _ = new_theory "invertedPendulumProofs";
...
Definition invertedPendulum_opt_real_spec_def:
  invertedPendulum_opt_real_spec =
  (λ w1.
   λ w2.
   λ w3.
   λ w4.
...
Theorem invertedPendulum_spec:
  ∀ w1 w2 w3 w4 d1 d2 d3 d4.
    ...
    let result = (invertedPendulum_opt_float_option w1 w2 w3 w4) in
      (∀ p.
        app (p:'ffi ffi_proj) ^(fetch_v "invertedPendulum" st)
          [d1; d2; d3; d4]
          (emp)
          (POSTv v.
           &DOUBLE_RES result v)) ∧
...
Theorem main_spec:
  ...
  invertedPendulum_side c1 c2 c3 c4 ⇒
  let
    result = invertedPendulum_opt_float_option c1 c2 c3 c4
  in
  app p ^(fetch_v "main" st)
    ...
    ∧
    invertedPendulum_float_returns (c1,c2,c3,c4) (compress_word (THE result)) ∧
    real$abs (fp64_to_real (compress_word (THE result)) -
      invertedPendulum_real_fun c1 c2 c3 c4) ≤ theErrBound
Proof
  simp[] \\ rpt strip_tac
  \\ first_x_assum (mp_then Any assume_tac (SIMP_RULE std_ss [] (INST_TYPE [“:'ffi”|->“:'a”] invertedPendulum_spec)))
  >- (
   xcf "main" st
   \\ xlet_auto >- (xcon \\ xsimpl)
...
Theorem main_whole_prog_spec:
  ...
  NONE
  ((=)
   (add_stdout fs (mlint$toString (&w2n (compress_word (THE (invertedPendulum_opt_float_option c1 c2 c3 c4)))))))
  ∧
  invertedPendulum_float_returns (c1,c2,c3,c4) (compress_word (THE (invertedPendulum_opt_float_option c1 c2 c3 c4))) ∧
  real$abs (fp64_to_real (compress_word (THE (invertedPendulum_opt_float_option c1 c2 c3 c4))) -
            invertedPendulum_real_fun c1 c2 c3 c4) ≤ theErrBound
Proof
...
val invertedPendulum_prog_tm = rhs (concl prog_rewrite);

val invertedPendulum_prog_def = Define`invertedPendulum_prog = ^invertedPendulum_prog_tm`;
...
Theorem invertedPendulum_semantics =
  full_semantics_prog_thm |> ONCE_REWRITE_RULE[GSYM invertedPendulum_prog_def]
...
Definition invertedPendulum_semantics_side_def:
  invertedPendulum_semantics_side (s1,s2,s3,s4) (c1,c2,c3,c4) ⇔
...
Theorem invertedPendulum_semantics_final:
  invertedPendulum_semantics_side (s1,s2,s3,s4) (c1,c2,c3,c4) ∧ init_ok ([fname;s1;s2;s3;s4],fs) ⇒
  ∃ (w:word64).
    CakeML_evaluates_and_prints ([fname;s1;s2;s3;s4],fs,invertedPendulum_prog) (toString w) ∧
    invertedPendulum_float_returns (c1,c2,c3,c4) w ∧
    real$abs (fp64_to_real w - invertedPendulum_real_fun c1 c2 c3 c4) ≤ theErrBound
Proof
  rpt strip_tac
  \\ fs[init_ok_def, CakeML_evaluates_and_prints_def, invertedPendulum_semantics_side_def]
  \\ first_x_assum (mp_then Any mp_tac invertedPendulum_semantics)
  \\ rpt (disch_then drule)
  \\ strip_tac \\ fs[]
...
Let InvertedPendulum be the following program ... using floating-point operations,
let InvertedPendulumReal be to_real(InvertedPendulum), where to_real syntactically replaces all
floating-point operations by their real-numbered counterparts, and let
InvertedPendulumOpt = optimise(InvertedPendulum, ids), where optimise syntactically transforms a
...
real_semantics(InvertedPendulumReal) = print(r), then there is a floating-point word w
such that semantics(InvertedPendulumOpt) = print(w), and |real(w)-r| ≤ error(InvertedPendulumOpt) ≤ the user given error constraint ε
(with respect to InvertedPendulumReal), where error uses the FloVer analysis tool to
compute an upper bound on the worst-case roundoff error between InvertedPendulumReal and InvertedPendulumOpt.

or

Let InvertedPendulumReal be the following program ... that uses real-number operations,
let InvertedPendulum be floatify(InvertedPendulumReal), where floatify syntactically turns all
...
Then if there is a real number r such that
real_semantics(InvertedPendulumReal) = print(r), then there is a floating-point word w
such that semantics(InvertedPendulumOpt) = print(w), and |real(w)-r| < error(InvertedPendulumReal),
where error uses the FloVer analysis tool to compute an upper bound on the
worst-case roundoff error between InvertedPendulumReal and InvertedPendulumOpt.
**)
