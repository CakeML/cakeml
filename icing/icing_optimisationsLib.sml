...
local
  val empty_plan_correct_thm =
    let val empty_rw_list_correct = MP (Q.SPEC ‘[]’ lift_rewriteFPexp_correct_list) empty_rw_correct
    in
      Q.SPEC ‘[]’ is_rewriteFPexp_correct_lift_perform_rewrites
      |> (fn th => MP th empty_rw_list_correct)
    end;
  ...
  | p1 :: ps => (* Cons case *)
      let
       ...
       in
        ...
        if (Term.compare(dtype_comb, “Label”) = EQUAL)
        then
          (full_plan,
