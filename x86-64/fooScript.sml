...
val labelled_bootstrap_code_eq =
  (PURE_ONCE_REWRITE_CONV[initial_bc_state_code_eq] THENC
   PURE_ONCE_REWRITE_CONV[compile_call_repl_step_eq] THENC
   computeLib.CBV_CONV cs THENC EVAL)
  ``initial_bc_state.code++REVERSE(SND(THE prim_env))++REVERSE(SND(SND(compile_repl_module)))
    ++REVERSE compile_call_repl_step``

val bootstrap_code_labels_ok0 = prove(
  ``code_labels_ok ^(rand(rhs(concl bootstrap_code_def)))``,
  PURE_REWRITE_TAC[bootstrap_code_labelled_def,bootstrap_bc_state_code,GSYM listTheory.APPEND_ASSOC] >>
  ...
  reverse conj_tac >- (
    simp[compile_call_repl_step_eq] >>
    EVAL_TAC >> simp[] ) >>
  match_mp_tac bytecodeLabelsTheory.code_labels_ok_append >>
  reverse conj_tac >- (
...
val bootstrap_code_labels_ok =
  ...
  |> CONV_RULE(RAND_CONV(REWR_CONV bootstrap_code_labelled_def  THENC
       (LAND_CONV(REWR_CONV bootstrap_bc_state_code))
       THENC (REWR_CONV labelled_bootstrap_code_eq)))
...
val bootstrap_code_eq = save_thm("bootstrap_code_eq",
  time (REWR_CONV bootstrap_code_def THENC
       (RAND_CONV(REWR_CONV bootstrap_code_labelled_def  THENC
       (LAND_CONV(REWR_CONV bootstrap_bc_state_code))
       THENC (REWR_CONV labelled_bootstrap_code_eq))) THENC
        eval)
  ``bootstrap_code``)
