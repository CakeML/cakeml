structure alt_basisLib :> alt_basisLib =
struct

  open basis ml_progLib basis_ffiTheory semanticsLib helperLib set_sepTheory
       cfHeapsBaseTheory
  open alt_basisTheory
  ...
  fun set_user_heap_thm thm =
    let
      val _ = print "Setting user heap theorem..."
      val _ = (user_heap_thm := [thm])
    in
      print "ok\n"
    end

  fun get_user_heap_thm () =
    case (!user_heap_thm) of
      [] => NONE
    | xs => SOME (List.hd xs)
  ...
  fun whole_prog_term st name spec =
    let
      val call_ERR = ERR "whole_prog_term"
      val th =
        whole_prog_spec_semantics_prog
          |> C MATCH_MP (st |> get_thm |> GEN_ALL |> ISPEC basis_ffi_tm)
          |> SPEC(stringSyntax.fromMLstring name)
