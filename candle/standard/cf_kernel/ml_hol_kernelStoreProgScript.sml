...
open ml_monadBaseTheory ml_monadProgTheory ml_monadStoreLib holKernelTheory

val _ = new_theory "ml_hol_kernelStoreProg";
...
(* Create and translate the store *)
val init_type_constants_def = Define `
init_type_constants = [(strlit"bool",0); (strlit"fun",2:num)]`;
(* val refs_get_the_type_constants = ``\state. state.the_type_constants``;
val refs_set_the_type_constants = ``\x state. state with the_type_constants := x``; *)
    ...
    Tyapp (strlit"fun")
      [Tyvar (strlit"A");
       Tyapp (strlit"fun")
         [Tyvar (strlit"A");
          Tyapp (strlit"bool") []]])]`;
(* val refs_get_the_term_constants = ``\state. state.the_term_constants``;
val refs_set_the_term_constants = ``\x state. state with the_term_constants := x``; *)

val init_axioms_def = Define `
  init_axioms = []:thm list`;
(* val refs_get_the_axioms = ``\state. state.the_axioms``;
val refs_set_the_axioms = ``\x state. state with the_axioms := x``; *)

val init_context_def = Define `
  init_context = ^(rhs(concl(holSyntaxTheory.init_ctxt_def)))`;
(* val refs_get_the_context = ``\state. state.the_context``;
val refs_set_the_context = ``\x state. state with the_context := x``; *)
  
(* val refs_init_list = [
  ("the_type_constants", init_type_constants_def, refs_get_the_type_constants,
  refs_set_the_type_constants),
  ("the_term_constants", init_term_constants_def, refs_get_the_term_constants,
  refs_set_the_term_constants),
  ("the_axioms", init_axioms_def, refs_get_the_axioms, refs_set_the_axioms),
  ("the_context", init_context_def, refs_get_the_context, refs_set_the_context)
]; *)
val refs_init_list = [
...
(* Perform the translation of the init values for the references of the store *)
...
val def = vsubst_def;
m_translate def;
val tm = (def (* rec *) |> m_translate; ``T``) handle BREAKPOINT tm => tm;
val tm = concl assoc_def |> strip_forall |> snd |> dest_eq |> snd |> rator |> rand
(* END DEBUG *)
