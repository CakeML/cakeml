...
Overload p[local] = ``Var (implode "p") Bool``
Overload FAp[local] = ``Forall (implode "p") Bool``
Overload q[local] = ``Var (implode "q") Bool``
Overload FAq[local] = ``Forall (implode "q") Bool``
Overload r[local] = ``Var (implode "r") Bool``
Overload FAr[local] = ``Forall (implode "r") Bool``
Overload f[local] = ``Var (implode "f") (Fun Bool (Fun Bool Bool))``
Overload A[local] = ``Tyvar (implode "A")``
Overload P[local] = ``Var (implode "P") (Fun A Bool)``
Overload x[local] = ``Var (implode "x") A``
Overload FAx[local] = ``Forall (implode "x") A``
...
Theorem bool_sig_instances:
   is_bool_sig sig ⇒
    instance (tmsof sig) (i:'U interpretation) (implode "T") Bool = (K (tmaof i (implode "T") [])) ∧
    instance (tmsof sig) i (implode "F") Bool = (K (tmaof i (implode "F") [])) ∧
    instance (tmsof sig) i (implode "==>") (Fun Bool (Fun Bool Bool)) = (K (tmaof i (implode "==>") [])) ∧
    instance (tmsof sig) i (implode "/\\") (Fun Bool (Fun Bool Bool)) = (K (tmaof i (implode "/\\") [])) ∧
    instance (tmsof sig) i (implode "\\/") (Fun Bool (Fun Bool Bool)) = (K (tmaof i (implode "\\/") [])) ∧
    instance (tmsof sig) i (implode "~") (Fun Bool Bool) = (K (tmaof i (implode "~") [])) ∧
    instance (tmsof sig) i (implode "!") (Fun (Fun A Bool) Bool) = (λτ. tmaof i (implode "!") [τ (implode "A")]) ∧
    instance (tmsof sig) i (implode "?") (Fun (Fun A Bool) Bool) = (λτ. tmaof i (implode "?") [τ (implode "A")])
Proof
...
val is_true_interpretation_def = xDefine"is_true_interpretation"`
  is_true_interpretation0 ^mem γ ⇔ (γ:'U tmass) interprets (implode "T") on [] as K True`
Overload is_true_interpretation = ``is_true_interpretation0 ^mem``

val is_and_interpretation_def = xDefine"is_and_interpretation"`
  is_and_interpretation0 ^mem γ ⇔ γ interprets (implode "/\\") on [] as K (Boolrel $/\)`
...
val is_implies_interpretation_def = xDefine"is_implies_interpretation"`
  is_implies_interpretation0 ^mem γ ⇔ γ interprets (implode "==>") on [] as K (Boolrel $==>)`
...
val is_forall_interpretation_def = xDefine"is_forall_interpretation"`
  is_forall_interpretation0 ^mem γ ⇔ γ
    interprets (implode "!") on [implode "A"] as
...
val is_or_interpretation_def = xDefine"is_or_interpretation"`
  is_or_interpretation0 ^mem γ ⇔ γ interprets (implode "\\/") on [] as K (Boolrel $\/)`
Overload is_or_interpretation = ``is_or_interpretation0 ^mem``

val is_false_interpretation_def = xDefine"is_false_interpretation"`
  is_false_interpretation0 ^mem γ ⇔ (γ:'U tmass) interprets (implode "F") on [] as K False`
Overload is_false_interpretation = ``is_false_interpretation0 ^mem``

val is_not_interpretation_def = xDefine"is_not_interpretation"`
  is_not_interpretation0 ^mem γ ⇔ γ interprets (implode "~") on [] as K (Abstract boolset boolset (λp. Boolean (p ≠ True)))`
...
Proof
  ...
  `FLOOKUP (tysof sig) (implode "bool") = SOME 0` by (
    qpat_x_assum`is_std_sig _` mp_tac >>
    simp[is_std_sig_def,Abbr`sig`,Abbr`ctx`]) >>
  `FLOOKUP (tysof sig) (implode "fun") = SOME 2` by (
    qpat_x_assum`is_std_sig _` mp_tac >>
    simp[is_std_sig_def,Abbr`sig`,Abbr`ctx`]) >>
  simp ints >>
  conj_asm1_tac >- (
    init_tac`Const (implode "T") Bool === TrueDef` >>
  ...
  conj_asm1_tac >- (
    init_tac `Const (implode "/\\") (Fun Bool (Fun Bool Bool)) === AndDef` >>
    ...
    metis_tac[] ) >>
  conj_asm1_tac >- (
    init_tac `Const (implode "==>") (Fun Bool (Fun Bool Bool)) === ImpliesDef` >>
    imp_res_tac is_std_interpretation_is_type >>
    imp_res_tac typesem_Bool >> simp[Boolrel_def] >>
    match_mp_tac (UNDISCH abstract_eq) >>
    imp_res_tac typesem_Fun >>
  ...
  conj_asm1_tac >- (
    init_tac `Const (implode "!") (Fun (Fun A Bool) Bool) === ForallDef` >>
    ...
    simp[typesem_def] >>
    Q.PAT_ABBREV_TAC`aa:'U = X (implode "A")` >>
    ...
    simp[mem_boolset] ) >>
  conj_asm1_tac >- (
    init_tac `Const (implode "?") (Fun (Fun A Bool) Bool) === ExistsDef` >>
    `τ = tyvof v` by simp[Abbr`v`] >> fs[] >>
    imp_res_tac is_std_interpretation_is_type >>
    imp_res_tac typesem_Bool >> simp[] >>
    imp_res_tac typesem_Fun >> simp[typesem_def] >>
    ...
    qx_gen_tac`pp` >> strip_tac >>
    simp[boolean_in_boolset] >>
    qspecl_then[`tmsof sig`,`i`,`implode "!"`,`Fun (Fun Bool Bool) Bool`,`Fun (Fun A Bool) Bool`,`[(Bool,A)]`]mp_tac instance_def >>
    ...
    fs[interprets_def] >>
    qpat_x_assum`∀t. is_type_valuation t ⇒ Z`(fn th => assume_tac th >> (qspec_then`(implode "A" =+ boolset)τ`mp_tac) th) >>
    impl_tac >- (
    ...
    simp[Abbr`i1`] >>
    simp[holds_def] >>
    `∀x a. x <: boolset ∧ a <: tyvof v (implode "A") ⇒ Abstract (tyvof v (implode "A")) boolset (i2 x) ' a = i2 x a` by (
      rw[] >>
      match_mp_tac apply_abstract_matchable >>
      rw[Abbr`i2`] >>
      apply_abstract_tac ) >>
    ...
    `∀x. x <: tyvof v (implode "A") ⇒ pp ' x <: boolset` by (rw[] >> apply_abstract_tac) >>
    simp[SIMP_RULE(srw_ss())[]apply_boolrel,boolean_eq_true] >>
    ntac 20 (pop_assum kall_tac) >>
    simp[boolean_def] >>
    metis_tac[mem_boolset] ) >>
  conj_asm1_tac >- (
    init_tac `Const (implode "\\/") (Fun Bool (Fun Bool Bool)) === OrDef` >>
    pop_assum kall_tac >>
    imp_res_tac is_std_interpretation_is_type >>
    imp_res_tac typesem_Bool >> simp[Boolrel_def] >>
    match_mp_tac (UNDISCH abstract_eq) >>
    ...
    simp[tyvars_def,STRING_SORT_def,LIST_UNION_def,LIST_INSERT_def,INORDER_INSERT_def,REV_ASSOCD] >>
    fs[interprets_def] >>
    qpat_x_assum`∀t. is_type_valuation t ⇒ tmaof i (implode "!") Z = Y`(fn th => assume_tac th >> (qspec_then`(implode "A" =+ boolset)τ`mp_tac) th) >>
    impl_tac >- (
      fs[is_type_valuation_def,combinTheory.APPLY_UPDATE_THM] >> rw[] >>
      metis_tac[boolean_in_boolset] ) >>
    simp[combinTheory.APPLY_UPDATE_THM] >>
    ...
    metis_tac[mem_boolset] ) >>
  conj_asm1_tac >- (
    init_tac`Const (implode "F") Bool === FalseDef` >>
    pop_assum kall_tac >>
    imp_res_tac is_std_interpretation_is_type >>
    imp_res_tac typesem_Bool >> simp[] >>
    imp_res_tac typesem_Fun >>
    ...
    match_mp_tac apply_abstract_matchable >>
    simp[mem_boolset] ) >>
  init_tac`Const (implode "~") (Fun Bool Bool) === NotDef` >>
  pop_assum kall_tac >>
  imp_res_tac is_std_interpretation_is_type >>
  imp_res_tac typesem_Bool >> simp[] >>
  fs[interprets_def] >>
...
Proof
  ...
  qspecl_then[`tmsof s`,`i`,`implode"==>"`]mp_tac instance_def >> simp[] >>
...
Proof
  ...
  qspecl_then[`tmsof s`,`i`,`implode"!"`]mp_tac instance_def >> simp[] >>
  disch_then(qspec_then`[y,Tyvar(implode"A")]`mp_tac) >>
