open preamble MiniMLTheory

val _ = new_theory "evaluateEquations"

(* functional equations for evaluate *)

val evaluate_raise = Q.store_thm (
"evaluate_raise",
`!cenv s env err bv.
  (evaluate cenv s env (Raise err) bv = (bv = Rerr (Rraise err)))`,
rw [Once evaluate_cases]);

val evaluate_lit = Q.store_thm(
"evaluate_lit",
`!cenv s env l r.
  (evaluate cenv s env (Lit l) r = (r = Rval (s,Litv l)))`,
rw [Once evaluate_cases]);

val evaluate_var = store_thm(
"evaluate_var",
``∀cenv s env n r. evaluate cenv s env (Var n) r =
  (∃v. (lookup n env = SOME v) ∧ (r = Rval (s,v))) ∨
  ((lookup n env = NONE) ∧ (r = Rerr Rtype_error))``,
rw [Once evaluate_cases] >>
metis_tac [])

val evaluate_fun = store_thm(
"evaluate_fun",
``∀cenv s env n e r. evaluate cenv s env (Fun n e) r = (r = Rval (s, Closure env n e))``,
rw [Once evaluate_cases])

val _ = export_rewrites["evaluate_raise","evaluate_lit","evaluate_fun"];

val evaluate_con = Q.store_thm(
"evaluate_con",
`!cenv env cn es r s1.
  (evaluate cenv s1 env (Con cn es) r =
   if do_con_check cenv cn (LENGTH es) then
     (∃err. evaluate_list cenv s1 env es (Rerr err) ∧
            (r = Rerr err)) ∨
     (∃vs s2. evaluate_list cenv s1 env es (Rval (s2,vs)) ∧
           (r = Rval (s2, Conv cn vs)))
   else (r = Rerr Rtype_error))`,
rw [Once evaluate_cases] >>
metis_tac []);

val evaluate_app = store_thm(
"evaluate_app",
``∀cenv s1 env op e1 e2 r. evaluate cenv s1 env (App op e1 e2) r =
  (∃v1 v2 env' e s2 s3 s4. 
     evaluate cenv s1 env e1 (Rval (s2,v1)) ∧ 
     (evaluate cenv s2 env e2 (Rval (s3,v2))) ∧
                  (do_app s3 env op v1 v2 = SOME (s4,env',e)) ∧
                  evaluate cenv s4 env' e r) ∨
  (∃v1 v2 s2 s3. 
     evaluate cenv s1 env e1 (Rval (s2,v1)) ∧ 
     (evaluate cenv s2 env e2 (Rval (s3,v2))) ∧
           (do_app s3 env op v1 v2 = NONE) ∧
           (r = Rerr Rtype_error)) ∨
  (∃v1 err s2. evaluate cenv s1 env e1 (Rval (s2,v1)) ∧ (evaluate cenv s2 env e2 (Rerr err)) ∧
            (r = Rerr err)) ∨
  (∃err. evaluate cenv s1 env e1 (Rerr err) ∧
         (r = Rerr err))``,
rw[Once evaluate_cases] >>
metis_tac [])

val evaluate'_raise = store_thm(
"evaluate'_raise",
``∀env s err r. evaluate' s env (Raise err) r = (r = Rerr (Rraise err))``,
rw [Once evaluate'_cases])

val evaluate'_lit = store_thm(
"evaluate'_lit",
``∀s env l r. evaluate' s env (Lit l) r = (r = Rval (s,Litv l))``,
rw [Once evaluate'_cases])

val evaluate'_fun = store_thm(
"evaluate'_fun",
``∀s env n e r. evaluate' s env (Fun n e) r = (r = Rval (s, Closure env n e))``,
rw [Once evaluate'_cases])

val _ = export_rewrites["evaluate'_raise","evaluate'_lit","evaluate'_fun"]

val evaluate'_con = store_thm(
"evaluate'_con",
``∀s env cn es r. evaluate' s env (Con cn es) r =
  (∃err. evaluate_list' s env es (Rerr err) ∧ (r = Rerr err)) ∨
  (∃vs s2. evaluate_list' s env es (Rval (s2,vs)) ∧ (r = Rval (s2,Conv cn vs)))``,
rw [Once evaluate'_cases] >>
metis_tac [])

val evaluate'_var = store_thm(
"evaluate'_var",
``∀s env n r. evaluate' s env (Var n) r =
  (∃v. (lookup n env = SOME v) ∧ (r = Rval (s,v))) ∨
  ((lookup n env = NONE) ∧ (r = Rerr Rtype_error))``,
rw [Once evaluate'_cases] >>
metis_tac [])

val evaluate_list'_thm = store_thm(
"evaluate_list'_thm",
``∀s env r.
  (evaluate_list' s env [] r = (r = Rval (s,[]))) ∧
  (∀e es. evaluate_list' s env (e::es) r =
   (∃v vs s2 s3. evaluate' s env e (Rval (s2,v)) ∧ 
                 evaluate_list' s2 env es (Rval (s3,vs)) ∧ 
                 (r = Rval (s3,v::vs))) ∨
   (∃err. evaluate' s env e (Rerr err) ∧
          (r = Rerr err)) ∨
   (∃v err s2. evaluate' s env e (Rval (s2,v)) ∧ evaluate_list' s2 env es (Rerr err) ∧
            (r = Rerr err)))``,
rw[] >-
  rw[Once evaluate'_cases] >>
rw[EQ_IMP_THM] >- (
  pop_assum (mp_tac o (SIMP_RULE (srw_ss()) [Once evaluate'_cases])) >>
  rw [] >> metis_tac[] ) >>
rw[Once evaluate'_cases] >>
metis_tac [])

val evaluate'_app = store_thm(
"evaluate'_app",
``∀s env op e1 e2 r. evaluate' s env (App op e1 e2) r =
  (∃v1 v2 env' e s2 s3 s4. 
                  evaluate' s env e1 (Rval (s2,v1)) ∧ 
                  (evaluate' s2 env e2 (Rval (s3,v2))) ∧
                  (do_app s3 env op v1 v2 = SOME (s4, env',e)) ∧
                  evaluate' s4 env' e r) ∨
  (∃v1 v2 s2 s3. 
           evaluate' s env e1 (Rval (s2,v1)) ∧ (evaluate' s2 env e2 (Rval (s3,v2))) ∧
           (do_app s3 env op v1 v2 = NONE) ∧
           (r = Rerr Rtype_error)) ∨
  (∃v1 err s2. evaluate' s env e1 (Rval (s2,v1)) ∧ (evaluate' s2 env e2 (Rerr err)) ∧
            (r = Rerr err)) ∨
  (∃err. evaluate' s env e1 (Rerr err) ∧
         (r = Rerr err))``,
rw[Once evaluate'_cases] >>
metis_tac [])

val evaluate'_log = store_thm(
"evaluate'_log",
``∀s env lg e1 e2 r. evaluate' s env (Log lg e1 e2) r =
  (∃v1 env' e s2. 
     evaluate' s env e1 (Rval (s2,v1)) ∧ (do_log lg v1 e2 = SOME e) ∧ 
     (evaluate' s2 env e r)) ∨
  (∃v1 s2. evaluate' s env e1 (Rval (s2,v1)) ∧ (do_log lg v1 e2 = NONE) ∧ (r = Rerr Rtype_error)) ∨
  (∃err. evaluate' s env e1 (Rerr err) ∧ (r = Rerr err))``,
rw[Once evaluate'_cases] >>
metis_tac [])

val _ = export_theory ()
