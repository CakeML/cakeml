...
val _ = Hol_reln ` (! menv cenv s env v.
T
==>
...
/\ (! menv cenv env op e2 v1 v2 menv' cenv' env' e3 bv s1 s2 count s3.
(evaluate F menv cenv s1 env e2 ((count,s2), Rval v2) /\
((do_app s2 menv cenv env op v1 v2 = SOME (s3,menv', cenv',env', e3)) /\
...
(evaluate F menv cenv s1 env e2 ((count,s2), Rval v2) /\
(do_app s2 menv cenv env op v1 v2 = NONE))
==>
...
/\ (! menv cenv env op v1 v2 menv' cenv' env' e3 bv s1 s2 count.
((do_app s1 menv cenv env op v1 v2 = SOME (s2, menv', cenv', env', e3)) /\
...
(do_app s menv cenv env op v1 v2 = NONE)
==>
...
val _ = Hol_reln ` (! menv cenv res s.
T
==>
...
val _ = Hol_reln ` (! menv cenv env e c res bv s1 s2.
