 ...
 >- (
   ...
   fs[Once small_eval_list_cases] >> rw[] >- (
     ...
     TRY (
       ...
       simp_tac(srw_ss()++boolSimps.DNF_ss)[Once RTC_CASES2] >>
       first_assum(match_exists_tac o concl) >> simp[] >>
       simp[e_step_reln_def,e_step_def,continue_def,Abbr`ctx`,application_thm,return_def] ) >>
     `(s3,t3) = (s2,t2)` by (
       imp_res_tac do_app_type_error ) >> fs[] >>
     simp_tac(srw_ss()++boolSimps.DNF_ss)[Once RTC_CASES_RTC_TWICE] >>
     first_assum(match_exists_tac o concl) >> simp[] >>
...
[reverse (cases_on `e'`) >>
     ...
     >- (
       ...
       pop_assum mp_tac >> rw[] >>
       every_case_tac >> fs[] >>
       TRY(PairCases_on `s` >>fs[do_app_def]>>NO_TAC) >>
       rw[Once evaluate_cases] >>
       rw[Once evaluate_cases] >>
       rw[Once evaluate_cases] >>
       fs [return_def] >>
       metis_tac[evaluate_ctxts_type_error,FORALL_PROD, pair_CASES]) >>
     metis_tac [evaluate_ctxts_type_error,do_con_check_build_conv, NOT_SOME_NONE, pair_CASES],
 fs [continue_def] >>
     cases_on `c` >>
     fs [] >>
