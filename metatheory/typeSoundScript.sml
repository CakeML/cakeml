...
val final_state_def = Define `
  ...
  (final_state (tenvC,env,Exp (Raise err),[]) = T) ∧
  (final_state _ = F)`;

val not_final_state = Q.prove (
`!tenvC env e c.
  ¬final_state (tenvC,env,Exp e,c) =
     (?x y. c = x::y) ∨
     (?l. e = Lit l) ∨
     (?cn es. e = Con cn es) ∨
     (?v. e = Var v) ∨
...
[cases_on `e''` >>
     ...
     [every_case_tac >>
          ...
          qexists_tac `tenv` >>
          rw [] >>
          rw [Once type_e_cases, Once type_ctxts_cases],
      fs [return_def] >>
          rw [] >>
...
`type_state tenvC (envC,envE,Exp e,[]) t`
