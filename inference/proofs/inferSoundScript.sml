...
val lookup_tenv_NONE = prove(``
  ...
  Induct>>full_simp_tac(srw_ss())[lookup_tenv_def]>>srw_tac[][]>>
  metis_tac[])
...
`!env tvs tenv.
  ...
  srw_tac[][]>>
  full_simp_tac(srw_ss()) [tenv_invC_def,lookup_tenv_def,bind_tvar_def,tenv_ok_def] >>
  Cases_on`tvs=0`>>full_simp_tac(srw_ss())[lookup_tenv_def]
  >-
    metis_tac[]
  >>
  srw_tac[][]>>
  imp_res_tac lookup_tenv_SOME>>
  pop_assum (qspec_then`0` assume_tac)>>full_simp_tac(srw_ss())[]>>
...
`(!t s' t' x.
 ...
 srw_tac[] [generalise_def, check_t_def, LET_THM, LAMBDA_PROD]
...
val deBruijn_subst_convert = prove(``
  ...
  srw_tac[][check_t_def]>>
  full_simp_tac(srw_ss())[convert_t_def,deBruijn_subst_def,infer_deBruijn_subst_def]
  >-
    (IF_CASES_TAC>>full_simp_tac(srw_ss())[EL_MAP,convert_t_def])
  >>
...
`!mn decls tenvT tenvM menv cenv env d st1 st2 decls' tenvT' cenv' env' tenv.
 ...
 full_simp_tac(srw_ss()) [infer_d_def, success_eqns, type_d_cases] >>
 full_simp_tac(srw_ss()) []
 >- (`?t env. v' = (t,env)` by (PairCases_on `v'` >> metis_tac []) >>
     ...
     full_simp_tac(srw_ss()) [every_shim] >>
     ...
     `num_tvs tenv' = tvs`
                  by (Q.UNABBREV_TAC `tenv'` >>
                      ...
                      full_case_tac >>
                      srw_tac[] [num_tvs_def, num_tvs_bvl2]) >>
     ...
     srw_tac[] [] >>
     `(init_infer_state:(num |-> infer_t) infer_st).subst = FEMPTY` by full_simp_tac(srw_ss()) [init_infer_state_def] >>
     ...
     (qexists_tac `convert_t (t_walkstar last_sub t)` >>
          ...
          srw_tac[] [] >|
          [srw_tac[] [empty_decls_def, convert_decls_def],
           srw_tac[] [ZIP_MAP, MAP_MAP_o, combinTheory.o_DEF] >>
               REPEAT (pop_assum (fn _ => all_tac)) >> 
               induct_on `env''` >>
               srw_tac[] [convert_env2_def, tenv_add_tvs_def, convert_env_def] >-
               (PairCases_on `h` >>
                    srw_tac[] []) >>
               srw_tac[] [MAP_MAP_o, combinTheory.o_DEF, remove_pair_lem],
           match_mp_tac infer_e_type_pe_determ >>
               MAP_EVERY qexists_tac [`menv`,`env`, `st'''`, `st''''`, `t1`] >>
               srw_tac[] [check_cenv_tenvC_ok]
               >- srw_tac[] [num_tvs_bvl2, num_tvs_def]
               >- metis_tac [tenv_alpha_def] >>
               full_simp_tac(srw_ss()) [] >>
               imp_res_tac generalise_none >>
               full_simp_tac(srw_ss()) [EVERY_MAP, LAMBDA_PROD] >>
      ...
      qexists_tac `num_tvs tenv'` >>
          ...
          CONJ_ASM1_TAC>-
           ...
           srw_tac[][weakE_def] >>
           Cases_on`ALOOKUP (tenv_add_tvs tvs' tenv'') x`>>full_simp_tac(srw_ss())[]>>
           Cases_on`x'`>>full_simp_tac(srw_ss())[]>>
           CASE_TAC>-
             (imp_res_tac ALOOKUP_FAILS>>
             imp_res_tac ALOOKUP_MEM>>
             imp_res_tac type_p_pat_bindings>>
             full_simp_tac(srw_ss())[tenv_add_tvs_def,MEM_MAP,PULL_EXISTS,EXISTS_PROD]>>
             pop_assum sym_sub_tac>>
             full_simp_tac(srw_ss())[Once LIST_EQ_REWRITE,MEM_EL,PULL_EXISTS,EL_MAP]>>
           ...
           discharge_hyps>-
             (full_simp_tac(srw_ss())[check_cenv_tenvC_ok,sub_completion_def,pure_add_constraints_def]>>
             CONJ_TAC>-
               (Cases_on`tvs'=0`>>full_simp_tac(srw_ss())[num_tvs_bvl2,num_tvs_def,bind_tvar_def])
             >>
             match_mp_tac tenv_invC_extend_tvar_empty_subst>>
             full_simp_tac(srw_ss())[tenv_invC_convert_env2,num_tvs_bvl2,num_tvs_def]>>
             metis_tac[tenv_alpha_def])
          >>
          srw_tac[][]>>
          imp_res_tac ALOOKUP_MEM>>
          qpat_assum `A=tenv_add_tvs B C` sym_sub_tac >>
          full_simp_tac(srw_ss())[convert_env2_def,ZIP_MAP,MAP_MAP_o,combinTheory.o_DEF]>>
          full_simp_tac(srw_ss())[MEM_MAP,PULL_EXISTS]>>
          full_simp_tac(srw_ss())[simp_tenv_invC_def,tenv_add_tvs_def,MEM_MAP,EXISTS_PROD]>>
          ...
          discharge_hyps>-
            ...
            >>
            Cases_on`tvs'=0`>>full_simp_tac(srw_ss())[bind_tvar_def,num_tvs_bvl2,num_tvs_def])
          >>
          srw_tac[][]>>
       ...
       qexists_tac`MAP convert_t subst'`>>full_simp_tac(srw_ss())[]>> 
      ...
      >>
       ...
       pop_assum(qspec_then `subst'`assume_tac)>>full_simp_tac(srw_ss())[]>>
       ...
       discharge_hyps>-
         ...
         CONJ_TAC>-
           (`check_s 0 (count init_infer_state.next_uvar) init_infer_state.subst ∧ t_wfs init_infer_state.subst` by
             full_simp_tac(srw_ss())[init_infer_state_def,check_s_def,t_wfs_def]>>
           full_simp_tac(srw_ss())[init_infer_state_def]>>
           imp_res_tac infer_e_check_s>>
           ntac 128 (pop_assum kall_tac)>>
           rev_full_simp_tac(srw_ss())[]>>
           pop_assum(qspec_then`0` assume_tac)>>rev_full_simp_tac(srw_ss())[]>>
           ...
           full_simp_tac(srw_ss())[SUBSET_DEF])
           >>
           first_x_assum(qspec_then`(q',t')` assume_tac)>>rev_full_simp_tac(srw_ss())[]>>
           imp_res_tac check_t_more5>>
           full_simp_tac(srw_ss())[SUBSET_DEF,EXTENSION])
       >>
       srw_tac[][]>>
 ...
 >- (full_simp_tac(srw_ss()) [success_eqns] >>
     ...
     `check_env (count (LENGTH l)) env''` 
                 by (Q.UNABBREV_TAC `env''` >>
                     srw_tac[] [MAP2_MAP, check_env_merge, check_env_letrec] >>
                     metis_tac [check_env_more, COUNT_ZERO, DECIDE ``0<=x:num``]) >> 
     `num_tvs tenv'' = tvs`
                 by  (Q.UNABBREV_TAC `tenv''` >>
                      srw_tac[] [num_tvs_bind_var_list] >>
                      Q.UNABBREV_TAC `tenv'` >>
                      full_simp_tac(srw_ss()) [bind_tvar_rewrites, num_tvs_bvl2, num_tvs_def]) >>
     ...
     >-
       srw_tac[] [empty_decls_def, convert_decls_def]
     >-
      (srw_tac[] [LENGTH_MAP, LENGTH_COUNT_LIST, MAP2_MAP, MAP_MAP_o, combinTheory.o_DEF] >>
          REPEAT (pop_assum (fn _ => all_tac)) >>
          induct_on `l` >>
          srw_tac[] [COUNT_LIST_def, tenv_add_tvs_def, convert_env_def, convert_env2_def] >-
          (PairCases_on `h` >> srw_tac[] []) >>
          srw_tac[] [MAP_MAP_o, MAP2_MAP, ZIP_MAP, LENGTH_COUNT_LIST, combinTheory.o_DEF, remove_pair_lem])
      >-
      (`LENGTH l = LENGTH funs_ts` by full_simp_tac(srw_ss()) [LENGTH_COUNT_LIST] >>
          full_simp_tac(srw_ss()) [MAP_ZIP, LENGTH_COUNT_LIST, MAP_MAP_o, combinTheory.o_DEF] >>
      ...
      ntac 4 (qpat_assum`∀ts s2. P ts s2`kall_tac) >>
          ...
          CASE_TAC>-(
            ...
            ntac 2 strip_tac >> full_simp_tac(srw_ss())[LENGTH_COUNT_LIST] >>
            disch_then kall_tac >>
            full_simp_tac(srw_ss())[tenv_add_tvs_def,MEM_MAP,PULL_EXISTS,EXISTS_PROD]>>
            full_simp_tac(srw_ss())[MAP2_MAP,MEM_MAP,PULL_EXISTS,EXISTS_PROD,LENGTH_COUNT_LIST] >>
            full_simp_tac(srw_ss())[MEM_ZIP,LENGTH_COUNT_LIST,ZIP_MAP,MAP_MAP_o,combinTheory.o_DEF,UNCURRY] >>
            full_simp_tac(srw_ss())[MEM_EL] >>
            metis_tac[FST,PAIR,PAIR_EQ])
           >>
           Cases_on`x'`>>full_simp_tac(srw_ss())[]>>
           first_assum (mp_tac o MATCH_MP(GEN_ALL infer_funs_complete|>REWRITE_RULE[GSYM AND_IMP_INTRO]))>>
           full_simp_tac(srw_ss())[check_cenv_tenvC_ok,check_env_def,tenv_alpha_def]>>
           ...
           VAR_EQ_TAC >>
           imp_res_tac ALOOKUP_MEM>>
           full_simp_tac(srw_ss())[tenv_add_tvs_def,MAP2_MAP,MAP_MAP_o,LENGTH_COUNT_LIST]>>
           full_simp_tac(srw_ss())[MEM_MAP,EXISTS_PROD]>>
           full_simp_tac(srw_ss())[MEM_ZIP,LENGTH_COUNT_LIST]>>
           full_simp_tac(srw_ss())[EL_MAP,LENGTH_COUNT_LIST,COUNT_LIST_GENLIST,EL_GENLIST]>>
         ...
         full_simp_tac(srw_ss())[sub_completion_def]>>
          Q.ISPECL_THEN [`tvs'`,`s'`] mp_tac (GEN_ALL generalise_subst_exist)>>
          discharge_hyps>-
            (full_simp_tac(srw_ss())[]>>metis_tac[pure_add_constraints_wfs])
          >>
         ...
         >-
           ...
           >-
             metis_tac[check_t_t_vars]
           >>
           full_simp_tac(srw_ss())[Abbr`ts`,MEM_MAP,MEM_GENLIST]>>
           match_mp_tac t_walkstar_check >>
           simp[] >>
           (*inferencer invariant*)
           simp[check_t_def] >>
           reverse conj_tac >- (
             imp_res_tac (last(CONJUNCTS infer_e_next_uvar_mono)) >> full_simp_tac(srw_ss())[] >>
             DECIDE_TAC ) >>
           match_mp_tac (check_s_more3 |> MP_CANON)>>
           qexists_tac `count st'.next_uvar`>>
           simp[] )
      ...
      >>
       ...
       discharge_hyps>-
         (
         full_simp_tac(srw_ss())[EXTENSION,SUBSET_DEF,Abbr`ts`]>>
         full_simp_tac(srw_ss())[MEM_MAP,PULL_EXISTS]>>
         imp_res_tac ALOOKUP_MEM>>
         full_simp_tac(srw_ss())[FORALL_PROD,EXISTS_PROD,MEM_GENLIST]>>
         CONJ_TAC>-
           metis_tac[]>>
         reverse CONJ_TAC>-
           metis_tac[]
         >>
         full_simp_tac(srw_ss())[EVERY_MAP,EVERY_GENLIST])
       >>
       srw_tac[][]>>
       metis_tac[pure_add_constraints_wfs,t_walkstar_SUBMAP,pure_add_constraints_success])
 >- (srw_tac[] [convert_decls_def, convert_env2_def, EVERY_MAP, DISJOINT_DEF, EXTENSION] >>
     full_simp_tac(srw_ss()) [EVERY_MAP, EVERY_MEM] >>
     Q.LIST_EXISTS_TAC [`set mdecls`, `set edecls`, `set tdecls`] >>
     srw_tac[] [MEM_MAP] >>
     metis_tac [])
 >- srw_tac[] [convert_decls_def, convert_env2_def, empty_decls_def]
 >- (srw_tac[] [convert_decls_def, convert_env2_def]>>metis_tac[MAP_ID]));
...
`!mn decls tenvT menv tenvM cenv env ds st1 decls' tenvT' cenv' env' st2 tenv.
 ...
 >- srw_tac[] [empty_decls_def,convert_decls_def, convert_env2_def, Once type_ds_cases] >>
 ...
 `tenvT_ok (merge_mod_env (FEMPTY,tenvT'') tenvT)` 
        by (match_mp_tac tenvT_ok_merge >>
            full_simp_tac(srw_ss()) [tenvT_ok_def, FEVERY_FEMPTY]) >>
 `check_env {} (env'' ++ env)` 
                 by full_simp_tac(srw_ss()) [check_env_def, init_infer_state_def] >>
 ...
 discharge_hyps >- (
   conj_tac >- (
     ...
     full_simp_tac(srw_ss())[check_env_def,convert_env2_def,EVERY_MAP] >>
   ...
   >> 
   full_simp_tac(srw_ss())[EXTENSION,MEM_MAP,PULL_EXISTS,EXISTS_PROD,convert_env2_def])>>
 strip_tac >>
 CONV_TAC(STRIP_QUANT_CONV(lift_conjunct_conv(same_const``type_d`` o fst o strip_comb))) >>
 first_assum(match_exists_tac o concl) >> simp[] >>
 full_simp_tac(srw_ss())[convert_append_decls,bvl2_append] >>
 first_assum(match_exists_tac o concl) >> simp[] >>
...
val check_weakE_sound = Q.prove (
  ...
  conj_tac >- (
    srw_tac[][anub_def,weakE_def,convert_env2_def] ) >>
  srw_tac[][] >>
  imp_res_tac(check_weakE_EVERY |> SPEC_ALL |> EQ_IMP_RULE |> fst |> SIMP_RULE(srw_ss())[PULL_EXISTS]) >>
  rev_full_simp_tac(srw_ss())[] >>
  Cases_on`ALOOKUP tenv1 n` >> full_simp_tac(srw_ss())[] >>
  `?tvs_impl t_impl. x = (tvs_impl,t_impl)` by metis_tac[PAIR] >> full_simp_tac(srw_ss())[] >>
  ...
  discharge_hyps_keep >- (
    full_simp_tac(srw_ss())[check_env_def,SUBSET_DEF,EVERY_MEM] ) >>
  srw_tac[] [convert_env2_def, weakE_def] >>
  srw_tac[][] >>
  ...
  full_simp_tac(srw_ss()) [] >>
     `t_wfs FEMPTY` by srw_tac[] [t_wfs_def] >>
     imp_res_tac t_unify_wfs >>
     ...
     `t_wfs (s |++ s')`
               by (`t_vR s = t_vR (s |++ s')`
                            by (srw_tac[] [t_vR_eqn, FUN_EQ_THM] >>
                                ...
                                full_simp_tac(srw_ss()) [flookup_update_list_none, flookup_update_list_some] >>
                                cases_on `FLOOKUP s x'` >>
                                full_simp_tac(srw_ss()) [flookup_update_list_none, flookup_update_list_some] >>
                                `FLOOKUP (FEMPTY |++ s') x' = SOME x''` by srw_tac[] [flookup_update_list_some] >>
                                pop_assum mp_tac >>
                                srw_tac[] [FLOOKUP_FUN_FMAP, t_vars_eqn] >>
                                srw_tac[] [FLOOKUP_FUN_FMAP, t_vars_eqn] >>
                                full_simp_tac(srw_ss()) [FLOOKUP_DEF]) >>
                   full_simp_tac(srw_ss()) [t_wfs_eqn]) >>
     `check_s tvs_spec (count tvs_impl) s`
                by (match_mp_tac t_unify_check_s >>
                    ...
                    `ALOOKUP tenv2 n = ALOOKUP (anub tenv2 acc) n` by (
                      CONV_TAC(RAND_CONV(REWR_CONV ALOOKUP_anub)) >> srw_tac[][] >>
                      metis_tac[anub_notin_acc,MEM,MAP,FST] ) >>
                    srw_tac[][]) >>
     qexists_tac `MAP (\n. convert_t (t_walkstar (s |++ s') (Infer_Tuvar n))) (COUNT_LIST tvs_impl)` >>
     srw_tac[] [LENGTH_COUNT_LIST, check_t_to_check_freevars, EVERY_MAP]
     >- (srw_tac[] [EVERY_MEM] >>
         ...
         `check_t tvs_spec {} (t_walkstar (s |++ s') (Infer_Tuvar n'))`
                     by (srw_tac[] [check_t_def] >>
                         match_mp_tac t_walkstar_check >>
                         srw_tac[] [check_t_def, FDOM_FUPDATE_LIST] >|
                         [full_simp_tac(srw_ss()) [check_s_def, fdom_fupdate_list2] >>
                              srw_tac[] [] >>
                              srw_tac[] [FUPDATE_LIST_APPLY_NOT_MEM] >>
                              `count tvs_impl ⊆ FDOM s ∪ set (MAP FST s')` by srw_tac[] [SUBSET_DEF] >|
                              [metis_tac [check_t_more5],
                               metis_tac [check_t_more5],
                               `FLOOKUP (s |++ s') uv = SOME ((s |++ s') ' uv)`
                                           by (srw_tac[] [FLOOKUP_DEF, FDOM_FUPDATE_LIST]) >>
                                   full_simp_tac(srw_ss()) [flookup_update_list_some] >|
                                   [imp_res_tac ALOOKUP_MEM >>
                                        full_simp_tac(srw_ss())[] >>
                                    ...
                                    pop_assum mp_tac >>
                                        srw_tac[] [FLOOKUP_DEF]]],
                          full_simp_tac(srw_ss()) [EXTENSION, MEM_COUNT_LIST] >>
                              res_tac >>
                              full_simp_tac(srw_ss()) [FDOM_FUPDATE_LIST]]) >>
         srw_tac[] [check_t_to_check_freevars])
     >- (
       ...
       full_simp_tac(srw_ss()) [] >>
       srw_tac[] [SIMP_RULE (srw_ss()) [MAP_MAP_o, combinTheory.o_DEF] (GSYM db_subst_infer_subst_swap2)] >>
       AP_TERM_TAC >>
...
imp_res_tac anub_tl_anub >> srw_tac[][] >>
full_simp_tac(srw_ss()) [check_flat_weakC_def, flat_weakC_def, success_eqns] >>
srw_tac[] [] >>
PairCases_on `h` >>
full_simp_tac(srw_ss()) [] >>
srw_tac[] [] >>
...
`!mn tenvT1 tenvT2.
 ...
 srw_tac[] [check_flat_weakT_def, flat_weakT_def, success_eqns] >>
...
Induct >>
srw_tac[] [check_freevars_def] >> metis_tac[]);
...
`!mn orig_tenvT mdecls tdecls edecls tenvT cenv env specs st decls' tenvT' cenv' env' st' init_decls.
 ...
 >- (srw_tac[] [Once type_specs_cases] >>
     qexists_tac `([],[],[])` >>
     full_simp_tac(srw_ss()) [empty_decls_def, convert_decls_def, append_decls_def, convert_env2_def])
 >- (srw_tac[] [Once type_specs_cases] >>
     res_tac >>
     ...
     qexists_tac `nub fvs` >>
     srw_tac[] [] >>
     full_simp_tac(srw_ss()) [LENGTH_MAP, convert_t_subst, convert_env2_def,
         LENGTH_COUNT_LIST,LENGTH_GENLIST] >>
     full_simp_tac(srw_ss()) [MAP_MAP_o, combinTheory.o_DEF, convert_t_def] >>
 ...
 >- (srw_tac[] [Once type_specs_cases] >>
     ...
     srw_tac[] [] >>
     PairCases_on `decls''` >>
     full_simp_tac(srw_ss()) [append_decls_def, convert_decls_def] >>
     full_simp_tac(srw_ss()) [PULL_EXISTS] >>
     qexists_tac `tenvT''''` >>
     srw_tac[] [] >>
     qexists_tac `(set decls''0,set decls''1,set decls''2)` >>
     srw_tac[] [union_decls_def, DISJOINT_DEF, EXTENSION, MEM_MAP] >>
     full_simp_tac(srw_ss()) [EVERY_MEM, EVERY_MAP, FUNION_ASSOC])
 >- (srw_tac[] [Once type_specs_cases, PULL_EXISTS] >>
     res_tac >>
     srw_tac[] [FUNION_FUPDATE_2] >>
     qexists_tac `decls''` >>
     qexists_tac `tenvT'''` >>
 ...
 >- (srw_tac[] [Once type_specs_cases] >>
     ...
     qexists_tac `tenvT'''` >>
     PairCases_on `decls''` >>
     srw_tac[] [convert_decls_def, append_decls_def] >>
     qexists_tac `convert_decls (decls''0,decls''1,decls''2)` >>
     full_simp_tac(srw_ss()) [convert_decls_def, union_decls_def, DISJOINT_DEF, EXTENSION, MEM_MAP] >>
     metis_tac [])
 >- (srw_tac[] [Once type_specs_cases, convert_decls_def] >>
     res_tac >>
     srw_tac[] [PULL_EXISTS] >>
     qexists_tac `append_decls decls'' ([],[mk_id mn tn],[])` >>
     PairCases_on `decls''` >>
     srw_tac[] [append_decls_def] >>
     qexists_tac `tenvT'''` >>
     qexists_tac `convert_decls (decls''0,decls''1,decls''2)` >>
     full_simp_tac(srw_ss()) [convert_decls_def, union_decls_def, DISJOINT_DEF, EXTENSION, MEM_MAP] >>
...
`!decls tenvT menv tenvM cenv env tenv top st1 decls' tenvT' menv' cenv' env' st2.
 ...
 rpt gen_tac >>
 `?mdecls tdecls edecls. decls = (mdecls,tdecls,edecls)` by metis_tac [pair_CASES] >>
 full_simp_tac(srw_ss()) [infer_top_def, success_eqns, type_top_cases, infer_sound_invariant_def] >>
 strip_tac >>
 `∃decls'' tenvT'' cenv'' env''. v' = (decls'',tenvT'',cenv'',env'')` by metis_tac [pair_CASES] >>
 full_simp_tac(srw_ss()) [success_eqns]
 >- (`∃mdecls''' tdecls''' edecls''' tenvT''' cenv''' env'''. v'' = ((mdecls''',tdecls''',edecls'''),tenvT''',cenv''',env''')` by metis_tac [pair_CASES] >>
     `flat_tenvT_ok (FEMPTY:flat_tenvT) ∧ check_flat_cenv [] ∧ check_env {} ([]:(tvarN, num # infer_t) alist)` 
                by srw_tac[] [flat_tenvT_ok_def, check_flat_cenv_def, check_env_def,
     ...
     >- (full_simp_tac(srw_ss()) [success_eqns] >>
         ...
         disch_then(qspecl_then[`tenvM`,`tenv`]mp_tac) >> simp[] >> strip_tac >>
         cases_on `o'` >>
         full_simp_tac(srw_ss()) [success_eqns, check_signature_def, check_signature_cases]
         >- (full_simp_tac(srw_ss()) [convert_menv_def] >>
             srw_tac[] [] >>
             full_simp_tac(srw_ss()) [convert_env2_def, convert_decls_def] >>
         ...
         >- (PairCases_on `v'` >>
             ...
             PairCases_on`decls'''`>>full_simp_tac(srw_ss())[convert_decls_def,append_decls_def] >>
             ...
             >- (PairCases_on `decls''` >>
                 full_simp_tac(srw_ss()) [convert_decls_def, weak_decls_def, check_weak_decls_def, append_decls_def,
                     list_subset_def, SUBSET_DEF, EVERY_MEM] >>
                 srw_tac[] [] >>
                 metis_tac [])
             >- metis_tac [check_flat_weakT_sound]))
     >- (match_mp_tac tenvT_ok_merge >>
         full_simp_tac(srw_ss()) [success_eqns, check_signature_def] >>
         srw_tac[] [tenvT_ok_def, FEVERY_FUPDATE, FEVERY_FEMPTY] >>
         cases_on `o'` >>
         ...
         first_x_assum(mp_tac o MATCH_MP(
           ...
           |> ONCE_REWRITE_RULE[GSYM AND_IMP_INTRO])) >>
         srw_tac[][])
     >- (full_simp_tac(srw_ss()) [success_eqns, check_menv_def] >>
         srw_tac[] [FEVERY_FUPDATE, FEVERY_FEMPTY] >>
         cases_on `o'` >>
         full_simp_tac(srw_ss()) [check_signature_def, success_eqns] >>
         srw_tac[] []
         ...
         >- (PairCases_on `v'` >>
             ...
             first_x_assum(mp_tac o MATCH_MP(
               ...
               |> CONV_RULE(STRIP_QUANT_CONV(LAND_CONV(lift_conjunct_conv is_eq)))
               |> ONCE_REWRITE_RULE[GSYM AND_IMP_INTRO])) >>
             full_simp_tac(srw_ss()) [check_env_def, check_flat_cenv_def]))
     >- (full_simp_tac(srw_ss()) [success_eqns, check_menv_def] >>
         srw_tac[] [] >>
         cases_on `o'` >>
         full_simp_tac(srw_ss()) [check_signature_def, success_eqns] >>
         srw_tac[] []
         >- (full_simp_tac(srw_ss()) [check_cenv_def] >>
             PairCases_on `cenv` >>
             full_simp_tac(srw_ss()) [merge_alist_mod_env_def, check_cenv_def])
         >- (PairCases_on `v'` >>
             full_simp_tac(srw_ss()) [success_eqns] >>
             srw_tac[] [] >>
             first_x_assum(mp_tac o MATCH_MP(
     ...
     >-
       ...
       `check_menv menv'` by 
         ...
         full_simp_tac(srw_ss())[check_menv_def,FEVERY_DEF]>>
         Cases_on`o'`>>full_simp_tac(srw_ss())[check_signature_def,success_eqns]
         >-
           metis_tac[check_env_def]
         >>
         PairCases_on`v'`>>full_simp_tac(srw_ss())[success_eqns]>>
         `check_env {} env'''` by 
            (`flat_tenvT_ok FEMPTY` by full_simp_tac(srw_ss())[flat_tenvT_ok_def,FEVERY_FEMPTY]>>
       ...
       match_mp_tac fmap_rel_FUNION_rels>>full_simp_tac(srw_ss())[])
     >>
     full_simp_tac(srw_ss())[UNCURRY,success_eqns] >>
     srw_tac[][convert_env2_def]) >>
 srw_tac[] []
 >- srw_tac[] [convert_menv_def]
 >- metis_tac [infer_d_sound]
 >- (match_mp_tac tenvT_ok_merge >>
     srw_tac[] [tenvT_ok_def] >>
     imp_res_tac infer_d_check >>
 ...
 >- (
   imp_res_tac infer_d_check >>
   ntac 6 (pop_assum kall_tac) >>
   srw_tac[][bind_var_list2_append] >>
   match_mp_tac tenv_ok_bind_var_list2 >>
   srw_tac[][num_tvs_bvl2,num_tvs_def,convert_env2_def,EVERY_MAP] >>
   full_simp_tac(srw_ss())[EVERY_MEM,UNCURRY,check_env_def] >>
   metis_tac[check_t_to_check_freevars])
 >-
   (match_mp_tac tenv_alpha_bind_var_list2>>
   srw_tac[][]>-
...
`!decls tenvT menv tenvM cenv env tenv prog st1 decls' tenvT' menv' cenv' env' st2.
 ...
 induct_on `prog` >>
 srw_tac[] [infer_prog_def, success_eqns]
 >- srw_tac[] [Once type_prog_cases, empty_decls_def, convert_decls_def, convert_menv_def, convert_env2_def]
 >- (PairCases_on `cenv` >>
     PairCases_on `tenvT` >>
     srw_tac[] [merge_mod_env_def, merge_alist_mod_env_def,convert_env2_def,convert_menv_def])
 >- (srw_tac[] [Once type_prog_cases] >>
     ...
     first_assum(match_exists_tac o concl) >> simp[] >>
     full_simp_tac(srw_ss())[convert_append_decls,bind_var_list2_append] >>
     first_assum(match_exists_tac o concl) >> simp[] >>
 ...
 >- (`?decls' tenvT' menv' cenv' env'. v' = (decls',tenvT',menv',cenv',env')` by metis_tac [pair_CASES] >>
     ...
     srw_tac[] [] >>
     res_tac >>
     full_simp_tac(srw_ss())[convert_env2_def,convert_menv_def,o_f_FUNION] >>
