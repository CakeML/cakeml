...
Theory heap_list_sort_monadic
Ancestors
  heap_list_sort ml_monadBase
Libs
...
(* Top-level sort function for now. This is then given a functional interface
   by a "run" wrapper once more monadic translation apparatus is in place. *)
...
Theorem LENGTH_add_to_heaps_step1_facts[local]:
  0 < LENGTH (add_to_heaps_step1 ts x) /\
  LENGTH (bs_tree_list_to_list (add_to_heaps_step1 ts x)) =
    LENGTH (bs_tree_list_to_list ts) + 1 /\
  LENGTH (add_to_heaps_step1 ts x) <= LENGTH ts + 1 /\
  (MAP SND (add_to_heaps_step1 ts x) = MAP SND (add_to_heaps_step1 ts y)) = T /\
  (LENGTH (add_to_heaps_step1 ts x) = LENGTH (add_to_heaps_step1 ts y)) = T
Proof
  simp [add_to_heaps_step1_def]
  \\ rpt (TOP_CASE_TAC \\ fs [tree_to_list_unfold])
QED

Theorem inv_add_tree_step1[local]:
  (EVERY (\(t,n). 0 < n /\ tree_balanced_height n t) ts ==>
    EVERY (\(t,n). 0 < n /\ tree_balanced_height n t) (add_to_heaps_step1 ts x)
  ) /\
  (EVERY (\(t,n). 0 < n /\ tree_balanced_height n t) ts /\
    SORTED ($<=) (TAKE 2 (MAP SND ts)) /\ SORTED ($<) (MAP SND (DROP 1 ts)) ==>
    SORTED ($<=) (TAKE 2 (MAP SND (add_to_heaps_step1 ts x))) /\
    SORTED ($<) (MAP SND (DROP 1 (add_to_heaps_step1 ts x)))
  )
Proof
  simp [add_to_heaps_step1_def]
...
Theorem insert_trees_adj_step1[local]:
  EVERY (\(t,n). 0 < n /\ tree_balanced_height n t) ts ==>
  insert_trees_inv R (add_to_heaps_step1 ts x_dc) x =
      insert_trees_inv R (add_to_heaps_step1 ts y_dc) x
Proof
  simp [add_to_heaps_step1_def]
  \\ rpt (TOP_CASE_TAC \\ fs [tree_balanced_height_def])
  \\ rw []
...
Theorem LENGTH_to_list_add_to_heaps[local]:
  LENGTH (bs_tree_list_to_list (add_to_heaps R ts x)) =
    LENGTH (bs_tree_list_to_list ts) + 1
Proof
  simp [add_to_heaps_def, LENGTH_list_of_insert_trees,
        LENGTH_add_to_heaps_step1_facts]
QED
...
Theorem inv_add_to_heaps[local]:
  (EVERY (\(t,n). 0 < n /\ tree_balanced_height n t) ts ==>
    EVERY (\(t,n). 0 < n /\ tree_balanced_height n t) (add_to_heaps R ts x)
  ) /\
  (EVERY (\(t,n). 0 < n /\ tree_balanced_height n t) ts /\
    SORTED ($<=) (TAKE 2 (MAP SND ts)) /\ SORTED ($<) (MAP SND (DROP 1 ts)) ==>
    SORTED ($<=) (TAKE 2 (MAP SND (add_to_heaps R ts x))) /\
    SORTED ($<) (MAP SND (DROP 1 (add_to_heaps R ts x)))
  )
Proof
  simp [add_to_heaps_def, MAP_SND_insert_trees_inv, MAP_DROP]
...
Theorem LENGTH_add_heap_to_heaps_facts[local]:
  tree_balanced_height ht t /\ 0 < ht ==>
  LENGTH (add_heap_to_heaps R ts t ht) = LENGTH ts + 1
  /\
  MAP SND (add_heap_to_heaps R ts t ht) = ht :: MAP SND ts
  /\
  LENGTH (bs_tree_list_to_list (add_heap_to_heaps R ts t ht)) =
  LENGTH (bs_tree_list_to_list ts) + LENGTH (bs_tree_to_list ht t) /\
  (EVERY (\(t, n). 0 < n /\ tree_balanced_height n t) ts ==>
    EVERY (\(t, n). 0 < n /\ tree_balanced_height n t) (add_heap_to_heaps R ts t ht)
  )
Proof
  rw [add_heap_to_heaps_def]
...
Theorem add_values_to_heaps_facts[local]:
  !xs ts.
  EVERY (\(t, n). 0 < n /\ tree_balanced_height n t) ts ==>
  LENGTH (bs_tree_list_to_list (add_values_to_heaps R ts xs)) =
    LENGTH (bs_tree_list_to_list ts) + LENGTH xs /\
  EVERY (\(t, n). 0 < n /\ tree_balanced_height n t) (add_values_to_heaps R ts xs) /\
  (SORTED $< (MAP SND (DROP 1 ts)) /\ SORTED $<= (TAKE 2 (MAP SND ts)) ==>
    SORTED $< (MAP SND (DROP 1 (add_values_to_heaps R ts xs))) /\
    SORTED $<= (TAKE 2 (MAP SND (add_values_to_heaps R ts xs))))
Proof
  Induct \\ simp [tree_to_list_unfold, add_values_to_heaps_def]
  \\ rw []
  \\ simp [inv_add_to_heaps, LENGTH_to_list_add_to_heaps]
...
Theorem add_to_sfx_heaps_step1_eq:
  ...
  0 < LENGTH oth_szs ==>
  ? oth_szs2.
  let ts2 = add_to_heaps_step1 ts x in
...
Proof
  rw []
  \\ simp [add_to_sfx_heaps_step1_def, add_to_heaps_step1_def]
...
Theorem add_to_sfx_heaps_eq:
  ...
  let ts2 = add_to_heaps R ts x in
  ? oth_szs2.
...
Proof
  ...
  \\ simp []
  \\ metis_tac [insert_trees_adj_step1, LENGTH_add_to_heaps_step1_facts]
QED
...
Theorem add_all_to_sfx_heaps_eq:
  ...
  let ts2 = add_values_to_heaps R ts xs in
  ? oth_szs2.
...
Proof
  Induct
  \\ simp [add_all_to_sfx_heaps_def, add_values_to_heaps_def]
  >- (
    simp [ml_monadBaseTheory.monad_eqs]
    \\ metis_tac []
  )
  ...
  \\ fs [markerTheory.Abbrev_def]
  \\ simp [LENGTH_to_list_add_to_heaps, inv_add_to_heaps]
  \\ rw [] \\ simp []
  \\ Cases_on `oths` \\ fs []
  \\ irule_at Any EQ_REFL
  \\ simp []
...
Theorem reinsert_tree_eq:
  ...
  reinsert_tree R i j ht (mk_st ((t, ht) :: ts, oths) (dc :: MAP SND ts, oth_szs)) =
    (let ts2 = add_heap_to_heaps R ts t ht in
        (M_success (), mk_st (ts2, oths) (MAP SND ts2, oth_szs)))
Proof
  simp [reinsert_tree_def, add_heap_to_heaps_def]
  \\ rw []
  \\ gs [tree_balanced_height_pos]
...
Theorem sfx_trees_to_list_eq:
  ...
  ?st'. sfx_trees_to_list R i j acc (mk_st (ts, oths) (MAP SND ts, oth_szs)) =
    (M_success (heaps_to_list R ts acc), st')
Proof
  Induct
  \\ REWRITE_TAC []
  ...
  >- (
    rw []
    \\ Cases_on `ts` \\ fs []
    \\ simp [ml_monadBaseTheory.monad_eqs, heaps_to_list_def]
...
*)
  ...
  >- (
    ...
    \\ simp [mk_st_move_others, bs_tree_to_list_tree_rec, heaps_to_list_def,
            add_heap_to_heaps_def]
    \\ first_x_assum irule
  ...
  >- (
    ...
    \\ simp [GSYM mk_st_move_others]
    \\ dep_rewrite.DEP_REWRITE_TAC [reinsert_tree_eq]
    \\ simp [LENGTH_add_heap_to_heaps_facts, MAP_DROP, bs_tree_list_to_list_rec]
    \\ conj_tac
    >- (
      Cases_on `t` \\ fs []
    )
    ...
    \\ fs [markerTheory.Abbrev_def, LENGTH_add_heap_to_heaps_facts, ADD1, MAP_DROP]
    \\ impl_tac
    >- (
      Cases_on `t` \\ fs []
    )
...
Theorem sort_via_sfx_trees_worker_eq:
  FST (sort_via_sfx_trees_worker R x xs st) = M_success (heap_list_sort R xs)
Proof
  simp [sort_via_sfx_trees_worker_def]
  \\ simp [FST_EQ_EQUIV, st_ex_ignore_bind_simp, return_bind_eq]
  ...
  \\ simp [add_values_to_heaps_facts]
