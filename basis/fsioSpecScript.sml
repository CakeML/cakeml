...
val input_aux_spec = Q.store_thm("input_aux_spec",
  `!s (fd :word8) fdv sv fs content pos count off. 
    len + off <= LENGTH buf ⇒ NUM count countv ⇒ 
    pos <= LENGTH content  ⇒ 
    WORD fd fdv ⇒ validFD (w2n fd) fs ⇒ NUM off offv ⇒ NUM len lenv ⇒ 
    w2n fd < 255 ⇒ (get_file_content fs (w2n fd) = SOME(content, pos)) ⇒
    app (p:'ffi ffi_proj) ^(fetch_v "IO.input_aux" (basis_st())) [fdv; bufv; offv; lenv; countv]
    (IOFS fs * W8ARRAY bufv buf)  
    (POSTv nv. &(NUM (count + MIN len (LENGTH content - pos)) nv) * 
       W8ARRAY bufv (insert_atI (TAKE (MIN len (LENGTH content -pos)) 
                                      (DROP pos (MAP (n2w o ORD) content)))
                                 off buf) *
       SEP_EXISTS k. IOFS (fsupdate fs (w2n fd) k (MIN (pos + len) (LENGTH content)) content))`,
  `?n. len <= n` by (qexists_tac`len` >> fs[]) >>
  FIRST_X_ASSUM MP_TAC >> qid_spec_tac`len` >>
  induct_on`n` >> rw[] >>
  (* TODO: too long! *)
  xcf "IO.input_aux" (basis_st()) >> fs[IOFS_def,IOFS_iobuff_def] >> xpull
  >-(rpt(xlet_auto >> xsimpl) >> xif >> instantiate >> xvar >> xsimpl >>
     fs[MIN_DEF] >> qexists_tac`0` >> fs[fsupdate_unchanged,insert_atI_NIL] >>
     xsimpl) >>

  rpt(xlet_auto >> xsimpl) >> xif
  >-(xvar >> xsimpl >> qexists_tac`0` >> 
     fs[MIN_DEF,fsupdate_unchanged,insert_atI_NIL] >> xsimpl) >>
...
val input_spec = Q.store_thm("input_spec",
  `!s (fd :word8) fdv sv fs content pos. 
    ...
    WORD fd fdv ⇒ validFD (w2n fd) fs ⇒ NUM off offv ⇒ NUM len lenv ⇒ 
    w2n fd < 255 ⇒ (get_file_content fs (w2n fd) = SOME(content, pos)) ⇒
    app (p:'ffi ffi_proj) ^(fetch_v "input" (basis_st())) [fdv; bufv; offv; lenv]
    (IOFS fs * W8ARRAY bufv buf)  
    (POSTv nv. &(NUM (MIN len (LENGTH content - pos)) nv) * 
       W8ARRAY bufv (insert_atI (TAKE (min len (LENGTH content -pos)) 
                                      (DROP pos (MAP (n2w o ORD) content)))
                                 off buf) *
       SEP_EXISTS k. IOFS (fsupdate fs (w2n fd) k (LENGTH content) content))`,
  xcf "input" (basis_st()) >> fs[IOFS_def,IOFS_iobuff_def] >>
  xpull >>
  fs[MAP,SND]
  fs[letrec_pull_params_def,Fun_body_def]
  CASE_TAC
  EVAL_TAC


  xfun_spec`input_aux` `!count buf (fd :word8) fdv sv fs content pos. 
    len + os <= LENGTH buf ⇒
    NUM off offv ⇒ NUM len lenv ⇒ NUM count countv ⇒ 
    app (p:'ffi ffi_proj) input_aux [offv; lenv; countv]
    (IOFS fs * W8ARRAY bufv buf)  
    (POSTv nv. &(NUM (count + min len (LENGTH content - pos)) nv) * 
       SEP_EXISTS k. IOFS (fsupdate fs (w2n fd) k (LENGTH content) content))`
  ...
  strip_tac >>
  `?n. strlen  <= n` by (qexists_tac`strlen s` >> fs[]) >>
  FIRST_X_ASSUM MP_TAC >> qid_spec_tac`s` >>
  Induct_on`n=len` >>
  xpull >> rename [`W8ARRAY _ buff`] >>
