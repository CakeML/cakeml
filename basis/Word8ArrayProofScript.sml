...
fun prove_array_spec op_name =
  ...
  fs [app_aw8alloc_def, app_aw8sub_def, app_aw8length_def, app_aw8update_def,
      app_aw8subbit_def, app_aw8updatebit_def, app_aalloc_def, app_asub_def, app_alength_def, app_aupdate_def,
      app_copyaw8aw8_def, app_copystraw8_def, app_copyaw8str_def] \\
  xsimpl \\ fs [INT_def, NUM_def, WORD_def, BOOL_def, w2w_def, UNIT_TYPE_def] \\
...
Theorem w8array_subBit_spec:
   !a av n nv.
     ...
     app (p:'ffi ffi_proj) ^(fetch_v "Word8Array.subBit" (basis_st())) [av; nv]
       (W8ARRAY av a)
       (POSTv v. cond (BOOL ((EL (n DIV 8) a) ' (n MOD 8)) v) * W8ARRAY av a)
Proof
  prove_array_spec "Word8Array.subBit"
QED

Theorem w8array_updateBit_spec:
   !a av n nv b bv.
     NUM n nv /\ n < 8 * LENGTH a /\ BOOL b bv ==>
     app (p:'ffi ffi_proj) ^(fetch_v "Word8Array.updateBit" (basis_st()))
       [av; nv; bv]
       (W8ARRAY av a)
       (POSTv v. cond (UNIT_TYPE () v) *
                 W8ARRAY av (LUPDATE (((n MOD 8) :+ b) (EL (n DIV 8) a))
                                     (n DIV 8) a))
Proof
  prove_array_spec "Word8Array.updateBit"
QED
