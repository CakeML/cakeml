...
void ffiputChar (char* c, char* a) {
  putchar(a[0]);
}

/* stderr */
void ffiputChar_err(char* c, char* a, long len) {
  putc(a[0], stderr);
}

/* stdin */

void ffigetChar (char* conf, char* a) {
  int c = getchar();
  if(c == EOF) {
    a[1] = 1;
  } else {
...
void ffifgetc (char* conf, char *a) {
...
void fficlose (char* conf, char *a) {
...
void ffiisEof (char* conf, char *a) {
  int c; /* not char, otherwise EOF is mapped to a valid char */
  if (infds[a[0]])
