...
let br = Break (0, 0)
let sp = Break (1, 0)
let unindent = Break (1, ~-1)

let paren s = Box (Hovs, 1, [Lit "("; br; s; br; Lit ")"])
...
let rec print_longident_pure = function
  ...
  | Longident.Ldot (left, right) ->
    ...
    Box (Hovp, 1, [left; br; Lit "."; Lit right])
  | Longident.Lapply (p0, p1) ->
    Box (Hovs, 1, [print_longident_pure p0; sp; print_longident_pure p1])
...
let rec path_prefix = function
  | Pident _ -> Lit ""
  | Pdot (p, _, _) -> Box (Hovp, 1, [print_path_pure p; Lit "."])
  | Papply (p, _) -> Box (Hovp, 1, [print_path_pure p; Lit " "])

(* Constructors don't have a fully specified path. Infer it from the type. *)
let make_constructor_path { cstr_name; cstr_res = { desc; }; } =
  match desc with
  | Tconstr (p, _, _) ->
    return @@ Box (Hovp, 1, [path_prefix p; Lit cstr_name])
...
let print_tupled xs = Box (Hovp, 0, intersperseMany [Lit ","; sp] xs)
...
let print_construct prec (f : int -> 'a -> (format_decl, string) result)
  ...
  | _ ->
    ...
    return @@ ifThen (prec > 1 && not ([] = xs)) paren @@
      Box (Hovp, 1, [fix_formatted_identifier name; args])
...
let rec print_expression incase prec expr =
  ...
  | Texp_let (r, bs, e) ->
    ...
    return @@ ifThen (prec > 1) paren @@ Box (Hovs, 0, [
      Lit "let"; Break (1, 1);
      Box (V, 0, intersperse sp bs'); unindent;
      Lit "in"; Break (1, 1);
      e'; unindent;
      Lit "end"
    ])
  | Texp_function (l, [c], p) ->
    print_case 0 "=>" c >>= fun c' ->
    return @@ ifThen (prec > 1) paren @@ Box (Hovp, 1, [Lit "fn"; sp] @
      if case_is_trivial c then
        (* fn x => E *)
        [c']
      else
        (* fn tmp__ => case tmp__ of P => E *)
        [Lit "tmp__"; sp; Lit "=>"; sp; Box (Hovs, 1, [
  ...
  | Texp_function (l, cs, p) ->
    ...
    return @@ ifThen (prec > 1 || incase) paren @@ Box (Hovp, 1, [
      Lit "fn"; sp; Box (Hovs, 1, [
        Lit "tmp__"; sp; Lit "=>"; sp;
        Box (Hovp, 2, [Lit "case"; sp; Lit "tmp__"; sp; Lit "of"; sp; cs'])
      ])
    ])
  (* Special cases *)
  | Texp_apply ({ exp_desc = Texp_ident (Pdot (Pident m, n, _), _, _) },
    ...
    print_expression incase 2 e1 >>= fun e1' ->
    return @@ Box (Hovp, 1, [e0'; sp; Lit op; sp; e1'])
  (* Normal function application *)
  | Texp_apply (e0, es) ->
    print_expression false 2 e0 >>= fun e0' ->
    mapM (function
      | l, Some e, Required -> print_expression incase 2 e
      | _ -> Bad "Optional and named arguments not supported."
    ) es >>= fun es' ->
    return @@ ifThen (prec > 1) paren @@
      Box (Hovp, 1, intersperse sp (e0' :: es'))
  | Texp_match (exp, cs, [], p) ->
    print_expression false 0 exp >>= fun exp' ->
    print_case_cases cs >>= fun cs' ->
    return @@ ifThen (prec > 0 || incase) paren @@ Box (Hovs, 1, [
      Box (Hovp, 2, [Lit "case"; sp; exp'; sp; Lit "of"]); cs'
    ])
  | Texp_try (exp, cs) ->
    print_expression false 1 exp >>= fun exp' ->
    print_case_cases cs >>= fun cs' ->
    return @@ ifThen (prec > 0) paren @@ Box (Hovs, 1, [
      exp'; sp; Lit "handle"; sp; cs'
    ])
  | Texp_tuple es -> mapM (print_expression false 0) es <&> print_tupled
  | Texp_construct (_, desc, es) ->
    print_construct prec (print_expression incase) desc es
  | Texp_ifthenelse (p, et, Some ef) ->
    print_expression false 0 p >>= fun p' ->
    print_expression false 0 et >>= fun et' ->
    print_expression incase prec ef >>= fun ef' ->
    return @@ ifThen (prec > 0) paren @@ Box (Hovs, 1, [
      Box (Hovp, 2, [Lit "if"; sp; p'; sp; Lit "then"]);
      et'; unindent; Lit "else"; sp; ef'
    ])
  (* E0; E1 *)
  | Texp_sequence (e0, e1) ->
    print_expression false 0 e0 >>= fun e0' ->
    print_expression incase 0 e1 >>= fun e1' ->
    return @@ paren @@ Box (Hovs, 0, [e0'; Lit ";"; sp; e1'])
  | _ -> Bad "Some expression syntax not implemented."

and print_case prec conn c =
  case_parts prec c >>= fun (pat, exp) ->
  match c.c_guard with
  | None -> return @@ Box (Hovp, 1, [pat; sp; Lit conn; sp; exp])
  | _ -> Bad "Pattern guards not supported."

and print_case_cases cs =
  (mapM (print_case 0 "=>") cs <&> function
    | [] -> []
    | c' :: cs' ->
      Box (H, 0, [Lit "  "; c'; sp]) ::
        BatList.map (fun c -> Box (H, 0, [Lit "| "; c; sp])) cs')
  >>= fun cs' ->
  return @@ Box (Hovs, 0, cs')

and case_parts prec c =
  print_pattern prec c.c_lhs >>= fun pat ->
  print_expression true 1 c.c_rhs >>= fun exp ->
  return (pat, exp)
...
and print_value_binding first_binding rec_flag vb =
  ...
  | Texp_function (l, [c], p), Recursive when case_is_trivial c ->
    ...
    let rec reduce_fn acc lastc =
      ...
      | _ -> mapM (print_pattern 0) (BatList.rev acc) >>= fun vs ->
             print_case 0 "=" lastc >>= fun c' ->
             return @@ Box (Hovp, 1,
               [Lit keyword; sp; name; sp] @ intersperse sp vs @ [sp; c']
             )
    in
    reduce_fn [] c
  ...
  | Texp_function (l, cs, p), Recursive ->
    print_case_cases cs >>= fun cs' ->
    return @@ Box (Hovp, 1, [
      Lit keyword; sp; name; sp; Lit "tmp__"; sp; Lit "="; sp; Box (Hovp, 1, [
        Lit "case"; sp; Lit "tmp__"; sp; Lit "of"; sp; cs'
      ])
    ])
  ...
  | _, Nonrecursive ->
    ...
    return @@ Box (Hovp, 1, [Lit keyword; sp; name; sp; Lit "="; sp; expr])
...
and print_ttyp_tuple ts =
  ...
  return @@ Box (Hovp, 1, intersperseMany [sp; Lit "*"; sp] ts')
...
let print_constructor_declaration decl =
  ...
  return @@ Box (Hovp, 1, [Lit decl.cd_name.txt] @
...
let rec print_module_expr expr =
  ...
  | Tmod_structure str ->
    ...
    mapM print_structure_item str_items >>= fun items ->
    return @@ Box (Hovs, 1, [Lit "struct"; sp] @ items @ [unindent; Lit "end"])
...
and print_structure_item str =
  ...
  | Tstr_eval (e, attrs) ->
    print_expression false 0 e >>= fun e' ->
    return @@ Box (Hovp, 1, [Lit "val"; sp; Lit "eval__"; sp; Lit "="; sp; e'])
  | Tstr_value (r, bs) ->
    mapiM (fun i -> print_value_binding (i = 0) r) bs >>= fun bs' ->
  ...
  | Tstr_exception constructor ->
    ...
    | Text_decl (ts, None) ->
      ...
      return @@ Box (Hovp, 1, [Lit "of"; sp; ts'])
    | Text_rebind (path, _) -> print_path path >>= fun p ->
                               return @@ Box (Hovp, 1, [Lit "="; sp; p])
...
let print_str_items ss =
  mapM print_structure_item ss >>= fun ss' ->
  return @@ Box (V, 0, ss')

let concat_items =
  BatList.filter (fun x -> Lit "" <> x)
  %> BatList.map (fun x -> Box (H, 0, [x; Lit ";"]))
  %> fun xs -> Box (V, 0, intersperse Newline xs)
...
let _ = output_result (
  mapM print_structure_item str_items >>= (return % concat_items)
)
