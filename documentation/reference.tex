...
start with an upper case letter. As an exception, the names \smlinline{true},
...
Conditionals are expressions that compute a value. They use the \smlinline{if},
\smlinline{then}, and \smlinline{else} keywords. For example,
\begin{smlcode}
...
A function is declared with the \smlinline{fun} keyword followed by the
...
Mutually recursive functions are supported with the \smlinline{and} keyword:
\begin{smlcode}
fun even x =
  if x = 0 then true
  else odd (x - 1)
...
An anonymous function can be created directly inside of an expression with the \smlinline{fn} keyword as follows:
\begin{smlcode}
val f =
...
arrows \smlinline{->}, and we used the \smlinline{fn} keyword twice to build a
...
expressions. A pattern has the same shape as the value that it matches. Patterns are separated by \smlinline{|}, and each pattern is accompanied by a \smlinline{=>} and expression. The \smlinline{case} expression finds the first pattern that matches the value given to it, and evaluates that pattern's expression.
\begin{smlcode}
fun is_red c =
  case c of
    Red => true
...
\todo{Describe where patters can appear besides case.}
...
CakeML supports the set of infix operators listed in Figure~\ref{precedences}.
  \todo{Implement o and before in the basis.}
...
\paragraph{\smlinline{before} and \smlinline{o}}
\todo{Write about these after implementing them.}

\paragraph{Suppressing infix status}
The \smlinline{op} keyword turns off the infix status the immediately following
infix operator.  For example, we can use \smlinline{op +} to get a prefix
addition function, and then partially apply it to an argument.
\begin{smlcode}
val add1 = op + 1
\end{smlcode}

The \smlinline{op} keyword also works in patterns, which lets us define our own
