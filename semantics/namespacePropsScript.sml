...
val id_to_name_mk_id = Q.store_thm ("id_to_namemods_mk_id[simp]",
  `!mn x. id_to_n (mk_id mn x) = x`,
 Induct_on `mn`
 >> rw [id_to_n_def, mk_id_def]);

val mk_id_surj = Q.store_thm ("mk_id_surj",
  `!id. ?p n. id = mk_id p n`,
 Induct_on `id`
 >> rw []
 >> metis_tac [mk_id_def]);
...
val nsAll_mono = Q.store_thm ("nsAll_mono[mono]",
  `(!id x. P id x ⇒ Q id x) ⇒ nsAll P e ⇒ nsAll Q e`,
  rw [nsAll_def]);

val nsSub_mono = Q.store_thm ("nsSub_mono[mono]",
...
val nsLookup_nsEmpty = Q.store_thm ("nsLookup_nsEmpty[simp]",
...
val alist_to_ns_nil = Q.store_thm ("alist_to_ns_nil[simp]",
  `alist_to_ns [] = nsEmpty`,
 rw [alist_to_ns_def, nsEmpty_def]);

val nsSub_nsEmpty = Q.store_thm ("nsSub_nsEmpty[simp]",
...
val nsAll_nsEmpty = Q.store_thm ("nsAll_nsEmpty[simp]",
  `!f. nsAll f nsEmpty`,
 rw [nsEmpty_def, nsAll_def]);

val nsAll2_nsEmpty = Q.store_thm ("nsAll2_nsEmpty[simp]",
  `!f. nsAll2 f nsEmpty nsEmpty`,
 rw [nsEmpty_def, nsAll2_def]);

val nsDom_nsEmpty = Q.store_thm ("nsDom_nsEmpty[simp]",
  `nsDom nsEmpty = {}`,
...
val nsAll2_nsSing = Q.store_thm ("nsAll2_nsSing[simp]",
...
val nsLookupMod_nsLift = Q.store_thm ("nsLookupMod_nsLift",
  `!mn e path.
...
val nsLookup_nsAppend_some = Q.store_thm ("nsLookup_nsAppend_some",
  `∀e1 id e2 v.
...
val nsAppend_to_nsBindList = Q.store_thm ("nsAppend_to_nsBindList",
...
val nsLookupMod_nsAppend_none = Q.store_thm ("nsLookupMod_nsAppend_none",
  `!e1 e2 path.
...
val nsAll_T = Q.store_thm ("eALL_T[simp]",
  `!e. nsAll (\n x. T) e`,
 rw [nsAll_def]);

val nsLookup_nsAll = Q.store_thm ("nsLookup_nsAll",
  `!env x P v. nsAll P env ∧ nsLookup env x = SOME v ⇒ P x v`,
 rw [nsAll_def]);

val nsAll_nsAppend = Q.store_thm ("nsAll_nsAppend",
...
val nsAll_nsBind = Q.store_thm ("nsAll_nsBind",
  `!P x v e. P (Short x) v ∧ nsAll P e ⇒ nsAll P (nsBind x v e)`,
 rw [nsAll_def, nsBind_def]
 >> Cases_on `id = Short x`
 >> fs []);

val nsAll_nsOptBind = Q.store_thm ("nsAll_nsOptBind",
...
val nsAll_alist_to_ns = Q.store_thm ("nsAll_alist_to_ns",
...
val nsAll_nsLift = Q.store_thm ("nsAll_nsLift[simp]",
...
val nsSub_refl = Q.store_thm ("nsSub_refl",
  `!P R. (!n x. P n x ⇒ R n x x) ⇒ !e. nsAll P e ⇒ nsSub R e e`,
 rw [nsSub_def]
 >> metis_tac [nsLookup_nsAll]);

val nsSub_nsBind = Q.store_thm ("nsSub_nsBind",
  `!R x v1 v2 e1 e2.
...
val nsSub_nsAppend2 = Q.store_thm ("nsSub_nsAppend2",
...
val nsSub_nsAppend_lift = Q.store_thm ("nsSub_nsAppend_lift",
  `!R mn e1 e1' e2 e2'.
...
val nsLookup_FOLDR_nsLift = Q.store_thm ("nsLookup_FOLDR_nsLift",
  `!e p k. nsLookup (FOLDR nsLift e p) (mk_id p k) = nsLookup e (Short k)`,
 Induct_on `p`
 >> rw [mk_id_def, nsLookup_def, nsLift_def]);

val nsLookup_FOLDR_nsLift_some = Q.store_thm ("nsLookup_FOLDR_nsLift_some",
  `!e p id v.
...
val nsLookupMod_FOLDR_nsLift_none = Q.store_thm ("nsLookupMod_FOLDR_nsLift_none",
...
val nsSub_compute_thm_general = Q.store_thm ("envSub_compute_thm_general",
  `!p R e1 e2.
...
val nsAll2_nsLookup2 = Q.store_thm ("nsAll2_nsLookup2",
  `!R e1 e2 n v2.
...
val nsAll2_nsLookup_none = Q.store_thm ("nsAll2_nsLookup_none",
  `!R e1 e2 n.
    nsAll2 R e1 e2
    ⇒
...
val nsAll2_nsBind = Q.store_thm ("nsAll2_nsBind",
  `!R x v1 v2 e1 e2.
...
val nsAll2_nsBindList = Q.store_thm ("nsAll2_nsBindList",
  `!R l1 l2 e1 e2.
...
val nsAll2_nsAppend = Q.store_thm ("nsAll2_nsAppend",
  `!R e1 e1' e2 e2'.
...
val nsAll2_alist_to_ns = Q.store_thm ("nsAll2_alist_to_ns",
...
val nsAll2_nsLift = Q.store_thm ("nsAll2_nsLift[simp]",
