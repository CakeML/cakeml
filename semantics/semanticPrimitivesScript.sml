...
val _ = Define `
 ((lit_same_type:lit -> lit -> bool) l1 l2=
   ((case (l1,l2) of
    ...
    | (Char _, Char _) => T
    | (StrLit _, StrLit _) => T
    | (Word x, Word y) => LENGTH x = LENGTH y
    | _ => F
  )))`;
...
(*val list_to_v : list v -> v*)
 ...
 ((list_to_v:(v)list -> v) []=  (Conv (SOME (TypeStamp "nil" list_type_num)) []))
...
(*val opw_lookup : opw -> list bool -> list bool -> list bool*)
val _ = Define `
 ((opw_lookup:opw ->(bool)list ->(bool)list ->(bool)list) op=  ((case op of
    Andw => band
  | Orw => bor
  | Xor => bxor
  | Add => fixadd
...
val _ = Define `
 ((opwb_lookup:opwb ->(bool)list ->(bool)list -> bool) op=  ((case op of
    Ltw => blt
  | Gtw => bgt
  | Leqw => bleq
  | Geqw => bgeq
...
val _ = Define `
 ((shift_lookup:shift ->(bool)list -> num ->(bool)list) sh=  ((case sh of
    Lsl => fixshiftl
  | Lsr => fixshiftr
  | Asr => fixasr
  | Ror => rotate
