...
open lem_pervasivesTheory lem_list_extraTheory lem_stringTheory libTheory astTheory lem_string_extraTheory environmentTheory ffiTheory lem_show_extraTheory;
...
val _ = Hol_datatype `
(*  'v *) sem_env =
  <| v : (varN, 'v) environment
   ; c : (conN, (num # tid_or_exn)) environment
   |>`;
...
val _ = type_abbrev( "env_ctor" , ``: (conN, (num # tid_or_exn)) environment``);
val _ = type_abbrev( "env_val" , ``: (varN, v) environment``);
...
(*import Show_extra*)
 ...
 val id_to_string_defn = Hol_defn "id_to_string" `

(id_to_string (Short s) = s)
/\
(id_to_string (Long x id) = (STRCAT x(STRCAT"."(id_to_string id))))`;

val _ = Lib.with_flag (computeLib.auto_import_definitions, false) Defn.save_defn id_to_string_defn;
