...
open lem_pervasivesTheory lem_list_extraTheory lem_stringTheory libTheory lem_string_extraTheory astTheory namespaceTheory ffiTheory lem_show_extraTheory;
 ...
 (do_app ((s: v store),(t: 'ffi ffi_state)) op vs= 
  ((case (op, vs) of
   ...
   if ((op = Divide) \/ (op = Modulo)) /\ (n2 = ( 0 : int)) then
     SOME ((s,t), Rerr (Rraise (prim_exn "Div"))) else
   ...
   if n < ( 0 : int) then SOME ((s,t), Rerr (Rraise (prim_exn "Subscript")))
   else
     let (s',lnum) =
   ...
   (case store_lookup lnum s of
         SOME (W8array ws) =>
   if i < ( 0 : int) then SOME ((s,t), Rerr (Rraise (prim_exn "Subscript")))
   else
     let n = (Num (ABS (I i))) in
     if n >= LENGTH ws then
       SOME ((s,t), Rerr (Rraise (prim_exn "Subscript"))) else
   ...
   (case vs_to_w8s s vs of
         SOME ws =>
   let (s',lnum) = (store_alloc (W8array ws) s) in
   SOME ((s',t), Rval (Loc lnum))
     | _ => NONE
   )
     | _ => NONE
   ...
   SOME
     ...
     (
     if (off < ( 0 : int)) \/
          ((len < ( 0 : int)) \/ (STRLEN str < Num (ABS (I (off + len)))))
     then Rerr (Rraise (prim_exn "Subscript")) else
       Rval
         (Litv
            (StrLit
               (IMPLODE
                  (
                  TAKE (Num (ABS (I len)))
                    (DROP (Num (ABS (I off))) (EXPLODE str))))))))
   ...
   if (off < ( 0 : int)) \/
        ((len < ( 0 : int)) \/
           ((STRLEN str < Num (ABS (I (off + len)))) \/
              ((dstoff < ( 0 : int)) \/
                 (LENGTH ds < Num (ABS (I (dstoff + len))))))) then
     SOME ((s,t), Rerr (Rraise (prim_exn "Subscript"))) else
     (case store_assign dst
             (W8array
                ((TAKE (Num (ABS (I dstoff))) ds ++
                    TAKE (Num (ABS (I len)))
                      (MAP (\ c .  i2w (int_of_num (ORD c))) (EXPLODE str)))
                   ++ DROP (Num (ABS (I (dstoff + len)))) ds)) s of
           SOME s' => SOME ((s',t), Rval (Conv NONE []))
       | _ => NONE
     )
     | _ => NONE
   ...
   SOME
     ...
     (
     if (off < ( 0 : int)) \/
          ((len < ( 0 : int)) \/ (LENGTH ws < Num (ABS (I (off + len)))))
     then Rerr (Rraise (prim_exn "Subscript")) else
       Rval
         (Litv
            (StrLit
               (IMPLODE
                  (
                  MAP (\ w .  CHR (w2n w))
                    (TAKE (Num (ABS (I len))) (DROP (Num (ABS (I off))) ws))))))))
     | _ => NONE
   )
...
(*import Show_extra*)
 ...
 val id_to_string_defn = Defn.Hol_multi_defns `

(id_to_string (Short s)=  s)
/\
(id_to_string (Long x id)=  (STRCAT x(STRCAT"."(id_to_string id))))`;

val _ = Lib.with_flag (computeLib.auto_import_definitions, false) (List.map Defn.save_defn) id_to_string_defn;
...
val _ = Define `
 ...
 val string_escape_defn = Defn.Hol_multi_defns `
...
val _ = Lib.with_flag (computeLib.auto_import_definitions, false) (List.map Defn.save_defn) string_escape_defn;
