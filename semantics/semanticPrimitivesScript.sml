...
(*val enc_real_uop : real_uop -> v*)
 val _ = Define `
  ...
  ((enc_real_uop:real_uop -> v) Real_Sqrt=  (Conv (SOME (TypeStamp "Real_sqrt" real_uop_type_num)) []))
/\
  ((enc_real_uop:real_uop -> v) Real_Neg=  (Conv (SOME (TypeStamp "Real_neg" real_uop_type_num)) []))
/\
  ((enc_real_uop:real_uop -> v) Real_Abs=  (Conv (SOME (TypeStamp "Real_abs" real_uop_type_num)) []))`;


(*val enc_real_bop : real_bop -> v*)
 val _ = Define `

  ((enc_real_bop:real_bop -> v) Real_Div=  (Conv (SOME (TypeStamp "Real_div" real_bop_type_num)) []))
/\
  ((enc_real_bop:real_bop -> v) Real_Mul=  (Conv (SOME (TypeStamp "Real_mul" real_bop_type_num)) []))
/\
  ((enc_real_bop:real_bop -> v) Real_Sub=  (Conv (SOME (TypeStamp "Real_sub" real_bop_type_num)) []))
/\
  ((enc_real_bop:real_bop -> v) Real_Add=  (Conv (SOME (TypeStamp "Real_add" real_bop_type_num)) []))`;


(*val enc_real_cmp : real_cmp -> v*)
 val _ = Define `

  ((enc_real_cmp:real_cmp -> v) Real_Equal=
     (Conv (SOME (TypeStamp "Real_equal" real_cmp_type_num)) []))
/\
  ((enc_real_cmp:real_cmp -> v) Real_GreaterEqual=
     (Conv (SOME (TypeStamp "Real_greaterequal" real_cmp_type_num)) []))
/\
  ((enc_real_cmp:real_cmp -> v) Real_Greater=
     (Conv (SOME (TypeStamp "Real_greater" real_cmp_type_num)) []))
/\
  ((enc_real_cmp:real_cmp -> v) Real_LessEqual=
     (Conv (SOME (TypeStamp "Real_lessequal" real_cmp_type_num)) []))
/\
  ((enc_real_cmp:real_cmp -> v) Real_Less=
     (Conv (SOME (TypeStamp "Real_less" real_cmp_type_num)) []))`;
...
/\
  ((enc_op:op -> v) (FpFromWord)=
     (Conv (SOME (TypeStamp "Fpfromword" op_type_num)) []))
/\
  ((enc_op:op -> v) (FpToWord)=
     (Conv (SOME (TypeStamp "Fptoword" op_type_num)) []))
/\
  ((enc_op:op -> v) (Real_bop x_10)=
     (Conv (SOME (TypeStamp "Real_bop" op_type_num)) [enc_real_bop x_10]))
/\
  ((enc_op:op -> v) (Real_uop x_9)=
     (Conv (SOME (TypeStamp "Real_uop" op_type_num)) [enc_real_uop x_9]))
/\
  ((enc_op:op -> v) (Real_cmp x_8)=
     (Conv (SOME (TypeStamp "Real_cmp" op_type_num)) [enc_real_cmp x_8]))
/\
  ((enc_op:op -> v) (RealFromFP)=
     (Conv (SOME (TypeStamp "Realfromfp" op_type_num)) []))
/\
...
(*val enc_scope: fp_opt -> v*)
Definition enc_scope_def:
  enc_scope Opt = (Conv (SOME (TypeStamp "Opt" fp_opt_num)) []) ∧
  enc_scope NoOpt = (Conv (SOME (TypeStamp "Noopt" fp_opt_num)) [])
End

(**
  ((enc_exp:exp -> v) (FpOptimise sc e)=
     (Conv (SOME (TypeStamp "Fpoptimise" exp_type_num))
         [enc_scope sc; enc_exp e]))`; **)
