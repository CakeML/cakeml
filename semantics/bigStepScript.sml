...
val _ = Hol_reln ` (! ck s1 s2 env d new_tds new_env.
(evaluate_dec ck NONE env s1 d (s2, Rval (new_tds, new_env)))
==>
evaluate_top ck env s1 (Tdec d) (s2, ([],new_tds), Rval ([], new_env)))

/\ (! ck s1 s2 env d err.
(evaluate_dec ck NONE env s1 d (s2, Rerr err))
==>
evaluate_top ck env s1 (Tdec d) (s2, ([],[]), Rerr err))

/\ (! ck s1 s2 env ds mn specs new_tds new_env.
...
/\ (! s1 s2 env top tops new_mods new_tds new_env.
(evaluate_top F env s1 top (s2, new_tds, Rval (new_mods, new_env)) /\
