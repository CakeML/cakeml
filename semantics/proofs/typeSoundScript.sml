...
val type_env_conv_thm = Q.prove (
  `∀ctMap envC tenvC.
     ...
     ∀cn tvs ts tn.
       (eLookup tenvC cn = SOME (tvs,ts,tn) ⇒
        eLookup envC cn = SOME (LENGTH ts,tn) ∧
        FLOOKUP ctMap (id_to_n cn,tn) = SOME (tvs, ts)) ∧
       (eLookup tenvC cn = NONE ⇒ eLookup envC cn = NONE)`,
 rw []
 >> imp_res_tac eAll2_eLookup2
 >> TRY (PairCases_on `v1`)
 >> fs [type_ctor_def]
 >> metis_tac [eAll2_eLookup_none]);
...
`!v vs.
 ...
 full_simp_tac(srw_ss())[] >>
 srw_tac[][] >>
 qpat_assum `type_v x0 x1 x2 (Conv x3 x4) x5` (mp_tac o SIMP_RULE (srw_ss()) [Once type_v_cases]) >>
 srw_tac[][] >>
 srw_tac[][Once type_v_cases] >>
 res_tac >>
 full_simp_tac(srw_ss())[ctMap_has_lists_def] >>
...
`!ctMap tenvS vs ts t.
 ...
 >- (
   ...
   >- (
     irule eAll2_eBind
     >> simp [build_rec_env_merge, eAppend_to_eBindList, add_tenvE_bvl]
     >> irule eAll2_eBindList
     >> simp []
...
val pat_type_sound = Q.store_thm ("pat_type_sound",
 `(∀(cenv : env_ctor) st p v bindings tenv ctMap tbindings t new_tbindings tenvS tvs.
   ctMap_ok ctMap ∧
   eAll2 (type_ctor ctMap) cenv tenv.c ∧
...
val lookup_var_sound = Q.store_thm ("lookup_var_sound",
  `!n tvs tenvE targs t ctMap tenvS env tenv.
    ...
    type_all_env ctMap tenvS env (tenv with v := add_tenvE tenvE tenv.v)
    ⇒
    ?v. eLookup env.v n = SOME v ∧ type_v tvs ctMap tenvS v (deBruijn_subst 0 targs t)`,
 rw [lookup_var_def, type_all_env_def]
 >> `eLookup (add_tenvE tenvE tenv.v) n = SOME (LENGTH targs, t)`
   suffices_by (
     rw []
     >> imp_res_tac eAll2_eLookup2
     >> fs []
 ...
 >> every_case_tac
 >> fs []
 >- metis_tac [eLookup_add_tenvE2]
 >- (
   irule eLookup_add_tenvE1
   >- metis_tac []
   >> fs [tenv_val_exp_ok_def]
   >> metis_tac [tveLookup_freevars])
 >- metis_tac [eLookup_add_tenvE3]);
...
val exp_type_sound = Q.store_thm ("exp_type_sound",
 `(!(s:'ffi state) env es r s' tenv tenvE ts tvs tenvS.
 ...
 (!(s:'ffi state) env v pes err_v r s' tenv tenvE t1 t2 tvs tenvS.
 ...
 >- (
   ...
   >- metis_tac [])
 >- (
   qpat_assum `type_e _ _ (Con _ _) _` mp_tac
   >> simp [Once type_e_cases]
   >> fs [is_value_def]
   >> split_pair_case_tac
   >> fs []
 ...
 >- (
   ...
   >- (
     ...
     >> qabbrev_tac `env' = eOptBind n (HD [x]) env.v`
     ...
     >> `type_all_env ctMap tenvS' (env with v := env') (tenv with v := add_tenvE tenv' tenv.v)`
       by (
         ...
         >> Cases_on `n`
         >> fs [opt_bind_name_def, environmentTheory.eOptBind_def, add_tenvE_def]
         >> irule eAll2_eBind
         >> simp [])
     >> disch_then drule
     >> rw []
 ...
 >- (
   ...
   >- (
     ...
     >> `eAll2 (λi v (tvs,t). type_v tvs ctMap tenvS v t)
           (eAppend (alist_to_env bindings') env.v)
           (add_tenvE (bind_var_list tvs bindings tenvE) tenv.v)`
       by (
         simp [eAppend_to_eBindList, add_tenvE_bvl]
         >> irule eAll2_eBindList
         >> simp []
         >> fs [LIST_REL_EL_EQN]
         >> rw []
...
val decs_type_sound = Q.store_thm ("decs_type_sound",
 `∀mn (st:'ffi state) env ds st' r tdecs1 ctMap tenvS tenv tdecs1' tenv'.
 ...
 >- ( (* case exception *)
   ...
   >- (
     fs [type_all_env_def]
     >> simp [type_ctor_def, FLOOKUP_FUNION, id_to_n_def, FLOOKUP_UPDATE])
   >> conj_asm1_tac
   >- (
     fs [good_ctMap_def]
     >> rw []
...
val tops_type_sound_no_extra_checks = Q.store_thm ("tops_type_sound_no_no_extra_checks",
 `∀(st:'ffi state) env tops st' env' r tdecs1 ctMap tenvS tenv tdecs1' tenv'.
 ...
 >- (
   ...
   >- (
     ...
     >> qexists_tac `extend_dec_tenv <| v := eLift mn tenv_impl.v; c := eLift mn tenv_impl.c; t := eLift mn tenv_spec.t |> tenv_no_sig`
...
val tops_type_sound = Q.store_thm ("tops_type_sound",
 `∀(st:'ffi state) env tops st' r checks tdecs1 ctMap tenvS tenv tdecs1' tenv'.
   evaluate_tops st env tops = (st',r) ∧
...
val prim_type_sound_invariants = Q.store_thm("prim_type_sound_invariants",
  ...
  \\ TRY (
    ...
    \\ rw[] \\ fs[])
  \\ rw[SUBSET_DEF] \\ fs[GSPECIFICATION, environmentTheory.eLookup_def, environmentTheory.eLookupMod_def]
  \\ EVAL_TAC
  >> pop_assum mp_tac
  >> rpt (CASE_TAC >> rw [] >> EVAL_TAC >> rw [id_to_n_def]));
