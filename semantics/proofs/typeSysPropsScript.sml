...
val type_env_cases = List.nth (CONJUNCTS type_v_cases, 1);
val consistent_mod_cases = List.nth (CONJUNCTS type_v_cases, 2);
...
val unchanged_env = Q.store_thm ("unchanged_env[simp]",
 ...
 rw [type_environment_component_equality, semanticPrimitivesTheory.environment_component_equality]);
...
val type_env_list_rel = store_thm("type_env_list_rel",
  ...
  ONCE_REWRITE_TAC [hd (tl (CONJUNCTS type_v_cases))] >>
  srw_tac[][bind_var_list2_def] >>
  PairCases_on `h` >>
...
val flat_tenv_tabbrev_ok_merge = Q.store_thm ("flat_tenv_tabbrev_ok_merge",
`!tenvT1 tenvT2.
...
induct_on `funs` >>
srw_tac[][] >>
qpat_x_assum `type_funs x2 x3 x4` (ASSUME_TAC o SIMP_RULE (srw_ss()) [Once type_e_cases]) >>
full_simp_tac(srw_ss())[] >>
...
`(!tenv e t.
 ...
 >- (cases_on `pes` >>
     ...
     qpat_x_assum `!x. P x` (ASSUME_TAC o Q.SPEC `(FST h, SND h)`) >>
     full_simp_tac(srw_ss())[] >>
...
induct_on `funs` >>
srw_tac[][] >>
qpat_x_assum `type_funs tenv funspat tenv'`
...
induct_on `funs` >>
srw_tac[][Once find_recfun_def] >>
qpat_x_assum `type_funs tenv (h::funs) tenv'`
            (ASSUME_TAC o SIMP_RULE (srw_ss ()) [Once type_e_cases]) >>
srw_tac[][] >>
full_simp_tac(srw_ss())[] >>
cases_on `fn' = fn` >>
...
val type_env_append = Q.store_thm ("type_env_append",
 ...
 >> fs [bind_var_list_def]
 >> simp [Once type_v_cases]
 >> qpat_x_assum `type_env _ _ (_::_) _` mp_tac
 >> simp [Once type_v_cases]
 >> rw []);
...
val type_env_eqn = Q.store_thm ("type_env_eqn",
`!ctMap tenvS.
...
`∀tenvS tenvC tenv.
 ...
 qpat_x_assum`X = env.m`(assume_tac o SYM) >> full_simp_tac(srw_ss())[] >>
 cases_on `n` >>
 full_simp_tac(srw_ss())[] >>
 srw_tac[][lookup_var_id_def, t_lookup_var_id_def] >>
 imp_res_tac type_lookup >>
 srw_tac[][] >>
 qpat_x_assum `consistent_mod_env tenvS x0 x1 x2` (ASSUME_TAC o SIMP_RULE (srw_ss()) [Once type_v_cases]) >>
 full_simp_tac(srw_ss())[] >>
 srw_tac[][] >>
...
`(!tvs ctMap tenvS v t. type_v tvs ctMap tenvS v t ⇒
 ...
 >- (
   ...
   >> last_x_assum drule
   >> rw []
   >> pop_assum (qspecl_then [`targs`,`tvs''`] mp_tac)
   >> simp []
   >> disch_then irule
 ...
 >- (qexists_tac `tenv` >>
     ...
     >- (qpat_x_assum `type_funs x y z` (fn x => ALL_TAC) >>
         induct_on `tenv'` >>
         full_simp_tac(srw_ss())[] >>
         srw_tac[][] >>
         PairCases_on `h` >>
...
qpat_x_assum `type_env ctMap tenvS env tenv_pat` (MP_TAC o SIMP_RULE (srw_ss ()) [Once type_env_cases]) >>
srw_tac[][] >>
full_simp_tac(srw_ss())[] >>
srw_tac[][] >>
full_case_tac >>
...
`!tenvS ctMap menv tenvM tenv env n.
 ...
 induct_on `menv` >>
 srw_tac[][] >>
 qpat_x_assum `consistent_mod_env x0 x1 x2 x3` (ASSUME_TAC o SIMP_RULE (srw_ss()) [Once consistent_mod_cases]) >>
 full_simp_tac(srw_ss())[] >>
 srw_tac[][] >>
