...
Theorem evaluate_ffi_intro:
    (∀(s:'a state) env e s' r.
     ...
     ∀(t:'b state).
       ...
       t.fp_canOpt = s.fp_canOpt /\ t.fp_choices = s.fp_choices
       ⇒
       evaluate t env e = (t with <| clock := s'.clock; refs := s'.refs; fp_opts := s'.fp_opts; fp_canOpt := s'.fp_canOpt; fp_choices := s'.fp_choices |>, r)) ∧
  (∀(s:'a state) env v pes errv s' r.
     ...
     ∀(t:'b state).
       ...
       t.fp_canOpt = s.fp_canOpt /\ t.fp_choices = s.fp_choices
       ⇒
       evaluate_match t env v pes errv = (t with <| clock := s'.clock; refs := s'.refs; fp_opts := s'.fp_opts; fp_canOpt := s'.fp_canOpt; fp_choices := s'.fp_choices|>, r))
Proof
  ho_match_mp_tac evaluate_ind
  \\ rw[]
