...
Theorem is_clock_io_mono_ret_fpOpt:
  is_clock_io_mono (\s. (s with fp_state := (s.fp_state with canOpt := F), Rval v)) s
Proof
  fs [is_clock_io_mono_def]
QED
...
Theorem is_clock_io_mono_fp_optimise:
  ! (s:'ffi state) env es.
    is_clock_io_mono (\ s. evaluate s env [e]) (s with fp_state := (s.fp_state with canOpt := (case sc of Opt => T | NoOpt => F)))==>
...
Proof
  ...
  \\ assume_tac (SIMP_RULE std_ss [] is_clock_io_mono_do_app) \\ fs[]
QED
...
local
  val trivial = rpt strip_tac \\ rveq \\ fs[fpState_component_equality, state_component_equality];
  val by_eq =
      `s1.fp_state.choices = s2.fp_state.choices`
        by (imp_res_tac fpSemPropsTheory.evaluate_fp_opts_inv \\ fs[dec_clock_def])
...
Proof
  ho_match_mp_tac evaluate_ind
  \\ rpt strip_tac \\ fs[evaluate_def, state_component_equality, fpState_component_equality]
...
Proof
  ...
  >- ((* First do a case split on the annotation because that changes how the
      ...
      \\ rpt strip_tac \\ rveq
      \\ fs[state_component_equality, fpState_component_equality]
      (* Case annot = `Opt` first *)
      \\ TRY (
          imp_res_tac fpSemPropsTheory.evaluate_fp_opts_inv
          \\ fs[fpState_component_equality, state_component_equality])
      (* Case annot = `NoOpt` *)
      \\ drule (CONJUNCT1 (SIMP_RULE std_ss [] evaluate_fp_intro_eq_opt))
      \\ disch_then (qspec_then `fp_state2 with canOpt := F` impl_subgoal_tac)
      \\ imp_res_tac fpSemPropsTheory.evaluate_fp_opts_inv
      \\ fs[fpState_component_equality, state_component_equality])
  >- (ntac 2 (TOP_CASE_TAC \\ fs[])
      \\ trivial)
...
Theorem evaluate_ffi_intro:
    (∀(s:'a state) env e s' r.
     ...
     ∀(t:'b state).
       ...
       evaluate t env e = (t with <| clock := s'.clock; refs := s'.refs; fp_state := s'.fp_state|>, r)) ∧
  (∀(s:'a state) env v pes errv s' r.
     evaluate_match s env v pes errv = (s',r) ∧
     s'.ffi = s.ffi ∧
     (∀outcome. r ≠ Rerr(Rabort(Rffi_error outcome)))
     ...
     ∀(t:'b state).
       t.clock = s.clock ∧ t.refs = s.refs /\
       t.fp_state = s.fp_state ==>
       evaluate_match t env v pes errv = (t with <| clock := s'.clock; refs := s'.refs; fp_state := s'.fp_state|>, r))
Proof
  ho_match_mp_tac evaluate_ind
  \\ rw[]
  ...
  >- (
    ...
    \\ `(t with fp_state := (t.fp_state with canOpt := T)).clock = s.clock` by  fs[state_component_equality]
    \\ res_tac
    \\ fs[state_component_equality])
  >- (
    rfs[evaluate_def]
...
local
  val eval_goal =
    ...
    st1.fp_state = st2.fp_state ==>
    compress_list v = v``;
  val eval_match_goal =
    ``\ st1 env v1 pats errv. ! st2 v2.
    evaluate_match st1 env v1 pats errv = (st2, Rval v2) ==>
    st1.fp_state = st2.fp_state ==>
    compress_list v2 = v2``;
  val indThm = evaluate_ind |> ISPEC eval_goal |> SPEC eval_match_goal;
  val evaluate_tac = rpt (TOP_CASE_TAC \\ fs[]) \\ rpt strip_tac \\ rveq \\ fs[];
  val solve_tac =
    evaluate_tac
    \\ (rename [`evaluate st1 env [e1] = (st2, _)`, `evaluate st2 env (e2::es) = (st3, _)`]
        ORELSE
       rename [`evaluate st1 env [e] = (st2, _)`, `evaluate_match st2 env _ _ _ = (st3, _)`])
    \\ `st1.fp_state = st2.fp_state /\ st2.fp_state = st3.fp_state`
        by (imp_res_tac evaluate_fp_opts_inv \\ fs[fpState_component_equality]
            ...
            \\ `st2.fp_state.choices = st3.fp_state.choices` by (fs[])
            \\ TRY (rpt (qpat_x_assum `! x. _ _ = _ _` (fn thm => fs[GSYM thm])))
            \\ fs[])
    \\ fs[compress_def]
in
Theorem evaluate_same_fp_state_compress_idem:
  (! st1 env e.
    ^eval_goal st1 env e) /\
  (! st1 env v1 pats errv.
    ^eval_match_goal st1 env v1 pats errv)
Proof
  match_mp_tac indThm \\ fs[]
  \\ rpt strip_tac \\ fs[compress_def]
  \\ TRY (qpat_x_assum `evaluate _ _ _ = _` mp_tac)
  \\ fs[evaluate_def]
  >- (solve_tac
      \\ imp_res_tac evaluate_sing \\ rveq \\ fs[compress_def])
  >- (rpt strip_tac \\ rveq \\ fs[compress_def])
  >- (evaluate_tac)
  >- (solve_tac)
  >- (evaluate_tac \\ fs[build_conv_def]
      \\ every_case_tac \\ fs[] \\ rveq \\ fs[compress_def, compress_list_reverse])
  >- (evaluate_tac
