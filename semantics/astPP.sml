...
(* Control whether best-effort SML compatibility is turned on *)
val sml_compat = ref false;
...
fun ttupPrint sys d t pg str brk blk =
  let val args = #1(listSyntax.dest_list (strip t))
      ...
      val spa = ""
  in
    case args of [] => str"unit"
    | _ => str"(">>printTuple sep (sys (pg,pg,pg) d) str args >>str ")"
  end;

val _=add_astPP("ttupprint", ``Attup ls``,genPrint ttupPrint);
...
fun prefixargsPrint uop sys d t pg str brk blk =
  let
    ...
    fun printListSML [] = str ""
      | printListSML [x] = str " " >> sys (Prec(0,"app"),pg,pg) d x
      | printListSML (x::xs) = str " " >> sys (Prec(0,"app"),pg,pg) d x >>str",">> printListSML xs
   in
     if !sml_compat then
        m_brack str pg ( str uop >> str"(" >>printListSML ls>> str")")
     else
...
val _=add_astPP ("arrayListAppendrealprint", ``App ListAppend ls``,genPrint (infixrealPrint "@"));
...
fun enable_sml_compat () = sml_compat:=true;
fun disable_sml_compat () = sml_compat:=false;

(*
enable_astPP_verbose();
disable_astPP_verbose();
``Attup [Atapp [] (Short "int"); Atapp [] (Short "reg_alloc_tag")]``
``Var(Long "foo" (Short("asdf")))``
val t = ``Dlet a b c``
``App (Aw8sub) [x;y]``
``App (Aw8length) [x]``
