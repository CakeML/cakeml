     ...
     if nt <> mkNT nm then NONE
       ...
       case args of
           [pt] =>
                ...
                OPTION_MAP (\tn. Ast_Tapp [] (SOME tn)) (ptree_Tyop pt)
         | [dt; opn] => do
...
val mkPatApp_def = Define`
  mkPatApp cn p =
    if cn = Short "ref" then Ast_Pref p
    else
      case p of
          Ast_Pcon NONE pl => Ast_Pcon (SOME cn) pl
        | _ => Ast_Pcon (SOME cn) [p]
`;
