...
val fix_clock_IMP = Q.prove(
  ...
  PairCases_on `x` \\ fs [fix_clock_def] \\ rw [] \\ fs []);
...
val evaluate_def = save_thm("evaluate_def",
  REWRITE_RULE [fix_clock_do_eval_res, fix_clock_evaluate, fix_clock_evaluate_fp_opt] evaluate_def
  |> INST_TYPE[alpha|->``:'ffi``] (* TODO: this is only broken because Lem sucks *));

val evaluate_ind = save_thm("evaluate_ind",
  REWRITE_RULE [fix_clock_do_eval_res, fix_clock_evaluate, fix_clock_evaluate_fp_opt] evaluate_ind
  |> INST_TYPE[alpha|->``:'ffi``] (* TODO: this is only broken because Lem sucks *));
