...
val _ = type_abbrev((*  'ffi *) "small_state" , ``: v environment # (v, 'ffi) store_ffi # exp_or_val # ctxt list``);
...
(*val push : forall 'ffi. environment v -> store_ffi v 'ffi -> exp -> ctxt_frame -> list ctxt -> e_step_result 'ffi*)
val _ = Define `
 (push env s e c' cs = (Estep (env, s, Exp e, ((c',env)::cs))))`;


(*val return : forall 'ffi. environment v -> store_ffi v 'ffi -> v -> list ctxt -> e_step_result 'ffi*)
val _ = Define `
