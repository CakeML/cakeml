...
open lem_pervasives_extraTheory libTheory astTheory environmentTheory ffiTheory semanticPrimitivesTheory;
    ...
    | (Cmat ()  ((p,e)::pes) err_v, env) :: c =>
        if ALL_DISTINCT (pat_bindings p []) then
          (case pmatch env.c (FST s) p v [] of
            ...
            | Match env' => Estep (( env with<| v := eAppend (alist_to_env env') env.v |>), s, Exp e, c)
          )
        else
          Eabort Rtype_error
    | (Clet n ()  e, env) :: c =>
        Estep (( env with<| v := eOptBind n v env.v |>), s, Exp e, c)
    | (Ccon n vs ()  [], env) :: c =>
