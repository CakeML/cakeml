...
((case id of
    ...
    | Long x y =>
        (case lookup instance_Basic_classes_Eq_var_dict x tenvM of
            (NONE) => (NONE)
          | (SOME tenvE') => lookup 
  instance_Basic_classes_Eq_var_dict y tenvE'
        )
  )))`;
...
((case lookup instance_Basic_classes_Eq_var_dict tv s of
      (NONE) => Tvar tv
    | (SOME(t)) => t
  )))
/\
...
val _ = Hol_reln ` type_p : num -> tenvC -> pat -> t -> (varN # t) list -> bool
/\ type_ps : num -> tenvC -> pat -> t list -> (varN # t) list -> bool (! tvs cenv n t.
...
val _ = Hol_reln ` type_e : tenvM -> tenvC -> tenvE -> exp -> t -> bool
/\ type_es : tenvM -> tenvC -> tenvE -> exp list -> t list -> bool
/\ type_funs : tenvM -> tenvC -> tenvE -> (varN # varN # exp) list -> (varN # t) list -> bool (! menv cenv tenv b.
T
==>
...
(lookup instance_Basic_classes_Eq_var_dict fn env' = (NONE)))))
==>
...
val _ = Hol_reln ` type_d :  modN option -> tenvM -> tenvC -> tenvE -> dec -> tenvC -> (varN, (num # t)) env -> bool (! tvs mn menv cenv tenv p e t tenv'.
(is_value e /\
((ALL_DISTINCT (pat_bindings p [])) /\
(type_p tvs cenv p t tenv' /\
...
val _ = Hol_reln `  type_ds :  modN option -> tenvM -> tenvC -> tenvE -> dec list -> tenvC -> (varN, (num # t)) env -> bool (! mn menv cenv tenv.
T
==>
...
val _ = Hol_reln ` type_specs :  modN option -> tenvC -> (varN, (num # t)) env -> specs -> tenvC -> (varN, (num # t)) env -> bool (! cenv tenv mn.
T
==>
...
type_specs mn cenv (bind x ((LENGTH fvs), type_subst (list_combine fvs ((MAP Tvar_db ((COUNT_LIST ((LENGTH fvs))))))) t) tenv) specs cenv' tenv')
==>
...
(! x.
    (case lookup instance_Basic_classes_Eq_var_dict x tenv_spec of
        (SOME (tvs_spec, t_spec)) =>
          (case lookup 
  instance_Basic_classes_Eq_var_dict x tenv_impl of
              (NONE) => F
...
val _ = Hol_reln ` check_signature :  modN option -> tenvC -> (varN, (num # t)) env ->  specs option -> tenvC -> (varN, (num # t)) env -> bool (! mn cenv tenv.
T
==>
...
val _ = Hol_reln ` type_top : t (! menv cenv tenv d cenv' tenv'.
...
val _ = Hol_reln ` type_prog : tenvM -> tenvC -> tenvE -> top list -> tenvM -> tenvC -> (varN, (num # t)) env -> bool (! menv cenv tenv.
T
==>
