...
val _ = Define `
 (bind_tvar tvs tenv_val=  (if tvs =( 0 : num) then tenv_val else Bind_tvar tvs tenv_val))`;
...
val _ = Define `
 ...
 ((case id of
      Short x => lookup_tenv_val x(( 0 : num)) tenv.v
    | Long x y =>
 ...
 (check_dup_ctors tds /\
  EVERY
    (\ (tvs,tn,ctors) . 
       ALL_DISTINCT tvs /\
       EVERY
         (\ (cn,ts) .  EVERY (check_freevars(( 0 : num)) tvs) ts /\ EVERY (check_type_names tenv_tabbrev) ts)
         ctors)
    tds /\
  ALL_DISTINCT (MAP (\p .  
...
(! ((p,e) :: LIST_TO_SET pes). ? bindings.
   ...
   type_e (tenv with<| v := (bind_var_list(( 0 : num)) bindings tenv.v)|>) e t))
==>
...
==>
type_e tenv (Var n) (deBruijn_subst(( 0 : num)) targs t))

/\ (! tenv n e t1 t2.
(check_freevars (num_tvs tenv.v) [] t1 /\
type_e (tenv with<| v := (Bind_name n(( 0 : num)) t1 tenv.v)|>) e t2)
==>
...
/\ (! tenv n e1 e2 t1 t2.
(type_e tenv e1 t1 /\
type_e (tenv with<| v := (opt_bind_name n(( 0 : num)) t1 tenv.v)|>) e2 t2)
==>
...
(type_funs (tenv with<| v := (bind_var_list(( 0 : num)) bindings tenv.v)|>) funs bindings /\
type_e (tenv with<| v := (bind_var_list(( 0 : num)) bindings tenv.v)|>) e t)
==>
type_e tenv (Letrec funs e) t)
...
val _ = Define `
 ...
 (! t1 tenv1 t2 tenv2.    
(type_p(( 0 : num)) tenv p t1 tenv1 /\ type_e tenv e t1 /\
    type_p(( 0 : num)) tenv p t2 tenv2 /\ type_e tenv e t2)
    ==>    
(tenv1 = tenv2)))`;
...
type_e tenv e t)
==>
type_d extra_checks mn decls tenv (Dlet p e) empty_decls (FEMPTY, [], tenv_add_tvs(( 0 : num)) bindings))

/\ (! extra_checks mn tenv funs bindings tvs decls.
(type_funs (tenv with<| v := (bind_var_list(( 0 : num)) bindings (bind_tvar tvs tenv.v))|>) funs bindings /\
(extra_checks ==>  
(! tvs' bindings'.
    type_funs (tenv with<| v := (bind_var_list(( 0 : num)) bindings' (bind_tvar tvs' tenv.v))|>) funs bindings' ==>
      weakE (tenv_add_tvs tvs bindings) (tenv_add_tvs tvs' bindings'))))
