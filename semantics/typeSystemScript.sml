...
(*val check_freevars : nat -> list tvarN -> t -> bool*)
 val check_freevars_defn = lemLib.lem_define `
...
(*val type_subst : Map.map tvarN t -> t -> t*)
 val type_subst_defn = lemLib.lem_define `
...
(*val deBruijn_inc : nat -> nat -> t -> t*)
 val deBruijn_inc_defn = lemLib.lem_define `
...
(*val deBruijn_subst : nat -> list t -> t -> t*)
 val deBruijn_subst_defn = lemLib.lem_define `
...
val _ = Define `
 (bind_tvar tvs tenv_val=  (if tvs =I 0 then tenv_val else Bind_tvar tvs tenv_val))`;
...
val _ = Define `
 ...
 ((case id of
      Short x => lookup_tenv_val x(I 0) tenv.v
    | Long x y =>
...
(*val check_type_names : tenv_tabbrev -> t -> bool*)
 val check_type_names_defn = lemLib.lem_define `
...
(*val type_name_subst : tenv_tabbrev -> t -> t*)
 val type_name_subst_defn = lemLib.lem_define `
 ...
 (check_dup_ctors tds /\
  EVERY
    (\ (tvs,tn,ctors) . 
       ALL_DISTINCT tvs /\
       EVERY
         (\ (cn,ts) .  EVERY (check_freevars(I 0) tvs) ts /\ EVERY (check_type_names tenv_tabbrev) ts)
         ctors)
    tds /\
  ALL_DISTINCT (MAP (\p .  
...
val _ = Define `
 ...
 (EVERY (check_freevars(I 0) []) ts))`;


(* For the value restriction on let-based polymorphism *)
(*val is_value : exp -> bool*)
 val is_value_defn = lemLib.lem_define `
...
(! ((p,e) :: LIST_TO_SET pes). ? bindings.
   ...
   type_e (tenv with<| v := (bind_var_list(I 0) bindings tenv.v)|>) e t))
==>
...
==>
type_e tenv (Var n) (deBruijn_subst(I 0) targs t))

/\ (! tenv n e t1 t2.
(check_freevars (num_tvs tenv.v) [] t1 /\
type_e (tenv with<| v := (Bind_name n(I 0) t1 tenv.v)|>) e t2)
==>
...
(! ((p,e) :: LIST_TO_SET pes) . ? bindings.
   ...
   type_e (tenv with<| v := (bind_var_list(I 0) bindings tenv.v)|>) e t2))
==>
type_e tenv (Mat e pes) t2)

/\ (! tenv n e1 e2 t1 t2.
(type_e tenv e1 t1 /\
type_e (tenv with<| v := (opt_bind_name n(I 0) t1 tenv.v)|>) e2 t2)
==>
...
(type_funs (tenv with<| v := (bind_var_list(I 0) bindings tenv.v)|>) funs bindings /\
type_e (tenv with<| v := (bind_var_list(I 0) bindings tenv.v)|>) e t)
==>
type_e tenv (Letrec funs e) t)
...
val _ = Define `
 ...
 (! t1 tenv1 t2 tenv2.    
(type_p(I 0) tenv.c p t1 tenv1 /\ type_e tenv e t1 /\
    type_p(I 0) tenv.c p t2 tenv2 /\ type_e tenv e t2)
    ==>    
(tenv1 = tenv2)))`;
...
(deBruijn_subst(I 0) subst t_impl = t_spec)
          )
        | NONE => T
    )))`;
...
ALL_DISTINCT (pat_bindings p []) /\
type_p(I 0) tenv.c p t bindings /\
type_e tenv e t)
==>
type_d extra_checks mn decls tenv (Dlet p e) empty_decls (FEMPTY, [], tenv_add_tvs(I 0) bindings))

/\ (! extra_checks mn tenv funs bindings tvs decls.
(type_funs (tenv with<| v := (bind_var_list(I 0) bindings (bind_tvar tvs tenv.v))|>) funs bindings /\
(extra_checks ==>  
(! tvs' bindings'.
    type_funs (tenv with<| v := (bind_var_list(I 0) bindings' (bind_tvar tvs' tenv.v))|>) funs bindings' ==>
      weakE (tenv_add_tvs tvs bindings) (tenv_add_tvs tvs' bindings'))))
