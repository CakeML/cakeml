...
open lem_pervasives_extraTheory libTheory astTheory environmentTheory semanticPrimitivesTheory;
...
val _ = Define `
 (bind_tvar tvs tenvE = (if tvs = 0 then tenvE else Bind_tvar tvs tenvE))`;
...
val _ = type_abbrev( "tenv_ctor" , ``: (conN, ( tvarN list # t list # tid_or_exn)) environment``);
val _ = type_abbrev( "tenv_val" , ``: (varN, (num # t)) environment``);
...
==>
type_d extra_checks mn decls tenv (Dlet p e)
  empty_decls <| v := (alist_to_env (tenv_add_tvs( 0) bindings)); c := eEmpty; t := eEmpty |>)
...
type_d extra_checks mn decls tenv (Dtabbrev tvs tn t)
  empty_decls <| v := eEmpty; c := eEmpty;
                 t := (eSing tn (tvs,type_name_subst tenv.t t)) |>)
...
type_d extra_checks mn decls tenv (Dexn cn ts)
  decls' <| v := eEmpty;
            c := (eSing cn ([], MAP (type_name_subst tenv.t) ts, TypeExn (mk_id mn cn)));
            t := eEmpty |>)`;
...
type_ds extra_checks mn decls tenv []
  empty_decls <| v := eEmpty; c := eEmpty; t := eEmpty |>)
...
type_specs mn tenvT (Sval x t :: specs)
  ...
  (extend_dec_tenv tenv
    <| v := (eSing x (LENGTH fvs, type_subst subst (type_name_subst tenvT t)));
       c := eEmpty;
       t := eEmpty |>))
...
type_specs mn tenvT (Sexn cn ts :: specs)
  ...
  (extend_dec_tenv tenv
   <| v := eEmpty;
      c := (eSing cn ([], MAP (type_name_subst tenvT) ts, TypeExn (mk_id mn cn)));
      t := eEmpty |>))
...
type_specs mn tenvT (Stype_opq tvs tn :: specs)
  ...
  (extend_dec_tenv tenv <| v := eEmpty; c := eEmpty; t := tenvT' |>))`;
...
T
==>
type_prog extra_checks decls tenv [] empty_decls <| v := eEmpty; c := eEmpty; t := eEmpty |>)
