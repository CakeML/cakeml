...
Definition assigned_free_vars_def:
  ...
  (assigned_free_vars (Call (Ret NONE rp (SOME (Handle _ p))) e es) =
     assigned_free_vars rp ++ assigned_free_vars p) ∧
  (assigned_free_vars (Call (Ret NONE rp NONE) e es) = assigned_free_vars rp) ∧
  (assigned_free_vars (Call (Ret (SOME rt) rp (SOME (Handle _ p))) e es) =
     rt :: assigned_free_vars rp ++ assigned_free_vars p) ∧
  (assigned_free_vars (Call (Ret (SOME rt) rp NONE) e es) = rt :: assigned_free_vars rp) ∧
