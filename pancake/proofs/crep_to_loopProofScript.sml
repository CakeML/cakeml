...
Proof
  ...
  >- ( cheat (*
   ...
   ‘comp_syntax_ok l'' (nested_seq q)’ by (
     fs [Abbr ‘q’, nested_seq_def] >>
     rpt (match_mp_tac comp_syn_ok_seq2 >>
          fs [comp_syn_ok_basic_cases]) >> cheat) >>
   strip_tac >>
   qpat_x_assum ‘comp_syntax_ok _ (nested_seq p')’ assume_tac >>
   drule assigned_vars_nested_seq_split >>
...
Proof
  ...
  >- (
   ...
   FULL_CASE_TAC >> fs [] >> rveq >>
   rfs [] >> rveq >> fs [state_component_equality, lookup_inter, lookup_insert] >>
...
Theorem evaluate_add_clock_eq:
  !p t res st ck.
   evaluate (p,t) = (res,st) /\ res  = NONE ==>
   evaluate (p,t with clock := t.clock + ck) = (res,st with clock := st.clock + ck)
Proof
  recInduct evaluate_ind \\ rpt strip_tac
  \\ pop_assum mp_tac \\ pop_assum mp_tac \\ once_rewrite_tac [evaluate_def]
  ...
  \\ imp_res_tac fix_clock_IMP_LESS_EQ \\ fs [] >> cheat
QED
...
Theorem bar:
  !p q t st r ck ck'.
   ...
   evaluate (nested_seq q,st with clock := ck' + st.clock) = (NONE,r) ==>
      evaluate (nested_seq (p ++ q),t with clock := ck + ck' + t.clock) =  (NONE,r)
Proof
  Induct >> rw []
  >- (
   fs [nested_seq_def, evaluate_def] >>
   ‘st with clock := ck' + st.clock = t with clock := ck + ck' + t.clock’ by
     fs [state_component_equality] >>
   fs []) >>
  fs [nested_seq_def] >>
  fs [evaluate_def] >>
  ...
  ‘s1 = s1' with clock := s1'.clock + ck'’ by cheat >>
  fs [] >>
  last_x_assum (qspecl_then [‘q’, ‘s1'’, ‘st’, ‘r’, ‘0’ , ‘ck'’] mp_tac) >>
  impl_tac
  >- (
   fs [] >>
...
Theorem evaluate_add_clock_seq:
  !p s res t q ck r. evaluate (p, s) = (res,t) /\ res = NONE /\
    evaluate (q, t with clock := t.clock + ck) = (NONE,r) ==>
     evaluate (Seq p q, s with clock := s.clock + ck) = (NONE,r)
Proof
  cheat
QED

Theorem evaluate_res_none_clock_not_zero:
  !p s res t. evaluate (p, s) = (res,t) /\ res = NONE ==>
  t.clock <> 0
Proof
  cheat
QED
...
Proof
  ...
  >- (
   ...
   >- (
    fs [] >>
    conj_tac >- metis_tac [evaluate_res_none_clock_not_zero] >>
    (*conj_tac >- metis_tac [comp_syn_cut_sets_mem_domain] >>*)
    CCONTR_TAC >> fs [] >>
    res_tac >> fs []) >> fs []) >> fs []
QED
...
Proof
  ...
  >- (
   ...
   ‘∃ck st.
    ...
    state_rel s st ∧ mem_rel ctxt s.memory st.memory ∧
    globals_rel ctxt s.globals st.globals ∧
    code_rel ctxt s.code st.code ∧ locals_rel ctxt l' s.locals st.locals’ by (
     ...
     disch_then (qspecl_then [‘nested_seq fp’, ‘ck'’, ‘st'’] mp_tac) >>
     fs [] >>
     strip_tac >>
     ‘Seq (nested_seq p) (nested_seq fp) = nested_seq (p ++ fp)’ by cheat >>
     fs [] >>
     pop_assum kall_tac >>
     qexists_tac ‘ck + ck'’ >>
     qexists_tac ‘st'’ >>
     ...
     fs [wlab_wloc_def] >>
     disch_then (qspecl_then [‘itmp’, ‘le’, ‘Word c’] mp_tac) >>
     impl_tac
     >- (
      fs [] >>
...
(*
   ...
   drule evaluate_add_clock_seq >>
   fs [] >>
   disch_then (qspecl_then [‘nested_seq np’, ‘0’] mp_tac) >>
   fs [] >>




   assume_tac evaluate_add_clock_seq >>
   pop_assum
   (qspecl_then [‘nested_seq (p1 ++ p2)’, ‘t with clock := ck + (ck' + t.clock)’,
                 ‘NONE’, ‘’] mp_tac) >>
   ...
   qpat_x_assum ‘evaluate (nested_seq p2,_) = _’ mp_tac >>
   drule evaluate_none_nested_seq_append >>
