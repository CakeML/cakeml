...
Proof
  ...
  qsuff_tac ‘∃ck st.
    ...
    state_rel s st ∧ mem_rel ctxt s.memory st.memory ∧
    globals_rel ctxt s.globals st.globals ∧
    ncode_rel ctxt s.code st.code ∧ locals_rel ctxt l' s.locals st.locals’
  >- (
   strip_tac >>
   qexists_tac ‘ck’ >>
   fs [wlab_wloc_def]) >>
...
Theorem call_preserve_state_code_locals_rel:
  !ns lns args s st (ctxt: 'a context) nl fname argexps prog loc.
   ...
   let nctxt = ctxt_fc ctxt.funcs ns lns ctxt.ceids in
        ...
        ncode_rel nctxt s.code st.code ∧
...
Proof
  ...
  >- (
   ...
   res_tac >> rfs [])
  >- (
   qpat_x_assum ‘ncode_rel ctxt s.code t.code’ assume_tac >>
   drule ncode_rel_intro >>
   strip_tac >> fs [] >>
   res_tac >> rfs [])
  >- (
