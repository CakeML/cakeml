...
Theorem state_rel_intro:
  ∀clks s t.
    ...
    in
      t.ffi = build_ffi (:'a) t.be ffi io_events ∧
      time_seq ffi (dimword (:'a)) ∧
      clocks_rel s.clocks t.locals clks ftm ∧
      ∃tm. FLOOKUP t.locals «sysTime» = SOME (ValWord (n2w tm)) ∧
           tm ≤ ftm
Proof
  rw [state_rel_def]
...
(*
  time should not pass in the pancake worlsd, which means that
  ffi call should not happen
*)
...
Theorem output_step:
  !prog os s s' w (t:('a,time_input) panSem$state) ns.
    ...
    code_installed t.code prog ∧
    output_rel t.locals (dimword (:α)) s.waitTime ∧
    FLOOKUP t.locals «event»   = SOME (ValWord 0w) ∧
    FLOOKUP t.locals «isInput» = SOME (ValWord 1w) ∧
    FLOOKUP t.locals «waitSet» = SOME (ValWord 0w) ∧
    ...
    ?ck t'.
      ...
      code_installed t'.code prog ∧
      state_rel (clksOf prog) s' t' ∧
      t'.ffi.ffi_state = nexts_ffi 1 t.ffi.ffi_state ∧
      t'.ffi.oracle = t.ffi.oracle ∧
      FLOOKUP t'.locals «sysTime» =
      ...
      (* how do we know that «wakeupAt» does not overflow *)
Proof
  rw [] >>
  fs [] >>
  fs [step_cases] >>
  ‘∃tt. s.waitTime = SOME tt’ by cheat >>
  fs [] >>
  fs [task_controller_def] >>
  fs [panLangTheory.nested_seq_def] >>
  qmatch_goalsub_abbrev_tac ‘evaluate (Seq _ q, _)’ >>
  ...
  strip_tac >>
  gs [eval_upd_clock_eq] >>
  ‘state_rel (clksOf prog) s (t with clock := t.clock + 1)’ by
    gs [state_rel_def] >>
  pairarg_tac >> gs [] >>
  pairarg_tac >> gs [] >>
  gs [dec_clock_def] >>
  ...
  ‘nt.memory ffiBufferAddr = Word (n2w (FST(t.ffi.ffi_state 0)))’ by (
    ...
    last_x_assum
    (qspecl_then
     [‘t with clock := 1 + t.clock’,
      ‘ft’] mp_tac)>>
    impl_tac
    >- (gs [nexts_ffi_def, ETA_AX] >> metis_tac []) >>
    strip_tac >>
    gs [Abbr ‘ft’, nexts_ffi_def, ETA_AX]) >>
  drule evaluate_assign_load >>
  gs [] >>
  disch_then (qspecl_then
              [‘ffiBufferAddr’, ‘tm’,
  ...
  fs [Once evaluate_def] >>
  ‘FLOOKUP
   (nnnt with locals := nnnt.locals |+ («isInput» ,ValWord 1w)).locals «isInput» =
   SOME (ValWord 1w)’ by fs [FLOOKUP_UPDATE] >>
  drule step_delay_eval_wait_zero >>
  ‘ FLOOKUP
    (nnnt with locals := nnnt.locals |+ («isInput» ,ValWord 1w)).
    locals «waitSet» = SOME (ValWord 0w)’ by (
    unabbrev_all_tac >>
    gs [FLOOKUP_UPDATE]) >>
  gs [] >>
  fs [Abbr ‘nnnt’, Abbr ‘nnt’, FLOOKUP_UPDATE] >>
  impl_tac
  >- gs [Abbr ‘nt’,  FLOOKUP_UPDATE, output_rel_def] >>
  strip_tac >> gs [] >>
  pop_assum kall_tac >>
  ...
  pop_assum kall_tac >>
  pop_assum kall_tac >>
  qpat_x_assum ‘_ = (NONE, _)’ kall_tac >>
  qmatch_goalsub_abbrev_tac ‘evaluate (Seq _ q, _)’ >>
  ...
  qmatch_asmsub_abbrev_tac ‘OPT_MMAP (eval nt) [_ ; _]’ >>
  ‘FLOOKUP nt.locals «sysTime» = SOME (ValWord (n2w (FST (t.ffi.ffi_state 0))))’ by
    fs [Abbr ‘nt’, FLOOKUP_UPDATE] >>
  ...
  fs [OPT_MMAP_def] >>
  fs [Once eval_def] >>
  qmatch_asmsub_abbrev_tac ‘(eval nt)’ >>
  ‘(λa. eval nt a) =
   eval nt’ by metis_tac [ETA_AX] >>
  fs [] >>
  fs [timeLangTheory.nClks_def] >>
  ‘LENGTH (clksOf prog) = LENGTH ns’ by gs [] >>
  gs [] >>
  (* event eval *)
  gs [Abbr ‘nt’, eval_def, FLOOKUP_UPDATE,
      output_rel_def] >>
  gs [lookup_code_def] >>
  fs [timeLangTheory.nClks_def] >>
  ‘LENGTH (clksOf prog) = LENGTH ns’ by gs [] >>
  ...
  >- (
    ...
    gs [FLOOKUP_UPDATE] >>
    cheat) >>
  gs [task_ret_defined_def, FLOOKUP_UPDATE] >>
  fs [nffi_state_def,  nexts_ffi_def] >>
  conj_tac >- cheat >>
  conj_tac >- cheat >>
  TOP_CASE_TAC >> gs []
  >- (
...
Theorem output_step:
  !prog os s s' w (t:('a,time_input) panSem$state) st ns.
    step prog (LAction (Output os)) s s' ∧
    state_rel (clksOf prog) s t ∧
    code_installed t.code prog ∧
    output_rel t.locals (dimword (:α)) s.waitTime t.ffi.ffi_state ∧
    ...
    (* for the time being  *)
    FLOOKUP t.locals «sysTime» = SOME (ValWord (n2w st)) ∧
    FLOOKUP t.locals «wakeUpAt» = SOME (ValWord (n2w st)) ∧
    FST (t.ffi.ffi_state 0) = st (* for state_rel *) ∧
    FLOOKUP t.locals «clks» =
    SOME (Struct (MAP ((λw. ValWord w) ∘ n2w) ns)) ∧
    EVERY (λn. n ≤ st) ns ∧
    LENGTH ns = nClks prog ∧
    labProps$good_dimindex (:'a) ∧
    (∀tms.
       ALOOKUP prog s.location = SOME tms ⇒
       conds_eval_lt_dimword (:α) (resetOutput s) tms ∧
    ...
    ?ck t'.
      ...
      code_installed t'.code prog ∧
      state_rel (clksOf prog) s' t'
      (* some props of state t' *)
Proof
  rw [] >>
  fs [step_cases] >>
  qmatch_asmsub_abbrev_tac
  ‘FLOOKUP _ «sysTime» = SOME (ValWord (n2w st))’ >>
  fs [task_controller_def] >>
  fs [panLangTheory.nested_seq_def] >>
  qmatch_goalsub_abbrev_tac ‘evaluate (Seq _ q, _)’ >>
  fs [evaluate_def] >>
  ...
  drule step_delay_eval_wait_zero >>
  disch_then (qspec_then ‘n2w st’ mp_tac) >>
  fs [] >>
  strip_tac >>
  fs [] >> rveq >> gs [] >>
  fs [Abbr ‘q’] >>
  ...
  fs [] >>
  fs [Once evaluate_def, eval_upd_clock_eq] >>
  gs [Once eval_def, eval_upd_clock_eq] >>
  imp_res_tac eval_normalisedClks >>
  gs [] >>
  fs [OPT_MMAP_def] >>
  fs [Once eval_def] >>
  ‘(λa. eval (t with clock := t.clock + 1) a) =
   eval (t with clock := t.clock + 1)’ by metis_tac [ETA_AX] >>
  fs [] >>
  pop_assum kall_tac >>
  assume_tac
  (INST_TYPE [``:'a``|->``:'a``,
              ``:'b``|->``:time_input``] opt_mmap_eval_upd_clock_eq) >>
  first_x_assum
  (qspecl_then
   [‘normalisedClks «sysTime» «clks» (nClks prog)’, ‘t’, ‘1’] assume_tac) >>
  fs [] >>
  pop_assum kall_tac >>
  pop_assum kall_tac >>
  (* event eval *)
  ...
  drule  (INST_TYPE [``:'a``|->``:'a``,
                     ``:'b``|->``:time_input``] pick_term_thm) >>
  fs [] >>
  disch_then (qspecl_then [‘nt’, ‘clksOf prog’,
                           ‘nclks’] mp_tac) >>
  impl_tac
  >- (
    gs [Abbr ‘nt’] >>
    ...
    ‘EL n (ZIP (xs,ns)) = (EL n xs, EL n ns)’ by (
      ...
      fs []) >>
    fs [Abbr ‘xs’] >>
    ‘EL n (REPLICATE (LENGTH (clksOf prog)) st) = st’ by (
      match_mp_tac EL_REPLICATE >>
      fs []) >>
    fs [] >>
    ...
    gs [clocks_rel_def, clkvals_rel_def] >>
    rveq >> gs [] >>
    fs [EVERY_MEM] >>
    fs [MAP_EQ_EVERY2] >>
    fs [LIST_REL_EL_EQN] >>
    qpat_x_assum ‘∀n. n < LENGTH ns' ⇒ _’ mp_tac >>
    disch_then (qspec_then ‘n’ mp_tac) >>
    fs [] >>
    strip_tac >>
    ‘EL n (ZIP (clksOf prog,ns')) =
     (EL n (clksOf prog), EL n ns')’ by (
      ...
      fs []) >>
    fs [] >>
    first_x_assum (qspec_then ‘EL n (clksOf prog)’ mp_tac) >>
    impl_tac >- metis_tac [MEM_EL] >>
    strip_tac >> fs [] >>
    ‘THE (FLOOKUP s.clocks (EL n (clksOf prog))) =
     tm - EL n ns'’ by fs [] >>
    fs [] >>
    ‘EL n ns = EL n ns'’ by (
      first_x_assum (qspec_then ‘n’ mp_tac) >>
      fs [] >>
      ‘EL n ns MOD dimword (:α) = EL n ns’ (
        match_mp_tac LESS_MOD >>
        gs [time_seq_def] >>
    ...
    fs [])
      ...
      ‘tm = st’ by cheat >>
      rveq >> gs [] >>
      qpat_x_assum ‘_ = MAP (λx. st) ns’ assume_tac >>
  ...
  strip_tac >>
  fs [] >>
  ‘is_valid_value t.locals «taskRet»
   ((retVal (resetOutput s) (clksOf prog) tclks wt dest):'a v)’ by cheat >>
  fs [] >>
  gs [set_var_def] >> rveq >> gs []  >>
  fs [Abbr ‘q’] >>
  unabbrev_all_tac >>
  fs []
...
(* we have pickTerm_ind *)
(* first proceed in the same direction *)
