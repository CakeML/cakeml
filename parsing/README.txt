...
The main function for user use might be one that targets the type `Ast$ast_dec`, or a list thereof.

In any case, the basic approach involves three(!) phases:

1. Use a PEG-based parser to parse the token list.
The output type for this phase is a parse-tree that conforms to the context-free grammar specified in `mmlGrammarTheory`.
...
For example, `ptree_Expr` takes a parse-tree meant to correspond to an expression and generates an `ast_exp` value (strictly, an option type).
