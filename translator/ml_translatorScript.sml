...
`!x k env k' res extra.
  ...
  ⇒
  evaluate (empty_state (k + extra)) env [x] =
    (empty_state (k' + extra), Rval [res])`,
 rw [empty_state_def]
 >> drule (hd (CONJUNCTS evaluatePropsTheory.evaluate_add_to_clock))
 >> rw []);
...
val Eval_Arrow = store_thm("Eval_Arrow",
  ...
  >> qexists_tac `k' + k + k'' + 1`
  >> rw []
  >> `evaluate (empty_state (k'+(k+k''+1))) env [x2] = (empty_state (0 + (k+k''+1)),Rval [res'])`
    by (irule clock_add_lemma >> simp [])
  >> fs []
  >> `evaluate (empty_state (k+(k''+1))) env [x1] = (empty_state (0+(k''+1)),Rval [res])`
    by (irule clock_add_lemma >> simp [])
  >> fs []
  >> rw []
  >> fs [evaluateTheory.dec_clock_def, empty_state_def]);

val Eval_Fun = store_thm("Eval_Fun",
  ``(!v x. a x v ==> Eval (write name v env) body (b ((f:'a->'b) x))) ==>
    Eval env (Fun name body) ((a --> b) f)``,
  SIMP_TAC std_ss [Eval_def,Arrow_def, evaluate_def]
  >> SIMP_TAC (srw_ss()) []
...
val Eval_Fun_Eq = store_thm("Eval_Fun_Eq",
  ...
  SIMP_TAC std_ss [Eval_def,Arrow_def, evaluate_def] \\ REPEAT STRIP_TAC
  \\ SIMP_TAC (srw_ss()) []
  \\ FULL_SIMP_TAC (srw_ss()) [AppReturns_def,Eval_def,do_opapp_def,
...
val Eval_Let = store_thm("Eval_Let",
  ...
  rw [Eval_def,Arrow_def, evaluate_def]
  >> first_x_assum drule
  >> rw []
  >> `evaluate (empty_state (k+k')) env [exp] = (empty_state (0+k'),Rval [res'])`
    by (irule clock_add_lemma >> simp [])
  >> qexists_tac `res''`
  >> qexists_tac `(k+k')`
  >> rw []
  >> fs [opt_bind_def, write_def]);
...
val Eval_Var_Short = store_thm("Eval_Var_Short",
  ...
  rw [Eval_def,evaluate_def, lookup_var_id_def]
  >> metis_tac []);
...
val Eval_Var_SWAP_ENV = store_thm("Eval_Var_SWAP_ENV",
  ...
  \\ SIMP_TAC (srw_ss()) [evaluate_def,Eval_def, lookup_var_id_def]);
...
val LOOKUP_VAR_THM = store_thm("LOOKUP_VAR_THM",
  ...
  FULL_SIMP_TAC std_ss [FORALL_PROD,lookup_var_def]
  \\ SIMP_TAC (srw_ss()) [evaluate_def,Eval_def,LOOKUP_VAR_def,
...
val Eval_Val_INT = store_thm("Eval_Val_INT",
  ...
  SIMP_TAC (srw_ss()) [evaluate_def,NUM_def,INT_def,Eval_def]
  >> metis_tac []);
...
val Eval_Val_UNIT = store_thm("Eval_Val_UNIT",
  ...
  SIMP_TAC (srw_ss()) [evaluate_def,UNIT_TYPE_def,Eval_def,
     build_conv_def,do_con_check_def]
  >> metis_tac []);

val Eval_Val_BOOL_T = store_thm("Eval_Val_BOOL_T",
  ``Eval env (App (Opb Leq) [Lit (IntLit 0); Lit (IntLit 0)]) (BOOL T)``,
  SIMP_TAC (srw_ss()) [evaluate_def,BOOL_def,Eval_def,
...
val Eval_Val_CHAR = store_thm("Eval_Val_CHAR",
  ...
  SIMP_TAC (srw_ss()) [CHAR_def,Eval_def, evaluate_def]
  >> metis_tac []);

val Eval_Val_STRING = store_thm("Eval_Val_STRING",
  ``!s. Eval env (Lit (StrLit s)) (STRING_TYPE (strlit s))``,
  SIMP_TAC (srw_ss()) [STRING_TYPE_def,Eval_def,evaluate_def]
  >> metis_tac []);
...
val Eval_Val_WORD = store_thm("Eval_Val_WORD",
  ``!w:'a word.
    ...
    Eval env (Lit (if dimindex(:'a) ≤ 8
             ...
             (WORD w)``,
  SIMP_TAC (srw_ss()) [WORD_def,Eval_def,evaluate_def]
  >> metis_tac []);
...
fun has_state_type v t = #1(dest_type (type_of v)) = "state"
val state_record_qp = record_qp false has_state_type
val empty_state_tac =
...
val Eval_Equality = store_thm("Eval_Equality",
  ...
  >> simp [evaluate_def]
  >> `evaluate (empty_state (k'+ k)) env [x2] = (empty_state (0+k),Rval [res'])`
    by (irule clock_add_lemma >> simp [])
  >> qexists_tac `k'+k`
  >> simp []
  \\ empty_state_tac
...
val Eval_Or = store_thm("Eval_Or",
  ...
  \\ REPEAT STRIP_TAC \\ FULL_SIMP_TAC std_ss []
  \\ REWRITE_TAC [evaluate_def] \\ SIMP_TAC (srw_ss()) []
  \\ Cases_on `b1` \\ fs []
  >- (
    qexists_tac `k`
    >> simp []
    >> fs [EVAL ``do_log Or (Boolv T) x``] \\ EVAL_TAC)
  >> `evaluate (empty_state (k+k')) env [x1] = (empty_state (0+k'),Rval [Boolv F])`
    by (irule clock_add_lemma >> simp [])
  >> qexists_tac `k+k'`
  >> simp []
...
val Eval_And = store_thm("Eval_And",
  ...
  \\ Cases_on `b1` \\ fs []
  >- (
    `evaluate (empty_state (k+k')) env [x1] = (empty_state (0+k'),Rval [Boolv T])`
      by (irule clock_add_lemma >> simp [])
    >> qexists_tac `k+k'`
    >> simp []
...
val Eval_If = store_thm("Eval_If",
  ...
  THEN1 (Q.EXISTS_TAC `res` \\ ASM_SIMP_TAC std_ss []
    >> `evaluate (empty_state (k+k')) env [x1] = (empty_state (0+k'),Rval [Boolv T])`
      by (irule clock_add_lemma >> simp [])
    >> qexists_tac `k+k'`
    >> ASM_SIMP_TAC (srw_ss()) [do_if_def])
  THEN1 (Q.EXISTS_TAC `res` \\ ASM_SIMP_TAC std_ss []
    >> `evaluate (empty_state (k+k')) env [x1] = (empty_state (0+k'),Rval [Boolv F])`
      by (irule clock_add_lemma >> simp [])
    >> qexists_tac `k+k'`
    >> fs [do_if_def,EVAL ``Boolv F = Boolv T``]));

val Eval_Bool_Not = store_thm("Eval_Bool_Not",
  ``Eval env x1 (BOOL b1) ==>
    Eval env (App Equality
      [x1; App (Opb Lt) [Lit (IntLit 0); Lit (IntLit 0)]]) (BOOL (~b1))``,
  rw [Eval_def,NUM_def,BOOL_def, evaluate_def, do_app_def]
  >> qexists_tac `k`
  \\ fs [empty_state_def]
  >> Cases_on `b1`
  >> simp [do_eq_def, opb_lookup_def, Boolv_def, ctor_same_type_def]);
...
val Eval_Var_SIMP = store_thm("Eval_Var_SIMP",
  ...
  ASM_SIMP_TAC (srw_ss()) [LOOKUP_VAR_def,lookup_var_id_def,write_def,
       lookup_var_def,Eval_def,evaluate_def,lookup_var_id_def]
  >> rw []
  >> metis_tac []);
...
val FUN_FORALL_PUSH1 = prove(
  ...
  >> metis_tac [clock_different_lemma]) |> GEN_ALL;
...
val Eval_Recclosure_ALT = store_thm("Eval_Recclosure_ALT",
  ...
  \\ POP_ASSUM MP_TAC \\ POP_ASSUM MP_TAC \\ REWRITE_TAC [evaluate_def]
...
val Eval_Eq_Fun = store_thm("Eval_Eq_Fun",
  ...
  SIMP_TAC std_ss [Eval_Var_SIMP,Eval_def, evaluate_def]
...
val Eval_Opn = prove(
  ...
  >> rw [evaluate_def]
  >> `evaluate (empty_state (k' + k)) env [x2] = (empty_state (0+k),Rval [Litv (IntLit n2)])`
    by (irule clock_add_lemma >> simp [])
  >> qexists_tac `k + k'`
  >> fs []
  >> rw [do_app_def, state_component_equality]);
...
val Eval_Opb = prove(
  ...
  SIMP_TAC std_ss [Eval_def,BOOL_def, INT_def] \\ SIMP_TAC std_ss [PRECONDITION_def]
  >> rw [evaluate_def]
  >> `evaluate (empty_state (k' + k)) env [x2] = (empty_state (0+k),Rval [Litv (IntLit n2)])`
    by (irule clock_add_lemma >> simp [])
  >> qexists_tac `k + k'`
...
val Eval_int_negate = store_thm("Eval_int_negate",
  ...
  >> qexists_tac `Litv (IntLit (-i))`
  >> qexists_tac `k`
  >> simp [do_app_def]
  >> fs [INT_def, opn_lookup_def]
...
val Eval_word_and = store_thm("Eval_word_and",
  ``Eval env x1 (WORD (w1:'a word)) /\
    ...
    Eval env (App (Opw (if dimindex (:'a) <= 8 then W8 else W64) Andw) [x1;x2])
      (WORD (word_and w1 w2))``,
  rw[Eval_def,WORD_def, evaluate_def]
  >> assume_tac (Q.SPECL [`x2`, `k'`] clock_add_lemma)
  >> pop_assum drule
  >> rw []
  >> qexists_tac `k + k'`
  >> simp [do_app_def, opw8_lookup_def,opw64_lookup_def, state_component_equality]
...
val Eval_w2n = store_thm("Eval_w2n",
  ...
  \\ TRY (* takes care of = 8 and = 64 cases *)
   (rw[evaluate_def,PULL_EXISTS]
    ...
    >> qexists_tac `k`
    >> simp [do_app_def, state_component_equality, NUM_def, INT_def, w2w_def]
    \\ assume_tac w2n_lt \\ rfs [dimword_def]
    >> NO_TAC)
  >> rw[evaluate_def,PULL_EXISTS]
  >> CONV_TAC SWAP_EXISTS_CONV
  >> qexists_tac `k`
  >> simp [do_app_def, state_component_equality, NUM_def, INT_def]
...
val Eval_n2w = store_thm("Eval_n2w",
  ...
  \\ TRY (* takes care of = 8 and = 64 cases *)
   (rw[evaluate_def,PULL_EXISTS]
    >> qexists_tac `k`
    >> fs [do_app_def, state_component_equality, NUM_def, INT_def, w2w_def, integer_wordTheory.i2w_def]
...
val Eval_word_lsl = store_thm("Eval_word_lsl",
  ...
  \\ fs [fcpTheory.CART_EQ,word_lsl_def,fcpTheory.FCP_BETA,w2w] 
  \\ rw [state_component_equality]
...
val Eval_Ord = store_thm("Eval_Ord",
  ...
  >> qexists_tac `k`
  >> simp [do_app_def]
  >> fs [CHAR_def, state_component_equality, NUM_def, INT_def]);
...
val tac =
  ...
  >> simp [do_app_def]
  >> empty_state_tac >>
  rw[semanticPrimitivesPropsTheory.do_app_cases,PULL_EXISTS] >> fs[CHAR_def] >>
...
val Eval_sub = store_thm("Eval_sub",
 ``!env x1 x2 a n v.
  ...
  fs [INT_ABS_NUM, state_component_equality] >>
  fs [LIST_REL_EL_EQN]);

val LIST_TYPE_v_to_list = store_thm("LIST_TYPE_v_to_list",
  ``∀l a v. LIST_TYPE a l v ⇒ OPTREL (LIST_REL a) (SOME l) (v_to_list v)``,
  Induct
  >> simp[LIST_TYPE_def,v_to_list_def,PULL_EXISTS, OPTREL_SOME]
  >> rw []
  >> first_x_assum drule
  >> rw [OPTREL_SOME]
  >> rw []);
...
val Eval_vector = store_thm("Eval_vector",
 ...
 >> drule LIST_TYPE_v_to_list
 >> rw [OPTREL_SOME, VECTOR_TYPE_def]
...
val evaluate_empty_store_lemma = prove(
 ``(!s env es r1.
      evaluate ^s env es = r1 ==> has_emp r1 ==> (s.refs = empty_store)) /\
   (!s env v pes errv r1.
      evaluate_match ^s env v pes errv = r1 ==> has_emp r1 ==> (s.refs = empty_store))``,
 ...
 >> fs []
 >> rw []
 >> rfs [evaluateTheory.dec_clock_def]
 >> fs [semanticPrimitivesPropsTheory.do_app_cases]
 >> rw []
 >> fs [store_alloc_def, store_assign_def, empty_store_def]);

val _ = temp_overload_on("has_emp_no_fail",
  ``\x. ((FST x).refs = empty_store) /\
        ~(SND x = (Rerr (Rabort Rtype_error)):('b,'c) result)``)

(* TODO: evaluate_ind now comes from Define rather than Hol_reln, so
 * derive_strong_induction doesn't work
...
val do_app_empty_store = prove(
  ...
  Cases_on`s3`>>
  SIMP_TAC std_ss [semanticPrimitivesPropsTheory.do_app_cases] \\ SRW_TAC [] []
...
(* TODO: This isn't true as stated because the clocks in the state do decrement.
 * Needs to be repaired in a way that will keep the automation working.
...
val IMP_PreImp = store_thm("IMP_PreImp",
  ...
  (* TODO: Not sure what to prove here, we don't have evaluate_list anymore.
...
(* TODO: This is no longer a good definition since most expressions will need a
 * certain clock (or bigger) to evaluate
val always_evaluates_def = Define `
  always_evaluates env exp =
    !(s1:unit state). ?s2 res. evaluate s1 env exp = (s2,Rval res)`;
...
val evaluate_Fun = store_thm("evaluate_Fun",
  ``evaluate s env [Fun n exp] = (s,Rval r) <=> r = [Closure env n exp]``,
  fs [evaluate_def]
  >> metis_tac []);

val evaluate_Var = store_thm("evaluate_Var",
  ``evaluate s env [Var (Short n)] = (s,Rval [r]) <=>
...
val Eval_evaluate_IMP = store_thm("Eval_evaluate_IMP",
  ``Eval env exp P /\
    evaluate (empty_state k) env [exp] = (empty_state k', Rval [v]) ==>
    P v``,
  fs [Eval_def] \\ rw []
  \\ imp_res_tac clock_different_lemma2
  >> rw []);
...
val evaluate_Mat = save_thm("evaluate_Mat",
  ``evaluate x env [Mat e pats] = (xx,Rval res)``
  |> (REWRITE_CONV [evaluate_def] THENC SIMP_CONV (srw_ss()) []))

val evaluate_match_Conv = save_thm("evaluate_match_Conv",
  ``evaluate_match st env args ((Pcon xx pats,exp2)::pats2) errv = (yyy,Rval y)``
  |> (REWRITE_CONV [evaluate_def] THENC
      SIMP_CONV (srw_ss()) [pmatch_def]))

val evaluate_match_rw = store_thm("evaluate_match_rw",
  ``evaluate_match st env args
    ...
    | No_match =>
        evaluate_match st env args pats2 errv = (yyy,Rval y)
    | Match env7 =>
        evaluate st (env with v := env7) [exp2] = (yyy,Rval y)
    | _ => F``,
