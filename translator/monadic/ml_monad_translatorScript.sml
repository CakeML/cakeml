...
val LIST_REL_EQ_LIST_CONJ_MAP = store_thm("LIST_REL_EQ_LIST_CONJ_MAP",
  ``!xs ys.
      LENGTH xs = LENGTH ys ⇒
...
val evaluate_Opdref_REF = Q.prove(
  ...
  !junk. evaluate (s with refs := s.refs ++ junk) env
     [App Opderef [Var (Short vname)]] = (s with refs := s.refs ++ junk, Rval [v])`,
  rw[evaluate_def]
  \\ rw[do_app_def]
