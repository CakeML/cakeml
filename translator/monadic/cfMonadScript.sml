...
Theorem EvalM_from_app2:
   !(eff_v:v) ARG1_TYPE ARG2_TYPE EXC_TYPE.
   (!x y s. ?r t. f x y s = (Success r, t)) /\
   (!x y xv yv s ret new_s.
     ARG1_TYPE x xv ==>
     ARG2_TYPE y yv ==>
     (f x y s = (Success ret, new_s)) ==>
     app (p: 'ffi ffi_proj) fun_v [xv;yv] (H s)
       (POSTv rv. &RET_TYPE ret rv * (H new_s)))
   ==>
   Eval env fun_exp1 (ARG1_TYPE x) /\
   Eval env fun_exp2 (ARG2_TYPE y) /\
   (nsLookup env.v fun_name = SOME fun_v) ==>
   EvalM F env st (App Opapp [Var fun_name; fun_exp1; fun_exp2])
    (MONAD RET_TYPE EXC_TYPE (f x y))
    (H, p)
Proof
  rw [EvalM_def] \\ fs [Eval_def]
  \\ first_x_assum (qspec_then `s.refs` strip_assume_tac)
  \\ fs [cfAppTheory.app_def, cfAppTheory.app_basic_def, evaluate_to_heap_def]
  \\ simp [MONAD_def]
  \\ first_x_assum (qspec_then ‘s.refs’ assume_tac) \\ fs[]
  \\ first_x_assum (qspecl_then [`x`,‘y’,`st`] strip_assume_tac) \\ fs []
  \\ first_assum drule
  \\ disch_then (mp_tac o CONV_RULE (RESORT_FORALL_CONV rev))
  \\ fs [REFS_PRED_def, set_sepTheory.STAR_def]
  \\ qabbrev_tac `rssy = s.refs++refs'`
  \\ sg `?hk. SPLIT (st2heap p (s with refs := rssy)) (u, hk)`
  >- fs [Abbr`rssy`, set_sepTheory.SPLIT_EQ,
         cfAppTheory.st2heap_with_refs_append,
         cfStoreTheory.st2heap_def, SUBSET_DEF]
  \\ qabbrev_tac `rssx = s.refs++refs''`
  \\ sg `?hk. SPLIT (st2heap p (s with refs := rssx)) (u, hk)`
  >- fs [Abbr`rssx`, set_sepTheory.SPLIT_EQ,
         cfAppTheory.st2heap_with_refs_append,
         cfStoreTheory.st2heap_def, SUBSET_DEF]
  \\ fs [Abbr`rssx`,Abbr‘rssy’]
  \\ rpt (disch_then drule) \\ rw []
  \\ fs [cfHeapsBaseTheory.POSTv_def, cfHeapsBaseTheory.POST_def]
  \\ FULL_CASE_TAC \\ fs [set_sepTheory.cond_def]
  \\ rw [evaluate_def, PULL_EXISTS]
